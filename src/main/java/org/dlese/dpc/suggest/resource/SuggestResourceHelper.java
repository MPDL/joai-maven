...
public class SuggestResourceHelper extends SuggestHelper {
	 ...
	 * directory that contains a default record (used for making new records), and a
	 * records directory (where suggested urls are stored). This stuff is all hard
	 * coded for now, but properties can be used to supply defaults (see
	 ...
	 * recordTemplate. We don't call readRecord (file) because we don't want the id
	 * to be set for the new record
	 *
	 ...
	 */
	public String putRecordToDCS(SuggestionRecord rec, ValidatorResults validatorResults) throws Exception {
		...
		try {
			newRecId = getRepositoryServiceClient().doPutRecord(adnDoc.asXML(), "adn", getDestCollection(), dcsStatus,
					dcsStatusNote);
