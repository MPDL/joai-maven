 ...
 *
 *         $Id $
 * @version $Id: SuggestCommentAction.java,v 1.9 2009/03/20 23:34:00 jweather
 *          Exp $
 */
public final class SuggestCommentAction extends SuggestAction {
	...
	protected ActionForward initializeSuggestor(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (id == null || id.trim().length() == 0) {
			errors.add("error", new ActionError("comment.id.required"));
		...
		try {
			...
			errors.add("error", new ActionError("comment.resource.not.found", id));
			saveErrors(request, errors);
			return mapping.findForward("home");
		}
	...
	protected ActionForward handleCancelCommand(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		errors.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("comment.cancel"));
		saveMessages(request, errors);
		return mapping.findForward("home");
	}
	...
	protected ActionForward handleSubmitCommand(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		scf.clear();
		actionMessages.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("comment.confirmation"));
	...
	protected ActionErrors validateSuggestForm(ActionForm form, ActionMapping mapping, HttpServletRequest request) {
		...
		if ((email == null) || (email.trim().equals(""))) {
			...
			try {
				...
				errors.add("email", new ActionError("generic.error", e.getMessage()));
			}
		}
	 ...
	 */
	protected CommentRecord createRecord(ActionForm form) throws Exception {
