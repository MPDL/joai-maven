...
public final class SuggestXMLAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest req,
		...
		try {
			...
			for (int i = 0; i < atts.length; i++) {
				...
				if (ind > -1) {
					rootElement.addAttribute(atts[i].substring(0, ind), atts[i].substring(ind + 1, atts[i].length()));
				}
			}
			...
			while (elements.hasMoreElements()) {
				String newElementName = (String) elements.nextElement();
				if (!newElementName.startsWith("rootElement") && !newElementName.startsWith("META_")
	 ...
	 * @exception Exception
	 */
	private void sendEmailMessages(HttpServletRequest req, String subjectPrefix, String newRecordId,
			SuggestXMLForm suggestForm) throws Exception {
		...
		if (notify_email != null) {
			...
			new SendEmail(context.getInitParameter("mail_type"), context.getInitParameter("mail_server")).doSend(
					notify_email, notify_email_from, subjectPrefix + regarding,
		...
		if (receipt_email != null) {
			new SendEmail(context.getInitParameter("mail_type"), context.getInitParameter("mail_server")).doSend(
					receipt_email, context.getInitParameter("receipt_email_from"),
					context.getInitParameter("receipt_subject"), context.getInitParameter("receipt_body_text"));
		}
	}
