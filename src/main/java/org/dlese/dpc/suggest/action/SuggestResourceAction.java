...
import org.dlese.dpc.suggest.resource.ResourceEmailer;
 ...
 *
 *         $Id $
 * @version $Id: SuggestResourceAction.java,v 1.7 2009/03/20 23:34:00 jweather
 *          Exp $
 */
public final class SuggestResourceAction extends SuggestAction {
	...
	protected SuggestResourceHelper getSuggestHelper() throws ServletException {
		if (suggestHelper == null) {
			try {
				suggestHelper = (SuggestResourceHelper) servlet.getServletContext()
						.getAttribute("SuggestResourceHelper");
				if (suggestHelper == null)
					throw new Exception();
			} catch (Throwable t) {
	 ...
	 *                NOT YET DOCUMENTED
	 */
	protected ActionForward initializeSuggestor(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	 ...
	 *                NOT YET DOCUMENTED
	 */
	protected ActionForward handleEditCommand(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	 ...
	 * @return NOT YET DOCUMENTED
	 */
	protected ActionForward handleCancelCommand(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	 ...
	 * @return NOT YET DOCUMENTED
	 */
	protected ActionForward handleDoneCommand(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {

		SuggestResourceForm srf = (SuggestResourceForm) form;
		ActionErrors errors = new ActionErrors();

		// make sure there is a value for URL. If the user has previously "cancelled"
		// and then
		// used the back buttons to return to an old form that *looks* like it holds
		// data, but really
	 ...
	 * @return NOT YET DOCUMENTED
	 */
	protected ActionForward handleSubmitCommand(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {
		...
		srf.clear();
		actionMessages.add(ActionMessages.GLOBAL_MESSAGE, new ActionMessage("submit.confirmation"));
	 ...
	 */
	protected ActionErrors validateSuggestForm(ActionForm form, ActionMapping mapping, HttpServletRequest request) {
		...
		if ((emailPrimary == null) || (emailPrimary.trim().equals(""))) {
			...
			try {
				...
				errors.add("emailPrimary", new ActionError("generic.error", e.getMessage()));
			}
		}
	...
	/**
	 * Validate a URL for proper syntax and then process it with the UrlValidator to
	 * enforce rules for accepting a new suggestion. Validation results are stored
	 * in the ActionForm, errors are returned to caller as ActionErrors.
	 * <p>
	 *
	 * Verify that a URL is unique within the destination collection using the
	 * configured searchService stored in the ServiceHelper. If either of these
	 * parameters are not specified, then urls are validated by default (no errors
	 * are returned).
	 *
	 * @param mapping
	 *            Description of the Parameter
	 ...
	 * @param form
	 *            NOT YET DOCUMENTED
	 * @return errors that tell why a url is not valid, or no errors if the url is
	 *         valid.
	 */
	protected ActionErrors validateUrl(ActionForm form, ActionMapping mapping, HttpServletRequest request) {
		...
		// look for a dup or similar url that is in the reference collection (if one is
		// defined)
	 ...
	 */
	protected ResourceRecord createRecord(ActionForm form) throws Exception {
