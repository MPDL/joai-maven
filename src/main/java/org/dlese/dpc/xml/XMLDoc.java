...
public class XMLDoc {
	...
	public static void main(String[] args) {
		...
		try {
			...
			for (ii = 0; ii < iters; ii++) {
				XMLDoc doc = new XMLDoc(xmlSource, isValidating, isNamespace, isExpand);
	...
	static boolean getBoolean(String stg) throws XMLException {
		boolean bres = false;
		if (stg.toLowerCase().equals("y"))
			bres = true;
		else if (stg.toLowerCase().equals("n"))
			bres = false;
		else
			mkerror("invalid boolean parm: \"" + stg + "\"");
		return bres;
	}
	...
	static int getInt(String stg) throws XMLException {
		int ires = 0;
		try {
	 ...
	 *            </ul>
	 *            If it represents a file, it must have 3 slashes and the path must
	 *            be fully qualified.
	 * @param validating
	 *            Is the parser to be validating?
	 * @param namespaceAware
	 *            Is the parser to be namespace aware?
	 * @param expandEntities
	 *            DESCRIPTION
	 * @exception XMLException
	 *                DESCRIPTION
	 */
	public XMLDoc(String systemid, // URI to the XML document.
	...
	/**
	 * A non-initialized XMLDoc. To read the actual XML, use method
	 * {@link #useXmlString(String, boolean,boolean,boolean)}.
	 */
	public XMLDoc() {
	}

	/**
	 * Reads and parses the XML string, which is then the source of the XML used in
	 * this XMLDoc.
	 */
	public void useXmlString(String xmlString, // An XML string
			boolean validating, // does parser validate
			boolean namespaceAware, // is parser namespace aware
			boolean expandEntities) // expand Entity refs
			throws XMLException {
	 ...
	 */
	private final void doParseDoc(InputSource xmlSource, boolean validating, // does parser validate
			boolean namespaceAware, // is parser namespace aware
			boolean expandEntities) // expand Entity refs
			throws XMLException {
		this.doc = null;
		...
		SimpleErrorHandler simpleErrorHandler = new SimpleErrorHandler(errors, warnings);
		bldr.setErrorHandler(simpleErrorHandler);

		try {
			doc = bldr.parse(xmlSource);
		} catch (IOException ioe) {
			mkerror("Could not read xml doc file \"" + systemid + "\"  exc: " + ioe.getMessage());
		} catch (SAXException sxe) {
			mkerror("Could not parse xml doc file \"" + systemid + "\"  exc: " + sxe.getMessage());
		}
	...
	/**
	 * Retrieves a single int from an XML document. The specified xpath must occur
	 * exactly once in the document. See {@link #getXmlFields getXmlFields} for
	 * additional doc on xpaths and on how occurances are counted.
	 *
	 * @param xpath
	...
	public int getXmlInt(String xpath) throws XMLException {
		...
		try {
			...
			mkerror("Invalid integer: \"" + stg + "\"  in xpath \"" + xpath + "\"  in file \"" + systemid + "\"");
		}
		return ires;
	}

	/**
	 * Retrieves a single boolean from an XML document. The specified xpath must
	 * occur exactly once in the document. The boolean value is case insensitive and
	 * may be encoded as one of: "true", "false", "yes", "no". See
	 * {@link #getXmlFields getXmlFields} for additional doc on xpaths and on how
	...
	public boolean getXmlBoolean(String xpath) throws XMLException {
		...
		if (stg.equalsIgnoreCase("true") || stg.equalsIgnoreCase("yes"))
			...
			mkerror("Invalid boolean: \"" + stg + "\"  in xpath \"" + xpath + "\"  in file \"" + systemid + "\"");
		return bres;
	}

	/**
	 * Retrieves a single String from an XML document. The specified xpath must
	 * occur exactly once in the document. See {@link #getXmlFields getXmlFields}
	 * for additional doc on xpaths and on how occurances are counted.
	 *
	 * @param xpath
	 *            The partial xpath specification.
	 * @return The xmlString value
	 * @exception XMLException
	 *                If the requested xpath was not found.
	 */
	...
	/**
	 * Returns all Elements in an XML document that match the specified partial
	 * xpath. The xpathspec specifies the tail end (right side) of an xpath. For
	 * example:
	 * <ul>
	 * <li>"gamma": return the contents of all elements named "gamma"
	 * <li>"beta/gamma": return the contents of all elements named "gamma" whose
	 * immediate parent is named "beta", no matter what the higher level nodes are
	 * named.
	 *
	 * <li>"alpha/beta/gamma": return the contents of all elements named "gamma"
	 * whose immediate ancestors going up the chain are "beta", "alpha".
	 * <li>"beta/gamma@tau": find all beta/gamma as above, and for those having an
	 ...
	 * <li><b>Never</b> returns a null or zero-length String. If the specified xpath
	 * or attribute exists in the document, but it's content is empty or all blank,
	 * it is <b>not</b> returned.
	 * <li>Never returns a null array. If there are no results, the returned array
	 * has length zero.
	 * <li>If the specified xpath is an interior node (not a leaf), returns the
	 * concatenation of all leafs under it. For example if the xpath is
	 * <code>"beta"</code> and the xml document contains:
	 * <p>
	 * <code>&lt;beta&gt;<br>
	 *        b1 <br>
	 *        &lt;gamma&gt; ggg &lt;/gamma&gt;<br>
	 ...
	 *
	 * @param minnum
	 *            Minimum number of times xpathspec must be found.<br>
	 *            If there are fewer than minnum occurances of the xpathspec
	 *            containing non-blank data, an XMLException is thrown.<br>
	 *            Occurances of xpathspec that are empty or all blank are not
	 *            counted: see Notes above.
	 * @param maxnum
	 *            Maximum number of times xpathspec must be found.<br>
	 *            If there are more than maxnum occurances of the xpathspec
	 *            containing non-blank data, an XMLException is thrown.<br>
	 ...
	 * @throws XMLException
	 *             if the minnum or maxnum constraints are violated.
	 */
	public Element[] getXmlElements(int minnum, // min num times xpathspec must be found
			int maxnum, // max num times, or 0 ==> no max
			String xpathspec) throws XMLException {
		int inode;
		int jj;
		...
		if (bugs >= 1)
			prtln("getXmlFields: xpath: \"" + xpath + "\"  num nodes: " + nodelist.getLength());
		...
		for (inode = 0; inode < nodelist.getLength(); inode++) {
			...
			for (jj = xpathparts.length - 1; jj >= 0; jj--) {
				...
				if (testnode.getNodeType() != Node.ELEMENT_NODE || !(testnode.getNodeName().equals(xpathparts[jj]))) {
					allmatch = false;
					break;
				}
		...
		if (numfound < minnum)
			mkerror("Element at xpath \"" + xpath + "\"  found less than " + minnum + " "
					+ ((minnum == 1) ? "time" : "times") + " in file \"" + systemid + "\"");
		if (maxnum > 0 && numfound > maxnum)
			mkerror("Element at xpath \"" + xpath + "\"  found more than " + minnum + " "
					+ ((minnum == 1) ? "time" : "times") + " in file \"" + systemid + "\"");
	...
	/**
	 * Returns all strings in an XML document that match the specified partial
	 * xpath. The xpathspec specifies the tail end (right side) of an xpath. For
	 * example:
	 * <ul>
	 * <li>"gamma": return the contents of all elements named "gamma"
	...
	public String[] getXmlFields(int minnum, // min num times xpathspec must be found
			int maxnum, // max num times, or 0 ==> no max
			String xpathspec) throws XMLException {
		int inode;
		int ii;
		...
		if (numfound < minnum)
			mkerror("xpath \"" + xpath + "\"  found less than " + minnum + " " + ((minnum == 1) ? "time" : "times")
					+ " in file \"" + systemid + "\"");
		if (maxnum > 0 && numfound > maxnum)
			mkerror("xpath \"" + xpath + "\"  found more than " + minnum + " " + ((minnum == 1) ? "time" : "times")
					+ " in file \"" + systemid + "\"");
	...
	/**
	 * Returns all strings in an XML document that match the specified partial
	 * xpath, or a zero-length array if none were found.
	 *
	 * @param xpathspec
	 *            the partial xpath specification.
	 * @return The xmlFields value
	...
	/**
	 * Returns the first occurance of the field that matches the specified partial
	 * xpath, or a an empty String if not found.
	 *
	 * @param xpathspec
	 *            the partial xpath specification.
	...
	/**
	 * Returns a concatenation of all the text and cdata content in a Node's
	 * subtree.
	 *
	 * @param nd
	 *            DESCRIPTION
	...
	/**
	 * Gets a human-readable validation error report if errors were found in the
	 * XML, otherwise returns an empty StringBuffer.
	 *
	...
	/**
	 * Gets a human-readable validation warning report if warnings were found in the
	 * XML, otherwise returns an empty StringBuffer.
	 *
	 ...
	 *                DESCRIPTION
	 */
	static void mkerror(String msg) throws XMLException {
