...
public class NamespaceRegistry {
	...
	public NamespaceRegistry() {
		// prtln ("\nNamespaceRegistry: namedDefaultCreationEnabled: " +
	...
	public void register(Namespace ns) {
		...
		/*
		 * NOTE: this implementation of uriMap assumes that there will be only one
		 * namespace per uri, which is probably not safe. In particular, if we define a
		 * "namedDefaultNamespace" then we have two namespaces with the same uri.
		 * HOWEVER, this implementation allows us to assume that the namespace returned
		 * for a given uri is the last-registered namespace, and that is a nice
		 * property, e.g., when we register a namedDefaultNamespace, then we get this
		 * when we ask for the namespace assigned to it's uri, and this is what we count
		 * on in schema processing when we call "getSchemaNamespace()".
		 */
		String uri = ns.getURI();
		Namespace existing = getNSforUri(uri);
		if (existing != NO_NAMESPACE) {
			// prtln ("\t NOTE: overwriting existing namespace with prefix: \"" +
			// existing.getPrefix() + "\"");
		}
	...
	public boolean isMultiNamespace() {
		...
		for (Iterator i = prefixMap.values().iterator(); i.hasNext();) {
			...
			String uri = ns.getURI();
			if (uri != this.schemaNamespaceUri && uri != this.xmlNamespaceUri) {
				uris.add(uri);
				// prtln ("\tadded " + uri);
			}
		}
	...
	public String getPrefixforUri(String uri) {
		...
		if (ns == Namespace.NO_NAMESPACE) {
			// prtln ("WARNING: getPrefixforUri() did not find namespace not found for " +
			// uri);
		}
		return ns.getPrefix();
	}
	 ...
	 *
	 * @param uri
	 *            The new targetNamespaceUri value
	 */
