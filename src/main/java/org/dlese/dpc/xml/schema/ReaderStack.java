...
/**
 * Mains a pushdown stack of SchemaReaders for use in StructureWalker to resolve
 * qualified names and find GobalDef instances from the GlobalDefMap.
 *
 * @author ostwald
...
public class ReaderStack {
	...
	/**
	 * Traverse the stack to find a SchemaReader that defines a namespace for the
	 * given prefix. Return null if a reader is not found.
	 */
	...
	/**
	 * Search the stack (moving from local to more global SchemaReaders) for the
	 * namespace belonging to the given prefix.
	 */
	...
	public GlobalDef getGlobalDef(String typeName) {
		...
		if (NamespaceRegistry.isQualified(typeName)) {
			...
			/*
			 * if (namespace == namespaces.getSchemaNamespace()) { // return null; prtln
			 * ("\t\t\t creating builtInType: " +
			 * NamespaceRegistry.makeQualifiedName(prefix, typeName)); return new
			 * BuiltInType (NamespaceRegistry.makeQualifiedName(prefix, typeName)); }
			 */
		}
	...
	/**
	 * Returns a list of Item instances - one for each element in the choice
	 * ReaderStack
	 *
