...
public class DateValidator {
	...
	/**
	 * First make sure value is valid as a "xsd:gYear" and then make sure the max
	 * inclusive value is "-0001"
	 */
	...
	public static boolean checkValidUnionDate(String value) throws Exception {
		...
		// We know the format is okay because it passed by parseUnionDate. Now we
		...
		try {
			...
			// if no seconds are provided, then we don't have to check the hours, seconds or
			// minutes
			...
			if (provided > -1) {
				// adjusting the calendar in accordance with timezone offset sincs the calendar
				// date with the
				// parsedDate. We could have instead set the calendar's time zone to UTC, but
				// then the PRINTED
			...
			// now check the parsed DAY, and MONTH (YEAR is assumed to be okay if it parsed)
			// against the provided values. if
			// they are different, this is because the "lenient" parsing "carried over" the
			// units
			// from one out of range field to another (if DAY was 32, then MONTH will be
			// incremented)
	...
	static String[] testCases = {"", " ", "2005", "2005-01", "2005-01-30", "2005-01-30T01:01:01Z",
			"2005-01-30T23:01:01z", "2005-13", "2005-01-33", "2005-01-30T00:01:01Z", "2005-01-30T24:01:01z",
			"2005-01-30T23:99:01z", "2005-01-30T23:59:60z"};
