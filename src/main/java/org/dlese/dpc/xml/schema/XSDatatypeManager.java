 ...
 * schema-based document. XSdatatypes validate a value against a defined
 * datatype, whether it is built-in or derived. Built-in data types are supplied
 * by Sun's XSDatatype library, data types specified by a particular schema are
 * derived by extending the built-in types (see <i>deriveType()</i>).
 *
 * @author ostwald
 *
 */
public class XSDatatypeManager {
	...
	/**
	 * Gets a XSDatatype class used to validate a value against a datatype declared
	 * or defined in an XML Schema. XSDatatypes are found either in the derivedTypes
	 * map or in the DatatypeFactory (which returns XSDatatyps for built-in types)
	 *
	 * @param typeName
	 *            XSDatatype name
	...
	public XSDatatype getTypeByName(String typeName) {
		...
		// if (dt == null) prtln ("getTypeByName() unable to find XSDatatype for " +
		// typeName);
		return dt;
	}
	 ...
	 * This is a pretty weak way of dealing with this problem, since in other cases,
	 * an error is thrown to signify an invalid value.
	 * <p>
	 *
	 * date values are intercepted and handled by
	 * {@link org.dlese.dpc.util.MetadataUtils#parseUnionDateType(String)}
	 *
	 * @param typeName
	 *            Description of the Parameter
	 * @param value
	 ...
	 */
	public boolean checkValid(String typeName, String value) throws Exception {
	...
	/**
	 * validates a String value against an XSDatatype. If the value is not valid an
	 ...
	 *                Description of the Exception
	 */
	public boolean checkValid(XSDatatype dt, String v) throws Exception {
		String errorMsg = "";
		try {
	...
	/*
	 * private void deriveSimpleTypesOld() { List keys =
	 * globalDefMap.getKeys(GlobalDef.SIMPLE_TYPE); List unionNodes = new
	 * ArrayList(); for (Iterator i = keys.iterator(); i.hasNext(); ) { String key =
	 * (String) i.next(); SimpleType def = (SimpleType) globalDefMap.getValue(key);
	 * if (def.isUnion()) { unionNodes.add(def); } else { XSDatatype dt =
	 * deriveType(def); if (dt != null) { addDerivedType(dt); } } } for (Iterator i
	 * = unionNodes.iterator(); i.hasNext(); ) { SimpleType unionNode = (SimpleType)
	 * i.next(); XSDatatype dt = deriveType(unionNode); if (dt != null) {
	 * addDerivedType(dt); } } }
	 */
	/**
	 * Derive a XSDatatype for the given SimpleType (a GlobalDef) by calling either
	 *
	 * @param def
	 *            A wrapper for a Simple Data Type definition
	 * @return Description of the Return Value
	 */
	private XSDatatype deriveType(SimpleType def) {
		...
		try {
			...
			// we know that BCType cannot be derived because of the date problems in the
			// Schema Spec,
	 ...
	 *                Description of the Exception
	 */
	private XSDatatype deriveByRestriction(SimpleType def) throws Exception {
	 ...
	 *                Description of the Exception
	 */
	public XSDatatype deriveByUnion(SimpleType def) throws Exception {
	 ...
	 * Currently requires that a "xsd:" prefix is present in the base attribute, and
	 * remove the prefix before returning remainder of base attribute.
	 * 
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		 * for (int i=0;i<args.length;i++) prtln (i + ": " + args[i]);
		 */
		if (mode.equals("type")) {
