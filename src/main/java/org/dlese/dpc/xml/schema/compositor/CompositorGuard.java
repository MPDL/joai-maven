 ...
 * occurrance schema constraints defined for a Compositor (e.g., All, Choice, or
 * Sequence). CompositorGuards determine whether there is "room" in a given
...
public abstract class CompositorGuard {
	 ...
	 * <p>
	 *
	 * A hook to force validation is provided but not used because we most often are
	 * dealing with instanceElements that are not valid (e.g., user has just added a
	 * element in the metadata editor). Absent the validity constraint, this method
	 * returns if the number of occurrences are less then the number of occurrances
	...
	/**
	 * Tests whether the instance element can accept a new member of "memberName" at
	 * the specified memberIndex.
	 *
	 * @param memberName
	 *            NOT YET DOCUMENTED
	 * @param memberIndex
	 *            NOT YET DOCUMENTED
	 * @return true if the instance element will accept the specified member.
	 */
	public boolean acceptsNewMember(String memberName, int memberIndex) {
		...
		// for sequence compositors with a single member, we simply call
		// acceptsNewMember()
	...
	/**
	 * Returns a list of occurrence instances that can be used to determine whether
	 * an instance document element satisfies the occurrence constraints of the
	 * schema.
	 *
	...
	protected final int getSubOccurrenceCount(CompositorMember cm, Element instanceElement) throws Exception {
		...
		CompositorGuard subCompsitorGuard = CompositorGuard.getInstance(cm.getSubCompositor(), instanceElement);
	...
	/**
	 * Given a leaf element name, find the CompositorMember that ultimately controls
	 * it
	 *
	 * @param leafName
	 *            NOT YET DOCUMENTED
	...
	public CompositorMember resolveLeafNameToCM(String leafName) throws Exception {
		...
		// sanity check - we MUST must find a member for this child - unless our
		// compository
		// has an "any" element
		...
		if (compositor.getInstanceNames().contains(leafName)) {
			...
			/*
			 * find which complex CompositorMembers this element belongs to - loop through
			 * compositor members and test complex members to see if "name" matches to a
			 * leafMemberName
			 */
	...
	/**
	 * Find the occurrence containing the nth element of memberName. This is used to
	 * determine which occurence can take a new member of memberName.
	 ...
	 * then it checks to see if the member can be added to that occurrence legally.
	 * <p>
	 *
		...
		for (Iterator i = occurrences.iterator(); i.hasNext();) {
			...
			for (int j = 0; j < elements.size(); j++) {
				...
				if (memberCounter >= memberIndex) {
					prtln("\t returning .. memberCounter: " + memberCounter + "  memberIndex: " + memberIndex);
					return o;
				}
			}
		}
	...
	/**
	 * An occurrence holds the instance elements contained in a single occurence of
	 * the compositor. For example, if a compositor has maxOccurs of 3 and there are
	 * 5 instanceElements, then that compositor would have two instances of the
	 * Occurrence class.
	 *
	 * @author ostwald
	 * @created June 26, 2006
	...
	private static void prtln(String s) {
		if (debug) {
			 ...
			 * s.substring(1); } System.out.println("CG: " + s);
			 */
