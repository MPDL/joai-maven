...
public class ChoiceGuard extends CompositorGuard {
	...
	/**
	 * Returns a list of element names representing the choices that can be legally
	 * made.
	 *
	 ...
	 * schema.
	 * <p>
	 * Walk down the buckets, when there is no room to add the current member, start
	 * an new occurrance. NOTE: the order in which we add members is aribitrary - we
	 * just can't overfill any single member.
	 * <p>
	 * 
	 * NOTE: this does not account for many of the children choice compositors can
	 * have: - choice - sequence - any - element - group
	 *
	...
	/**
	 * NOTE: if any of the choice compositor members has a minOccurs of 0, then
	 * there need not be any occurrances!
	 *
	 ...
	 * <P>
	 * Takes into account whether there is a legal choice to be made, as well as the
	 * maxOccurs constraint of the compositor.
	 *
	 * @return NOT YET DOCUMENTED
	 */
	...
	private boolean acceptsNewMember(String name) {
		...
		// mutuallyExclusive does NOT inforce the semantics of the choice compositor,
		// but
		// it does enforce the intent of the multi-choice in the dlese_anno 1.0.00
		// framework

		boolean ret = false;

		if (mutuallyExclusive) {
			// this test does not allow choices to be made more than one time, reguardless
			// of schema semantics
			...
			ret = (getOccurrencesCount() < compositor.getMaxOccurs() && occurCnt < 1);
		} else {
	 ...
	 */
	/*
	 * protected List getOccurrences() throws Exception { prtln
	 * ("getOccurrences()"); occurrenceCounter = new OccurrenceCounter();
	 * occurrences = new ArrayList(); List groups = getElementGroups();
	 * 
	 * // debugging prtln ("\t " + groups.size() + " groups found"); //
	 * printElementGroups();
	 * 
	 * // prtln (" iterating through groups ..."); for (Iterator i =
	 * groups.iterator(); i.hasNext(); ) { ElementGroup group = (ElementGroup)
	 * i.next(); String name = group.name; // prtln ("\t " + name); try {
	 * CompositorMember member = compositor.getLeafMember(name); if (member == null)
	 * { throw new Exception("couldnt find member for " + name); } Occurrence
	 * occurrence = new Occurrence(member); occurrence.elements = group.elements;
	 * occurrenceCounter.inc (name); occurrences.add(occurrence);
	 * 
	 * } catch (Exception e) { throw new Exception("error with \"" + name + "\": " +
	 * e.getMessage()); } } return occurrences; }
	 */
