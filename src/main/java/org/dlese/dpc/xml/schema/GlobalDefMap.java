...
/**
 * Stores GlobalDef instances, which are wrappers for important elements of an
 * XML Schema, including
 * <ul>
 * <li>Simple and ComplexType definitions,
 * <li>Goup definitions, and
 * <li>Global Element and Global Attribute declarations
 * </ul>
 * <p>
 * The definitions are stored in a two-level mapping. The first level is by
 * namespaceUri, so there is a map for each namespace. The individual namespace
 * maps store all the GlobalDefs for that namespace. To access a particular
 * globalDefinition, the <i>name</i> of the definition (e.g., the type name) is
 * required, as well as the <i>namespaceUri</i> for the namespace in which the
 * definition is defined.
 * <p>
 * This class also contains a NamespaceRegistry that stores the namespaces
 * defined at the global level of the schema. NOTE: it is possible that
 * schemafiles define and use namespaces that are not visible at the top-level
 * of the schema. In this case, there will be entries in the GlobalDefMap for
 * these namespaces, but the namespaces themselves will not be registered in the
 * NamespaceRegistry.
 * 
 *
 * 
...
public class GlobalDefMap {
	...
	public Map getNsMap(String nsUri) {
		...
		if (nsMap == null) {
			if (nsUri.equals(namespaces.schemaNamespaceUri) || nsUri.equals(namespaces.xmlNamespaceUri)) {
	...
	/**
	 * Accessor method for retrieving a specific named GlobalDef. Namespace prefixes
	 * for the default name space (or the targetNameSpace if there is no
	 * defaultNamespace defined), are stripped.
	 *
	 * @param name
	 *            Description of the Parameter
	...
	public GlobalDef getValue(String name, String nsUri) {
		...
		if (!nsMap.containsKey(name)) {
			// for Built-ins, we create a new object and place it in the globalDefMap under
			// the schemaNamespaceUri
			if (nsUri.equals(namespaces.schemaNamespaceUri) || nsUri.equals(namespaces.xmlNamespaceUri)) {
	 ...
	 * @exception Exception
	 *                if the GlobalDef is already in the map
	 */
	public void setValue(String name, GlobalDef def, Namespace ns) throws Exception {

		/*
		 * // debugging String s = "setValue()"; s += "\n\t name: " + name; s +=
		 * "\n\t def: " + def.getQualifiedName(); s += "\n\t namespace: " +
		 * ns.getPrefix() + ": " + ns.getURI(); prtln (s);
		 */
