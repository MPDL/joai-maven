...
/**
 * Implementation of {@link org.dlese.dpc.xml.schema.compositor.CompositorGuard}
...
public class SequenceGuard extends CompositorGuard {
	...
	public boolean acceptsNewMember() {
		...
		// for sequence compositors with a single member, we accept a new member if the
		// last occurrance has
		// any room ...
		...
		if (compositor.getMembers().size() == 1 && oCount == compositor.getMaxOccurs()) {
			...
			// prtln(" .. singleton: elements: " + lastOccurrence.count + ", maxOccurs: " +
			// memberMaxOccurs);
			// printOccurrences();

			return (lastOccurrence.count < memberMaxOccurs);
		} else {
			// prtln(" .. occurrences: " + oCount + ", compositor.maxOccurs: " +
			// compositor.getMaxOccurs());
	 ...
	 * leafMembersNames) we collect them in the same bucket.
	 * <p>
	 *
	 * Each bucket instance keeps track of how many elements it contains. Later, the
	 * buckets list is split into Occurrences.
	 *
	 * @return CMBuckets found in the instance document element to be tested
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	private List getCMBuckets() throws Exception {
		...
		for (Iterator i = instanceMembers.iterator(); i.hasNext();) {
			...
			if (cm != lastCM) {
				...
				// new bucket UNLESS ...
				if (lastCM != null && !lastCM.hasSubCompositor() && !cm.hasSubCompositor()) {
	...
	/*
	 * Return occurrences build from a CompositorMember bucket.<p> Bucket elements
	 * are used in order to populate occurrences. The instanceElements are consummed
	 * greedily (maximum number of of members available and allowed are used). When
	 * a member does not legally follow it's predecessor, start a new occurrence.<p>
	 * throw Exceptions for: - an instanceElement that can't be matched against a CM
	...
	private List normalizeBucket(CMBucket bucket) throws Exception {
		...
		while (bucketElements.hasNext()) {
			...
			/*
			 * if currentCM is complex we make recursive call - getSubOccurrences with this
			 * instanceElement we then check subOoccurrenceCount against the
			 * currentCM.maxOccurs
			 */
