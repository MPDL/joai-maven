...
/**
 * Compositor class specifies methods for accessing and validating the "members"
 * of a Compositor Element (i.e., All, Sequence and Choice), as well as the
 * acceptsNewMember method. The members are a list of CompositorMember instances
 * which represent the child elements of the compositor element.
 *
 * @author ostwald
 */
public abstract class Compositor {
	...
	/**
	 * Initialize this compositor with occurance information and instantiate
	 * compositor members
	 */
	protected void init() {
		if (element == null) {
		...
		for (Iterator i = element.elementIterator(); i.hasNext();) {
			...
			/*
			 * WARNING: this.anyTypeMember implies that there can only be ONE anyType member
			 * per compositor, but this is NOT THE CASE!
			 */
	...
	/**
	 * Return a list of CompositoMembers made from the substitionGroup for the given
	 * headElement.
	 *
	 * @param headElement
	 *            NOT YET DOCUMENTED
	 ...
	 * CompositorMembers for the leaf elements (those members that can no longer be
	 * expanded. LeafMembers are important because only they are present in an
	 * instanceDocument.
	 * <p>
	 *
	 ...
	 * <p>
	 *
	 * NOTE: Does this algorithm take derivedContentModels into account?? I.e., how
	 * are complexContent (extensions) handled??
	 *
		...
		if (leafMembers == null) {
			...
			for (Iterator i = element.elementIterator(); i.hasNext();) {
				...
				if (e_name.equals("element")) {
					if (ref != null) {
						// this is a reference to a GlobalElement - we need to check if it can be
						// expanded
			...
			for (Iterator i = element.elementIterator(); i.hasNext();) {
				...
				if (e_name.equals("element")) {
					if (ref != null) {
						...
						String instanceName = parent.getSchemaReader()
								.getInstanceQualifiedName(e.attributeValue("name"));
						names.add(instanceName);
					}
				} else if (e_name.equals("group")) {
					if (ref == null) {
	...
	public final List getInstanceNames() {
		...
		for (Iterator i = getMembers().iterator(); i.hasNext();) {
			...
			if (!member.hasSubCompositor()) {
				...
				if (typeDef != null && typeDef.isGlobalElement() && ((GlobalElement) typeDef).isHeadElement()) {
	 ...
	 * <p>
	 *
	 * NOTE: this might be okay for Sequence and All, but it doesn't work for CHOICE
	 * compositors, since only one member can contribute PER OCCURRANCE!
	 *
	...
	protected static void prtln(String s) {
		if (debug) {
			 ...
			 * s.substring(1); } System.out.println("Compositor: " + s);
			 */
