...
public class SchemaNodeMap {
	...
	public boolean init() {
		 ...
		 * realized that "getValue" and "containsKey" methods were not working properly
		 * (e.g., containsKey would return true when a key was not really in the map,
		 * and getValue would return THE WRONG VALUE when the key wasn't in the map! It
		 * workes fine under HashMap.
		 * 
		 */
	...
	/**
	 * Comparator to sort the keyMap in document order. Element paths are processed
	 * in doc order, so we map the paths to an incrementing counter, and then
	 * maintain the map's order using the counter values.
	 *
