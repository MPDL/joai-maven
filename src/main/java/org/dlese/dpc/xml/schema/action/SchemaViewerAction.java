...
public final class SchemaViewerAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		// RemoteSearcher rs = (RemoteSearcher)
		// servlet.getServletContext().getAttribute("RemoteSearcher");
		...
		/*
		 * if ((xmlFormat == null) || (xmlFormat.trim().length() == 0)) { // Default
		 * Framework is ADN prtln ("xmlFormat is unknown: assigning adn" +
		 * svf.getXmlFormat()); framework = getMetaDataFramework("adn"); // framework =
		 * setFramework (svf.getXmlFormat(), svf); } else { framework =
		 * getMetaDataFramework (xmlFormat); }
		 */
		...
		if (command.equals("doPath")) {
			...
			if ((path == null) || (path.trim().length() == 0)) {
				errors.add(errors.GLOBAL_ERROR, new ActionError("generic.error", "View didn't get an path!"));
				saveErrors(request, errors);
				return mapping.findForward("schema.index");
			} else {
				if (path.endsWith("/"))
			...
			/*
			 * GlobalDef globalDef = schemaHelper.getGlobalDefFromXPath(path); if (globalDef
			 * == null) { errors.add(errors.GLOBAL_ERROR, new
			 * ActionError("schemaviewer.def.notfound.error", path)); saveErrors(request,
			 * errors); return mapping.findForward("schema.index"); } else {
			 * svf.setGlobalDef(globalDef); // svf.setTypeName(globalDef.getName()); }
			 */

			SchemaNode schemaNode = schemaHelper.getSchemaNode(path);
			if (schemaNode == null) {
				errors.add(errors.GLOBAL_ERROR, new ActionError("schema.schemaNode.notfound.error", path));
				saveErrors(request, errors);
				return mapping.findForward("schema.index");
			} else {
		...
		if (command.equals("doType")) {
			if ((typeName == null) || (typeName.trim().length() == 0)) {
				errors.add(errors.GLOBAL_ERROR, new ActionError("generic.error", "Didn't get an type name!"));
				saveErrors(request, errors);
				return mapping.findForward("schema.index");
			} else {
				svf.setPath(path);
			...
			// GlobalDef globalDef =
			// (GlobalDef)schemaHelper.getGlobalDefMap().getValue(typeName);
			...
			if (globalDef == null) {
				errors.add(errors.GLOBAL_ERROR, new ActionError("schemaviewer.def.notfound.error", typeName));
				saveErrors(request, errors);
				return mapping.findForward("schema.index");
			} else {
	...
	private FrameworkRegistry getRegistry() throws ServletException {
		FrameworkRegistry reg = (FrameworkRegistry) servlet.getServletContext().getAttribute("frameworkRegistry");
		if (reg == null) {
	...
	private ActionErrors setGlobalDef(String path, SchemaHelper schemaHelper, SchemaViewerForm svf) {
		...
		if (globalDef == null) {
			errors.add(errors.GLOBAL_ERROR, new ActionError("schemaviewer.def.notfound.error", path));
		} else {
			svf.setGlobalDef(globalDef);
	...
	private MetaDataFramework getMetaDataFramework(String xmlFormat) throws ServletException {
		 ...
		 * .getAttribute("frameworkRegistry"); return reg.getFramework (xmlFormat);
		 */
	...
	private ActionErrors setFramework(MetaDataFramework mdf, SchemaViewerForm svf) throws ServletException {
		...
		// mdf = (MetaDataFramework)
		// servlet.getServletContext().getAttribute(xmlFormat);
		ActionErrors errors = new ActionErrors();
		String errorMsg = "";
		if (mdf == null) {
		...
		if (schemaNode == null) {
			errors.add(errors.GLOBAL_ERROR, new ActionError("schema.schemaNode.notfound.error", rootPath));
			return errors;
		}
		svf.setSchemaNode(schemaNode);
