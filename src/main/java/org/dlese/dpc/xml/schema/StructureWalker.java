...
public class StructureWalker {
	...
	public StructureWalker(Element schemaRootElement, SchemaHelper schemaHelper) throws SchemaHelperException {
		...
		/*
		 * See NamespaceRegistry.getNamedDefaultNamespace() How it works determines
		 * whether namespaces will be enabled or not
		 */
		/*
		 * defaultNamespace = namespaces.getNamedDefaultNamespace(); if
		 * (defaultNamespace != Namespace.NO_NAMESPACE) { namespaceEnabled = true; }
		 */
		...
		// REQUIRED to add namespaces to root element so the local namespace definitions
		// disappear
		...
		if (namespaceEnabled) {
			...
			// add schemaLocation attribute and schemaInstance/schemaURI namespace
			// declaration
			...
			for (Iterator i = namespaces.getNamespaces().iterator(); i.hasNext();) {
				...
				if (!targetNSUri.equals(ns.getURI()) && ns.getURI() != namespaces.getSchemaNamespace().getURI()
						&& !ns.getPrefix().equals("")) {
		...
		/*
		 * // for debugging - prints out the namespaces associated with the
		 * instanceDocument NamespaceRegistry nsr = new NamespaceRegistry();
		 * nsr.registerNamespaces(instanceDocument); prtln(nsr.toString(), 1);
		 */
	}
	 ...
	 * <p>
	 *
	 * we need to retain the schemaNodes for these elements, but we don't want them
	 * to exist in the instance document.
	 */
	private void pruneSubstituteNodes() {
		...
		while (schemaNodes.hasNext()) {
			...
			try {
				...
				prtln("WARNING: pruneSubstituteNodes unable to process element for " + schemaNode.getXpath() + ": "
						+ e.getMessage(), 2);
			}

			if (instanceElement == null) {
				// prtln("\t ... WARNING pruneSubstitutionNodes did not find instanceElement",
				// 1);
			...
			if (schemaNode.isSubstitutionGroupMember()) {
				// box ("pruneSubstituteNodes about to remove child from instance document ...",
				// 1);
	...
	private Namespace popNS() {
		...
		prtln("  ..... currentNS is now (" + getCurrentNamespace().getPrefix() + ": " + getCurrentNamespace().getURI(),
				1);
		return ns;
	}
	 ...
	 *
	 * @param e
	 *            schema Element representing the attribute
	 * @param parent
	 *            The parent of this attribute in the instance Document
	 * @param attrName
	 *            The attribute name
	 * @exception Exception
	 *                Description of the Exception
	 ...
	 *            The attribute name
	 * @param attrType
	 *            typeName (if present) overrides type specified in schema element
	 * @exception Exception
	 *                Description of the Exception
	 */
	 ...
	 *
	 * @param e
	 *            an instance element definition represented as a
	 *            {@link org.dom4j.Element}
	 * @return The typedef (a GlobalDef) for the given Schema Element or null if
	 *         either a typeName is not present or a GlobalDef cannot be found for
	 *         the typeName.
	 * @exception Exception
	 *                Description of the Exception
	 */
	private GlobalDef getElementTypeDef(Element e) throws Exception {
		...
		if (typeName == null) {
			...
			/*
			 * errorMsg = "getElementTypeDef(): typeName not found for typeDef element: " +
			 * e.asXML(); throw new Exception(errorMsg);
			 */
		}
	 ...
	 * designates a built-in datatype, as opposed to a derived type.
	 * <p>
	 *
	 * An qualified attribute value is determined to be BuiltIn if it has the same
	 * prefix as the qualified name of the element in which it is defined.
	 * <p>
	 *
	 * E.g., in the following element, the type is "xs:string", which is a built-in
	 * type: &lt;xs:element name="artist" type="xs:string"/>.
	 *
	 * @param e
	 *            Description of the Parameter
	 ...
	 *
	 * @param e
	 *            current schema Element to process
	 * @param parent
	 *            instanceDoc Element to which we attach new instanceElement
	 * @exception Exception
	 *                if this element cannot be processed
	 */
	private void processSchemaElement(Element e, Element parent) throws Exception {
		String elementName = e.getName();
		prtln("\n================================================================"
				+ "\nprocessSchemaElement() with elementName: " + elementName + "\n\t" + e.asXML() + "\n\tparent path: "
				+ parent.getPath() + "\n================================================================", 1);

		/*
		 * if the element name is "element" (as opposed to a type definition) we ASSUME
		 * we are looking at a TYPED element or a global Reference. Each element
		 * encounted in this way will result in a new SchemaNode being created and a new
		 * element added to the instanceDocument
		 */
		if (elementName.equals("element")) {
			...
			if (elementName.equals("all") || elementName.equals("sequence") || elementName.equals("choice")) {
			...
			} else if (elementName.equals("simpleContent") || elementName.equals("complexContent")) {
			...
			/*
			 * else if (elementName.equals("annotation")) { String parentXPath =
			 * getPath(parent); prtln ("\nANNOTATION! parent path: " + parentXPath, 4);
			 * prtln (Dom4jUtils.prettyPrint(e), 1); SchemaNode schemaNode =
			 * (SchemaNode)this.schemaNodeMap.getValue(parentXPath); if (schemaNode == null)
			 * { prtln ("\t SchemaNode not found!", 1); } Element docElement = e.element
			 * ("documentation"); if (docElement != null && schemaNode != null) { String
			 * documentation = docElement.getTextTrim();
			...
			/*
			 * if (e.element("annotation") != null) { String xpath = getPath(e); prtln
			 * ("\nANNOTATION!  path: " + xpath, 4); prtln (Dom4jUtils.prettyPrint(e), 1);
			 * SchemaNode schemaNode = (SchemaNode)this.schemaNodeMap.getValue(xpath); if
			 * (schemaNode == null) { prtln ("\t SchemaNode not found!", 1); } Element
			 * docElement = (Element)e.selectSingleNode ("//documentation"); if (docElement
			 * != null && schemaNode != null) { String documentation =
			 * docElement.getTextTrim(); schemaNode.setDocumentation(documentation); } }
			 */
		}
	}
	...
	private void processModelGroup(Element e, Element parent) throws Exception {
		...
		// now we've got our model group, and we can process it just as we would a
		// complexType?
	...
	private void processAnyElement(Element e, Element parent) throws Exception {
		...
		prtln("\t processAnyElement() instantiating SchemaNode with built-in type (name: " + typeDef.getName() + ")",
				2);
	...
	/**
	 * Process a schema element defining an attribute.
	 * <p>
	 *
	...
	private void processInstanceAttribute(Element e, Element parent) throws Exception {
		...
		if (globalDef.isGlobalAttribute()) {
			...
			if (ns != getCurrentNamespace() && !NamespaceRegistry.isQualified(attrName)) {
				...
				attrName = NamespaceRegistry.makeQualifiedName(topLevelPrefix,
						NamespaceRegistry.stripNamespacePrefix(attrName));
			}
			...
			// the attribute NAME is supplied by the name attribute of the referred to
			// GlobalAttribute
			...
			/*
			 * prtln ("\t\tcalling globalDefMap.getValue (" + attrType +
			 * ") from processInstanceAttribute", 1); GlobalDef attrTypeDef = (GlobalDef)
			 * globalDefMap.getValue (attrType); if (attrTypeDef == null) attrTypeDef = new
			 * BuiltInType (attrType);
			 */
	...
	private void processBuiltIn(Element e, Element parent) throws Exception {
		...
		/*
		 * QName qname = df.createQName(e.attributeValue("name"),
		 * getCurrentNamespace()); Element child = df.createElement(qname);
		 */
	...
	/**
	 * this method will indeed insert the right elements, but we do we want them in
	 * the instance document?? i'm thinking that we DO want them in the
	 * schemaNodeMap, but not in the instanceDocument ... so we can remove them from
	 * the instance document
	 *
	 * @param globalElementDef
	 *            the GlobalElement type definition containing the sub group element
	 * @param e
	 *            current schema Element to process
	 * @param parent
	 *            instanceDoc Element to which we attach new instanceElement
	...
	private void processInstanceElement(Element e, Element parent) throws Exception {
		...
		/*
		 * for SimpleTypes and ComplexTypes, we create a SchemaNode for this element and
		 * create an instanceDocument element. In the case of ComplexTypes, we then
		...
		if (globalDef.isSimpleType() || globalDef.isComplexType()) {
			...
			if (isExtensionElement(e)) {
				/*
				 * we don't want to create a new schemaNode for "extension elements", which
				 * extend a schemaNode that has already been created. Extension elements are
				 * chiefly used in Simple and ComplexContent elements of ComplexType
				 * definitions. When extension elements are encountered during expansion , a
				 * dummy element of the extension's base type is created and then expanded in
				 * place of the original element. (search this buffer for
				 * 'elementName.equals("extension")' to see where in this method they are
				 * processed)
				 */
			...
			if (globalDef.isSimpleType()) {
				// we've already added the child for this simple type, there is nothing else to
				// do
				...
				/*
				 * if (ns.getPrefix().equals("")) { prtln
				 * ("\t\t NAMESPACE PREFIX IS EMPTY, assigning default namespace", 1); ns =
				 * defaultNamespace; }
				 */
				pushNS(ns);
				...
				for (Iterator i = children.iterator(); i.hasNext();) {
					...
					/*
					 * try { processSchemaElement(childElement, child); } catch (Exception
					 * expandException) { String msg =
					 * "processSchemaElement encountered error processing element of complexType: "
					 * + expandException.getMessage(); expandException.printStackTrace(); throw new
					 * Exception(msg); }
					 */
				}
				popNS();
			...
			/*
			 * we're dealing with a REFERENCE to a global Element (globalDef wraps this
			 * global Element) we only handle the cases where the referred to global element
			 * has a datatype of either built-in or GlobalDef.SIMPLE_TYPE! in these cases, a
			 * SchemaNode is created that points to the referred to type, and processing of
			 * this branch of the document stops (processSchemaElement is not called
			 * recursively);
			 */
	 ...
	 * the extension's base type. The dummy element has an attribute of "extension"
	 * so it can be identified and handled properly when it is fed back into
	 * processSchemaElement and expanded as if it were an element of the baseType.
	 * <p>
	 *
	 ...
	 *
	 * @param e
	 *            The schema extention element being processed
	 * @param parent
	 *            The instanceDoc parent of the extention (a simple or
	...
	private void processExtension(Element e, Element parent) throws Exception {
		...
		// embed namespace information into the dummy element so it is available to
		// "isBuiltInType"
		...
		/*
		 * // print out some DEBUGGING info if (parentName.equals("catalog")) {
		 * prtln("parent name = " + parentName, 1); prtln("base = " + baseType, 1);
	...
	/**
	 * Process restriction element of a ComplexContent, which redefines the content
	 * model by including only a subset of the elements defined in the base. We are
	 * not interested in validating the restriction, so we simply pass the content
	 * of the restriction element back into processSchemaElement as if there was no
	 * restriction element at all
	 *
	 * @param e
	 *            current schema Element to process
	 * @param parent
	 ...
	 * referred-to global Element).
	 * <P>
	 *
	 * E.g., &lt;xs:element ref="sh:album"/>
	 * <p>
	 *
	 * Algorith: Add a child to the instanceDocument, and process the typeDefinition
	 * of the referred-to Global Element.
	 *
	 * @param globalDef
	 *            the referred-to GlobalElement definition
	 * @param e
	 *            the Element to process
	 * @param parent
	 *            parent in the instance document
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	...
	private Map getUrisFromNR() {
		...
		for (Iterator i = namespaces.getPrefixMap().keySet().iterator(); i.hasNext();) {
			...
			/*
			 * if (prefix.trim().length() > 0) uris.put(prefix, ns.getURI());
			 */
