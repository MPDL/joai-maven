...
public class GenericType implements GlobalDef {
			...
			for (Iterator i = memberTypeDefs.iterator(); i.hasNext();) {
				...
				if (values == null) {
					...
					if (!simpleType.isComboUnionType()) {
						prtln("WARNING: getEnumerationValues got a comboUnionType: "
								+ memberTypeDef.getQualifiedName());
					}
				} else {
	...
	/**
	 * Create the leafType name of an enumerated type. Many of the enumarations have
	 * a set of human readable values, or "leafTypes". By convention, these
