...
public class SimpleType extends GenericType {
	...
	/**
	 * Returns the typeDef for the base enumeration type, or null if this SimpleType
	 * is not an enumeration
	 */
	public GlobalDef getEnumerationBaseType() {
		if (!isEnumeration())
			return null;
		Element restriction = (Element) getElement()
				.selectSingleNode(NamespaceRegistry.makeQualifiedName(xsdPrefix, "restriction"));
		if (restriction == null)
			return null;
	...
	/**
	 * comboUnion types are enumerations with one additional member of either
	 * "xsd:string" or "stringTextType". The are rendered as a textInput with a
	 * drop-down menu of choices
	 */
	public boolean isComboUnionType() {

		/*
		 * We can have at most one non-enumeration type, and (for now) it has to be
		 * "${XSD}:string" or "stringTextType" - where ${XSD} stands for the XML
		 * NamespaceSchema prefix at the instanceDoc level (rather than at the level of
		 * the schema file at which this type is defined.
		 */
		if (isUnion()) {

			String instanceLevelXsdPrefix = this.schemaReader.resolveToInstancePrefix(this.getXsdPrefix());

			List nonUnionFields = Arrays.asList(new String[]{
					NamespaceRegistry.makeQualifiedName(instanceLevelXsdPrefix, "string"), "stringTextType"});
	...
	public List getSimpleEnumerationValues() {
		// prtln ("getSimpleEnumerationValues() for " +
		// this.getQualifiedInstanceName());
