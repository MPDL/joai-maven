...
public class ComplexType extends SimpleType {
	...
	// contentModel holds the "name" of the first element in the ComplexType
	// definition
	 ...
	 *
	 * @param element
	 *            Schema element defining type
	 * @param location
	 *            File in which type definition exists
	 * @param namespace
	 *            NOT YET DOCUMENTED
	 * @param schemaReader
	 *            reader instance that processed this definition
	 */
	...
	Element deriveExtensionModelElement() {
		...
		try {
			...
			prtln("\nroot model Element (" + NamespaceRegistry.nsToString(root.getNamespace()) + "): "
					+ pp(modelElement));
	...
	/**
	 * Returns true if this complexType defines a derived text only model (i.e., it
	 * has a "simpleContent" element as first child.
	 *
	 ...
	 *
	 * @return The base extension type name if this ComplexType has simpleContent,
	 *         null otherwise.
	 */
