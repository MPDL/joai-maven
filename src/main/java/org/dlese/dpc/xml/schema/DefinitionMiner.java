...
public class DefinitionMiner {
	...
	/**
	 * Returns true if the unqualified names in this schema file should be converted
	 * to the namedDefault (prefix = "this") namespace, addressing the problem that
	 * we cannot process multi-namespace documents having a "default" namespace
	 * (i.e., with no prefix).
	 * <p>
	 *
	 ...
	 * namespace was some OTHER namespace.
	 * <p>
	 *
	...
	private boolean schemaDocNeedsConverting() {
		...
		// prtln("defaultNS: " +
		// NamespaceRegistry.nsToString(namespaces.getDefaultNamespace()));
	 ...
	 *
	 * @param name
	 *            The feature to be added to the GlobalDef attribute
	 * @param globalDef
	 *            The feature to be added to the GlobalDef attribute
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 ...
	 *
	 * @param uri
	 *            The feature to be added to the SchemaFile attribute
	 * @param reader
	 *            The feature to be added to the SchemaReader attribute
	 */
	 ...
	 *                Description of the Exception
	 */
	private Document getParsedDoc(URI uri) throws SchemaHelperException {
		Document doc = null;
		if (parsedDocs.containsKey(uri.toString())) {
			...
			if (uri.getScheme().equals("file")) {
				...
				/*
				 * catch (WebServiceClientException she) { throw new
				 * SchemaHelperException(she.getMessage()); }
				 */
			}
	...
	private String ambiguousRootElementMsg(TreeMap candidates) {
		...
		msg += NL + "framework config must specify one of the following global " + "elements as the rootElement:";
	 ...
	 *
	 * @param uri
	 *            the particular file to be processed
	 * @param defaultTargetNamespaceURI
	 *            NOT YET DOCUMENTED
	 * @exception SchemaHelperException
	 *                NOT YET DOCUMENTED
	 */
	protected void processSchemaFile(URI uri, String defaultTargetNamespaceURI) throws SchemaHelperException {
		...
		// addSchemaReader before actually reading to prevent infinite loop with
		// mutually refering schema files
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 1) {
			...
			if (def != null) {
				/*
				 * switch (def.getDataType()) { case GlobalDef.SIMPLE_TYPE: def =
				 * (SimpleType)def; break; case GlobalDef.COMPLEX_TYPE: def = (ComplexType)def;
				 * break; default: System.out.println ("found XSD Node was of unhandled type: "
				 * + def.getDataType()); break; }
				 */
