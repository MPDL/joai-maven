...
/**
 * SchemaReader traverses an XML schema, which may be represented using multiple
 * files, and extracts the key schema elements (including Data Type definitions
 * and global elements). These key schema elements are represented as GlobalDefs
 * and stored in a {@link GlobalDefMap} which is keyed by element name.
 * 
 *
 * @author ostwald
 */
public class SchemaReader {
	 ...
	 */
	public SchemaReader(Document doc, URI source, DefinitionMiner definitionMiner, String defaultTargetNamespaceURI)
	...
	private void setTargetNamespace(String defaultTargetNamespaceURI) {
		...
		// if the targetNamespace has not been registered, then create it (as the
		// default) and update
	...
	public boolean isBuiltIn(String qualifiedTypeName) {
		if (NamespaceRegistry.getNamespacePrefix(qualifiedTypeName).equals("xml"))
			return true;
		return (NamespaceRegistry.isQualified(qualifiedTypeName) && NamespaceRegistry
				.getNamespacePrefix(qualifiedTypeName).equals(namespaces.getSchemaNamespace().getPrefix()));
	}
	 ...
	 * determined.
	 * 
	 * NOTE: this can be refactored to use getInstanceQualifiedName (returned prefix
	 * must be resolved into a namespace before the call to globalDefMap.getValue
	 * (baseName, namespace)
	 */
	...
	private void getTypeDefs(String typeSpec) throws Exception {
		...
		for (Iterator iter = list.iterator(); iter.hasNext();) {
			...
			try {
				...
				/*
				 * prtlnErr ("\tReason: " + t.getMessage()); t.printStackTrace();
				 */
			}
		}
	}
