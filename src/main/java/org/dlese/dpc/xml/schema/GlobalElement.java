...
/**
 * Wrapper for global element definitions in an XML Schema, which are defined as
 * an immediate child of the schema element.
 * <p>
 *
 * Global element definitions have an XPath of the form
 * "/&lt;xsd:schema/&lt;xsd:element ...>", where the "xsd" prefix refers to a
 * prefix that is mapped to the W3C Schema for Datatype Definitions (having a
 * namespace uri of "http://www.w3.org/2001/XMLSchema").
 * <p>
 *
 * GlobalElement definitions are associated with a type in one of the following
 * ways:
 * <ul>
...
public class GlobalElement extends GlobalDeclaration {
	...
	/**
	 * NOTE: substitutionGroupMembers must have the same type as head or be derived
	 * from head's type
	 */
	...
	public String toString() {
		...
		String nl = "\n\t";
		/*
		 * s += nl + "type: " + type; s += nl + "location: " + getLocation(); if
		 * (getNamespace() != null && getNamespace() != Namespace.NO_NAMESPACE) s += nl
		 * + "namespace: " + getNamespace().getPrefix() + ": " +
