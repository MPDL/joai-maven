 ...
 * is required, and its xpath. SchemaNodes are stored in a
 * {@link org.dlese.dpc.xml.schema.SchemaNodeMap} that is keyed by XPaths.
 *
 * @author ostwald
 *         <p>
 *
...
public class SchemaNode {
	...
	/**
	 * Constructor for the SchemaNode object used for Schema elements that refer to
	 * Global Elements, such as attributes and elements
	 *
	 * @param typeName
	 *            data type of the referred-to element
	 * @param e
	 *            schema element that refers to a global element
	 * @param xpath
	...
	/**
	 * NOT USED!! Only override existing attributes if the particular attribute is
	 * explicitly defined in the donor element.
	 */
	...
	/**
	 * SubstitutionElement is the instance Element, complete with children, for this
	 * schemaNode. Since this is a substitution element, we don't want to recompute
	 * it. NOTE: why do individual schemaNodes have to store this element? why isn't
	 * it just stored ONCE (i.e., in the headElement?)
	 */
	...
	/**
	 * gets names of the attributes of the schema element (NOT the instance
	 * element!)
	 */
	...
	/**
	 * Gets the xpath for this SchemaNode in the context of the
	 * SchemaHelper.instanceDocument.
	 *
	...
	/**
	 * Finds a validating type for ComplexTypes defining derived Models. For these
	 * cases, the validatingType is found by recursively traversing the type
	 * definition tree until a Simple or Built-in data type is located for the
	 * extension element.
	 *
	 * @param globalDef
	 *            Type definnition for which we want to find the validating type
	...
	private GlobalDef findValidatingType(GlobalDef typeDef) {
		...
		if (typeDef.isComplexType()) {
			/*
			 * we are working with a ComplexType. If it has an exension type (i.e., if it is
			 * a derived model) then we must determine the validating type for the extension
			 * element.
			 */
	...
	public boolean hasSequenceCompositorEXPERIMENTAL() {
		...
		if (isDerivedModel()) {
			...
			/*
			 * try { ComplexType typeDef = (ComplexType)getTypeDef(); ComplexType
			 * extensionRootType = (ComplexType) typeDef.getExtensionRootType(); prtln
			 * ("\t ... extensionRootType: " + extensionRootType.getName()); return
			 * (extensionRootType.getCompositorType() == Compositor.SEQUENCE); } catch
			 * (Throwable t) { prtln ("hasSequenceCompositor: " + t); t.printStackTrace(); }
			 */
		}
		return false;
	}
	...
	private void setIsChoiceMember() {
		...
		if (!parentNode.hasChoiceCompositor()) {
			// prtln ("\t parentNode does not have a choice compositor - isChoiceMember is
			// false");
			isChoiceMember = false;
			return;
		}
	 ...
	 */
	/*
	 * public static void prtln(String s) { if (debug)
