...
public class DocMap {
	 ...
	 * @return text of the node referred to by the xpath or an empty string if the
	 ...
	 * If there is no node at the specified path, a new one is created.
	 * <p>
	 *
	 * ISSUE: what if there is no value (one way this happens is when there is a
	 * field in the form but no value is supplied by user. In this case we shouldn't
	 * bother creating a new node because there is no value to insert. But what if
	 * there was formerly a value and the user has deleted it? if the node is an
	 * Element, then should that element be deleted? (The user should be warned
	 * first!) if the node is an attribute, then should that attribute be deleted?
	 * (the user won't be warned, since this does not have the "ripple" effect that
	 * deleting an element can have. (maybe the user should be warned only if the
	 * element has children with values).
	 *
	 * @param key
	 *            unencoded xpath
	 * @param val
	 *            value to assign to node at xpath
	 */
	public void put(Object key, Object val) {
		...
		/*
		 * String trimmed_val = ""; if (val != null) trimmed_val = ((String)
		 * val).trim(); node.setText(trimmed_val);
		 */
		node.setText(val != null ? (String) val : "");
	}

	/**
	 * Tests xpath against provided schema (via SchemaHelper) before putting value,
	 * creating a new Node if one is not found at xpath.
	 *
	 * @param xpath
	 *            NOT YET DOCUMENTED
	 * @param value
	...
	public boolean hasChildren(String xpath) {
		...
		/*
		 * We DO want to check in the case of an element that can contain text which
		 * ALSO has attributes. So we can do this check IF the typeDef is the right kind
		 * ...
		 */
	 ...
	 * parent node. One way to do this would be to clear the contents of the parent
	 * node, but i'm not sure this is safe. so instead we will find all nodes
	 * matching the "deindexed" xpath and then remove them individually.
	 *
	 * @param xpath
	 *            xpath to an enumeration node
	 * @return true if successful
	 * @exception Exception
	 *                if unable to remove siblings
	 */
	public boolean removeSiblings(String xpath) throws Exception {
		Node node = doc.selectSingleNode(xpath);
		if (node == null) {
	 ...
	 * @return the total number of siblings with the same element name (including
	 *         the node specified by the xpath)
	 */
	...
	private Element newElement(String encodedXPath) {
		...
		if (parent == null) {
			// prtln("newElement() failed to find parent node: " + parentPath + " ...
			// creating ... ");
	...
	/**
	 * Called from metadata controller to add an element at a specific place in the
	 * document. The xpath specified by encodedReferencePath provides an insertion
	 * point for the new element. Namely, we want to insert the new element
	 * immdediately following the encodedReferencePath.
	 *
	 * @param e
	 *            The feature to be added to the Element attribute
	 * @param encodedReferencePath
	 *            The feature to be added to the Element attribute
	...
	public boolean addElement(Element e, String encodedReferencePath) {
		...
		Element parent = (Element) selectSingleNode(parentPath);
		if (parent == null) {
			// prtlnErr ("addElement() failed to find parent node: " + parentPath + " ...
			// creating ... ");
			parent = newElement(parentPath);
			if (parent == null) {
	 ...
	 * information (insertion point is immediatlely following the node identified by
	 * targetPath).
	 *
	 * @param element
	 *            element to be inserted
	 * @param parent
	 *            parent in which to insert element
	 * @param targetPath
	...
	public void insertElement(Element element, Element parent, String targetPath) throws Exception {
		// prtln("insertElement() child: " + element.getName() + " parent: " +
		// parent.getName() + "\n\ttargetPath: " + targetPath);

		int targetIndex = XPathUtils.getIndex(targetPath);

		// if index is 0, or if the parent is empty, then we have to find insertion
		// point using the compositor!
	...
	/**
	 * Insert an element in it's parent element using compositor. The compositor can
	 * provide information that allows us to place the new element at the correct
	...
	private void untargetedInsert(Element element, Element parent) throws Exception {
		...
		if (schemaHelper == null || !schemaHelper.hasSequenceCompositor(parentPath)) {
			 ...
			 * of the following member if no like-named elements exist. when the
			 * childMemberIndex is greater than the (target) memberIndex, then it is time to
			 * insert.
			 */
	...
	/**
	 * Insert an element into the parent immediately following the targetIndex. For
	 * example, if a parent element had three children elements, and the targetIndex
	 * were 2, then the new element would be inserted after the second element and
	 * before the third. NOTE: throws exception if targetIndex is greater than one
	 * and an element corresponding to targetIndex does not exist.
	 *
	 * @param element
	 *            element to be inserted
	 * @param parent
	 *            parent in which to be inserted
	 * @param targetIndex
	 *            index after which to be inserted
	 * @exception Exception
	 *                if targetIndex is greater than one and an element
	...
	private void targetedInsert(Element element, Element parent, int targetIndex) throws Exception {
		...
		// when the editor needs to create a new repeating node for an empty parent, it
		// supplies an index of 1 ...
		if (targetIndex == 1) {
			untargetedInsert(element, parent);
		} else {
	 ...
	 * @exception Exception
	 *                if a new node cannot be created
	 */
	 ...
	 * @exception Exception
	 *                if unable to create a new node at xpath
	 */
	public Node createNewNode(String xpath) throws Exception {
		...
		// sanity check: parent must be an element (cause attributes can't have
		// children)
