...
public class SchemaUtils {
	...
	public static void showDerivedDataTypes(SchemaHelper schemaHelper) {
		...
		while (i.hasNext()) {
			...
			/*
			 * s += NT + "key: " + key + " getName(): " + def.getName() + " displayName(): "
			 * + def.displayName(); s += "  getNamespaceUri(): " + def.getNamespaceUri();
			 */
			s += NT + cnt++ + ": " + key;
		}
		...
		for (Iterator i = schemaNodeMap.getKeys().iterator(); i.hasNext();) {
			...
			if (nodeDocs != null || typeDocs != null) {
				...
				if (typeDocs != null)
					s += NL + "TYPE: " + typeDocs;
				/*
				 * if (nodeDocs == null) s += NT + " no Node documentation"; else s += NL +
				 * "NODE: " + nodeDocs; if (typeDocs == null) s += NT +
				 * " no Type documentation"; else s += NL + "TYPE: " + typeDocs;
				 */
			} else {
				// s += NL + key;
			}
	 ...
	 *
	 * @param schemaHelper
	 *            schemaHelper instance for particular framework.
	 */
	public static void showNodesHavingRepeatingComplexSingletons(SchemaHelper schemaHelper) {
		...
		for (Iterator i = schemaNodeMap.getKeys().iterator(); i.hasNext();) {
			...
			// s += NL + "xpath: " + key;
			SchemaNode schemaNode = (SchemaNode) schemaNodeMap.getValue(key);
			if (schemaNode != null && schemaNode.getTypeDef().isComplexType()
	...
	public static void showChoiceNodes(SchemaHelper schemaHelper) {
		...
		for (Iterator i = choicePaths.iterator(); i.hasNext();) {
			...
			s += NL + schemaNode.getXpath() + " min: " + choice.getMinOccurs() + "   max: " + choice.getMaxOccurs();
		}
	...
	public static void showXSDStringExtensionFields(SchemaHelper schemaHelper) {
		SchemaNodeMap schemaNodeMap = schemaHelper.getSchemaNodeMap();
		String s = "";
		String stringType = NamespaceRegistry.makeQualifiedName(schemaHelper.getSchemaNamespace().getPrefix(),
				"string");
		for (Iterator i = schemaNodeMap.getKeys().iterator(); i.hasNext();) {
			String key = (String) i.next();
			SchemaNode schemaNode = (SchemaNode) schemaNodeMap.getValue(key);
			if (schemaNode.isDerivedModel() && schemaNode.getValidatingType().getName().equals(stringType)) {
				s += NL + key + " (" + schemaNode.getTypeDef().getQualifiedName() + ")";
			}
		}
	...
	/**
	 * show fields of xxx:string dataType, where xxx refers to the namespace prefix
	 * for the schemaNamespace.
	 *
	 * @param schemaHelper
	 *            NOT YET DOCUMENTED
	 */
	...
	public static void showComboUnionFields(SchemaHelper schemaHelper) {
		...
		for (Iterator i = globalDefMap.getValues().iterator(); i.hasNext();) {
			...
			// prtln ("name: " + def.getName() + " dataType: " + def.getDataType() + "
			...
			if (schemaHelper.isComboUnionType(def)) {
				...
				// for (Iterator v=schemaHelper.getEnumerationValues(def.getName(),
				// false).iterator();v.hasNext();) {
	...
	/**
	 * Based on logic of RendererImpl.renderRepeatingElement, prints out the name of
	 * the class instantiated to render provided schemaNode.
	 * <p>
	 *
