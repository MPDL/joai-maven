 ...
 * <p>
 *
 * Compositor Members are created at schema-processing time, but used to provide
 * run-time services. For example, at run time, the compositor is used, along
 * with the CompositorGuard instance, to determine if a child element may be
 * added to an existing instance document. In order to serve run-time purposes,
 * the Compositor members must be accessible via qualified element names as they
 * appear in the instance document. The instance-level namespace prefix might be
 * different from that associated with the compositor members in the schema file
...
public class CompositorMember {
	 ...
	 *
	 * @param element
	 *            Schema Element representing this member
	 * @param compositor
	 *            the compositor instance of which this is a member
	 */
	...
	public CompositorMember(Element element, Compositor compositor) {
		...
		if (memberElementName.equals("element")) {
			...
			name = NamespaceRegistry.makeQualifiedName(schemaReader.getNamespaces().getSchemaNamespace(), "any");

			/*
			 * // debugging prtln ("\n*** this CMType is any (" + name + ")"); prtln
			 * ("element: " + this.element.asXML()); prtln ("--------\n");
			 */
			cmType = ANY;
		...
		} else if (memberElementName.equals("all") || memberElementName.equals("choice")
	 ...
	 * <p>
	 *
	 * This constructor is used to create CompositorMembers from the GlobalElements
	 * stored as substitutionGroup of a headElement (see
	 * Compositor.getSubstitionGroupMembers).
	 * <p>
	 *
	 * <p>
	 *
	 ...
	 * GlobalElements. Otherwise return an empty List.
	 * <p>
	 *
	 * NOTE: evaluation of substitutionGroupMembers is lazy - it must be performed
	 * AFTER all the GlobalDefs in the schema have been created, otherwise there is
	 * a chance that the globalElement, if one exists, will not be found in the
	 * GlobalDefMap.
	 *
	...
	public List getSubstitutionGroupMembers() {
		if (substitutionGroupMembers == null) {
			// prtln ("\n getSubstitutionGroupMembers for " +
			// this.getInstanceQualifiedName());
			...
			if (globalElement == null) {
				...
				/*
				 * if (globalDef == null) prtln ("\t globalDef not found for " + name); else
			...
			/*
			 * if (globalElement != null) { prtln ("found globalElement\n" +
	...
	/**
	 * Gets the instanceQualifiedNames of the substitutionGroupMembers, if this
	 * Member is a headElement.
	 *
	 * @return InstanceQualifiedNames of this Members substitutionGroupMembers
	 */
	public List getSubstitutionGroupMemberNames() {
		// prtln ("getSubstitutionGroupMemberNames for " +
		// this.getInstanceQualifiedName());
	...
	/**
	 * Is this member a headElement (having a substitutionGroup that specifies
	 * element names that can be subsituted for this member.
	 *
	...
	/**
	 * Returns this Member's inline compositor (Choice, Sequence, All) if there is
	 * one.
	 *
	...
	public CompositorGuard getParentCompositorGuard() {
		...
		if (parentCompositorGuard == null) {
			...
			/*
			 * prtln ("\t parentCompositorGuard not found"); prtln
			 * ("\t compositorGuard element: " + getElement().asXML()); / construct
			 * parentCompositorGuard - the compositorGuard in the / parentCompositor that
			 * manages the instanceList elements.
			 */
		}
	...
	/*
	 * Get the qualified name of this member as it appears in an instance document,
	 * i.e., having the proper namespace prefix according to the namespaces as
