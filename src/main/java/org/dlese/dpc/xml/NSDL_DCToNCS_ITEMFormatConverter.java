...
/**
 * Converts from NSDL_DC format to the NCS_ITEM format. NOTE: assumes nsdl_dc is
 * version 1.02 and first transforms to nsdl_dc 1.03 before converting to
 * ncs_item.
 *
 * @author Ostwald
 * @version $Id: NSDL_DCToNCS_ITEMFormatConverter.java,v 1.3 2009/03/20 23:34:01
 *          jweather Exp $
...
public class NSDL_DCToNCS_ITEMFormatConverter implements XMLDocumentFormatConverter {
	...
	/**
	 * Performs XML conversion from ADN to oai_dc format. Characters are encoded as
	 * UTF-8.
	 *
	 * @param xml
	 *            XML input in the 'adn' format.
	 * @param docReader
	 *            A lucene doc reader for this record.
	 * @param context
	...
	public String convertXML(String xml, XMLDocReader docReader, ServletContext context) {
		...
		try {
			...
			Transformer format_transformer = XSLTransformer.getTransformer(format_transform_file.getAbsolutePath(),
					xsl2Transformer);

			/*
			 * when working with nsdl-dc-v1.03 source we had to first transform to
			 * nsdl-dc-v1.02 before applying format transform to ncs-item.
			 */
			// String transformed_content = XSLTransformer.transformString(xml,
			// version_transformer);
			// transformed_content = XSLTransformer.transformString(transformed_content,
			// format_transformer);

			/*
			 * when working with nsdl-dc-v1.02-020 source - no nsdl-dc version transform is
			 * required
			 */
