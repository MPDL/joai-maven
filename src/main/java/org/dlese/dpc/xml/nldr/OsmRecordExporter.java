...
public class OsmRecordExporter extends NldrMetadataRecordExporter {
	...
	static List EMBARGO_TYPES = Arrays.asList(new String[]{"Author embargo", "Donor embargo", "Publisher embargo"});
	...
	public static String getExportXml(String xml, String collection, String repositoryUrl) throws Exception {
		...
		for (int i = 0; i < allAssetNodes.size(); i++) {
			...
			try {
				...
				String citableUrl = (rec.isEmbargoed() ? "" : urlOfRecord + "/" + new File(physicalUrl).getName());
	...
	boolean isEmbargoed() {
		...
		for (Iterator i = drEls.iterator(); i.hasNext();) {
			...
			if (this.EMBARGO_TYPES.contains(dateRange.type) && dateRange.getStartDate().before(now)
	...
	/*
	 * static void batchParseDate () { String[] testCases = { "", " ", "2005",
	 * "2005-01", "2005-01-30", "2005-01-30T01:01:01Z", "2005-01-30T23:01:01z",
	 * "2005-13", "2005-01-33", "2005-01-30T00:01:01Z", "2005-01-30T24:01:01z",
	 * "2005-01-30T23:99:01z", "2005-01-30T23:59:60z" };
	 * 
	 * for (int i = 0; i < testCases.length; i++) { String dateString =
	 * testCases[i]; parseDateString (dateString); } }
	 * 
	 * static void parseDateString (String dateStr) { try { Date parsed =
	 * MetadataUtils.parseUnionDateType(dateStr); prtln ("parsed: " +
	 * parsed.toGMTString()); } catch (Exception e) { prtln
	 * ("couldn't parse dateString: \"" + dateStr + "\""); } }
	 */
