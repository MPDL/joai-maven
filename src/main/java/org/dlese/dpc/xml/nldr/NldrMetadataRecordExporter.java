 ...
 * citableUrls"). NLDR metadata records may involve mulitple-namespaces and may
 * conttain "assets" (primary content in the NLDR).
 *
...
public abstract class NldrMetadataRecordExporter {
	 ...
	 * <dd>/*[local-name()='record']/*[local-name()='relation'][@type='Has part']
	 * </dl>
	 *
	 *
	 * @param s
	 *            xpath as a qualifed string
	...
	public static String makeXPath(String s) {
		...
		for (int i = 1; i < splits.length; i++) {
			...
			if (segment.startsWith("@")) {
				...
				/*
				 * prtln ("elementName: " + elementName); prtln ("attrValueSpecifier: " +
				 * attrValueSpecifier);
				 */
	...
	public String getXmlStripped() {
		if (recordNode == null) {
			...
			try {
				StringBuffer xml = XMLConversionService
						.stripXmlDeclaration(new BufferedReader(new StringReader(recordNode.asXML())));
				return xml.toString();
			} catch (Throwable t) {
				return "";
			}
