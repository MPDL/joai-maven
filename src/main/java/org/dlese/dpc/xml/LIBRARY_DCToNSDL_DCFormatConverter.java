...
public class LIBRARY_DCToNSDL_DCFormatConverter implements XMLFormatConverter {
	...
	public static void main(String[] args) throws Exception {
		prtln("\n===============================================");
		/*
		 * prtln ("TransformTester"); String xslt = "library_dc-v1.1-to-oai_dc.xsl";
		 * String xform_path = xsl_dir + "/" + xslt; File file = new File(xform_path);
		 * if (!file.exists()) throw new Exception ("transform does not exist at: " +
		 * xform_path);
		 */
	...
	public String convertXML(String xml, ServletContext context) {
		...
		try {
			Transformer format_transformer = XSLTransformer
					.getTransformer(library_dc_to_oai_dc_transform_file.getAbsolutePath());
