...
/**
 * Converts from ADN format to the NSDL DC format. Converts DLESE-specific IDs
 * to URLs.
 *
...
public class ADNToNSDLDCFormatConverter implements XMLDocumentFormatConverter {
	...
	// Fields in NSDL_DC format that may contain IDs that should be replaced with
	// URLs
	private final static String[] fieldsToReplace = {"dct:isVersionOf", "dct:hasVersion", "dct:isReplacedBy",
			"dct:replaces", "dct:isRequiredBy", "dct:requires", "dct:isPartOf", "dct:hasPart", "dct:isReferencedBy",
			"dct:references", "dct:isFormatOf", "dct:conformsTo"};
	...
	/**
	 * Performs XML conversion from ADN to nsdl_dc format. Characters are encoded as
	 * UTF-8.
	 *
	 * @param xml
	...
	private void handleRelationField(String xPathToField, Document xmlDocument) throws Throwable {
		...
		if (nodes != null) {
			for (int i = 0; i < nodes.size(); i++) {
				...
				if (!relation.toLowerCase().matches(".*http\\:\\/\\/.*|.*ftp\\:\\/\\/.*")) {
					ResultDocList results = index.searchDocs("id:" + SimpleLuceneIndex.encodeToTerm(relation));
