...
public class XPathUtils {
	 ...
	 *
	 * @param s
	 *            path containing el-encoded brackets
	 ...
	 *
	 * @param s
	 *            xpath possibly containing index notation
	 * @return equivalent XPath
	 */
	...
	/**
	 * Returns the name of the node referred to by the (possibly jsp-encoded) xpath,
	 * stripped of indexing information.
	 *
	 * @param xpath
	 *            NOT YET DOCUMENTED
	 ...
	 * Element) of a given xpath. Removes indexing information only on the terminal
	 * element (leaf), NOT on the rest of the path (as
	 * {@link #normalizeXPath (String)} does).
	 *
	 * @param xpath
	 *            XPath as String
	 ...
	 * ordering except for when xpath indexing is involved, e.g.,
	 * "/record/general[3]/foo[5]" is "less than" "/record/general[4]/foo[6]".
	 * <p>
	 *
	 ...
	 * @return 0 if the two paths are equal, -1 if path1 is "less than" path 2, 1
	 *         otherwise
	 */
	public static int compare(String path1, String path2) {
		...
		if (splits1.length == 1 && splits2.length == 1) {
			// compare path segments. we know they aren't equal, but they might differ only
			// by index
			...
			if (sib1.equals(sib2)) {
				return Integer.valueOf(XPathUtils.getIndex(path1))
						.compareTo(Integer.valueOf(XPathUtils.getIndex(path2)));
			}
			return -1;
		} else {
