...
/**
 * Subclassed from JDOM's <code>XMLOutputter</code>, in order to tweak output.
 * JDOM documentation should be consulted for actual details.
 *
...
public class XMLWriter extends XMLOutputter {
	...
	protected void printElementContent(Element element, Writer out, int indentLevel, NamespaceStack namespaces,
		...
		if (stringOnly) {
			...
			while (itr.hasNext()) {
				...
				if (content instanceof String) {
					...
					if (justOutput == CDATA.class && textNormalize && startsWithWhite(scontent)) {
						out.write(" ");
					}
					...
					if (justOutput == String.class && textNormalize && endedWithWhite) {
						out.write(" "); // padding
					}
			...
			while (itr.hasNext()) {
				...
				if (content instanceof Comment) {
					...
					if (!isWhitespace((String) content)) {
						...
						if (justOutput == CDATA.class && textNormalize && startsWithWhite(scontent)) {
							out.write(" ");
						} else if (justOutput != CDATA.class && justOutput != String.class) {
							// printString("@", out);
							maybePrintln(out);
							indent(out, indentLevel);
						}
					...
					if (justOutput == String.class && textNormalize && endedWithWhite) {
						out.write(" "); // padding
					} else if (justOutput != String.class && justOutput != CDATA.class) {
						maybePrintln(out);
						indent(out, indentLevel);
					}
					printCDATA((CDATA) content, out);
	...
	/*
	 * void printElements(Element element, int indent) { // Print out elements,
	 * starting with no indention try { Iterator i =
	 * element.getContent().iterator(); while (i.hasNext()) { for (int j=0;
	 * j<indent; j++) { System.out.print(" "); } Object obj = i.next(); if (obj
	 * instanceof Element) { System.out.println(obj.toString());
	 * printElements((Element)obj, indent + 5); } else if (obj instanceof String) {
	 * //String tmp = ((String)obj).trim(); //if (tmp.length() > 0) {
	 * System.out.println("[String: " + (String)obj + "]"); //} } else if (obj
	 * instanceof Comment) { System.out.println(obj.toString()); } else {
	 * System.out.println("Object class = " + obj.getClass()); }
	 * 
	 * // // 0 is indentation // // printElement(doc.getRootElement(), writer, 0);
	 * // } else if (obj instanceof Comment) { // maybePrintln(writer); //
	 * writer.print(((Comment)obj).getSerializedForm()); // } } } catch
	 * (NoSuchElementException e) { // No elements to print } }
	 */
}
