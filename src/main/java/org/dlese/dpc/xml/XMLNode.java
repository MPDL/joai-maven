...
/**
 * Creates an object wrapper for JDOM XML elements and for accessing their
 * content. The primary purpose of this class is to provide object mapping
 * between the JDOM tree hierarchy and the HashMap maintained by
 * {@link XMLRecord}. Child and parent relationships in the XML element
 * hierarchy are mirrored by the <code>XMLNode</code> structure.
 * <p>
 * The path strings created and maintained by this class reflect the path that
 * would be traversed through the JDOM tree hierarchy to reach the wrapped
 * element. Path strings are extended as nodes are added, by appending the
 * element's name and its number of occurrence. The path strings then serve as
 * keys to the map maintained by <code>XMLRecord</code>.
 * <p>
 ...
 * technical requirement of the root element record. Here is the corresponding
 * XML:
 * 
 * <pre>
 *	&lt;record&gt;
 ...
 * </pre>
 * <p>
 * By knowing the path structure, a calling method may ask
 * <code>XMLRecord</code> for any node anywhere in the hierarchy. The node then
 * provides the caller with convenient access methods to the element and its
 * attributes.
 * 
...
public class XMLNode {
	 ...
	 * This is not the same as its index.
	 */
	...
	/**
	 * Retrieve the node's parent node, which is null if this node wraps the root
	 * element.
	 * 
	...
	public void addChild(XMLNode node) {
		...
		// System.err.println("Adding node " + node.getName() + " to path " + path + "
		// at index " + place);
	}

	/**
	 * Gets the child node that lives at a specific index of this node's children
	 * array.
	 * <p>
	 ...
	 * This is useful when retrieving multiple occurring elements.
	 * 
	 * @param nodeName
	...
	/**
	 * Finds the place where a new node should be inserted. JDOM does not provide
	 * for inserting elements at a given location, so this helps keep track of a
	 * node's location, for constructing the key paths and for inserting like
	...
	/**
	 * Returns the path which maps this node's element location in the JDOM tree
	 * hierarchy.
	 * 
	 ...
	 * 
	 * @param parentPath
	 *            The path of the node which is or will be this node's parent.
	 */
	...
	/**
	 * Adds a comment to the element of this node. If other comments already exist
	 * then this one is added after the other comments.
	 * 
	 * @param value
	 *            String comment to be added.
	 */
	...
	/**
	 * Gets an identified comment from the element of this node, if it exists. If a
	 * comment has been added with an identifier, this method will browse the
	 * available comments for a match. The identifier can exist anywhere in the
	 * comment.
	 * 
	 * @param identifier
	...
	/**
	 * Removes the first identified comment from the element of this node, if one
	 * exists. A comment may be added with an identifier, and this method will
	 * browse the available comments for a matching identifier. The identifier can
	 * exist anywhere in the comment. Only the first occurrence is removed.
	 * 
	 * @param identifier
	 *            String to match as an identifier.
	...
	/**
	 * Tests whether this node wraps a leaf element, which is useful for knowing how
	 * to add content. (IMS and DLESE DTDs do not allow text to be added to non-leaf
	 * elements.
	 * 
	 ...
	 * <p>
	 * NOTE: This implementation is built on JDOM 4, which provides weak access for
	 * reading and writing element content. JDOM 5 has deprecated this approach, and
	 * JDOM 6 is even more flexible and robust. Future implementations of this class
	 * should reflect the updates in JDOM.
	 * 
	 * @return The text content of this node's element, or an empty string if there
	 *         is no text content.
	 */
	...
	/**
	 * Sets the textual content of the element of this node, if it is a leaf
	 * element. (The requirement to be a leaf element is an IMS and DLESE DTD
	 * requirement, which a more general implementation should not restrict.)
	 * <p>
	 * NOTE: This implementation is built on JDOM 4, which provides weak access for
	 * reading and writing element content. JDOM 5 has deprecated this approach, and
	 * JDOM 6 is even more flexible and robust. Future implementations of this class
	 ...
	 * 
	 * @param value
	 *            The text to set as content for the element of this node.
	 */
	...
	/**
	 * Retrieves a named attribute from the element of this node, if the attribute
	 * exists.
	 * 
	 * @param attName
	...
	public String getAttribute(String attName) {
		try {
			...
			// System.err.println("Error: retrieving " + attName + " info for " +
			// element.getName());
		}
		return null;
	}
	 ...
	 * children.lastIndexOf(node); children.remove(index); }
	 * 
	 * public Element getElementCopy() { Element e = new Element(element.getName(),
	 * element.getNamespace()); e.setAttributes(element.getAttributes());
	 * e.setMixedContent(element.getMixedContent());
	 * //System.err.println("parent = " + e.getParent()); return e; }
	 */
