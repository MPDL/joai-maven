...
/**
 * XMLValidator uses Xerces to parse and validate xml files in a directory or an
 * individual xml file you specify. Each xml file should contain a DOCTYPE
...
public class XMLValidator {
	 ...
	 *
	 * @param filePath
	 *            A single XML file or directory contianing XML files to be
	 *            validated.
	...
	/**
	 * Validate a single XML file or directory of XML files, processing only the xml
	 * files that match the ID list provided. Same as calling validate(String,
	 * String [], null).
	 *
	 * @param filePath
	 *            A single XML file or directory contianing XML files to be
	 ...
	 *            An array of all the ID names found in the directory indicated in
	 *            filePath to be validated. If null, then the list of files is
	 *            obtained from the filePath directory.
	 * @return StringBuffer A StringBuffer containing the validation report.
	 * @exception FileNotFoundException
	 *                DESCRIPTION
	 * @exception IOException
	 ...
	 *            An array of all the ID names found in the directory indicated in
	 *            filePath to be validated. If null, then the list of files is
	 *            obtained from the filePath directory. the file list is obtained
	 * @param reportFilePath
	 *            The file where a validation report will be appended. If null then
	 *            nothing is written.
	 * @return StringBuffer A StringBuffer containing the validation report.
	 * @exception Exception
	 *                DESCRIPTION
	 */
	public StringBuffer validate(String filePath, String[] inputFileNames, String reportFilePath,
			Date modifiedSinceDate) throws Exception {
		...
		/*
		 * Enumeration e = System.getProperties().propertyNames(); while
		...
		if (dir.isFile()) {
			...
			// .xml files only
			// Process only the files in the directory that match the inputFileNames
			// provided
			else
			...
			for (int i = 0; i < files.length; i++) {
				if (modifiedSinceDate != null
						&& (new File(dirName, files[i]).lastModified() < modifiedSinceDate.getTime()))
					numUpToDateFiles++;
				else
		...
		/*
		 * if (numWarningFiles == 1) buff.append( numWarningFiles +
		 * " file has a warning.\n"); else buff.append(numWarningFiles +
		 * " files have warnings.\n");
		 */
	...
	/**
	 * Validates an XML String to the external DTD or schema that is present in the
	 * String. The String must contain a DTD or schema declaration within. Returns
	 * null if the String is valid or an appropriate error message if not.
	 *
	 * @param s
	 ...
	 * Validates an XML String to the external DTD or schema that is present in the
	 * String. The String must contain a DTD or schema declaration within. Returns
	 * null if the String is valid or an appropriate error message if not. The
	 ...
	 * @return Null if valid, else a String containing an appropriate error and/or
	 *         warning message.
	 */
	 ...
	 * <ul>
	 * <li><code>file:///usr/local/nonesuch.xml</code> - an XML document on the
	 * local system.
	 * <li><code>http://www.nonesuch.com/nonsuch.xml</code> - an XML document on the
	 * Internet.
	 * </ul>
	 *
	 *
	 ...
	 * <li><code>file:///usr/local/nonesuch.xml</code> - an XML document on the
	 * local system.
	 * <li><code>http:///www.nonesuch.com/nonsuch.xml</code> - an XML document on
	 * the Internet.
	 * </ul>
	 *
	 *
	 ...
	 * File. The File must contain a DTD or schema declaration within. Returns null
	 * if the File is valid or an appropriate error message if not.
	 *
	 * @param f
	 ...
	 * Validates an XML File to the external DTD or schema that is present in the
	 * File. The File must contain a DTD or schema declaration within. Returns null
	 * if the File is valid or an appropriate error message if not. The output
	 * message may contain only warnings if showWarnings is set to true.
	 *
	 * @param f
	 *            An XML File, which must include a DTD or schema declaration.
	 * @param showWarnings
	 ...
	 * @return DESCRIPTION
	 */
	private final static boolean doValidate(InputSource input, String source, StringBuffer messages,
			boolean showWarnings) {
		boolean isValid = true;
		try {
			// Set systemID so parser can find the dtd with a relative URL in the source
			// document.
	...
	private void parseAndValidate(String dir, String filename) {
		try {
			...
			/*
			 * if (handler.warning && !handler.containsDTD) // warning { buff.append
			 * ("WARNING: " + filename + "\n"); buff.append(warningBuff.toString());
			 * numWarningFiles++; }
			 */
		} catch (Exception e) {
