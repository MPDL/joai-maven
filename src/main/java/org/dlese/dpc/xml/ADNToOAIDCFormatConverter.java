...
/**
 * Converts from ADN format to the OAI DC format. Converts DLESE-specific IDs to
 * URLs.
 *
		...
		if (nodes != null) {
			for (int i = 0; i < nodes.size(); i++) {
				...
				if (!relation.toLowerCase().matches(".*http\\:\\/\\/.*|.*ftp\\:\\/\\/.*")) {
					...
					ResultDocList results = index.searchDocs("id:" + SimpleLuceneIndex.encodeToTerm(relatedID));
