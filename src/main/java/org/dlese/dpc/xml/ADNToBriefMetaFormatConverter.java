...
/**
 * Converts from ADN format to the BriefMeta format. See the briefMeta
 * <a href="http://www.dlese.org/Metadata/briefmeta/0.1.01/record.xml">sample
 * record</a> and
 * <a href="http://www.dlese.org/Metadata/briefmeta/0.1.01/brief-record.xsd">
 * schema</a> .
 *
...
public class ADNToBriefMetaFormatConverter implements XMLFormatConverter {
	...
	public String convertXML(String xml, ServletContext context) {
		...
		try {
			...
			output.append("<briefRecord xmlns=\"http://adn.dlese.org\" "
			...
			output.append(
					HTMLTools.encodeCharacterEntityReferences(adnXmlDoc.getXmlString("general/description"), false));
			output.append("</description>\n");

			output.append("<url>");
			output.append(HTMLTools
					.encodeCharacterEntityReferences(adnXmlDoc.getXmlString("technical/online/primaryURL"), false));
			output.append("</url>\n");
			...
			if (gradeRanges != null && gradeRanges.length > 0) {
				...
				for (int i = 0; i < gradeRanges.length; i++)
					output.append("  <gradeRange>" + HTMLTools.encodeCharacterEntityReferences(gradeRanges[i], false)
							+ "</gradeRange>\n");
				output.append("</gradeRanges>\n");
			}
			...
			if (subjects != null && subjects.length > 0) {
				...
				for (int i = 0; i < subjects.length; i++)
					output.append("  <subject>" + HTMLTools.encodeCharacterEntityReferences(subjects[i], false)
							+ "</subject>\n");
				output.append("</subjects>\n");
			}
			...
			if (resourceTypes != null && resourceTypes.length > 0) {
				...
				for (int i = 0; i < resourceTypes.length; i++)
					output.append("  <resourceType>"
							+ HTMLTools.encodeCharacterEntityReferences(resourceTypes[i], false) + "</resourceType>\n");
				output.append("</resourceTypes>\n");
			}
