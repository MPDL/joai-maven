...
/**
 * Converts from DLESE Collect format to a localized DLESE Collect format, which
 * contains no namespace declarations. Localized XML may be accessed using XPath
 * notation without the local-name() function, making it easier to use.
 *
...
public class DLESECollectToLocalizedFormatConverter implements XMLFormatConverter {
	 ...
	 *
	 * @param xml
	 *            XML input in the 'dlese_collect' format.
	 * @param context
	 *            Servlet context
	...
	public String convertXML(String xml, ServletContext context) {
		// return
		// Dom4jUtils.localizeXml(xml.replaceFirst("<\\?xml.+version=.+encoding=.+\\?>",
		// ""), "itemRecord");
