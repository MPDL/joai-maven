 ...
 * relationships of NDR Metadata Objects.
 * <p>
 *
 * More Info:
 * <li>Metadata overview:
 * http://wiki.nsdl.org/index.php/Community:NDR/ObjectTypes#Metadata
 *
 * <li>Metadata data model:
 * http://wiki.nsdl.org/index.php/Community:NCore/Model/Objects/Metadata
 *
 * <li>Metadata API requests:
 * http://wiki.nsdl.org/index.php/Community:NDR/APIRequestsByObject#Metadata_requests
 *
 * @author ostwald
 */
public class MetadataReader extends NdrObjectReader {
	 ...
	 * @exception Exception
	 *                if handle does not correspond to existing Metatadata object
	 */
	 ...
	 *            nativeFormat of the collection to which this metata belongs
	 * @exception Exception
	 *                if handle does not correspond to existing Metatadata object
	 */
	public MetadataReader(String handle, String nativeDataStreamFormat) throws Exception {
		...
		if (getObjectType() != MYTYPE)
			throw new Exception(
					"Provided handle (" + handle + ") does not refer to a metadata object (" + getObjectType() + ")");
	}
	...
	/**
	 * Gets the ncs:recordId property of the Metadata object, which is defined only
	...
	/**
	 * Gets the ncs:status property of the Metadata object, which is defined only if
	 * this object is managed by the NCS
	 *
	...
	/**
	 * Returns true if the ncs:status property of the Metadata object (which is only
	 * present if this object is managed by the NCS) is NCS_FINAL_STATUS.
	 *
	 ...
	 * @exception Exception
	 *                if the native datastream could not be read or processed
	 */
	...
	public Document getCanonicalNsdlDcItemRecord() throws Exception {
		...
		try {
			...
			throw new Exception(
					"could not create canonicalNsdlDcItemRecord for " + this.getHandle() + ": " + t.getMessage());
		}
	}
