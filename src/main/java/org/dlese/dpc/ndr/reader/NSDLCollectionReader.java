...
/**
 * Provides accesess to NSDL Collections in the NDR - exposing collection
 * management information collected from each NDR Object that defines a
 * collection, as well as the NDR Objects.
 * <p>
 *
...
public class NSDLCollectionReader {
	...
	MetadataReader findCollectionMetadata(String aggHandle, String resourceHandle) throws Exception {
		...
		if (mdHandle == null || mdHandle.trim().length() == 0)
			throw new Exception(
					"collection metadata not found for\n\tagg: " + aggHandle + "\n\tres: " + resourceHandle);
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length > 0)
			aggHandle = args[0];
		/*
		 * prtln ("args.length = " + args.length); for (int i=0;i<args.length;i++) prtln
		 * ("\t" + i + ": " + args[i]);
		 */
