 ...
 * NDR responses make heavy use of namespaces to avoid element naming colisions.
 *
 * @author ostwald
 *         <p>
 *
...
public class Tester {
	...
	static void parentObjectTester(GroupingObjectReader reader) throws Exception {
		...
		/*
		 * for (Iterator i=reader.getMemberHandles().iterator();i.hasNext();) { prtln
		 * ("\t" + (String)i.next()); }
		 */
	...
	static void getTestNode(NdrObjectReader reader) {
		...
		// String path =
		// "ndr:NSDLDataRepository/ndr:NDRObject/ndr:properties/fedora-view:createdDate";
