 ...
 * <p>
 *
 * Initialized using an Agent Identity obtained from
 * {@link org.dlese.dpc.ndr.apiproxy.NDRConstants} (see getNdrAccess method)
 * NOTE: methods that change an objects state should return a HANDLE and not the
 * OBJECT, since the object will not reflect it's actaul state anymore!!
 *
...
public class NDRToolkit {
	...
	/**
	 * Instantiates a Client instance, configures the client properties to enable it
	 * to authenticate with the NDR, and finally obtains access from the client.
	 * <p>
	 *
	 * NOTE: AgentHandle and privateKey are NOT required for read operations.
	 * <p>
	 *
	 * This version gets the ndrAccess WITHOUT USING PROPERTIES FILE, but instead by
	 * relying on NDRConstants values ...
	 *
	 ...
	 * @exception Exception
	 *                if handle is not a Resource or Resource cannot be found for
	 *                handle
	 */
	 ...
	 * @exception Exception
	 *                if handle is not a MetadataProvider or MetadataProvider cannot
	 *                be found for handle
	 */
	 ...
	 * @exception Exception
	 *                if handle is not a Aggregator or Aggregator cannot be found
	 *                for handle
	 */
	...
	public void findNsdlOrgAggregators() throws Exception {
		...
		Aggregator nsdlCollections = this.getAggregator("2200/NSDL_Collection_of_Collections_Aggregator");
		...
		// Property.Name statusProp = new Property.Name(ncsNamespace.getURI(), "status",
		// ncsNamespace.getPrefix());
	...
	public Agent getAgent() {
		String handle = config.getProperty("ndr.client.authentication.agentHandle");
		if (handle == null)
			return null;
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		 * String resHandle = "2200/test.20090410170951993T"; Resource res =
		 * tester.getResource(resHandle); String dataPath = "C:/tmp/star.gif"; // String
		 * dataPath = "C:/tmp/box.jpg";
		 */
		/*
		 * String dataPath = "C:/tmp/CollectionsAPI.png"; Resource res =
		 * tester.newResource (); File dataFile = new File(dataPath); try {
		 * tester.setResourceContent(res, dataFile); } catch (Exception e) {
	 ...
	 *                NOT YET DOCUMENTED
	 */
	public String setResourceContent(Resource resource, byte[] content, String label, String mimeType)
			throws Exception {
		...
		// no REAL reason to set label (other than to make up for fact that DS attrs
		// aren't updated)
		/*
		 * Property.Name myLabel_prop = new Property.Name("http://ns.nsdl.org/myns#",
		 * "mine", "myLabel"); Collection labels = resource.getProperties
		 * (myLabel_prop); prtln (labels.size() + " props found");
		 * resource.removeProperties (labels);
		 * 
		 * myLabel_prop = new Property.Name("http://ns.nsdl.org/myns#", "mine",
		 * "myLabel"); resource.addProperty(myLabel_prop, label);
		 */
		resource.commit();
