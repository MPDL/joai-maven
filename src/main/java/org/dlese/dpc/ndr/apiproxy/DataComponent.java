...
public class DataComponent extends InputXMLComponent {
	...
	private Element getDataStreamCmd(String format) {
		...
		/*
		 * we don't know if the format element will have a command parent (e.g., "add",
		 * "replace") or if it will be a direct child of "data"). can't figure out how
		 * to construct single xpath to catch both cases, so we use both ...
		 */
	...
	/**
	 * Inserts the provided info stream element into the datastream designated by
	 * "format".
	 *
	 * @param format
	 *            data stream in which to insert info
	 * @param info
	 *            the info stream Element
	 * @exception Exception
	 *                if datastream does not exist for "format"
	 */
	 ...
	 * @param content
	 *            the datastream content
	 * @param action
	 *            the command action (e.g., "add", "delete") or null (the default).
	 * @exception Exception
	...
	public void addDataStreamCmd(String format, Element content, String action) throws Exception {
		...
		if (action == null || !action.equals("delete")) {
			...
			/*
			 * else { prtln ("\n Adding " + format + " datastream"); pp (content); prtln
			 * (" -------  end datastream --------\n"); }
			 */
	 ...
	 * addServiceDescriptionCmd (content, null); } public void
	 * addServiceDescriptionCmd (Element content, String action) { this.addCommand
	 * (content, action); }
	 */
}
