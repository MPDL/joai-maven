...
public class InfoXML {
	...
	public InfoXML(String _xml) {
		try {
			...
			// http://www.xslt.com/html/xsl-list/2005-03/msg01059.html indicates we must
			// give
	...
	private Element getTagElement(String _tag, String _namespacePrefix, String _namespaceURI) {
		...
		try {
			// TODO this isn't complete, as we can have any arbitrary tag and values for
			// them infinitely deep
			XPath xpath = DocumentHelper
					.createXPath("//" + (_namespacePrefix != null ? _namespacePrefix + ":" : "") + _tag.trim());
			SimpleNamespaceContext ns = new SimpleNamespaceContext();
