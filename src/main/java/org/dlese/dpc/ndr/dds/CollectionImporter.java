...
public class CollectionImporter {
	...
	public String getCollectionKeyFromDDS(String _collectionName) {
		...
		try {
			URLConnection connection = new URL("http://www.dlese.org/dds/services/ddsws1-1?verb=ListCollections")
					.openConnection();
			...
			XPath xpath = DocumentHelper
					.createXPath("//collections/collection[vocabEntry=\"" + _collectionName + "\"]/searchKey/text()");
	...
	public long importCollection(String _vocabEntry, boolean _serializeMetaObjects) {
		...
		if (_vocabEntry == null) // default = comet.xml test
		{
			...
			if (metaObjects.size() > 0) {
				...
				String dcDesc = "<dc:title>Comet Test Collection</dc:title>\n"
						+ "<dc:description>TEST COLLECTION IMPORT FROM DLESE</dc:description>\n"
						+ "<dc:identifier>http://dlese.org/</dc:identifier>\n";

				/*
				 * TODO : update MDP signature String metadataProviderHandle =
				 * proxy.addMetaDataProvider( dcDesc, new String[][] { {"metadataProviderFor",
				 * proxy.getAgentHandle() } } );
				 */
				...
				for (Entry<String, ArrayList<String>> m : metaObjects.entrySet()) {
					...
					/*
					 * TODO : update the resource and metatadata signatures String resHandle =
					 * proxy.addResource( resourceURL, new String[][]{} );
					 * 
					 * String metadataHandle = proxy.addMetadata( resourceMetadata, null, new
					 * String[][] { {"metadataFor",resHandle},
				...
				if (_serializeMetaObjects) {
					try {
						FileOutputStream fileOut = new FileOutputStream("c:\\ndr\\serialized\\metaObjects."
								+ System.currentTimeMillis() / Math.pow(10, 8) + ".ser");
	...
	private Document getCollectionDocumentFromWebService(String _key) {
		...
		try {
			URLConnection connection = new URL(
					"http://www.dlese.org/dds/services/ddsws1-1?verb=Search&ky=" + _key + "&n=200&s=0")
							.openConnection();
	...
	public void loadCollection() {
		if (metaObjects == null) {
			...
			for (Iterator iter = recordNodes.iterator(); iter.hasNext();) {
				...
				XPath resURL = DocumentHelper
						.createXPath("./ddsws:metadata/adn:itemRecord/adn:technical/adn:online/adn:primaryURL");
				XPath resId = DocumentHelper.createXPath(
						"./ddsws:head/ddsws:additionalMetadata/ddsws:adn/ddsws:annotatedBy/ddsws:record/ddsws:head/ddsws:id");// adn:adn/adn:annotatedBy/adn:record/adn:head/adn:id");
				...
				if (resourceId != null && resourceURL != null && metadata != null) {
					...
					metaObjects.put(resourceId.getTextTrim(),
