...
import org.dlese.dpc.ndr.reader.NDRCollectionReader;
...
public class NDRCollectionUtils {
	...
	static void reportCollections(List aggHandles) throws Exception {
		...
		for (int i = start; i < aggHandles.size() && i < end; i++) {
			...
			try {
				...
				log(getRecord(coll));
				/*
				 * if (!coll.aggHandle.equals ("2200/test.20071205133522873T")) { log
				 * (getRecord(coll)); deleteCollection (coll.mdpHandle); }
				 */
			} catch (Exception e) {
		...
		if (mdpHandles != null) {
			for (Iterator i = mdpHandles.iterator(); i.hasNext();)
				try {
					...
					/*
					 * ModifyMetadataProviderRequest request = new
					 * ModifyMetadataProviderRequest(mdp.getHandle()); request.addCommand
					 * ("property", "replace", "null", "delete"); request.submit ();
					 */
	...
	/**
	 * Deletes all objects associated with provided mdpHandle, including aggregator
