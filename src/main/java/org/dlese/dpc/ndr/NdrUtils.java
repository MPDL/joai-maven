 ...
 * <p>
 *
 * NOTE: many NDR proxy functions require a ndrPrivateKey. Edit this file to set
 * the "ndrPrivateKeyFile" attribute to a key file accessible to your filesystem
...
public class NdrUtils {
	 ...
	 *
	 * @param ndrServer
	 *            the ndr server api url (e.g., "http://ndrtest.nsdl.org/api")
	 */
	 ...
	 * <li>ndr.private.key - (path to privatekey file for ncs.agent.handle) required
	...
	/**
	 * Returns true if the provided handle is for a MetadataProvider that is part of
	 * a NDR Collection Definition (as opposed to a MDP that is used to group
	 ...
	 * <li>if there is a metadata object that is "metadataFor" BOTH the aggregator
	 * and the resource, then the mdp was an NDR Collection MetadataProvider.
	 * </ol>
	 *
	 *
	 * @param mdpHandle
	 ...
	 * @exception Exception
	 *                when ndrRequest results in error from NDR
	 */
	...
	public static String findResourceOld(String url) {
		 ...
		 * running into a strange situation in which the findResource request returns
		 * BOTH a handle and an error element. NOTE: the handle is returned as a child
		 * of the root, unlike in the normal situation assumed by response.getHandle().
		 * This condition usually indicates the resource is still related to a deleted
		 * metadata object. I have discovered that this condition still allows us to
		 * reuse the resource in a new metadata relation.
		 */
		if (handle == null) {
			try {
	 ...
	 *
	 * @param handle
	 *            handle for NDR Object
	 * @param state
	 *            The new objectState value
	 * @exception Exception
	 *                if NDR Object not found for handle
	 */
	 ...
	 *
	 * @param reader
	 *            NDR reader for object for which state will be set
	 * @param state
	 *            The new objectState value
	 * @exception Exception
	 */
	...
	/**
	 * Gets the metadataProvider stored in the NDR associated with given collection
	 * key.
	 *
	 * @param collection
	 *            collection key
	...
	public static MetadataProviderReader getMetadataProvider(String collection) throws Exception {
		...
		// request.addCommand("relationship", "metadataProviderFor",
	 ...
	 *
	 * @param collection
	 *            collectionKey associated with a collection stored in the NDR.
	 ...
	 * @exception Exception
	 *                If url does not resolve to well-formed xml.
	 */
	...
	/**
	 * Gets the handles for all MetadataProvider objects associated with the aggent
	 * associated with specified agentHandle (which defaults to the dleseAgent).
	 * <p>
	 *
	...
	/**
	 * Gets List of MDP handles that have not been modified since provided threshold
	 * Date.
	 *
	 * @param thresholdDate
	 *            NOT YET DOCUMENTED
	 ...
	 * @exception Exception
	 *                if NDR object cannot be deleted
	 */
	 ...
	 *            handle of Object to be deleted
	 * @param cascade
	 *            flag to delete all dependent objects (not working in NDR)
	 * @return response object as InfoXML instance
	 * @exception Exception
	 *                if NDR object cannot be deleted
	 */
	 ...
	 *            collectionKey
	 * @exception Exception
	 *                if NDR objects cannot be deleted
	 */
	...
	public static void deleteNDRCollection(String mdpHandle) throws Exception {
		...
		try {
			...
			/*
			 * // NOTE: cascade does not appear to be working! prtln
			 * ("deleting mdp with cascase = true"); return deleteNDRObject (mdpHandle,
			 * true);
			 */
	 ...
	 *
	 * @param dateStr
	 *            Date in string form to be converted to a Date Object
	 * @param dateFormat
	 *            NOT YET DOCUMENTED
	 * @return Date object for dateStr, or Date corresponding to 0 milliseconds if
	 *         the dateStr cannot be parsed.
	 */
