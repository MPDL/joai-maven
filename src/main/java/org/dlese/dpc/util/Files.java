...
/**
 * Contains methods for performing common operations on files and directories
 * such as reading, moving, deleting and copying. Also contains methods used to
 * encode and decode IDs from filenames.
 *
...
public final class Files {
	 ...
	 *                If error.
	 */
	public final static StringBuffer readFile(String filePath) throws IOException {
	 ...
	 *                If error.
	 */
	public final static StringBuffer readFile(File f) throws IOException {
		// Note the FileReader is closed in the overloaded readFile method...
		return readFile(new FileReader(f));
	}

	/**
	 * Reads a file to a StringBuffer that will be in the given character encoding,
	 * for example UTF-8. The file that is read is assumed to be in the default
	 * system character encoding. See <a href=
	 * 'http://java.sun.com/docs/books/tutorial/i18n/text/convertintro.html'>Converting
	 * Non-Unicode Text Tutorial</a> .
	 *
	 * @param f
	 *            The File to read
	 * @param encoding
	 *            The encoding to write to, or null to use the default system
	 *            encoding
	 * @return The content of the File in the given encoding
	 * @exception IOException
	 *                If error
	 */
	public final static StringBuffer readFileToEncoding(File f, String encoding) throws IOException {

		// The following two lines are just for debugging...
		// OutputStreamWriter out1 = new OutputStreamWriter(new
		// ByteArrayOutputStream());
		// System.out.println("readFile() encoding: " + out1.getEncoding() + " file
	 ...
	 *                If error.
	 */
	public final static StringBuffer readInputStream(InputStream is) throws IOException {
		IOException e = null;
	...
	/**
	 * Reads a Reader into a StringBuffer using a BufferedReader and closes the
	 * Reader when done.
	 *
	 * @param rdr
	 *            A Reader.
	 ...
	 *                If error.
	 */
	private final static StringBuffer readFile(Reader rdr) throws IOException {
		if (rdr == null)
			return null;
	 ...
	 *                If error
	 */
	public final static void writeFile(String content, File file) throws IOException {
	 ...
	 *                If error
	 */
	public final static void writeFile(String content, String filePath) throws IOException {
	 ...
	 *                If error.
	 */
	public final static void writeFile(StringBuffer content, File file) throws IOException {
	 ...
	 *                If error.
	 */
	public final static void writeFile(StringBuffer content, String filePath) throws IOException {
		writeFile(content.toString(), filePath);
	}

	/**
	 * Recursively copies the contents of one directory to another. The source and
	 * destination File objects must be directories and must exist, otherwise
	 * nothing is done. Existing files in the destination directory with the same
	 * name as those in the source directory are overwritten.
	 *
	 * @param sourceDir
	...
	/**
	 * Copys the contents of one file to another, as bytes. E.g., does not convert
	 * from and then back to characters. May be used for both text and binary files.
	 *
	 * @param infile
	 *            The file to copy.
	 * @param outfile
	...
	/**
	 * Escapes a Windows directory path by removing the first colon so that it can
	 * be used as the end part of another directory path. For example, if the path
	 * is C:/mydirectory/myfile.txt the output of this method will be
	 * C/mydirectory/myfile.txt. If the path does not begin with an upper-case
	 * character and a colon then nothing is done.
	 *
	 * @param path
	 *            The original path.
	...
	/**
	 * Gets a record ID from its file name. Assumes the file name is of the form
	 * ID.xml where ID is the record id. For example DLESE-000-000-000-001.xml.
	 *
	 * @param file
	 *            The file.
	...
	/**
	 * Appends a path into a string in such a way that lexicographic sorting gives
	 * the same results as a walk of the file hierarchy. Thus null (\u0000) is used
	 * both to separate directory components. May be decoded back to a file path
	 * using {@link #fid2path(String fid)}.
	 *
	 * @param f
	...
	/**
	 * Appends a path and date into a string in such a way that lexicographic
	 * sorting gives the same results as a walk of the file hierarchy. Thus null
	 * (\u0000) is used both to separate directory components and to separate the
	 * path from the date.
	 *
	 * @param f
	 *            The file whoes path will be encoded.
