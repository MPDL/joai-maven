...
/**
 * This class holds a number of handy static methods for use in HTML and XML
 * processing.
 *
...
public class HTMLTools {
	...
	/**
	 * Encodes a regular String with caracter entity references, for example the
	 * ampersand character &amp; becomes &amp;amp;. See
	 * <a href="http://www.w3schools.com/html/html_entitiesref.asp"> Entities
	 * Reference </a> for a nice table of character entity references. Ampersands
	 * that are already encoded will not be touched. If the input string is an SGML
	 * variant like HTML or XML then all occurances of the characters &lt;, &gt; and
	 * &quot; are left alone.
	 *
	 * @param input
	 *            A String.
	...
	public static String encodeCharacterEntityReferences(String input, boolean isSGML) {
		...
		for (int i = 0; i < input.length(); i++) {
			...
			if (tchr == '\"' && !isSGML)
				...
				bufa.append("&gt;");
			else if (tchr == '&' && input.length() <= (i + 4))
				bufa.append("&amp;");
			else if (tchr == '&' && !(input.charAt(i + 1) == 'a' && input.charAt(i + 2) == 'm'
					&& input.charAt(i + 3) == 'p' && input.charAt(i + 4) == ';'))
				bufa.append("&amp;");
			// Copyright symbol (c)
			else if (tchr == '\u00A9')
				bufa.append("&#169;");
			/*
			 * // en dash else if (tchr == '\u2013') bufa.append("&#8211;"); // em dash else
			 * if (tchr == '\u2014') bufa.append("&#8212;"); // left single quotation mark
			 * else if (tchr == '\u2018') bufa.append("&#8216;"); // right single quotation
			 * mark else if (tchr == '\u2019') bufa.append("&#8217;"); // left double
			 * quotation mark else if (tchr == '\u201C') bufa.append("&#8220;"); // right
			 * double quotation mark else if (tchr == '\u201D') bufa.append("&#8221;");
			 */
			else
				bufa.append(tchr);
		}
	...
	/**
	 * Encodes an Array of chars so that they will be valid inside JavaScript
	 * quotes.
	 *
	 * @param chars
	 *            An array of chars
