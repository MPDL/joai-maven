...
/**
 * This class holds a number of handy static methods for generating unique ids,
 * random numbers within a given range, parsing dates, executing command-line
...
public class Utils {
	...
	/**
	 * Converts a String that contains a recognizable date/time to a Java Date
	 * object. See {@link java.text.SimpleDateFormat} for syntax information for the
	 * format String. For example a valid date String might look like
	 * '2003-04-27MST' with a date format specifier of 'yyyy-MM-ddz' where yyyy
	 * indicates the year, MM the month, dd the day and z the general time zone
	 * (GMT, MST, PST, etc).
	 *
	 * @param dateString
	 ...
	 *            '2003-04-27MST'.
	 * @param dateFormat
	 *            The format of the date String as specified in
	 *            {@link java.text.SimpleDateFormat}, for example 'yyyy-MM-ddz'.
	 * @return The Date object.
	 * @exception ParseException
	 *                If unable to interpret the date String using the given format
	 *                specifier.
	 */
	public final static Date convertStringToDate(String dateString, String dateFormat) throws ParseException {
		return new SimpleDateFormat(dateFormat).parse(dateString);
	}

	/**
	 * Converts a Java Date that into a formatted String. See
	 * {@link java.text.SimpleDateFormat} for syntax information for the format
	 * String. For example a valid date format specifier might be '"MMM' 'dd',
	 * 'yyyy' 'z"' where yyyy indicates the year, MMM the month, dd the day and z
	 * the time zone (GMT, MST, PST, etc).
	 *
	 * @param date
	 *            A Java Date object.
	 * @param dateFormat
	 *            The format of the date String to be output as specified in
	 *            {@link java.text.SimpleDateFormat}, for example 'yyyy-MM-ddz'.
	 * @return The formatted String.
	 * @exception ParseException
	 *                If unable to interpret the date Date using the given format
	 *                specifier.
	 */
	public final static String convertDateToString(Date date, String dateFormat) throws ParseException {
	...
	/**
	 * Print the elapsed time that occured beween two points of time as recorded in
	 * java Date objects.
	 *
	 * @param start
	 *            The start Date.
	 ...
	 *            The end Date.
	 * @param msg
	 *            A message inserted in front of the elapsed time string.
	 */
	...
	/**
	 * Converts a long that represents time in milliseconds to a String that
	 * displays the time in minutes and seconds.
	 *
	 * @param milliseconds
	 *            Time in millisoconds.
	...
	/**
	 * Returns true if the object in the first parameter contains the Object in the
	 * second parameter according to the Objects equals method. The first parameter
	 * can be an Array, Collection (List, Set, etc.), Map or String. If the Object
	 * in the first parameter is a String, returns true if the Object in the second
	 * parameter is a subString of the first. If the Object in the first parameter
	 * is a Map, returns true if one of it's keys is equal to the Object in the
	 * second parameter.
	 * <p>
	 *
	 ...
	 *            The target Object
	 * @param subject
	 *            The subject Object for comparison
	...
	/**
	 * Puts items in a Map, creating a new TreeMap if null is passed in for the
	 * myMap argument, otherwise updating the Map with the key/value pair. TreeMap
	 * keeps its keys sorted lexagraphically.
	 *
	 * @param myMap
	 *            A Map or null
	 * @param key
	 *            A key for insertion in the Map
	 * @param value
	 *            A value for insertion in the Map, or null to insert empty String
	...
	/**
	 * Puts items in a Map, creating a new Map if null is passed in for the myMap
	 * argument, otherwise updating the Map with the key/value pair. Keys remain
	 * sorted by the *values* in the Map.
	 *
	 * @param myMap
	 *            A Map or null
	 * @param key
	 ...
	 *            A value for insertion in the Map, or null to insert empty String
	 * @param ascending
	 *            'ascending' or 'descending' (defaults to ascending if other value
	 *            is passed)
	 * @return The existing Map or a new TreeMap with the key/value pair inserted
	 */
	...
	/**
	 * Gets the Map from a String in the Java properties format of the form
	 * property=value one per line.
	 *
	 * @param propertiesString
	 *            A Java Properties String
	 * @return A Map of properties, or null if unable to parse
	 */
	public static Map getPropertiesMap(String propertiesString) {
		try {
			Properties properties = new Properties();
			// Fix the string to make sure the line ending string is actually a line ending
			// char:
	 ...
	 * @param errorOutput
	 *            A StringBuffer that will be pupulated with the normal output after
	 *            execution, if any, or null not to use it
	 * @param standardOutput
	 *            A StringBuffer that will be pupulated with the error output after
	 *            execution, if any, or null not to use it
	 ...
	 *                If unable to encode using UTF-8.
	 */
	public final static String URLEncoder(String text) throws UnsupportedEncodingException {
		if (text == null)
			return null;
		try {
	...
	/**
	 * Encodes a string used in the URL sent to the redirect server and ensures it
	 * does not contain problematic characters for the Apache 1 mod_redirect rules
	 * (the character sequence %2F is replaced with /).
	 *
	 * @param text
	 *            Unencoded text
	 * @return Text that is encoded for use in a URL
	 ...
	 *                If unable to encode using UTF-8.
	 */
	public final static String RedirectEncoder(String text) throws UnsupportedEncodingException {
		if (text == null)
			return "";
	 ...
	 * expression with the given replacement. If replacement is empty or null, all
	 * matches are removed from the source String.
	 *
	 * @param source
	 *            The source String
	 * @param regEx
	...
	/**
	 * Converts a Lucene String-encoded date to a Date Object. If the String can not
	 * be converted, returns null.
	 *
	 * @param dateString
	 *            A Lucene String-encoded date
	...
	/**
	 * Converts a date String of the form YYYY-mm-dd, YYYY-mm, YYYY or
	 * yyyy-MM-ddTHH:mm:ssZ to a searchable Lucene (v2.x) lexical date String of the
	 * form 'yyyyMMddHHmmss', or null if unable to parse the date String.
	 *
	 * @param dateString
	 *            A date String
	...
	/**
	 * Gets a ResourceBundle from a properties file that is in a Jar file or class
	 * path within this application's runtime environment.
	 *
	 * @param propsFileName
	 ...
	 *
	 * @param dayOfWeek
	 *            A Calenday.DAY_OF_WEEK field value
	...
	/**
	 * Encodes a String to a single term for searching over fields that have been
	 * indexed encoded. Encodes spaces but leaves the wild card '*' un-encoded for
	 * searching.
	 *
	 * @param string
	 *            A String to encode
	 * @return An encoded String that may be used for searches
	 * @see org.dlese.dpc.index.SimpleLuceneIndex#encodeToTerm(String, boolean,
	 *      boolean)
	 */
	...
	/**
	 * Encodes a String to a String for searching over fields that have been indexed
	 * encoded. Preserves the wild card '*' and spaces ' '.
	 *
	 * @param string
	...
	/**
	 * Gets a global system unique ID. This algorithm is reasonably guaranteed to be
	 * correct within a single running JVM.
	 *
