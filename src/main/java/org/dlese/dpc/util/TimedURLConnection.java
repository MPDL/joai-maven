...
/**
 * A {@link java.net.URLConnection} wrapper that allows a connection timeout to
 * be set, support for gzip streaming, GET and POST data. A timout is useful for
 * applications that need to retrieve content from a URL without hanging if the
 * remote server does not respond within a given period of time. Throws a
 * {@link URLConnectionTimedOutException} if the connection is not made within
 * the allotted time or a {@link java.io.IOException} if the connection fails
 * for some other reason, such as an HTTP type 500 or 403 error.
 * <p>
 *
 * The static methods {@link #importURL} and {@link #getInputStream} are
...
public class TimedURLConnection {
	...
	/**
	 * Imports the content of a given URL into a String using the default character
	 * encoding, timing out if the remote server does not respond within the given
	 * number of milliseconds. A timeout set to 0 disables the timeout (e.g. timeout
	 * of infinity). If the connection is http, then redirects are followed. Uses
	 * gzip encoding if the server supports it. Throws an IOException if an http
	 * connection returns something other than status 200.
	 *
	 * @param url
	 *            The URL to import
	 * @param timeOutPeriod
	...
	/**
	 * Imports the content of a given URL into a String using the given character
	 * encoding timing out if the remote server does not respond within the given
	 * number of milliseconds. A timeout set to 0 disables the timeout (e.g. timeout
	 * of infinity). If the connection is http, then redirects are followed. Uses
	 * gzip encoding if the server supports it. Throws an IOException if an http
	...
	/**
	 * Imports the content of a given URL into a String using the given character
	 * encoding, using POST data if indicated or GET, timing out if the remote
	 * server does not respond within the given number of milliseconds. A timeout
	 * set to 0 disables the timeout (e.g. timeout of infinity). If the connection
	 * is http, then redirects are followed. Uses gzip encoding if the server
	 * supports it. Throws an IOException if an http connection returns something
	 ...
	 *            parm1=value1&amp;param2=value2 or null to use GET (pass all params
	 *            in the url)
	 * @return A String containing the contents of the URL
	 * @exception IOException
	 *                If IO Error
	...
	/**
	 * Gets an InputStream for the given URL, timing out if the remote server does
	 * not respond within the given number of milliseconds. A timeout set to 0
	 * disables the timeout (e.g. timeout of infinity). Supports gzip compression
	 * (returns a GZIPInputStream if the server supports it). If the connection is
	 * http, then redirects are followed. Throws an IOException if an http
	 * connection returns something other than status 200.
	 *
	 * @param url
	 *            The URL to import
	...
	public static InputStream getInputStream(URL url, String postData, int timeOutPeriod)
		...
		try {
			...
			try {
				...
				throw new URLConnectionTimedOutException("Reached connection time-out period (" + timeOutPeriod
						+ "ms) for URL '" + url + "'. Details: " + ste.getMessage());
			}
			...
			// Since we requested qzip output above, check to see if the content was
			// returned
	...
	/**
	 * Gets the last modified time of the URL, timing out if the remote server does
	 * not respond within the given number of milliseconds. A timeout set to 0
	 * disables the timeout (e.g. timeout of infinity). Throws an IOException if an
	 * http connection returns something other than status 200.
	 *
	 * @param url
	 *            The URL to check
	 * @param timeOutPeriod
