...
/**
 * Utility class for mapping "static" URIs (i.e. "catalog_(\S+).htm") to dynamic
 * ones (i.e. "view_resource.do?description=$1") using regexes
 *
		...
		while ((forwardPage == null) && i.hasNext()) {
			...
			if (m.matches()) {
				...
				for (int j = 1; j <= m.groupCount(); j++) {
					...
					if (ind > -1) {
						mapto = mapto.substring(0, ind) + m.group(j) + mapto.substring(ind + 2, mapto.length());
					}
				}
				forwardPage = mapto;
