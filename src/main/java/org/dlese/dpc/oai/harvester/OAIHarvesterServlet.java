...
public final class OAIHarvesterServlet extends HttpServlet {
	...
	public void init(ServletConfig conf) throws ServletException {
		...
		/*
		 * int updateFrequency = 8; String fileSyncronizationFrequency = (String)
		 * context.getInitParameter("updateFrequency"); try { if
		 * (fileSyncronizationFrequency != null) { updateFrequency =
		 * Integer.parseInt(fileSyncronizationFrequency); } } catch (Throwable nfe) {
		...
		SimpleLuceneIndex harvestLogIndex = new SimpleLuceneIndex(
				harvesterDataDir.getAbsolutePath() + "/harvest_log_index");
		...
		try {
			...
			f.mkdir();
			harvesterSettings = new SimpleDataStore(f.getAbsolutePath(), true);
		} catch (Throwable e) {
		...
		if (shs != null) {
			...
			/*
			 * for(int i = 0; i < scheduled.length; i++) scheduled[i].init(new
			 * SimpleHarvestMessageHandler(),scheduled[i].getHarvestDir());
			 */
		}
		...
		ScheduledHarvestManager scheduledHarvestManager = new ScheduledHarvestManager(harvesterSettings,
				new File(GeneralServletTools.getAbsolutePath("WEB-INF/harvested_records", context)), harvestLogIndex,
				harvestTimeOutMilliseconds);
	...
	private void addXslConverter(String paramVal, XMLConversionService xmlConversionService) throws ServletException {
		...
		xmlConversionService.addXslStylesheet(vals[1], vals[2],
				GeneralServletTools.getAbsolutePath("WEB-INF/xsl_files/" + vals[0], getServletContext()));

	}

	private void addJavaConverter(String paramVal, XMLConversionService xmlConversionService) throws ServletException {
	...
	public void destroy() {
		...
		ScheduledHarvestManager scheduledHarvestManager = (ScheduledHarvestManager) getServletContext()
				.getAttribute("scheduledHarvestManager");
