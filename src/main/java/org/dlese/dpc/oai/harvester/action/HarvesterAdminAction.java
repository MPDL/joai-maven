...
public final class HarvesterAdminAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest req,
		...
		SimpleDataStore ds = (SimpleDataStore) servletContext.getAttribute("harvesterSettings");

		ScheduledHarvestManager shm = (ScheduledHarvestManager) servletContext.getAttribute("scheduledHarvestManager");
		...
		if (haf.getCommonDirs() == null) {
			...
			if (shhash != null) {
				...
				/*
				 * for (int f = 0; f < i; f++) { prtln(dirs[f]); }
				 */
		...
		try {
			...
			if (req.getParameter("doHarvest") != null) {
				...
				if (baseURL.length() == 0) {
					errors.add("baseURL", new ActionError("generic.message", "Please provide a base URL."));
				} else if (!(baseURL.startsWith("http://") || baseURL.startsWith("https://"))) {
					errors.add("baseURL",
							new ActionError("generic.message", "The baseURL must begin with http:// or https://"));
				}
					...
					try {
						...
						errors.add("until",
								new ActionError("generic.message",
										"Until date must be of the form YYYY-MM-DDThh:mm:ssZ or YYYY-MM-DD. "
												+ e.getMessage()));
					}
				}
				if (!errors.isEmpty()) {
				...
				if (errors.isEmpty()) {
					ScheduledHarvest sh = new ScheduledHarvest("One-time harvest", setSpec, baseURL, metadataPrefix,
							"0", "days", null, "disabled", null, true, false, true, doZipFiles, "");
					...
					if (shm.isRunningOneTimeHarvest(sh)) {
						...
						errors.add("harvestErr", new ActionError("generic.message",
								"A harvest of " + sh.getBaseURL() + set + " is already in progress."));
					...
					shm.oneTimeHarvest(sh, fromDate, untilDate, (File) ds.get(Keys.HARVESTED_DATA_DIR),
					...
					errors.add("runOneTimeHarvest", new ActionError("generic.message",
							"A harvest of " + sh.getBaseURL() + set + " has been started."));
			...
			if (req.getParameter("scheduledHarvest") != null) {
				...
				if (paramVal.equals("add")) {
					...
					sh = new ScheduledHarvest(req.getParameter("shRepositoryName"), req.getParameter("shSetSpec"),
							req.getParameter("shBaseURL"), req.getParameter("shMetadataPrefix"),
							req.getParameter("shHarvestingInterval"), req.getParameter("shIntervalGranularity"),
							req.getParameter("shRunAtTime"), reg, new File(direc), split, false, def, doZipFiles,
							allowD);

					/*
					 * if (shm.containsValue(sh)) { prtln("contains value."); return
					 * mapping.findForward("display.harvester.settings"); }
					 */
					...
					if (prevSh != null) {
						...
						if (sh.getBaseURL().equals(prevSh.getBaseURL()) && sh.getSetSpec().equals(prevSh.getSetSpec())
					...
					// Ensure harvests only occur once even if link is clicked multiple times.
					/*
					 * Long ses = new Long(req.getParameter("mySes")); if
					 * (mySessions.containsKey(ses)) mySessions.remove(ses); else return
					 * mapping.findForward("display.harvester.settings");
					 */
					Long shUid = new Long(req.getParameter("shUid"));
					...
					if (!shm.containsScheduledHarvest(shUid)) {
						errors.add("harvestErr",
								new ActionError("generic.message", "The requested harvest is no longer configured."));
						saveErrors(req, errors);
						return mapping.findForward("display.harvester.settings");
					}
	 ...
	 *                DESCRIPTION
	 */
	private boolean setDefaults(SimpleDataStore ds, ServletContext servletContext) throws Exception {

		// Grab settings:
		File harvestedDataDir = (File) ds.get(Keys.HARVESTED_DATA_DIR);
		if (harvestedDataDir == null) {
			harvestedDataDir = new File(GeneralServletTools
					.getAbsolutePath("WEB-INF" + File.separator + "harvested_records", servletContext));
