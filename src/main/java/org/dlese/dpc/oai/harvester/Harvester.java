...
/**
 * Harvests metadata from an <a href="http://www.openarchives.org/">OAI</a> data
 * provider, saving the results to file or returning the raw XML as an array of
 * Strings. Supports data providers that use resumption tokens for <a href=
 * "http://www.openarchives.org/OAI/2.0/openarchivesprotocol.htm#FlowControl">
 * flow control</a> , selective harvesting by <a href=
 * "http://www.openarchives.org/OAI/2.0/openarchivesprotocol.htm#SelectiveHarvestingandDatestamps">
 * date</a> or <a href=
 * "http://www.openarchives.org/OAI/2.0/openarchivesprotocol.htm#SelectiveHarvestingandSets">
 * set</a> , gzip <a href=
 * "http://www.openarchives.org/OAI/2.0/openarchivesprotocol.htm#ResponseCompression">
 * response compression</a> and other protocol features. Supports OAI protocol
 * versions 1.1 and <a href=
 * "http://www.openarchives.org/OAI/2.0/openarchivesprotocol.htm">2.0</a> .
 * <p>
 *
 * To perform a harvest, use one of the following methods:
 * <ul>
 * <li>The static <b>harvest</b> method (for general use):
 * <code>{@link #harvest}}</code><br/>
 * <br/>
 * </li>
 * <li>The static <b>main</b> method (for command-line use):
 * <code>{@link #main}</code><br/>
 * <br/>
 * </li>
 * <li>The non-static <b>doHarvest</b> method (provides a few additional
 * options): {@link #doHarvest}. <br/>
 * <br/>
 * </li>
 * </ul>
 * Use of this API assumes familiarity with the
 * <a href="http://www.openarchives.org/OAI/2.0/openarchivesprotocol.htm"> OAI
 * protocol</a> .
 * <p>
 *
 *
...
public class Harvester implements ErrorHandler {
	...
	/**
	 * Command line interface for the harvester. Harvest status messages are output
	 * to standard out.
	 * <p>
	 *
	 * Arguments (required arguments must be in this order, optional arguments may
	 * be in any order):
	 * <ul>
	 * <li>outdir (required) - Path to the directory to write the harvested record
	 * files, for example "." or "/home/user/harvested_files"
	 * <li>baseURL (required) - Base URL to harvest from, for example
	 ...
	 * <li>[ -from:fromDate ] (optional) - The harvest from date, for example,
	 * -from:2003-12-31T23:59:59Z
	 *
	 * <li>[ -until:untilDate ] (optional) - The harvest until date, for example,
	 * -until:2004-12-31T23:59:59Z
	 *
	 * <li>[ -splitBySet:true|False ] (optional) - True to save each record in
	 * separate directories split by set inside outdir, false to save all records to
	 * the root of outdir (default is false)
	 * <li>[ -writeHeaders:true|False ] (optional) - True to have OAI headers
	 * written to the output, false not to (default is false)
	 * </ul>
	 *
	 *
	 * @param args
	...
	public static void main(String[] args) {
		...
		String[][] resmat = null;
		try {
			resmat = harvest(baseURL, metadataPrefix, setSpec, from, until, outdir, splitBySet, hmh, oaiChangeListener,
					writeHeaders, harvestAll, harvestAllIfNoDeletedRecord, 240000);
	...
	/**
	 * Harvest the given provider, saving the resulting metadata to file or
	 * returning the results as an array of Strings. A HarvestMessageHandler may be
	 * specified to capture harvest progress messages. Use a
	 * {@link SimpleHarvestMessageHandler} to have harvest messages sent to standard
	 * out. A {@link OAIChangeListener} may be specified to recieve messages about
	 * chages to harvested records.
	 *
	 * @param baseURL
	 *            The baseURL of the data provider, for example
	 *            "http://www.dlese.org/oai/provider"
	 * @param metadataPrefix
	 *            The metadataPrefix, for example "oai_dc"
	 * @param setSpec
	 *            The set to harvest, for example "testset", or null to harvest all
	 *            sets
	 * @param from
	 *            The from date, for example "2003-12-31T23:59:59Z", or null for
	 *            none
	 * @param until
	 *            The until date, for example "2003-12-31T23:59:59Z", or null for
	 *            none
	 * @param outdir
	 *            The path of output dir. If null or "", we return the String[][]
	 *            array; if specified we return null
	 * @param msgHandler
	 *            A handler for status messages that occur during the harvest, or
	 *            null to ingnore messages
	 * @param oaiChangeListener
	 *            The OAIChangeListener that will recieve notifications, or null for
	 *            none
	 * @param writeHeaders
	 *            True to have OAI headers written to the output, false not to
	 * @param harvestAll
	 *            True to delete previous harvested record files and harvest all
	 *            records again from scratch; false to preserve previous record
	 *            files and replace or delete only those that have changed
	 * @param harvestAllIfNoDeletedRecord
	 *            True to harvest all record files from scratch if deleted records
	 *            are not supported
	 * @param splitBySet
	 *            True to save each record in separate directories split by set
	 *            inside outdir, false to save all records to the root of outdir
	 ...
	 *                If serious error
	 * @exception OAIErrorException
	 *                If OAI error
	 */
	public static String[][] harvest(String baseURL, String metadataPrefix, String setSpec, Date from, Date until,
			String outdir, boolean splitBySet, HarvestMessageHandler msgHandler, OAIChangeListener oaiChangeListener,
			boolean writeHeaders, boolean harvestAll, boolean harvestAllIfNoDeletedRecord, int timeOutMilliseconds)
			throws Hexception, OAIErrorException {
		Harvester hvst;

		hvst = new Harvester(msgHandler, oaiChangeListener, timeOutMilliseconds);

		String[][] res = hvst.doHarvest(baseURL, metadataPrefix, setSpec, from, until, outdir, splitBySet, " ", " ",
				writeHeaders, harvestAll, harvestAllIfNoDeletedRecord);
		return res;
	}
	 ...
	 *            harvest progresses, or null if none.
	 * @param oaiChangeListener
	 *            The OAIChangeListener that will recieve notifications, or null for
	 *            none.
	 * @param timeOutMilliseconds
	...
	/**
	 * Gracefully kills the harvest after the current record is finished being
	 * harvested.
	 */
	public void kill() {
	...
	/**
	 * Sets the number of records harvested before statusMessage notifications to
	 * the HarvestMessageHandler are made.
	 *
	 * @param numRecords
	 *            The new numRecordsForNotification value
	 */
	...
	/**
	 * Gets the endTime when the havest completed either because of an error or at
	 * the end of a successful harvest. Returns 0 if the harvest is still in
	 * progress.
	 *
	...
	/**
	 * Gets the current number of records that have been harvested by this
	 * harvester. This number increases as the harvest progresses.
	 *
	...
	/**
	 * Gets the number of resumption tokens that have currently been issued by the
	 * data provider. This number increases as the harvest progresses. This number
	 * gives a rough indication of the progression and duration of the harvest.
	 *
	...
	/**
	 * Performs the harvest. Note that his method is not safe for multiple harvests
	 * - a separate Harvester instance should be created for each havest performed.
	 * <P>
	 *
	 *
	 *
	 * @param metadataPrefix
	 *            metadataPrefix. e.g., "oai_dc", or null to harvest all formats
	 * @param setSpec
	 *            set. e.g., "testset" or null for none.
	 * @param from
	 *            from date. May be null.
	 * @param until
	 *            until date. May be null.
	 * @param outdir
	 *            path of output dir. If null or "", we return the String[][] array;
	 *            if specified we return null.
	 * @param writeHeaders
	 *            True to have oai headers written to file, false not to. <br>
	 *            The directory structure under outdir is: <br>
	 *            outdir/set/subset/subset/metadataPrefix/oaiId_hdr.xml OAI header
	 *            <br>
	 *            outdir/set/subset/subset/metadataPrefix/oaiId_data.xml OAI
	 *            contents <br>
	 *
	 * @param baseURL
	 ...
	 *            To split set
	 * @param zipName
	 *            Name of the zip file to save to, or null for no zipping
	 * @param zDir
	 *            Directory of the zipfile
	 * @param harvestAll
	 *            True to delete previous harvested records and harvest all records
	 *            again from scratch
	 * @param harvestAllIfNoDeletedRecord
	 *            True to harvest all records from scratch if deleted records are
	 *            not supported
	 * @return If outdir is specified returns null; if outdir is null or "", returns
	 *         one row for each record harvested. Each row has two elements:
	 *         <ul>
	 ...
	 *                If serious error.
	 * @exception OAIErrorException
	 *                If OAI error was returned by the data provider.
	 */
	public String[][] doHarvest(String baseURL, String metadataPrefix, String setSpec, Date from, Date until,
			String outdir, boolean splitBySet, String zipName, String zDir, boolean writeHeaders, boolean harvestAll,
			boolean harvestAllIfNoDeletedRecord) throws Hexception, OAIErrorException {

		// this.outputDir = new File(OAIUtils.getHarvestedDirBaseURLPath(outdir,
		...
		/*
		 * int z; if (oD.exists()) z = Count(oD); else z = 0;
		 * prtln("Count at beginning is " + z); if ((splitBySet != true) && (recordCount
		 * != z)) { recordCount = z; }
		 */
		synchronized (this) {
			if (hasDoneHarvest) {
				throw new Hexception(
						"This harvester has already performed a harvest. Please use a new Harvester instance.");
			}
			hasDoneHarvest = true;
			isRunning = true;
		}
		...
		try {
			...
			for (ipref = 0; ipref < prefices.length; ipref++) {
				...
				while (true) {
					...
					if (resumption == null) {
						...
						reqMessage = "A request for ListRecords with resumptionToken " + resumption
								+ " has been made. Establishing connection with the data provider...";
			...
			/*
			 * z = Count(oD); prtln("Count at end is " + z); if ((splitBySet != true) &&
			 * (recordCount != z)) { recordCount = z; } prtln("recordCount is " +
			 * recordCount);
			 */
			// Create a zip file of this harvest so that it can be directly downloaded from
			// the UI
			...
			int c = recordCount;
			if (msgHandler != null) {
				msgHandler.completedHarvestMessage(c, resumpCount, baseURL, setSpec, startTime, endTime,
						fullZipFileName, getGranularityString(), getDeletedRecordString());
			}
			isRunning = false;
			...
			endTime = System.currentTimeMillis();
			if (msgHandler != null) {
				msgHandler.oaiErrorMessage(oaie.getOAIErrorCode(), oaie.getOAIErrorMessage(), getGranularityString(),
	...
	/**
	 * Extracts records from an XML document. If outdir is null, appends them to
	 * reslist. If outdir is not null, writes them to the appropriate subdir of
	 * outdir.
	 *
	 * @param prefix
	 *            The metadata prefix.
	 * @param doc
	 *            The XML document in OAI format.
	 * @param reslist
	 *            The result list, used only when outdir is null.
	 * @param outdir
	 *            The output directory.
	 * @param baseURL
	 *            The base URL
	 * @param writeHeaders
	 *            True to have header files written.
	 * @param splitBySet
	 ...
	 *                If OAI error.
	 */
	private String extractRecords(String prefix, Document doc, LinkedList reslist, String outdir, String baseURL,
			boolean splitBySet, boolean writeHeaders) throws Hexception, OAIErrorException {
		String resumption = null;
		...
		try {
			...
			throw new Hexception(
					"The data provider returned an invalid response to the ListRecords request: " + e.getMessage());
		}
	...
	/**
	 * Extracts the content portion of a single record within an OAI XML document.
	 * If outdir is null, appends them to reslist. If outdir is not null, writes
	 * them to the appropriate subdir of outdir.
	 *
	 * @param prefix
	 *            The metadata prefix.
	 * @param hdr
	 *            The &lt;header&gt; element for the record within the OAI XML
	 *            document.
	 * @param reslist
	 *            The result list, used only when outdir is null.
	 * @param outdir
	...
	private void extractContent(String prefix, Element hdr, LinkedList reslist, String outdir, Document doc,
			String baseURL, boolean splitBySet, boolean writeHeaders) throws Hexception {
		...
		// The following should set an xmlns declaration for the xsi namespace. Will not
		// do anything if already set
		if (contele != null)
		...
		if (outdir == null) {
			if (deleted) {
				reslist.add(new String[]{encodeIdentifier(identstg), "deleted"});
			} else {
				...
				reslist.add(new String[]{encodeIdentifier(identstg), wtr.toString()});
	 ...
	 *
	 * @param request
	 *            The http request, e.g., "http://www.x.com/..."
	...
	private Document getDoc(String request) throws Hexception {
		...
		try {
			...
			if (exc.getMessage().matches(".*respcode.*")) {
				msg = "The request for data resulted in an invalid response from the provider."
						+ " The baseURL indicated may be incorrect or the service may be unavailable."
						+ " HTTP response: " + exc.getMessage();
			} else {
				msg = "The request for data resulted in an invalid response from the provider. Error: "
						+ exc.getMessage();
			}
			throw new Hexception(msg);
		}
	 ...
	 * @param ele
	 *            The xml Element subtree to write to file
	 * @param doc
	 *            The Document
	 * @return True if content previously existed in the given file and the content
	 *         is the same as the new content provided
	 * @exception Hexception
	 *                If exception
	 */
	private boolean writedoc(String fname, Element ele, Document doc) throws Hexception {
		try {
			...
			throw new Hexception("cannot write file \"" + fname + "\"  reason: " + ioe);
		}
	}

	/**
	 * Finds the first immediate child of the specified Element having the specified
	 * tag; throws Hexception if none found.
	 *
	 * @param ele
	 *            DESCRIPTION
	 * @param tag
	...
	/**
	 * Finds the first immediate child of the specified Element having the specified
	 * tag; returns null if none found.
	 *
	 * @param ele
	 *            DESCRIPTION
	 * @param tag
	...
	/**
	 * Finds the first following sibling of the specified Element having the
	 * specified tag; throws Hexception if none found.
	 *
	 * @param ele
	 *            DESCRIPTION
	 * @param tag
	 ...
	 *                DESCRIPTION
	 */
	private Element mustFindSibling(Element ele, String tag) throws Hexception {
	...
	/**
	 * Finds the first following sibling of the specified Element having the
	 * specified tag; returns null if none found.
	 *
	 * @param ele
	 *            DESCRIPTION
	 * @param tag
	...
	/**
	 * Finds the first following sibling of the specified Element having either of
	 * the specified tags; returns null if none found.
	 *
	 * @param ele
	 *            DESCRIPTION
	 * @param taga
	...
	private Element findSibling(Element ele, String taga, String tagb) {
		...
		while (nd != null) {
			if (nd.getNodeType() == Node.ELEMENT_NODE
					&& (nd.getNodeName().equals(taga) || nd.getNodeName().equals(tagb))) {
				res = (Element) nd;
				break;
			}
			nd = nd.getNextSibling();
	...
	/**
	 * Appends to resbuf the concatenation of all text node content under the
	 * specified node.
	 *
	 * @param nd
	 *            DESCRIPTION
	...
	/**
	 * Sets the granularity and deleted record support of the data provider being
	 * harvested. Granularity is used for the "from" and "until" arguments: either
	 * GRAN_DAY or GRAN_SECOND. Deleted record support is used to determine whether
	...
	private void getIdentifyInfo(String baseURL) throws Hexception, OAIErrorException {
		int ii;

		String request = baseURL + "?verb=Identify";
		if (msgHandler != null) {
			msgHandler.statusMessage(
					"A request for Identify has been made. Establishing connection with the data provider...");
		}
		Document doc = getDoc(request);
		...
		try {
			...
			try {
				...
				throw new Hexception(
						"The data provider returned an invalid response to the Identify request: " + e.getMessage());
			}
		...
		if (gran.equals("YYYY-MM-DD")) {
			...
			throw new Hexception(
					"provider supports an invalid granularity according to the OAI protocol. Invalid response: "
							+ gran);
		}
		...
		if (deletedRecord.equalsIgnoreCase("no")) {
			...
			throw new Hexception(
					"provider shows an invalid deleted record support according to the OAI protocol. Invalid response: "
							+ deletedRecord);
		}
	...
	private String[] getPrefices(String baseURL) throws Hexception, OAIErrorException {
		int ii;

		String request = baseURL + "?verb=ListMetadataFormats";
		if (msgHandler != null) {
			msgHandler.statusMessage(
					"A request for ListMetadataFormats has been made. Establishing connection with the data provider...");
		}
		Document doc = getDoc(request);
	...
	/**
	 * Encode an identifier or file name segment.
	 * <p>
	 *
	 *
	 *
	...
	/**
	 * Formats a date as specified in section 3.3 of
	 * http://www.openarchives.org/OAI/2.0/openarchivesprotocol.htm.
	 * <p>
	 *
	 * If granularity is GRAN_DAY, the format is "yyyy-MM-dd". If granularity is
	 * GRAN_SECOND, the format is "yyyy-MM-ddTHH:mm:ssZ".
	 *
	 * @param granularity
	 *            GRAN_DAY or GRAN_SECOND
	 * @param dt
	 *            The Date
	...
	/**
	 * Parses a date string as specified in section 3.3 of
	 * http://www.openarchives.org/OAI/2.0/openarchivesprotocol.htm.
	 * <p>
	 *
	 * If the date contains ":", the format is "yyyy-MM-ddTHH:mm:ssZ". Otherwise the
	 ...
	 *                DESCRIPTION
	 */
	private static Date parseDate(String stg) throws Hexception {
		Date res = null;
