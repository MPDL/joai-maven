 ...
 * @version $Id: ScheduledHarvestManager.java,v 1.23.2.3 2013/05/27 04:05:39
 *          jweather Exp $
...
public final class ScheduledHarvestManager {
	 ...
	 *
	 * @param dataStore
	 *            The data store that will hold persistent data for this manager.
	 * @param initialHarvestDir
	 *            The initial directory where harvested files will be saved.
	 * @param harvestLogIndex
	 *            The harvest log index used to log harvests.
	 * @param timeOutMilliseconds
	 *            Number of milliseconds the harvester will wait for a response from
	 *            the data provider before timing out
	 */
	public ScheduledHarvestManager(SimpleDataStore dataStore, File initialHarvestDir, SimpleLuceneIndex harvestLogIndex,
	...
	/**
	 * Removes log entries showing entrytype status inprogress and replaces them
	 * with an error message indicateing that the given harvest was terminated by a
	 * server shut-down or crash. This method should only be called once upon server
	 * initialization.
	 */
	public void removeInProgresstLogEntries() {
		...
		ResultDocList harvestsInProgress = harvestLogIndex.searchDocs("entrytype:inprogress");
		...
		for (int i = 0; i < harvestsInProgress.size(); i++) {
			...
			HarvestLogWriter writer = new HarvestLogWriter(reader.getRepositoryName(), reader.getBaseUrl(),
					reader.getSet(), reader.getUid());

			updateDocs[i] = writer.logEntry(reader.getHarvestUidLong(), reader.getStartTimeLong(), -1,
					reader.getNumHarvestedRecordsInt(), reader.getNumResumptionTokensInt(),
					HarvestLogWriter.COMPLETED_SERIOUS_ERROR, reader.getHarvestDir(), reader.getZipFilePath(),
					reader.getSupportedGranularity(), reader.getDeletedRecordSupport(), reader.getOaiErrorCode(),
	...
	/**
	 * Adds a ScheduledHarvest to this manager or replaces an existing one with the
	 * same ID.
	 *
	 * @param sh
	 *            The ScheduledHarvest to add.
	 */
	 ...
	 *
	 * @param shUid
	 *            The UID of the ScheduledHarvest
	 * @param harvestAll
	 *            True to harvest all records, false to only harvest records that
	 *            have changed since the last harvest.
	 * @param harvestAllIfNoDeletedRecord
	 *            True to harvest all records from scratch if deleted records are
	 *            not supported
	 * @param override
	 *            True to perform the harvest regardless of status, false to perform
	...
	private void performHarvest(Long shUid, boolean harvestAll, boolean harvestAllIfNoDeletedRecord, boolean override) {
		...
		ScheduledHarvest sh = (ScheduledHarvest) shs.get(shUid);
		if (sh == null || (!sh.getEnabledDisabled().equalsIgnoreCase("enabled") && !override)) {
			return;
		}
		Harvester harvester;
		synchronized (this) {
			...
			IndexingHarvestMsgHandler handler = new IndexingHarvestMsgHandler(harvestLogIndex, sh.getRepositoryName(),
					sh.getBaseURL(), sh.getSetSpec(), sh.getUid().toString(), 50, sh.getHarvestDir());
		...
		Date harvestTime = doHarvest(harvester, sh.getBaseURL(), sh.getMetadataPrefix(), sh.getSetSpec(), from, null,
				dH, sh.getDoZipResult(), zipName, zD, sh.getSplitBySet(), sh, harvestAllIfNoDeletedRecord);
	 ...
	 *
	 * @param sh
	 *            A ScheduledHarvest with appropriate settings.
	 * @param from
	 *            From Date
	 * @param until
	 *            Until Date
	 * @param harvestDir
	 *            Directory where harvested records will be saved.
	 * @param harvestAllIfNoDeletedRecord
	 *            NOT YET DOCUMENTED
	 */
	private void performOneTimeHarvest(ScheduledHarvest sh, Date from, Date until, File harvestDir,
		...
		doHarvest(harvester, sh.getBaseURL(), sh.getMetadataPrefix(), sh.getSetSpec(), from, until, dH,
				sh.getDoZipResult(), zipName, zD, sh.getSplitBySet(), sh, harvestAllIfNoDeletedRecord);
	 ...
	 *            not supported
	 */
	public void oneTimeHarvest(ScheduledHarvest sh, Date from, Date until, File harvestDir,
			boolean harvestAllIfNoDeletedRecord) {
	 ...
	 *
	 * @param from
	 *            From time, or null for none.
	 * @param until
	 *            Until time, or null for none.
	 * @param harvestDir
	 *            Directory where files are saved.
	 * @param harvester
	 *            The harvester to use
	 * @param baseURL
	 *            base URL
	 ...
	 *            set
	 * @param doZip
	 *            True to perform zipping
	 * @param zipName
	 *            Zip file name
	 * @param splitBySet
	 *            True to split by set
	 * @param zDir
	 *            Zip files directory
	 * @param sh
	 *            The ScheduledHarvest
	 ...
	 *         successfully.
	 */
	private Date doHarvest(Harvester harvester, String baseURL, String metadataPrefix, String set, Date from,
			Date until, File harvestDir, boolean doZip, String zipName, String zDir, boolean splitBySet,
			ScheduledHarvest sh, boolean harvestAllIfNoDeletedRecord) {
		...
		try {
			harvester.doHarvest(baseURL, metadataPrefix, set, from, until, harvestDir.getAbsolutePath(), splitBySet,
					zipNameUsed, zDir, false, sh.getHarvestAll(), harvestAllIfNoDeletedRecord);
			...
			e.printStackTrace();
			// messageHandler.statusMessage("Internal Harvester error. Exception: " +
			// e.getMessage());
		} catch (Throwable e) {
			// messageHandler.statusMessage("Internal Harvester error. Exception: " +
			// e.getMessage());
	...
	/**
	 * Gets the date and time to begin the harvest by parsing a String in 24 hour
	 * time format such as 12:24 or 23:15.
	 *
	 * @param startTimeString
	 *            The hour and minute in 24 hour time, for example 23:15
	...
	private class HarvestThread extends Thread {
		 ...
		 *
		 * @param shUid
		 *            The ID of the ScheduledHarvest to run.
		 * @param harvestAll
		 *            True to harvest all records, false to only harvest records that
		 *            have changed since the last harvest.
		 * @param harvestAllIfNoDeletedRecord
	...
	private class OneTimeHarvestThread extends Thread {
		 ...
		 *            Directory where files are saved.
		 * @param sh
		 *            The ScheduledHarvest to run.
		 * @param harvestAllIfNoDeletedRecord
		 *            True to harvest all records from scratch if deleted records are
		 *            not supported
		 */
		public OneTimeHarvestThread(ScheduledHarvest sh, Date from, Date until, File harvestDir,
				boolean harvestAllIfNoDeletedRecord) {
			this.from = from;
			this.until = until;
			this.harvestDir = harvestDir;
