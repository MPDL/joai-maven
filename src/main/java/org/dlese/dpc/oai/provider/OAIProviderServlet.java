...
/**
 * A servlet used to configure and manage a library repository. The repository
 * may be used for OAI, for discovery or for cataloging.
 *
 * @author John Weatherley
 * @version $Id: OAIProviderServlet.java,v 1.35.2.1 2012/02/15 23:28:03 jweather
 *          Exp $
 */
public final class OAIProviderServlet extends HttpServlet {
	...
	public void init(ServletConfig conf) throws ServletException {
		...
		try {
			repositoryConfigDir = new File(GeneralServletTools
					.getAbsolutePath((String) servletContext.getInitParameter("repositoryConfigDir"), servletContext));
		} catch (Throwable e) {
		...
		// Launch and init the RepositoryManager, if it has not already been defined by
		// DDSServlet or elsewhere:
		rm = (RepositoryManager) servletContext.getAttribute("repositoryManager");
		if (rm == null) {
			rm = new RepositoryManager(repositoryConfigDir, repositoryDataDir.getAbsolutePath(), updateFrequency,
					maxNumFilesToIndex, false);
		...
		// Set the server URL that is used to display the baseURL in OAI responses and
		// elsewhere
		...
		SimpleLuceneIndex webLogIndex = new SimpleLuceneIndex(repositoryDataDir.getAbsolutePath() + "/web_log_index");
		...
		if (rm.getXMLConversionService() == null) {
			...
			try {
				XMLConversionService xmlConversionService = XMLConversionService.xmlConversionServiceFactoryForServlets(
						getServletContext(), xslFilesDirecoryPath, xmlCachDir, true);
			...
			while (enumeration.hasMoreElements()) {
				...
				if (param.toLowerCase().startsWith("fileindexingclass")) {
					...
					try {
						...
						prtlnErr("Error: setFileIndexingClasses(): could not find class '" + vals[1].trim()
								+ "'. for format " + format + " " + e);
						e.printStackTrace();
						continue;
					}
	 ...
	 *                servlet error
	 */
	private void badreq(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
