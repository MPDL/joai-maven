...
public final class OAIUtils {
	...
	/**
	 * Gets an ISO8601 UTC datestamp string of the form yyyy-MM-ddTHH:mm:ssZ from a
	 * Date.
	 *
	 * @param date
	 *            The Date
	...
	/**
	 * Converts an ISO8601 UTC datestamp String of the form yyyy-MM-ddTHH:mm:ssZ or
	 * the short form yyyy-MM-dd to a Java Date. See
	 * <a href="http://www.w3.org/TR/NOTE-datetime">ISO8601 </a> and
	 * <a href="http://www.openarchives.org/OAI/2.0/openarchivesprotocol.htm#Dates">
	 * OAI date info</a> for more info. If the short form yyyy-MM-dd is given, this
	 * method adds the String t01:00:00z to it to produce an ISO8601 compliant date
	 * time.
	 *
	 * @param datestamp
	 ...
	 *                If unable to interpret the datestamp.
	 */
	...
	/**
	 * Converts an ISO8601 UTC datastamp String of the form yyyy-MM-ddTHH:mm:ssZ or
	 * the short form yyyy-MM-dd to a Java Date. See
	 * <a href="http://www.w3.org/TR/NOTE-datetime">ISO8601 </a> and
	 * <a href="http://www.openarchives.org/OAI/2.0/openarchivesprotocol.htm#Dates">
	 * OAI date info</a> for more info. If the short form yyyy-MM-dd is given, this
	 ...
	 *            A datestamp in UTC format.
	 * @param increment
	 *            Number of seconds to increment the date, positive or negative, or
	...
	public final static Date getDateFromDatestamp(String datestamp, long increment) throws ParseException {
		...
		// Since we're using a Date parser, time begins at January 1, 1970, 00:00:00
		// GMT,
		// We want to return no matches rather than badArgument, so convert the year to
		// 1970.
		int year = 0;
		try {
	...
	/**
	 * Converts an ISO8601 UTC datastamp String of the form yyyy-MM-ddTHH:mm:ssZ to
	 * a long. See <a href="http://www.w3.org/TR/NOTE-datetime">ISO8601</a> and
	 * <a href="http://www.openarchives.org/OAI/2.0/openarchivesprotocol.htm#Dates">
	 * OAI date info</a> for more info.
	 *
	 * @param datestamp
	 *            A datestamp in UTC format.
	...
	/**
	 * Creates an appropriate directory path where harvested file(s) are saved
	 * similar to wget paths based on the URI.
	 *
	 * @param basePath
	...
	/**
	 * Creates an appropriate directory path where harvested file(s) are saved based
	 * on the baseURL, format and set. The set name is encoded for file system
	 * compliance.
	 *
	 * @param basePath
	 *            The base output directory.
	 * @param setname
	 *            The OAI setSpec harvested, or null if none.
	 * @param prefix
	 *            The metadataPrefix (format) harvested.
	 * @param baseURL
	 *            The baseURL of the OAI data provider.
	 * @return The path where the harvested files are saved.
	 */
	...
	/**
	 * Translations used in encoding identifiers and resumption tokens. Also can be
	 * used to encode file names but not directory paths.
	 * <p>
	 *
	 * See section 3.1.1.3 in
	 ...
	 *
	 */
	private final static String[][] encodings = {{"%", "%25"}, // this one must be first
			{"/", "%2F"}, {"?", "%3F"}, {"#", "%23"}, {"=", "%3D"}, {"&", "%26"}, {":", "%3A"}, {";", "%3B"},
			{" ", "%20"}, {"+", "%2B"}};
	...
	/**
	 * Encode an identifier or resumption token. Can also be used to encode chars
	...
	/**
	 * Replaces all occurances in stg of key with value. Cannot use Java's
	 * string.replaceAll since it chokes on the characters used in regular
	 * expressions.
	 *
	 * @param stg
	 ...
	 *            Key to replace
	 * @param value
	 *            Value that will replace key
	 * @return Resulting String.
	 */
	private final static String replaceall(String stg, String key, String value) {
		int ix;
		int previx;
