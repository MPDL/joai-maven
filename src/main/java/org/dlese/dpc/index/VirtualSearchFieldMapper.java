...
/**
 * 
 * Maps virtual search field/term pairs to Lucene Queries. For example, the
 * field/term pair cats:cougar might
 * 
 * be mapped to the Lucene Query (cougar OR puma OR "mountain lion"). To define
 * the mappings, the API can be
 * 
 * given one or more XML files that contain &lt;virtualSearchField&gt; elements
 * that contain XML queries
 * 
 * parsed by the {@link org.dlese.dpc.index.queryParser.XMLQueryParser} class.
 * See <a
 * 
 * href="../../../../javadoc-includes/VirtualSearchFieldMapper-sample.xml">
 * sample XML file</a> . Mappings
 * 
 * can also be added or removed directly using the API. The API returns Queries
 * for the given field/term
 * 
 * pairs, and a
 * {@link org.dlese.dpc.index.queryParser.FieldExpansionQueryParser} may be used
 * to apply the
 * 
...
public class VirtualSearchFieldMapper {
	 ...
	 * 
	 * @param parser
	 *            The QueryParser used to parse Lucene queries that are defined in
	...
	/**
	 * 
	 * Gets the Lucene Query that is defined for the given virtual field/term pair,
	 * or null if none avaialable.
	 *
	 * 
	 * 
	 * @param virtualField
	 *            The virtual search field
	 * 
	 * @param virtualTerm
	 *            The virtual search term
	 * 
	...
	/**
	 * 
	 * Gets the Lucene Query as a String for the given virtual field and term, or
	 * null if not avaialable.
	 *
	 * 
	 * 
	 * @param virtualField
	 ...
	 *            The virtual search field
	 * 
	 * @return The number of virtual terms configured for this field or zero if the
	 *         given field
	 * 
	 *         does not exist
	 * 
	 ...
	 *            The virtual search term
	 * 
	 * @return True if the given virtual field exists and the given virtual term is
	 *         configured for
	 * 
	 *         that field
	 * 
	...
	/**
	 * 
	 * Sets the Query assigned for the given field and term, overwriting any
	 * previous definition.
	 *
	 * 
	 * 
	 ...
	 * 
	 * @param query
	 *            The new Query value for this field/term
	 * 
	 */
	 ...
	 * 
	 * Sets the Query assigned for the given field and term, overwriting any
	 * previous definition. The query
	 * 
	 * String provided must conform to the grammar understood by the {@link
	 * 
	 * org.apache.lucene.queryParser.QueryParser} Object that was passed into the
	 * constructor.
	 *
	 * 
	 * 
	...
	/**
	 * 
	 * Removes the definition that is defined for the given field and term. After
	 * removal, subsequent calls to
	 * 
	 * {@link #getQuery} or {@link #getQueryString} for this field/term pair will
	 * return null.
	 *
	 * 
	 * 
	 ...
	 *            The term
	 * 
	 * @return True if a mapping existed for this field/term pair and was removed,
	 *         otherwise false
	 * 
	 */
	...
	/**
	 * 
	 * Removes all term definitions that are defined for the given field. After
	 * removal, subsequent calls to
	 * 
	 * {@link #getQuery} or {@link #getQueryString} will return null for all
	 ...
	 *            The field to remove
	 * 
	 * @return True if one or more mappings existed for this field and were removed,
	 *         otherwise
	 * 
	 *         false
	 * 
	...
	/**
	 * 
	 * Clears all field/term definitions that are defined. After calling this
	 * method, subsequent calls to {@link
	 * 
	...
	/**
	 * 
	 * Adds all virtual field/term query definitions (virtualSearchField elements)
	 * contained in the given XML
	 * 
	 ...
	 * 
	 * @param xmlConfig
	 *            The VirtualSearchFieldMapper configuration XML string
	 * 
	 * @exception Exception
	 *                If error parsing the String
	 * 
	 */
	 ...
	 * 
	 * @param xmlConfigFile
	 *            The VirtualSearchFieldMapper configuration XML file
	 * 
	 * @exception Exception
	 *                If error parsing or reading the file
	 * 
	 */
	 ...
	 * 
	 * @param xmlConfig
	 *            The URL to the VirtualSearchFieldMapper configuration XML file
	 * 
	 * @exception Exception
	 *                If error parsing or reading the url
	 * 
	 */
	...
	/**
	 * 
	 * Extracts all virtualSearchField elements from any location in the XML, and
	 * adds them to the
	 * 
	 * configuration. Note that even if an exception is thrown by this method, some
	 * mappings may have been
	 * 
	 * successfully added.
	 *
	 * 
	 * 
	 * @param node
	 *            The XML document or node to traverse
	 * 
	 * @return The number of virtualSearchFields that were found and configured
	 * 
	 * @exception Exception
	 *                If error processing one or more virtualSearchField elements
	 * 
	 */
	...
	private int addAllVirtualSearchFields(Node node) throws Exception {
		...
		if (exceptions != null) {
			...
			String msg = exceptions.size() + " error(s) found, " + numConfigured
					+ " virtualSearchField(s) successfully configured";
	...
	private void addVirtualSearchField(Element virtualSearchField) throws Exception {
		...
		if (field == null || field.trim().length() == 0)
			...
			throw new Exception(
					"Error parsing document: element <virtualSearchField> must have a single, non-empty 'field' attribute. Error found at "
							+ virtualSearchField.getPath());
	...
	private void addVirtualSearchTermDefinition(Element virtualSearchTermDefinition, String field) throws Exception {
		...
		if (term == null || term.trim().length() == 0)
			...
			throw new Exception(
					"Error parsing document: element <virtualSearchTermDefinition> must have a single, non-empty 'term' attribute. Error found at "
							+ virtualSearchTermDefinition.getPath());
		...
		if (virtualDefinitionElement == null || virtualDefinitionElement.size() == 0)
			...
			throw new Exception(
					"Error parsing document: <virtualSearchTermDefinition> child element '<Query>' is empty or missing. Error found at "
							+ virtualSearchTermDefinition.getPath());

		if (virtualDefinitionElement.size() > 1)

			throw new Exception(
					"Error parsing document: element <virtualSearchTermDefinition> must contain a single child element within the '<Query>' element but contains more than one. Error found at "
							+ virtualSearchTermDefinition.getPath());
	...
	/**
	 * 
	 * Outputs a String representation of the fields/terms/queries that are defined
	 * in this
	 * 
