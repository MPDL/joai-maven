...
/**
 * Creates a Lucene {@link org.apache.lucene.document.Document} from any XML
 * file by stripping the XML tags to extract and index the content. The reader
 * for this type of Document is XMLDocReader.
 * <p>
 *
 * <b>The Lucene Document fields that are created by this class are (in addition
 * the the ones listed for
 * {@link org.dlese.dpc.index.writer.FileIndexingServiceWriter}):</b> <br>
 * <br>
 * <code><b>collection</b> </code> - The collection associated with this
 * resource. <br>
 *
 *
...
public abstract class XMLFileIndexingWriter extends FileIndexingServiceWriter {
	...
	/**
	 * Returns the ids for the item being indexed. If more than one record catalogs
	 * the same item, this represents the primary ID.
	 *
	...
	/**
	 * Returns the unique primary record ID for the item being indexed. If more than
	 * one record catalogs the same item, this represents the primary ID.
	 *
	 * @return The id String
	 * @exception Exception
	 *                If error
	...
	/**
	 * Gets the ids of related records. The Map key contains the relationship
	 * (isAnnotatedBy, etc.) and the Map value contains a List of Strings that
	 * indicate the ids of the target records.
	 *
	 * @return The related ids value, or null if none
	 * @exception IllegalStateException
	 *                If called prior to calling method #indexFields
	...
	/**
	 * Gets the urls of related records. The Map key contains the relationship
	 * (isAnnotatedBy, etc.) and the Map value contains a List of Strings that
	 * indicate the urls of the target records.
	 *
	 * @return The related urls value, or null if none
	 * @exception IllegalStateException
	 *                If called prior to calling method #indexFields
	...
	/**
	 * Returns unique collection keys for the item being indexed. For example "dcc"
	 * (single collection) or "dcc dwel" (multiple collections). If more than one
	 * collection is provided, the first one must be the primary collection. May be
	 * overridden by sub-classes as appropriate (overridden by
	...
	/**
	 * Return the geospatial BoundingBox footprint that represnets the resource
	 * being indexed, or null if none apply. Override if nessary.
	 *
	...
	/**
	 * This method is called prior to processing and may be used to for any
	 * necessary set-up. This method should throw and exception with appropriate
	 * message if an error occurs.
	 *
	 * @param source
	 *            The source file being indexed
	 * @param existingDoc
	 *            An existing Document that currently resides in the index for the
	 *            given resource, or null if none was previously present
	 * @exception Exception
	 *                If an error occured during set-up.
	 */
	public abstract void init(File source, Document existingDoc) throws Exception;

	/**
	 * Return unique IDs for the item being indexed, one for each collection that
	 * catalogs the resource. For example "DLESE-000-000-000-001" (single ID) or
	 * "DLESE-000-000-000-036 COMET-60" (multiple IDs). If more than one ID is
	 * present, the first one is the primary.
	 *
	 * @return The id(s)
	 * @exception Exception
	 *                This method should throw and Exception with appropriate error
	...
	/**
	 * Return a title for the document being indexed, or null if none applies. The
	 * String is tokenized, stored and indexed under the field key 'title' and is
	 * also indexed in the 'default' field.
	 *
	 ...
	 * The String is tokenized, stored and indexed under the field key 'description'
	 * and is also indexed in the 'default' field.
	 *
	...
	/**
	 * Return the URL(s) to the resource being indexed, or null if none apply. If
	 * more than one URL references the resource, the first one is the primary. The
	 * URL Strings are tokenized and indexed under the field key 'uri' and is also
	 * indexed in the 'default' field. It is also stored in the index untokenized
	 * under the field key 'url.'
	 *
	 ...
	 * fields, false if handled by the sub-class. If true, the content is indexed
	 * using the #addToDefaultField method.
	 *
	 * @return True to have the full XML content indexed in the 'default' and
	 *         'stems'
	 */
	public abstract boolean indexFullContentInDefaultAndStems();

	/**
	 * Returns the date used to determine "What's new" in the library, or null if
	 * none is available.
	 *
	...
	/**
	 * Returns the type of category for "What's new" in the library, or null if none
	 * is available. Must be a simple lower case String with no spaces, for example
	 * 'itemnew,' 'itemannocomplete,' 'itemannoinprogress,' 'annocomplete,'
	 * 'annoinprogress,' 'collection'.
	 *
	 ...
	 * When implementing this method, use the add method of the
	 * {@link org.apache.lucene.document.Document} class to add a
	 * {@link org.apache.lucene.document.Field}.
	 * <p>
	 *
	 * The following Lucene {@link org.apache.lucene.document.Field} types are
	 * available for indexing with the
	 ...
	 * <br>
	 * Field.UnIndexed(string name, string value) -- not tokenized, not indexed,
	 * stored<br>
	 * Field(String name, String string, boolean store, boolean index, boolean
	 * tokenize) -- allows control to do anything you want
	 * <p>
	 *
	 * Example code:<br>
	 ...
	 * &nbsp;<code> newDoc.add(Field.Text("mycustomefield", customContent));</code>
	 * <br>
	 * <code>}</code>
	 *
	 * @param newDoc
	 *            The new {@link org.apache.lucene.document.Document} that is being
	 *            created for this resource
	 * @param existingDoc
	 *            An existing {@link org.apache.lucene.document.Document} that
	 *            currently resides in the index for the given resource, or null if
	...
	/**
	 * Adds the full content of the XML to the default search field. Strips the XML
	 * tags to extract the content. Will not work properly if the XML is not
	 * well-formed.
	 * <p>
	 *
	 *
	...
	protected void addCustomFields(Document newDoc, Document existingDoc, File sourceFile) throws Exception {
		...
		// If no ID provided by sub-class, use the file name: simply remove the ".xml"
		// from the end of the filename to get the ID.
		...
		// If indicated by sub-class, index the 'default', 'admindefault', and 'stems'
		// fields with the full XML content (Note: This performs the addToDefaultField
		// and addToAdminDefaultField):
		...
		// --- Index things this item relates to (is an annotation for, etc), e.g.
		// isRelatedTo:
		...
		if (relatedIdsMap != null) {
			...
			while (it.hasNext()) {
				...
				// prtln("processing id relation: relationshipName: " + relationshipName + "
				// ids: " + Arrays.toString(ids.toArray()));

				// Index the IDs so these docs can be retrieved later:
				for (int i = 0; i < ids.size(); i++) {
					itemAssignsRelationships = true;
					// newDoc.add(new Field("indexedRelationIds.isRelatedTo", ids.get(i).toString(),
					// Field.Store.YES, Field.Index.NOT_ANALYZED));
					newDoc.add(new Field("assignsRelationshipById." + relationshipName, ids.get(i).toString(),
							Field.Store.YES, Field.Index.NOT_ANALYZED));
					newDoc.add(new Field("assignsRelationshipById", ids.get(i).toString(), Field.Store.YES,
							Field.Index.NOT_ANALYZED));
				}
				// newDoc.add(new Field("indexedRelations", "isRelatedTo", Field.Store.YES,
				// Field.Index.NOT_ANALYZED));
				newDoc.add(new Field("assignedRelationshipsById", relationshipName, Field.Store.YES,
						Field.Index.NOT_ANALYZED));
			}
		} else {
		...
		if (relatedUrlsMap != null) {
			...
			while (it.hasNext()) {
				...
				// prtln("processing url relation: relationshipName: " + relationshipName + "
				// urls: " + Arrays.toString(ids.toArray()));
				...
				for (int i = 0; i < urls.size(); i++) {
					...
					// newDoc.add(new Field("indexedRelationIds.isRelatedTo", ids.get(i).toString(),
					// Field.Store.YES, Field.Index.NOT_ANALYZED));
					newDoc.add(new Field("assignsRelationshipByUrl." + relationshipName, urls.get(i).toString(),
							Field.Store.YES, Field.Index.NOT_ANALYZED));
					newDoc.add(new Field("assignsRelationshipByUrl", urls.get(i).toString(), Field.Store.YES,
							Field.Index.NOT_ANALYZED));
				}
				// newDoc.add(new Field("indexedRelations", "isRelatedTo", Field.Store.YES,
				// Field.Index.NOT_ANALYZED));
				newDoc.add(new Field("assignedRelationshipsByUrl", relationshipName, Field.Store.YES,
						Field.Index.NOT_ANALYZED));
			}
		}

		// Mark if this item assigns a relationship:
		newDoc.add(new Field("assignedRelationshipIsDefined", (itemAssignsRelationships ? "true" : "false"),
				Field.Store.YES, Field.Index.NOT_ANALYZED));

		// JW - Should we assitn the "isRelatedTo" relationship to this?
		if (itemAssignsRelationships) {
			// luceneDoc.add(new Field("indexedRelations", "isRelatedTo", Field.Store.YES,
			// Field.Index.NOT_ANALYZED));
		}

		// ------ [end] Standard XML indexing handled by XMLIndexer ------------

		// ------ Index relations for this item (records that have a relation to this
		// (isAnnotatedBy, etc.) ------------
		...
		if (recordDataService != null) {
			...
			fieldContent = "";
			ArrayList completedAnnoCollectionKeys = recordDataService
					.getCompletedAnnoCollectionKeysFromResultDocs(myAnnoResultDocs);
			if (completedAnnoCollectionKeys != null && completedAnnoCollectionKeys.size() > 0) {
				...
				// prtln("itemannocompletedcollectionkeys for " + this.getId() + " is: " +
				// fieldContent);
				newDoc.add(new Field("itemannocompletedcollectionkeys", fieldContent, Field.Store.YES,
		...
		if (collections != null && collections.length > 0) {
			...
			if (!getCollections()[0].equals("configuredcollections")) {
				newDoc.add(new Field(getFieldName("key", "dlese_collect"),
						getFieldContent(collections, "key", "dlese_collect"), Field.Store.YES, Field.Index.ANALYZED));
			}
		}

		// Store the ID for the collection I am a member of. (The first time the index
		// is built, the DocReader for the 'collect' collection is not available):
		...
		if (wndate != null)
			newDoc.add(new Field("wndate", DateFieldTools.dateToString(wndate), Field.Store.YES,
					Field.Index.NOT_ANALYZED));
		String wntype = getWhatsNewType();
		if (wntype != null)
			newDoc.add(new Field("wntype", wntype, Field.Store.YES, Field.Index.ANALYZED));

		// ------ Check that this record does not already exist in the repository, throw
		// ErrorDocException if needed ------
		if (!isMakingDeletedDoc()) {
			...
			if (ids != null && ids.length > 0) {
				...
				for (int i = 0; i < ids.length; i++) {
					...
					// Verify that this record's ID (the first ID) does not already exist in the
					// repository:
					if (i == 0) {

						File recordIdAlreadyInRepository = null;

						// Check for duplicate IDs in the repository (not de-duping, but actual dup
						// IDs), if not temp file for checking:
						if (!getSourceFile().getName().equals("temp_dlese_repository_put_record_file.xml")) {
							...
							if (recordIdAlreadyInRepository == null) {
								...
								idQ.add(new TermQuery(new Term("docsource", getDocsource())),
								...
								if (results != null && results.size() > 0) {
									...
									if (reader instanceof XMLDocReader) {
										...
										if (xmlDocReader.getId().equals(ids[i])) {
											...
											// prtln("Dup ID not matched for ID '" + getIds()[i] + "' found instead '" +
											// xmlDocReader.getId() + "'");
										}
									}
								}
							...
							// Check if another file with the same ID exists in the current indexing
							// session:
							if (recordIdAlreadyInRepository == null) {
								...
								if (sessionAttributes != null) {
									...
									if (fileInDirectory != null && getSourceFile().exists()
											&& fileInDirectory.exists()) {
										// prtln("found dup ID in same dir: " + getIds()[i] + " dup path: " +
										// fileInDirectory.getAbsolutePath() + " my path: " +
										// this.getSourceFile().getAbsolutePath());
							...
							// If this ID is already in the repository, throw an Exception and store the dup
							// info:
							if (recordIdAlreadyInRepository != null) {
								errorDocException = makeErrorDocException(getIds()[i], idsEncoded[i],
										recordIdAlreadyInRepository);
							} else {
			 ...
			 * if(encodedIdsString != null) { addDocToRemove("id", encodedIdsString.trim());
			 * }
			 */
	...
	private void indexRelations(Document luceneDoc) throws Exception {
		...
		try {
			...
			ResultDocList relatedDocs = getIndex().searchDocs(idQ);
			if (relatedDocs == null || relatedDocs.size() == 0) {
				...
				for (int i = 0; i < relatedDocs.size(); i++) {
					...
					List myRelationTypes = (List) xmlDocReader.getAssignedRelationshipsForItemsMap()
							.get(getPrimaryId());

					// prtln("indexRelations() for id: " + getPrimaryId() + " myRelationTypes:" +
					// (myRelationTypes == null ? " null" :
					// Arrays.toString(myRelationTypes.toArray())));
					...
					if (myRelationTypes != null) {
						for (int j = 0; j < myRelationTypes.size(); j++) {
							...
							XMLIndexer xmlIndexer = new XMLIndexer(xmlDocReader.getXml(), xmlDocReader.getDoctype(),
							...
							// Index the IDs so these docs can be retrieved later:
							luceneDoc.add(new Field("indexedRelationIds." + relationType, xmlDocReader.getId(),
									Field.Store.YES, Field.Index.NOT_ANALYZED));
							luceneDoc.add(new Field("indexedRelations", relationType, Field.Store.YES,
									Field.Index.NOT_ANALYZED));
			...
			luceneDoc.add(new Field("hasIndexedRelation", (hasIndexedRelation ? "true" : "false"), Field.Store.YES,
					Field.Index.NOT_ANALYZED));
		}
	}
	 ...
	 *
	 * @param relatedDocs
	 *            An array of ResultDocs that contain the records that are related
	 *            to this one
	 * @param luceneDoc
	 *            The Document to add the fields to
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	/*
	 * private void indexRelationZZZ(ResultDoc[] relatedDocs, Document luceneDoc)
	 * throws Exception { if (relatedDocs != null && relatedDocs.length > 0) { List
	 * relatedIds = new ArrayList(); for (int i = 0; i < relatedDocs.length; i++) {
	 * XMLDocReader xmlDocReader = (XMLDocReader) (relatedDocs[i].getDocReader());
	 * 
	 * String relationType = "isAnnotatedBy";
	 * 
	 * // Index all xPaths for this item XMLIndexer xmlIndexer = new
	 * XMLIndexer(xmlDocReader.getXml(), xmlDocReader.getDoctype(),
	 * getXmlIndexerFieldsConfig()); xmlIndexer.setXPathFieldsPrefix("/relation." +
	 * relationType + "/");
	 * 
	 * // Index just the XPath fields: xmlIndexer.indexXpathFields(luceneDoc);
	 * relatedIds.add(xmlDocReader.getId()); }
	 * 
	 * // Index the IDs so these docs can be retrieved later: for (int i = 0; i <
	 * relatedIds.size(); i++) { luceneDoc.add(new Field("indexedRelationIds." +
	 * relationType, relatedIds.get(i).toString(), Field.Store.YES,
	 * Field.Index.NOT_ANALYZED)); }
	 * 
	 * luceneDoc.add(new Field("indexedRelations", relationType, Field.Store.YES,
	 * Field.Index.NOT_ANALYZED));
	 ...
	 *
	 * @param annoResultDocs
	 *            An array of anno ResultDocs
	 * @param newDoc
	 *            The Document to add the fields to
	 */
	private void indexAnnoRatings(ResultDocList annoResultDocs, Document newDoc) {
		...
		newDoc.add(new Field("itemannonumratings", new DecimalFormat("00000").format(numRatings), Field.Store.YES,
				Field.Index.NOT_ANALYZED));

		// A String of all the ratings assigned to this resource, as numbers (e.g. '1 1
		// 2 4 2 3')
		...
		if (numRatings > 0 || totalRating > 0) {
			...
			// prtln("ave rating: " + aveRating + " string: " +
			// formatter.format(aveRating));
			newDoc.add(new Field("itemannoaveragerating", formatter.format(aveRating), Field.Store.YES,
					Field.Index.NOT_ANALYZED));
		}
	}

	private ErrorDocException makeErrorDocException(String id, String encodedId, File recordIdAlreadyInRepository) {
		ErrorDocException errorDocException = new ErrorDocException(
				"The ID '" + id + "' that was found in this file is the same as the ID already found in file '"
						+ recordIdAlreadyInRepository + "'",
				"dupIdError");
		...
		// prtln("Throwing an ErrorDocException for dup ID: " + id + " path: " +
		// recordIdAlreadyInRepository.getAbsolutePath());
	 ...
	 *
	 * @param existingDoc
	 *            An existing FileIndexingService Document that currently resides in
	 *            the index for the given file
	 * @return A Lucene FileIndexingService Document
	 * @exception Throwable
	 *                Thrown if error occurs
	 */
	public synchronized Document getDeletedDoc(Document existingDoc) throws Throwable {
		...
		try {
			...
			if (resultDocs != null && resultDocs.size() > 0) {
				...
				if (!existingDocReader.getDocsource().equals(recordInRepository.getDocsource())) {
					// prtln("getDeletedDoc() There was an existing doc in the index in another
					// location, not deleting");
					return null;
				} else {
					// prtln("getDeletedDoc() The existing doc in the index is this one...
					// deleting");
				}
			}

			// Make a new Document and return it
			XMLFileIndexingWriterFactory xmlFileIndexingWriterFactory = new XMLFileIndexingWriterFactory(
					getRecordDataService(), getIndex(), getXmlIndexerFieldsConfig());
			XMLFileIndexingWriter xmlFileIndexingWriter = xmlFileIndexingWriterFactory
					.getIndexingWriter(recordInRepository.getCollection(), recordInRepository.getNativeFormat());
			xmlFileIndexingWriter.setIsMakingDeletedDoc(true);
			FileIndexingServiceData fileIndexingServiceData = xmlFileIndexingWriter.create(null, existingDoc,
					getFileIndexingPlugin(), getSessionAttributes());
			newDocument = fileIndexingServiceData.getDoc();
			if (newDocument != null) {
				// prtln("Returning a deletedDoc for id: " + recordInRepository.getId() + "
				// file: " + recordInRepository.getDocsource());
			}
		} catch (Throwable t) {
			// t.printStackTrace();
			throw t;
	...
	protected ResultDocList getMyAnnoResultDocs() throws Exception {
		if (_myAnnoResultDocs == null) {
			RecordDataService recordDataService = getRecordDataService();
			if (recordDataService != null) {
				// Get annotation for this record only. If I am a multi-doc, these (should)
				// include all annos for all records
	...
	/**
	 * Gets the XMLIndexerFieldsConfig to use for XML indexing, or null if none
	 * available.
	 *
	...
	/**
	 * Gets the vocab encoded keys for the given values, separated by the '+'
	 * symbol.
	 *
	 * @param values
	 *            The valuse to encode.
	 * @param useVocabMapping
			...
			if (str.length() > 0) {
				...
				if (useVocabMapping != null && vocab != null) {
					try {
						...
						ret.append(str);
						// prtlnErr("Warning: Unable to get vocab mapping for '" + useVocabMapping + ":"
						// + str + "'. Using unencoded value instead. Reason: " + t);
					}
				} else {
					ret.append(str);
				}
	...
	/**
	 * Gets the field ID, for example 'gr', for a given vocab, for example
	 * 'gradeRange'. If unable to get the field ID, the vocab field String is
	 * returned unchanged.
	 *
	 * @param vocabFieldString
	 *            The field, for example 'gradeRange'
	 * @param metadataFormat
	 *            The metadata format, for example 'adn'
	...
	protected String getFieldName(String vocabFieldString, String metadataFormat) throws Exception {
		...
		if (vocab == null) {
			...
			try {
				...
				fieldName = vocabFieldString;
				// prtlnErr("Warning: Unable to get vocab mapping for '" + vocabFieldString +
				// "'. Using unencoded value instead. Reason: " + t);
			}
		}
	...
	/**
	 * Gets the appropriate terms from a string array of metadata fields. Uses all
	 * terms found after the last colon ":" found in the string.
	 *
	 * @param vals
	...
	/**
	 * Gets the DLESECollectionDocReader for the collection in which this item is a
	 * part, or null if not available.
	 *
	...
	protected DleseCollectionDocReader getMyCollectionDoc() {
		if (myCollectionDocReader == null) {
			...
			try {
				...
				// When the index is first built, the 'collect' collection is not avaialble
				// until it is written to the index
	...
	/**
	 * Gets the oaiModtime for the given File or Document, set to 3 minutes in the
	 * future to account for any delay in indexing updates.
	 *
	 * @param sourceFile
	 *            The source file
	 * @param existingDoc
