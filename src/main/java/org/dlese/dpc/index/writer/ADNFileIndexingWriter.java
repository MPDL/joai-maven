...
/**
 * Creates a Lucene {@link org.apache.lucene.document.Document} from an ADN-item
 ...
 * <br>
 * <code><b>doctype</b> </code> - Set to 'adn'. Stored. Note: the actual
 * indexing of this field happens in the superclass
 * {@link org.dlese.dpc.index.writer.FileIndexingServiceWriter}.<br>
 * <code><b>additional fields</b> </code> - A number of additional fields are
 * defined. See the Java code for method
 * {@link #addFrameworkFields(Document, Document)} for details. <br>
 * <br>
 *
 *
...
public class ADNFileIndexingWriter extends ItemFileIndexingWriter {
	...
	/**
	 * Create a ADNFileIndexingWriter that indexes the given collection in field
	 * collection.
	 */
	 ...
	 *
	 * @param isDupDoc
	 *            False to force this to be processed as a non-dup
	 */
	public ADNFileIndexingWriter(boolean isDupDoc) {
		// prtln("ADNFileIndexingWriter() isDupDoc: " + isDupDoc + " NumInstances: " +
		// num_instances);
		num_instances++;
	 ...
	 */
	public void initItem(File source, Document existingDoc) throws Exception {
		...
		try {
			...
			// Design note: If making a deleted doc, get content from getFileContent(), not
			// the file:
			if (isMakingDeletedDoc()) {
				myXmlDoc = new XMLDoc();
				myXmlDoc.useXmlString(getFileContent(), this.isValidationEnabled(), true, true);
			} else {
				myXmlDoc = new XMLDoc("file:///" + source.getAbsolutePath(), this.isValidationEnabled(), true, true);
			}

			FileIndexingService fileIndexingService = getFileIndexingService();
			if (fileIndexingService != null) {
				// If the indexer has not been configured to de-dup the records, then don't use
				// the de-duper
			...
			/*
			 * if(isDupDoc) prtln("indexing dup ID: " + myID + " singleRecord:" +
			 * singleRecord + " isDupDoc:" + isDupDoc); else prtln("indexing ID: " + myID +
			 * " singleRecord:" + singleRecord + " isDupDoc:" + isDupDoc);
			 */
			...
			if (recordDataService != null && !recordDataService.isIdMapperDisabled()) {
				...
				if (tmpMyMmdRec == null) {
					...
					// throw new MmdException("Id mapper service has no entry for ID:" + myID + " in
			...
			if (recordDataService == null || singleRecord || !hasIdMapperData) {
				...
				/*
				 * if(recordDataService == null)
				 * prtln("indexing as single doc because recordDataService is null"); else
			...
			// Index a potential multi-doc.
				...
				if (qualifiedAssociatedMmdRecs != null && qualifiedAssociatedMmdRecs.length > 0) {
					...
					for (int i = 0; i < qualifiedAssociatedMmdRecs.length; i++) {
						...
						if (!collectionsWithMultiDups.contains(collKey)) {
							// prtln(myID + " has a legit dup: " + qualifiedAssociatedMmdRecs[i].getId() + "
							// from " + qualifiedAssociatedMmdRecs[i].getCollKey());
							mmdRecsList.add(qualifiedAssociatedMmdRecs[i]);
						}
						// else
						// prtln(myID + " has a NON-legit dup: " + qualifiedAssociatedMmdRecs[i].getId()
						// + " from " + qualifiedAssociatedMmdRecs[i].getCollKey());
					}
				 ...
				 * ); else prtln(
				 * "Dup docs were found by IDMapper. qualifiedAssociatedMmdRecs.length = " +
				 * qualifiedAssociatedMmdRecs.length);
				 */
				// Check for dups in the same collection and throw an error
				if (true) {
					/*
					 * Issue: if we index more than one item in a given collection that is
					 * identified as a dup, other collections that have a record that is also a dup
					 * for the record will index ALL of them in one big multi-doc. The totals for a
					 * given collection will not be correct.
					 */
					MmdRec[] dupRecsInMyCollection = recordDataService
							.getAssociatedMMDRecs(RecordDataService.QUERY_SAME, myID, super.getCollections()[0]);
					if (dupRecsInMyCollection != null && dupRecsInMyCollection.length > 0) {
						...
						if (!dupRecsInMyCollection[0].getId().equals(myID)) {
							...
							String message = "Dup records were found in collection " + super.getCollections()[0]
									+ ". IDs " + m + " appear to catalog the same resource. " + "Record ID "
									+ dupRecsInMyCollection[0].getId() + " is being indexed and not this one.";
							// prtln(message);
				...
				if (qualifiedAssociatedMmdRecs == null || qualifiedAssociatedMmdRecs.length == 0)
					...
					for (int i = 0; i < qualifiedAssociatedMmdRecs.length; i++) {
						// prtln("Found associated dup record for id " + myID + ": " +
						// qualifiedAssociatedMmdRecs[i].getId() + " " +
						// qualifiedAssociatedMmdRecs[i].getCollKey());
						try {
					...
					if (adnXmlDoc.length > 1) {
						...
						for (int i = 0; i < additionalIds.size(); i++) {
							...
							indexIndividualDupDoc((String) additionalIds.get(i), (String) additionalCollections.get(i),
									dupDocFile);
						}
					}
					// prtln("A total of " + adnXmlDoc.length + " existing associated records were
					// found for id " + myID);
				}
			}
	...
	/**
	 * Returns unique collection keys for the item being indexed, separated by
	 * spaces. For example 'dcc,' 'comet' or 'dwel'. Since this may be a multi-doc,
	 * it may have multiple collections, so overridding the default getCollection()
	 * method.
	 *
	 ...
	 * accessioned. If this is a multi-doc, returns the oldest accession date of the
	 * bunch, corresponding to the first time this resource appeared in the library.
	 *
	...
	protected Date getAccessionDate() throws Exception {
		try {
			...
			for (int i = 0; i < adnXmlDoc.length; i++)
				accessionDates[i] = MetadataUtils
						.parseUnionDateType(adnXmlDoc[i].getXmlString("metaMetadata/dateInfo@accessioned"));
	...
	protected Date getCreationDate() throws Exception {
		try {
			...
			for (int i = 0; i < adnXmlDoc.length; i++)
				creationDates[i] = MetadataUtils
						.parseUnionDateType(adnXmlDoc[i].getXmlString("metaMetadata/dateInfo@created"));
	 ...
	 * class that is used to read this type of
	 * {@link org.apache.lucene.document.Document}, which is "ItemDocReader".
	 *
	 ...
	 * resource, not including myMmdRec.
	 *
	...
	protected MmdRec[] getAssociatedMmdRecs() {
		...
		if (recordDataService != null && associatedMmdRecs == null) {
			try {
				associatedMmdRecs = recordDataService.getAssociatedMMDRecs(RecordDataService.QUERY_OTHER, myID,
						super.getCollections()[0]);
			} catch (Exception e) {
	...
	/**
	 * Returns the MmdRecs for all records that catalog this resouce, including
	 * myMmdRec.
	 *
	...
	/**
	 * Gets the id(s) for this item. If multiple IDs exists, the first one is the
	 * primary.
	 *
	 * @return The id value
	 * @exception Exception
	...
	/**
	 * Returns the item's cost. The String is stored and indexed under the field key
	 * 'cost'.
	 *
	 ...
	 *
	 * @param newDoc
	 *            The feature to be added to the FrameworkFields attribute
	 * @param existingDoc
	 *            The feature to be added to the FrameworkFields attribute
	 * @exception Exception
	 *                If an error occurs
	...
	protected final void addFrameworkFields(Document newDoc, Document existingDoc) throws Exception {
		...
		if (terms != null && terms.trim().length() > 0)
			newDoc.add(new Field(getFieldName("gradeRange", metadataFormat), terms, Field.Store.YES,
		...
		if (terms != null && terms.trim().length() > 0)
			newDoc.add(new Field(getFieldName("resourceType", metadataFormat), terms, Field.Store.YES,
					Field.Index.ANALYZED));
		...
		if (terms != null && terms.trim().length() > 0)
			newDoc.add(
					new Field(getFieldName("subject", metadataFormat), terms, Field.Store.YES, Field.Index.ANALYZED));
		...
		if (terms != null && terms.trim().length() > 0)
			newDoc.add(new Field(getFieldName("contentStandard", metadataFormat), terms, Field.Store.YES,
					Field.Index.ANALYZED));
	...
	/**
	 * Sets the whether this writer should write a single record doc rather than a
	 * multi-item doc.
	 *
	 * @param isSingleDoc
	 *            The new isSingleDoc value
	 */
	...
	private void indexIndividualDupDoc(String id, String coll, File sourceFile) throws Exception {
		SimpleLuceneIndex dupItemsIndex = (SimpleLuceneIndex) getFileIndexingService().getAttribute("dupItemsIndex");
		...
		/*
		 * prtln("ID terms:\n"); List idTerms = dupItemsIndex.getTerms("id"); for(int i
		 * = 0; i< idTerms.size();i++) prtln("   " + (String)idTerms.get(i));
		 */
	...
	// -------------- Essential controlled vocabs (These should be moved to
	// itemFileInexingWriter... ----------------
	...
	/**
	 * Gets the oraganizations institution name. ADN xPath
	 * lifecycle/contributors/contributor/organization/instName
	 *
	...
	/**
	 * Gets the persons institution name. ADN xPath
	 * lifecycle/contributors/contributor/person/instName
	 *
	...
	/**
	 * Gets the persons institution department name. ADN xPath
	 * lifecycle/contributors/contributor/person/instDept
	 *
	...
	/**
	 * Takes the XPath provided and extracts each node at that level from each of
	 * the multi-docs and converts it to an indexable phrase.
	 *
	 * @param xPath
	 *            The XPath
	...
	/**
	 * Gets all place names as text. Place names are extracted from the following
	 * XPaths: <nobr> general/simplePlacesAndEvents/placeAndEvent/place,</nobr>
	 * <nobr>
	 * geospatialCoverages/geospatialCoverage/boundBox/bbPlaces/place/name</nobr>
	 * and <nobr>
	...
	protected String getPlaceNames() {
		...
		for (int i = 0; i < adnXmlDoc.length; i++) {
			...
			tmp = adnXmlDoc[i]
					.getXmlFields("geospatialCoverages/geospatialCoverage/detGeos/detGeo/detPlaces/place/name");
			if (tmp != null)
				for (int j = 0; j < tmp.length; j++)
					if (!vals.contains(tmp[j]))
						vals.add(tmp[j]);
	...
	/**
	 * Gets all event names as text. Event names are extracted from the following
	 * XPaths: <nobr> general/simplePlacesAndEvents/placeAndEvent/event,</nobr>
	 * <nobr>
	 * geospatialCoverages/geospatialCoverage/boundBox/bbEvents/event/name</nobr>
	 * and <nobr>
	...
	protected String getEventNames() {
		...
		for (int i = 0; i < adnXmlDoc.length; i++) {
			...
			tmp = adnXmlDoc[i]
					.getXmlFields("geospatialCoverages/geospatialCoverage/detGeos/detGeo/detEvents/event/name");
			if (tmp != null)
				for (int j = 0; j < tmp.length; j++)
					if (!vals.contains(tmp[j]))
						vals.add(tmp[j]);
	...
	/**
	 * Gets all temporal coverage names as text. Temporal coverage names are
	 * extracted from the following XPaths:
	 * <nobr>general/simpleTemporalCoverages/description,</nobr> and <nobr>
