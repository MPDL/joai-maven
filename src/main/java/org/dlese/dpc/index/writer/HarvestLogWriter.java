 ...
 * <code><b>readerclass</b> </code> - The class which is used to read
 * {@link org.apache.lucene.document.Document}s created by this writer, which is
 * 'WebLogReader'.<br>
 * <code><b>admindefaultfield</b> </code> - The default field that holds all
 * content for searching. 'WebLogReader'.<br>
 * <br>
 *
 *
...
public class HarvestLogWriter implements DocWriter {
	 ...
	 * {@link org.apache.lucene.document.Document}, which is "HarvestLogReader".
	 *
	 ...
	 *
	 * @param from
	 *            The from date or null if none used
	 * @param until
	 *            The until date or null if none used
	 */
	 ...
	 *            Num resumption tokens issued
	 * @param messageType
	 *            The message type
	 * @param harvestedRecordsDir
	 *            Dir where harvested records are saved
	 * @param zipFilePathName
	 *            Zip file
	 * @param supportedGranularity
	 *            Granularity
	 * @param deletedRecordSupport
	 *            Deleted record support
	 * @param oaiErrCode
	 *            OAI error code, or null
	 * @param message
	 *            Message to be logged
	 * @return Document to be inserted into the index
	 */
	public final Document logEntry(long harvestUid, long startTime, long endTime, int recordCount, int resumptionCount,
			int messageType, String harvestedRecordsDir, String zipFilePathName, String supportedGranularity,
			String deletedRecordSupport, String oaiErrCode, String message) {
		...
		try {
			...
			doc.add(new Field("numharvestedrecords", Integer.toString(recordCount), Field.Store.YES,
					Field.Index.ANALYZED));
			doc.add(new Field("numresumptiontokens", Integer.toString(resumptionCount), Field.Store.YES,
					Field.Index.ANALYZED));
			try {
				doc.add(new Field("starttime", DateFieldTools.timeToString(startTime), Field.Store.YES,
						Field.Index.ANALYZED));
			} catch (Throwable e) {
				doc.add(new Field("starttime", "-1", Field.Store.YES, Field.Index.ANALYZED));
			}
			try {
				doc.add(new Field("endtime", DateFieldTools.timeToString(endTime), Field.Store.YES,
						Field.Index.ANALYZED));
			} catch (Throwable e) {
	...
	private Document addRequiredFields(Document doc, StringBuffer adminDefaultBuffer) {
		...
		addToAdminDefaultField(new SimpleDateFormat("EEE MMM d, yyyy h:mm:ss a zzz").format(new Date(curTime)),
				adminDefaultBuffer);
