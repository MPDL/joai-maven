...
/**
 * Holds a configuration for XML fields indexed by XMLIndexer and used in
 * PerFieldAnalyzer.
 *
...
public class XMLIndexerFieldsConfig {
	/*
	 * TO DO: - add config for indexing in XMLIndexer - document
	 */
			...
			for (int i = 0; i < nodes.size(); i++) {
				try {
					...
					if (xmlFormat.length() > 0 && schema.trim().length() > 0) {
						prtlnErr("Not valid to specify both an XML format and a schema. Skipping xmlFormat: '"
								+ xmlFormat + "' schema: '" + schema + "'");
						continue;
						...
						if (fieldName.length() > 0) {
							if (analyzer.length() > 0)
								...
								if (type.equals("text"))
									...
									throw new Exception("Valid field type or analyzer must be specified for field '"
											+ fieldName + "'. Found type:'" + type + "' analyzer:'" + analyzer + "'");
							}
						}
					}
					// prtln("fieldAnalyzers: " + fieldAnalyzers + " size: " +
					// fieldAnalyzers.size());
				} catch (Exception e) {
	...
	/**
	 * Gets a Map of field/analyzer pairs where keys are field or schema names and
	 * values are the corresponding Analyzer class names as Strings for the custom
	 * fields that are defined in this configuration.
	 *
	...
	/**
	 * Gets the configuration Document for a given xmlFormat or schema. Example
	 * xmlFormat keys are 'oai_dc', 'library_dc'. An example schema is
	...
	/**
	 * Determine if the given xmlFormat or schema has a configuration. Example
	 * xmlFormat keys are 'oai_dc', 'library_dc'. An example schema is
	 * 'http://www.openarchives.org/OAI/2.0/oai_dc.xsd'.
	 *
	 * @param xmlFormatOrSchema
