...
public class IndexingTools {
	...
	/**
	 * String used to separate and preserve phrases indexed as text, includes
	...
	public final static void addToDefaultAndStemsFields(Document myDoc, String content) {
		...
		// Note that the Analyzer will handle converting the tokens in this field to
		// their stem form.
	...
	/**
	 * Creates a String separated by the phrase separator term from the text of each
	 * of the Element or Attributes dom4j Nodes provided. The input list may be
	 * null.
	 * <p>
	 *
	...
	/**
	 * Creates a String separated by the phrase separator term from each of the
	 * Strings provided. The input list may be null.
	 * <p>
	 *
	 *
	 *
	 ...
	 * {@link #makeSeparatePhrasesFromNodes(List nodes)} or
	 * {@link #makeSeparatePhrasesFromStrings(List strings)}.
	 *
	 * @param separatedPhrases
	...
	/**
	 * Creates a String separated by spaces from the text of each of the Element or
	 * Attributes dom4j Nodes provided. The input list may be null.
	 * <p>
	 *
	 * A call to this method might look like:<br>
	 ...
	 * {@link #makeStringFromNodes(List nodes)}.
	 *
	 * @param separatedWords
	 *            DESCRIPTION
	...
	/**
	 * Extracts all {@link org.apache.lucene.analysis.Token}s from a Lucene query
	 * using the given {@link org.apache.lucene.analysis.Analyzer}.
	 *
	 * @param textToParse
	 *            The text to analyze with the analyzer
	 * @param analyzer
	 *            The analyzer to use
	 * @param field
	 *            The field this Analyzer should interpret the text as, or null to
	 *            use 'default'
	...
	/**
	 * Extracts all terms in any field from a Lucene query using the given
	 * {@link org.apache.lucene.analysis.Analyzer} and places them into a HashMap.
	 * The keys in the HashMap correspond to the field, the values are Lists of the
	 * terms in that field.
	 *
	 * @param textToParse
	 *            The text to analyze with the analyzer
	 ...
	 */
	/*
	 * public final static HashMap getAnalyzedTermsHashMap(String textToParse,
	 * String defaultField, Analyzer analyzer) { Reader reader = new
	 * StringReader(textToParse); TokenStream in =
	 * analyzer.tokenStream((defaultField == null ? "default" : defaultField),
	 * reader); HashMap map = new HashMap(); ArrayList termList = new ArrayList();
	 * Token token; List list; try { for (; ; ) { token = in.next(); if (token ==
	 * null) break; list = map.get(token.) termList.add(token.termText()); } } catch
	 * (Throwable t) {} return (String[]) termList.toArray(new String[]{}); }
	 */
	/**
	 * Creates a StringBuffer to display the tokens created by a given analyzer.
	 * Output is of the form: [token1] [token2].
	 *
	 * @param textToParse
	 *            The text to analyze with the analyzer
	 * @param analyzer
