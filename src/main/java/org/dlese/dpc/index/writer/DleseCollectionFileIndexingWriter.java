...
/**
 * Used to write a Lucene {@link org.apache.lucene.document.Document} for a
 * DLESE Collection XML record. The reader for this type of
 * {@link org.apache.lucene.document.Document} is
 * {@link org.dlese.dpc.index.reader.DleseCollectionDocReader}.
 * <p>
 *
 *
 *
...
public class DleseCollectionFileIndexingWriter extends XMLFileIndexingWriter {
	...
	/**
	 * Gets the numInstances attribute of the DleseCollectionFileIndexingWriter
	 * class
	 *
	 ...
	 * @exception Exception
	 *                If error reading XML.
	 */
	protected String getFullTitle() throws Exception {
		List nodes = getDom4jDoc().selectNodes(
	 ...
	 * {@link org.dlese.dpc.repository.RepositoryManager.putRecord} when the
	 * collection was created inside an additionalMetadata element, or null.
	 *
	 * @return The additional metadata element as an String, or null if none.
	 */
	public String getAdditionalMetadata() {
		try {
			Node additionalMetadataNode = getDom4jDoc()
	...
	protected String getPartOfDRC() throws Exception {
		List nodes = getDom4jDoc()
				.selectNodes("/*[local-name()='collectionRecord']/*[local-name()='access']/*[local-name()='drc']");
		if (nodes.size() != 1)
			return "false";
		Element element = (Element) nodes.get(0);
		return element.getText();
	...
	/**
	 * Gets the collectionStatus attribute of the DleseCollectionFileIndexingWriter
	 * object
	 *
	...
	/**
	 * Gets the collection key used to identify the items in the collection this
	 * record refers to. For example, dcc or comet.
	 *
	...
	protected String getKey() throws Exception {
		List nodes = getDom4jDoc()
				.selectNodes("/*[local-name()='collectionRecord']/*[local-name()='access']/*[local-name()='key']");
		if (nodes.size() != 1)
	...
	public String[] getUrls() throws Exception {
		List nodes = getDom4jDoc().selectNodes(
				"/*[local-name()='collectionRecord']/*[local-name()='access']/*[local-name()='collectionLocation']");
		if (nodes.size() == 0)
			return null;
		Element element = (Element) nodes.get(0);
	...
	protected String getScopeUrl() throws Exception {
		List nodes = getDom4jDoc().selectNodes(
				"/*[local-name()='collectionRecord']/*[local-name()='general']/*[local-name()='policies']/*[local-name()='policy'][@type='Collection scope']/@url");
	...
	protected String getReviewProcessUrl() throws Exception {
		List nodes = getDom4jDoc().selectNodes(
				"/*[local-name()='collectionRecord']/*[local-name()='general']/*[local-name()='reviewProcess']/@url");
		if (nodes.size() > 1)
	...
	protected String getReviewProcess() throws Exception {
		List nodes = getDom4jDoc().selectNodes(
				"/*[local-name()='collectionRecord']/*[local-name()='general']/*[local-name()='reviewProcess']");
		if (nodes.size() > 1)
	...
	protected String getCost() throws Exception {
		List nodes = getDom4jDoc()
				.selectNodes("/*[local-name()='collectionRecord']/*[local-name()='general']/*[local-name()='cost']");
	...
	protected String getKeywords() throws Exception {
		List nodes = getDom4jDoc().selectNodes(
				"/*[local-name()='collectionRecord']/*[local-name()='general']/*[local-name()='keywords']/*[local-name()='keyword']");
		String tmp = "";
		if (nodes.size() > 1)
	...
	protected String[] getGradeRanges() throws Exception {
		List nodes = getDom4jDoc().selectNodes(
				"/*[local-name()='collectionRecord']/*[local-name()='general']/*[local-name()='gradeRanges']/*[local-name()='gradeRange']");
	...
	protected String[] getSubjects() throws Exception {
		List nodes = getDom4jDoc().selectNodes(
				"/*[local-name()='collectionRecord']/*[local-name()='general']/*[local-name()='subjects']/*[local-name()='subject']");
		ArrayList vals = new ArrayList();
		String tmp;
		for (int i = 0; i < nodes.size(); i++) {
			tmp = ((Element) nodes.get(i)).getText();
	...
	/**
	 * Gets the docType attribute of the DleseCollectionFileIndexingWriter, which is
	 * 'dlesecollect.'
	 *
	 ...
	 * {@link org.apache.lucene.document.Document}, which is
	...
	/**
	 * Returns the accession date or null if this collection is not currently
	 * accessioned.
	 *
	...
	protected Date getAccessionDate() throws Exception {
		if (!getAccessionStatus().equals("accessioned"))
			return null;
		Node dateElement = getDom4jDoc().selectSingleNode(
				"/collectionRecord/approval/collectionStatuses/collectionStatus[@state='Accessioned']");
		if (dateElement != null)
	...
	/**
	 * Returns the date used to determine "What's new" in the library. Just returns
	 * the file mod date.
	 *
	...
	/**
	 * Gets a report detailing any errors found in the XML validation of the
	 * collection record, or null if no error was found.
	 *
	...
	protected final void addFields(Document newDoc, Document existingDoc, File sourceFile) throws Exception {
		...
		// The first time the index is built, the 'collect' DocReader is not available,
		// so let RecordDataService know the collect ID:
		...
		if (accessionDate != null)
			newDoc.add(new Field("collaccessiondate", DateFieldTools.dateToString(accessionDate), Field.Store.YES,
		...
		// Then parse it for indexing
		newDoc.add(
				new Field("reviewprocessuri", IndexingTools.tokenizeURI(value), Field.Store.NO, Field.Index.ANALYZED));
		addToDefaultField(IndexingTools.tokenizeURI(value));
		...
		newDoc.add(new Field(getFieldName("gradeRange", metadataFormat),
				getFieldContent(getGradeRanges(), "gradeRange", metadataFormat), Field.Store.YES,
				Field.Index.ANALYZED));

		String terms;
		newDoc.add(new Field(getFieldName("subject", metadataFormat),
				getFieldContent(getSubjects(), "subject", metadataFormat), Field.Store.YES, Field.Index.ANALYZED));
	...
	/**
	 * Creates a Lucene {@link org.apache.lucene.document.Document} from an existing
	 * CollectionFileIndexing Document by setting the field "deleted" to "true" and
	 * making the modtime equal to current time.
	 *
	 * @param existingDoc
	 *            An existing FileIndexingService Document that currently resides in
	 *            the index for the given resource.
	 * @return A Lucene FileIndexingService Document with the field "deleted" set to
	 *         "true" and modtime set to current time.
	 * @exception Throwable
	 *                Thrown if error occurs
	 */
	public synchronized Document getDeletedDoc_OFF_2006_08_23(Document existingDoc) throws Throwable {
		if (existingDoc == null)
	...
	/**
	 * Gets the status of the collection based on the values in the collection-level
	 * record.
	 *
	 * @param doc
	...
	public final static String getCurrentCollectionStatus(org.dom4j.Document doc) {
		List nodes = doc.selectNodes(
				"/*[local-name()='collectionRecord']/*[local-name()='approval']/*[local-name()='collectionStatuses']/*[local-name()='collectionStatus']");
	...
	/**
	 * Allows sorting of a Collection accession status XML Node, by date giving
	 * precedence to status = accessioned if dates are equal.
	 *
	 * @author John Weatherley
	 */
	public static class CollectionAccessionStatusComparator implements Comparator {
		 ...
		 *
		 * @param O1
		 *            A collectionStatus dom4j XML Node from a dlese_collect XML
		 *            Document.
		 * @param O2
		 *            A collectionStatus dom4j XML Node from a dlese_collect XML
		 *            Document.
