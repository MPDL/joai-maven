...
/**
 * Used to write a Lucene {@link org.apache.lucene.document.Document} for a NCS
 * Collection XML record.
 *
...
public class NCSCollectionFileIndexingWriter extends XMLFileIndexingWriter {
	...
	/**
	 * Gets the docType attribute of the NCSCollectionFileIndexingWriter, which is
	 * 'ncs_collect.'
	 *
	 ...
	 * {@link org.apache.lucene.document.Document}, which is "XMLDocReader".
	 *
	...
	protected final void addFields(Document newDoc, Document existingDoc, File sourceFile) throws Exception {
		...
		if (nodes.size() == 0) {
			...
			for (int i = 0; i < nodes.size(); i++) {
				Element element = (Element) nodes.get(i);
				newDoc.add(
						new Field("ncsCollectPathwayName", element.getText(), Field.Store.YES, Field.Index.ANALYZED));
			}
		}
		...
		for (int i = 0; i < nodes.size(); i++) {
			Element element = (Element) nodes.get(i);
			newDoc.add(
					new Field("ncsCollectCollectionSubject", element.getText(), Field.Store.YES, Field.Index.ANALYZED));
		}
		...
		for (int i = 0; i < nodes.size(); i++) {
			Element element = (Element) nodes.get(i);
			newDoc.add(
					new Field("ncsCollectCollectionPurpose", element.getText(), Field.Store.YES, Field.Index.ANALYZED));
		}
