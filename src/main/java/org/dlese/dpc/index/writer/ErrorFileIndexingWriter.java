...
/**
 * Writes a Lucene {@link org.apache.lucene.document.Document} that represents
 * an error that has occured in in indexing a File. This writer is used by
 * {@link org.dlese.dpc.index.FileIndexingService}. The
 * {@link org.dlese.dpc.index.writer.FileIndexingServiceWriter} may throw a
 * {@link ErrorDocException}, which can be used to specific specific fields to
 * index with the exception.
 * <p>
 *
 * <br>
 ...
 * {@link org.dlese.dpc.index.writer.FileIndexingServiceWriter}) all fields
 * specified in the {@link ErrorDocException} are stored plus:</b> <br>
 * <br>
 * <code><b>error</b> </code> - Set to 'true' for all documents indexed by this
 * writer. Not stored.<br>
 * <code><b>errormsg</b> </code> - The description of the error, for display.
 * Stored. <br>
 * <code><b>errordoctype</b> </code> - The type of error thrown as specified by
 * {@link ErrorDocException}. Defaults to 'generic'. Stored. <br>
 * <code><b>exception</b> </code> - The name of the Java exception that was
 * thrown. Stored. <br>
 * <br>
 *
 *
...
public class ErrorFileIndexingWriter extends FileIndexingServiceWriter {
	 ...
	 * {@link org.apache.lucene.document.Document}, which is "ErrorDocReader".
	 *
	 ...
	 *
	 * @param exception
	 *            The exception that was thrown when the error occured.
	 */
	...
	protected final void addCustomFields(Document newDoc, Document existingDoc, File sourceFile) throws Exception {
			...
			if (exception instanceof ErrorDocException) {
				...
				if (keywordFieldNames != null)
					for (int i = 0; i < keywordFieldNames.length; i++)
						newDoc.add(new Field(keywordFieldNames[i],
								errorDocException.getKeywordFieldValue(keywordFieldNames[i]), Field.Store.YES,
								Field.Index.NOT_ANALYZED));

				String[] textFieldNames = errorDocException.getTextFieldNames();
				if (textFieldNames != null)
					for (int i = 0; i < textFieldNames.length; i++)
						newDoc.add(new Field(textFieldNames[i], errorDocException.getTextFieldValue(textFieldNames[i]),
		...
		newDoc.add(new Field("docdirenc", SimpleLuceneIndex.encodeToTerm(getSourceDir().getAbsolutePath()),
