...
/**
 * Adds index fields to a Lucene {@link org.apache.lucene.document.Document}
 * from any well-formed XML. Individual field names are derived from the xPath
 * to each element and attribute in the XML instance document. Fields are
 * encoded to support text, keyword and stemmed search. Also creates standard
 * fields for IDs, URLs, title, description and geospatial bounding box
 * footprint. The 'default' and 'stems' fields are also indexed as text and
 * stemmed text, respectively.
 * <p>
 *
 * A {@link XMLIndexerFieldsConfig} may be supplied to configure specific search
 * fields for given XML formats. If a field is defined in the
 * XMLIndexerFieldsConfig, and content is avialable at the given xPath, it will
 * override the value set for ids, urls, title or description. In addition,
 * field values configured by schema override those configured by xmlFormat.
 *
...
public class XMLIndexer {
	...
	/**
	 * Sets whether to index the default, admindefault, and stems field for this
	 * record.
	 *
	 * @param indexDefaultAndStemsField
	 *            The value to assign indexDefaultAndStemsField.
	 * @exception IllegalStateException
	 *                If called after method #indexFields has been called
	 */
	...
	/**
	 * Returns unique IDs for the item being indexed encoded for indexing. If more
	...
	/**
	 * Sets the ids of related records. The Map key contains the relationship
	 * (isAnnotatedBy, etc.) and the Map value contains a List of Strings that
	 * indicate the ids of the target records.
	 *
	 * @param relations
	 *            Map of related record ids .
	 * @exception IllegalStateException
	 *                If called after method #indexFields has been called
	 */
	...
	/**
	 * Sets the urls of related records. The Map key contains the relationship
	 * (isAnnotatedBy, etc.) and the Map value contains a List of Strings that
	 * indicate the urls of the target records.
	 *
	 * @param relations
	 *            Map of related record urls.
	 * @exception IllegalStateException
	 *                If called after method #indexFields has been called
	 */
	...
	/**
	 * Sets the value of xPathFieldsPrefix, which is appended at the front of the
	 * xPath fields when indexed. Set to null to use none (default).
	 * 
	 * @param xPathFieldsPrefix
	 ...
	 * 
	 * @param boundingBox
	 *            The value to assign boundingBox.
	 */
	...
	/**
	 * Gets the full content of each Element in the XML. Attribute content is not
	 * included. If this is a Java Bean, gets the contnet of all Bean properties.
	...
	/**
	 * Gets the full content of each Attribute in the XML. Element content is not
	 * included. Method #indexFields must be called prior to using this method.
	 *
	 ...
	 *
	 * @param localizedXmlDocument
	 *            A localized XML Document
	 * @param xmlFormat
	 *            The XML format being indexed, for example adn or oai_dc
	 * @param xmlIndexerFieldsConfig
	 *            The config, or null if not used
	 */
	public XMLIndexer(org.dom4j.Document localizedXmlDocument, String xmlFormat,
	 ...
	 *                If error
	 */
	public XMLIndexer(String xmlString, String xmlFormat, XMLIndexerFieldsConfig xmlIndexerFieldsConfig)
			throws Exception {
		this.xmlFormat = xmlFormat;
		this.xmlIndexerFieldsConfig = xmlIndexerFieldsConfig;
	...
	/**
	 * Indexes the contents of the XML, adding fields to the Lucene Document that is
	 * supplied.
	 *
	 * @param luceneDoc
	 *            The {@link org.apache.lucene.document.Document} to add fields to
	 * @exception Exception
	 *                If error, provides an appropriate message to display in
	 *                indexing reports.
	 */
	 ...
	 *                indexing reports.
	 */
	private void indexCustomFields(String xmlFormatOrSchema, org.apache.lucene.document.Document luceneDoc)
			throws Exception {

		if (xmlIndexerFieldsConfig == null)
			return;

		// Loop through each field and add the contents of the xPath to the index for
		// that field
				...
				for (Object xp : xPaths) {
					try {
						...
						for (Object c : contentNodes) {
							...
							prtln("Adding configured field to index:\n  fieldName:" + fieldName + " store:" + fieldStore
									+ " tokenize:" + fieldIndex + "\n  content:'" + indexContent + "'");
							if (indexContent.length() > 0)
								luceneDoc.add(new Field(fieldName, indexContent, fieldStore, fieldIndex));
						}
					} catch (Exception e) {
						throw new Exception("Error indexing custom field:'" + fieldName + "' for format:'"
								+ xmlFormatOrSchema + "': " + e.getMessage());
					}
				}
			}
		}
	...
	/**
	 * Extracts the standard field data from the XML, such as title, description,
	 * url, id, boundingBox, if configured.
	 *
	 * @param xmlFormatOrSchema
	 *            The xmlFormat or schema
	 * @exception Exception
	...
	private void extractStandardFields(String xmlFormatOrSchema) throws Exception {
		...
		if (configXmlDoc != null) {
			...
			for (int j = 0; j < fields.size(); j++) {
				...
				for (Object xp : xPaths) {
					try {
						...
						throw new Exception("Error indexing standard field:'" + fieldName + "' for format:'"
								+ xmlFormatOrSchema + "': " + e.getMessage());
					}
				}
			...
			// If any bounding box data was indicated, index it or print warning message if
			// data error:
			if (geoBBSouth != null || geoBBWest != null || geoBBEast != null || geoBBNorth != null) {
				try {
					setBoundingBox(new BoundingBox(Double.parseDouble(geoBBNorth), Double.parseDouble(geoBBSouth),
							Double.parseDouble(geoBBEast), Double.parseDouble(geoBBWest)));
				} catch (NumberFormatException ne) {
					String id = "";
					if (ids != null && ids.length > 0)
						id = " for ID " + ids[0];
					prtlnErr("Unable to index Bounding Box coordinates" + id + ". Invalid coordinate value: "
							+ ne.getMessage());
				} catch (NullPointerException npe) {
					String id = "";
					if (ids != null && ids.length > 0)
						id = " for ID " + ids[0];
					prtlnErr("Unable to index Bounding Box coordinates" + id
							+ ". One or more coordinates were null or missing.");
				} catch (Exception e) {
					String id = "";
					if (ids != null && ids.length > 0)
						id = " for ID " + ids[0];
	...
	/**
	 * Extracts the relationship data from the XML including the target ids and urls
	 * and relationship type 'isAnnotatedBy' ect.
	 *
	 * @param xmlFormatOrSchema
	 *            The xmlFormat or schema
	 * @exception Exception
	...
	private void extractRelationships(String xmlFormatOrSchema) throws Exception {
		...
		if (configXmlDoc != null) {
			...
			prtln("extractRelationships() for " + xmlFormatOrSchema + " (found " + relationships.size()
					+ " XPaths defined)");
				...
				for (Object xp : xPaths) {
					try {
						...
						if (type == null || !(type.equalsIgnoreCase("id") || type.equalsIgnoreCase("url")))
							throw new Exception(
									"XMLIndexerFieldsConfig relationships: xpath type must be either 'id' or 'url' but found: "
											+ type);
						List contentNodes = xmlDoc.selectNodes(xPath);
						for (Object c : contentNodes) {
							String indexContent;
						...
						throw new Exception("Error extracting relationships:'" + relationName + "' for format:'"
								+ xmlFormatOrSchema + "': " + e.getMessage());
					}
				}
			}
	...
	private void warnIfExceedsMaxOccurances(int maxOccurances, int numOccurances, String fieldName) {
		if (numOccurances > maxOccurances) {
			System.out.println("XMLIndexer warning: " + numOccurances + " values for field '" + fieldName
					+ "' were found in the XML instance document but only " + maxOccurances
					+ (maxOccurances == 1 ? "occurance is" : "occurances are")
	...
	private void indexStandardFields(org.apache.lucene.document.Document luceneDoc) throws Exception {
		...
		if (urls != null && urls.length > 0) {
			...
			for (int i = 1; i < urls.length; i++) {
				...
				luceneDoc.add(
						new Field("uri", IndexingTools.tokenizeURI(urls[i]), Field.Store.NO, Field.Index.ANALYZED));
	...
	/**
	 * Indexes the content of each element and attribute in the source XML as
	 * individual search fields, using the xPath to the element or attribute as the
	 * field name. If an xPath field prefix has been indicated it will be inserted
	 * at the beginning of the field path.
	 *
	 * @param luceneDoc
	 *            The {@link org.apache.lucene.document.Document} to add fields to
			...
			for (int i = 0; i < nodes.size(); i++) {
				...
				if (node instanceof Element) {
					...
					if (text.length() > 0 && luceneDoc != null) {
						luceneDoc.add(new Field(prefix + "/text/" + node.getPath(), text, Field.Store.YES,
								Field.Index.ANALYZED));
						luceneDoc.add(new Field(prefix + "/stems/" + node.getPath(), text, Field.Store.NO,
								Field.Index.ANALYZED)); // No need to stem: handled by the PerFieldAnalyzer...
						luceneDoc.add(new Field(prefix + "/key/" + node.getPath(), text, Field.Store.NO,
								Field.Index.NOT_ANALYZED));
						luceneDoc.add(
								new Field("indexedXpaths", node.getPath(), Field.Store.NO, Field.Index.NOT_ANALYZED));
					...
					for (int j = 0; j < attributes.size(); j++) {
						...
						if (attText.length() > 0 && luceneDoc != null) {
							luceneDoc.add(new Field(prefix + "/text/" + attribute.getPath(), attText, Field.Store.YES,
									Field.Index.ANALYZED));
							luceneDoc.add(new Field(prefix + "/stems/" + attribute.getPath(), attText, Field.Store.NO,
									Field.Index.ANALYZED)); // No need to stem: handled by the PerFieldAnalyzer...
							luceneDoc.add(new Field(prefix + "/key/" + attribute.getPath(), attText, Field.Store.NO,
									Field.Index.NOT_ANALYZED));
							luceneDoc.add(new Field("indexedXpaths", attribute.getPath(), Field.Store.NO,
									Field.Index.NOT_ANALYZED));
	...
	/**
	 * Indexes Java Bean XML that was encoded with the java.beans.XMLEncoder class,
	 * using the bean properties as field names. If this is not Java Bean encoded
	 * XML, nothing is done, returns false.
	 *
	 * @param luceneDoc
	 *            The {@link org.apache.lucene.document.Document} to add fields to
	 * @exception Exception
	...
	public boolean indexJavaBeanFields(org.apache.lucene.document.Document luceneDoc) throws Exception {
		...
		try {
			...
			if (!rootEle.getName().equals("java") || classAtt == null
					|| !classAtt.getText().equals("java.beans.XMLDecoder")) {
		...
		if (beanClass != null && beanClass.length() > 0)
			luceneDoc.add(
					new Field("/javaBean.class//key//className", beanClass, Field.Store.YES, Field.Index.NOT_ANALYZED));
				...
				if (propertyAtt != null) {
					...
					for (int j = 0; j < stringElements.size(); j++) {
						...
						if (value.trim().length() > 0) {
							...
							luceneDoc.add(new Field("/javaBean.property//text//" + propertyName, value, Field.Store.YES,
									Field.Index.ANALYZED));
							luceneDoc.add(new Field("/javaBean.property//stems//" + propertyName, value, Field.Store.NO,
									Field.Index.ANALYZED)); // No need to stem: handled by the PerFieldAnalyzer...
							luceneDoc.add(new Field("/javaBean.property//key//" + propertyName, value, Field.Store.NO,
									Field.Index.NOT_ANALYZED));
