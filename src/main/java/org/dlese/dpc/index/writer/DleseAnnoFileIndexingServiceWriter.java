...
/**
 * Creates a Lucene {@link org.apache.lucene.document.Document}s for a DLESE
 * annotation record.
 * <p>
 *
 * The Lucene fields that are created by this class are:
 * <ul>
 * <li><code>collection</code> - The collection associated with this resource.
 * Stored. Appended with a '0' at the beginning to support wildcard
 * searching.</li>
 * <li>Others...</li>
 * </ul>
 *
...
public class DleseAnnoFileIndexingServiceWriter extends XMLFileIndexingWriter {
	...
	/**
	 * Gets the numInstances attribute of the DleseAnnoFileIndexingServiceWriter
	 * class
	 *
	...
	public void init(File source, Document existingDoc) throws Exception {
		...
		if (isMakingDeletedDoc()) {
			...
			dleseAnnoXmlDoc.useXmlString(getFileContent(), this.isValidationEnabled(), true, true);
		} else {
			String sourcePath = source.getAbsolutePath();
			if (sourcePath.startsWith("/"))
				sourcePath = sourcePath.substring(1, sourcePath.length());

			dleseAnnoXmlDoc = new XMLDoc("file:///" + source.getAbsolutePath(), this.isValidationEnabled(), true, true);
		}

		// Determine the whether the status of the anno is 'completed' or not
		statusIsCompleted = (getStatus().matches(".*completed"));

		// If the item-level record for this annotation is not in the index, abort
		// indexing.
		/*
		 * if(recordDataService != null){ ResultDoc itemResultDoc =
		 * recordDataService.getItemResultDoc(getItemId()); if (itemResultDoc == null){
		 * abortIndexing(); prtln("anno record will not be created: item-level record "
		 * + getItemId() + " does not exist in the index."); } // If there is an
		 * item-level record, touch it's file so it will be re-indexed to capture this
		 * anno. else{
		 * if(Files.touch(((FileIndexingServiceDocReader)itemResultDoc.getDocReader()).
		 * getDocsource())) prtln("anno: touching file for item-level record " +
		 * getItemId() + " so it will be re-indexed."); else
	...
	/**
	 * Gets the status as a String, which is normalized to be the same for v1.0 and
	 * v0.1 of the framework. One of 'completed', 'in progress' or 'retired'.
	 *
	...
	/**
	 * Gets the format as a String, which is normalized to be the same for v1.0 and
	 * v0.1 of the framework. One of 'audio', 'graphical', 'text' or 'video'.
	 *
	...
	private String getFormat() {
		if (formatString == null) {
			...
			if (formatString.length() == 0) {
				...
				formatString = formatString.replaceFirst(" annotation completed", "")
						.replaceFirst(" annotation in progress", "");
			}
		}
		return formatString;
	}
	...
	/**
	 * Returns the the first available of item/statusOf@date, service/date@modified
	 * or service/date@created. Note that the statusOf@date was deprecated as of
	 * anno framework v1.0.
	 *
	...
	/**
	 * Returns 'annocomplete,' 'annoinprogress,' 'drcannocomplete,' or
	 * 'drcannoinprogress'.
	 *
	...
	/**
	 * Adds additional custom fields that are unique to the dlese anno document
	...
	protected void addFields(Document newDoc, Document existingDoc, File sourceFile) throws Exception {
		...
		if (fieldContent != null && fieldContent.length() > 0) {
			...
			if (date != null)
				newDoc.add(new Field("annodate", DateFieldTools.dateToString(date), Field.Store.YES,
