 ...
 * DLESE news-opps XML record. The reader for this type of
 * {@link org.apache.lucene.document.Document} is
 * {@link org.dlese.dpc.index.reader.NewsOppsDocReader}.
 * <p>
 *
 *
 *
...
public class NewsOppsFileIndexingWriter extends XMLFileIndexingWriter {
	...
	/**
	 * Gets the docType attribute of the NewsOppsFileIndexingWriter, which is
	 * 'news_opps.'
	 *
	 ...
	 * {@link org.apache.lucene.document.Document}, which is "NewsOppsDocReader".
	 *
	 ...
	 * news-opps record, or null if no error was found.
	 *
	...
	protected final void addFields(Document newDoc, Document existingDoc, File sourceFile) throws Exception {
		...
		if (node != null) {
			date = MetadataUtils.parseUnionDateType(node.getText());
			newDoc.add(new Field(FIELD_NS + "archivedate", DateFieldTools.dateToString(date), Field.Store.YES,
		...
		if (node != null) {
			date = MetadataUtils.parseUnionDateType(node.getText());
			newDoc.add(new Field(FIELD_NS + "recordCreationdate", DateFieldTools.dateToString(date), Field.Store.YES,
					Field.Index.NOT_ANALYZED));
		}
		...
		date = getPostDate();
		if (date != null)
			newDoc.add(new Field(FIELD_NS + "postdate", DateFieldTools.dateToString(date), Field.Store.YES,
					Field.Index.NOT_ANALYZED));
		...
		for (int i = 0; i < nodes.size(); i++) {
			...
			newDoc.add(new Field(FIELD_NS + element.attributeValue("type") + "date", DateFieldTools.dateToString(date),
		...
		// Announcements
		value = IndexingTools
				.makeSeparatePhrasesFromNodes(getDom4jDoc().selectNodes("/news-oppsRecord/announcements/announcement"));
		if (value != null) {
		...
		// Keywords
		value = IndexingTools
				.makeSeparatePhrasesFromNodes(getDom4jDoc().selectNodes("/news-oppsRecord/keywords/keyword"));
		if (value != null) {
		...
		// Audiences
		value = IndexingTools
				.makeSeparatePhrasesFromNodes(getDom4jDoc().selectNodes("/news-oppsRecord/audiences/audience"));
		if (value != null) {
		...
		// Diversities
		value = IndexingTools
				.makeSeparatePhrasesFromNodes(getDom4jDoc().selectNodes("/news-oppsRecord/diversities/diversity"));
		if (value != null) {
		...
		// Cities
		value = IndexingTools
				.makeSeparatePhrasesFromNodes(getDom4jDoc().selectNodes("/news-oppsRecord/locations/location/@city"));
		if (value != null) {
		...
		// Sponsor
		value = IndexingTools.makeSeparatePhrasesFromNodes(getDom4jDoc()
				.selectNodes("/news-oppsRecord/contributors/contributor[@role='Sponsor']/organization/instName"));
		if (value != null) {
