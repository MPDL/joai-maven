...
/**
 * Creates a Lucene {@link org.apache.lucene.document.Document} from a DLESE-IMS
 * XML source file.
 * <p>
 *
 ...
 * <br>
 * <code><b>doctype</b> </code> - Set to 'dlese_ims'. Stored. Note: the actual
...
public class DleseIMSFileIndexingWriter extends ItemFileIndexingWriter {
	...
	/**
	 * Gets the docType attribute of the DleseIMSFileIndexingWriter, which is
	 * 'dlese_ims.'
	 *
	...
	/**
	 * Returns the items keywords. An empty String or null is acceptable. The String
	 * is tokenized, stored and indexed under the field key 'keywords' and is also
	 ...
	 * The String is tokenized, stored and indexed under the field key 'creator' and
	 * is also indexed in the 'default' field.
	 *
	...
	protected String getCreator() throws Exception {
		return null;
		/*
		 * list.clear(); list = (List) map.getValue("catalogers"); i = list.iterator();
		 * while (i.hasNext()) { DleseBean.Contributor person = (DleseBean.Contributor)
		 * i.next(); newDoc.add(new Field("contributorrole", person.getValue("role"),
		 * Field.Store.YES, Field.Index.ANALYZED)); //addToDefaultField(
		 * person.getValue( "role" ) ); newDoc.add(new Field("contributorname",
		 * person.name(), Field.Store.YES, Field.Index.ANALYZED)); newDoc.add(new
		 * Field("contributor", person.getValue("role") + "=" +
		 * person.getValue("lastname"), Field.Store.YES, Field.Index.ANALYZED));
		 * //addToDefaultField( person.name() ); newDoc.add(new Field("contributororg",
		 * person.getValue("org"), Field.Store.YES, Field.Index.ANALYZED));
	...
	/**
	 * Returns the date used to determine "What's new" in the library, which is null
	 * (unknown).
	 *
	...
	protected final void addFrameworkFields(Document newDoc, Document existingDoc) throws Exception {
		...
		newDoc.add(new Field(vocab.getTranslatedField("adn", "topic"), indexedList, Field.Store.YES,
				Field.Index.ANALYZED));
		addToDefaultField(indexedList);

		indexedList = listToString((List) map.getValue("audiences"), "learningcontext");
		newDoc.add(new Field(vocab.getTranslatedField("adn", "learningcontext"), indexedList, Field.Store.YES,
				Field.Index.ANALYZED));
		addToDefaultField(indexedList);
		...
		// This is strange--why does resourcetype not use the listToString() method like
		// the other
		// controlled vocabs? Because the list returned by map.getValue() isn't a list
		// of strings,
		// but rather a list of DleseBean.ResourceType objects. But, why? Inconsistency
		// isn't desirable.
		...
		newDoc.add(new Field(vocab.getTranslatedField("adn", "resourcetype"), buf.toString(), Field.Store.YES,
				Field.Index.ANALYZED));
		...
		while (i.hasNext()) {
			...
			newDoc.add(new Field("contributor", person.getValue("role") + "=" + person.getValue("lastname"),
		...
		while (i.hasNext()) {
			...
			String geotext = (geoReference.getValue("begintime_description") + " "
					+ geoReference.getValue("endtime_description") + " " + geoReference.getValue("place_event_name1")
					+ " " + geoReference.getValue("place_event_name2") + " "
					+ geoReference.getValue("place_event_name3") + " " + geoReference.getValue("place_event_name4")
					+ " " + geoReference.getValue("place_event_name5") + " "
					+ geoReference.getValue("place_event_name6") + " " + geoReference.getValue("place_event_name7")
					+ " " + geoReference.getValue("place_event_name8")).trim();
			if (geotext.length() > 0) {
	...
	/**
	 * Takes a list of values and concatenates them into a single string, seperated
	 * by spaces. If useVocabMapping is specified, then it is used to map the values
	 * to their controlled vocabulary IDs.
	 *
	 * @param list
	 *            A list that contains Strings
	 * @param useVocabMapping
	 *            Is this a controlled vocab? If so, map to the encoded ID using
	 *            this string as the field identifier (pass "" otherwise)
	 * @return DESCRIPTION
	 * @exception Exception
	 */
