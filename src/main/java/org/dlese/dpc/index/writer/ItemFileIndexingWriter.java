...
/**
 * Abstract class for writing a Lucene
 * {@link org.apache.lucene.document.Document} for a collection of item-level
 * metadata records of a specific format (DLESE IMS, ADN-Item, ADN-Collection,
 * etc). The reader for this type of {@link org.apache.lucene.document.Document}
 * is {@link org.dlese.dpc.index.reader.XMLDocReader} or
 * {@link org.dlese.dpc.index.reader.ItemDocReader}.
 * <p>
 *
 * <br>
 * <b>The Lucene {@link org.apache.lucene.document.Document} fields that are
 * created by this class are (in addition the the ones listed for
 * {@link org.dlese.dpc.index.writer.FileIndexingServiceWriter}):</b> <br>
 * <br>
 * <code><b>title</b> </code> - The tile for the resource. Stored.<br>
 * <code><b>description</b> </code> - The description for the resource. Stored.
 * <br>
 ...
 * <br>
 * <code><b>metadatapfx</b> </code> - The metadata prefix (format) for this
 * record, for example 'adn' or 'oai_dc'. Stored. Appended with a '0' at the
 * beginning to support wildcard searching.<br>
 * <code><b>accessionstatus</b> </code> - The accession status for this record.
 * Stored. Appended with a '0' at the beginning to support wildcard searching.
 * <br>
 * <code><b>annotypes</b> </code> - Annotataion types that are refer to this
 * record. Keyword. <br>
 * <code><b>annopathways</b> </code> - Annotataion pathways that are refer to
 * this record. Keyword. <br>
 * <code><b>associatedids</b> </code> - A list of record IDs that refer to the
 * same resource. Keyword. <br>
 * <code><b>valid</b> </code> - Indicates whether the record is valid [true |
 * false]. Not stored.<br>
 * <code><b>validationreport</b> </code> - Text describing an error in the
 * validation of the data for this record. Stored. Only indexed if there was a
 * validation error indicated by the valid field containing false. <br>
 * <br>
 *
 *
...
public abstract class ItemFileIndexingWriter extends XMLFileIndexingWriter {
	...
	// My id's include this record's ID at the first position. If this is a
	// multi-doc
	...
	// Get annotation for this record only. If I am a multi-doc, these include all
	// annos for all records
	private ResultDocList _myAnnoResultDocs = null;

	// Get ALL annotations for this resource from all records associated with the
	// resource
	...
	/**
	 * Returns the item's keywords sorted and separated by the '+' symbol. An empty
	 * String or null is acceptable. The String is tokenized, stored and indexed
	 * under the field key 'keywords' and is also indexed in the 'default' field.
	 *
	...
	/**
	 * Returns the items creator's last name. An empty String or null is acceptable.
	 * The String is tokenized, stored and indexed under the field the 'default'
	 * field only.
	 *
	...
	/**
	 * Returns the items creator's full name. An empty String or null is acceptable.
	 * The String is tokenized, stored and indexed under the field key 'creator'.
	 *
	...
	/**
	 * Returns the accession status of this record, for example 'accessioned'. The
	 * String is tokenized, stored and indexed under the field key
	 * 'accessionstatus'.
	 *
	...
	/**
	 * Returns the accession date for the item, or null if this item is not
	 * accessioned.
	 *
	 * @return The accession date for the item, or null if this item is not
	 *         accessioned.
	 * @exception Exception
	 *                This method should throw and Exception with appropriate error
	 *                message if an error occurs.
	...
	/**
	 * Returns the content of the item this record catalogs, or null if not
	 * available. For example the full HTML text of the Web page.
	 *
	...
	/**
	 * Returns the MmdRecs for records in other collections that catalog the same
	 * resource. Does not include myMmdRec.
	 *
	...
	/**
	 * Returns the MmdRecs for all records associated with this resouce, including
	 * myMmdRec.
	 *
	...
	/**
	 * Returns the content type of the item this record catalogs, or null if not
	 * available. For example "text/html" or "html".
	 *
	...
	/**
	 * Returns the IDs of related resources that are cataloged by ID, or null if
	 * none are present
	 *
	...
	/**
	 * Returns the URLs of related resources that are cataloged by URL, or null if
	 * none are present
	 *
	 ...
	 * &nbsp;<code> newDoc.add(new Field("mycustomefield", customContent));</code>
	 * <br>
	 * <code>}</code>
	 *
	 * @param newDoc
	...
	/**
	 * Returns a unique document type key for this kind of record, corresponding to
	 * the format type. For example "adn," "dlese_ims," or "dlese_anno". The string
	...
	/**
	 * Gets a report detailing any errors found in the validation of the data, or
	 * null if no error was found. This could be implemented by simply performing
	 * XML schema validation on the file, or can involve more customized validation
	 * of the data if necessary. This method is called after all other methods that
	 * access the data ({@link #getTitle()},
	 * {@link #addFrameworkFields(Document, Document)}, etc.) so that data
	 * verification can be done during those calls, if needed.
	 *
	 * @return Null if no data validation errors were found, otherwise a String that
	 *         details the nature of the error.
	 * @exception Exception
	 *                If error in performing the validation.
	 */
	 ...
	 *            The source file being indexed.
	 * @param existingDoc
	 *            A Document that previously existed in the index for this item, if
	 *            present
	 * @exception Exception
	 *                Thrown if error reading the XML map
	 */
	 ...
	 * Overrides method in XMLFileIndexingWriter because IDs need initializing.
	 *
	...
	/**
	 * Adds fields to the index that are common to all item-level documents. These
	 * include the title, description, id and url as well as collection, accession
	 * status, annotation references, and collection(s).
	 *
	 * @param newDoc
	 *            The new Document that is being created for this resource
	 * @param existingDoc
	 *            An existing Document that currently resides in the index for the
	 *            given resource, or null if none was previously present
	 * @param sourceFile
	 ...
	 *                If an error occurs
	 */
	protected final void addFields(Document newDoc, Document existingDoc, File sourceFile) throws Exception {
		...
		if (recordDataService != null) {
			...
			/*
			 * // Flag anno if (myAnnoResultDocs != null && myAnnoResultDocs.length > 0)
			 * newDoc.add(new Field("itemhasanno", "true", Field.Store.YES,
			 * Field.Index.ANALYZED)); else newDoc.add(new Field("itemhasanno", "false",
			 * Field.Store.YES, Field.Index.ANALYZED)); // Anno types fieldContent = "";
			 * fieldList = recordDataService.getAnnoTypesFromResultDocs(myAnnoResultDocs);
			 * if (fieldList != null && fieldList.size() > 0) { for (int i = 0; i <
			 * fieldList.size(); i++) fieldContent += ((String)
			 * fieldList.get(i)).replaceAll(" ", "+") + " "; newDoc.add(new
			 * Field("itemannotypes", fieldContent, Field.Store.YES, Field.Index.ANALYZED));
			 * } // Anno pathways fieldContent = ""; fieldList =
			 * recordDataService.getAnnoPathwaysFromResultDocs(myAnnoResultDocs); if
			 * (fieldList != null && fieldList.size() > 0) { for (int i = 0; i <
			 * fieldList.size(); i++) fieldContent += ((String)
			 * fieldList.get(i)).replaceAll(" ", "+") + " "; newDoc.add(new
			 * Field("itemannopathways", fieldContent, Field.Store.YES,
			 * Field.Index.ANALYZED)); } // Anno collection keys, e.g. {06, 09} fieldContent
			 * = ""; fieldList =
			 * recordDataService.getCollectionKeysFromResultDocs(myAnnoResultDocs); if
			 * (fieldList != null && fieldList.size() > 0) { for (int i = 0; i <
			 * fieldList.size(); i++) { fieldContent += (String) fieldList.get(i); if (i <
			 * (fieldList.size() - 1)) fieldContent += "+"; }
			 * //prtln("itemannocollectionkeys for " + this.getId() + " is: " +
			 * fieldContent); newDoc.add(new Field("itemannocollectionkeys", fieldContent,
			 * Field.Store.YES, Field.Index.ANALYZED)); } // Anno collection keys e.g. {06,
			 * 09} for those with status completed only fieldContent = ""; ArrayList
			 * completedAnnoCollectionKeys =
			 * recordDataService.getCompletedAnnoCollectionKeysFromResultDocs(
			 * myAnnoResultDocs); if (completedAnnoCollectionKeys != null &&
			 * completedAnnoCollectionKeys.size() > 0) { for (int i = 0; i <
			 * completedAnnoCollectionKeys.size(); i++) { fieldContent += (String)
			 * completedAnnoCollectionKeys.get(i); if (i <
			 * (completedAnnoCollectionKeys.size() - 1)) fieldContent += "+"; }
			 * //prtln("itemannocompletedcollectionkeys for " + this.getId() + " is: " +
			 * fieldContent); newDoc.add(new Field("itemannocompletedcollectionkeys",
			 * fieldContent, Field.Store.YES, Field.Index.ANALYZED)); } // Anno status
			 * fieldContent = ""; fieldList =
			 * recordDataService.getAnnoStatusFromResultDocs(myAnnoResultDocs); if
			 * (fieldList != null && fieldList.size() > 0) { for (int i = 0; i <
			 * fieldList.size(); i++) fieldContent += ((String)
			 * fieldList.get(i)).replaceAll(" ", "+") + " "; newDoc.add(new
			 * Field("itemannostatus", fieldContent, Field.Store.YES,
			 * Field.Index.ANALYZED)); }
			 * 
			 * // Anno formats fieldContent = ""; fieldList =
			 * recordDataService.getAnnoFormatsFromResultDocs(myAnnoResultDocs); if
			 * (fieldList != null && fieldList.size() > 0) { for (int i = 0; i <
			 * fieldList.size(); i++) fieldContent += ((String) fieldList.get(i)) + " ";
			 * newDoc.add(new Field("itemannoformats", fieldContent, Field.Store.YES,
			 * Field.Index.ANALYZED)); }
			 * 
			 * // Anno rating information and statistics indexAnnoRatings(myAnnoResultDocs,
			 * newDoc);
			 */
			// Part of DRC?
			/*
			 * The resourcs in the in DRC if either A or B are true: A. One or more ADN
			 * records that catalog the resource comes from a DRC collection (as indicated
			 * by it's collection record)
			 * 
			 * B. The logic for determining whether an annotated resources is in the DRC is
			 * the following: 1. The anno's collection record shows it is part of DRC AND 2.
			 * The anno's status is /annotationRecord/item/statusOf@status 'xxx completed'
			 * (or is empty) AND 3. The anno's pathway (/annotationRecord/item/pathway) is
			 * not empty
			 */
			...
			if (collDoc == null)
				throw new FileIndexingServiceException(
						"Unable to get collection-level record for collection key:" + getCollections()[0]);
			if (collDoc.isPartOfDRC() || recordDataService.hasDRCItem(assocItemResultDocs)
			...
			for (int i = 0; allAnnoResultDocs != null && i < allAnnoResultDocs.size(); i++) {
				annoCollectionDoc = (DleseCollectionDocReader) ((DleseAnnoDocReader) allAnnoResultDocs.get(i)
						.getDocReader()).getMyCollectionDoc();
			...
			// Additional collections keys (vocab mgr encoded) that refer to the same
			// resource, including anno collections:
			...
			ArrayList completedAnnoCollectionKeys = recordDataService
					.getCompletedAnnoCollectionKeysFromResultDocs(getMyAnnoResultDocs());
			...
			if (fieldList != null && fieldList.size() > 0) {
				...
				newDoc.add(new Field("associatedcollectionkeys", associatedcollectionkeys, Field.Store.YES,
						Field.Index.ANALYZED));
			}
		...
		// If no record data service, mark the item as having associated ids
			...
			/*
			 * newDoc.add(new Field("itemhasanno", "false", Field.Store.YES,
			 * Field.Index.ANALYZED));
			 */
			newDoc.add(new Field("hasassociatedids", "false", Field.Store.YES, Field.Index.ANALYZED));
			DleseCollectionDocReader myCollectionDoc = getMyCollectionDoc();
		...
		if (accessionDate != null)
			newDoc.add(new Field("accessiondate", DateFieldTools.dateToString(accessionDate), Field.Store.YES,
					Field.Index.NOT_ANALYZED));

		// Creation date
		Date creationDate = getCreationDate();
		if (creationDate != null)
			newDoc.add(new Field("creationdate", DateFieldTools.dateToString(creationDate), Field.Store.YES,
					Field.Index.NOT_ANALYZED));
	 ...
	 *
	 * @param newDoc
	 *            The new Document that is being created.
	 * @param existingDoc
	 *            The existing Document for this item, if available.
	 */
	private void indexContent(Document newDoc, Document existingDoc) {
		...
		if (content != null && content.trim().length() != 0 && contentType != null
		...
		newDoc.add(new Field("itemContentIndexedFields",
				(contentIndexed.trim().length() == 0 ? "noContentIndexedEmpty" : contentIndexed), Field.Store.YES,
				Field.Index.ANALYZED));

	}

	/**
	 * Returns the date used to determine "What's new" in the library, which is the
	 * item's accession date.
	 *
	...
	/**
	 * Returns 'itemnew' or 'itemannoinprogress' or 'itemannocomplete' whichever
	 * came most recelntly.
	 *
