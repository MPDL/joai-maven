	...
	public BoundingBox(double north, double south, double east, double west) throws IllegalArgumentException {
		...
		if (!testLat(north))
			throw new IllegalArgumentException(
					"North latitute must be in the range of -90 to 90. Invalid value: " + north);
		else if (!testLat(south))
			throw new IllegalArgumentException(
					"South latitute must be in the range of -90 to 90. Invalid value: " + south);
		else if (!(north >= south))
			throw new IllegalArgumentException(
					"North latitue must be greater than or equal to south. North: " + north + " South: " + south);
		else if (!testLon(east))
			throw new IllegalArgumentException(
					"East longitude must be in the range of -180 to 180. Invalid value: " + east);
		else if (!testLon(west))
			throw new IllegalArgumentException(
					"West longitude must be in the range of -180 to 180. Invalid value: " + west);
		else if (west > east) {
			// If bounding box extent crosses 180/-180, cull the search bounding box to
			// largest of one side or the other to be compatible with the search algorithm:
			if (distance(west, 180d) > distance(-180d, east))
				east = 179.999999d;
			else
				west = -179.999999d;
			// throw new IllegalArgumentException("West longitude may not be greater than
			// east. West: " + west + " East: " + east);
		}
