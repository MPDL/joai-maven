...
/**
 * A simple API for searching, reading and writing
 * <a href="http://lucene.apache.org/"> Lucene</a> indexes.
 *
...
public final class SimpleLuceneIndex {
	...
	/*
	 * To Do (June 2008): - Refactor listDocs methods or consider alternatives to
	 * eliminate getting all Documents - jconsole tests show spike in memory with
	 * large result sets. - Look at all instances where Documents are referenced to
	 * make sure memory is not unnecessarily locked up
	 */
	...
	/**
	 * The number of milliseconds to wait after the last index update before running
	 * the optimizer (30 minutes)
	 */
	private final static long OPTIMIZE_DELAY_TIME = 1800000;
	// private final static long OPTIMIZE_DELAY_TIME = 2000; // test every 3
	// seconds...

	/**
	 * The number of records updated before the optimizer is run. Use -1 to never
	 * run the optimizer based on number of records updated (optimizer will be run
	 * based on time elapsed since the last update only)
	 */
	private final static long NUM_RECORDS_FOR_OPTIMIZER = -1;

	// This increases the number of clauses for wildcard and other searches (default
	// is 1024):
	private final static int MAX_CLAUSE_COUNT = 30000;

	/**
	 * Indicates update operations will be blocked until the current one returns.
	 * When this is passed into a method, the method will not return until the
	 * update operation has completed.
	 */
	public final static boolean BLOCK = true;

	/**
	 * Indicates update operations will be allowed while others are still in
	 * progress. When this is passed into a method, the method will return
	 * immediately rather than waiting for the update operation to complete.
	 */
	...
	// The max number of search re-tries if there was an exception (which can occur
	// during index updating)
	private final static int MAX_TRIES = 3;

	private OptimizeMonitorThread optimizeMonitorThread = null;

	/**
	 * Initializes or creates an index at the given location using a default search
	 * field named "default" and a StandardAnalyzer for index searching and
	 * creation.
	 *
	 * @param indexDirPath
	 *            The directory where the index is located or will be created.
	 */
	...
	/**
	 * Initializes or creates an index at the given location using a default search
	 * field named "default" and the given Analyzer.
	 *
	 * @param indexDirPath
	 *            The directory where the index is located or will be created.
	 * @param analyzer
	 *            The default Analyzer to use for searching and index creation
	 */
	...
	/**
	 * Initializes or creates an index at the given location using the default
	 * search field, additional stop words and analyzer indicated.
	 *
	 * @param indexDirPath
	 *            The directory where the index is located or will be created.
	 * @param defaultField
	 ...
	 *            The default Analyzer to use for searching and index creation
	 */
	public SimpleLuceneIndex(String indexDirPath, String defaultField, Analyzer analyzer) {
		try {
			if (analyzer == null)
	...
	private void init() {
		// If an old SimpleLuceneIndex (before 2005-04-28) is here, convert it to the
		// new style...
		...
		try {
			...
			try {
				synchronized (_updateLock) {
					...
					IndexWriter tmpWriter = new IndexWriter(FSDirectory.open(luceneIndexDir), this.getAnalyzer(), true,
	...
	/**
	 * Creates and loads a new IndexReader and IndexSearcher. Should be called only
	 * within a synchronized (_updateLock) block after any IndexWriter.close()
		...
		try {
			synchronized (_updateLock) {
				synchronized (_readerSearcherUpdateLock) {
					...
					/*
					 * Note: FSDirectory.open instantiates SimpleFSDirectory on Windows and
					 * NIOFSDirectory on Linux. Some NIO ClosedChannelExceptions were seen on Linux
					 * when using FSDirectory.open so trying SimpleFSDirectory instead...
					 */
					// _myReader = IndexReader.open(FSDirectory.open(luceneIndexDir));
					_myReader = IndexReader.open(new SimpleFSDirectory(luceneIndexDir)); // The basic directory reader,
																							// blocks on IO
																							// operations...
	...
	/**
	 * Sets an attribute that will be available for access in search results by
	 * calling {@link org.dlese.dpc.index.reader.DocReader#getAttribute(String)} or
	 * {@link ResultDoc#getAttribute(String)}.
	 *
	 * @param key
	...
	/**
	 * Gets an attribute from this SimpleLuceneIndex. Note that these attributes are
	 * available for access in search results by calling
	 * {@link org.dlese.dpc.index.reader.DocReader#getAttribute(String)} or
	 * {@link ResultDoc#getAttribute(String)}. The key 'thisIndex' returns this
	 * index.
	 *
	 * @param key
	 *            The key used to reference the attribute.
	 * @return The Java Object that is stored under the given key or null if none
	 *         exists.
	 * @see ResultDoc#getAttribute(String)
	 * @see org.dlese.dpc.index.reader.DocReader#getAttribute(String)
	 */
	...
	/**
	 * Performs a search over the index using the qiven query String, returning an
	...
	/**
	 * Performs a search over the index using the qiven query String and Analyzer,
	 ...
	 * @param query
	 *            The query to perform over the index.
	 * @param docReaderAttributes
	 *            Attributes that are included for use in DocReaders via the
	 *            ResultDocConfig.
	 * @return An ordered array of ranked results matching the given query.
	 * @see #setOperator(int operator)
	 * @see #getOperator()
	...
	/**
	 * Performs a search over the index using the Query object, returning an ordered
	 * array of matching ranked results.
	 *
	 * @param query
	 *            The Query to search over the index.
	 * @param docReaderAttributes
	 *            Attributes that are included for use in DocReaders via the
	 *            ResultDocConfig.
	 * @return An ordered array of ranked results matching the given query.
	...
	/**
	 * Performs a search over the index using the qiven query String, default field
	 * and Filter, returning an ordered array of matching ranked results.
	 *
	 * @param query
	 *            The query to perform over the index.
	 * @param defaultField
	...
	/**
	 * Performs a search over the index using the qiven query String and Filter
	 * using the pre-defined default field, returning an ordered array of matching
	 * ranked results.
	 *
	 * @param query
	 *            The query to perform over the index.
	...
	/**
	 * Performs a search over the index using the qiven Query and Filter using the
	 * pre-defined default field, returning an ordered array of matching ranked
	 * results.
	 *
	 * @param query
	 *            The Query to perform over the index.
	...
	/**
	 * Performs a search over the index using the qiven Query using the pre-defined
	 * default field, returning an ordered array of matching ranked results.
	 *
	 * @param query
	 *            The Query to perform over the index.
	 * @return An ordered array of ranked results matching the given query.
	...
	/**
	 * Performs a search over the index using the qiven Query Object and Filter
	 * using the pre-defined default field, returning an ordered array of matching
	 * ranked results.
	 *
	 * @param query
	 ...
	 *            The query to perform over the index.
	 * @param filter
	 *            A Filter used for the search or null for none.
	 * @param sortBy
	 *            A Sort to apply to the results or null to use relevancy ranking
	 * @param docReaderAttributes
	 *            Attributes that are included for use in DocReaders via the
	 *            ResultDocConfig or null for none
	 * @param analyzer
	 *            The analyzer to use, or null to use the default
	 * @return An ordered array of ranked results matching the given query.
	 * @see #setOperator(int operator)
	 * @see #getOperator()
	 */
	public ResultDocList searchDocs(String query, Filter filter, Sort sortBy, HashMap docReaderAttributes,
			Analyzer analyzer) {
		try {
	...
	/**
	 * Performs the actual search. See
	 * {@link #searchDocs(String, Filter, Collector)} for description. Note that
	 * this method uses a single copy of QueryParser, so it's not thread safe. Care
	 * should be taken to ensure that only one thread uses the search at a time.
	 *
	 * @param query
	 *            The query String or Query to perform over the index.
	 * @param defaultField
	 *            The default field to search in, or null to use the pre-configured
	 *            default field.
	 * @param filter
	 *            A filter used for the search, or null to use none.
	 * @param analyzer
	 *            The Analyzer to use, or null to use the default Analyzer then
	 *            KeywordFieldAnalyzer combo.
	 * @param docReaderAttributes
	 *            Attributes that will be available for access in ResulDocs.
	 * @param numTries
	 *            The current number of times this method has been called for a
	 *            given search
	 * @param sort
	 *            A Sort to apply to the results or null to use relevancy ranking
	 * @return An ordered List of ranked results matching the given query, or empty
	 *         List (never null)
	 * @see #setOperator(int operator)
	 * @see #getOperator()
	 */
	private final ResultDocList doSearchDocs(Object query, String defaultField, Filter filter, Sort sort,
			Analyzer analyzer, HashMap docReaderAttributes, int numTries) {
		...
		try {
			...
			// If query is a String...
				...
				if (defaultField == null && analyzer == null) {
					...
					// operator = "OR";
					// System.out.print("\nSimpleLuceneIndex 1 query is: " + query + " default
					// operator " + operator);
			...
			try {
					...
					if (_mySearcher != null) {
						...
						if (sort == null)
							...
							try {
								...
								// If the sort field is tokenized it can throw this exception, so perform the
								// search without sorting:
								topDocs = _mySearcher.search(luceneQueryObj, filter, MAX_NUM_HITS);
								prtlnErr("Requested sort field '" + sort
										+ "' appears to be invalid. Sort fields must contain a single token only (e.g. should not be analyzed). Sort criteria was ignored...");
							}
						}
					}
				}
				...
				prtlnErr("doSearchDocs() caught exception after " + numTries + " tries for query ' " + luceneQueryObj
						+ "' msg: " + e);
				e.printStackTrace();
			}
			...
			if ((topDocs == null || topDocs.totalHits == 0) && doKeywordSearch) {
				...
				try {
					...
					if (!(e instanceof ParseException) && numTries < MAX_TRIES)
						return doSearchDocs(query, defaultField, filter, sort, analyzer, docReaderAttributes,
								++numTries);
					// prtlnErr("doSearchDocs() 2 caught exception after " + numTries + " tries: " +
					// e);
					// e.printStackTrace();
				}
			}

			ResultDocConfig resultDocConfig = new ResultDocConfig(queryString, luceneQueryObj, filter,
					docReaderAttributes, this);
			...
			if (topDocs == null) {
				...
				// prtln("doSearchDocs() returning empty ResultDocList.size(): " +
				// resultDocList.size());
	...
	/**
	 * Searches the index and returns a ResultDocCollection that is backed by Lucene
	 * {@link org.apache.lucene.search.Hits}.
	 *
	 * @param operator
	 ...
	 */
	/*
	 * public final ResultDocCollection search( Object query, String defaultField,
	 * Filter filter, Analyzer analyzer, HashMap docReaderAttributes, int numTries)
	 * { if (query == null) return null; ResultDocConfig conf = null; Hits hits =
	 * null; try { // If an Analyzer is supplied, use only it. If no // Analyzer is
	 * supplied and this is not a Query, first use // the default Analyzer, then //
	 * a KeywordAnalyzer if no matches were found. boolean doKeywordSearch = true;
	 * Query q; if (query instanceof Query) { conf = new
	 * ResultDocConfig(query.toString(), filter, docReaderAttributes, this); q =
	 * (Query) query; doKeywordSearch = false; } else { conf = new
	 * ResultDocConfig((String) query, filter, docReaderAttributes, this); if
	 * (defaultField == null && analyzer == null) { // String operator = "AND"; //
	 * if(queryParser.getOperator() == QueryParser.DEFAULT_OPERATOR_OR) // operator
	 * = "OR"; // System.out.print("\nSimpleLuceneIndex 1 query is: " + query +
	 * " default operator " + operator); // q = getQueryParser().parse((String)
	 * query); } else { if (defaultField == null) defaultField = defaultSearchField;
	 * if (analyzer == null) analyzer = getAnalyzer(); else doKeywordSearch = false;
	 * QueryParser qp = new QueryParser(defaultField, analyzer);
	 * qp.setDefaultOperator(getLuceneOperator());
	 * //System.out.print("\nSimpleLuceneIndex 2 query is: " + query); q =
	 * qp.parse((String) query); } } //prtln("Parsed Query: '" + q.toString() +
	 * "'"); // Peform the search over the current read index try { synchronized
	 * (_readerSearcherUpdateLock) { if (_mySearcher != null) hits =
	 * _mySearcher.search(q, filter); } } catch (Throwable e) { if (!(e instanceof
	 * ParseException) && numTries < MAX_TRIES) return search(query, defaultField,
	 * filter, analyzer, docReaderAttributes, ++numTries);
	 * //prtlnErr("search() 1 caught exception after " + numTries + " tries: " + e);
	 * //e.printStackTrace(); } // If there were no matches, try searching using a
	 * KeywordFieldAnalyzer if ((hits == null || hits.length() == 0) &&
	 * doKeywordSearch) { if (defaultField == null) defaultField =
	 * defaultSearchField; QueryParser qp = new QueryParser(defaultField, new
	 * KeywordAnalyzer()); qp.setDefaultOperator(getLuceneOperator());
	 * //System.out.print("\nSimpleLuceneIndex 3 query is: " + query); q =
	 * qp.parse((String) query); // Peform the search over the current read index
	 * try { synchronized (_readerSearcherUpdateLock) { if (_mySearcher != null)
	 * hits = _mySearcher.search(q, filter); } } catch (Throwable e) { if (!(e
	 * instanceof ParseException) && numTries < MAX_TRIES) return search(query,
	 * defaultField, filter, analyzer, docReaderAttributes, ++numTries);
	 * //prtlnErr("search() 1 caught exception after " + numTries + " tries: " + e);
	 * //e.printStackTrace(); } } //System.out.println(". Num results: " +
	 * collector.results().length + "\n"); return new ResultDocCollection(hits,
	 * conf); } catch (Throwable e) { if (!(e instanceof ParseException) && numTries
	 * < MAX_TRIES) return search(query, defaultField, filter, analyzer,
	 * docReaderAttributes, ++numTries);
	 * //prtlnErr("search() 1 caught exception after " + numTries + " tries: " + e);
	 * //e.printStackTrace(); } return null; }
	 */
	/**
	 * Sets the boolean operator used during searches. Once set, the given boolean
	 * operator will be used for all subsequent searches. If this method is never
	 * called the boolean operator defaults to OR.
	 *
	 * @param operator
	 *            The new boolean operator value.
	 * @see #DEFAULT_OR
	 * @see #DEFAULT_AND
	 */
	...
	/**
	 * Gets a new instance of the {@link org.apache.lucene.queryParser.QueryParser}
	 * used by this SimpleLuceneIndex that uses it's Analyzers, defaultField and
	 * boolean operator settings.
	 *
	...
	public final QueryParser getQueryParser() {
		...
		// System.out.println("getQueryParser() using analyzer class: " +
		// defaultAnalyzer.getClass().getName());
		return parser;
	}

	/**
	 * Gets a new instance of the {@link org.apache.lucene.queryParser.QueryParser}
	 * used by this SimpleLuceneIndex that uses it's Analyzers and boolean operator
	 * settings, allowing one to specify the default search field.
	 *
	 * @param defaultSearchField
	 *            The search field used as default when none is specified in the
	 *            query
	 * @return The QueryParser used by this SimpleLuceneIndex with the given default
	 *         search field
	 */
	...
	/**
	 * Gets the boolean operator that is currently being used for searches as a
	 * String (AND or OR).
	 *
	...
	/**
	 * Gets the name of the field that is searched by default if no field is
	 * indicated.
	 *
	...
	/**
	 * Gets a list of all {@link org.apache.lucene.document.Document}s in the index.
	 * Note: This method loads all Documents and requires a large amount of memory
	 * for large result sets (consider using search instead).
	 *
	...
	/**
	 * Gets a list of all {@link org.apache.lucene.document.Document}s in the index
	 * that match the given term in the given field. Note: This method loads all
	 * Documents and requires a large amount of memory for large result sets
	...
	/**
	 * Gets a list of all {@link org.apache.lucene.document.Document}s in the index
	 * that match the given terms in the given field. Note: This method loads all
	 * Documents and requires a large amount of memory for large result sets
	 * (consider using search instead).
	 *
	 * @param field
	...
	/**
	 * Calls the callback function of cal for each document matching the terms in
	 * the given field
	 * 
	 * @param cal
	 * @param field
	...
	/**
	 * Calls the callback function of cal for each document matching the term in the
	 * given field
	 * 
	 * @param cal
	 * @param field
	...
	/**
	 * Helper function to create a list of Documents that match the given term in
	 * the given field.
	 *
	 * @param field
	 *            The field searched.
	...
	private final ArrayList doListDocs(String field, String term) {
		...
		// prtln("doListDocs() field:" + field + " term:" + term + " num results: " +
		// list.size());
		return list;
	}
	...
	/**
	 * Gets a list of all fields in the index listed alphabetically. Depending on
	 * the state of the index, the list may contain fileds that are empty, meaning
	 * all terms for the given field have been deleted and there are no possible
	 * matching queries within the field.
	 *
	...
	/**
	 * Gets a Map of Lists that contain the terms for each field in the index. The
	 * keys in the Map are Strings that represent all fields in the index. The List
	 * that is returned for each key contains all terms that are in the index for
	 * the given field.
	 *
	 ...
	 * Implementation note: this method is not efficient. If you need to use this
	 * method frequently, consider caching the results and using
	 * {@link #getLastModifiedCount()} to determe when to update the cache.
	 *
	 * @param field
	...
	/**
	 * Gets a Map of all terms that are in the index under the given field. The keys
	 * in the map are Strings that list the terms. The values in the Map are
	 * Integers that hold the total count of the terms in the given field across all
	 * documents.
	 * <p>
	 *
	 * Implementation note: this method is not efficient. If you need to use this
	 ...
	 * @param field
	 *            The indexed field name.
	 * @return Map containing terms/counts for all terms in the index under the
	 *         given field.
	 */
	...
	/**
	 * Gets a Map of all terms that are in the index. The keys in the map are
	 * Strings that list the terms. The values in the Map are Integers that hold the
	 * total count of the terms across all documents.
	 * <p>
	 *
	 * Implementation note: this method is not efficient. If you need to use this
	 * method frequently, consider caching the results and using
	...
	/**
	 * Gets a Map of all terms that are in the index under the given fields. The
	 * keys in the map are Strings that list the terms. The values in the Map are
	 * Integers that hold the total count of the terms in the given fields across
	 * all documents.
	 * <p>
	 *
	 * Implementation note: this method is not efficient. If you need to use this
	 ...
	 * Gets a Map of all terms that are in the index under the given fields. The
	 * keys in the map are Strings that list the terms. The values in the Map are
	 * {@link TermDocCount} Objects, which contain the term count, the total number
	 * of documents containing the term in one or more of the given field(s), and a
	 * list of fields in which the term appears.
	 * <p>
	 *
	 * Implementation note: this method is not efficient. If you need to use this
	 * method frequently, consider caching the results and using
	 * {@link #getLastModifiedCount()} to determe when to update the cache. Also,
	 * this method is considerably slower when more than one field is requested.
	 * This is because an extry query is required for each term that is found.
	 * <p>
	 *
	 *
	 *
	 * @param fields
	 *            The indexed field names.
	 * @return Map containing a {@link TermDocCount} Object for all terms in the
	 *         index under the given fields.
	 * @see TermDocCount
	 */
	 ...
	 */
	/*
	 * public List listStopWords() { String[] stopwords = null; try {
	 * LuceneIndexConfig cfg = getConfig(); stopwords = (cfg.stopWords == null) ?
	 * StopAnalyzer.ENGLISH_STOP_WORDS : cfg.stopWords; return
	 * Arrays.asList(stopwords); } catch (Exception e) {} return null; }
	 */
	/**
	...
	/**
	 * Adds a Document to the index. Blocks all other update operations until
	 * complete.
	 *
	 * @param doc
	 *            The Document to add.
	...
	/**
	 * Adds a group of Documents to the index. Blocks all other update operations
	 * until complete.
	 *
	 * @param docs
	...
	/**
	 * Removes all Documents that match the given term within the given field. This
	 * is useful for removing a single document that is indexed with a unique ID
	 * field, or to remove a group of documents mathcing the same term for a given
	 * field. Blocks all other index update operations until this is complete.
	 *
	 * @param field
	...
	/**
	 * See {@link #removeDocs(String,String)} for description. Adds the ability to
	 * control whether blocking occurs during the update.
	 *
	 * @param field
	 *            The field that is searched.
	 * @param value
	...
	/**
	 * Removes all documents that match the given terms within the given field. This
	 * is useful for removing all individual documents that are indexed with a
	 * unique ID field. Blocks all other index update operations until this is
	 * complete.
	 *
	 * @param field
	 *            The field that is searched.
	...
	/**
	 * Updates the index by first deleting the documents that match the value(s)
	 * indicated in <code>deleteValues</code> in the field <code>deleteField,</code>
	 * then adding the documents in <code>addDocs</code>. Assuming the
	 * <code>deleteField</code> contains a unique ID for the Document, the Document
	 * may be removed by indicating the ID in the <code>deleteValues</code> list. To
	 * replace an entry in the index for a single item, supply the item's ID in the
	 * <code>deleteValues</code> list and supply the new Document for the item in
	 * the <code>addDocs</code> list.
	 *
	 * @param deleteField
	 *            The field searched for <code>deleteValues</code>.
	 * @param deleteValues
	 *            The value matched in <code>deleteField</code> to indicate which
	 *            document(s) to delete.
	 * @param addDocs
	 *            An array of Documents to add to the index
	 * @param block
	 ...
	 * Updates the index by first deleting the documents that match the value(s)
	 * indicated in <code>deleteValues</code> in the field <code>deleteField,</code>
	 * then adding the documents in <code>addDocs</code>. See
	 * {@link #update(String, String[], Document[], boolean)} for description.
	 * Performs an update with blocking on.
	 *
	 * @param deleteField
	 *            The field searched for <code>deleteValues</code>.
	 * @param deleteValues
	 *            Array of Strings containing the value matched in
	 *            <code>deleteField</code> to indicate which document(s) to delete
	 * @param addDocs
	 ...
	 *
	 * @param deleteField
	 *            The field searched for <code>deleteValue</code>.
	 * @param deleteValue
	 *            Matching docs are deleted.
	 * @param addDocs
	 *            These Docs are added to the index
	 * @param block
	 ...
	 *            Matching docs are deleted.
	 * @param addDoc
	 *            The Doc to be added to the index
	 * @param block
	 *            Block or run in background.
	 * @return True if no errors.
	 */
	 ...
	 *            The field searched for <code>deleteValues</code>.
	 * @param deleteValues
	 *            ArrayList of Strings containing the value matched in
	 *            <code>deleteField</code> to indicate which document(s) to delete
	 * @param addDocs
	...
	public boolean update(String deleteField, ArrayList deleteValues, ArrayList addDocs, boolean block) {
		...
		try {
			return update(deleteField, (String[]) deleteValues.toArray(new String[]{}),
					(Document[]) addDocs.toArray(new Document[]{}), block);
		} catch (Throwable e) {
	...
	/**
	 * Gets the version number of the last time the index was modified by adding,
	 * deleting or changing a document. The version number counts the number of
	 * times the index was modified. If the index is deleted and rebuilt, the count
	 * will continue to be incremented until the next time the JVM is re-started.
	 * After the JVM has been re-started, the count will resume with the count of
	...
	/**
	 * Indicates whether the index is currently being updated or modified. This
	 * means documents are in the process of being added or removed from the index.
	 *
	...
	/**
	 * Instructs the indexer to stop processing updates. Once complete, the index
	 * will be ready for future updating and searching but any additions or
	 * deletions that had not been completed will be lost. This method may take
	 * several seconds to return.
	 *
	 * @see #isIndexing()
	 */
	 ...
	 *
	 * @param deleteField
	 *            The field name for which deletes should be checked.
	 * @param deleteValues
	 *            Values found in the deleteField when matched, the Document will be
	 *            deleted from the index.
	 * @param addDocs
	 *            Documents to add to the index after the deletes are performed.
	 * @return True if successful.
	 */
	private final boolean doUpdateIndex(String deleteField, String[] deleteValues, Document[] addDocs) {
		...
		synchronized (_updateLock) {
			...
			try {
				...
				tmpWriter = new IndexWriter(FSDirectory.open(luceneIndexDir), getAnalyzer(), false,
						IndexWriter.MaxFieldLength.UNLIMITED);
				isIndexing = true;

				/*
				 * if (deleteValues != null && deleteValues.length > 0)
				 * prtln("processing deleteField: " + deleteField + " deleteValues[0]: " +
				 * deleteValues[0]); else prtln("processing deleteField: " + deleteField +
				...
				if (deleteField != null && deleteValues != null && deleteValues.length > 0) {
					...
					for (int i = 0; i < deleteValues.length && !abortUpdate; i++) {
						if (deleteValues[i] != null) {
							try {
								...
								// changed = true;
								// prtln("Deleting " + deleteField + ":" + deleteValues[i] + " had " + count + "
								// deletions");
					...
					/*
					 * } catch (IOException ioe) { // this means we had trouble opening a reader on
					 * the writeDirectory?! prtlnErr("doUpdateIndex() deletion exception: " + ioe);
					 * try { if (tmpWriter != null) { tmpWriter.close(); tmpWriter = null; } } catch
					 * (Throwable t) { prtlnErr("doUpdateIndex() error closing writer: " + t); }
					 * loadNewReaderAndSearcher(); isIndexing = false; return false; }
					 */
				} else {
				...
				try {
					...
					/*
					 * if (tmpWriter != null) { tmpWriter.close(); tmpWriter = null; }
					 */
				} catch (IOException ioe) {
	...
	/**
	 * Monitors the index and performs delayed optimization if enough time has
	 * elapsed since the last write operation.
	 *
	 * @author jweather
	 */
	private class OptimizeMonitorThread extends Thread {
		 ...
		 *
		 * @param syncObj
		 *            The object used to synchronize indexing write operations
		 */
						...
						if ((System.currentTimeMillis() - last_update_time) > OPTIMIZE_DELAY_TIME) {
							synchronized (_updateLock) {
								if (isRunning) {
									IndexWriter tmpWriter = null;
									try {
										tmpWriter = new IndexWriter(FSDirectory.open(luceneIndexDir), getAnalyzer(),
	...
	/**
	 * Escapes all Lucene QueryParser reserved characters with a preceeding \. The
	 * resulting String will be interpereted by the QueryParser as a single term.
	 *
	 * @param term
	 *            The original String
	...
	/**
	 * Escapes the Lucene QueryParser reserved characters with a preceeding \ except
	 * those included in preserveChars.
	 *
	 * @param term
	 *            The original String
	 * @param preserveChars
	...
	/**
	 * Encodes a String to an appropriate format that can be indexed as a single
	 * term using a StandardAnalyzer. White-space is also encoded and incorporated
	 * into the single term. Note that this can not be unencoded. Save the value of
	 * the term in a separate field if it needs to be retrieved for display.
	 * <p>
	 *
	 * Specifically: each letter or number character is left unchanded. All other
	 * characters are encoded as the letter 'x' followed by the integer value of the
	 * character, for example '@' is encoded as 'x64'.
	 *
	 * @param s
	 ...
	 * Encodes a String to an appropriate format that can be indexed as a single
	 * term using a StandardAnalyzer. White-space is also encoded and incorporated
	 * into the single term. Leaving the wild card '*' char un-encoded will produce
	 * a String that can be used to search encoded terms using wild cards. Note that
	 * this can not be unencoded. Save the value of the term in a separate field if
	 * it needs to be retrieved for display.
	 * <p>
	 *
	 * Specifically: each letter or number character is left unchanded. All other
	 * characters are encoded as the letter 'x' followed by the integer value of the
	...
	/**
	 * Encodes a String to an appropriate format that can be indexed as a single
	 * term or terms using a StandardAnalyzer. Leaving the space char un-encoded
	 * will produce a String that will be tokenized by the space char into
	 * individual terms. Leaving the wild card '*' char un-encoded will produce a
	 * String that can be used to search encoded terms using wild cards. Note that
	 * this can not be unencoded. Save the value of the term in a separate field if
	 * it needs to be retrieved for display.
	 * <p>
	 *
	 ...
	 *            True to have the '*' char encoded, false to leave it un-encoded.
	 * @param encodeSpace
	 *            True to have the space ' ' char encoded, false to leave it
	 *            un-encoded.
