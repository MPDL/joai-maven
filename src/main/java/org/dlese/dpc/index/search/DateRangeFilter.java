...
/**
 * Filters Lucene search results based on a range of Dates or times. This
 * implements similar functionality as the the deprecated Lucene 1.4 DateFilter.
 * Assumes that fields are indexed using the Lucene DateTools format of
 * yyyyMMddHHmmss, to a granularity of seconds.
 *
...
public class DateRangeFilter extends Filter {
	 ...
	 * inclusively. Uses time resolution to seconds.
	 *
	 * @param f
	 *            The field name
	 * @param from
	...
	/**
	 * Constructs a filter for field f matching dates on or before date. Uses time
	...
	public static DateRangeFilter Before(String field, Date date) {
		return new DateRangeFilter(
				TermRangeFilter.Less(field, DateTools.dateToString(date, DateTools.Resolution.SECOND)));
	}

	/**
	 * Constructs a filter for field f matching dates on or after date. Uses time
	 * resolution to seconds.
	 *
	 * @param field
	 *            Field name
	...
	public static DateRangeFilter After(String field, Date date) {
		return new DateRangeFilter(
				TermRangeFilter.More(field, DateTools.dateToString(date, DateTools.Resolution.SECOND)));
	}

	/**
	 * Constructs a filter for field f matching times on or before time. Uses time
	 * resolution to seconds.
	 *
	 * @param field
	 *            Field name
	...
	/**
	 * Constructs a filter for field f matching times on or after time. Uses time
	 * resolution to seconds.
	 *
	 * @param field
	 *            Field name
