 ...
 */
/**
 * Indexes files into a {@link org.dlese.dpc.index.SimpleLuceneIndex} and
 * automatically updates the index whenever changes to the files are made. This
 * class uses a {@link org.dlese.dpc.index.writer.FileIndexingServiceWriter} to
 * create the Lucene {@link org.apache.lucene.document.Document}s that are
 * placed in the {@link org.dlese.dpc.index.SimpleLuceneIndex}. This class looks
 * for changes made to items in a directory of files and updates the index
 * automatically by adding, updating or deleting items as appropriate. The
 * frequency for update checkes is configurable. There should be only one
 * instance of this class for each {@link org.dlese.dpc.index.SimpleLuceneIndex}
...
public final class FileIndexingService {
	...
	/**
	 * Indicates that indexing completed successfully, but one or more item was
	 * indexed with errors
	 */
	...
	/**
	 * Indexes files to the given {@link org.dlese.dpc.index.SimpleLuceneIndex},
	 * checking for changes in the files and reindexing them at the given update
	 * frequency. Validation of files is enabled by default.
	 *
	 * @param index
	 *            The {@link org.dlese.dpc.index.SimpleLuceneIndex} that will be
	 *            populated and updated with Documents created from files
	 * @param updateFrequency
	 *            The frequency by which files are checked for updates, in seconds.
	 *            Zero or less indicates no updates should be performed.
	 * @param fileIndexingServiceDataDir
	 *            The directory where serialized data will be stored
	 * @param maxNumFilesToIndex
	 *            Max number of files to index per iteration
	 ...
	 * @param idFieldToRemove
	 *            An ID field whoes docs should be removed if found in duplicate.
	 * @param fileIndexingServiceDataDir
	 *            Dir where persistent data files will be stored
	 * @param maxNumFilesToIndex
	 *            The number of files to index per iteration
	 * @see #setValidationEnabled(boolean validateFiles)
	 */
	public FileIndexingService(SimpleLuceneIndex index, long updateFrequency, boolean saveDeletes,
			String idFieldToRemove, String fileIndexingServiceDataDir, int maxNumFilesToIndex) {
		this.index = index;
	...
	/**
	 * Sets an attribute Object that will be available for access here and from the
	 * FileIndexingServiceWriters.
	 *
	 * @param key
	...
	/**
	 * Stops the indexing process if it is currently taking place. This method may
	...
	/**
	 * Determines whether the indexing process is waiting to stop. This should be
	 * called by indexing threads in this class prior to indexing a block of files.
	 * If true, they should stop their indexing and decrement numIndexingQue;
	 *
	...
	/**
	 * Start or restarts the timer thread with the given update frequency. Same as
	...
	public void startTimerThread(long updateFrequency) {
		...
		if (updateFrequency > 0) {
			...
			timer.schedule(new FileSyncTask(), 6000, ((updateFrequency > 0) ? updateFrequency * 1000 : 60000));
	...
	/**
	 * Sets whether or not to validate the files being indexed and create a
	 * validation report, which is indexed. If set to true, the files will be
	 * validated, otherwise they will not. Default is true.
	 *
	 * @param validateFiles
	 *            True to validate, else false.
	...
	/**
	 * Adds a directory of files to be monitored for changes, or replaces the
	 * current one if one exists with the same absolute path.
	 *
	 * @param sourceFileDirectory
	 *            The file direcory that will be monitored for updates.
	 * @param indexingPriority
	 *            The feature to be added to the Directory attribute
	 * @param documentWriterClass
	 *            The feature to be added to the Directory attribute
	 * @param documentWriterConfigAttributes
	 *            The feature to be added to the Directory attribute
	 * @param plugin
	 *            The feature to be added to the Directory attribute
	 * @return True if the directory was added successfully.
	 */
	public boolean addDirectory(String sourceFileDirectory, Class documentWriterClass,
			HashMap documentWriterConfigAttributes, FileIndexingPlugin plugin, int indexingPriority) {
		...
		// Make sure index is in the config since it's used by XMLFileIndexingWriter and
		// others
	 ...
	 * @return True if the directory was added successfully.
	 */
	public boolean addDirectory(File srcDir, Class documentWriterClass, HashMap documentWriterConfigAttributes,
			FileIndexingPlugin plugin, int indexingPriority) {
		...
		// To do: if a dir was deleted, then added back, need to notify index to
		// re-index those files...
		synchronized (sourceDirs) {
			sourceDirs.put(srcDir.getAbsolutePath(), new FISDirInfo(srcDir, documentWriterClass,
					documentWriterConfigAttributes, plugin, this, indexingPriority));
		}
		synchronized (sourceDirsHistory) {
			sourceDirsHistory.put(srcDir.getAbsolutePath(), new FISDirInfo(srcDir, documentWriterClass,
					documentWriterConfigAttributes, plugin, this, indexingPriority));
		}
		return true;
	}
	 ...
	 * @return True if this directory is already configured for indexing, false
	 *         otherwise.
	 */
	...
	/**
	 * Gets a HashMap of all directories that are configured in this
	 * FileIndexingService, keyed by absolute path.
	 *
	...
	/**
	 * Gets a new fileIndexingServiceWriter that is configured for the given source
	 * directory, or null if none exists.
	 *
	 * @param srcDir
	 *            Absolute path to the src directory for the file
	...
	/**
	 * Deletes the files in the given directory from the index and removes it from
	 * the configuration. Assumes the directory was previously added to the index
	 * using the {@link #addDirectory} method.
	 *
	 * @param sourceFileDirectory
	...
	/**
	 * Updates the index to reflect the files in the directories this service is
	 * monitoring, with the option to run the update in the background. Any new,
	 * deleted or modified files that appear in the directories will be reflected in
	 * the index.
	 *
	 * @param reindexAll
	 *            True to reindex all files regardless of file mod date, False to
	 *            reindex only those files that have changed since the last
	 *            indexing.
	 * @param observer
	...
	/**
	 * Updates the index to reflect the files in the directory indicated, which must
	 * have been previously added to this FileIndexingService using
	 * {@link #addDirectory}. Any new, deleted or modified files that appear in the
	 * directory will be reflected in the index.
	 *
	 * @param reindexAll
	 *            True to reindex all files regardless of file mod date, False to
	 *            reindex only those files that have changed since the last
	 ...
	 *            The file to index.
	 * @param plugin
	 *            A FileIndexingPlugin or null.
	 * @exception FileIndexingServiceException
	 *                If unable to index
	 */
	public void indexFile(File fileToIndex, FileIndexingPlugin plugin) throws FileIndexingServiceException {
		// prtln("indexFile...");
		synchronized (_indexingSync) {
			...
			try {
				if (isIndexingWaitingToStop())
					throw new FileIndexingServiceException("Indexing stop signal has been requested. File index file: "
							+ fileToIndex.getAbsolutePath() + " will not be indexed.");
				if (fileToIndex == null)
				...
				if (fileToIndex.isDirectory())
					throw new FileIndexingServiceException(
							"File '" + fileToIndex.getAbsolutePath() + "' is a directory, not a file");
	...
	/**
	 * Updates the index for just the single directory of files indicated. Any new,
	 * deleted or modified files that appear in the directory will be reflected in
	 * the index. This process may take several minutes to return depending on the
	 * number of files needing to be updated. If this FileIndexingService has not
	 * been configured for this directory, no indexing takes place.
	 *
	 * @param reindexAll
	 *            True to reindex all records regardless of file mod date.
	 * @param directory
	 *            The directory to index
	 * @param observer
	 *            The FileIndexingObserver that will be notified when indexing is
	...
	private void synchIndexWithFiles(boolean reindexAll, File directory, FileIndexingObserver observer) {
		synchronized (_indexingSync) {
			...
			String message = "";
			if (isIndexingWaitingToStop()) {
				message = "Indexing stop signal has been requested. Directory: " + directory.getAbsolutePath()
						+ " will not be indexed.";
				// prtln(message);
				try {
					if (observer != null)
			...
			if (!dirIsConfigured) {
				message = "Error indexing files. The directory " + directory.getAbsolutePath()
						+ " is not configured for indexing.";
				addIndexingMessage(message);
				prtlnErr(message);
				try {
					if (observer != null)
			...
			if (result == INDEXING_ERROR) {
				message = "Errors occured while indexing directory '" + directory.getName()
						+ "'. Some or all items did not "
				...
				message = "Completed indexing directory '" + directory.getName()
						+ "' successfully, however one or more items was indexed with errors. Total time " + elapsedTime
						+ ". Total number of items in index: " + index.getNumDocs();
				addIndexingMessage(message);
				...
				message = "Completed indexing directory '" + directory.getName() + "' successfully. Total time "
						+ elapsedTime + ". Total number of items in index: " + index.getNumDocs();
				addIndexingMessage(message);
			}
			// Notify the observer
	...
	/**
	 * Updates the index to reflect the files in the directories this service is
	 * monitoring. Any new, deleted or modified files that appear in the directories
	 * will be reflected in the index. This process may take several minutes to
	 * return depending on the number of files needing to be updated.
	 *
	 * @param reindexAll
	 *            True to reindex all records regardless of file mod date.
	 * @param observer
	...
	private void synchIndexWithFiles(boolean reindexAll, FileIndexingObserver observer) {
		synchronized (_indexingSync) {
			...
			if (result == INDEXING_ERROR) {
				...
				synchronized (sourceDirs) {
					message = "Completed indexing " + sourceDirs.size() + " directories successfully. Total time "
							+ elapsedTime + ". Total number of items in index: " + index.getNumDocs();
				}
				addIndexingMessage(message);
			}
	...
	/**
	 * Removes any documents in the index that belong to a directory that is
	 * configured in this FileIndexingService but does not exist or to a directory
	 * that was once configured but was subsequently deleted using the
	 * deleteDirectory() method. Nothing is done if the the directory exists and is
	 * configured for indexing in this FileIndexingService.
	 */
	 ...
	 * that was once configured but was subsequently deleted using the
	 * deleteDirectory() method. Nothing is done if the the directory exists and is
	 * configured for indexing in this FileIndexingService. This method does not
	 * effect whether the direcory is configured for indexing. Use method
	 * deleteDirectory() to remove a directory for indexing from this
	 * FileIndexingService.
	 *
	 * @param dirToRemove
	 *            A direcory whoes indexed files will be removed from the index if
	 *            appropriate.
	 */
	private final void removeDirFromIndex(File dirToRemove) {
		try {
			...
			// prtln("removeDirFromIndex(): " + dirToRemove + " exists:" +
			// dirToRemove.exists() + " isConfigured:" + dirIsConfigured);
			...
			if (!dirIsConfigured || !dirToRemove.exists()) {
				...
				if (dirInfo != null) {
					...
					// prtlnErr("removeDirFromIndex(): could not find dirInfo for dir " +
	 ...
	 * Removed documents will either be saved in the index and marked as deleted
	 * (indicated by the Lucene field "deleted" being indexed as "true"), or removed
	 * from the index altogether as determined by the parameter passed in at the
	 * constructor. This is useful for removing a single document that is indexed
	 * with a unique ID field, or to remove a group of documents mathcing the same
	 * term for a given field. For example you might pass in an ID of a record that
	 * needs to be removed along with the ID field that it is indexed under, or the
	 * file path corresponding to a record along with the field "docsource." Note
	 * this is the same as {@link SimpleLuceneIndex#removeDocs(String,String)} but
	 * is synchronized with other operations occuring in this FileIndexinService and
	 * handles deletes accordingly.
	 *
	 * @param field
	 *            The field that is searched.
	 * @param term
	 *            The term that is matched for removal.
	 * @param docWriter
	 *            The FileIndexingServiceWriter to use
	 */
	 ...
	 * (indicated by the Lucene field "deleted" being indexed as "true"), or removed
	 * from the index altogether as determined by the parameter passed in at the
	 * constructor. This is useful for removing multiple documents that are indexed
	 * with a unique ID field. For example you might pass in an array of IDs needing
	 * to be removed. Note this is the same as
	 * {@link SimpleLuceneIndex#removeDocs(String,String[])} but is synchronized
	 * with other operations occuring in this FileIndexinService and handles deletes
	 * accordingly.
	 *
	 * @param field
	 *            The field that is searched.
	 * @param terms
	 *            The terms that are matched for removal.
	 * @param docWriter
	 *            The FileIndexingServiceWriter to use
	 */
	 ...
	 * Removed documents will either be saved in the index and marked as deleted
	 * (indicated by the Lucene field "deleted" being indexed as "true"), or removed
	 * from the index altogether as determined by the parameter passed in to this
	 * method. This is useful for removing a single document that is indexed with a
	 * unique ID field, or to remove a group of documents mathcing the same term for
	 * a given field. For example you might pass in an ID of a record that needs to
	 * be removed along with the ID field that it is indexed under, or the file path
	 * corresponding to a record along with the field "docsource." Note this is the
	 * same as {@link SimpleLuceneIndex#removeDocs(String,String)} but is
	 * synchronized with other operations occuring in this FileIndexinService and
	 * handles deletes accordingly.
	 *
	 * @param field
	 *            The field that is searched.
	...
	public final void removeDocs(String field, String term, FileIndexingServiceWriter docWriter,
		...
		// Note: Method refactored on 8/29/06 to call the following instead of full
		// redundent implementaion here
	 ...
	 * (indicated by the Lucene field "deleted" being indexed as "true"), or removed
	 * from the index altogether as determined by the parameter passed in to this
	 * method. This is useful for removing multiple documents that are indexed with
	 * a unique ID field. For example you might pass in an array of IDs needing to
	 * be removed. Note this is the same as
	 * {@link SimpleLuceneIndex#removeDocs(String,String[])} but is synchronized
	 * with other operations occuring in this FileIndexinService and handles deletes
	 * accordingly.
	 *
	 ...
	 *            deleted, else they will be removed from the index.
	 * @param docWriter
	 *            Writer used to perform the delete.
	 */
	public final void removeDocs(String field, String[] terms, FileIndexingServiceWriter docWriter,
			boolean saveDeletes) {
		synchronized (_indexingSync) {
			...
			if (saveDeletes) {
				...
				/**
				 * This class implements the functionality of deleteDocs The index updates are
				 * performed with chunks of chunksize, set to maxNumFilesToIndex
				 * 
				...
				// RemoveDocCallback c = new RemoveDocCallback(primaryIndex, maxNumFilesToIndex,
				// field, terms);
	 ...
	 * Requires that the file for the given document is still in it's original
	 * location. If the file is not in it's original location then the index will
	 * remove the document without updating and it will not be marked as deleted.
	 * This is useful for updating a single document that is indexed with a unique
	 * ID field, or to update a group of documents mathcing the same term for a
	 * given field. For example you might pass in an ID of a record that needs
	 * updating along with the ID field that it is indexed under, or the file path
	 * corresponding to a record that needs updating along with the field
	 * "docsource."
	 *
	 * @param field
	 *            The field that is searched.
	 * @param term
	 *            The term that is matched for updates.
	 * @param reindexAll
	 ...
	 * This is useful for updating multiple documents that are indexed with a unique
	 * ID field. For example you might pass in an array of IDs needing to be updated
	 * along with the ID field that it is indexed under, or an array of file paths
	 * corresponding to records that need updating along with the field "docsource."
	 *
	 * @param field
	 *            The field that is searched.
	 * @param terms
	 *            The terms that are matched for updates.
	 * @param reindexAll
	 *            True to reindex all matching results, false to reindex only those
	 *            matches whoes files have been modified since the last update.
	 * @return The number of matching documents to be updated.
	...
	/**
	 * Reindexes Documents managed by this FileIndexingService that match the given
	 * Lucene query.
	 *
	 * @param query
	...
	/**
	 * Synchronizes the files with the index by adding, deleting or updating the
	 * appropriate entries in the index when files change.
	 *
	 * @param index
	 *            The index to store the entries
	 * @param srcDir
	 *            The source file directory.
	 * @param reindexAll
	 *            True to force re-indexing of all files.
	 * @return The final indexing status
	 * @exception FileIndexingServiceException
	 *                If error
	 */
	private int synchDirectory(File srcDir, boolean reindexAll, SimpleLuceneIndex index)
		...
		String message = srcDir.getAbsolutePath() + "-start--numADN:" + ADNFileIndexingWriter.getNumInstances()
				+ "-numCOLL:" + DleseCollectionFileIndexingWriter.getNumInstances() + "-numANNO:"
				+ DleseAnnoFileIndexingServiceWriter.getNumInstances();
		if (debugMemory)
		...
		message = srcDir.getAbsolutePath() + "-beforGC--numADN:" + ADNFileIndexingWriter.getNumInstances() + "-numCOLL:"
				+ DleseCollectionFileIndexingWriter.getNumInstances() + "-numANNO:"
				+ DleseAnnoFileIndexingServiceWriter.getNumInstances();
		if (debugMemory)
			Utils.runCommand("/home/jweather/bin/log_process_memory.sh " + message, null, null);
		// Run garbage collection...
		MemoryCheck.runGC();
		message = srcDir.getAbsolutePath() + "-end--numADN:" + ADNFileIndexingWriter.getNumInstances() + "-numCOLL:"
				+ DleseCollectionFileIndexingWriter.getNumInstances() + "-numANNO:"
				+ DleseAnnoFileIndexingServiceWriter.getNumInstances();
		if (debugMemory)
			Utils.runCommand("/home/jweather/bin/log_process_memory.sh " + message, null, null);
	...
	/**
	 * Indexes or re-indexes the files or Documents indicated. Supported Objects are
	 * File, Document and ResultDoc.
	 *
	 * @param docsToUpdate
	 *            Must be of type File[], Document[] or ResultDocList.
	 * @param srcDir
	 *            The source file directory or null. Must be supplied if
	 *            docsToUpdate is of type File[], else it's not used.
	 * @param reindexAll
	 *            True to reindex all documents even if a document alredy exists for
	 *            the item, else false to only index those items whoes file mod time
	 ...
	 *                If error.
	 */
	private int doIndexObjects(Object docsToUpdateObj, File srcDir, boolean reindexAll, SimpleLuceneIndex index)
			throws FileIndexingServiceException {
		...
		synchronized (_indexingSync) {
			...
			if (isIndexingWaitingToStop()) {
				numIndexingQue--;
				// prtln("Indexing has been requested to stop. Method indexObjects() is
				// stopping...");
			...
			if (docsToUpdateObj instanceof File[]) {
				...
				numIndexingQue--;
				throw new FileIndexingServiceException(
						"The docsToUpdate Object must be of type File[], Document[] or ResultDocList");
			}
			...
			try {
				...
				// Initialize the idMapper ToDo: refactor so this does not need to occur each
				// time!
				if (recordDataService != null)
					recordDataService.initIdMapper();
				...
				for (int i = 0; i < docsToUpdateList.size() && !stopIndexing; i++) {
					...
					// Handle Document [] or ResultDocList
						...
						if (object_type == DOCUMENT_ARRAY) {
							...
							tmpReader = new SimpleFileIndexingServiceDocReader(
									((ResultDoc) docsToUpdateList.get(i)).getDocument());
						}
						...
						myDocWriter = getNewFileIndexingServiceWriter(srcDir.getAbsolutePath());
						myDocWriter.setValidationEnabled(validateFiles);
						// System.out.println("just got a new DocWriter Not File " + (myDocWriter ==
					...
					if (thisDoc != null && thisDoc.size() == 1) {
						...
						// Convert file mod time using DateFieldTools.timeToString to ensure same
						// granularity:
						String file_mtime = DateFieldTools.timeToString(fileToUpdate.lastModified());

						if (reindexAll || !doc_mtime.equals(file_mtime) || isErrorDoc || isDeleted) {
							...
							try {
								...
								if (indexErrors) {
									...
									// e.printStackTrace();
									ErrorFileIndexingWriter errDoc = new ErrorFileIndexingWriter(e);
					...
					if (addDocs.size() >= MAX_SIZE) {
						/*
						 * prtln("synchDirectory2 adding " + addDocs.size() + " files and removing " +
						 * removeFilesByPath.size() + " files");
						 */
						// Remove docs that the writers have requested to be removed and update the
						// index
				...
				// Note: The below listDocs line creates a spike in memory (jconsole debugging).
				// Refactor!
				...
				/**
				 * This class implements the functionality of listDocs and the former loop over
				 * its result It removes files that are in the index but not in the directory It
				 * avoids the previous memory spike, by treating each document separately and
				 * only storing filenames of documents that need to be removed
				 * 
				 * @author Timo Proescholdt <timo@proescholdt.de>
				 *
				 */
				class CleanIndexCallback implements Callback {
					...
					public CleanIndexCallback(FileIndexingServiceWriter myDocWriter, boolean saveDeletes,
							Hashtable filesHash, ArrayList removeFilesByPath, ArrayList addDocs) {
				...
				CleanIndexCallback c = new CleanIndexCallback(myDocWriter, saveDeletes, filesHash, removeFilesByPath,
						addDocs);
				...
				/*
				 * prtln("synchDirectory adding " + addDocs.size() + " files and removing " +
				 * removeFilesByPath.size() + " files");
				 */
				// Remove docs that the writers have requested to be removed and update the
				// index
	...
	private void removeDocsFromIndex(SimpleLuceneIndex index, HashMap docsToRemove) {
		...
		for (int i = 0; i < removeFields.length; i++) {
			...
			// prtln("removeDocsFromIndex(): removing " + values.size() + " from field " +
			// removeFields[i]);
	...
	/**
	 * Data structure that holds information about a directory of metadata files and
	 * factory for creating the appropriate FileIndexingServiceWriter for handling
	 * this directory. The indexing priority may also be indicated (PRIORITY_0 to
	 * PRIORITY_9). If none is specified the default priority is set to PRIORITY_5.
	 * High-numbered priorities will get indexed before low-numbered priorities by
	 * the {@link org.dlese.dpc.index.FileIndexingService}.
	 *
	 * @author John Weatherley
	 */
	class FISDirInfo implements Serializable, Comparable {
		 ...
		 * @param docWriterClass
		 *            The {@link FileIndexingServiceWriter} class that handles indexing
		 *            this type of file
		 * @param docWriterConfigAttributes
		 *            Config attributes passed into the
		 *            {@link FileIndexingServiceWriter}
		 * @param plugin
		 *            {@link FileIndexingPlugin} passed into the
		 *            {@link FileIndexingServiceWriter}
		 * @param fis
		 *            The FileIndexingService, which is passed in to the
		 *            {@link FileIndexingServiceWriter}
		 * @param indexingPriority
		 *            The indexing priority
		 */
		public FISDirInfo(File srcDir, Class docWriterClass, HashMap docWriterConfigAttributes,
				FileIndexingPlugin plugin, FileIndexingService fis, int indexingPriority) {
			this.srcDir = srcDir;
		...
		/**
		 * A factory method that instantiates a new FileIndexingServiceWriter for
		 * handling this type of file and initializing it for writing.
		 *
		 * @return A new FileIndexingServiceWriter
		 */
		public FileIndexingServiceWriter getDocWriter() {
			if (docWriterClass == null) {
				// Later: implement to return a default writer (perhaps self-configures based on
				// file extension, etc...).
				return null;
			} else {
				try {
		...
		/**
		 * Compares two DirInfos by the index priority. Collections.sort() or
		 * Arrays.sort() can thus be used to sort a list of DirInfos by priority.
		 *
		 * @param o
		 *            The DirInfos to compare
		 * @return Returns a negative integer, zero, or a positive integer as this
		 *         object is less than, equal to, or greater than the specified object.
		 * @exception ClassCastException
		 *                If the object passed in is not a DirInfos.
		 */
	...
	/**
	 * Synchronizes new, deleted or modified files with a Lucene index at a regular
	 * interval.
	 *
	 * @author John Weatherley
	 */
	...
	private void addIndexingMessage(String msg) {
		ArrayList indexingMessages = (ArrayList) dataStore.get("INDEXING_STATUS_MESSAGES");
	...
	// -------------------- Testing methods -------------------
	/**
	 * Starts a FileMoveTester iff one is not already initialized. The
	 * FileMoveTester simulate moving files in and out of the sourceFile directory,
	 * for testing purposes only. Warning: FileMoveTester moves metadatafiles. Only
	 * use with test records!)
	 *
	 * @param docRoot
