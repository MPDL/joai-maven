...
/**
 * Creates a Lucene Query from an XML representation of a query. See
 * <a href="../../../../../javadoc-includes/XMLQueryParser-sample.xml">sample
 * Query XML file</a> . Currently supports term, phrase, boolean and Lucene
 * query syntax formatted queries, with option to set the boost factor assigned
 * to any query (default boost is 1.0). If a Lucene query syntax based query may
 * be found in the Query XML, then a Lucene QueryParser must be supplied,
...
public class XMLQueryParser {
	 ...
	 *            A XML Query
	 * @param queryParser
	 *            QueryParser used to parse Lucene syntax based queries and tokenize
	 *            the text
	 ...
	 *
	 * @param queryXml
	 *            A file containing an XML Query
	 * @param queryParser
	 *            QueryParser used to parse Lucene syntax based queries and tokenize
	 *            the text
	...
	public static Query getLuceneQuery(File queryXml, QueryParser queryParser) throws Exception {
		return doGetLuceneQuery(Dom4jUtils.getXmlDocument(new URL("file://" + queryXml.getAbsolutePath())),
				queryParser);
	}
	 ...
	 *
	 * @param queryXml
	 *            A URL to a file containing an XML Query
	 * @param queryParser
	 *            QueryParser used to parse Lucene syntax based queries and tokenize
	 *            the text
	 * @return The Lucene Query object representation of this XML Query
	...
	private static Query doGetLuceneQuery(Document xmlDoc, QueryParser queryParser) throws Exception {
		...
		if (queryElement.size() > 1)
			throw new Exception("Error parsing Query: '<Query>' may contain only 1 child element but "
					+ queryElement.size() + " were found.");

		return getLuceneQuery((Element) queryElement.get(0), queryParser);
	}

	/**
	 * Gets the Lucene Query representation for the given XML Query starting at the
	 * Query element.
	 *
	 * @param queryElement
	 *            A dom4j representation of the Query element
	 * @param queryParser
	 *            QueryParser used to parse Lucene syntax based queries and tokenize
	 *            the text
	 * @return The Lucene Query object representation of this XML Query
	...
	public static Query getLuceneQuery(Element queryElement, QueryParser queryParser) throws Exception {
		// Check if requested to exclude from results, which is an error if not within a
		// boolean clause
		String excludeOrRequire = queryElement.attributeValue("excludeOrRequire");
		if (excludeOrRequire != null)
			throw new Exception(
					"Error parsing document: attribute excludeOrRequire may only be used when the query is enclosed in an encompassing <booleanQuery>. Error found at: "
							+ queryElement.getUniquePath());
		...
		if (queryElement.getName().equals("booleanQuery"))
			...
			throw new Exception("Error parsing document: invalid element name '<" + queryElement.getName() + ">' at "
					+ queryElement.getUniquePath());
	}
	...
	private static Query makeBooleanQuery(Element booleanQueryElement, QueryParser queryParser) throws Exception {
		...
		if (operator == null)
			throw new Exception(
					"Error parsing document: element <booleanQuery> must contain an attribite named 'type' that contains the value 'AND' or 'OR'. Error found at "
							+ booleanQueryElement.getUniquePath());
			...
			throw new Exception(
					"Error parsing document: element <booleanQuery> must contain an attribite named 'type' that contains the value 'AND' or 'OR' but value '"
							+ operator + "' was found. Error found at " + booleanQueryElement.getUniquePath());
		...
		for (Iterator i = booleanQueryElement.elementIterator(); i.hasNext();) {
			...
			if (excludeOrRequire != null) {
				...
				if (excludeOrRequire.equalsIgnoreCase("exclude")) {
					...
					throw new Exception(
							"Error parsing document: the value of attribute excludeOrRequire must be one of 'exclude', 'require' or 'neither' but '"
	...
	private static Query makeLuceneQuery(Element luceneQueryElement, QueryParser queryParser) throws Exception {
		...
		if (luceneQueryElement.getName().equals("textQuery")) {
			...
			if (type == null || type.trim().length() == 0)
				throw new Exception(
						"Error parsing document: element <textQuery> has empty or missing attribute 'type'. Error found at "
								+ luceneQueryElement.getUniquePath());
			...
			if (type.equals("matchAllTerms") || type.equals("matchAnyTerm")) {
				...
				synchronized (queryParser) {
					...
					do {
						...
						if (token != null) {
							if (isRequired)
								booleanQuery.add(new TermQuery(new Term(field, token.term())),
			...
			// Add support for RangeQuery and others here later...
				...
				throw new Exception("Error parsing document: element <textQuery> type attribute was found to be '"
						+ type
						+ "' but must be of one of 'matchAllTerms', 'matchAnyTerm', 'matchPhrase' or 'matchKeyword'. Error found at "
						+ luceneQueryElement.getUniquePath());
		}
		...
		// Handle luceneQuery:
			...
			if (queryString == null)
				throw new Exception(
						"Error parsing document: element <luceneQuery> is missing the attribute 'query'. Error found at "
								+ luceneQueryElement.getUniquePath());
			if (queryParser == null)
				throw new Exception("Error parsing document: <luceneQuery> element found at "
						+ luceneQueryElement.getUniquePath() + " but the Lucene queryParser is null");

			synchronized (queryParser) {
				try {
					query = queryParser.parse(queryString);
				} catch (ParseException pe) {
					throw new Exception("These was a Lucene query syntax error found at "
							+ luceneQueryElement.getUniquePath() + ". Error was: " + pe.getMessage());
				}
			}
		} else
			throw new Exception("Error parsing document: invalid element name '<" + luceneQueryElement.getName()
					+ ">'. Error found at " + luceneQueryElement.getUniquePath());
	...
	private static Query applyBoost(Element queryElement, Query query) throws Exception {
		...
		if (boost != null && boost.length() > 0) {
			...
			try {
				...
				throw new Exception("Error parsing document: boost value was not valid (" + nfe.getMessage()
						+ "). Value must be a number, for example 1.5, 2.4. Error found at " + path);
			}
			query.setBoost(b);
		}
		return query;
