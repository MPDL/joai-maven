 ...
 * annotation-level metadata record. The index writer that is responsible for
 * creating this type of Lucene {@link org.apache.lucene.document.Document} is a
 * {@link org.dlese.dpc.index.writer.DleseAnnoFileIndexingServiceWriter}.
 *
...
public class DleseAnnoDocReader extends XMLDocReader {
	...
	/**
	 * Gets the String 'DleseAnnoDocReader,' which is the key that describes this
	 ...
	 * for display in the UI. The reader type determines which getter methods are
	 * available.
	 *
	 ...
	 * Available since anno framwork v1.0.
	 *
	...
	/**
	 * Gets the status of the annotation, which is one of 'completed', 'in progress'
	 * or 'retired'. Note that status used to also contain the format string but
	 * there is now a separate field for format - see {@link #getFormat}. The status
	 * String here is the same regardless of whether the underlying record is in
	...
	/**
	 * Gets the format of the annotation, which is one of 'audio', 'graphical',
	 * 'text', or 'video'. The format String here is the same regardless of whether
	 * the underlying record is in framework version 1.0 or 0.1. Note that format
	 * used to be part of the status in v0.1 - see {@link #getStatus}.
	 *
	...
	/**
	 * Gets the type of annotation, for example 'Review', 'Comment', 'Educational
	 * standard', etc.
	 *
	...
	/**
	 * Gets the annotation date as a Date object, available only for annos in verion
	 * 0.1.
	 *
