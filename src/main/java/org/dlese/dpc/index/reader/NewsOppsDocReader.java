...
import org.dlese.dpc.index.writer.NewsOppsFileIndexingWriter;
 ...
 * news and opportunities metadata record. The index writer that is responsible
 * for creating this type of Lucene {@link org.apache.lucene.document.Document}
 * is a {@link org.dlese.dpc.index.writer.NewsOppsFileIndexingWriter}.
 *
...
public class NewsOppsDocReader extends XMLDocReader {
	...
	/**
	 * Gets the String 'NewsOppsDocReader,' which is the key that describes this
	...
	public String[] getAnnouncements() {
		return IndexingTools
				.extractSeparatePhrasesFromString(doc.get(NewsOppsFileIndexingWriter.FIELD_NS + "announcement"));
	}
	...
	public String[] getAudiences() {
		return IndexingTools
				.extractSeparatePhrasesFromString(doc.get(NewsOppsFileIndexingWriter.FIELD_NS + "audience"));
	}
	...
	public String[] getDiversities() {
		return IndexingTools
				.extractSeparatePhrasesFromString(doc.get(NewsOppsFileIndexingWriter.FIELD_NS + "diversity"));
	}

	/**
	 * Gets the locations, which are two character state or country codes, for
	 * example 'CO'.
	 *
	...
	/**
	 * Gets an array of city state strings for display. These are comprised of a
	 * city, if provided, followed by a comma followed by the state abbreviation.
	 * For example 'Boulder, CO'.
	 *
	 * @return An array of city-state strings, for example 'Boulder, CO'
	 */
	public String[] getCityStates() {
		return IndexingTools
				.extractSeparatePhrasesFromString(doc.get(NewsOppsFileIndexingWriter.FIELD_NS + "cityStates"));
	}
