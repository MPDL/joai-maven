...
/**
 * A {@link java.util.Map} for accessing the data stored in a Lucene
 * {@link org.apache.lucene.document.Document} as field/value pairs. The data is
 * loaded from the index only when requested via the Map accessor methods (e.g.
 * lazy loading), making access to very large Documents possible in an efficient
 * manner. Each stored field in the {@link org.apache.lucene.document.Document}
 * corresponds to a key in the Map. For example documentMap.get("title") gets
 * the text that was indexed and stored under the field name "title" for the
 * Document. Supports read operations only. A LazyDocumentMap is available from
 * search results by calling {@link org.dlese.dpc.index.ResultDoc#getDocMap()}.
 * <p>
 *
...
public class LazyDocumentMap implements Map {
	 ...
	 *
	 * @param doc
	 *            The Lucene Document that is exposed by this Map.
	 */
	public LazyDocumentMap(Document doc) {
		myDoc = doc;
		// System.out.println( "fieldNameList.size(): " + getFieldNames().size() +
		// "fieldValuesList.size(): " + getFieldValues().size() +"this.size(): " +
		// this.size() + " fields: " + this.getFieldNames());
	}
	...
	/**
	 * Gets the names of all fields in the document as Strings. This is similar to
	 * #keySet, but does not load all field/values into the Map and is therefore
	 * more efficient if only some fields are needed.
	 *
	...
	/**
	 * Gets the text content of the given Lucene field as a String or null if the
	 * given field is not available or was not stored in the index. Text includes
	 * all values stored in the index for the given field.
	 *
	 * @param fieldName
	 *            A Lucene {@link org.apache.lucene.document.Document} field name
	 ...
	 * {@link org.apache.lucene.document.Document}. This is similar to
	 * #getFieldNames but it also loads all field/values into the Map and is
	 * therfore more expensive than #getFieldNames, especially for large Documents.
	 *
	 ...
	 * {@link org.apache.lucene.document.Document}. Each {@link java.util.Map.Entry}
	 * Object in the Set contains the field name (key) and corresponding field
	 * value. The field value will be an empty String if the given field was not set
	 ...
	 * @param fieldName
	 *            A field name
	 * @return True if the field exists in the Lucene
	 *         {@link org.apache.lucene.document.Document}
	 */
	 ...
	 * @param value
	 *            A field value
	 * @return True if the field value is present in this
	 *         {@link org.apache.lucene.document.Document}
	 */
