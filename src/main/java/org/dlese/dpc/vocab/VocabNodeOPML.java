...
public class VocabNodeOPML implements VocabNode {
	...
	public VocabNodeOPML(OPMLTree.TreeNode node) {
		String metaFormat = "adn";
		if (node != null) {
			// a TreeNode.topTree.conceptKey holds
			// "metaFormat/metaVersion/audience/language/fieldName"
	...
	private void init(OPMLTree.TreeNode node, VocabList parent, String metaFormat) {
		...
		if ((getNodeAttribute(node, "display") != null) && (getNodeAttribute(node, "display").equals("false"))) {
			noDisplay = true;
		} else {
			noDisplay = false;
		}
		noDisplayOriginal = noDisplay;
		if ((getNodeAttribute(node, "wrap") != null) && (getNodeAttribute(node, "wrap").equals("true"))) {
			wrap = true;
		} else {
			wrap = false;
		}
		if ((getNodeAttribute(node, "collapsible") != null) && (getNodeAttribute(node, "collapsible").equals("true"))) {
			collapsible = true;
		} else {
			collapsible = false;
		}
	...
	public synchronized String getDescription(PageContext page) {
		if (((description == null) || (description.equals(""))) && src != null) {
			...
			if ((description.indexOf("<title>") > -1) || (description.indexOf("<TITLE>") > -1)) { // 404 not found case
				return null;
			}
	 ...
	 *
	 * @param noDisplayOriginal
	 *            The new noDisplayOriginal value
	 */
	 ...
	 *
	 * @param isLastInSubList
	 *            The new isLastInSubList value
	 */
