...
public final class MetadataVocabInputState implements Serializable {
	private StringBuffer vocabQueryString = new StringBuffer("");// part of request query string that holds vocab
																	// settings
	private StringBuffer vocabFeedbackString = new StringBuffer("");// store "Your selections" feedback for no
																	// Javascript case
	...
	public void setState(HttpServletRequest req) {
		...
		MetadataVocab vocab = (MetadataVocab) servletContext
				.getAttribute((String) servletContext.getInitParameter("metadataVocabInstanceAttributeName"));
		if (vocab != null) {
			...
			for (int i = 0; i < ids.size(); i++) {
				...
				if ((vals != null) && (vals.length > 0) && (vals[0].length() > 0)) {
					vocabFeedbackString
							.append(vocab.getUiFieldLabel(servletContext.getInitParameter("metadataVocabAudience"),
									servletContext.getInitParameter("metadataVocabLanguage"), (String) ids.get(i), true)
									+ ": ");
					for (int j = 0; j < vals.length; j++) {
						vocabQueryString.append("&" + ids.get(i) + "=" + vals[j]);
						vocabFeedbackString
								.append(vocab.getUiValueLabel(servletContext.getInitParameter("metadataVocabAudience"),
										servletContext.getInitParameter("metadataVocabLanguage"), (String) ids.get(i),
										vals[j], true) + ", ");
					}
	 ...
	 * @param system
	 *            The new state value
	 * @deprecated As of MetadataUI v1.0, replaced by
	 *             <a href="#setState(javax.servlet.http.HttpServletRequest)">
	 *             setState()</a>
	 */
	 ...
	 *
	 * @param field
	 *            Vocab field encoded ID
	 * @param value
	 *            Vocab value encoded ID
	 * @return boolean indicating whether the specified vocab node is selected
	 */
