 ...
 * <p>
 *
 * Assigns encoded IDs to new vocabularies in their fields files, and checks the
 * integrity of the existing IDs against a serialized "database" (HashMap) of
 * encoded ID values as mapped against field/vocab pairs.
 * </p>
 *
...
public class MetadataVocabAssignIDs {
	...
	private final static String SERIALIZED_IDS_HIGHEST = "state/highest_id_encodings.dat";// serialized encodings
																							// filename
	...
	/**
	 * Assign IDs and check the integrity of the existing ones against a "database"
	 * (serialized HashMaps) of IDs.
	 */
	public synchronized void assignIDs() {
		if (!(new File(SERIALIZED_IDS_HIGHEST).exists())) {
			...
			// The following was used to initialize the MUI 1.0 "database" of highest ID
			// values:
			...
			try {
				...
				while (loaderFileText.matches("[\\s\\S]*<fields>[^<]+</fields>[\\s\\S]*")) {
					String fieldsBuildFile = loaderFileText.replaceFirst("[\\s\\S]*<fields>([^<]+)</fields>[\\s\\S]*",
							"$1");
					loaderFileText = loaderFileText.replaceFirst("([\\s\\S]*)<fields>[^<]+</fields>([\\s\\S]*)",
							"$1$2");
				...
				for (int i = 0; i < fieldsList.size(); i++) {
					...
					while (loaderFileText.matches("[\\s\\S]*<file>[^<]+</file>[\\s\\S]*")) {
						...
						loaderFileText = loaderFileText.replaceFirst("([\\s\\S]*)<file>[^<]+</file>([\\s\\S]*)",
								"$1$2");
		...
		try {
			...
			if (xml.matches("[\\s\\S]*<field [^>]*id\\s*=\\s*\"[^\"]+\"[\\s\\S]*")) {
				...
				while (ind > -1) {
					...
					String tagVocab = tag.replaceFirst("<termAndDeftn[\\s\\S]+vocab\\s*=\\s*\"([^\"]+)\"[\\s\\S]*",
							"$1");
					if (!tag.matches("[\\s\\S]+id\\s*=\\s*\"\\S+\"[\\s\\S]*")) {
						String databaseID = (String) vocabIDs.get(fieldID + "/" + tagVocab);
						if (databaseID != null) {
							fileErrors += "\n\tVocab '" + tagVocab + "' has no ID--should be '" + databaseID
									+ "'!  Please add this attribute back to the XML and re-run this ID assignment script.";
							...
							// System.out.println( "Registering new ID: " + fieldID + "/" + tagVocab + " = "
							// + newID );
						...
						String databaseID = (String) vocabIDs.get(fieldID + "/" + tagVocab);
						if (databaseID == null) {
							fileErrors += "\n\tVocab '" + tagVocab + "' has ID '" + existingID
									+ "' when none exists in the database!  Please remove the 'id' attribute from the XML and re-run this ID assignment script.";
							errors.put(file, fileErrors);
						} else if (!databaseID.equals(existingID)) {
							fileErrors += "\n\tVocab '" + tagVocab + "' has ID '" + existingID
									+ "' that differs from database value '" + databaseID
									+ "'!  Please fix this in the XML and re-run this ID assignment script.";
							errors.put(file, fileErrors);
						}
						// The following was used for "database" initialization from MUI 1.0 set of
						// existing IDs:
						// System.out.println( "Init registering new ID: " + fieldID + "/" + tagVocab +
						// " = " + existingID );
