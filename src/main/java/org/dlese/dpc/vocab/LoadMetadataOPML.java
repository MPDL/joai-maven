...
public class LoadMetadataOPML implements org.xml.sax.ContentHandler {
	...
	/**
	 * Loads an instance of a MetadataVocab object and sets it as a servlet context
	 * attribute.
	 *
	 * @param servletContext
	 *            The java servlet context in which to store the loaded vocabularies
	 * @param vocabTextFile
	 *            Optional (can be NULL) file where vocabs get listed as text
	 *            strings (for use by log analysis/reporting)
	 * @param contextAttributeName
	 *            The MetadataVocab instance is stored in app scope as this
	 * @param configFile
	 *            Base loader file (lists groups/fields listings)
	 * @param debug
	 */
	public static synchronized void getMetadataVocabInstance(String configFile, String contextAttributeName,
			ServletContext servletContext, String vocabTextFile, boolean debug) {
	...
	public void load() {
		...
		try {
			...
			for (int i = 0; i < uiFiles.size(); i++) {
				String opmlFile = ((String) uiFiles.get(i)).replaceAll("\\\\", "/").replaceAll("//", "/")
						.replaceAll(" ", "%20").replaceFirst("http:/", "http://");
				try {
	 ...
	 */
	private static String getAbsolutePath(String fname, ServletContext servletContext) throws ServletException {
