...
public class MetadataVocabServlet extends HttpServlet {
	...
	private synchronized void instantiateVocab() {
		...
		metadataVocabInstanceAttributeName = (String) getServletContext()
		...
		System.out.println(getDateStamp() + " MetadataVocabServlet initialized, with MetadataVocab stored as '"
	...
	public synchronized void loadVocabs() {
		...
		LoadMetadataOPML.getMetadataVocabInstance(metadataGroupsLoaderFile, metadataVocabInstanceAttributeName,
				getServletContext(), vocabTextFile, debug);
		synchWithCollectionManager(getServletContext());
		if (newVocabsChecker == null) {
			newVocabsChecker = new NewVocabsChecker(this, metadataGroupsLoaderFile, getServletContext(),
					metadataVocabInstanceAttributeName);
	...
	/**
	 * Grab the MetadataVocab and RepositoryManager from the servlet context, and if
	 * both exist, invoke the synchronization of the CM "enabled" states with their
	 * corresponding vocabulary "display" states.
	 *
	 * @param context
	 */
