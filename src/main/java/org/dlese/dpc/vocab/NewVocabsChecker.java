...
public class NewVocabsChecker extends Thread {
	 ...
	 * @param loaderServlet
	 */
	public NewVocabsChecker(MetadataVocabServlet loaderServlet, String loaderFile, ServletContext servletContext,
			String instanceName) {
	...
	private synchronized String getLastUpdate() {
		if (loaderFile.startsWith("http://")) {
			try {
				...
				String path = loaderFile.substring(loaderFile.indexOf("/", 8), loaderFile.length()).replaceAll("//",
						"/");
