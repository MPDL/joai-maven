...
public class DLESEMetadataVocabTracker implements MetadataVocabTracker {
	 ...
	 *
	 * @param vocab
	 *            MetadataVocab instance using this tracker
	 * @param sqlDriver
	 *            driver class for SQL DB
	 * @param sqlURL
	 *            URL of SQL DB
	 * @param sqlUser
	 *            SQL user
	 ...
	 * @param vocabTextFile
	 */
	public DLESEMetadataVocabTracker(MetadataVocab vocab, String sqlDriver, String sqlURL, String sqlUser,
			String sqlPassword, String vocabTextFile) {
		this.vocab = vocab;
		...
		if ((sqlURL != null) && !sqlURL.equals("")) {
			...
			try {
				...
				if (gotToSelect) {
					// Exception must be from SELECT statement, so it is assumed the vocab table
					// doesn't exist yet:
	...
	/**
	 * Assign a unique system Id for a new vocabulary value. ALL Ids will start with
	 * 0, so that Lucene * queries will be possible.
	 *
	...
	private String getHighestId(String metadataFieldSystemId) {
		...
		try {
			...
			rs = stmt
					.executeQuery("SELECT * FROM vocab_fields WHERE metadataFieldId = '" + metadataFieldSystemId + "'");
			if (rs.next()) {
				stmt.close();
				return rs.getString("highestValueId");
			} else {
				try {
					stmt.executeQuery("INSERT INTO vocab_fields (metadataFieldId, highestValueId) " + "VALUES ('"
							+ metadataFieldSystemId + "', '00')");
					return "-1";
				} catch (Exception ex) {
					vocab.reportError("SQL error");
					ex.printStackTrace();
	...
	private void setHighestId(String metadataFieldSystemId, String newHighest) {
		try {
			stmt = con.createStatement();
			stmt.executeQuery("UPDATE vocab_fields SET highestValueId = '" + newHighest + "' WHERE metadataFieldId = '"
					+ metadataFieldSystemId + "'");
		} catch (Exception e) {
			vocab.reportError("SQL error");
			e.printStackTrace();
		} finally {
	...
	private void insertNewIdIntoValuesTable(String metadataField, String metadataValue, String systemId) {
		try {
			stmt = con.createStatement();
			stmt.executeQuery(
					"INSERT INTO vocab_values (metadataFieldId, metadataValue, systemId, createdDate, retiredDate) "
							+ "VALUES ('" + metadataField + "', '" + stringUtil.escapeQuotesSQL(metadataValue) + "', '"
							+ systemId + "', NOW(), '')");
			stmt.close();
		} catch (Exception e) {
			vocab.reportError("SQL error");
	...
	public void registerUiLabels(HashMap uiSystems, HashMap uiLabelOfSystemIds) {
		if (usingDatabase) {
			try {
				stmt = con.createStatement();
				rs = stmt
						.executeQuery("SELECT id, metadataFieldId, metadataValue, systemId, uiLabels FROM vocab_values "
								+ "WHERE retiredDate = '';");
				while (rs.next()) {
					...
					if (dbLabels == null) {
						...
						updateStmt.executeQuery("UPDATE vocab_values SET uiLabels='"
								+ stringUtil.escapeQuotesSQL(xmlLabels) + "' WHERE id='" + sqlId + "'");
						updateStmt.close();
						...
						updateStmt.executeQuery(
								"INSERT INTO vocab_values (metadataFieldId, metadataValue, systemId, uiLabels, createdDate, retiredDate) "
										+ "VALUES ('" + fieldId + "', '" + stringUtil.escapeQuotesSQL(metaValue)
										+ "', '" + valueId + "', '" + stringUtil.escapeQuotesSQL(xmlLabels)
										+ "', NOW(), '')");
						updateStmt.close();
					}
				}
	 ...
	 * @return
	 */
	private String concatUiSystemLabels(String fieldId, String valueId, HashMap uiSystems, HashMap uiLabelOfSystemIds) {
	 ...
	 * retired values!) that simply lists each field/value system ID, along with the
	 * metadataValue. This text file is used by the log analysis scripts to generate
	 * spreadsheet headers that are human-readable (i.e., instead of "gr=01" you get
	 * "gradeLevel=DLESE:Graduate or professional"). This was done to avoid having
	 * to connect the log analysis scripts to the database, and so that existing
	 * analysis code could still be used (the text format written is the same as
	 * used by V1 of the DDS).
	 */
