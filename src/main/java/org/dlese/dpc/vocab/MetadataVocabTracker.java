 ...
 *
 * <ol>
 * <li>Error checking, encoded Id assignment, and UI label change tracking are
 * OPTIONAL, as reflected in the isUsingDatabase() method</li>
 * <li>Once an encoded Id has been assigned, it may never be re-used, but can
 * only be retired when/if its vocab value is removed</li>
 * <li>Only vocab VALUE encoded Ids are automatically assigned. FIELD Ids must
 * be assigned manually, so a tracker will assume that they already exist.</li>
 * </ol>
 *
 *
...
public interface MetadataVocabTracker {
	 ...
	 *
	 * @param uiSystems
	 *            map whose keys indicate all the current ui_label "system"
	 *            attributes
	 * @param uiLabelOfSystemIds
	 *            map encoded Ids to UI labels
	 */
