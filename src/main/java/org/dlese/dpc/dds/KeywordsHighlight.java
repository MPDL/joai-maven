...
public final class KeywordsHighlight {
	 ...
	 * list, and then strip them out, returning the string for further processing by
	 * KeywordsHighlight(). NOTE: Lucene, and therefore this parser, only considers
	 * " as a quoted phrase delimeter, not '!
	 *
	 * @param words
	 * @return
	 */
	 ...
	 * @param text
	 * @param addWbr
	 *            if true, replace '/' with '/<wbr>' (allows URLs to wrap)
	 * @return DESCRIPTION
	 */
	public String highlight(String text, boolean addWbr) {
		...
		for (int i = 0; i < numKeywords; i++) {
			if ((keywords[i].length() > 0) && (!keywords[i].equals("or")) && (!keywords[i].equals("and"))) {
				text = replace(text, keywords[i]);
			}
		}
		return text.replaceAll("<HPREFIX>", HIGHLIGHT_PREFIX).replaceAll("</HPREFIX>", HIGHLIGHT_POSTFIX);
	}
		...
		while (ind2 > -1) {
			...
			if (!Character.isLetterOrDigit(ch1) && !Character.isLetterOrDigit(ch2)) {
				...
				ret.append(in.substring(ind1, ind2))
						.append("<HPREFIX>" + in.substring(ind2, ind2 + findLength) + "</HPREFIX>");
			} else {
	...
	/**
	 * Gets the firstHighlightIndex attribute of the KeywordsHighlight object--when
	 * keyword highlighting is performed, firstHighlightIndex is loaded with the
	 * index of the first occurrence of a keyword within the given string.
	 *
