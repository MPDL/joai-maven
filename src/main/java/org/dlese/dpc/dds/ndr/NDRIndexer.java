...
public class NDRIndexer implements ItemIndexer {
	...
	/**
	 * Initialize and configure. This gets called once when the class is first
	 * created and is called by updateCollections() when re-loading the list of
	 * collections.
	 *
	 * @exception Exception
	 *                If error
	...
	private void configureAndInitialize() throws Exception {
		synchronized (this) {
			...
			try {
				...
				String msg = "Unable to read the NDR configuration file '" + configFile + "'. Message: "
						+ t.getMessage();
				prtlnErr(msg);
			...
			// For read-only operations, no agent handle or private key file is necessary
			// (pass in null):
	 ...
	 *
	 * @param collectionKey
	 *            The collection key/id, or null for all
	 * @exception Exception
	 *                If error
	 */
	public void indexCollections(String collectionKey) throws Exception {
		// If collectionKey is null, index all collections, if not, index only that
		// collection...
		...
		// Initialize required NDR attributes and Sets TransformerFactory to a XSL 1.0
		// version:
		...
		try {
			...
			if (formatType == null || formatType.trim().length() == 0
					|| !(formatType.equalsIgnoreCase("canonical_nsdl_dc") || formatType.equalsIgnoreCase("native")))
			...
			if (!abortIndexing) {
				for (int i = 0; i < ndrCollectionInfos.size(); i++) {
					...
					try {
						...
						String msg = "Error retrieving NDR MetadataProvider. Skipping collection '"
								+ collectionInfo.getHandle() + "'. Message: " + e.getMessage();
						prtlnErr(msg);
	...
	public void updateCollections() throws Exception {
		...
		// prtln("getNcsCollections() returned: " +
		// Arrays.toString(ncsCollections.toArray()));
		...
		try {
			...
			if (ndrErrorHasOccured == false) {
				...
				for (int i = 0; i < ndrCollectionInfos.size(); i++) {
					...
					if (collectionKey == null) {
						prtlnErr(
								"MetadataProviderReader returned null collectionKey/setSpec. Skipping collection for MDP "
										+ mdpReader.getMetadataProviderFor());
					} else {
		...
		// Do not delete/remove collections if there has been an error collecting the
		// list of collections.
		if (ndrErrorHasOccured) {
			prtln("updateCollections() ndrErrorHasOccured... ");
			collectionIndexer.printStatusMessage(
					"One or more errors occured while updating the collections. No collections will be deleted...");
			return;
		}
	...
	private void putCollection(MetadataProviderReader mdpReader, String collectionKey, String format,
			String additionalMetadata) throws Exception {
		...
		String collectionDescription = "This collection resides in the NSDL Data Repository (NDR). It's NDR setSpec is "
				+ mdpReader.getSetSpec();
		...
		try {
			collectionIndexer.putCollection(collectionKey, collectionFormat, collectionName, collectionDescription,
					additionalMetadata);
			// collectionIndexer.putCollection(collectionKey, collectionFormat,
			// collectionName, collectionDescription, additionalMetadata);
			...
			if (pce.getErrorCode().equals(PutCollectionException.ERROR_CODE_COLLECTION_EXISTS_IN_ANOTHER_FORMAT)) {
				collectionIndexer.printStatusMessage("Collection '" + collectionName
						+ "' is in the repository but in a different format. Deleting...");
				collectionIndexer.deleteCollection(collectionKey);
				collectionIndexer.printStatusMessage("Adding collection '" + collectionName
						+ " with the new format (key:" + collectionKey + " new format:" + collectionFormat + ")");
				collectionIndexer.putCollection(collectionKey, collectionFormat, collectionName, collectionDescription,
						additionalMetadata);
			}
		}
	...
	/*
	 * private List getNcsCollections() throws Exception { return
	 * doGetCollections("ncs"); } private List getNdrCollectionHandles() throws
	 * Exception { return doGetCollections("ndr"); } private List
	 * doGetCollections(String type) throws Exception { // Type must be one of "ncs"
	 * or "ndr" ArrayList collectionInfos = new ArrayList(); if (configXmlDoc ==
	 * null) throw new
	 * Exception("Unable to configure NDR collections. No config file found at " +
	 * configFile); List nodes =
	 * configXmlDoc.selectNodes("/NDR_Collections/collection[@type='" + type +
	 * "']"); if (nodes == null) return collectionInfos; // Get the handle or key:
	 * for (int i = 0; i < nodes.size(); i++) { String handle = (String) ((Node)
	 * nodes.get(i)).valueOf("handle"); if (handle == null || handle.length() == 0)
	 * handle = (String) ((Node) nodes.get(i)).valueOf("key"); if (handle != null &&
	 * handle.length() > 0) collectionInfos.add(handle); } return collectionInfos; }
	 */

	/*
	 * private MetadataProviderReader getNcsMetadataProvider(String collectionKey)
	 * throws Exception { MetadataProviderReader mdpReader = null; // Grab the
	 * collection handle from the NDR: mdpReader =
	 * getMetadataProvider(collectionKey); prtln("MetadataProvider for " +
	 * collectionKey); prtln("\t" + "collectionId: " + mdpReader.getCollectionId());
	 * prtln("\t" + "collectionName: " + mdpReader.getCollectionName()); prtln("\t"
	 * + "nativeFormat: " + mdpReader.getNativeFormat()); return mdpReader; }
	 */
	...
	private void indexCollection(String collectionKey, MetadataProviderReader mdpReader, String format,
			String additionalMetadata) throws Exception {
		if (abortIndexing)
			return;

		/*
		 * if (printOutput) { prtln("-----------------\nCollection Record");
	 ...
	 *            The collection key
	 * @param format
	 *            The format to index for this collection, "canonical_nsdl_dc" for
	 *            canonical
	 */
	public void indexItemRecords(MetadataProviderReader mdp, String collectionKey, String format) {
		...
		CollectionIndexingSession collectionIndexingSession = collectionIndexer
				.getNewCollectionIndexingSession(collectionKey);
		...
		for (Iterator i = mdHandles.iterator(); i.hasNext() && !abortIndexing;) {
			...
			try {
				...
				prtln("ID: " + mdReader.getRecordId() + " status: " + mdReader.getStatus() + " isValid: "
						+ mdReader.getIsValid());
				...
				// Index NCS native item: Make sure we're grabbing final, valid records only for
				// NCS collections:
				if (!mdReader.isNcsMetadata() || (mdReader.getIsFinal() && mdReader.getIsValid())) {
					prtln("Put record for collection:" + collectionKey + " format:" + xmlFormat + " no: " + (++count)
							+ " id:" + use_id);
			...
			if (numProcessed > 0 && numProcessed % messagesFrequency == 0) {
				String msg = "Processed " + numProcessed + " of " + mdHandles.size() + " records for collection '"
						+ mdp.getSetName() + " thus far (" + numSuccess + " successful, " + numErrors + " errors)";
				if (errorThrown != null)
					msg += ". One or more records could not be indexed because of errors. Most recent error message: "
							+ errorThrown;
				printStatusMessage(msg);
			}
		}

		String msg = "Finished indexing collection '" + mdp.getSetName() + "'. Processed " + numProcessed + " records ("
				+ numSuccess + " successful, " + numErrors + " errors)";
		if (errorThrown != null)
			msg += ". One or more records could not be indexed because of errors. Most recent error message: "
					+ errorThrown;
		printStatusMessage(msg);
	...
	private List getCollectionsFromRemoteNCS() throws Exception {
		...
		List nodes = configXmlDoc
				.selectNodes("/NDR_Collections/ncsManagedCollections/ncsCollectionStatusFilter/ncsCollectionStatus");
		...
		String searchQueryConstraint = configXmlDoc
				.valueOf("/NDR_Collections/ncsManagedCollections/searchQueryConstraint");
		try {
		...
		// Loop until all collection records have been fetched from the NCS Web service,
		// and build a DOM:
		String serviceRequestUrl = null;
		for (int i = 0; i < 200; i++) {
			serviceRequestUrl = ncsSearchApiBaseUrl + "?verb=Search&xmlFormat=ncs_collect" + nscStatusParams + "&n=" + n
					+ "&s=" + s + searchQueryConstraint;

			prtln("Fetching collections from NCS. Request: " + serviceRequestUrl);
			Document fetched = Dom4jUtils
					.getXmlDocument(Dom4jUtils.localizeXml(TimedURLConnection.importURL(serviceRequestUrl, timoutMs)));
			...
			if (ddsErrorCode != null && ddsErrorCode.equals("noRecordsMatch")) {
				prtln("No matching collections found in the NCS (noRecordsMatch). Request made was: "
						+ serviceRequestUrl);
				break;
			}
			...
			if (numResults == null || numResults.trim().length() == 0 || numResults.equals("0"))
				throw new Exception(
						"No collections records available from NCS (numResults element was missing or empty... must be an error). Request made was: "
								+ serviceRequestUrl);

			else {
				// List metadataProviderHandles =
				// fetched.selectNodes("/DDSWebService/Search/results/record/head/additionalMetadata/ndr_info/metadataProviderHandle");
				...
				for (int j = 0; j < ncsCollectRecords.size() && collectionInfos.size() < MAX_NUM_COLLECTIONS; j++) {
					String handle = ((Node) ncsCollectRecords.get(j))
							.valueOf("head/additionalMetadata/ndr_info/metadataProviderHandle");
			...
			/*
			 * NDR explorer: http://ncs.nsdl.org/mgr/ndr/ndr.do?command=explore&handle=2200/
			 * 20061002130324510T Some example handles for dev: Math forum MP:
			 * 2200/20061002124657491T Math Forum aggregator: 2200/20061002124656295T Math
			 * Forum metadata: 2200/20091103184302321T 2200/20091103184717170T MSP2 MP:
			 * 2200/20061002130512109T MSP2 metadata: 2200/20091209135045391T
		...
		// prtln("getCollectionsFromRemoteNCS() returning: " +
		// Arrays.toString(collectionInfos.toArray()));
		return collectionInfos;
	}
