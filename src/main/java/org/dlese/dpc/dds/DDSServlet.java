...
/**
 * Provided as an administrative and intialization servlet for the Digital
 * Discovery System (DDS).
 *
...
public class DDSServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			// Set up the data source(s). Possible values are: fileSystem (default), or list
			// of ItemIndexers:
			...
			// By default, only use records located in the ../dlese_collect/collect
			...
			String resourceResultLinkRedirectURL = (String) servletContext
					.getInitParameter("resourceResultLinkRedirectURL");
			...
			// Indexing days of the week as a comma separated list of integers, for example
			// 1,3,5 where 1=Sunday, 2=Monday, 7=Saturday etc.
			...
			// Remove any Lucene locks that may have persisted from a previous dirty
			// shut-down:
			...
			String annotationPathwaysSchemaUrl = (String) servletContext
					.getInitParameter("annotationPathwaysSchemaUrl");
			...
			// ORIGINAL vocabs based on terms/groups files - stores in conext under
			// "MetadataVocab"
			/*
			 * new LoadMetadataVocabs( vocabConfigDir, "DDS_Vocabulary.xml",
			 * "org.apache.xerces.parsers.SAXParser", (ServletContext)getServletContext(),
			 * sqlDriver, sqlURL, sqlUser, sqlPassword, vocabTextFile );
			 */
			// The MetadataVocabServlet puts the MetadataVocab object in the context under
			// "MetadataVocab"
			...
			try {
				repositoryConfigDir = new File(
						getAbsolutePath((String) servletContext.getInitParameter("repositoryConfigDir")));
			...
			try {
				itemIndexerConfigDir = new File(
						getAbsolutePath((String) servletContext.getInitParameter("itemIndexerConfigDir")));
			} catch (Throwable e) {
				prtlnErr("Error getting itemIndexerConfigDir: " + e);
			}

			// If an IndexingManager is being used, don't run the RepositoryManager file
			// indexing timer.
			...
			rm = new RepositoryManager(repositoryConfigDir, itemIndexerConfigDir, repositoryDataDir.getAbsolutePath(),
					indexLocation, indexingStartTimeFileSystem, indexingDaysOfWeek,
					new RecordDataService(dbURL, metadataVocab, collBaseDir, annotationPathwaysSchemaUrl), true, true);

			// Set up the default metadata audience and language used by the indexer and
			// search resultDocs for display:
			...
			// Disable regular OAI-PMH responses to ListRecords and ListIdentifiers requests
			// (accept ODL requests only)
			...
			if (idMapperExclusionFile != null && idMapperExclusionFile.length() > 0
					&& !idMapperExclusionFile.equals("none")) {
			...
			SimpleLuceneIndex dupItemsIndex = new SimpleLuceneIndex(index.getIndexLocation() + "/dup_items_index",
					index.getAnalyzer());
			...
			if (rm.getXMLConversionService() == null) {
				File xslFilesDirecoryPath = new File(
						GeneralServletTools.getAbsolutePath("WEB-INF/xsl_files/", getServletContext()));
				File xmlCachDir = new File(GeneralServletTools
						.getAbsolutePath(repositoryData + File.separator + "converted_xml_cache", getServletContext()));

				try {
					XMLConversionService xmlConversionService = XMLConversionService
							.xmlConversionServiceFactoryForServlets(getServletContext(), xslFilesDirecoryPath,
									xmlCachDir, true);
	 ...
	 *
	 * @param rm
	 *            The new fileIndexingPlugins value
	 */
			...
			while (enumeration.hasMoreElements()) {
				...
				if (param.toLowerCase().startsWith("fileindexingplugin")) {
					...
					try {
						...
						// System.out.println("Adding plugin: " + plugin.getClass().getName() + " for
						// format " + format);
						rm.setFileIndexingPlugin(format, plugin);
					} catch (Throwable e) {
						prtlnErr("Error: setFileIndexingPlugins(): could not instantiate class '" + vals[0].trim()
								+ "'. " + e);
						continue;
					}
				}
	...
	/**
	 * The standard required servlet method, just parses the request header for
	 * known parameters. The <code>doPost</code> method just calls this one. See
	 * {@link HttpServlet} for details.
	 *
	 * @param request
	 * @param response
	 * @exception ServletException
	...
	/**
	 * Sets the "noDisplay" property of collection vocab nodes according the results
	 * of the repository manager's getEnabledSetsHashMap()
	 *
	 * @param vocab
	 *            The new collectionsVocabDisplay value
	 * @param rm
	 *            The new collectionsVocabDisplay value
	 */
	 ...
	 *            Description of the Parameter
	 */
	private static void doSetCollectionsVocabDisplay(MetadataVocab vocab, RepositoryManager rm,
		...
		for (int i = 0; i < nodes.size(); i++) {
			...
			// If the UI groups file said not to display (dwelanno for example), then don't
			// override that:
	...
	/**
	 * Gets the absolute path to a given file or directory. Assumes the path passed
	 * in is eithr already absolute (has leading slash) or is relative to the
	 * context root (no leading slash). If the string passed in does not begin with
	 * a slash ("/"), then the string is converted. For example, an init parameter
	 * to a config file might be passed in as "WEB-INF/conf/serverParms.conf" and
	 * this method will return the corresponding absolute path
	 * "/export/devel/tomcat/webapps/myApp/WEB-INF/conf/serverParms.conf."
	 * <p>
	 *
	 * If the string that is passed in already begings with "/", nothing is done.
	 * <p>
	 *
	 * Note: the super.init() method must be called prior to using this method, else
	 ...
	 * @exception ServletException
	 *                An exception related to this servlet
	 */
	private String getAbsolutePath(String fname) throws ServletException {
