 ...
 * with the DDSQueryAction Struts Action class.
 *
...
public final class DDSQueryForm extends DDSViewResourceForm implements Serializable {
	 ...
	 *
	 * @param add
	 *            The feature to be added to the DateString attribute
	 */
	 ...
	 *
	 * @param add
	 *            The feature to be added to the DateStringForUI attribute
	 */
	 ...
	 *
	 * @param str
	 *            The new resourceResultLinkRedirectURL value
	 */
	...
	/**
	 * Gets the search results returned by the
	 * {@link org.dlese.dpc.index.SimpleLuceneIndex} as a List (compatible with
	 * c:forEach iteration)
	 *
	...
	public String getHasViewableReview(int index) {
		...
		if (annoStatus != null) {
			for (int i = 0; i < annoStatus.length; i++) {
				if (annoStatus[i].equals("Review in progress") || annoStatus[i].equals("Review stopped")) {
					ret = "false";
				}
			}
		}
	 ...
	 *
	 * @param numResources
	 *            The totalNumResources that are currently discoverable.
	 */
			...
			if (pagingLinks.length() == 0) {
				...
				if (first > 0) {
					...
					outStr[outCount++] = " <a href='" + PAGING_URL + startStr
							+ "' class='pagingLink' title='Previous page of resources'>&lt;&lt;</a>&nbsp;";
				}
				...
				for (int i = 0; i < RESULTS_FROM_CURRENT; i++) {
					if (fcStart >= 0) {
						...
						outStr[outCount++] = "<a href='" + PAGING_URL + startStr + "' class='pagingLink'>"
								+ Integer.toString(++fcCount) + "</a>";
					}
					fcStart += dif;
				}
