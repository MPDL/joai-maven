...
import org.dlese.dpc.dds.action.form.DDSEditRecordForm;
import org.dlese.dpc.index.ResultDocList;
import org.dlese.dpc.index.SimpleLuceneIndex;
import org.dlese.dpc.index.reader.XMLDocReader;
import org.dlese.dpc.index.writer.FileIndexingPlugin;
import org.dlese.dpc.index.writer.SimpleFileIndexingPlugin;
...
public final class DDSEditRecordAction extends Action {
	...
	/**
	 * Processes the DDS request by forwarding to the appropriate corresponding JSP
	 * page for rendering.
	 *
	 * @param mapping
	...
	private ActionForward doGetRecord(DDSEditRecordForm def, RepositoryManager rm, ActionMapping mapping,
	...
	private ActionForward doPutRecord(DDSEditRecordForm def, RepositoryManager rm, ActionMapping mapping,
			HttpServletRequest request, HttpServletResponse response) {
		ActionErrors errors = new ActionErrors();
	...
	private ActionForward doDeleteRecord(DDSEditRecordForm def, RepositoryManager rm, ActionMapping mapping,
			HttpServletRequest request, HttpServletResponse response) {
				...
				try {
					if (rm.deleteRecord(deleteRecord))
						...
						errors.add("error", new ActionError("generic.message",
								"Record ID " + deleteRecord + " not found. No delete was performed."));
				} catch (Throwable e) {
