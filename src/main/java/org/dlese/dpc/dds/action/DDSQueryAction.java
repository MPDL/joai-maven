...
public final class DDSQueryAction extends Action {
	 ...
	 * {@link #getBooleanQuery(String[] terms, int operator, boolean useStemming)}
	 * method.
	 */
	...
	public final static String[] SEARCH_TYPE_NAMES = {"search", "histogram", "whatsnew", "ddsws-user-search",
			"ddsws-search", "odl-search", "rss-general", "rss-whatsnew", "jshtml-search"};

	/**
	 * A list of all fields that are indexed in encoded form via
	 * SimpleLuceneIndex.encodeToTerm()
	 */
	public final static String[] ENCODED_FIELDS = {"id", "urlenc"};

	/**
	 * A list of aliases that map user-typed field names to the field name that
	 * exists in the index.
	 */
	public final static String[][] FIELD_ALIASES = {{"url", "urlenc"}};
	 ...
	 * response by forwarding to a JSP that will create it. A
	 * {@link org.dlese.dpc.index.SimpleLuceneIndex} must be available to this class
	 * via a ServletContext attribute under the key "index." Returns an
	 * {@link org.apache.struts.action.ActionForward} instance that maps to the
	 * Struts forwarding name "simple.query," which must be configured in
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			/*
			 * Design note: Only one instance of this class gets created for the app and
			 * shared by all threads. To be thread-safe, use only local variables, not
			 * instance variables (the JVM will handle these properly using the stack). Pass
			...
			RepositoryManager rm = (RepositoryManager) servlet.getServletContext().getAttribute("repositoryManager");
	 ...
	 *                if a servlet exception occurs
	 */
	public ActionForward handleMetadataSearchRequest(ActionMapping mapping, DDSQueryForm queryForm,
			HttpServletRequest request, HttpServletResponse response, RepositoryManager rm)
		...
		if (request.getQueryString().equals("q=") || request.getQueryString().equals("q=&s=0")) {
		...
		queryForm.setResourceResultLinkRedirectURL(
				(String) servlet.getServletContext().getAttribute("resourceResultLinkRedirectURL"));

		String metadataVocabInstanceAttributeName = (String) servlet.getServletContext()
				.getInitParameter("metadataVocabInstanceAttributeName");
		MetadataVocab vocab = (MetadataVocab) servlet.getServletContext()
				.getAttribute(metadataVocabInstanceAttributeName);
		...
		// Set vocab input state using info. in request (and vocab system
		// dds.descr.en-us)
		...
		try {
			...
			// Set up bean values:
			if ((request.getParameter("hist") != null) && (request.getParameter("hist").equals("true"))) {
			...
			if (request.getParameter("q") != null) {
				...
				DDSStandardSearchResult ddsSearchResult = ddsStandardQuery(request, null, rm, vocab,
						servlet.getServletContext(), searchType);
			...
			if (queryForm != null) {
				errors.add("error", new ActionError("generic.error", "There was a server problem: " + t));
	...
	/**
	 * Performs textual and field-based searches limited to discoverable items only
	 * and using pre-defined search logic. Used by DDS search and DDSWebServices
	 * search.
	 *
	 * @param request
	 *            The HTTP request
	 ...
	 *            The ServletContext
	 * @param searchType
	 *            The searchType that gets logged
	 * @param additionalQueryOrConstraint
	...
	public static DDSStandardSearchResult ddsStandardQuery(HttpServletRequest request,
			String additionalQueryOrConstraint, RepositoryManager rm, MetadataVocab vocab, ServletContext context,
			int searchType) {
		...
		if (basicQuery == null && (geospatialQuery != null
				|| (selectedCollections != null && selectedCollections[0].length() > 0)
				|| (gradeRanges != null && gradeRanges[0].length() > 0) || (wnto != null && wnto.length() > 0)
				|| (wnfrom != null && wnfrom.length() > 0) || (resourceTypes != null && resourceTypes[0].length() > 0)
		...
		constraintsQuery += (constraintsQuery.length() > 0 ? " AND " : "") + reviewedBoostingQuery + " AND "
				+ rm.getMultiDocBoostingQuery() + " AND " + discoverableItemsQuery;
		...
		if (index != null) {
			...
			if (sortBy != null) {
				...
				if (sortBy.indexOf("date") >= 0) {
					...
					// Collections.sort(resultDocs, new LuceneFieldComparator(sortBy,
		...
		// Determine the start position for displaying results (used here for logging
		// only)
		int startPosition = -1;
		if (s != null) {
			try {
		...
		logQuery(q, context, request, 200, -1, numSearchResults, rm.getNumDiscoverableADNResources(), startPosition,
				searchType);
	 ...
	 *
	 * @param query
	 *            The feature to be added to the ConstraintToQuery attribute
	...
	/**
	 * Generates a geospatial bounding box Lucene Query from the necessary
	 * parameteres in an http request, which are geoPredicate, geoBBNorth,
	 * geoBBWest, geoBBEast, geoBBSouth. If the query crosses the 180/-180 longitude
	 * it is split into two query regions, one on each side, joined by boolean
	 * clause.
	 *
	 * @param request
	 *            An http request that may contain geospatial parameters
	 * @return A geospatial Query, or null if no no geospatial parameters were
	 *         supplied
	 * @exception Exception
	 *                If there is an error with one or more of the geospatial
	 *                parameters
	 */
	public static Query getGeospatialQuery(HttpServletRequest request) throws Exception {
		...
		try {
			// If the query crosses the 180/-180 longitude, split the query into two regions
			// on either side:
			...
			// Return the Query, throws an exception if parameteres are invalid. Encodes BB
			// fields as "northCoord", "southCoord", "eastCoord", "westCoord":
	...
	/**
	 * Gets a query that limits a search to vocab-managed fields such as gradeRange
	 * (gr), resourceType (re), subject (su), contentStandard (cs) and collection
	 * (ky). The query returned begins with AND. If no sucy parameters exist in the
	 * request, returns an empty String.
	 *
	 * @param request
	 *            The HTTP request
	 * @param vocab
	 *            The metadata vocab used
	 * @param docReaderAttributes
	 *            The doc reader attributes
	 * @return Gets a query that limits a search to vocab-managed fields, or an
	 *         empty String if no such field parameters exist in the request.
	 */
	public final static String getVocabParamsQueryString(HttpServletRequest request, MetadataVocab vocab,
		...
		if (selectedCollections != null && selectedCollections[0].length() > 0) {
			String collectionsQuery = "(" + getCollectionQueryTerm(selectedCollections[0]);
			for (int i = 1; i < selectedCollections.length; i++) {
				collectionsQuery += " OR " + getCollectionQueryTerm(selectedCollections[i]);
			}
			collectionsQuery += ")";
	 ...
	 *                If error parsing
	 */
	public final static Filter getWhatsNewDateFilter(String wnfrom, String wnto) throws ParseException {
		// Handle What's New date searching
		DateRangeFilter whatsNewFilter = null;

		if (wnfrom != null && wnto == null && (wnfrom.equals("recent") || wnfrom.equals("rss"))) {
			Calendar cal = Calendar.getInstance(TimeZone.getDefault());
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
			...
			whatsNewFilter = new DateRangeFilter("wndate", MetadataUtils.parseUnionDateType(wnfrom),
					MetadataUtils.parseUnionDateType(wnto));
		} else if (wnfrom == null && wnto != null) {
	...
	/**
	 * Gets the Lucene query string necessary to pull out records for a given
	 * collection. This method maps the query to the set of records that belong to
	 * the collection. This value may be ANDed with others to produce a compound
	 * boolean query.
	 *
	 * @param ky
	 *            The vocab key, for example 06
	...
	public static String getCollectionQueryTerm(String ky) {
		...
		/*
		 * The default query searches by collection key OR by the item's annotation
		 * collection key (mapped to those items that have one or more annos with
		 * status=completed). Because a search on the collection key and the anno
		 * collection key are guaranteed to return non-overlapping sets of ADN
		 * resources, it works to search on both in all cases. For the web servcice,
		 * this means a search by collection will return both formats, if available. For
		 * example a search on the CRS collection will return both dlese_anno and adn
		 * XML records.
		 */
		else {
	 ...
	 *
	 * @param q
	 *            A keyword query typed by a user.
	 * @param rm
	 *            The RepositoryManager
	 * @param context
	 *            The servlet context
	 * @return The query formatted for boosting title and for searching URLs and
	 *         IDs.
	...
	private final static Query formatQuery(String q, RepositoryManager rm, ServletContext context)
		...
		// --- First: format common queries that appear at the beginning of the user's
		// query ---

		// Include the field, then a term that does not contain ( or ), then a space or
		// end of input, then anything else
		...
		try {
			while (m.find()) {
				...
				// System.out.println("field: '" + field + "' term: '" + term + "' space: '" +
				// space + "' rest: '" + rest + "'");
				if (field != null && term != null) {
					...
					if (field.equals("url:")) {
						...
						if (url != null) {
							field = "urlenc:";
							term = SimpleLuceneIndex.encodeToTerm("http*" + url.getHost().replaceFirst("www\\.", ""),
									false) + "*";
						}
					...
					// System.out.println("field: '" + field + "' escaped-term: '" + term + "' rest:
					// '" + rest + "'");
	...
	/**
	 * Formats the fields/terms in the query string by replacing the aliased field
	 * names and encoding terms that are in fields that have been indexed encoded.
	 *
	 * @param q
	 *            The raw query by user
	...
	/**
	 * Encodes a query string such that each term that is part of a field that has
	 * been encoded in the index using
	 * {@link org.dlese.dpc.index.SimpleLuceneIndex}.encodeToTerm() is replaced with
	 * the encoded form for searching. May be used in conjuction with
	 * {@link #replaceAliasedFieldNames(String)}.
	 * <p>
	 *
	 * Examples:<br>
	 * <blockquote> <code>
	...
	public final static StringBuffer formatEncodedFields(String q) {
		...
		Pattern p = Pattern
				.compile("(" + encFieldsRegEx + ")((\\(|\")([\\S &&[^\"\\^\\)\\(]]+)(\"|\\))|([\\S&&[^\\^]]+)(\\B))");
		Matcher m = p.matcher(q);
		StringBuffer sb = new StringBuffer();
		while (m.find()) {
			// Group 4 will contain the terms inside quotes or parens, e.g.
			// url:(http://my.com http://your.com)
	...
	/**
	 * Substitutues the searchable field names for the common field names that
	 * user's may use in their query string. May be used in conjuction with
	 * {@link #formatEncodedFields(String)}.
	 * <p>
	 *
	 * Examples:<br>
	 * <blockquote> <code>
	 ...
	 * @param operator
	 *            The operator to use (AND|OR).
	 * @param useStemming
	 *            True to stem the tersm, false to leave unchanged.
	...
	/**
	 * Examines the term for the presence of the string url: http: or site: and then
	 * encodes it appropriately for searching in the urlenc Lucene field, or returns
	 * term if no encoding is needed.
	 *
	 * @param term
	 *            The term to encode
	...
	private final static String getFieldTerm(String term) {
		if (term.startsWith("http://")) {
			...
			term = "(urlenc:" + SimpleLuceneIndex.encodeToTerm("http://" + url.getHost(), false) + "* OR urlenc:"
					+ SimpleLuceneIndex.encodeToTerm("http://www." + url.getHost(), false) + "*)";
			return term;
		}
		/*
		 * else if (term.startsWith("id:")) { term = term.substring(3, term.length());
		 * term = "id:" + SimpleLuceneIndex.encodeToTerm(term, false); return term; }
		 */
		else {
			return term;
		}
	...
	/**
	 * This method must be called at just after the user's query has been processed
	 * by Lucene. It is used to log user queries.
	 *
	 * @param query
	 *            The text query entered by the user or submitted by the web service
	 *            client, or null if none was submitted with the request
	 * @param statusCode
	 *            The http status code
	 * @param contentLen
	 *            The length of the data returned, in bytes
	 * @param numSearchResults
	 *            The number of search results returned by the query
	 * @param totalRecords
	 *            The total number of records in the system at the time of the query
	 * @param rank
	 *            The rank of the item(s) retunred on the page. If this is a results
	 *            page, then this is rank of the first item on the page.
	 * @param servletContext
	 *            The servletContext
	 * @param request
	 *            The HTTP request
	 * @param searchType
	 *            The search type (search, histogram, what's new, ws-uql, ws-lql,
	 *            etc)
	 * @return True iff successful, else false
	 */
	public final static synchronized boolean logQuery(String query, ServletContext servletContext,
			HttpServletRequest request, int statusCode, int contentLen, int numSearchResults, int totalRecords,
			int rank, int searchType) {

		if (logger == null) {
			String logfile = (String) servletContext.getAttribute("queryLogFile");
			...
			// LogManager will create the file if needs to but the surrounding directory
			// must already exist.
			try {
				logger = DleseLogManager.getClfLogger(Level.FINEST,
	...
	/**
	 * Get a string for the free-text portion of the query log entry. The text
	 * contains the keywords entered by the user as well as all the metadata
	 * criteria selected in the interface at the time of the search.
	 *
	 * @param req
	 *            The HTTP request.
	 * @param query
	 ...
	 * @param searchType
	 *            DESCRIPTION
	 * @return A string suitable for output in the free-text portion of a query log
	 *         entry.
	 */
	...
	/**
	 * Gets the metadata vocab criteria selected at the time of the query formatted
	 * in a mannor suitable to be output in the free-text portion of the query log
	 * entry.
	 *
	 * @param metadataVocabString
	 *            The metadata input criteria as returned by the current
	 *            MetadataVocabInputState object
	 * @return The metadata vocab criteria selected at the time of the query
	 *         formatted in a mannor suitable to be output in the free-text portion
	 *         of the query log entry.
	 */
	...
	/**
	 * Creates a comma-separated string of all user-selected terms for a particular
	 * vocab category. Assumens that the string passed out is in the format output
	 * by the MetadataVocabInputState object.
	 *
	 * @param vocabCategory
	 *            The category of vocab terms that should be extracted, for example
	 *            'learningcontext' or 'resourcetype'
	 * @param metadataVocabString
	 *            The MetadataVocabInputState object's output string containing the
	 *            current vocabs selected.
