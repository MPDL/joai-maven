 ...
 * conjunction with the {@link org.dlese.dpc.dds.action.form.DDSAdminQueryForm}
...
public final class DDSAdminQueryAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		// if (request.getParameterMap() == null || request.getParameterMap().size() ==
		// 0)
		// sqf.setResults(null);

		// If a new search was requested, make sure other params are reset in the
		// session bean:
	 ...
	 *                if a servlet exception occurs
	 */
	public ActionForward handleMetadataSearchRequest(ActionMapping mapping, DDSAdminQueryForm queryForm,
		...
		/*
		 * ArrayList mapperErrors = new ArrayList(); mapperErrors.add(SELECT_NONE); List
		 * tmp = rm.getIndexedIdMapperErrors(); if (tmp != null)
		 * mapperErrors.addAll(tmp); queryForm.setIdMapperErrors(mapperErrors);
		 */
		/*
		 * ArrayList accessionStatus = new ArrayList(); accessionStatus.add(SELECT_ALL);
		 * tmp = rm.getIndexedAccessionStatuses(); if (tmp != null)
		...
		try {
			...
			if (request.getParameter("q") != null) {
				...
				/*
				 * String selectedCollection = request.getParameter("ky"); if
				 * (selectedCollection != null && selectedCollection.length() > 0 &&
				 * !selectedCollection.equals(SELECT_ALL)) { query = "(" + query + ") AND ky:" +
				 * selectedCollection; }
				 */
				...
				if (selectedAccessionStatuses != null && selectedAccessionStatuses.length > 0
				...
				if (searchFormats != null && searchFormats.size() > 0
				...
				if (selectedIndexedFormats != null && selectedIndexedFormats.length > 0
			...
			// Display the record full view:
				...
				if (id.length() == 0) {
					errors.add("message",
							new ActionError("generic.message", "No ID indicated. Please supply an ID number."));
			...
			// Display the record in the given format. Record id and metadataFormat should
			// be in the same request:
				...
				if (metadata != null && metadata.length() > 0) {
					...
					// (?s) sets the DOTALL mode, making . match line endings as well as all other
					// chars.
			...
			// Display the text in the given file given an item id:
				...
				String metadata = ((FileIndexingServiceDocReader) resultDocs.get(0).getDocReader())
						.getFullContentEncodedAs("UTF-8");
				// String metadata = Files.readFileToEncoding( ((XMLDocReader)
				// resultDocs[0].getDocReader()).getFile(), "UTF-8").toString();
				if (metadata != null && metadata.length() > 0) {
	...
	/**
	 * Handle a request to search over web logs and forwared to the appropriate jsp
	 ...
	 *                if a servlet exception occurs
	 */
	public ActionForward handleWebLogSearchRequest(ActionMapping mapping, DDSAdminQueryForm form,
			HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {

		SimpleLuceneIndex webLogIndex = (SimpleLuceneIndex) servlet.getServletContext().getAttribute("webLogIndex");
		...
		try {
			...
			// No recognizable param existed:
				...
				errors.add("message",
						new ActionError("generic.message", "The web loq request is not valid in this context."));
				saveErrors(request, errors);
				return mapping.findForward("weblog.query");
			}
	...
	/*
	 * private final void setCollectionsAndFormats( SimpleLuceneIndex index,
	 * DDSAdminQueryForm queryForm, RepositoryManager rm) { if
	 * (index.getLastModifiedCount() > indexLastModified) { indexLastModified =
	 * index.getLastModifiedCount(); // Get collections in the index: List cols =
	 * index.getTerms("collection"); collections = new ArrayList();
	 * collections.add(SELECT_ALL); if (cols != null) collections.addAll(cols);
	 * collectionLabels = new ArrayList(); collectionLabels.add(SELECT_ALL); if
	 * (cols != null) { for (int i = 0; i < cols.size(); i++) { String col =
	 * (String) cols.get(i); collectionLabels.add(col.substring(1, col.length())); }
	 * } if (rm != null) { formats = new ArrayList(); formats.add(SELECT_ALL);
	 * formats.addAll(rm.getAvailableFormatsList()); List tmp =
	 * rm.getIndexedFormats(); indexedFormats = new ArrayList();
	 * indexedFormats.add(SELECT_ALL); indexedFormats.addAll(tmp);
	 * indexedFormatsLabels = new ArrayList(); indexedFormatsLabels.add(SELECT_ALL);
	 * for (int i = 0; i < tmp.size(); i++) { String fmt = (String) tmp.get(i);
	 ...
	 * queryForm.setFormats(formats); queryForm.setIndexedFormats(indexedFormats);
	 ...
	 */
	/*
	 * public String getNextResultsUrl() { if (resultDocs == null || start < 0)
	 * return null; int e = start + numPagingRecords; int n = resultDocs.length; if
	 * (e >= n) return null; String end = getEnd(); if (end == null) return null;
	 * return "q=" + getQe() + "&s=" + end + getNonPaigingParams(); }
	 */
	/**
	 * Gets the HTTP parameters that should be used to retrieve the previous set of
	 * results.
	 *
	 * @param isDebugOutput
	 *            The new debug value
	 */
	/*
	 * public String getPrevResultsUrl() { if (resultDocs == null || start <= 0)
	 * return null; int p = start - numPagingRecords; int prev = p > 0 ? p : 0;
	 * return "q=" + getQe() + "&s=" + prev + getNonPaigingParams(); }
	 */
	// -------------- Debug ------------------
