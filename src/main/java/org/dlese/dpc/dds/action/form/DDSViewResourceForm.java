...
public class DDSViewResourceForm extends ActionForm implements Serializable {
	 ...
	 *
	 * @param append
	 *            The new vocabCacheFeedbackString value
	 */
	 ...
	 *
	 * @param vocabCacheGroup
	 *            The new vocabCacheGroup value
	 */
	 ...
	 *
	 * @param id
	 *            The new primaryResultDocId value
	 */
	 ...
	 *
	 * @param primaryResultDoc
	 *            The new primaryResultDoc value
	 */
	 ...
	 * object
	 *
	 * @param primaryResultDocCollectionKey
	 *            The new primaryResultDocCollectionKey value
	 */
			...
			for (int i = 0; i < annoResults.size(); i++) {
				...
				if ((annoReader != null)
						&& (annoReader.getType().equals("Review")
								|| annoReader.getType().equals("Average scores of aggregated indices")
								|| annoReader.getType().equals("Editor's summary"))
						&& (annoReader.getPathway().length() > 0) && !annoReader.getStatus().matches(".*progress")) {
					return "true";
				}
			}
		}
	...
	public String getHasOtherReviews() {
		...
		if (annoResults != null) {
			for (int i = 0; i < annoResults.size(); i++) {
				DleseAnnoDocReader annoReader = (DleseAnnoDocReader) annoResults.get(i).getDocReader();
				if ((annoReader != null) && annoReader.getType().equals("Review")
						&& annoReader.getPathway().length() == 0) {
					return "true";
				}
			}
	...
	/**
	 * Gets the hasChallengingLearningContexts attribute of the DDSViewResourceForm
	 * object
	 *
	...
	public String getHasChallengingLearningContexts() {
		ResultDocList annoResults = ((ItemDocReader) primaryResultDoc.getDocReader()).getAnnotationResultDocs();
		for (int i = 0; i < annoResults.size(); i++) {
			if (((DleseAnnoDocReader) annoResults.get(i).getDocReader()).getType()
					.equals("Information on challenging teaching and learning situations")) {
				return "true";
			}
		}
		return "false";
