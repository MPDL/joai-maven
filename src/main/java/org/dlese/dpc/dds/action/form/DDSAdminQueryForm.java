 ...
 * {@link org.dlese.dpc.index.SimpleLuceneIndex}. This class works in conjuction
 * with the {@link org.dlese.dpc.dds.action.DDSAdminQueryAction} Struts Action
 * class.
 *
...
public final class DDSAdminQueryForm extends VocabForm implements Serializable {
	 ...
	 *
	 * @param vocabInputState
	 *            The new vocabInputState value
	 */
	 ...
	 *
	 * @param showQueryOptions
	 *            The new showQueryOptions value
	 */
	 ...
	 */
	/*
	 * public void setIdMapperErrors(List errors) { idMapperErrors = errors;
	 * idMapperErrorLabels = null; if (idMapperErrors != null &&
	 * idMapperErrors.size() > 0) { idMapperErrorLabels = new ArrayList(); String
	 * tmp; for (int i = 0; i < idMapperErrors.size(); i++) { tmp = (String)
	 * idMapperErrors.get(i); if (tmp.equals(DDSAdminQueryAction.SELECT_NONE))
	 * idMapperErrorLabels.add(DDSAdminQueryAction.SELECT_NONE); else if
	 * (tmp.equals("noerrors"))
	 * idMapperErrorLabels.add(DDSAdminQueryAction.NO_ERRORS); else
	...
	public List getCollections() {
		try {
			if (getServlet().getServletContext().getAttribute("reload_admin_collection_menus") != null)
				collections = null;
			if (collections == null) {
				RepositoryManager rm = (RepositoryManager) servlet.getServletContext()
	 ...
	 * prtln("setCollectionLabels() size "); this.collectionLabels =
	 * collectionLabels; }
	 */
	/**
	 ...
	 */
	/*
	 * public List getCollectionLabels() { if(collectionLabels == null){ List cols =
	 * getCollections(); if(cols != null){ collectionLabels = new } } return
	 * collectionLabels; }
	 */
	/**
	 ...
	 *
	 * @param formatsLabels
	 *            The new indexedFormatsLabels value
	 */
	...
	public String[] getSifmts() {
		/*
		 * if (selectedIndexedFormats == null) { selectedIndexedFormats = new String[1];
		 * if (getIndexedFormats() == null || getFormats().size() == 0) {
		 * selectedIndexedFormats[0] = ""; } else { selectedIndexedFormats[0] = (String)
		 * getIndexedFormats().get(0); } }
		 */
	 ...
	 *
	 * @param selectedIndexedFormats
	 *            The new sifmts value
	 */
	...
	/**
	 * Gets the collection that has been selected by the user in the UI via a Select
	 * tag. For example '0dcc'. For use with a Struts select tag that does not have
	...
	/**
	 * Sets the collection that has been selected by the user in the UI via a Select
	 * tag. For example '0dcc'. For use with a Struts select tag that does not have
	 * multiple selection enabled.
	 *
	...
	/**
	 * Gets the collections that have been selected by the user in the UI. For
	 * example '06' '006'. For use with a Struts select tag that has multiple
	 * selection enabled.
	 * <p>
	 *
	 *
	 ...
	 * DESCRIPTION
	 *
	 * @param selectedAccessionStatuses
	 *            DESCRIPTION
	 */
	 ...
	 *
	 * @param selectedIdMapperErrors
	 *            The new selectedIdMapperErrors value
	 */
	...
	/**
	 * Sets the collection that has been selected by the user in the UI. For example
	...
	/**
	 * Sets the collection that has been selected by the user in the UI. For example
	 * '06' '008'. For use with a Struts select tag that has multiple selection
	 * enabled.
	 *
	 * @param ky
	 *            The new sc value
	...
	/**
	 * Gets the selected collection(s) parameter to be inserted in the URL that gets
	...
	/*
	 * public void setDocReader(XMLDocReader docReader) { this.docReader =
	 * docReader; } public XMLDocReader getDocReader() { return docReader; }
	 */
	/**
	 * Gets the search results returned by the
	 * {@link org.dlese.dpc.index.SimpleLuceneIndex}.
	 *
	...
	/**
	 * Gets the IDMapper Mmd record for this ADN resource, otherwise null. If the
	 * given record is not in ADN, null will be returned.
	 *
	...
	/**
	 * Gets the IDMapper Mmd records for dupulicate resources in other collections,
	 * or null.
	 *
	...
	public MmdRec[] getMmdDupesInOtherCollections() {
		try {
			...
			RecordDataService rdc = getRepositoryManager().getRecordDataService();
			Query idMapperQueryObject = rdc.getIdMapperQueryObject();
			MmdRec[] recs = rdc.getAssociatedMMDRecs(RecordDataService.QUERY_OTHER, dr.getId(), dr.getCollection(),
					idMapperQueryObject);
			rdc.closeIdMapperQueryObject(idMapperQueryObject);
			return recs;
		} catch (Throwable t) {
	...
	/**
	 * Gets the IDMapper Mmd records for dupulicate resources in the same as this
	 * resource collection, or null.
	 *
	 * @return The duplicate Mmd Records, or null
	 */
	public MmdRec[] getMmdDupesInSameCollection() {
		try {
			...
			RecordDataService rdc = getRepositoryManager().getRecordDataService();
			Query idMapperQueryObject = rdc.getIdMapperQueryObject();
			MmdRec[] recs = rdc.getAssociatedMMDRecs(RecordDataService.QUERY_SAME, dr.getId(), dr.getCollection(),
					idMapperQueryObject);
			rdc.closeIdMapperQueryObject(idMapperQueryObject);
			return recs;
		} catch (Throwable t) {
	...
	public ArrayList getNrqParams() {
		...
		while (params.hasMoreElements()) {
			...
			if (!param.equals("rq") && !param.equals("s")) {
	 ...
	 *
	 * @param numPagingRecords
	 *            The new numPagingRecords value
	 */
	...
	public final String getNonPaigingParams() {
		...
		/*
		 * if (request == null) return null; Enumeration params =
		 * request.getParameterNames(); String param; String vals[]; StringBuffer
		 * addParams = new StringBuffer(); try{ while (params.hasMoreElements()) { param
		 * = (String) params.nextElement(); if (!param.equals("q") &&
		 * !param.equals("s")) { vals = request.getParameterValues(param); for (int i =
		 * 0; i < vals.length; i++){ addParams.append("&" + param + "=" +
		 * URLEncoder.encode(vals[i],"utf-8")); } } } }catch(Exception e){
		 * addParams.toString(); } return addParams.toString();
		 */
	}
