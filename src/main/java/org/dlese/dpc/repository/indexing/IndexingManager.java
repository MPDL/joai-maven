...
/**
 * Manages indexing processes from external sources to RepositoryManager that
 * implement the {@link IndexingEventHandler} Interface.
 *
...
public class IndexingManager {
	...
	/**
	 * Adds an event handler that performs the indexing actions, sets the config
	 * directory and fires the configure and init event.
	 *
	 * @param itemIndexerClassName
	 *            An event handler that implements the ItemIndexer Interface.
	 * @exception Exception
	 *                If error
	 */
	...
	/**
	 * Fire this event to indicate to watchers that the indexer is ready to accept
	 * indexing calls.
	 *
	 * @param itemIndexerClassName
	 *            An ItemIndexer to fire the event on, or null to fire the event on
	 *            all handlers
	 * @exception Exception
	 *                If error
	 */
	...
	/**
	 * Fire this event to indicate to watchers that they should update their
	 * collections using CollectionIndexer.putCollection() and
	 * CollectionIndexer.deleteCollection().
	 *
	 * @param itemIndexerClassName
	 *            An ItemIndexer to fire the event on, or null to fire the event on
	 *            all handlers
	...
	/**
	 * Fire this event to indicate to watchers that they should perform indexing for
	 * the given collection if they are managing it using
	 * {@link CollectionIndexer#putRecord}, {@link CollectionIndexer#deleteRecord},
	 * {@link CollectionIndexer#deletePreviousSessionRecords}.
	 *
	 * @param collectionKey
	 *            The collection key that should be indexed
	 * @param itemIndexerClassName
	 *            An ItemIndexer to fire the event on, or null to fire the event on
	 *            all handlers
	 * @exception Exception
	...
	public void fireIndexCollectionEvent(String collectionKey, String itemIndexerClassName) throws Exception {
		IndexingEvent indexingEvent = new IndexingEvent(IndexingEvent.BEGIN_INDEXING_COLLECTION, collectionKey,
				collectionIndexer);
		fireEvent(indexingEvent, itemIndexerClassName);
	}
	...
	/**
	 * Fire this event to indicate to watchers that they should perform indexing for
	 * ALL collections that they know about using
	 * {@link CollectionIndexer#putRecord}, {@link CollectionIndexer#deleteRecord},
	 * {@link CollectionIndexer#deletePreviousSessionRecords}.
	 *
	 * @param itemIndexerClassName
	...
	public void fireIndexAllCollectionsEvent(String itemIndexerClassName) throws Exception {
		IndexingEvent indexingEvent = new IndexingEvent(IndexingEvent.BEGIN_INDEXING_ALL_COLLECTIONS, null,
				collectionIndexer);
		fireEvent(indexingEvent, itemIndexerClassName);
	}

	/**
	 * Fire this event to indicate to watchers that they should abort indexing at
	...
	/**
	 * Fire this event to indicate to watchers that they update their configuration
	 * settings. This is called automatically once at startup.
	 *
	 * @param itemIndexerClassName
	 *            An ItemIndexer to fire the event on, or null to fire the event on
	 *            all handlers
	...
	public void fireConfigureAndInitializeEvent(String itemIndexerClassName) throws Exception {
		IndexingEvent indexingEvent = new IndexingEvent(IndexingEvent.CONFIGURE_AND_INITIALIZE, null,
				collectionIndexer);
		fireEvent(indexingEvent, itemIndexerClassName);
	}
	 ...
	 *
	 * @param msg
	 *            The feature to be added to the IndexingMessage attribute
	 */
	protected void addIndexingMessage(String msg) {
		indexingMessages.add(
				getSimpleDateStamp() + " " + msg + " [Index size: " + repositoryManager.getIndex().getNumDocs() + "]");
	...
	private class IndexingEventThread extends Thread {
		...
		public void run() {
			...
			for (int i = 0; i < currentSize; i++) {
				...
				try {
					// If no ItemIndexer has been indicated, fire the event on all, otherwise only
					// fire the event on the given ItemIndexer instance:
	...
	/**
	 * Starts or restarts the indexing timer thread to run every 24 hours, beginning
	 ...
	 *            The time of day at which start the indexing process in H:mm (24
	 *            hour time) format, for example 0:35 or 23:35, or null to disable
	 *            auto indexing.
	...
	public void startIndexingTimer(String indexingStartTime, String indexingDaysOfWeek) throws Exception {
		...
		try {
			...
			_indexingStartTimeDate = Utils.convertStringToDate(year + " " + dayInYear + " " + indexingStartTime,
					"yyyy D H:mm");

			Calendar startCal = new GregorianCalendar();
			// prtln("Current day-of-week is: " +
			// Utils.getDayOfWeekString(startCal.get(Calendar.DAY_OF_WEEK)));
		...
		if (_indexingDaysOfWeek != null) {
			...
			for (int i = 0; i < _indexingDaysOfWeek.length; i++)
				daysOfWeekMsg += Utils.getDayOfWeekString(_indexingDaysOfWeek[i])
						+ (i == _indexingDaysOfWeek.length - 1 ? "" : ", ");
		}
	...
	/**
	 * Gets the indexingStartTime Date, representing the time of day the indexer
	 * will run, or null if no indexing cron is being used.
	 *
