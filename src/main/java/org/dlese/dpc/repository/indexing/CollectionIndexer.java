...
public class CollectionIndexer {
	...
	/**
	 * Put a collection in the repository. If the collection key does not exist, the
	 * collection will be created otherwise it will be updated with this new
	 * information.
	 *
	 * @param collectionKey
	 *            The collection key, for example 'dcc'
	 * @param format
	 *            The XML format for the records that will reside in the collection
	 * @param name
	 *            A display name for the collection
	 * @param description
	 *            A long description for the collection
	 ...
	 *                If error
	 */
	public void putCollection(String collectionKey, String format, String name, String description,
	...
	/**
	 * Put a record in a collection, adding or replacing the given record in the
	 * repository.
	 *
	 * @param recordXml
	 *            The XML for this record
	 * @param collectionKey
	 *            The collection this record should be put
	 * @param id
	 *            The ID of the record - ignored if the ID can be derived from the
	 *            record XML
	 * @param session
	 *            An indexing session ID - used for deleting records later after a
	 *            new indexing session occurs
	 * @exception Exception
	 *                If error
	 */
	public void putRecord(String recordXml, String collectionKey, String id, CollectionIndexingSession session)
			throws Exception {
		SetInfo setInfo = repositoryManager.getSetInfo(collectionKey);
		if (setInfo == null)
			throw new Exception("Collection '" + collectionKey + "' is not configured");

		// printPrivateStatusMessage("Adding record ID '" + id + "', collection '" +
		// collectionKey + "'");
	...
	/**
	 * Gets the newCollectionIndexingSession attribute of the CollectionIndexer
	 * object
	 *
	 * @param collectionKey
	 *            The collection
	...
	/**
	 * Post a status message to the indexing process to let admins know the current
	 * indexing status.
	 *
	 * @param msg
	 *            The message to post
	...
	/**
	 * Gets the existingCollectionIndexingSession attribute of the CollectionIndexer
	 * object
	 *
	 * @param sessionId
	 *            The session ID
	...
	public void deletePreviousSessionRecords(CollectionIndexingSession currentSession) {
		...
		if (indexedSessions != null) {
			for (int i = 0; i < indexedSessions.size(); i++) {
				CollectionIndexingSession indexedSession = getExistingCollectionIndexingSession(
						(String) indexedSessions.get(i));
				// prtln("Session ID found: " + indexedSession + " collection: " +
				// indexedSession.getCollection());

				if (currentSession.getCollection().equals(indexedSession.getCollection())
						&& !currentSession.equals(indexedSession)) {
					int numBefore = index.getNumDocs();
					// String msg = "Deleting records for session: " + indexedSession + ". Num docs:
					// " + index.getNumDocs();
					// prtln(msg);
					// printPrivateStatusMessage(msg);
					index.removeDocs("indexSessionId", indexedSession.getSessionId());

					String msg = "Deleted records for session: " + indexedSession + ". Num deleted: "
							+ (numBefore - index.getNumDocs());
					printPrivateStatusMessage(msg);
				}
			}
		}
