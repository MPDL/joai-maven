...
import org.dlese.dpc.index.writer.XMLFileIndexingWriter;
import org.dlese.dpc.index.writer.XMLFileIndexingWriterFactory;
...
/**
 * Manages a repository of metadata files for use in discovery, cataloging, OAI
 * and other applications. Provides and maintains a Lucene index of the
 * metadata, with support for multiple collections and metadata formats.
 * <p>
 *
 *
 *
 * @author John Weatherley
 * @version $Id: RepositoryManager.java,v 1.187.2.1 2012/02/15 23:28:03 jweather
 *          Exp $
 */
public class RepositoryManager {
	...
	/**
	 * Specifies in the
	 * {@link #setFileIndexingPlugin(String xmlFormat, FileIndexingPlugin plugin)}
	 * method that the given plugin should be used when indexing all XML formats.
	 */
	 ...
	 *
	 * @param indexCollectionRecords
	 *            True to index the collections records when called
	...
	public int init(boolean indexCollectionRecords) {
		...
		xmlFileIndexingWriterFactory = new XMLFileIndexingWriterFactory(recordDataService, getIndex(),
				xmlIndexerFieldsConfig, additionalIndexers);

		index.setAttribute("repositoryManager", this);

		// Set up file indexer to remove docs altogether or save them as status deleted
		if (removeDocs)
			fileIndexingService = new FileIndexingService(index, 0, repositoryDataDir + "/file_indexing_service_data",
					maxFilesToIndex);
		else
			...
			try {
				...
				if (startTime.compareTo(oneHourFromNow) < 0) {
					if (dayInYear == 365) {
						...
						dayInYear++;
					startTime = Utils.convertStringToDate(year + " " + dayInYear + " " + indexingStartTime,
							"yyyy D H:mm");
				}
					...
					if (indexingDaysOfWeek != null && indexingDaysOfWeek.trim().length() > 0) {
						...
						for (int i = 0; i < indexingDaysOfWeekStrings.length; i++) {
							...
							if (indexingDaysOfWeekArray[i] < 1 || indexingDaysOfWeekArray[i] > 7)
								throw new Exception(
										"Value must be an integer from 1 to 7 but found " + indexingDaysOfWeekArray[i]);
						}
					}
				} catch (Throwable t) {
				...
				prtlnErr(
						"Syntax error in context parameter 'indexingStartTime' or 'indexingDaysOfWeek'. Indexing timer cron not started: "
								+ e.getMessage());
			}
	...
	/**
	 * Loads a new index at the current location and closes the previous one, if one
	 * was open.
	 *
	 * @exception Exception
	...
	private void loadNewIndex() throws Exception {
		...
		// Set up an Analyzer for indexing and searching that handles each field
		// appropriately (based on the config provided)
		...
		// Configure the core Analyzers to use on a per-field basis, overriding any
		// definitions provided above:
		perFieldAnalyzer
				.addAnalyzersInBundle(Utils.getPropertiesResourceBundle("RepositoryManagerLuceneAnalyzers.properties"));
		perFieldAnalyzer.addAnalyzersInBundle(
				Utils.getPropertiesResourceBundle("FileIndexingPluginLuceneAnalyzers.properties"));
	...
	/**
	 * Changes the frequency of reindexing to the new value. Same as
	 * {@link #startTimerThread(long updateFrequency)}.
	 *
	...
	/**
	 * Start or restarts the indexer thread with the given update frequency. Same as
	 * {@link #changeUpdateFrequency(long updateFrequency)}.
	 *
	 * @param updateFrequency
	 *            The number of seconds between index updates.
	 */
	...
	/**
	 * Start or restarts the timer thread with the given update frequency, beginning
	 * at the specified time/date. Use this method to schedule the timer to run as a
	 * nightly cron, beginning at the time you wish the indexer to run.
	 *
	 * @param updateFrequency
	 *            The number of seconds between index updates.
	 * @param startTime
	 *            The time at which start the indexing process.
	 */
				...
				try {
					...
					if (indexingDaysOfWeekArray != null) {
						...
						for (int i = 0; i < indexingDaysOfWeekArray.length; i++)
							daysOfWeekMsg += Utils.getDayOfWeekString(indexingDaysOfWeekArray[i])
									+ (i == indexingDaysOfWeekArray.length - 1 ? "" : ", ");
					}

					prtln("Indexing timer is scheduled to start "
							+ Utils.convertDateToString(startTime, "EEE, MMM d, yyyy h:mm a zzz") + ", and run every "
							+ Utils.convertMillisecondsToTime(freq) + " on " + daysOfWeekMsg + ".");
	...
	/**
	 * Deletes the repository index and re-initializes a new, empty one in its
	 * place.
	 */
	...
	/**
	 * Instructs the indexer to stop indexing wherever its at in the process. This
	...
	/**
	 * Gets the version number of the last time the repository index was modified by
	 * adding, deleting or changing an item. The version number counts the number of
	 * times the index was modified.
	 *
	...
	/**
	 * Gets the time the sets configuation status was last modified (enabled,
	 * disabled, added, deleted).
	 *
	 ...
	 *
	 * @param repositoryConfigDir
	 *            The directory where config files reside (default search/boosting
	 *            fields, other config)
	 * @param repositoryDataDir
	 *            The directory where the repository data and certain configs
	 *            resides.
	 * @param updateFrequency
	 *            How often to update the index, in minutes.
	 * @param maxFilesToIndex
	 *            Max files to index per block.
	 * @param removeDocs
	 *            True to remove documents completely from the index, false to keep
	 *            the documents in the index but marked as deleted.
	 */
	public RepositoryManager(File repositoryConfigDir, String repositoryDataDir, int updateFrequency,
			int maxFilesToIndex, boolean removeDocs) {
	 ...
	 *            The directory where config files reside (default search/boosting
	 *            fields, other config)
	 * @param itemIndexerConfigDir
	 *            The directory where ItemIndexer configs are located
	 * @param repositoryDataDir
	 *            Directory where admin data is stored.
	 * @param indexLocation
	 *            Directory where index is stored.
	 * @param indexingStartTime
	 *            The time of day to start the indexer, in H:mm, for example 0:11 or
	 *            23:11.
	 * @param indexingDaysOfWeek
	 *            The days of week to run the indexer as a comma separated list of
	 *            integers, for example 1,3,5 where 1=Sunday, 2=Monday, 7=Saturday
	 *            etc. or null for all days.
	 * @param recordDataService
	 *            The RecordDataService to use, or null if none is needed.
	 * @param removeDocs
	 *            True to remove documents completely from the index, false to keep
	 *            the documents in the index but marked as deleted.
	 * @param reindexAllFiles
	 *            True to instruct the indexer to delete and reindex each file
	 *            during each indexing pass, false to only reindex those files whoes
	 *            mod time have changed (more efficient).
	 */
	public RepositoryManager(File repositoryConfigDir, File itemIndexerConfigDir, String repositoryDataDir,
			String indexLocation, String indexingStartTime, String indexingDaysOfWeek,
			RecordDataService recordDataService, boolean removeDocs, boolean reindexAllFiles) {
		// prtln("RepositoryManager()");
	...
	/**
	 * Gets the path to an XML file the defines IDs that the IDMapper service should
	 * exclude from its list of duplicates.
	 *
	...
	/**
	 * Sets the path to an XML file the defines IDs that the IDMapper service should
	 * exclude from its list of duplicates. Set to null to use none
	 *
	 * @param filePath
	 *            The new idMapperExclusionFilePath value, or null to specify none
	 */
	...
	/**
	 * Gets the interval by which the index is updated to check for changes in the
	 * meatdata files, in minutes. A value of 0 indicates the index is not updated
	 * automatically.
	 *
	 ...
	 *
	 * @return The the date the indexer scheduled to start, or null if not
	 *         available.
	 */
	...
	/**
	 * Gets the days of the week the indexer will run as an array of
	 * Calendar.DAY_OF_WEEK fields, or null to indicate all days of the week.
	 *
	...
	/**
	 * Sets the metadataVocabAudienceDefault attribute of the RepositoryManager
	 * object
	 *
	 * @param audience
	 *            The new metadataVocabAudienceDefault value
	 */
	...
	/**
	 * Gets the metadataVocabAudienceDefault attribute of the RepositoryManager
	 * object
	 *
	...
	/**
	 * Sets the metadataVocabLanguageDefault attribute of the RepositoryManager
	 * object
	 *
	 * @param language
	 *            The new metadataVocabLanguageDefault value
	 */
	...
	/**
	 * Gets the metadataVocabLanguageDefault attribute of the RepositoryManager
	 * object
	 *
	 ...
	 *
	 * @param override
	 *            True to overried all existing settings, false to leave existing
	 *            values untouched.
	 */
	...
	/**
	 * Gets the numIdentifiersResults per resumptionToken to be returned in OAI
	...
	/**
	 * Sets the numIdentifiersResults per resumptionToken to be returned in OAI
	 * ListIdentifiers requests.
	 *
	 * @param value
	 *            The new numIdentifiersResults value
	 */
	...
	/**
	 * Gets the numRecordsResults per resumptionToken to be returned in OAI
	...
	/**
	 * Sets the numRecordsResults per resumptionToken to be returned in OAI
	 * ListRecords requests.
	 *
	 * @param value
	 *            The new numRecordsResults value
	 */
	...
	/**
	 * Removes an object from the adminData datamanager iff it was present, else
	 * does nothing.
	 *
	 * @param key
	 ...
	 *
	 * @param value
	 *            The new providerStatus value [ENABLED or DISABLED]
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	 ...
	 *
	 * @param value
	 *            The new harvesterStatus value [ENABLED or DISABLED]
	 */
	 ...
	 * This equates to the namespace-identifier portion of the OAI Identifier
	 * Format. An example namespace-identifier is 'dlese.org'. See
	 * <a href="http://www.openarchives.org/OAI/2.0/guidelines-oai-identifier.htm">
	 * OAI Identifier Format</a> for information.
	 *
	 * @return The repositoryIdentifier, which is the namespace-identifier portion
	 *         of the OAI identifier format.
	 */
	...
	/**
	 * Gets the OAI Identifier Format's scheme and namespace-identifier prefix, for
	 * example 'oai:dlese.org:'. This prefix should be appended to the beginning of
	 * the local-identifier to create the fully-qualified OAI Identifier for a given
	 * item in the repository. See
	 * <a href="http://www.openarchives.org/OAI/2.0/guidelines-oai-identifier.htm">
	 * OAI Identifier Format</a> for information.
	 *
	 * @return The OAI Identifier Format's scheme and namespace-identifier prefix,
	 *         for example 'oai:dlese.org:'.
	 */
	 ...
	 *
	 * @param value
	 *            The new earliestDatestamp value
	 */
	...
	/**
	 * Gets the OAI-PMH deletedRecord support, which depends on whether deleted
	 * records are removed from the repository index or kept as status deleted.
	 * Valid responses include "no" (deletions not keept or advertised) or
	 * "transient" (deletions are kept and advertised, unless the repository index
	 * is reset).
	 *
	...
	/**
	 * Gets the granularity attribute of the RepositoryManager object. Legitimate
	 * values are YYYY-MM-DD and YYYY-MM-DDThh:mm:ssZ with meanings as defined in
	 * ISO8601.
	 *
	...
	/**
	 * Sets the granularity attribute of the RepositoryManager object. Legitimate
	 * values are YYYY-MM-DD and YYYY-MM-DDThh:mm:ssZ with meanings as defined in
	 * ISO8601.
	 *
	 * @param value
	...
	/**
	 * Sets the end portion of the OAI baseURL that is being used, for example
	 * 'provider'.
	 *
	 * @param baseUrlEnding
	 *            The new providerBaseUrlEnding value
	 */
	...
	/**
	 * Sets the serverUrl (scheme, hostname and port) displayed in the OAI data
	 * provider baseUrl and elsewhere. Example values: 'http://www.example.com' or
	 * 'http://www.example.com:8080'. If set to null (default), then the scheme,
	 * hostname and port number will be determined automatically by examining the
	 * URL that was requested by the client.
	 *
	 * @param serverUrl
	 *            The new serverUrl value
	 */
	 ...
	 * "none" indicates no compression is supported. Others: gzip ; compress.
	 *
	 * @param value
	 *            The feature to be added to the Compression attribute
	 */
	 ...
	 *
	 * @param value
	 *            The compression value to remove.
	 */
	 ...
	 *
	 * @param i
	 *            The compression ArrayList item to remove.
	 */
	 ...
	 *
	 * @param i
	 *            The index of the description to remove
	...
	/**
	 * Add the given admin Description to the repository. The aDescription is used
	 * in the Identify request. If the given Description already exists, nothing is
	 * changed.
	 *
	 * @param index
	 *            The index.
	 ...
	 *
	 * @param i
	 *            The description ArrayList index to remove.
	 */
	...
	/**
	 * Gets an array of IP address regular expressions that are trusted for access
	 * to non-discoverable items through the web service. The regular expressions
	 * may be used directly in the String.matches() method. For example:
	 * <p>
	 *
	...
	public String[] getTrustedWsIpsArray() {
		if (trustedWsIpsArray == null) {
			...
			// Convert to an appropriate regular expression (assumes numbers, periods and *
			// wildcard only):
	...
	/**
	 * Gets a Comma-separated String of IP address regular expressions that are
	 * trusted for access to non-discoverable items through the web service.
	 *
	...
	/**
	 * Sets the trustedWsIps attribute of the RepositoryManager, which should
	 * contain a comma-separated list of IP addresses for IPs that may access
	 * non-discoverable items through the web service. These may contain numbers,
	 * periods and the * wildcard only, for example 128.166.26.*.
	 *
	 * @param value
	 *            The new trustedWsIps value
	 */
	...
	/**
	 * Gets a single record from the repository by ID, restricted to those records
	 * that are avaiable/discoverable for OAI.
	 *
	 * @param id
	 *            The ID for the item
	 * @return The record or null if not available
	...
	public ResultDoc getRecordOai(String id) {
		...
		try {
			if (removeInvalidRecords)
				fullQuery = getIndex().getQueryParser().parse("id:" + SimpleLuceneIndex.encodeToTerm(id)
						+ " AND !valid:false" + " AND (" + getDiscoverableRecordsQuery() + ")");
			else
				fullQuery = getIndex().getQueryParser().parse(
						"id:" + SimpleLuceneIndex.encodeToTerm(id) + " AND (" + getDiscoverableRecordsQuery() + ")");
		} catch (Throwable t) {
	...
	/**
	 * Gets the results from a standard OAI-PMH ListIdentifiers or ListRecords query
	 * or an ODL search request.
	 *
	 * @param format
	 *            The metadata format.
	 * @param set
	 *            The set, or null.
	 * @param from
	 *            The from time, or null.
	 * @param until
	 *            The until time, or null.
	 * @return The queryResults value.
	 * @exception Exception
	 *                If error occues, exception will contain a message
	...
	public ResultDocList getOaiQueryResults(String format, String set, String from, String until) throws Exception {
		// prtln("getOaiQueryResults(): format: " + format + " set: " + set + " from: "
		// + from + " until: " + until);
		return getOdlQueryResults(format, set, from, until, null);
	}

	/**
	 * Gets the results of an OAI-PMH ListRecords or ListIdentifiers request or an
	 * OLD search. Set queryString to null to perform a standard OAI-PMH search and
	 * non-null for ODL search.
	 *
	 * @param format
	 *            The metadata format.
	 * @param set
	 *            The set to search over, or null.
	 * @param from
	 *            The from time, or null.
	 * @param until
	 *            The until time, or null.
	 * @param queryString
	 *            Must be non-null to indicate ODL search and null to indicate
	 *            regular OAI-PMH request.
	 * @return The queryResults value.
	 * @exception Exception
	 *                If error occues, exception will contain a message meant for
	 *                human consumption in response error messages.
	 * @see #setOaiFilterQuery
	 */
	public ResultDocList getOdlQueryResults(String format, String set, String from, String until, String queryString)
			throws Exception {

		// if (queryString != null)
		// prtln("getOdlQueryResults(): format: " + format + " set: " + set + " from: "
		// + from + " until: " + until + " query: " + queryString);

		// Standard OAI-PMH request (indicated by empty query):
		if (queryString == null) {
			// If this repsository is not allowed to support regular OAI-PMH requests,
			// return null
			if (!isOaiPmhEnabled)
				return null;
		...
		try {
			if (from != null && until == null) {
				...
				// If the times are the same, increment the 'until' time so that the result will
				// match records with that exact datestamp.
				if (until.toLowerCase().equals(from.toLowerCase()))
					dateFilter = new DateRangeFilter("oaimodtime", OAIUtils.getDateFromDatestamp(from, 0),
							OAIUtils.getDateFromDatestamp(until, 1));
				else
					dateFilter = new DateRangeFilter("oaimodtime", OAIUtils.getDateFromDatestamp(from, 0),
							OAIUtils.getDateFromDatestamp(until, 0));
				if (until.length() != from.length())
					throw new OAIErrorException(
		...
		// Construct the query for this ODL or OAI-PMH ListRecords or ListIdentifiers
		// request
		String query = "(" + queryString + ")" + " AND " + convertableFormatsQuery;
		if (removeInvalidRecords)
	 ...
	 * The given filter query persists until this method is called again or the
	 * RepositoryManager is reinstantiated. Pass null to this method to remove the
	 * filter query.
	 *
	 * @param luceneQuery
	 *            The Lucene query to filter records
	...
	private Query getFullOaiFilterQuery(String format) {
		...
		String query = getDiscoverableRecordsQuery() + " AND " + convertableFormatsQuery;
		if (removeInvalidRecords)
			query += " AND !valid:false";
		return null;
	}
	 ...
	 *
	 * @param formats
	 *            Formats that we want converted to.
	...
	/**
	 * Gets a query that limits a search to only those xml formats that can be
	 * converted to the format indicated. The conversion is done using the
	 * XmlConversionService.
	 *
	 * @param toFormat
	 *            The xml format to convert to
	 * @return A query that will return only records that can be converted to the
	 *         given format, or null if no convertable formats are avialable.
	 */
	...
	/**
	 * Determines whether the given ID is in the repository. Note that if given
	 * document was indexed but is not valid, this method MAY return false.
	 *
	 * @param id
	 *            An ID.
	...
	/**
	 * Determines whether the given set is in the repository index. Note that if the
	 * set is in the index it may also be disabled or all records in the set may
	 * have been moved to status deleted, if the set was removed from the
	 * repository.
	 *
	 * @param set
	 *            A set.
	...
	/**
	 * Gets all possible metadata formats that may be disiminated by this
	 * RepositoryManager. This includes formats that are available by conversion
	 * from the native format via the XMLConversionService.
	 *
	...
	/**
	 * Gets all formats that exist natively in the index. If none exist, and empty
	 * list is returned.
	 *
	...
	/**
	 * Gets the metadata format conversions that are available for the given format,
	 * including the given format.
	 *
	 * @param fromFormat
	 *            The initial format.
	...
	private final Hashtable getMetadataFormatsConversions(String fromFormat) {
		...
		if (xmlConversionService == null || fromFormat == null)
			return formats;
		ArrayList availableFormats = xmlConversionService.getAvailableFormats(fromFormat);
		for (int j = 0; j < availableFormats.size(); j++) {
			formats.put(availableFormats.get(j), new MetadataFormatInfo((String) availableFormats.get(j), "", ""));
		}
		return formats;
	}
	...
	public Hashtable getAvailableFormats(String id) {
		...
		if (resultDocs.size() > 0) {
			String nativeFormat = (String) ((FileIndexingServiceDocReader) resultDocs.get(0).getDocReader())
					.getDoctype();
	...
	/**
	 * Determines whether the given directory is configured in the repository. Note:
	 * Each directory should only be configured in the repository once. This method
	 * can be used to check prior to adding a directory.
	 *
	 ...
	 *
	 * @param set
	 *            The name of the set or, if the vocab mgr is being used, the name
	 *            of the encoded set key.
	 * @return True if the given set is currently configured in this repository and
	 *         is enabled.
	 */
	...
	/**
	 * Gets a List of file set key Strings such as 'dcc' or 'comet' that are
	...
	public ArrayList getConfiguredSets() {
		...
		/*
		 * String sets = ""; for(int i = 0; i < configuredSets.size(); i++) sets +=
		 * configuredSets.get(i) + " ";
		 */
		return (ArrayList) configuredSets.clone();
	}

	/**
	 * Gets a HashMap of SetInfos (directories of files) that are currently
	 * configured in this repository keyed by their set spec, for example dcc. If
	 * non, then empty Map is returned.
	 *
	...
	/**
	 * Gets a List of native file format Strings that are currently configured in
	 * this repository. This does not include formats that may be converted from the
	 * native formats via the XMLConversionService.
	 *
	...
	/**
	 * Gets the configured sets (directories of files) that are currently enabled in
	 * this repository. If the vacab manager is being used then the setSpec will be
	 * encoded into the vocab key, else it will not.
	 *
	 * @return The enabledSets value.
	 */
	public ArrayList getEnabledSets() {
		// Refactoring note: can remove the vocab mgr and just use the plan old set spec
		// in all cases...
	...
	/**
	 * Gets the configured sets that are currently enabled in this repository. If
	 * the vocab manager is being used then the setSpecs are encoded, else they are
	 * not. Both the key and the values in the HashMap are the same.
	 *
	 * @return The enabledSets value.
	 */
	...
	/**
	 * Gets the configured sets that are currently disabled in this repository. If
	 * the vocab manager is being used then the setSpecs are encoded, else they are
	 * not. Both the key and the values in the HashMap are the same.
	 *
	...
	/**
	 * Gets the configured sets that are currently enabled in this repository as a
	 * String suitable for use in s Lucene query, or an empty string if no sets are
	 * enabled.
	 *
	...
	/**
	 * Gets a query that limits a search to only those sets or collections that are
	 * not disabled.
	 *
	 * @return A query that limits a search to only sets that are not disabled, or
	 *         an empty String if none are disabled.
	 */
	/*
	 * public final String getRemoveDisabledSetsQuery() { // 5-26-2004 note: tried
	 * this method but not sure it works... needs debug... String query = null; //
	 * Remove disabled sets if (getDisabledSets().size() > 0) { query =
	 * "(collection:0* AND (!collection:0" + getDisabledSets().get(0); for (int i =
	 * 1; i < getDisabledSets().size(); i++) query += " OR !collection:0" +
	 * getDisabledSets().get(i); } else return null; query += "))"; return query; }
	 */
	/**
	 * Gets the query string used to limit searches to only those ADN items-level
	 * records that should be displayed in discovery.
	 *
	 * @return The query used to get only item-level records that are discoverable.
	...
	public final String getDiscoverableItemsQuery() {
		...
		String res = "accessionstatus:" + MmdRec.statusNames[MmdRec.STATUS_ACCESSIONED_DISCOVERABLE];
	...
	/**
	 * Gets the query string used to limit searches to only those records, of any
	 * format (item, collection, anno, etc.), that should be accessable in discovery
	 * or via the web service.
	 *
	 * @return The query used to get only records that are discoverable. The value
	 *         returned is always non-null.
	 */
	...
	/**
	 * Gets the query string used to limit searches to only those records, of any
	 * format (item, collection, anno, etc.), that should be accessable for OAI
	 * including 'enabled' deleted records. Removes all records in disabled file
	 * directories.
	 * <p>
	 *
	 * Refactoring note: Consider using this for method
	 * {@link #getDiscoverableRecordsQuery} instead.
	 *
	...
	/**
	 * Gets the query string used to limit searches to only those items that have an
	 * appropriate status for discovery. This does not limit by items whoes
	 * collection is not discoverable.
	 *
	 * @return The query string used to limit searches to only those items that have
	 *         an appropriate status for discovery. The value returned is always
	 *         non-null.
	 */
	...
	/**
	 * Gets the number of descrete ADN item-level resources that have been indexed
	...
	/**
	 * Gets the number of descrete resources of any format (adn, dlese_anno,
	 * news_opps, etc) that have been indexed and are currently discoverable.
	 *
	 * @return The number of resources of any format that are currently
	 *         discoverable.
	 */
	 ...
	 *
	 * @return A query String used to boost results that have DRC reviews above all
	 *         other criteria.
	 */
	...
	/**
	 * Gets the boosting factor used to rank resources that are referenced by more
	 * than one record.
	 *
	...
	/**
	 * Sets the boosting factor used to rank resources that are referenced by more
	 * than one record.
	 *
	 * @param boostFactor
	 *            The new boosting factor used to rank items in multiple records.
	 * @exception Exception
	 *                If error.
	 */
	...
	/**
	 * Sets whether OAI-PMH ListRecords and ListIdentifiers functionality is enabled
	 * for this repository. Set to false to disable regular OAI-PMH ListRecord and
	 * ListIdentifier resonses. QDL ListRecord and ListIdentifier requests are not
	 * effected.
	 *
	 * @param isEnabled
	 *            False to disable regular ListRecord and ListIdentifier OAI-PMH
	 *            responses
	 */
	...
	/**
	 * Gets whether OAI-PMH ListRecords and ListIdentifiers functionality is enabled
	 * for this repository. False means regular ListRecord and ListIdentifier
	 * responses are disabled. QDL ListRecord and ListIdentifier are always
	 * available, regardless.
	 *
	 * @return False means regular ListRecord and ListIdentifier OAI-PMH responses
	 *         are disabled
	 */
	 ...
	 *
	 * @param boostFactor
	 *            The new boosting factor used to rank items in the DRC.
	 * @exception Exception
	 *                If error.
	 */
	...
	/**
	 * Gets the boosting factor used to rank items with matching terms in the title
	 * field.
	 *
	 * @return The boosting factor used to rank items with matching terms in the
	 *         title field.
	 * @deprecated As of 12/2004, boosting behavior is controlled by properties set
	 *             in the RepositoryManager config.
	 */
	...
	/**
	 * Sets the boosting factor used to rank items with matching terms in the title
	 * field. Value must be zero or greater.
	 *
	 * @param boostFactor
	 ...
	 * @deprecated As of 12/2004, stemming behavior is controlled by properties set
	 *             in the RepositoryManager config.
	 */
	 ...
	 *            true to enable stemming, false to disable it.
	 * @deprecated As of 12/2004, stemming behavior is controlled by properties set
	 *             in the RepositoryManager config.
	 */
	...
	/**
	 * Gets a Query for the given Lucene query String that is expanded using the
	 * default search fields, boosting fields and virtual search field mappings that
	 * are configured in this RepositoryManager. The Query may then be used to
	 * search the index to find matching results.
	 *
	 * @param queryString
	 *            The query String
	 * @return The Query, expanded
	 * @exception org.apache.lucene.queryParser.ParseException
	 *                If error parsing the query String
	 * @see #getDefaultSearchFields
	 * @see #getFieldsUsedForBoosting
	 * @see #getBoostingValues
	 * @see #getVirtualSearchFieldMapper
	 */
	public Query getExpandedSearchQuery(String queryString) throws org.apache.lucene.queryParser.ParseException {
		return FieldExpansionQueryParser.parse(queryString, getIndex().getAnalyzer(), getDefaultSearchFields(),
				getFieldsUsedForBoosting(), getBoostingValues(), getVirtualSearchFieldMapper(),
				getIndex().getLuceneOperator());
	}

	/**
	 * Gets the names of the fields that are serched for terms that match a users
	 * query. These are defined in the file named search_fields.properties found in
	...
	public String[] getDefaultSearchFields() {
		if (defaultSearchFields == null) {
			...
			try {
				...
				System.out.println("RepositoryManager: No search field config file available at '" + conf
						+ "'. Using the default search field configuration instead.");
			}
	...
	/**
	 * Gets the names of the index fields that are used to boost records that match
	 * a users query. These are defined in the file named search_fields.properties
	 * found in the repository config directory.
	 *
	...
	public String[] getFieldsUsedForBoosting() {
		if (fieldsUsedForBoosting == null) {
			...
			try {
				...
				System.out.println("RepositoryManager: No search boosting config file available at '" + conf
						+ "'. Using the default search boosting settings instead.");
			}
	...
	/**
	 * Gets the boosting values for fields used to boost search results. These are
	 * defined in the file named search_fields.properties found in the repository
	 * config directory.
	 *
	...
	public Map getBoostingValues() {
		...
		if (boostingValues == null) {
			...
			try {
				...
				System.out.println("RepositoryManager: No boosting fields config file available at '" + conf
	...
	/**
	 * Instructs the RepositoryManager to re-read and re-load the values found in
	...
	/**
	 * Gets the SetInfos (directories of files) that are currently configured in the
	 * repository.
	 *
	 * @return The setInfos.
	 * @see SetInfo
	...
	/**
	 * Gets the SetInfos (directories of files) that are currently configured in the
	 * repository as a HashMap keyed by setInfo.directory().
	 *
	...
	/**
	 * Gets a copy of the SetInfos (directories of files) that are currently
	 * configured in the repository. The copy is suitable for modifying without
	 * effecting the data in the repository.
	 *
	...
	/**
	 * Gets the SetInfo (directory of files) at the give index, suitable for reading
	 * but not modifying.
	 *
	 * @param i
	...
	/**
	 * Gets the SetInfo (directory of files) by the given set key, for example
	 * 'dcc'.
	 *
	 * @param key
	 *            The set key, for example 'dcc'
	...
	/**
	 * Adds a new set of files to the repository configuration, but does not index
	 * them.
	 *
	 * @param setInfo
	 *            The SetInfo to add to the reposigory configuration.
	 * @exception Exception
	 *                If one of the directories is already configured or none is
	 *                specified.
	 */
		...
		synchronized (setInfosLock) {
			...
			for (int i = 0; i < dirInfos.size(); i++) {
				...
				if (fileIndexingService.isDirectoryConfigured(new File(dirInfo.getDirectory()))) {
					throw new Exception(
							"This directory is already configured in the repository: " + dirInfo.getDirectory());
				}
			}
	 ...
	 *
	 * @param i
	 *            The index of the SetInfo to remove.
	 * @see SetInfo
	 */
	...
	/**
	 * Removes the cofiguration (SetInfo) for the given set of files from the
	 * repository and deletes the index entries for that set of files.
	 *
	 * @param setSpec
	 *            The setSpec, or collection, to remove for example 'dcc'
	 * @return The removed SetInfo if successful, null if the given set was not
	 *         found
	 */
	...
	/**
	 * Replace a given SetInfo object with a new one, updating the index as
	 * appropriate.
	 *
	 * @param key
	 *            The set key for the set to replace, for example 'dcc'
	 * @param newSetInfo
	 *            The new SetInfo
	 */
	 ...
	 *
	 * @param i
	 *            The index into the array of SetInfos to remove
	 * @param newSetInfo
	 *            The new SetInfo
	 */
	public void replaceSetInfo(int i, SetInfo newSetInfo) {
		...
		if (newSetInfo.getDirInfos().equals(currentSetInfo.getDirInfos())) {
			// prtln("replaceSetInfo() NOT updating the index for the setSpec... only change
			// is name");
			synchronized (setInfosLock) {
				arrayListReplaceItem(i, newSetInfo, Keys.SET_INFOS);
			}
	...
	/**
	 * Resets the configuredSets, configuredFormats, enabledSets, and disabledSets
	 * data structures whenever a change is made to one of them.
	 */
	...
	/**
	 * Gets the ResultDocs for those records that had errors and could not be
	 * indexed, or null if none exist.
	 *
	...
	/**
	 * Gets the ResultDocs for all records in the index, excluding records with
	 * errors, or null if none exist.
	 *
	...
	/**
	 * Gets the total number of OAI status deleted records. Records are set to
	 * status deleted when their source file has been removed (if removeDocs has
	 * been set to false in the RepositoryManager).
	 *
	...
	/**
	 * Gets the total number of records that are not OAI status deleted. Records are
	 * set to status deleted when their source file has been removed (if removeDocs
	 * has been set to false in the RepositoryManager).
	 *
	...
	/**
	 * Gets the total number of all OAI status deleted records that did not come
	 * from any of the existing file directories configured in the
	 * RepositoryManager.
	 *
	...
	/**
	 * Gets the ResultDocs for all OAI status deleted records that did not come from
	 * any of the existing file directories configured in the RepositoryManager.
	 *
	...
	/**
	 * Gets a Query that will return all OAI status deleted records that did not
	 * come from any of the existing file directories configured in the
	 * RepositoryManager.
	 *
	...
	public final Query getDeletedDocsNotFromAnyDirectoryQuery() {
		...
		if (setInfos != null) {
			...
			for (int i = 0; i < setInfos.size(); i++) {
				...
				dirsQ.add(new TermQuery(new Term("docdir", setInfo.getDirectory().trim())),
	...
	/**
	 * Places the Directory into the FileIndexingService, replacing the existing one
	 * if there is one present with same File absolute path.
	 *
	 * @param dirInfo
	 *            The dirInfo.
	 * @param setSpec
	...
	private final void putDirInIndex(DirInfo dirInfo, String setSpec) {
		...
		/*
		 * try { writer = xmlFileIndexingWriterFactory.getIndexingWriter(setSpec,
		 * xmlFormat); } catch (Exception e) { prtlnErr("Unable to add indexer for " +
		 * setSpec + ": " + e); return; }
		 */
		...
		/*
		 * if (plugin != null) writer.setFileIndexingPlugin(plugin); else { plugin =
		 * (FileIndexingPlugin) fileIndexingPlugins.get(PLUGIN_ALL_FORMATS); if (plugin
		 * != null) writer.setFileIndexingPlugin(plugin); }
		 */
		...
		// XMLFileIndexingWriterFactory.indexerClasses.get(xmlFormat);
		// if (writerClass == null) {
		// writerClass = (Class)
		// XMLFileIndexingWriterFactory.indexerClasses.get("default_handler_class");
		...
		// We want to ensure dlese_collect gets indexed first, then dlese_anno, then all
		// others
	...
	/**
	 * Sets the fileIndexingPlugin that will be used during indexing. In order to
	 * take effect, this method MUST be called prior to calling the {@link #init()}
	 * method. An individual plugin may be specified for each XML format and a
	 * global plugin may also be specified for use with all formats. Plugin
	 * precedence is determined as follows: if a plugin has been set for a specific
	 * XML format, use it; else if a plugin has been set for all formats, use it;
	 * else if a plugin has not been set for either, use none.
	 * <p>
	 *
	 * To configure one or more Lucene Analyzers to use on a per-field basis for
	 * your plugins, provide a properties file within your application's class path
	 * named 'FileIndexingPluginLuceneAnalyzers.properties'. See
	 * {@link org.dlese.dpc.index.analysis.PerFieldAnalyzer} for details.
	 *
	 * @param xmlFormat
	 *            The XML format for which this plugin is used or
	 *            {@link #PLUGIN_ALL_FORMATS} to specify that the plugin should be
	 *            used with all formats
	 * @param plugin
	 *            The FileIndexingPlugin that will be used
	...
	/**
	 * Puts a record into the repository, replacing the existing record if one
	 * exists. If the record format is dlese_anno then the annotated record itemID
	 * will be reindexed to include the annotation data.
	 *
	 * @param recordXml
	 *            The record XML as a String
	 * @param xmlFormat
	 *            The XML format of the record, for example 'adn'
	 * @param collection
	 *            The collection vocab for the collection or set, for example 'dcc'
	 * @param id
	 *            A unique identifier for the record, for example
	 *            DLESE-000-000-000-001. This parameter is ignored and may be null
	 *            if the ID can be derived from the record XML.
	 * @param saveXmlAsFile
	 *            True to save the XML record as a file on disc as well as in the
	 *            index, false to only save it in the index
	 * @return The ID of the record as inserted, which may be different than the ID
	 *         requested
	 * @exception RecordUpdateException
	 *                If unable to insert the record for any reason
	 */
	public String putRecord(String recordXml, String xmlFormat, String collection, String id, boolean saveXmlAsFile)
	 ...
	 *            A plugin to use while indexing, or null to specify none. This
	 *            plugin takes precedence over a previously configured plugin. If
	 *            none is supplied here, the previously configured plugin will be
	 *            used if available.
	 * @param saveXmlAsFile
	 *            True to save the XML record as a file on disc as well as in the
	 *            index, false to only save it in the index
	 ...
	 *                If unable to insert the record for any reason
	 */
	public String putRecord(String recordXml, String xmlFormat, String collection, String id,
			FileIndexingPlugin indexingPlugin, boolean saveXmlAsFile) throws RecordUpdateException {
	 ...
	 *            True to index the records that this item assigns a relation for
	 * @return The ID of the record as inserted, which may be different than the ID
	 *         requested
	 * @exception RecordUpdateException
	 *                If unable to insert the record for any reason
	 */
	private String doPutRecord(String recordXml, String xmlFormat, String collection, String id,
			FileIndexingPlugin indexingPlugin, boolean saveXmlAsFile, boolean indexRelations)
			throws RecordUpdateException {
		synchronized (publicUpdateApiLock) {
			...
			if (collectionSetInfo == null)
				throw new RecordUpdateException("Collection '" + collection
						+ "' is not configured in this repository. The collection must exist before records can be put.");
			if (!collectionSetInfo.getFormat().equals(xmlFormat))
				throw new RecordUpdateException("Format '" + xmlFormat + "' was specified, however collection '"
						+ collection + "' is configured for format '" + collectionSetInfo.getFormat() + "'");
			...
			if (!fileIndexingService.isDirectoryConfigured(metadataDir))
				throw new RecordUpdateException("The directory '" + metadataDir.getAbsolutePath()
						+ "' is not configured in the FileIndexingService. '");
			...
			try {
				XMLFileIndexingWriterFactory xFactory = new XMLFileIndexingWriterFactory(getIndex(),
				...
				prtln("putRecord() verifying record id: "
						+ (newRecordId == null ? "[ID not yet available]" : newRecordId));
				...
				// Grab the ID from the writer. If the ID could not be derived from the XML it
				// will be 'temp_dds_put_record_file'
				newRecordId = writer.getPrimaryId();
				if (newRecordId == null)
					newRecordId = "";

				// If the ID could not be derived from the XML, make sure that one was supplied
				// or throw an exception
				if (newRecordId.equals("temp_dlese_repository_put_record_file")) {
					if (id == null || id.trim().length() == 0)
						throw new RecordUpdateException(
								"ID cannot be determined from the record XML and id param is null");
					newRecordId = id;
				}

				prtln("putRecord() saving record id: " + newRecordId);

				// For refactor, use idvalue instead here, in deleteRecord and elsewhere where
				// IDs are used. Some issues...
				// ResultDoc[] results = index.searchDocs(new TermQuery(new Term("idvalue",
				// newRecordId)));
				...
				if (results != null && results.size() > 0) {
					...
					if (!curCollection.equals(collection))
						throw new RecordUpdateException("Record ID '" + newRecordId
								+ "' already exists in the repository, but is in collection '" + curCollection + "'");
				}
				...
				if (!tempFile.renameTo(writeToFile) && !Files.copy(tempFile, writeToFile))
					throw new RecordUpdateException("Unable to create new file at location '"
							+ writeToFile.getAbsolutePath() + ".' Permissions may not allow writing files.");

				fileIndexingService.indexFile(writeToFile, indexingPlugin);

				// Handle reindexing records that this item assigns a relationship for and the
				// records related to this:
				if (indexRelations) {

					// Reindex myself to get the relations related to me (JW - Is this necessary?
					// there may be another way... pull in the relations in the reader using ID?
					// Check if there are relations to me before reindexing...)
					...
					if (idsToReindex != null) {
						for (int i = 0; i < idsToReindex.size(); i++) {
							...
							try {
								...
								prtlnErr("putRecord() successfully saved record '" + newRecordId
										+ "' but unable to (re)index the related resource '" + reindexId + "': " + t);
							}
						}
					}
					...
					if (urlsToReindex != null && urlsToReindex.size() > 0) {
						...
						for (int j = 0; j < urlsToReindex.size(); j++) {
							urlQ.add(new TermQuery(new Term("url", (String) urlsToReindex.get(j))),
				...
				if (!saveXmlAsFile) {
					// Delete the file, but save the directories so FileIndexingService keeps the
					// records...
					if (writeToFile != null)
						writeToFile.delete();

					/*
					 * File formatDir = metadataDir.getParentFile(); try { metadataDir.delete(); }
					 * catch (Throwable t) { prtlnErr("Unable to delete '" + metadataDir + "': " +
					 * t); } try { formatDir.delete(); } catch (Throwable t) {
	...
	/**
	 * Deletes a record from the repository if one exists with the given ID. Returns
	 * false if no such record exists, else true if the deletion was successful.
	 * Reindexes all records that were related to the item, such as annotations, to
	 * remove related fields from the index.
	 *
	 * @param id
	 *            The unique identifier for the record, for example
	 *            DLESE-000-000-000-001.
	 * @return True if the deletion was successful, false if no such record exists.
	 * @exception Exception
	 *                If unable to delete the record for any reason
	 */
	public boolean deleteRecord(String id) throws Exception {
		synchronized (publicUpdateApiLock) {
			...
			// Determine if we need to re-index any related documents to remove data from
			// the index:
			...
			/*
			 * String xmlFormat = xmlDocReader.getDoctype(); String relatedRecordId = null;
			 * boolean saveXmlAsFile = true; if (xmlFormat.equals("dlese_anno")) {
			...
			for (int i = 0; i < results.size(); i++) {
				...
				if (delFile.exists()) {
					delFile.delete();
					if (delFile.exists())
						throw new Exception("Unable to delete file " + delFile.getAbsolutePath()
								+ " from disk. Most likely cause is access denied.");
				}
			}
			...
			// Re-index the related doc (if this is an annotation) to remove related data
			// from the index:
			/*
			 * if (relatedRecordId != null) { try { reindexRecord(relatedRecordId, null,
			 * saveXmlAsFile, false); } catch (RecordUpdateException t) { // Related record
			 * ID does not exist (probably OK...) } catch (Throwable t) {
			 * prtlnErr("deleteRecord() successfully deleted record '" + id +
			 * "' but unable to (re)index the annotated resource '" + relatedRecordId +
			 * "': " + t); } }
			 */
	...
	/**
	 * Reindexes an existing record in the repository, updating any related data
	 * from outside of the record XML such as annotations or data from a
	 * FileIndexingPlugin, replacing the existing index entry. This is equivilent to
	 * calling #getRecord to fetch the record XML and then calling #putRecord with
	 * the same XML.
	 *
	 * @param id
	 *            A unique identifier for the record, for example
	 ...
	 * @exception RecordUpdateException
	 *                If record ID does not exist or unable to reindex the record
	 *                for any reason
	 */
	public String reindexRecord(String id, FileIndexingPlugin indexingPlugin, boolean saveXmlAsFile,
			boolean indexRelations) throws RecordUpdateException {
		synchronized (publicUpdateApiLock) {
			ResultDoc resultDoc = getRecord(id);
			if (resultDoc == null)
				throw new RecordUpdateException(
						"Unable to reindex record '" + id + "'. Record not found in the repository.");
	...
	/**
	 * Instructs the repository to index all of it's files. The index will add,
	 * update or delete the entries for each file in the repository. The indexAll
	 * parameter indicates whether to index all files regardless of modification or
	 * only those files that have been modified, added or deleted since the last
	 * index update. This method returns immediately and execution occurs in a
	 * background thread. Progress may be monitored using method
	 * {@link #getIndexingMessages}.
	 *
	 * @param observer
	 *            The FileIndexingObserver that will be notified when indexing is
	 *            complete, or null to use none
	 * @param indexAll
	 *            True to index all files, false to index modified files only
	 */
	...
	/**
	 * Instructs the repository to index all of the files for the given
	 * set/collection. The index will add, update or delete the entries for each
	 * file in the collection. The indexAll parameter indicates whether to index all
	 * files regardless of modification or only those files that have been modified,
	 * added or deleted since the last index update. This method returns immediately
	 * and execution occurs in a background thread. Progress may be monitored using
	 * method {@link #getIndexingMessages}.
	 *
	 * @param set
	 *            The set key, for example 'dcc'
	 * @param observer
	...
	public final boolean indexCollection(String set, FileIndexingObserver observer, boolean indexAll) {
		...
		if (si == null) {
			try {
				if (observer != null)
					observer.indexingCompleted(FileIndexingObserver.INDEXING_COMPLETED_ERROR,
							"Collection '" + set + "' is not configured in the repository.");
			} catch (Throwable t) {
			}
			return false;
		}
	...
	/**
	 * Sets the absolute path to the collectionRecordsLocation and the
	 * metadataRecordsLocation. If supplied, the RepositoryManager will index the
	 * Collecton Records and they can then be used to configure the collections
	 * found in this repository. This must be called prior to calling the init()
	 * method.
	 *
	 * @param collectionRecordsLocation
	 *            The absolute path to a directory of DLESE collection-level XML
	 *            records.
	 * @param metadataRecordsLocation
	 *            The absolute path to a directory containing item-level metadata.
	 *            All metadata files must reside in sub-directores by format and
	 *            collection, for example: metadataRecordsLocation +
	...
	public final void setRecordsLocation(String collectionRecordsLocation, String metadataRecordsLocation) {
		...
		// prtlnErr("Warning. collectionRecordsDir " + collectionRecordsLocation + " is
		// not a valid directory.");
		this.metadataRecordsLocation = metadataRecordsLocation;
	}

	/**
	 * Gets the path for the directory of collect-level records this
	 * RepositoryManager is using, or empty string if none is configured.
	 *
	...
	/**
	 * Gets the path for the directory of metadata records this RepositoryManager is
	...
	/**
	 * Put a collection in the repository. The collection can then have items added
	 * to it with the putRecord method.
	 *
	 * @param collectionKey
	 *            The unique collection key, for example 'dcc'. The key and record
	 ...
	 * @exception PutCollectionException
	 *                If error occurs, indicates the type of error
	 */
	public String putCollection(String collectionKey, String xmlFormat, String title, String description,
			String additionalMetadata) throws PutCollectionException {
		synchronized (publicUpdateApiLock) {
			// To do: Another putCollection() method that takes a collection records - see
			// DDSServlet
			if (collectionKey == null)
				throw new PutCollectionException("collectionKey cannot be null",
						PutCollectionException.ERROR_CODE_BAD_KEY);
			if (xmlFormat == null)
				throw new PutCollectionException("xmlFormat cannot be null",
						PutCollectionException.ERROR_CODE_BAD_FORMAT_SPECIFIER);
			if (title == null)
			...
			if (!collectionKey.matches("[a-zA-Z0-9_\\-\\.]+"))
				throw new PutCollectionException(
						"collectionKey must match [a-zA-Z0-9_\\\\-\\\\.]+ but found: " + collectionKey,
						PutCollectionException.ERROR_CODE_BAD_KEY);
			if (!xmlFormat.matches("[a-zA-Z0-9_\\-\\.]+"))
				throw new PutCollectionException("xmlFormat must match [a-zA-Z0-9_\\\\-\\\\.]+ but found: " + xmlFormat,
						PutCollectionException.ERROR_CODE_BAD_TITLE);
			...
			try {
				if (!isSetConfigured("collect")) {
					...
					org.dom4j.Document masterCollectionDoc = Dom4jUtils.getXmlDocument(
							Files.readFileFromJarClasspath("/org/dlese/dpc/repository/COLLECTION-RECORD-TEMPLATE.xml")
									.toString());

					masterCollectionDoc = updateCollectionRecord(masterCollectionDoc,
							...
							null, "dlese_collect", "collect", "COLLECTIONS-COLLECTION-001", new Date());

					String masterCollection = XMLUtils
							.stripXmlDeclaration(new BufferedReader(new StringReader(masterCollectionDoc.asXML())))
							.toString();

					collectionRecordsDir.mkdirs();
					try {
						Files.writeFile(masterCollection,
								new File(collectionRecordsDir, "COLLECTIONS-COLLECTION-001.xml"));
					} catch (Exception e) {
						throw new PutCollectionException("Failed to write file, possible permission problem: " + e,
								PutCollectionException.ERROR_CODE_IO_ERROR);
					}
			...
			try {
				...
				// If we have an existing collection with this key, update the existing XML
				// record...
				SetInfo existingSet = getSetInfo(collectionKey);
				if (results.size() > 0 && existingSet != null) {
					DleseCollectionDocReader collDoc = (DleseCollectionDocReader) results.get(0).getDocReader();

					// Dissallow changing the xmlFormat:
					if (!collDoc.getFormatOfRecords().equals(xmlFormat)) {
						throw new PutCollectionException(
								"Collection '" + collectionKey + "' already exists with xmlFormat '"
										+ collDoc.getFormatOfRecords() + "'. Cannot change the xmlFormat to '"
										+ xmlFormat + ".' Changing the xmlFormat is not allowed.",
								PutCollectionException.ERROR_CODE_COLLECTION_EXISTS_IN_ANOTHER_FORMAT);
					}
					...
					xmlDoc = updateCollectionRecord(xmlDoc, title, description, additionalMetadata, "DDS_UNCHANGED",
							"DDS_UNCHANGED", "DDS_UNCHANGED", null);

					collId = xmlDoc.valueOf(
							"/*[local-name()='collectionRecord']/*[local-name()='metaMetadata']/*[local-name()='catalogEntries']/*[local-name()='catalog']/@entry");

					collRecordXml = XMLUtils.stripXmlDeclaration(new BufferedReader(new StringReader(xmlDoc.asXML())))
							.toString();
				...
				// If no previous Collection document exists, create a new one from template:
					...
					org.dom4j.Document xmlDoc = Dom4jUtils.getXmlDocument(
							Files.readFileFromJarClasspath("/org/dlese/dpc/repository/COLLECTION-RECORD-TEMPLATE.xml")
									.toString());

					xmlDoc = updateCollectionRecord(xmlDoc, title, description, additionalMetadata, xmlFormat,
							collectionKey, collId, accessionDate);
					...
					SetInfo set = new SetInfo(title, collectionKey, description, isEnabled, dir, xmlFormat, collId);
				...
				prtlnErr("putCollection() error for collection: '" + title + "' collectionKey:" + collectionKey
						+ " xmlFormat:" + xmlFormat + e);
				...
				try {
					...
					throw new PutCollectionException("There was an exception creating the collection: " + e.toString()
							+ " plus an addition exception when trying to delete the temporary collection record: "
							+ e2.toString(), PutCollectionException.ERROR_CODE_INTERNAL_ERROR);
				}
	 ...
	 *
	 * @param collectionRecordDoc
	 *            A collection record Document (may be from the template)
	 * @param title
	 *            Title (must not be null)
	 * @param description
	 *            Description (can be null)
	 * @param additionalMetadata
	 *            Additional metadata (String, XML, or null)
	 * @param xmlFormat
	 *            Format specifier or "DDS_UNCHANGED" to leave unchanged
	 * @param key
	 *            Collection key or "DDS_UNCHANGED" to leave unchanged
	 * @param id
	 *            Record ID or "DDS_UNCHANGED" to leave unchanged
	 * @param accessionDate
	 *            Accession date or null to leave unchanged
	 * @return An updated Document
	 * @exception Exception
	 *                If error
	 */
	private org.dom4j.Document updateCollectionRecord(org.dom4j.Document collectionRecordDoc, String title,
			String description, String additionalMetadata, String xmlFormat, String key, String id, Date accessionDate)
			throws Exception {

		if (!key.equals("DDS_UNCHANGED"))
			collectionRecordDoc
					.selectSingleNode(
							"/*[local-name()='collectionRecord']/*[local-name()='access']/*[local-name()='key']")
					.setText(key);

		if (!xmlFormat.equals("DDS_UNCHANGED"))
			collectionRecordDoc.selectSingleNode(
					"/*[local-name()='collectionRecord']/*[local-name()='access']/*[local-name()='key']/@libraryFormat")
					.setText(xmlFormat);
		...
		if (accessionDate != null)
			collectionRecordDoc.selectSingleNode(
					"/*[local-name()='collectionRecord']/*[local-name()='approval']/*[local-name()='collectionStatuses']/*[local-name()='collectionStatus']/@date")
					.setText(new SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss'Z'").format(accessionDate));

		// Update the short title:
		Node shortTitleNode = collectionRecordDoc.selectSingleNode(
				"/*[local-name()='collectionRecord']/*[local-name()='general']/*[local-name()='shortTitle']");
		...
		Node fullTitleNode = collectionRecordDoc.selectSingleNode(
				"/*[local-name()='collectionRecord']/*[local-name()='general']/*[local-name()='fullTitle']");
		if (fullTitleNode != null)
			fullTitleNode.detach();

		Node descriptionNode = collectionRecordDoc.selectSingleNode(
				"/*[local-name()='collectionRecord']/*[local-name()='general']/*[local-name()='description']");
		if (descriptionNode == null)
			descriptionNode = ((Branch) collectionRecordDoc
					.selectSingleNode("/*[local-name()='collectionRecord']/*[local-name()='general']"))
		...
		Node additionalMetadataNode = collectionRecordDoc
				.selectSingleNode("/*[local-name()='collectionRecord']/*[local-name()='additionalMetadata']");
		...
		if (additionalMetadata != null && additionalMetadata.trim().length() > 0) {
			...
			try {
				additionalMetadataDocument = Dom4jUtils
						.getXmlDocument("<additionalMetadata>" + additionalMetadata + "</additionalMetadata>");
			} catch (Throwable t) {
				throw new PutCollectionException("Error processing additionalMetadata argument: " + t.getMessage(),
						PutCollectionException.ERROR_CODE_BAD_ADDITIONAL_METADATA);
			}
			((Branch) collectionRecordDoc.selectSingleNode("/*[local-name()='collectionRecord']"))
					.add(additionalMetadataDocument.getRootElement());
		}
	 ...
	 *
	 * @param collectionKey
	 *            The unique collection key, for example 'dcc'
	 * @return True if the collection existed and was deleted, false if no such
	 *         collection exists
	 * @exception Exception
	 *                Description of the Exception
	 */
	public boolean deleteCollection(String collectionKey) throws Exception {
		synchronized (publicUpdateApiLock) {
			...
			if (filesDirectory.isDirectory() && filesDirectory.listFiles() != null
					&& filesDirectory.listFiles().length > 0 && !filesDirectory.canWrite())
			...
			try {
				...
				if (filesDirectory.listFiles() != null && filesDirectory.listFiles().length > 0)
					deleteException = new Exception(
							"Collection was removed from the index and repository, however there was a error deleting files from disk. "
									+ filesDirectory.listFiles().length + " files still reside in directory "
									+ filesDirectory.getAbsolutePath() + " - most likely cause is access denied.");
			} catch (Throwable t) {
				deleteException = new Exception(
						"Collection was removed from the index and repository, however there was a problem deleting one or more of the record files from disk: "
								+ t.getMessage());
			}
	 ...
	 *
	 * @param indexCollectionRecs
	 *            True to index the collection records, false not to.
	 */
	public void loadCollectionRecords(boolean indexCollectionRecs) {
		...
		// prtln("loadColletctionRecords() starting... indexCollectionRecs:" +
		// indexCollectionRecs);

		// prtln("\nloadCollectionRecords() loading dir " +
		// collectionRecordsDir.getAbsolutePath() + "\n");
		...
		for (int i = 0; i < colls.size(); i++) {
			...
			try {
				name = ((Element) (doc.selectNodes("//*[local-name()='general']/*[local-name()='shortTitle']").get(0)))
						.getText();
				setSpec = ((Element) (doc.selectNodes("//*[local-name()='access']/*[local-name()='key']").get(0)))
						.getText();
				id = ((Attribute) (doc.selectNodes(
						"//*[local-name()='metaMetadata']/*[local-name()='catalogEntries']/*[local-name()='catalog']/@entry")
						.get(0))).getText();
				format = ((Attribute) (doc
						.selectNodes("//*[local-name()='access']/*[local-name()='key']/@libraryFormat").get(0)))
								.getValue();
				...
				newSetInfo = new SetInfo(name, setSpec, "", enabled, dir, format, id);
	...
	/**
	 * Recursively grab all the collection level records in the given directory and
	 * sub-directories.
	 *
	 * @param collRecords
	 *            An empty or partially filled ArrayList of collection record XML
	 *            Documents.
	 * @param dir
	 *            A directory that contains collection-level records.
	 * @return An ArrayList of dom4j XML Documents, one for each collection-level
	 *         record.
	 */
	private ArrayList getCollectionRecords(ArrayList collRecords, File dir) {
		...
		if (dir.isDirectory()) {
			...
			/*
			 * if(!dir.exists()) prtlnErr("getCollectionRecords(): Directory '" + dir +
			 * "' does not exist"); else if(dir.isFile())
			 * prtlnErr("getCollectionRecords(): Directory '" + dir +
	 ...
	 *
	 * @param i
	 *            The index of the e-mail to get
	 * @return The adminEmails value
	 */
	...
	/**
	 * Add the given admin e-mail to the repository. The admin e-mail is used in the
	 * Identify request. If the given e-mail already exists, nothing is changed.
	 *
	 * @param email
	 *            The admin e-mail to be added.
	 */
	...
	/**
	 * Removes the given admin e-mail from the repository. The admin e-mail is used
	 * in the Identify request. If no such e-mail exists, nothing is done.
	 *
	 * @param email
	 *            The admin e-mail to be removed.
	 */
	...
	/**
	 * Removes the given admin e-mail at the given ArrayList index from the
	 * repository. If no such e-mail exists, nothing is done.
	 *
	 * @param i
	 *            The admin e-mail ArrayList index to be removed.
	 */
	...
	/**
	 * Sets the metadata namespace for the given metadataPrefix (XML format). If a
	 * metadata namespace is already configured it will be replaced with the new
	 * value. These are used in the OAI ListMetadataFormats response.
	 *
	 * @param metadataPrefix
	 *            The metadata specifier, for example oai_dc, adn, dlese_anno
	 * @param metadataNamespace
	 *            The namspace URI for the given XML format
	...
	/**
	 * Gets the metadata namespace for the given metadataPrefix (XML format). These
	 * are used in the OAI ListMetadataFormats response.
	 *
	 * @param metadataPrefix
	 *            The metadata specifier, for example oai_dc, adn, dlese_anno
	 ...
	 * These are used in the OAI ListMetadataFormats response.
	 *
	 * @param metadataPrefix
	 *            The metadata specifier, for example oai_dc, adn, dlese_anno
	 * @return True if successful
	...
	/**
	 * Gets a Map of metadata namespaces for all metadataPrefixes (XML formats) in
	 * the repository, or null if none exist. These are used in the OAI
	...
	/**
	 * Sets the metadata schema URL for the given metadataPrefix (XML format). If a
	 * schema URL is already configured it will be replaced with the new value.
	 * These are used in the OAI ListMetadataFormats response.
	 *
	 * @param metadataPrefix
	 *            The metadata specifier, for example oai_dc, adn, dlese_anno
	 * @param metadataSchema
	 *            The schema URL for the given XML format
	 * @return True if successful
	 */
	...
	/**
	 * Gets the metadata schema URL for the given metadataPrefix (XML format). These
	 * are used in the OAI ListMetadataFormats response.
	 *
	 * @param metadataPrefix
	 *            The metadata specifier, for example oai_dc, adn, dlese_anno
	...
	/**
	 * Gets a Map of metadata schema URLs for all metadataPrefixes (XML formats) in
	 * the repository, or null if none exist. These are used in the OAI
	 * ListMetadataFormats response.
	 *
	...
	/**
	 * Sets the default schema and namespace for the given XML format, used in the
	 * OAI ListMetadataFormats response. Value passed in must be of the form
	 * [metadataPrefix]|[schema]|[namespace], for example
	 * 'oai_dc|http://www.openarchives.org/OAI/2.0/oai_dc.xsd|http://www.openarchives.org/OAI/2.0/oai_dc/'.
	 * If a namespace or schema value is already configured for the given
	 * metadataPrefix, no change will be made. Values may be changed later using
	 * #setMetadataSchemaURL and #setMetadataNamespace.
	 *
	 * @param info
	 *            String of the form [metadataPrefix]|[schema]|[namespace]
	 * @exception Exception
	 *                If unable to parse the String properly
	 */
	public void setDefaultXmlFormatInfo(String info) throws Exception {
		...
		// Set the default schema and namespace for the given metadataPrefix, only if a
	...
	/**
	 * Sets the XMLConversionService used by this RepositoryManager and puts it into
	 * the index as an attribute under the key xmlConversionService for use in the
	 * XMLDocReaders.
	 *
	 * @param cs
	 *            The new XMLConversionService.
	 */
	...
	/**
	 * Gets the XMLConversionService used by this RepositoryManager, or null if none
	 * available.
	 *
	...
	/**
	 * Get the directory where the repository persistent data and certain configs
	 * resides, including the collections configs, specified by the init param
	 * repositoryData.
	 *
	...
	/**
	 * Gets the OAI sets configuration XML for this repository, used to generate the
	 * ListSets OAI response. See
	 * <a href="../../../../javadoc-includes/ListSets-config-sample.xml">sample
	 * ListSets XML config file</a> .
	 *
	...
	/**
	 * Loads the OAI sets configuration from file and configures the corresponding
	 * set mappings in this repository. See
	 * <a href="../../../../javadoc-includes/ListSets-config-sample.xml">sample
	 * ListSets XML config file</a> . If not file exists, nothing is done.
	 *
	 * @exception Exception
	 *                If error parsing or loading the config
	 */
	...
	/**
	 * Sets the OAI sets configuration for this repository and saves the XML to
	 * file, replacing all previous definitions. See
	 * <a href="../../../../javadoc-includes/ListSets-config-sample.xml">sample
	 * ListSets XML config file</a> . The virtual field 'setSpec' is used to define
	 * the OAI sets in the repository.
	 *
	 * @param xml
	 *            The new listSetsConfigXml
	 * @exception Exception
	 *                If unable to parse
	 */
	private void setListSetsConfigXml(String xml) throws Exception {
		// If there is an Exception when parsing, throw it without modifying the current
		// settings
		VirtualSearchFieldMapper tmpVirtualSearchFieldMapper = new VirtualSearchFieldMapper(
				getIndex().getQueryParser());
	...
	/**
	 * Sets the definition for a given OAI set, initializing it in the repository
	 * and writing the ListSets config XML to persistent file. If the given set
	 * already exists, it will be re-defined, if not, a new set will be added to the
	 * existsing sets in the config XML file.
	 *
	 * @param setDefinitionsForm
	 *            A bean holding the set definiton info
	 * @exception Exception
	 *                If error
	 */
	...
	/**
	 * Gets the OAI sets associated with this record ID as a List of Strings, for
	 * example 'mySet' or null.
	 *
	 * @param id
	 *            The record ID (without OAI prefix)
	...
	/**
	 * Gets the number of records that are in the given OAI set excluding deletions,
	 * or -1 if no such set is configured.
	 *
	 * @param setSpec
	 *            The OAI setSpec
	 * @return The number of non-deleted records in the set or -1 if the given set
	 *         does not exist
	...
	/**
	 * Gets the number of deleted records that are in the given OAI, or -1 if no
	 * such set is configured.
	 *
	 * @param setSpec
	 *            The OAI setSpec
	 * @return The number of deleted records in the set or -1 if the given set does
	 *         not exist
	...
	/**
	 * Gets the VirtualSearchFieldMapper that defines the virtual field/term
	 * definitions for this RepositoryManager. Dev note: may want to add RM methods
	 * to update/add stuff to the VirtualSearchFieldMapper, but for now this method
	 * can be used to get and then modify it.
	 *
	...
	private class FileIndexingTask extends TimerTask {
		...
		public void run() {
			...
			// prtln("RepositoryManager.FileIndexingTask() just before
			...
			if (fileIndexingService != null) {
				...
				if (debug)
					observer = new SimpleFileIndexingObserver("Background indexing timer task",
							"Beginning to index files...");
	...
	private final boolean arrayListAddItem(Object o, String key) {
		...
		try {
			...
			// prtlnErr("Error adding item \"" + o + "\" to ArrayList \"" + key + ":\" " +
			// e);
			return false;
		}
	...
	private final Object arrayListGetItem(int i, String key, boolean asCopy) {
		...
		try {
			...
			// prtlnErr("Error getting index \"" + i + "\" from ArrayList \"" + key + ":\" "
			// + e);
		}
		return null;
	}
	...
	private final boolean arrayListRemoveItem(Object o, String key) {
		...
		try {
			...
			// prtlnErr("Error removing item \"" + o + "\" from ArrayList \"" + key + ":\" "
			// + e);
			return false;
		}
		return true;
	...
	private final boolean arrayListRemoveItem(int i, String key) {
		...
		try {
			...
			// prtlnErr("Error removing index \"" + i + "\" from ArrayList \"" + key + ":\"
			// " + e);
			return false;
		}
		return true;
	...
	private final boolean arrayListReplaceItem(int index, Object o, String key) {
		...
		try {
			...
			// prtlnErr("Error removing index \"" + index + "\" from ArrayList \"" + key +
			// ":\" " + e);
			return false;
		}
		return true;
	...
	private final boolean hashMapPutItem(String key, Object o, String hashMapPersistanceKey) {
		...
		try {
			...
			// prtlnErr("Error setting item \"" + key + "\" to HashMap \"" +
			// hashMapPersistanceKey + ":\" " + e);
			return false;
		}
	...
	private final Object hashMapGetItem(String key, String hashMapPersistanceKey, boolean asCopy) {
		...
		try {
			...
			// prtlnErr("Error getting item \"" + key + "\" from HashMap \"" +
			// hashMapPersistanceKey + ":\" " + e);
		}
		return null;
	}
	...
	private final boolean hashMapRemoveItem(String key, String hashMapPersistanceKey) {
		...
		try {
			...
			// prtlnErr("Error removing item \"" + key + "\" from ArrayList \"" +
			// hashMapPersistanceKey + ":\" " + e);
			return false;
		}
		return true;
	 ...
	 *
	 * @param s
	 *            The text that will be output to error out.
	 */
