...
/**
 * Implementation of <strong>Action</strong> that handles all OAI-PMH and ODL
 * requests.
 *
...
public final class RepositoryAction extends Action {
	...
	/**
	 * Processes the OAI-PMH request and creates the corresponding HTTP response by
	 * forwarding to a JSP page for rendering.
	 *
	 * @param mapping
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			// A count of the number of args that aren't "rt" or "verb" for further
			// validation
			String arg = null;
			String param = null;
			...
			/*
			 * prtln("Params: "); while (params.hasMoreElements()) { arg = ((String)
			 * params.nextElement()); prtln(arg + "=" + request.getParameter(arg)); if
			 * (!arg.equals("rt") && !arg.equals(OAIArgs.VERB)) num_args++; }
			 */
			RepositoryManager rm = (RepositoryManager) servlet.getServletContext().getAttribute("repositoryManager");

			webLogIndex = (SimpleLuceneIndex) servlet.getServletContext().getAttribute("webLogIndex");
			...
			if (verbs.length > 1) {
				rf.addOaiError(OAICodes.BAD_VERB,
						"Only one verb argument must be present, but " + verbs.length + " were found.");
				return (mapping.findForward("oaipmh.error"));
			}
			...
			// Handle the OAI-PMH requests:
				...
				return doListRecordsOrIdentifiers(request, mapping, num_args, rm, rf, OAIArgs.LIST_RECORDS,
						numListRecordsResults);
			} else if (verb.equals(OAIArgs.LIST_IDENTIFIERS)) {
				return doListRecordsOrIdentifiers(request, mapping, num_args, rm, rf, OAIArgs.LIST_IDENTIFIERS,
						numListIdentifiersResults);
	...
	/**
	 * Handles the OAI Identify request. From the spec, this request has the
	 ...
	 *            The HTTP request.
	 * @param mapping
	 *            The Struts ActionMapping used for forwarding.
	 * @param num_args
	 *            The number of arguments other than the OAI verb or rt.
	 * @param rm
	 *            The model used to fetch OAI data.
	 * @param rf
	 *            The bean used to handle the response.
	 * @return The page to forward to that will render the response, or null if
	 *         handled here.
	 */
	protected ActionForward doIdentify(HttpServletRequest request, ActionMapping mapping, int num_args,
			RepositoryManager rm, RepositoryForm rf) {

		if (num_args > 0) {
			rf.addOaiError(OAICodes.BAD_ARGUMENT,
					"There must be no arguments included with the '" + OAIArgs.IDENTIFY + "' request");
		}
	...
	/**
	 * Handles the OAI GetRecord request. From the spec, this request has the
	 * following properties:
	 * <p>
	 *
	 * Arguments:<br>
	 * identifier - a required argument that specifies the unique identifier of the
	 * item in the repository from which the record must be disseminated.<br>
	 * metadataPrefix - a required argument that specifies the metadataPrefix of the
	 ...
	 * metadataPrefix can be disseminated from the item identified by the value of
	 * the identifier argument. The metadata formats supported by a repository and
	 * for a particular record can be retrieved using the ListMetadataFormats
	 * request.
	 * <p>
	 *
	 ...
	 * supported by the item identified by the value of the identifier argument <br>
	 ...
	 *         handled here.
	 */
	protected ActionForward doGetRecord(HttpServletRequest request, ActionMapping mapping, int num_args,
			RepositoryManager rm, RepositoryForm rf) {
		...
		if (identifier == null)
			rf.addOaiError(OAICodes.BAD_ARGUMENT, "GetRecord must include the argument '" + OAIArgs.IDENTIFIER + "'");
		...
		if (identifier != null && format != null && num_args > 2)
			rf.addOaiError(OAICodes.BAD_ARGUMENT,
					"The " + OAIArgs.GET_RECORD + " request must only include the arguments '" + OAIArgs.IDENTIFIER
							+ "' and '" + OAIArgs.METADATA_PREFIX + "'");
		...
		if (identifier != null && identifier.length() > 0 && format != null) {
			...
			if (isMySetDisabled) {
				rf.addOaiError(OAICodes.ID_DOES_NOT_EXIST,
						"The identifier '" + identifier + "' does not exist in this repository.");
			} else {
				if (!statusDeleted && (content == null || content.trim().length() == 0)) {
					if (!rm.isIdInRepository(idStripped))
						...
						rf.addOaiError(OAICodes.CANNOT_DISSEMINATE_FORMAT,
								"The format '" + format + "' is not available for identifier '" + identifier + "'");
				}
	...
	/**
	 * Handles the OAI ListMetadataFormats request. From the spec, this request has
	 * the following properties:
	 * <p>
	 *
	 * Arguments:<br>
	 * identifier - an optional argument that specifies the unique identifier of the
	 * item for which available metadata formats are being requested. If this
	 ...
	 * supported by a repository does not mean that it can be disseminated from all
	 ...
	 *         handled here.
	 */
	protected ActionForward doListMetadataFormats(HttpServletRequest request, ActionMapping mapping, int num_args,
			RepositoryManager rm, RepositoryForm rf) {

		if (request.getParameter(OAIArgs.IDENTIFIER) == null && num_args >= 1)
			rf.addOaiError(OAICodes.BAD_ARGUMENT, "The " + OAIArgs.LIST_METADATA_FORMATS
					+ " request may only include the optional argument '" + OAIArgs.IDENTIFIER + "'");
		...
		if (identifier != null) {
			...
			if (resultDocs == null || resultDocs.size() == 0)
				rf.addOaiError(OAICodes.ID_DOES_NOT_EXIST,
						"The identifier '" + originalIdentifier + "' does not exist.");
			else
	...
	/**
	 * Handles the OAI ListRecords or ListIdentifiers requests. From the spec, these
	 * requests have the following properties:
	 * <p>
	 *
	 * Arguments:<br>
	 * from - an optional argument with a UTCdatetime value , which specifies a
	 * lower bound for datestamp-based selective harvesting.<br>
	 * until - an optional argument with a UTCdatetime value , which specifies a
	 * upper bound for datestamp-based selective harvesting.<br>
	 * metadataPrefix - a required argument, which specifies that headers should be
	 * returned only if the metadata format matching the supplied metadataPrefix is
	 * available or, depending on the repository's support for deletions, has been
	 * deleted. The metadata formats supported by a repository and for a particular
	 * item can be retrieved using the ListMetadataFormats request.<br>
	 * set - an optional argument with a setSpec value , which specifies set
	 * criteria for selective harvesting. <br>
	 * resumptionToken - an exclusive argument with a value that is the flow control
	 * token returned by a previous ListIdentifiers request that issued an
	 * incomplete list.
	 * <p>
	 *
	 * Error and Exception Conditions:<br>
	 ...
	 *            The bean used to handle the response.
	 * @param numResults
	 *            The number of results to provide in the response before returning
	 *            a resumptionToken.
	 * @param requestType
	 *            The requestType, which must be either ListIdentifiers or
	 *            ListRecords.
	 * @return The page to forward to that will render the response, or null if
	 *         handled here.
	 */
	protected ActionForward doListRecordsOrIdentifiers(HttpServletRequest request, ActionMapping mapping, int num_args,
			RepositoryManager rm, RepositoryForm rf, String requestType, int numResults) {
		...
		if (resumptionToken != null && (set != null || format != null || from != null || until != null)) {
			rf.addOaiError(OAICodes.BAD_ARGUMENT,
					"The argument '" + OAIArgs.RESUMPTION_TOKEN + "' must be supplied without other arguments");
		} else if (format == null && resumptionToken == null)
			rf.addOaiError(OAICodes.BAD_ARGUMENT, "Missing argument. " + requestType + " must include the argument '"
		...
		if (set != null && set.startsWith("dleseodlsearch")) {
			...
			try {
				...
				rf.addOaiError(OAICodes.BAD_ARGUMENT, "Error in the ODL parameter: " + e.getMessage()
			...
			if (!rf.hasErrors() && odl.getNumResultsToReturn() > Integer.parseInt(rm.getNumRecordsResults())) {
				rf.addOaiError(OAICodes.BAD_ARGUMENT,
						"Error in the ODL parameter: the maximum allowable length of results to return is "
								+ rm.getNumRecordsResults() + ", however the number requested was "
								+ odl.getNumResultsToReturn());
			}
			...
			if (!rf.hasErrors() && odl != null) {
				...
				DDSQueryAction.logQuery(odl.getQueryString(), servlet.getServletContext(), request, 200, -1,
						(results == null ? 0 : results.size()), DDSServicesAction.getNumRecords(rm), odl.getOffsetInt(),
						DDSQueryAction.SEARCHTYPE_ODL_SEARCH);

				// If no results:
				if (results == null || results.size() == 0)
					if (odl.getSet() == null)
						rf.addOaiError(OAICodes.NO_RECORDS_MATCH, "The odl search query '" + odl.getQueryString()
								+ "' with format '" + format + "' had no matches.");
					else
						rf.addOaiError(OAICodes.NO_RECORDS_MATCH, "The odl search query '" + odl.getQueryString()
								+ "' with set '" + odl.getSet() + "' and format '" + format + "' had no matches.");
				else if (results.size() <= odl.getOffsetInt()) {
					rf.addOaiError(OAICodes.NO_RECORDS_MATCH,
							"There were " + results.size() + " matching results, " + " however he requested offset of "
									+ odl.getOffsetInt() + " is greater than or equal to the number of results.");
				}
				// If one or more results:
				else {
					rf.setResumptionToken("<resumptionToken completeListSize=\"" + results.size() + "\" cursor=\""
							+ odl.getOffset() + "\" />");
		...
		// ------- Handle standard OAI-PMH ---------
			...
			// If regular OAI-PMH ListRecords and ListIdentifiers requests are disabled,
			// indicate as such
			...
			if (!rf.hasErrors() && rth != null) {
				...
				if (results == null || results.size() == 0) {
					...
					rf.addOaiError(OAICodes.NO_RECORDS_MATCH, "There are no matching records for request: " + msg);
				}
				// If one or more results:
				else {
		...
		// Forward to either ListIdentifiers or ListRecords jsp handlers.
			...
			logRequest(request, requestTypeForLog, null, Integer.toString(results.size()), rf.getResultsOffset(), nrtr);
	 ...
	 *            The HTTP request.
	 * @param requestType
	 *            The request type, for example Identify.
	 */
	 ...
	 *            The request
	 * @param requestType
	 *            The type of OAI request (ListRecords, GetRecord, etc)
	 * @param errors
	 *            If error
	 */
	private final void logRequest(HttpServletRequest request, String requestType, ArrayList errors) {
	 ...
	 *            The request.
	 * @param requestType
	 *            The request type, for example Identify or ListRecords.
	 * @param errors
	 *            OAI errors, or null.
	 * @param repositorySize
	 *            Size of the repository, or null.
	 * @param resultsOffset
	 *            Offset to the beginning of the results returned, or null.
	 * @param numResultsReturned
	 *            The total number of results being returned to the client, or null.
	 */
	private final void logRequest(HttpServletRequest request, String requestType, ArrayList errors,
			String repositorySize, String resultsOffset, String numResultsReturned) {
		if (webLogIndex == null)
			return;
		...
		if (errors != null && errors.size() != 0) {
			...
			if (resultsOffset != null && numResultsReturned != null && repositorySize != null)
				webNote.append(" " + numResultsReturned + " records were returned out of " + repositorySize
						+ ", beginning at record number " + resultsOffset + ".");
		}
		...
		// prtln("logging request '" + webNote + "' log level: " + logLevel + " means: "
		// + doLog);
		if (doLog)
	...
	/**
	 * Handles the OAI ListSets request. From the spec, this request has the
	 * following properties:
	 * <p>
	 *
	 * Arguments<br>
	 * resumptionToken - an exclusive argument with a value that is the flow control
	 * token returned by a previous ListSets request that issued an incomplete list
	 * <p>
	 *
	 * Error and Exception Conditions<br>
	 * badArgument - The request includes illegal arguments or is missing required
	 ...
	 *         handled here.
	 */
	protected ActionForward doListSets(HttpServletRequest request, ActionMapping mapping, int num_args,
			RepositoryManager rm, RepositoryForm rf) {
		// prtln("doListSets()");

		if (request.getParameter(OAIArgs.RESUMPTION_TOKEN) == null && num_args >= 1)
			rf.addOaiError(OAICodes.BAD_ARGUMENT, "The " + OAIArgs.LIST_SETS
					+ " request may only include the argument '" + OAIArgs.RESUMPTION_TOKEN + "'");

		if (!rm.getHasOaiSetsConfigured())
			rf.addOaiError(OAICodes.NO_SET_HIERARCHY, "This repository does not currently have any sets defined");
	...
	private class ODLSearchHandler {
		 ...
		 *            The odl set
		 * @exception Exception
		 *                If unable to parse the odl set.
		 */
		public ODLSearchHandler(String setSpec) throws Exception {
			String[] odlParams = setSpec.split("\\/");

			if (odlParams.length != 5)
				throw new Exception(
						"Wrong number of ODL parameters. Be sure to escape / with %2F in the query string and set");
			...
			set = odlParams[2];
			// Unescape set (replace %2F with /) (slashes are not allowed in the setSpec,
			// but just in case...)
			if (set != null)
		 ...
		 *            The total number of results that will be returened.
		...
		public String getSet() {
			if (set == null || set.toLowerCase().equals("null") || set.toLowerCase().equals("all"))
				return null;
			return set;
		}
	}
	...
	/**
	 * Creates a resumptionToken from an OAI-PMH request for ListIdentifiers or
	 * ListRecords, and provides the set, format, from and until arguments that are
	 * associted with the request.
	 *
	 * @author John Weatherley
	 * @version $Id: RepositoryAction.java,v 1.37 2010/07/14 00:18:49 jweather Exp $
	 */
	private class ResumptionTokenHandler {
		 ...
		 *
		 * @param previousToken
		 *            The previous ResumptionToken taken from the request.
		 * @param numResultsToReturn
		 *            The number or results to return.
		 * @exception Exception
		 *                If the resumptionToken can not be parsed properly.
		 */
		public ResumptionTokenHandler(String previousToken, int numResultsToReturn) throws Exception {
			...
			/*
			 * prtln("resumptionToken parsed args are: "); for (int i = 0; i < args.length;
			 * i++) prtln(args[i]);
			 */
		 ...
		 *
		 * @param set
		 *            The set OAI-PMH argument, or null.
		 * @param format
		 *            The format OAI-PMH argument.
		 * @param from
		 *            The from OAI-PMH argument, or null.
		 * @param until
		 *            The until OAI-PMH argument, or null.
		 * @param numResultsToReturn
		 *            The number or results to return.
		 */
		public ResumptionTokenHandler(String set, String format, String from, String until, int numResultsToReturn) {
			this.set = set;
			this.format = format;
			this.from = from;
			this.until = until;
		...
		/**
		 * Gets the numRecordsBeingReturned attribute of the ResumptionTokenHandler
		 * object
		 *
		 * @param totalNumResults
		 ...
		 *
		 * @param totalNumResults
		 *            The total number of results that are in the complete result set.
		...
		public String getNextToken(int totalNumResults) {
			...
			if (totalNumResults <= nextCursor) {
				return "<resumptionToken completeListSize=\"" + totalNumResults + "\" cursor=\"" + cursor + "\"/>";
			}

			// This is not yet the last token...
			StringBuffer out = new StringBuffer();
			out.append("<resumptionToken completeListSize=\"" + totalNumResults + "\" cursor=\"" + cursor + "\">");

			out.append(cursor + DELIM + nextCursor + DELIM + totalNumResults + DELIM
					+ ((format == null) ? NP : format.trim()) + DELIM + ((set == null) ? NP : set.trim()) + DELIM
					+ ((from == null) ? NP : from.trim()) + DELIM + ((until == null) ? NP : until.trim()));
