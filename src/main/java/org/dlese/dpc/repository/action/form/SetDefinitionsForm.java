...
public final class SetDefinitionsForm extends ValidatorForm implements Serializable {
	...
	/**
	 * Resets the data in the form to default values. Needed by Struts if form is
	...
	public ActionErrors validate(ActionMapping mapping, HttpServletRequest request) {
		...
		if (rm != null && setSpec != null && setSpec.trim().length() > 0) {
			...
			// If editing an existing setSpec, verify that the new setSpec does not conflict
			// with another existing setSpec
		...
		// Verify that the setSpec meets the protocol specifications for URI unreserved
		// characters:
		if (setSpec != null) {
	...
	/**
	 * Gets the includedDirs that are defined for the set but that do not exist in
	 * the repository.
	 *
	...
	/**
	 * Gets the Excluded Dirs that are defined for the set but that do not exist in
	 * the repository.
	 *
	 * @return The includedDirsNotInRepository value
	 */
	...
	public boolean getIsDefinitionEmpty() {
		...
		return (isEmpty(includedDirs) && isEmpty(includedQuery) && isEmpty(includedFormat) && isEmpty(includedTerms)
				&& isEmpty(excludedDirs) && isEmpty(excludedQuery) && isEmpty(excludedTerms));
	}
