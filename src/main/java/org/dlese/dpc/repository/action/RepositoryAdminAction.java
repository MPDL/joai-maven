 ...
 * @version $Id: RepositoryAdminAction.java,v 1.77 2009/12/22 01:54:56 jweather
 *          Exp $
 */
public final class RepositoryAdminAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest req,
		...
		// The IndexingManager is used if an external data source is managing the index
		// rather than the internal FileIndexingService
		...
		if (isCancelled(req)) {
			// errors.add("message", new ActionError("generic.message", "Canceled - nothing
			// done"));
		...
		try {
			...
			// Add an OAI set definition (Note: This sets forms config is a good action/form
			// forwrding scheme to follow):
			...
			if (command != null) {
				...
				if (command.equals("changeProviderStatus")) {
					...
					errors.add("message", new ActionError("generic.message",
							"The data provider has been " + status.toLowerCase() + "."));
					saveErrors(req, errors);
					return mapping.findForward("display.repository.settings");
				}

				// Delete an OAI set definition (Note: This sets forms config is a good
				// action/form forwrding scheme to follow)
				if (command.equals("deleteSetDefinition")) {
					...
					try {
						...
						errors.add("error", new ActionError("generic.error",
								"An error occured while trying to delete set '" + setName + "'. Message: " + t));
						saveErrors(req, errors);
						return mapping.findForward("display.repository.settings");
					}

					errors.add("message",
							new ActionError("generic.message", "The set '" + setName + "' has been deleted"));
					saveErrors(req, errors);
				...
				if (command.equals("updateSetDefinition")) {
					...
					try {
						updateSetDefinitionsForm = OAISetsXMLConfigManager
								.getSetDefinitionsForm(rm.getListSetsConfigXml(), setSpec);
					} catch (Throwable t) {
						errors.add("error", new ActionError("generic.error",
								"An error occured while trying to edit a set. Message: " + t));
						saveErrors(req, errors);
						return mapping.findForward("display.repository.settings");
					}
					if (updateSetDefinitionsForm == null) {
						errors.add("error", new ActionError("generic.error",
								"Can not edit set '" + setSpec + "' because it does not exist"));
						saveErrors(req, errors);
						return mapping.findForward("display.repository.settings");
					}
				...
				if (command.equals("updateRepositoryInfo")) {
					...
					try {
						...
						errors.add("error", new ActionError("generic.error",
								"An error occured while trying to update the repository info. Message: " + t));
						saveErrors(req, errors);
						return mapping.findForward("display.repository.settings");
					}
				...
				if (command.equals("indexingManagerAction")) {
					...
					if (indexingManager == null) {
						errors.add("error", new ActionError("generic.error",
								"Can not index from external source - no IndexingManager is configured in the system"));
						saveErrors(req, errors);
						return mapping.findForward("display.repository.settings");
					}
					...
					if (imCommand.equals("indexAllCollections")) {
						try {
							...
							errors.add("message", new ActionError("generic.message",
									"IndexingManager: Indexing all collections has begun."));
						} catch (Throwable t) {
							errors.add("error", new ActionError("generic.error",
									"There was an error reported from the IndexingManager: " + t));
							saveErrors(req, errors);
					...
					if (imCommand.equals("abortIndexing")) {
						try {
							...
							errors.add("message", new ActionError("generic.message",
									"IndexingManager: Indexing requested to stop..."));
						} catch (Throwable t) {
							errors.add("error", new ActionError("generic.error",
									"There was an error reported from the IndexingManager: " + t));
							if (true || t instanceof java.lang.NullPointerException)
				...
				// Delete a set/collection from the repository config and index (this is the
				// file dirs):
				if (command.equals("removeSetBySetSpec")) {
					String setSpec = req.getParameter("setSpec");
					if (setSpec == null || setSpec.length() == 0) {
						errors.add("error",
								new ActionError("generic.error", "Can not remove set - no setSpec specified"));
						saveErrors(req, errors);
						return mapping.findForward("display.repository.settings");
					}
					...
					if (si != null)
						errors.add("message", new ActionError("generic.message",
								"'" + si.getName() + "' was removed from the respository"));
					else
						errors.add("message", new ActionError("generic.message",
								setSpec + " was not configured and so was not removed"));
					saveErrors(req, errors);
				...
				if (command.equals("addMetadataDir")) {
					...
					MetadataDirectoryInfoForm metadataDirsForm = (MetadataDirectoryInfoForm) req
							.getAttribute("metadataDirsForm");
				...
				if (command.equals("Update modified files")) {
					rm.indexFiles(new SimpleFileIndexingObserver("Update modified files action", "Starting indexing"),
							false);
				...
				if (command.equals("Reindex all files")) {
					...
					errors.add("message",
							new ActionError("generic.message", "Changes may take several several minutes to appear"));
					errors.add("showIndexMessagingLink", new ActionError("generic.message", ""));
					saveErrors(req, errors);
					return mapping.findForward("display.repository.settings");
				}
				...
				if (command.equals("reindexCollection")) {
					...
					if (isIndexedByIndexingManager) {
						try {
							...
							errors.add("message", new ActionError("generic.message",
									"Records for '" + displayName + "' are being indexed."));
						} catch (Throwable t) {
							errors.add("error", new ActionError("generic.error", "Records for '" + displayName
									+ "' could not being indexed. There was an error: " + t.getMessage()));
						}
					}
					// Handle indexing from FileIndexingService:
						...
						if (rm.indexCollection(key, new SimpleFileIndexingObserver(
								"Collection indexer for '" + displayName + "'", "Starting indexing"), indexAll)) {
							errors.add("message", new ActionError("generic.message",
									"Files for '" + displayName + "' are being indexed."));
							errors.add("message", new ActionError("generic.message",
									"Changes may take several several minutes to appear"));
							errors.add("showIndexMessagingLink", new ActionError("generic.message", ""));
						} else {
							errors.add("error", new ActionError("generic.error", "'" + displayName
									+ "' is not configured in the repository. Unable to index files."));
						}
					}
					saveErrors(req, errors);
					return mapping.findForward("display.repository.settings");
				...
				if (command.equals("loadOAISetsConfig")) {
					...
					errors.add("message",
							new ActionError("generic.message", "OAI List Sets configuration has been loaded: " + sets));
					saveErrors(req, errors);
					return mapping.findForward("display.repository.settings");
				}
				...
				if (command.equals("reloadRepositoryConfig")) {
					...
					try {
						PropertiesManager ddsConfigProperties = (PropertiesManager) servletContext
								.getAttribute("ddsConfigProperties");
					...
					errors.add("message",
							new ActionError("generic.message", "Configuration files have been re-loaded."));
					saveErrors(req, errors);
					return mapping.findForward("display.repository.settings");
				}
				...
				if (command.equals("setTrustedIps")) {
					...
					if (trustedIps != null) {
						...
						raf.setTrustedWsIps(rm.getTrustedWsIps());
						errors.add("message",
								new ActionError("generic.message", "The trusted web service IPs have been updated."));
						saveErrors(req, errors);
					}
					return mapping.findForward("display.repository.settings");
				}
				...
				if (command.equals("Reload vocabulary")) {
					if (vocab != null) {
						LoadMetadataVocabs loadVocabs = (LoadMetadataVocabs) servlet.getServletContext()
								.getAttribute("LoadMetadataVocabs");
						if (loadVocabs != null) {
							loadVocabs.load();
						}
				...
				if (command.equals("ubf")) {
					...
					if (req.getParameter("titleBoostFactor") != null) {
						try {
							...
							if (!(titleBoostFactor >= 0.0))
								errors.add("error", new ActionError("generic.error",
										"Title boost factor must be a number greater than or equal to zero."));
							else
								rm.setTitleBoostFactor(titleBoostFactor);
						} catch (Throwable e) {
							errors.add("error", new ActionError("generic.error", "Incorrect value " + e.getMessage()
									+ ". Title boost factor must be a number greater than or equal to zero."));
						}
					}
					...
					if (req.getParameter("drcBoostFactor") != null) {
						try {
							...
							if (!(drcBoostFactor >= 0.0))
								errors.add("error", new ActionError("generic.error",
										"DRC boost factor must be a number greater than or equal to zero."));
							else
					...
					if (req.getParameter("multiDocBoostFactor") != null) {
						try {
							...
							if (!(multiDocBoostFactor >= 0.0))
								errors.add("error", new ActionError("generic.error",
										"Multi-record boost factor must be a number greater than or equal to zero."));
							else
					...
					if (req.getParameter("stemmingBoostFactor") != null) {
						try {
							...
							if (!(stemmingBoostFactor >= 0.0))
								errors.add("error", new ActionError("generic.error",
										"Stemming boost factor must be a number greater than or equal to zero."));
					...
					if (req.getParameter("resetDefaultBoosting") != null) {
						try {
							...
							errors.add("error", new ActionError("generic.error",
									"Unable to update boosting values to defaults: " + e.getMessage()));
						}
					}
					...
					if (errors.isEmpty()) {
						...
						raf.setStemmingEnabled(Boolean.toString(rm.isStemmingEnabled()));
						errors.add("message",
								new ActionError("generic.message", "Search boosting factors have been updated."));
					}
			...
			// ---------- Handle all auto set/collection configuration from collection-level
			// records ----------------------
			...
			if (req.getParameter("currentSetSpec") != null) {
				...
				if (req.getParameter("add") != null) {
					if (!req.getParameter("currentSetSpec").equals("")) {
						String currentSetDescription = req.getParameter("currentSetDescription");
						...
						if (!isValidSet(req.getParameter("currentSetName").trim(),
								req.getParameter("currentSetSpec").trim(), currentSetDescription, raf, req, true, rm)) {
							return mapping.findForward("edit.repository.settings");
						}
						...
						SetInfo set = new SetInfo(req.getParameter("currentSetName").trim(),
								req.getParameter("currentSetSpec").trim(), currentSetDescription, enableNewSets, null,
								null, null);
					...
					if (req.getParameter("disableSet") != null || req.getParameter("enableSet") != null) {
						...
						// Use old method
							...
							if (set != null && set.getSetSpec().equals(req.getParameter("currentSetSpec").trim())
						...
						MetadataVocabServlet vocabsServlet = (MetadataVocabServlet) servlet.getServletContext()
								.getAttribute("MetadataVocabServlet");
					...
					// Remove a set
						...
						// Make sure we have the right set!
						if (rmSet != null && rmSet.getName().equals(req.getParameter("currentSetName").trim())
					...
					// Update the set:
						...
						if (set != null) {
							...
							boolean isNewSetSpec = (setSpec != null && set.getSetSpec().equals(setSpec) ? false : true);
							if (!isValidSet(setName, setSpec, setDescription, raf, req, isNewSetSpec, rm)) {
								return mapping.findForward("edit.repository.settings");
							}
				...
				raf.setSets(rm.getSetInfos());
				if (vocab != null && servletContext.getAttribute("suppressSetCollectionsVocabDisplay") == null) {
			...
			if (req.getParameter("dirConf") != null) {
				...
				if (dirConf.equals("add")) {
					...
					File f = new File(dir);
					if (!f.isDirectory()) {
						errors.add("message", new ActionError("generic.error", "Error: '" + dir
								+ "' is not a valid directory on the server. Please ensure the directory exists and try again."));
						saveErrors(req, errors);
						return mapping.findForward("display.repository.settings");
					}
			...
			if (req.getParameter("currentSetFormat") != null
				...
				if (req.getParameter("add") != null) {
					if (setDirInfoIndex == -1) {
						...
						if (rm.isDirectoryConfigured(setDirectory)) {
							errors.add("error", new ActionError("generic.message",
									"The directory indicated is already taken. Each directory may only be configured once."));
							saveErrors(req, errors);
							return mapping.findForward("edit.repository.settings");
						}
						tmp.addDirInfo(req.getParameter("currentSetFormat").trim(),
								req.getParameter("currentSetDirectory").trim());
					...
					if (req.getParameter("remove") != null) {
						prtln("removing dir: ");
						// prtln("getting setIndex: " + setIndex + " setDirInfoIndex: " +
						// setDirInfoIndex);
						SetInfo currentSet = rm.getSetInfoCopy(setIndex);
						DirInfo di = new DirInfo(req.getParameter("currentSetDirectory").trim(),
								req.getParameter("currentSetFormat").trim());
						...
						if (currentSet.getDirInfo(setDirInfoIndex).equals(di)) {
							...
							errors.add("message", new ActionError("generic.message",
									"The directory of files has been removed from the index."));
							saveErrors(req, errors);
						} else {
					...
					// Update a direcotry/format tuple for this set:
						...
						modSet.setDirInfo(setDirInfoIndex, req.getParameter("currentSetDirectory"),
	...
	private final boolean isValidSet(String setName, String setSpec, String setDescription, RepositoryAdminForm raf,
			HttpServletRequest req, boolean checkSetSpec, RepositoryManager rm) {
		ActionErrors errors = new ActionErrors();
		 ...
		 * errors.add("currentSetFormat", new ActionError("errors.setFormat")); File f =
		 * new File(set.getDirectory()); prtln("directory: " + f.getAbsolutePath()); if
		 * (!f.isDirectory()) errors.add("currentSetDirectory", new
		 * ActionError("errors.setDirectory"));
		 */
		if (errors.isEmpty()) {
	...
	private ActionForward doAddNewMetadataFileDir(ActionErrors errors, ActionMapping mapping, RepositoryManager rm,
			HttpServletRequest req, RepositoryAdminForm raf, MetadataDirectoryInfoForm addMetadataDirsForm) {
		...
		if (errors.size() == 0) {
			prtln("Adding metadata file dir: " + dirNickname + " " + dirMetadataFormat + " " + dirPath + " "
			...
			try {
				...
				SetInfo newSet = new SetInfo(dirNickname, setSpec, "", "true", dirPath, dirMetadataFormat, setSpec);
				...
				// Begin the iindexer
				rm.indexCollection(setSpec,
						new SimpleFileIndexingObserver("Indexer for '" + dirNickname + "'", "Starting indexing"), true);
				errors.add("showIndexMessagingLink", new ActionError("generic.message", ""));
				errors.add("message", new ActionError("generic.message",
						"Directory '" + newSet.getName() + "' was added and is being indexed"));
			} catch (Throwable t) {
				errors.add("error",
						new ActionError("generic.error", "There was an error adding the directory: " + t.getMessage()));
	...
	private ActionForward doEditMetadataFileDir(ActionErrors errors, ActionMapping mapping, RepositoryManager rm,
			HttpServletRequest req, RepositoryAdminForm raf, MetadataDirectoryInfoForm editMetadataDirsForm) {
		...
		try {
			...
			/*
			 * if(!prevSetInfo.getDirectory().equals(dirPath) ||
			 * !prevSetInfo.getFormat().equals(dirMetadataFormat)){ boolean wasRemoved =
			 * rm.removeSetBySetSpec(prevSetInfo.getSetSpec()); if (wasRemoved)
			 * errors.add("message", new ActionError("generic.message", setSpec +
			 * " was removed from the respository")); else errors.add("message", new
			 * ActionError("generic.message", setSpec +
			 * " was not configured and so was not removed")); }
			 */

			SetInfo updatedSetInfo = new SetInfo(dirNickname, setSpec, prevSetInfo.getDescription(),
					prevSetInfo.getEnabled(), dirPath, dirMetadataFormat, setSpec);
			...
			// Begin the indexer
			// rm.indexCollection(setSpec,new SimpleFileIndexingObserver("Indexer for '" +
			// dirNickname + "'", "Starting indexing"),true);

			errors.add("showIndexMessagingLink", new ActionError("generic.message", ""));
			errors.add("message", new ActionError("generic.message",
					"Information and configuration was updated for directory '" + updatedSetInfo.getName() + "'"));
		} catch (Throwable t) {
			errors.add("error",
					new ActionError("generic.error", "There was an error adding the directory: " + t.getMessage()));
	...
	private ActionForward doAddSetDefinition(ActionErrors errors, ActionMapping mapping, RepositoryManager rm,
			HttpServletRequest req, RepositoryAdminForm raf, SetDefinitionsForm setDefinitionsForm) {
		...
		if (prevSetSpec != null && !prevSetSpec.equals(setDefinitionsForm.getSetSpec())) {
			...
			try {
				...
				errors.add("error", new ActionError("generic.error", "An error occured while trying to delete set '"
						+ setDefinitionsForm.getSetName() + "'. Message: " + t));
				saveErrors(req, errors);
				return mapping.findForward("display.repository.settings");
			}
		}
		...
		errors.add("message", new ActionError("generic.message",
				"The set '" + setDefinitionsForm.getSetName() + "' was added successfully."));
		saveErrors(req, errors);
		return mapping.findForward("display.repository.settings");
	}

	private ActionForward doSetRepositoryInfo(ActionErrors errors, ActionMapping mapping, RepositoryManager rm,
			HttpServletRequest req, RepositoryAdminForm raf, RepositoryInfoForm repositoryInfoForm) {
		...
		try {
			...
			errors.add("error",
					new ActionError("generic.error", "Error saving repository information: " + t.getMessage()));
			if (t instanceof NullPointerException)
				t.printStackTrace();
			saveErrors(req, errors);
			return mapping.findForward("display.repository.settings");
	 ...
	 *
	 * @param rm
	 *            The new namespaceAndSchemaDefs value
	 * @param xmlFormat
	 *            The new namespaceAndSchemaDefs value
	 * @param namespace
	 *            The new namespaceAndSchemaDefs value
	 ...
	 * @return Null iff no validation errors were found, else a String containing an
	...
	private final String validateXML(String s) {
		...
		if (s.indexOf("schemaLocation") == -1) {
			return "SCHEMA NOT PRESENT: The schema location must be defined in the "
	...
	/**
	 * Gets the index associated with a request parameter of the form myParameter[i]
	 * where the collection index is indicated in brackets.
	 *
	 * @param paramName
