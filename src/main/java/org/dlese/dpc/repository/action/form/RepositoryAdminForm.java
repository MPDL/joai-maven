 ...
 * @version $Id: RepositoryAdminForm.java,v 1.27 2010/03/16 21:42:01 jweather
 *          Exp $
 */
public final class RepositoryAdminForm extends RepositoryForm implements Serializable {
	...
	/**
	 * Gets the frequency by which the index is updated to reflect changes that
	 * occur in the metadata files. A return of 0 indicates no automatic updating
	 * occurs.
	 *
	...
	/**
	 * Sets the frequency by which the index is updated to reflect changes that
	 * occur in the metadata files. A return of 0 indicates no automatic updating
	 * occurs.
	 *
	 * @param frequency
	 *            The new updateFrequency, in minutes.
	 */
	...
	/**
	 * Gets the date and time the indexer is/was scheduled to start, for example
	 * 'Dec 2, 2003 1:35 AM MST'.
	 *
	 * @return The date and time the indexer is scheduled to start, or empty String
	 *         if not available.
	 */
	...
	/**
	 * Gets the time of day the indexer isscheduled to start, for example '1:35 AM
	 * MST'.
	 *
	 * @return the time of day the indexer isscheduled to start, or empty String if
	 *         not available.
	 */
	 ...
	 *            The new indexingStartTime value
	 */
	...
	public String getIndexingDaysOfWeek() {
		if (indexingDaysOfWeek == null)
			...
			for (int i = 0; i < indexingDaysOfWeek.length; i++)
				daysOfWeekMsg += Utils.getDayOfWeekString(indexingDaysOfWeek[i])
						+ (i == indexingDaysOfWeek.length - 1 ? "" : ", ");
			return daysOfWeekMsg;
		}
	}

	/**
	 * Sets the idexingDaysOfWeek as an array of Calendar.DAY_OF_WEEK fields, or
	 * null if none.
	 *
	 * @param daysOfWeek
	 *            The new idexingDaysOfWeek value
	 */
	 ...
	 *
	 * @param numErrors
	 *            The number of indexing errors that are present.
	 */
	 ...
	 *
	 * @param value
	 *            The new repositoryIdentifier value
	 */
	 ...
	 *
	 * @param value
	 *            The new currentAdminEmail value
	 */
	 ...
	 *
	 * @param value
	 *            The new currentDescription value
	 */
	...
	/**
	 * Gets the path for the directory of collect-level records the
	...
	/**
	 * Gets the path for the directory of metadata records the RepositoryManager is
	...
	public String getConfigDirLocation() {
		try {
			RepositoryManager rm = (RepositoryManager) getServlet().getServletContext()
					.getAttribute("repositoryManager");
			if (rm != null)
	 ...
	 *
	 * @param value
	 *            The new currentSetDescription value
	 */
	 ...
	 *
	 * @param value
	 *            The new currentSetDirectory value
	 */
	 ...
	 *
	 * @param value
	 *            The new currentSetFormat value
	 */
	 ...
	 *
	 * @return The boosting factor used to rank resources that have multiple
	 *         records.
	 */
	...
	/**
	 * Grabs the base directory where collections metadata files are located, or
	 * null if not configured.
	 *
	 * @return Base directory where the collections reside.
	 */
	public String getCollectionsBaseDir() {
		String collectionsBaseDir = getServlet().getServletContext().getInitParameter("collBaseDir");
	...
	public ArrayList getValidCollectionKeys() {
		String collectionKeySchemaUrl = getServlet().getServletContext().getInitParameter("collectionKeySchemaUrl");
	...
	public ArrayList getValidMetadataFormats() {
		String metadataFormatSchemaUrl = getServlet().getServletContext().getInitParameter("metadataFormatSchemaUrl");
		...
		if (validMetadataFormats == null) {
			try {
				...
				List nodes = document
						.selectNodes("//xsd:simpleType[@name='itemFormatType']/xsd:restriction/xsd:enumeration");
				for (Iterator iter = nodes.iterator(); iter.hasNext();) {
					Node node = (Node) iter.next();
	...
	public ActionErrors validate(ActionMapping mapping, HttpServletRequest request) {
		...
		try {
			...
			if (currentSetFormat != null && currentSetFormat.matches(".*--.*"))
				errors.add("currentSetFormat",
						new ActionError("generic.message", "You must select a metadata format."));

			/*
			 * if (currentAdminEmail != null && !emailIsValid(currentAdminEmail))
			 * errors.add("currentAdminEmail", new ActionError("errors.adminEmail")); if
			 * (currentSetDirectory != null && currentSetDirectory.length() != 0 &&
			 * request.getParameter("remove") == null && !(new
			 * File(currentSetDirectory.trim()).isDirectory())) errors.add("setDirectory",
			 * new ActionError("errors.setDirectory")); if (repositoryIdentifier != null &&
			 * repositoryIdentifier.length() != 0 && !repositoryIdentifier.matches(
			 * "[a-zA-Z][a-zA-Z0-9\\-]*(\\.[a-zA-Z][a-zA-Z0-9\\-]+)+"))
			 * errors.add("namespaceIdentifier", new
			 * ActionError("errors.namespaceIdentifier"));
			 */
		...
		return errors;
		/*
		 * if (currentSetSpec != null || add.equals("t") &&
		 * !setSpecIsValid(currentSetSpec)) errors.add("currentSetSpec", new
		 * ActionError("errors.setSpec")); if (currentSetName != null || add.equals("t")
		 * && currentSetName.length() == 0 ) errors.add("currentSetName", new
		 * ActionError("errors.setName"));
		 */
	}
