...
/**
 * Data structure used in the {@link RepositoryManager} to hold information
 * about a set of metadata files. Note that as of jOAI v3.x (Feb 2006), this
 * class is no longer used to define OAI sets but is now only used to define the
 * files that are configured in the {@link RepositoryManager}. OAI sets are now
 * defined separately.
 *
...
public class SetInfo implements Serializable, Comparable {
	 ...
	 * @param setSpec
	 *            The oai setSpec that will be used to reference this set
	 * @param dir
	 *            A directory where files for this set are located
	 * @param format
	 *            The native metadata format for the files in the above directory
	 * @param description
	 *            A description for this set, in XML form.
	 * @param enabled
	 *            [true | false]
	 * @param id
	 *            DESCRIPTION
	...
	/**
	 * Gets the metadata format of the files in the primary directory configured for
	 * this set, or an empty String if none is configured. The primary directory is
	...
	/**
	 * Gets the primary directory path configured for this set, or an empty String
	 * if none is configured. The primary directory is the first one configured.
	 *
	...
	/**
	 * Determines whether a {@link DirInfo} that matches the attributes of the given
	 * {@link DirInfo} already exists in this SetInfo.
	 *
	 * @param dirInfo
	 *            A {@link DirInfo} to check for existance.
	...
	/**
	 * Sets data in the SetInfo for display in the UI. This includes things like the
	 * number of items indexed for this collection, the number if items that are
	 * discoverable for this collection, the number of files for this collection,
	 * and the number of index errors that were detected for this collection. Also
	 * the collection accession status.
	 *
	 * @param rm
	 *            The RepositoryManager whoes index contains the collections
	 *            configured by this SetSpec.
	 */
	public void setSetInfoData(RepositoryManager rm) {
		try {
			// numDiscoverable = rm.getIndex().numDocs(rm.getDiscoverableItemsQuery() + "
			// AND collection:0" + getSetSpec());
			...
			deletedQ.add(new TermQuery(new Term("docdir", new File(getDirectory()).getAbsolutePath())),
			...
			errQ.add(new TermQuery(new Term("docdir", new File(getDirectory()).getAbsolutePath())),
					BooleanClause.Occur.MUST);
	 ...
	 */
	/*
	 * public String getNumDiscoverable() { if (numDiscoverable == -1) return
	 * "Not available"; return Integer.toString(numDiscoverable); }
	 */
	/**
	 ...
	 *
	 * @param format
	 *            The feature to be added to the DirInfo attribute
	 * @param dir
	 *            The feature to be added to the DirInfo attribute
	 */
	...
	/**
	 * Sets an attribute Object that will be available for access using the given
	 * key. The object MUST be serializable.
	 *
	 * @param key
	...
	/**
	 * Provides a String representataion for this SetInfo. This method may be used
	 * for debugging to see what is in the SetInfo. This method is also used it the
	...
	public boolean equals(Object o) {
		...
		try {
			...
			/*
			 * return ( ((((SetInfo) o).name == null && this.name == null) || ((SetInfo)
			 * o).name.equals(this.name)) && ((((SetInfo) o).setSpec == null && this.setSpec
			 * == null) || ((SetInfo) o).setSpec.equals(this.setSpec)) && ((((SetInfo)
			 * o).dirInfos == null && this.dirInfos == null) || ((SetInfo)
			 * o).dirInfos.equals(this.dirInfos)) );
			 */
		} catch (Throwable e) {
			// Catch null pointer...
			return false;
	...
	/**
	 * Compares two SetInfos by the Set Name. Collections.sort() or Arrays.sort()
	 * can thus be used to sort a list of SetInfos by Name.
	 *
	 * @param o
	 ...
	 *                If the object passed in is not a SetInfo.
	 */
