...
/**
 * Provides data related to a given collection-level record such as its
 * associated IDs and annotations. This class is used by class
 * {@link org.dlese.dpc.index.writer.ItemFileIndexingWriter} to insert data into
 * the index for each record at index creation time.
 *
...
public class RecordDataService {
	...
	/**
	 * Constructor for a RecordDataService that has access to an MmdRec Query
	 * service.
	 *
	 * @param dbUrl
	 ...
	 * @param vocab
	 *            The MetadataVocab to use, or null if not needed.
	 * @param collBaseDir
	 *            Description of the Parameter
	 * @param annotationPathwaysSchemaUrl
	 *            URL to the annotation pathways schema
	 * @see org.dlese.dpc.services.mmd.Query
	 */
	public RecordDataService(String dbUrl, MetadataVocab vocab, String collBaseDir,
	 ...
	 *
	 * @param newVocab
	 *            The new MetadataVocab that will be used.
	 */
	 ...
	 *
	 * @param collectCollectionID
	 *            The value to assign collectCollectionID.
	 */
	 ...
	 * {@link #getMmdRec(String, String, Query)} or
	 * {@link #getAssociatedMMDRecs(int, String, String, Query)}. The Query may be
	 * used multiple times. After use, the caller must be sure to close the Query
	 * using the {@link #closeIdMapperQueryObject(Query)}.
	 *
	...
	/**
	 * Initializes the local Query Object. This must be called prior to use over a
	 * collection of files.
	 */
	public void initIdMapper() {
		// prtln("initIdMapper()");
		if (dbUrl == null || dbUrl.trim().toLowerCase().equals("userandomids") || IDMAPPER_DISABLED) {
			return;
		}
	...
	/**
	 * Closes the local Query Object, releasing it's resources. This should be
	 * called after finished using the idmapper for a collection of files.
	 */
	...
	public String getFilePathForId(MmdRec rec) {
		...
		String pathToRecord = collBaseDir + "/" + rec.metastyleNames[rec.getMetastyle()] + "/" + rec.getCollKey() + "/"
				+ rec.getFileName();
		return pathToRecord;
	}
	...
	/**
	 * Gets the IDs for records that catalog the same resource, not including the ID
	 * that is given.
	 *
	 * @param id
	 *            The id to the item-level record - will be ommitted from the list
	 *            of associatedIDs
	 * @param associatedMmdRecs
	 *            An array of MmdRecs
	...
	/**
	 * Gets all errors identified by the ID mapper service for the given records, or
	 * null if none exist.
	 *
	 * @param mmdRecs
	...
	/**
	 * Gets the associated MMD records for the given IDs, records that catalog the
	 * same resource, or null if none exist, using the given Query Object. Splits
	 * the IDs and collectionKeys by spaces and uses the first one.
	 *
	 * @param IDs
	 *            An ID(s) to an item-level resource, separated by spaces - the
	 *            first is used
	 * @param collectionKeys
	 *            The collection keys for the collection of files, for example "dcc"
	 *            or "comet", separated by spaces - the first is used
	 * @param queryType
	...
	public MmdRec[] getAssociatedMMDRecs(int queryType, String IDs, String collectionKeys, Query qo) {
		...
		try {
			...
			/*
			 * String msg = ""; if(associatedRecordsData == null ||
			 * associatedRecordsData.length == 0) msg = " had 0 matches"; else{ msg =
			 * " returned"; for(int i = 0; i < associatedRecordsData.length; i++) msg+= " "
			 * + associatedRecordsData[i].getId(); }
			 * 
			 * prtln("getAssociatedMMDRecs() for ID: " + ID + " type: " + (queryType ==
			 * Query.QUERY_SAME ? "same" : "other") + msg);
			 */
		} catch (Throwable e) {
	...
	/**
	 * Gets the associated MMD records for the given IDs, records that catalog the
	 * same resource, or null if none exist, using the local Query Object. Splits
	 * the IDs and collectionKeys by spaces and uses the first one.
	 *
	 * @param IDs
	 *            An ID(s) to an item-level resource, separated by spaces.
	 * @param collectionKeys
	 *            The collection keys for the collection of files, for example "dcc"
	 *            or "comet", separated by spaces.
	 * @param queryType
	 *            Specify to look in other or the same collection
	 * @return The associated MmdRecs
	 */
	 ...
	 *
	 * @param id
	 *            The ID of a given record, for example DLESE-000-000-000-001.
	 * @param collectionKey
	...
	public MmdRec getMmdRec(String id, String collectionKey, Query qo) {
		// prtln("getMmdRec(): id:" + id + " coll: " + collectionKey + (qo == null? "qo
		// null": "qo not null"));
		if (qo == null)
			return null;
	...
	/**
	 * Gets random test associatedIDs for the given ID (records that reference the
	 * same resource) or null if none exist. *** For testing purposes only ***
	 *
	 * @param ID
	 *            An ID to an item-level resource.
	 * @return The associatedIDs value
	 */
	private String[] getRandomAssociatedIDs(String ID) {
		...
		for (int i = 0; i < random_num_ids; i++) {
			ids[i] = (((ItemDocReader) resultDocs.get(Utils.getRandomIntBetween(0, resultDocs.size())).getDocReader())
					.getId());
		}
	...
	/**
	 * Gets the accession status of the given records or, if multiple resources get
	 * all statuses separated by spaces.
	 *
	 * @param mmdRecs
	 *            The MmdRecords
	...
	/**
	 * Gets the ResultDoc for the given item level metata record id but only if it
	 * should be displayed in discovery. Returns null if none exists.
	 *
	 * @param itemId
	...
	/**
	 * Gets the item result docs for each of the ids listed regardless, returning
	 * only those that should be displayed in discovery.
	 *
	 * @param itemIds
	 *            An array of IDs.
	...
	/**
	 * Gets the ResultDoc for the given item-level metata record id regardless of
	 * the records status, or null if none exists.
	 *
	 * @param itemId
	 *            The ID to a given item-level record.
	...
	/**
	 * Gets the item result docs for each of the ids listed regardless of their
	 * status.
	 *
	 * @param itemIds
	 *            An array of IDs.
	 ...
	 *
	 * @param itemId
	 *            The ID to a given XML record.
	 * @param displayable
	 *            True to return discovery displayable items, false to return all
	 *            items.
	 * @return The ResultDoc for that item.
	 */
	 ...
	 * @param displayable
	 *            Whether to return only those IDs that are currently displayable in
	 *            discovery or not.
	...
	public ResultDocList getDleseAnnoResultDocs(String[] ids) {
		...
		try {
			...
			idQ += ")";
			ResultDocList results = getIndex().searchDocs(idQ, new KeywordAnalyzer());
			if (results == null || results.size() == 0) {
	...
	/**
	 * Gets all annotation types in the given set of DleseAnnoDocReaders, or null if
	 * none exist.
	 *
	...
	/**
	 * Gets a list of keys, for example {06, 04}, for each of the annotation
	 * collections that have at least one status completed record.
	 *
	 * @param annoResultDocs
	...
	/**
	 * Gets all annotation pathways in the given set of DleseAnnoDocReaders, or null
	 * if none exist. Currently, this is not used!
	 *
	 * @param annoResultDocs
	 *            An array of ResultDocs
	...
	/**
	 * Return true if there is an annotation present that is part of the DRC. The
	 * definition of DRC is obtains from the current pathways.xsd schema at the DPC.
	 *
	 * @param annoResultDocs
	 *            Annotation records.
	...
	/**
	 * Return true if there is an item present that is part of the DRC as determined
	 * by the items collection.
	 *
	 * @param itemResultDocs
	...
	/**
	 * Return a HasMap of valid DRC pathways from the pathways.xsd schema. Both the
	 * keys and values cantain the exact Strings of the valid pathways.
	 *
	...
	/**
	 * Gets all collection keys encoded by Vocab Mgr, for example {06, 08}, for the
	 * given records.
	 *
