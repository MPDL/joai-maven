...
/**
 * Holds data related to a single item-level record such as it's annotations and
 * records that refer to the same resource. This class is used by class
 * {@link org.dlese.dpc.index.writer.ItemFileIndexingWriter} and
...
public class RecordData {
	...
	/**
	 * Contruct the record data for an item-level record. Created by
	 * {@link RecordDataService}.
	 *
	 * @param annoDocs
	 *            DleseAnnoDocReaders for records that annotate this item.
	 * @param associatedItemsDocs
	 *            ItemDocReaders for items that refer to the same resource.
	...
	/**
	 * Gets DocReaders for all item-level records that refer to the same resource,
	...
	/**
	 * Gets DocReaders for all annotation records that reference this resource, or
	 * null if none exist.
	 *
	...
	/**
	 * Gets all annotataion types in the index that refer to this record, or null if
	 * none exist.
	 *
	 * @return The annoTypes value or null.
	 */
	/*
	 * public ArrayList getAnnoTypeszzz() { if (annoDocs == null || annoDocs.length
	 * == 0) return null;
	 * 
	 * if (annoTypes != null) return annoTypes;
	 * 
	 * annoTypes = new ArrayList(annoDocs.length); String type; for (int i = 0; i <
	 * annoDocs.length; i++) { type = annoDocs[i].getType(); if
	 * (!annoTypes.contains(type)) annoTypes.add(type); } return annoTypes; }
	 */

	/**
	 * Gets all annotataion pathways in the index that refer to this record, or null
	 * if none exist.
	 *
	 * @return The annoPathways value or null.
	 */
	/*
	 * public ArrayList getAnnoPathwayszzz() { if (annoDocs == null ||
	 * annoDocs.length == 0) return null;
	 * 
	 * if (annoPathways != null) return annoPathways;
	 * 
	 * annoPathways = new ArrayList(annoDocs.length); String pathway; for (int i =
	 * 0; i < annoDocs.length; i++) { pathway = annoDocs[i].getPathway(); if
	 * (!annoPathways.contains(pathway)) annoPathways.add(pathway); } return
	 * annoPathways; }
	 */

	/**
	 * Gets the associatedIDs for this record, records that catalog the same
	 * resource. Returns only IDs for those records that are currently in the index.
	 *
