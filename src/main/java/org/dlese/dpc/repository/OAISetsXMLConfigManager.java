 ...
 * {@link RepositoryManager}. See
 * <a href="../../../../javadoc-includes/ListSets-config-sample.xml">sample
 * ListSets XML config file</a> .
 *
 * @author John Weatherley
 * @version $Id: OAISetsXMLConfigManager.java,v 1.8 2009/03/20 23:33:54 jweather
 *          Exp $
 */
public class OAISetsXMLConfigManager {
	...
	/**
	 * Reads the ListSets config XML to extract the set definition for a given set
	 * into a SetDefinitionsForm bean.
	 *
	 * @param listSetsXml
	 *            The ListSets config XML to read
	 * @param setSpec
	 *            The setSpec to read
	 * @return The setDefinitionsForm, or null if none configred for that setSpec
	 * @exception Exception
	 *                If error parsing the XML
	 */
	public static SetDefinitionsForm getSetDefinitionsForm(String listSetsXml, String setSpec) throws Exception {
		...
		setDefinitionsForm.setIncludedFormat(setElement.valueOf(
				"virtualSearchField/virtualSearchTermDefinition/Query//booleanQuery/textQuery[@field='xmlFormat']"));
		setDefinitionsForm.setIncludedQuery(setElement.valueOf(
				"virtualSearchField/virtualSearchTermDefinition/Query/booleanQuery/luceneQuery[not(@excludeOrRequire='exclude')]"));
		setDefinitionsForm.setExcludedQuery(setElement.valueOf(
				"virtualSearchField/virtualSearchTermDefinition/Query/booleanQuery/luceneQuery[@excludeOrRequire='exclude']"));

		// Handle include clauses

		// Get included dirs
		List includedDirs = setElement.selectNodes(
				"virtualSearchField/virtualSearchTermDefinition/Query//booleanQuery/textQuery[@field='docdir' and not(@excludeOrRequire='exclude')]");
		...
		List terms = setElement.selectNodes(
				"virtualSearchField/virtualSearchTermDefinition/Query//booleanQuery/textQuery[@field='default' and not(@excludeOrRequire='exclude')]");
		String text = "";
		...
		// Get excluded dirs
		List exdludedDirs = setElement.selectNodes(
				"virtualSearchField/virtualSearchTermDefinition/Query/booleanQuery/textQuery[@field='docdir' and @excludeOrRequire='exclude']");
		...
		/*
		 * dirStrings = setDefinitionsForm.getExcludedDirs(); for (int i = 0; i <
		 * exdludedDirs.size(); i++) dirStrings[i] = ((Node)
		 * exdludedDirs.get(i)).getText();
		 * setDefinitionsForm.setExcludedDirs(dirStrings);
		 */

		// Get excluded terms/phrases
		terms = setElement.selectNodes(
				"virtualSearchField/virtualSearchTermDefinition/Query/booleanQuery/textQuery[@field='default' and @excludeOrRequire='exclude']");
		text = "";
		for (int i = 0; i < terms.size(); i++) {
			text += ((Node) terms.get(i)).getText();
			if (i < terms.size() - 1)
	...
	/**
	 * Removes the given OAI set definition from the ListSets config XML file and
	 * writes it to disc.
	 *
	 * @param setsConfigFile
	 *            The ListSets config file
	 * @param setSpec
	 *            The set to remove
	...
	/**
	 * Sets the definition for a given OAI set, writing the ListSets config XML to
	 * file. If the given set already exists, it will be re-defined, if not, a new
	 * set will be added to the existsing sets in the config XML file.
	 *
	 * @param setsConfigFile
	 *            File to read/write
	 * @param sb
	 *            The bean that contains the set definition values
	 * @exception Exception
	 *                If error
	 */
	public static void setOAISetSpecDefinition(File setsConfigFile, SetDefinitionsForm sb) throws Exception {
		...
		try {
			...
			if ((setDescription != null && setDescription.trim().length() > 0)
					|| (setUrl != null && setUrl.trim().length() > 0)) {
			...
			Element virtualSearchTermDefinition = virtualSearchField.addElement("virtualSearchTermDefinition")
					.addAttribute("term", setSpec.trim());
				...
				for (int i = 0; i < includedTermsPhrases.length; i++) {
					if (includedTermsPhrases[i] != null && includedTermsPhrases[i].trim().length() > 0) {
						...
						if (includedTermsPhrases[i].trim().indexOf(' ') > 0)
							type = "matchPhrase";
						includedTermsPhrasesClauses
								.add(createTextQueryElement("default", type, includedTermsPhrases[i].trim()));
					}
				}
			...
			if (includedDirs != null) {
				...
				for (int i = 0; i < includedDirs.length; i++) {
					if (includedDirs[i] != null && includedDirs[i].trim().length() > 0)
						includedDirsClauses
								.add(createTextQueryElement("docdir", "matchKeyword", includedDirs[i].trim()));
				}
			...
			// If there is no inclusion clauses, assume all records should be included in
			// the set:
	...
	private static Element createTextQueryElement(String fieldName, String fieldType, String termOrPhrase)
			throws Exception {
		/*
		 * if (fieldName == null || fieldName.trim().length() == 0) throw new
		 * Exception("fieldName must not be empty or null"); if (fieldType == null ||
		 * fieldType.trim().length() == 0) throw new
		 * Exception("fieldType must not be empty or null"); if
		 * (!fieldType.equals("matchAnyTerm") && !fieldType.equals("matchPhrase")) throw
		 * new Exception("fieldType must be either 'matchAnyTerm' or 'matchPhrase'");
		 */
