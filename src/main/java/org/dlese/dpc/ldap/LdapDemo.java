...
public class LdapDemo {
	...
	void testit(String[] args) throws Exception {
		LdapEntry entry;
		LdapEntry[] entries;
		boolean hitexc;
		...
		String[] attrNames = new String[]{"DLESEloginName", "DLESEnameLast", "DLESEnameFirst"};
		...
		String[][] smithAttrStgs = new String[][]{
				// Required attributes:
				{"DLESEou", "people"}, {"DLESEloginName", smithUid}, // unique UID
				{"userPassword", smithPswd}, // password
				{"DLESEnameLast", "Smith"}, // surname
				{"DLESEnameFirst", "Sally"}, {"DLESEemailPrimary", "smith@ucar.edu"}, // email

				// Optional attributes:
				{"DLESEaffiliation1", "UCAR"}, // affiliated institution
				{"DLESEfocus", "graduate students"}, {"DLESEprofResp", "administrator"},
		...
		String[][] owensAttrStgs = new String[][]{
				// Required attributes:
				{"DLESEou", "people"}, {"DLESEloginName", owensUid}, // unique UID
				{"userPassword", owensPswd}, // password
				{"DLESEnameLast", "Owens"}, // surname
				{"DLESEnameFirst", "John"}, {"DLESEemailPrimary", "owens@Cornell.edu"}, // email
		...
		entries = client.search(null, // base = null: start at top of DB tree
				"objectclass=*", // filter
		...
		client.addUserEntry(smithUid, // the new UID
				smithAttrStgs); // array of attributes

		// Add new user: Oscar Owens
		client.addUserEntry(owensUid, // the new UID
				owensAttrStgs); // array of attributes
		prtall("LdapDemo: after add users: smith", client, "DLESEnameLast=*smith*");
		prtall("LdapDemo: after add users: owens", client, "DLESEnameLast=*owens*");
		prtall("LdapDemo: after add users", client, "(| (DLESEnameLast=*smith*) (DLESEnameLast=*owens*))");

		prtln("LdapDemo: userAuthenticates for owens: " + client.userAuthenticates(owensUid, owensPswd));
		...
		entry = client.getUserAttributes(smithUid, // authentication UID
				smithPswd, // password
		...
		hitexc = false;
		try {
			entry = client.getUserAttributes("noneSuchUid", // UID we want attributes for
					attrNames); // the attributes we want returned.
		...
		if (!hitexc)
			throw new Exception("could getattr on nonexistant entry");

		// Search for DLESEnameLast="*smith*";
		// return all attributes of each entry found.
		entries = client.search(smithUid, // authentication UID
				smithPswd, // password
		...
		entries = client.search(smithUid, // authentication UID
				...
				null, // base = null: start at top of DB tree
				"DLESEnameLast=*smith*", // filter
				attrNames, 0); // maxres = 0 implies return all results
		prtentries("LdapDemo: search DLESEnameLast=*smith*, selected attrs", entries);

		// Add DLESEfocus
		// If DLESEfocus doesn't exist, it will be created.
		// If DLESEfocus does and is not SINGLE-VALUE,
		// the value will be added.
		client.addUserAttributeValue(smithUid, // authentication UID
				smithPswd, // password
		...
		prtall("LdapDemo: after add another DLESEfocus", client, "DLESEnameLast=*smith*");

		// Change DLESEfocus value
		// Any previous values of DLESEfocus are removed.
		client.setUserAttribute(smithUid, // authentication UID
				smithPswd, // password
				smithUid, // UID to change
				"DLESEfocus", // attribute to change
				new String[]{"biosphere", "solid earth"} // new value of attr
		);
		prtall("LdapDemo: after change DLESEfocus", client, "DLESEnameLast=*smith*");

		// Remove a single DLESEfocus value
		client.removeUserAttributeValue(smithUid, // authentication UID
				smithPswd, // password
				smithUid, // UID to change
				"DLESEfocus", "biosphere");
		prtall("LdapDemo: after remove DLESEfocus value", client, "DLESEnameLast=*smith*");
		...
		if (!hitexc)
			throw new Exception("remove value again should have failed");
		...
		client.removeUserAttributeValue(smithUid, // authentication UID
				...
				smithUid, // UID to change
				"DLESEfocus", null);
		prtall("LdapDemo: after remove entire DLESEfocus", client, "DLESEnameLast=*smith*");
		...
		prtall("LdapDemo: after remove entire DLESEurl", client, "DLESEnameLast=*smith*");
		...
		if (!hitexc)
			throw new Exception("remove attr again should have failed");
		...
		prtall("LdapDemo: after add back DLESEurl", client, "DLESEnameLast=*smith*");
		...
		try {
			...
			prtall("LdapDemo: after set DLESEurl to null", client, "DLESEnameLast=*smith*");
		...
		client.storeUserObject(smithUid, // authentication UID
				smithPswd, // password
		...
		Object getObj = client.getUserObject(smithUid, // authentication UID
				smithPswd, // password
				smithUid, // UID where the object is stored
				"testObjectA"); // object name
		prtln("\nLdapDemo: getObj after store object, using smithUid: (" + getObj.getClass().getName() + "): \""
				+ getObj + "\"");

		// Retrieve a Java Object using admin uid
		getObj = client.getUserObject(adminUid, // authentication UID
				adminPswd, // password
				smithUid, // UID where the object is stored
				"testObjectA"); // object name
		prtln("\nLdapDemo: getObj after store object, using Admin: (" + getObj.getClass().getName() + "): \"" + getObj
				+ "\"");

		// Retrieve attributes associated with stored Java object
		// Deprecated, since we don't store user attributes.
		// Maybe someday we will.
		entry = client.getUserObjectAttributes(smithUid, // authentication UID
				smithPswd, // password
				smithUid, // UID where the object is stored
				"testObjectA", // object name
		...
		getObj = client.getUserObject(smithUid, // authentication UID
				smithPswd, // password
				smithUid, // UID where the object is stored
				"testObjectA"); // object name
		prtln("\nLdapDemo: getObj after store null: (" + getObj.getClass().getName() + "): \"" + getObj + "\"");

		// Remove the object
		client.removeUserObject(smithUid, // authentication UID
				smithPswd, // password
				smithUid, // UID where the object is stored
				"testObjectA"); // object name
		...
		// NOT RECOMMENDED!
		client.renameUserEntry(smithUid, // old UID
				"ysmith"); // new UID
		prtall("LdapDemo: after rename ssmith to ysmith", client, "DLESEnameLast=*smith*");

		// Rename it back
		client.renameUserEntry("ysmith", // old UID
				smithUid); // new UID
		prtall("LdapDemo: after rename ysmith back to ssmith", client, "DLESEnameLast=*smith*");
		...
		testList("lists/open/testa/testb/geology", // list name
		...
		testList("lists/closed/secretList", // list name
				client, // the LdapClient to use for testing
				owensUid, // UID of list owner
				owensPswd, // password
				smithUid, // the UID to use for testing
		...
		boolean existsFlag;
		existsFlag = client.userExists(smithUid); // UID to test
		...
		if (!hitexc)
			throw new Exception("remove entry again should have failed");
	 ...
	 * Tests mail lists.
	 */
	void testList(String listName, // the list name
		...
		client.createList(listName, // name of the list
				owensUid); // UID of list owner
		prtall("LdapDemo: after create list " + listName, client, "dleseListName=*");

		try {
			// Add name to list, using self for authorization
			// This will succeed for open lists,
			// and will fail for closed lists.
			client.addListName(smithUid, // authentication UID
					smithPswd, // password
					listName, // name of the list
					smithUid); // UID to add to the list
			prtall("LdapDemo: after self add name to list " + listName, client, "dleseListName=*");

			// Remove name from list, using self for authorization
			client.removeListName(smithUid, // authentication UID
					smithPswd, // password
					listName, // name of the list
					smithUid); // UID to remove from the list
			prtall("LdapDemo: after self remove name from list " + listName, client, "dleseListName=*");
		} catch (Exception exc) {
			prtln("\nLdapDemo: testList: after self add/removed: failed. exc: " + exc + "\n");
		}

		// Again add name to list, using the list owner for authorization
		client.addListName(owensUid, // authentication UID
				owensPswd, // password
				listName, // name of the list
				smithUid); // UID to add to the list
		prtall("LdapDemo: after owner add name to list " + listName, client, "dleseListName=*");

		// Get all members of the list, as a user
		String[] uids;
		uids = client.getListNames(smithUid, // authentication UID
				smithPswd, // password
				listName); // name of the list
		prtln("LdapDemo: display list uids, as user auth: entries found: " + uids.length);
		for (ii = 0; ii < uids.length; ii++) {
			prtln("    uid: \"" + uids[ii] + "\"");
		}

		// Get all members of the list, as the list owner
		uids = client.getListNames(owensUid, // authentication UID
				owensPswd, // password
				listName); // name of the list
		prtln("LdapDemo: display list uids, as owner auth: entries found: " + uids.length);
		for (ii = 0; ii < uids.length; ii++) {
			prtln("    uid: \"" + uids[ii] + "\"");
		}

		// Get all members of the list, as the admin
		uids = client.getListNames(adminUid, // authentication UID
				adminPswd, // password
				listName); // name of the list
		prtln("LdapDemo: display list uids, as admin auth: entries found: " + uids.length);
		for (ii = 0; ii < uids.length; ii++) {
			prtln("    uid: \"" + uids[ii] + "\"");
		}
		...
		client.removeListName(owensUid, // authentication UID
				owensPswd, // password
				listName, // name of the list
				smithUid); // UID to remove from list
		prtall("LdapDemo: after owner remove name from list " + listName, client, "dleseListName=*");

		// Remove entire list
		client.removeEntireList(listName); // name of the list
		prtall("LdapDemo: after remove entire list " + listName, client, "dleseListName=*");
	}
	 ...
	 *            The LdapClient
	 * @param filter
	 *            The filter expression to use. To print <b>all</b> entries in the
	 *            database, use: <code>filter = "objectclass=*";</code>
	 */

	void prtall(String msg, LdapClient client, String filter) throws LdapException {
		if (filter == null)
			filter = "objectclass=*";
		LdapEntry[] entries = client.search(null, // base = null: start at top of DB tree
				filter, null, // attrNames = null: return all attributes
				0); // maxres = 0 implies return all results
	 ...
	 */
	void prtentries(String msg, LdapEntry[] entries) {
