...
/**
 * Performs XML validation and gzipping of the pre-compiled HTTP response
 * content. A caller wishing to invoke this filter should use an HTTP request
 * parameter 'rt' set to 'validate.'
 * <p>
 *
 * This is implemented using a servlet response Filter that checks the content
 * for validity against an XML schema or DTD, which must be referenced in the
 * content itself. This filter also perform gzip compression of all output, if
 * supported by the browser. See More Servlets and JavaServer Pages, chapter 9
...
public final class XMLValidationFilter extends FilterCore {
	...
	/**
	 * Performs XML validation and gzipping of the HTTP response content. A caller
	 * wishing to envoke this filter should use an HTTP request parameter 'rt' set
	 * to 'validate.'
	 *
	 * @param request
	 *            The request
