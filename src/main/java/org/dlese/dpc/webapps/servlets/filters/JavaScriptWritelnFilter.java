...
/**
 * Converts JSP, servlet, HTML or other output into a JavaScript writeln
 * statement or places it into a JavaScript variable. Activated by specifying a
 * query parameter - if none is supplied, the output is left unchanged (except
 * for Gziping). This Filter may be used to create a JavaScript proxy for
 * conetnt. Gzip compresses the response if the browser supports it.
 * <p>
 *
 * To use, configure the filter to be activated for the pages you want in your
 * web.xml configuration. Then send a request to the page with the one or more
 * of the following query parameters:
 * <p>
 *
 * rt=jswl - Instructions the filter to output the content as a document.write(
 * content ); statement<br>
 * rt=jsvar - Instructs the filter to place the content into a JavaScript
 * variable named 'jsvar'.<br>
 * jsvarname=myvarname - (optional) When jsvar is used, this instructs the
 * filter to place the ouput into the a variable by the name provided. If not
 * supplied, defaults to 'jsvar'.
 * <p>
 *
 * Examples:
 * <p>
 ...
 * @version $Id: JavaScriptWritelnFilter.java,v 1.9 2009/03/20 23:34:01 jweather
 *          Exp $
 */
public final class JavaScriptWritelnFilter extends FilterCore {
	...
	/**
	 * Converts the http response into a JavaScript output if parameter rt=jsvar or
	 * rt=jswl. Gzip compresses the response regardless.
	 *
	 * @param request
	 *            The request
	 * @param response
	...
	public void init(FilterConfig config) throws ServletException {
		 ...
		 * debug = true; //prtln("Outputting debug info"); } } catch (Throwable e) {} }
		 */
	}
