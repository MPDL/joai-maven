...
/**
 * Performs post-processing of an XML stream and gzips the response if supported
 * by the browser. Supported post-processing actions include XML validation, XML
 * transform to localized XML, and XML to JSON serialization.
 * <ul>
 * <li>To invoke XML validation, pass HTTP request parameter rt=validate.</li>
 * <li>To invoke XML transform, pass HTTP request parameter transform=localize
 * (localize only value currently supported).</li>
 * <li>To invoke XML to JSON serialization, pass HTTP request parameter
 * output=json and optionally callback=myCallbackFn to wrap the response in a
 * callback function named by the paramater value.</li>
 *
 * </ul>
 * <p>
 *
 * The XML validation is implemented using a servlet response Filter that checks
 * the content for validity against an XML schema or DTD, which must be
 * referenced in the content itself.
 * <p>
 *
 * The JSON serialization is implemented using the json.org XML serializer and
 * appropriate classes must be included in your classpath. See
 * <a href="http://www.json.org/javadoc/"> http://www.json.org/javadoc/</a> .
 *
...
public class XMLPostProcessingFilter extends FilterCore {
	...
	public final void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		if (rt != null && rt.equals("validate")) {
			...
			if (message != null) {
				...
				ouput.append(
						"ERROR: This XML is NOT VALID:</font><br><br>\n" + OutputTools.htmlEncode(message) + "<br>");
			} else {
		...
		// Handle JSON serialization, do it...
			...
			try {
				...
				try {
					responseContent = new JSONStringer().object().key("JSON-ERROR").value(t.getMessage()).endObject()
							.toString();
				} catch (Throwable t2) {
		...
		// Handle XML Transform, do it...
			...
			try {
				...
				try {
					responseContent = "<DDSWebService><error>Error transforming XML service response: " + t.getMessage()
							+ "</error></DDSWebService>";
				} catch (Throwable t2) {
				}
			}
	 ...
	 *
	 * @param config
	 *            The FilterConfig object that holds the ServletContext and init
	 *            information.
	 * @exception ServletException
	 *                If an error occurs
	 */
	public void init(FilterConfig config) throws ServletException {
		/*
		 * if (context == null) { try { context = config.getServletContext(); if
		 * (((String) context.getInitParameter("debug")).toLowerCase().equals("true")) {
		 * debug = true; //prtln("Outputing debug info"); } } catch (Throwable e) {} }
		 */
	}
