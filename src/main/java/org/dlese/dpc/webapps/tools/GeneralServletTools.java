...
public final class GeneralServletTools {
	...
	/**
	 * Returns the context path for the webapp, for example '/dds' or '/' for the
	 * root context.
	 *
	 * @param servletContext
	 *            The ServletContext object for the webapp
	 * @return The context path for the webapp, for example '/dds' or '/', or null
	...
	/**
	 * Gets the URL that refers to the current server (scheme, hostname and port),
	 * for example "http://www.dlese.org" or "http://host:8080" or
	 * "http://localhost:8080".
	 *
	 * @param req
	 *            The request.
	...
	/**
	 * Gets the URL that refers to the current server and servlet context, for
	 * example "http://www.dlese.org/dds" or "http://domain.org:8080/context" or or
	...
	/**
	 * Gets the query string supplied in the request, for example
	 * "q=ocean&s=0&n=10". This is the same as
	 * {@link javax.servlet.http.HttpServletRequest#getQueryString()}, but works
	 * even if the request has been forwarded.
	 *
	 * @param request
	 *            The request
	...
	/**
	 * Reconstructs the URL the client used to make the request, even if the page
	 * has been forwarded for example via struts (action.do). The returned URL
	...
	public static StringBuffer getRequestURL(HttpServletRequest req) {
		...
		// Make sure we have the original request Object, not the wrapper, so we get the
		// original requested URI, not the URI it was forwarded to!
	 ...
	 * string in the first line of the HTTP request. Works properly, even if the
	 * page has been forwarded for example via struts (action.do).
	 *
	 * @param req
	 *            The request.
	 ...
	 * <p>
	 *
	 * Note: the HttpServlet init() (super.init()) method must be called prior to
	 * using this method, else a ServletException is thrown.
	 *
	 * @param fname
	 ...
	 */
	public final static String getAbsolutePath(String fname, ServletContext servletContext) throws ServletException {
		String fullname;
	...
	public final static String getAbsolutePath(String fname, String docRoot) {
		...
		if (fname.startsWith("/") || fname.matches("[A-Z]:.+") || fname.startsWith(docRoot) || fname.startsWith("../")
