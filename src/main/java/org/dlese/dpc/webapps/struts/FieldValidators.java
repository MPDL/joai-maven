...
/**
 * Static methods used in the Struts validation framework that implement custom
 * validation actions. The static methods are configured in validator-rules.xml
 * and applied to a specific form in your app using validation.xml, which are
 * typically located in the WEB-INF directory of your web application.
 *
...
public class FieldValidators {
	...
	/**
	 * Validates that the field value is an existing directory on the server that
	 ...
	 */
	public static boolean validateIsDirectory(Object bean, ValidatorAction va, Field field, ActionMessages messages,
			Validator validator, HttpServletRequest request, ServletContext servletContext) {
	 ...
	 */
	public static boolean validateNamespaceIdentifier(Object bean, ValidatorAction va, Field field,
			ActionMessages messages, Validator validator, HttpServletRequest request, ServletContext servletContext) {
		// Get the value the user entered:
		String repositoryIdentifier = ValidatorUtils.getValueAsString(bean, field.getProperty());
		boolean isValid = (repositoryIdentifier == null || repositoryIdentifier.length() == 0
	...
	/**
	 * Validates that the field value is a directory that is not already configured
	 * in the repository (RepositoryManager). Checks the request parameter 'edit'
	 * for the previous directory setInfo, if editing.
	 *
	 * @param bean
	 *            The Struts bean
	 * @param va
	 ...
	 * @return True if the directory exists
	 */
	public static boolean validateDirectoryNotInRepository(Object bean, ValidatorAction va, Field field,
			ActionMessages messages, Validator validator, HttpServletRequest request, ServletContext servletContext) {
