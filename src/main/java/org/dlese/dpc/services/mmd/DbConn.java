...
public class DbConn {
	...
	public DbConn(int bugs, String dbUrl) throws MmdException {
		...
		if (ix >= 0)
			printableUrl = printableUrl.substring(0, ix + pswdtag.length()) + "(omitted)";
		...
		try {
			...
			mkerror("could not open db connection to URL \"" + printableUrl + "\"  exc: " + sqe);
		}
		if (bugs >= 1)
	...
	public void closeDb() throws MmdException {
		...
		if (conn != null) {
			...
			try {
				...
				mkerror("could not close db connection to URL \"" + printableUrl + "\"");
			}
			conn = null;
		}
	}
	...
	public String getDbString(String sqlstg) throws MmdException {
		...
		Object[][] resmat = getDbTable(sqlstg, new String[]{"string"}, // types
		...
		if (bugs >= 10)
			prtln("getDbString: sqlstg: \"" + sqlstg + "\"  result: \"" + resval + "\"");
		return resval;
	}
	...
	public void updateDb(String sqlstg) throws MmdException {
		if (bugs >= 10)
			prtln("updateDb: sqlstg: \"" + sqlstg + "\"");

		logit("updateDb: sqlstg: \"" + sqlstg + "\"");

		int upnum = 0;
		try {
			upnum = stmt.executeUpdate(sqlstg);
		} catch (SQLException sqe) {
			mkerror("updateDb: db update failed. exc: \"" + sqe + "\" sqlstg: " + sqlstg);
		}
		/// if (upnum != 1)
	...
	/**
	 * Retrieves a table from the DB: each array row represents one DB row, and each
	 * array column represents a DB column.
	 *
	 * @param sqlstg
	 *            The SQL string used to query the DB.
	 * @param types
	 *            An array of the types of the returned columns. Valid values are:
	 *            <table border=1>
	 *            <tr>
	 *            <th>Specified type</th>
	 *            <th>Returned java type</th>
	 *            </tr>
	 *            <tr>
	 *            <td>"boolean"</td>
	 *            <td>Boolean</td>
	 ...
	 *            <tr>
	 *            <td>"date"</td>
	 *            <td>Long (milliseconds since 1970)</td>
	 *            </tr>
	 *            <tr>
	 *            <td>"double"</td>
	 *            <td>Double</td>
	...
	public Object[][] getDbTable(String sqlstg, String[] types, boolean allownull) // allow nulls in any field
			throws MmdException {
		int ii, jj;
		...
		try {
			...
			if (rs == null)
				mkerror("getDbTable: empty db result from sqlstg: \"" + sqlstg + "\"");
					...
					try {
						...
						if (typestg.equals("boolean"))
							...
							if (tmpts == null)
								...
								rowvals[jj] = new Long(tmpts.getTime());
							...
							mkerror("getDbTable: unknown type");

						if ((!allownull) && rowvals[jj] == null)
							mkerror("getDbTable: found a null value" + "  in row " + (reslist.size() + 1) + "\n"
									+ "  in column " + ix + " from query:\n" + "  " + sqlstg);
					} catch (SQLException sqe) {
						mkerror("getDbTable: could not get db info for" + " (1-origin) field: " + ix + " ("
								+ rs.getObject(ix) + ")  from sqlstg: \"" + sqlstg + "\"  exc: " + sqe);
					}
				} // for jj
				reslist.add(rowvals);
			} // while rs.next
			...
			mkerror("getDbTable: could not get db info from sqlstg: \"" + sqlstg + "\"  exc: " + sqe);
		} finally {
			if (rs != null) {
				try {
					rs.close();
	...
	public static String dbstringcom(Timestamp ts) throws MmdException {
		if (ts == null)
			return dbstringcom((String) null);
		else
			return dbstringcom(ts.toString());
	}

	public static String dbstring(Timestamp ts) throws MmdException {
		if (ts == null)
			return dbstring((String) null);
		else
			return dbstring(ts.toString());
	}
	...
	/**
	 * Creates a string suitable for MySQL commands, including escaped quotes and
	 * the surrounding single quotes.
	 *
	 * Throws MmdException if the input string contains an invalid char, unless
	 * forceit == true in which case invalid chars are ouput as format "0xabcd".
	 */
			...
			for (ii = 0; ii < stg.length(); ii++) {
				...
				if (legalchars.indexOf(cc) < 0 && !(cc >= 0xa0 && cc <= 0xff)) {
					if (forceit) {
						...
						mkerror("invalid char (dec " + (int) cc + ") in db update string: \"" + stg + "\"");
				} else if (cc == '\\')
					bufa.append("\\\\");
				else if (cc == '\'')
					bufa.append("\\\'");
