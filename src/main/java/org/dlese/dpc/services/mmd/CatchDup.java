...
public class CatchDup {
	 ...
	 *
	 * @param c1
	 *            Checksum of the First Resource
	 * @param c2
	 *            Checksum of the Second Resource
	 * @param u1
	 *            Url of the First Resource
	 * @param u2
	 *            Url of the Second Resource
	 * @param id1
	 *            ID of the First Resource
	 * @param id2
	 *            ID of the Second Resource
	 * @param exc
			...
			try {
				if (!documentIsLoaded) {
					...
					if (urlConnection instanceof java.net.HttpURLConnection) {
						...
						istm = TimedURLConnection.getInputStream(URI.create(this.exclusion_file).toURL(),
								timeOutPeriod);
					} else {
				...
				// If both IDs are in the non-dups file as a whole, dig deeper to see if they
				// are in the same non-dups group:
						...
						for (Iterator iter = list.iterator(); iter.hasNext();) {
							...
							if (ids != null) {
								...
								if ((found1 == true) && (found2 == true)) {
									...
									prtln("both IDs were in the non-dups list, returning false. id1:" + id1 + " id2:"
											+ id2);
									return false;
								}
							}
				...
				prtlnErr("Error reading the IDMapper dup exclusion file " + this.exclusion_file + " Reason: "
						+ " The URL Connection timed out.");
			} catch (IOException ioe) {
				prtlnErr(
						"Error reading the IDMapper dup exclusion file. The UrlConnection threw an IOException while attempting to connect to "
								+ this.exclusion_file + " : " + ioe);
