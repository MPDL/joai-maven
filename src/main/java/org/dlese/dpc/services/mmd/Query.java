...
public class Query {
	...
	/**
	 * The queryType parameter for the findDups method: only return results <b> not
	 * </b> from the collection collKey.
	 */
	...
	/**
	 * Returns the collection name associated with a collection key, or null if none
	 * found.
	 *
	 * @param collKey
	 *            DESCRIPTION
	...
	public String getCollectionName(String collKey) throws MmdException {
		String res = dbconn
				.getDbString("SELECT collName FROM idmapCollection" + " WHERE collKey = " + dbconn.dbstring(collKey));
		return res;
	}

	/**
	 * Returns the directory containing the XML records of the given collection key,
	...
	public String getDirectory(String collKey) throws MmdException {
		String res = dbconn
				.getDbString("SELECT dirPath FROM idmapCollection" + " WHERE collKey = " + dbconn.dbstring(collKey));
		return res;
	}
	...
	public MmdRec getMmdRec(String collKey, String id) throws MmdException {
		...
		try {
			dbmat = dbconn.getDbTable(
					"SELECT fileName, primaryUrl, status, firstAccessionDate,"
							+ " lastMetaModDate, recCheckDate, primaryContent, primarycontentType" + " FROM idmapMmd"
							+ " WHERE collKey = " + dbconn.dbstring(collKey) + " AND id = " + dbconn.dbstring(id),
					new String[]{"string", "string", "string", "date", "date", "date", "string", "string"}, // types
					true);// allow nulls in fields
		...
		if (dbmat == null) {
			dbmat = dbconn.getDbTable(
					"SELECT fileName, primaryUrl, status, firstAccessionDate," + " lastMetaModDate, recCheckDate"
							+ " FROM idmapMmd" + " WHERE collKey = " + dbconn.dbstring(collKey) + " AND id = "
							+ dbconn.dbstring(id),
					new String[]{"string", "string", "string", "date", "date", "date"}, // types
					true);// allow nulls in fields
		}
		...
		if (dbmat.length == 1) {
			...
			String metastyle = dbconn.getDbString(
					"SELECT metastyle FROM idmapCollection" + " WHERE collKey = " + dbconn.dbstring(collKey));

			// Get error messages from most recent check.
			dbmat = dbconn.getDbTable(
					"SELECT msgType, fileName, xpath, urllabel, url," + " msg, auxinfo" + " FROM idmapMessages"
							+ " WHERE collKey = " + dbconn.dbstring(collKey) + " AND id = " + dbconn.dbstring(id)
							+ " AND recCheckDate = " + dbconn.dbstring(new Timestamp(recCheckDate)),
					new String[]{"string", "string", "string", "string", "string", "string", "string"}, true);// allow
																												// nulls
																												// in
																												// fields
			...
			if (dbmat.length > 0) {
				...
				for (ii = 0; ii < dbmat.length; ii++) {
					...
					int msgType = DpcErrors.getType(msgtypestg);
					warnings[ii] = new MmdWarning(msgType, errfilename, xpath, urllabel, url, msg, auxinfo);
				}
			}

			mmdRec = new MmdRec(collKey, id, fileName, status, metastyle, firstAccessionDate, lastMetaModDate,
					recCheckDate, primaryContent, primarycontentType, warnings);
		} // if dbmat.length == 1
		else
			mmdRec = null;
	...
	/**
	 * Updates the status of a single record. The new status must be one of the
	 * legal values defined in {@link MmdRecord MmdRecord}.
	 *
	 * @param collKey
	 *            The collection key.
	 * @param id
	...
	public void setStatus(String collKey, String id, String newStatus) throws MmdException {
		...
		dbconn.updateDb("UPDATE idmapMmd SET status = " + dbconn.dbstring(newStatus) + " WHERE collKey = "
				+ dbconn.dbstring(collKey) + " AND id = " + dbconn.dbstring(id));
	}
	 ...
	 *            <li>QUERY_OTHER: only return results <b> not </b> from collection
	 *            collKey
	 ...
	 * @return An array of MmdRecod, each of which represents a resource from a
	 *         different collection having possibly identical content.
	 * @exception MmdException
	 *                DESCRIPTION
	 */
	...
	public MmdRec[] findDups(int queryType, String collKey, String id, String exc) throws MmdException {
		...
		MmdRec[] resmat = null;
		// String exc = new
		// String("http://www.dlese.org/Metadata/documents/xml/nondups.xml");

		dbmat = dbconn.getDbTable(
				"SELECT primaryUrl, primaryChecksum FROM idmapMmd" + " WHERE collKey = " + dbconn.dbstring(collKey)
						+ " AND id = " + dbconn.dbstring(id),
		...
		/// + "\" not found in collection \"" + collKey + "\"");
			...
			if (checksum != 0) {
				...
				dbmat = dbconn.getDbTable(
						"SELECT collKey, id, primaryUrl, primaryChecksum FROM idmapMmd" + " WHERE ( primaryChecksum = "
								+ dbconn.dbstring(checksum) + " OR primaryUrl = " + dbconn.dbstring(primaryUrl) + " ) "
								+ qtypestg,
				...
				qtypestg += " AND hasFile!=0";
				dbmat = dbconn.getDbTable(
						"SELECT collKey, id, primaryUrl, primaryChecksum FROM idmapMmd" + " WHERE primaryUrl = "
								+ dbconn.dbstring(primaryUrl) + qtypestg,
						new String[]{"string", "string", "string", "long"}, // types
						false);// allow nulls in fields
	...
	static void mkerror(String msg) throws MmdException {
