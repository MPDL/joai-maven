...
/**
 * Represents a resource to be scanned, and the results from that scan. Each
 * ResourceDesc represents one resource but may have multiple PageDescs: for the
 * primary URL, mirrors, related sites, etc.
 */
...
class ResourceDesc {
	 ...
	 *            The collection key.
	 * @param fileName
	 *            The full file path of the id's XML file.
	 * @param pages
	 *            The PageDescs associated with this resource.
	 */

	ResourceDesc(String collKey, String metastyle, String dirPath, String fileName, String[] urlOnlyTests)
	...
	public String toString() {
		return "id: " + id + "  primary URL: \"" + getPrimaryUrl() + "\"  num warnings: " + numWarnings();
	}
	...
	int getUrlOnlyTestsLength() {
		int ires = 0;
		if (urlOnlyTests != null)
			ires = urlOnlyTests.length;
		return ires;
	}
	...
	boolean testPrimary() {
		...
		if (numpages > 0 && pages[0].urllabel != null && pages[0].urllabel.equals("primary-url"))
			bres = true;
		return bres;
	}
	...
	Iterator warningIterator() {
		Iterator iter = null;
		if (warnBuf != null)
			iter = warnBuf.iterator();
		return iter;
	}

	boolean hasSevereError() {
		boolean bres = false;
		if (warnBuf != null && warnBuf.hasSevereError())
			bres = true;
		return bres;
	}
