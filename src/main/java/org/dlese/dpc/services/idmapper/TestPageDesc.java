...
class TestPageDesc {
	...
	TestPageDesc(String[] args) {
		...
		int timeout = 0;
		try {
			timeout = Integer.parseInt(timeoutstg);
		...
		try {
			...
			while (true) {
				...
				if (urlstg.length() > 0 && !urlstg.startsWith("#")) {
					...
					ResourceDesc rsd = new ResourceDesc(collKey, metastyle, dirPath, fileName, urlOnlyTests);

					PageDesc page = new PageDesc(rsd, "test/xpath", "primary-url", urlstg, PageDesc.CKSUMTYPE_STD);
					rsd.addPage(page);

					page.processPage(bugs, threadId, timeout);
					prtln("\nTestPageDesc: respcode: " + page.respcode + " (" + DpcErrors.getMessage(page.respcode)
							+ ")" + "  url: \"" + urlstg + "\"" + "  pagewarning: " + page.pagewarning);
