...
public class Idmap {
	 ...
	 * 
	 *            props=[properties file name] # Required dir=[input directory] #
	 *            Required coll=[metadata format]/[collection key] # Optional, may
	 *            be repeated email=normal | force | print | [email address]
	 * 
	 *            Testing/debug parms ... bugs=[debugLevel] # Set debug level.
	 *            default is 0. checknet=[y/n] # Check network before beginning.
	 *            Default is y maxthreads=[num] # Force maxthreads. Default is 0:
	 *            use the props file number
	 * 
	 *            Example: to check links on all active collections: java
	 *            org.dlese.dpc.services.idmapper.Idmap props=somePropsFile
	 *            dir=/export/dlese/records Example: to test two individual
	 *            collections (adl and dcc), both using adn format, and print the
	 *            output: java org.dlese.dpc.services.idmapper.Idmap
	 *            props=somePropsFile dir=/export/dlese/records coll=adn/adl
	 *            coll=adn/dcc email=print
	 */
	 ...
	 *
	 * @param msg
	 *            the command line parameters required to run Idmapper from the
	 *            command line
	 */
		...
		try {
			...
			for (iarg = 0; iarg < args.length; iarg++) {
				...
				if (arg.startsWith("bugs=")) {
					...
					if (collParms == null) {
						...
						System.arraycopy(collParms, 0, tmpParms, 0, collParms.length);
			...
			if (forceMaxThreadsParm != null) {
				try {
					...
					badparms("Invalid \"maxthreads\" spec: \"" + forceMaxThreadsParm + "\"");
				}
			}
			...
			if (checkNetFlagParm != null) {
				...
				if (tmpstg.equals("n") || tmpstg.equals("no")) {
					...
					badparms("Invalid \"checknet\" spec: \"" + checkNetFlagParm + "\"");
				}
			}
			...
			if (propsfile.indexOf("://") < 0) {
				if (propsfile.startsWith("/")) {
					...
					propsfile = "file://" + System.getProperty("user.dir") + "/" + propsfile;
				}
			}
			propsdoc = new XMLDoc(propsfile, false,
					// validating
					false,
					// namespaceAware
					false);
			...
			if (collParms == null) {
				...
				dbmat = dbconn.getDbTable(
						"SELECT collKey, metaStyle, dirPath FROM idmapCollection" + " WHERE collActive = "
								+ dbconn.dbstring(1),
			...
			for (ii = 0; ii < dbmat.length; ii++) {
				...
				try {
					checkLinks(bugs, propsdoc, collKey, metastyle, curdate, basedirParm, subdir, emailType, emailParm,
							forceMaxThreads, checkNetFlag);

				} catch (IdmapException iex) {
					hitError = true;
					prtln("Idmap.const: caught Exception for:" + "  collKey: \"" + collKey + "\"" + "  metastyle: \""
							+ metastyle + "\"" + "  basedirParm: \"" + basedirParm + "\"" + "  subdir: \"" + subdir
							+ "\"" + "  Exception: " + iex);
					iex.printStackTrace();
	...
	private void openDB(XMLDoc propsdoc) throws IdmapException, XMLException {
		...
		try {
			...
			mkerror("db driver not found.  Insure \"" + dbClass + "\" is in the CLASSPATH.  exc: " + cnf);
		} catch (InstantiationException iex) {
			mkerror("db driver not found.  Insure \"" + dbClass + "\" is in the CLASSPATH.  exc: " + iex);
		} catch (IllegalAccessException iex) {
			mkerror("db driver not found.  Insure \"" + dbClass + "\" is in the CLASSPATH.  exc: " + iex);
		}
		dbUrl = "jdbc:mysql://" + dbHost + ":" + dbPort + "/" + dbName + "?autoReconnect=true&user=" + dbUser
		// + "?user=" + dbUser
				+ "&password=";
		...
		try {
			...
			mkerror("could not open db connection to URL \"" + printableUrl + "\"  exc: " + mde);
		}
		if (bugs >= 1) {
	...
	private void closeDb() throws IdmapException {
		if (dbconn != null) {
			...
			try {
				...
				mkerror("could not close db connection to URL \"" + printableUrl + "\"  exc:" + mde);
			}
			dbconn = null;
		}
	}
	...
	/**
	 * Performs all link checking: Create list of ResourceDescs, call runthreads to
	 * retrieve them all, store the results.
	 *
	 * @param bugs
	 *            debug level
	 * @param propsdoc
	 *            XMLDoc of Properties file.
	 * @param collKey
	 *            The collection key.
	 * @param metastyle
	 *            metadata style: one of the MS_ values defined in
	 *            {@link org.dlese.dpc.services.mmd.MmdRec MmdRec}.
	 * @param curdate
	 *            The current data, in milliseconds since 1970.
	 * @param basedirParm
	 *            The base directory containing subdir
	 * @param subdir
	 *            The subdirectory under basedirParm containing the XML metadata
	 *            files.
	 * @param emailType
	 * @param emailParm
	...
	void checkLinks(int bugs, XMLDoc propsdoc, String collKey, String metastyle, long curdate, String basedirParm,
			String subdir, int emailType, String emailParm, int forceMaxThreads, boolean checkNetFlag)
		...
		while (tok.hasMoreTokens()) {
			...
			if (iday == -1) {
				mkerror("Property \"emailDays\" has an invalid day: \"" + daystg + "\"");
			}
			emailDays[iday] = true;
		}
		...
		if (cksumtypestg.equals("standard")) {
			...
			mkerror("Property \"cksumtype\" is invalid: \"" + cksumtypestg + "\"");
		}
		...
		if (checkNetFlag) {
			String[] netcheckurls = propsdoc.getXmlFields(0, 0, "networkcheck/url");
			checkNetwork(bugs, maxThreads, timeoutSeconds, netcheckurls, emailHost, emailFrom, emailType, emailAddrs,
					emailSubject);
		}
		...
		try {
			String tmpstg = dbconn.getDbString(
					"SELECT collKey FROM idmapCollection" + " WHERE collKey = " + dbconn.dbstring(collKey));
		} catch (MmdException mex) {
			dbconn.updateDb("INSERT INTO idmapCollection" + " (collKey, collActive, collName, metastyle, dirPath)"
					+ " VALUES(" + dbconn.dbstringcom(collKey) + dbconn.dbstringcom(0)
					// initially not active
					+ dbconn.dbstringcom(collKey)
					// temp collName = collKey
					+ dbconn.dbstringcom(metastyle) + dbconn.dbstring(subdir) + ")");
		}
		...
		ResourceDesc[] rsds = createResourceList(bugs, collKey, metastyle, dirPath, cksumtype);
		...
		if (rsds.length == 0) {
			...
			msgbuf.append(subj + "\n");
			msgbuf.append("No XML files were found for collection: " + collKey + "\n" + "in the specified directory: "
					+ dirPath);
			try {
				sendemail(emailHost, emailFrom, emailAddrs, subj, msgbuf.toString());
			} catch (Exception exc) {
			...
			// various DB tables.
			storeResults(collKey, metastyle, curdate, historyDays, ivitalCutoff, rsds);

			logit("returned from storeResults");

			// If it's time to send email, do so
			boolean sentEmail = testEmailTime(bugs, collKey, metastyle, dirPath, rsds.length, curdate, emailDays,
					emailHost, emailFrom, emailType, emailAddrs, emailSubject);

			// If we sent email, update the lastEmailDate
			if (sentEmail) {
				dbconn.updateDb("UPDATE idmapCollection SET lastEmailDate = " + dbconn.dbstring(new Timestamp(curdate))
						+ " WHERE collKey = " + dbconn.dbstring(collKey));
			}

			// Find the num of warnings we issued.
			Object[][] dbmat = dbconn.getDbTable(
					"SELECT COUNT(*) FROM idmapMessages" + " WHERE collKey = " + dbconn.dbstring(collKey)
							+ " AND recCheckDate = " + dbconn.dbstring(new Timestamp(curdate)),
			...
			if (dbmat != null && dbmat.length > 0) {
				try {
					...
					mkerror("checkLinks: invalid numwarns: \"" + dbmat[0][0] + "\"");
				}
			}

			// Update idmapCollection to show check date and num warnings.
			dbconn.updateDb("UPDATE idmapCollection SET collCheckDate = " + dbconn.dbstring(new Timestamp(curdate))
					+ ", numResources = " + dbconn.dbstring(rsds.length) + ", numWarnings = "
					+ dbconn.dbstring(numwarns) + " WHERE collKey = " + dbconn.dbstring(collKey));

			dbconn.updateDb("INSERT INTO idmapCheckDate" + " (collKey, collCheckDate)" + " VALUES("
					+ dbconn.dbstringcom(collKey) + dbconn.dbstring(new Timestamp(curdate)) + ")");
		}
		// else at least one XML file was found
	 ...
	 *            timeout value for sockets, in seconds
	 * @param emailHost
	 *            host name of the email server
	 * @param emailFrom
	 *            "from" name to be used in email messages
	 * @param emailType
	 *            email cmd line parm: EMAIL_NORMAL, EMAIL_FORCE, EMAIL_PRINT, or
	 *            EMAIL_SINGLE
	 * @param emailAddrs
	 *            array of "to" names for email messages
	 * @param emailSubject
	 *            initial part of the email "Subject" line
	 * @param netcheckurls
	 *            DESCRIPTION
	 * @exception IdmapException
	 *                DESCRIPTION
	...
	void checkNetwork(int bugs, int maxThreads, int timeoutSeconds, String[] netcheckurls, String emailHost,
			String emailFrom, int emailType, String[] emailAddrs, String emailSubject) throws IdmapException {
		int ii;
		...
		for (ii = 0; ii < numpages; ii++) {
			rsds[ii] = new ResourceDesc("network check",
					// collKey
			...
			rsds[ii].addPage(
					new PageDesc(rsds[ii], "no xpath", "netcheck-url", netcheckurls[ii], PageDesc.CKSUMTYPE_STD));
		}
		...
		for (ii = 0; ii < numpages; ii++) {
			if (bugs >= 1) {
				...
				prtln("checkNetwork " + ii + ": respcode: " + page.respcode + "  resptime: " + page.resptime
						+ "  URL: \"" + page.urlstg + "\"");
			}
		...
		if (warnBuf.length() > maxdownfrac * rsds.length) {
			...
			StringBuffer msgbuf = new StringBuffer();
			msgbuf.append(subj + "\n");
			msgbuf.append("The network is not reliable right now.\n" + warnBuf.length() + " out of "
					+ netcheckurls.length + " reference sites were not accessible.\n");
			msgbuf.append("Error messages from the network check reference" + " sites are:\n\n");
	 ...
	 *            {@link org.dlese.dpc.services.mmd.MmdRec MmdRec}.
	 * @param dirPath
	 *            The path of the directory containing collKey's XML files.
	 * @param cksumtype
	 *            DESCRIPTION
	 * @return An array of Resoucedesc, each of which describes a resource (an Id)
	 *         to be checked. Each ResourceDesc may contain multiple PageDescs, one
	 *         for each URL: primary, mirrors, related sites, etc.
	 * @exception IdmapException
	 *                DESCRIPTION
	 * @exception XMLException
	 *                DESCRIPTION
	...
	ResourceDesc[] createResourceList(int bugs, String collKey,
			// collection key
			String metastyle, File dirPath, int cksumtype) throws IdmapException, XMLException {
		int ii;
		...
		String[] urlOnlyTests = new String[]{"http://meted.comet.ucar.edu/fogstrat/ic31/ic311/frames"};
		...
		for (ii = 0; ii < xmlfiles.length; ii++) {
			...
			ResourceDesc rsd = new ResourceDesc(collKey, metastyle, dirPath.getAbsolutePath(), file.getName(),
					urlOnlyTests);
			try {
				...
				rsd.addWarning(new Warning(DpcErrors.IDMAP_BAD_XML_FILE, errorid, rsd.getFullFileName(), null,
						// xpath
						null,
						// urllabel
						null,
		...
		ResourceDesc[] rsds = (ResourceDesc[]) rsdlist.toArray(new ResourceDesc[0]);
	...
	/**
	 * Gets the XML doc in: dirPath/rsd.fileName. Extracts all the important urls
	 * and email addresses, from the XML doc, checks them for syntax, and fills in
	 * the ResourceDesc from them.
	 * <p>
	 *
	 ...
	 *            {@link org.dlese.dpc.services.mmd.MmdRec MmdRec}.
	 * @param rsd
	 *            The ResourceDesc to be filled in.
	 * @param dupmap
	 *            HashMap used for checking for duplicate ids. # dupmap maps: String
	...
	private void checkResource(String metastyle, ResourceDesc rsd, HashMap dupmap, int cksumtype)
		...
		XMLDoc doc = new XMLDoc("file://" + rsd.getFullFileName(),
	...
	/**
	 * Returns the CRC32 checksum of a file. Used to determine if a resource XML
	 * file has changed.
	 *
	 * @param fileName
	 *            DESCRIPTION
	...
	long getFileChecksum(String fileName) throws IdmapException {
	...
	/**
	 * For metastyle MS_ADN ("adn") records, extracts all the important urls and
	 * email addresses, checks them for syntax, and fills in the ResourceDesc from
	 * them.
	 *
	 * @param rsd
	 *            The ResourceDesc to be filled in.
	 * @param doc
	 *            The XML document for the resource.
	 * @param dupmap
	 *            HashMap used for checking for duplicate ids. # dupmap maps: String
	 *            resourceId -> ResourceDesc.
	 * @param cksumtype
	...
	void check_adn(ResourceDesc rsd, XMLDoc doc, HashMap dupmap, int cksumtype) throws IdmapException, XMLException {
		int ii;
		String[] fields;
		String xpath;
		...
		String[][] syntaxXpaths = {{"keyword", "general/keywords/keyword@url"},
				{"life-org-url", "lifecycle/contributors/contributor/organization/instUrl"},
				{"meta-org-url", "metaMetadata/contributors/contributor/organization/instUrl"},
				{"meta-terms", "metaMetadata/termsOfUse@URI"},
				{"appr-org-url", "approval/contributors/contributor/organization/instUrl"},
				{"geo-bbsrc", "geospatialCoverages/geospatialCoverage/boundBox/bbSrcIDandURL@URL"},
				{"geo-bbplc", "geospatialCoverages/geospatialCoverage/boundBox/bbPlaces/IDandURL@URL"},
				{"geo-bbevt", "geospatialCoverages/geospatialCoverage/boundBox/bbEvents/IDandURL@URL"},
				{"geo-detsrc", "geospatialCoverages/geospatialCoverage/detGeos/detGeo/detSrcIDandURL@URL"},
				{"geo-detplc", "geospatialCoverages/geospatialCoverage/detPlaces/place/IDandURL@URL"},
				{"geo-detevt", "geospatialCoverages/geospatialCoverage/detEvents/event/IDandURL@URL"},
				{"time-url", "temporalCoverages/timeAndPeriod/periods/period/IDandURL@URL"}};
		...
		String[][] emailXpaths = {{"life-person-email", "lifecycle/contributors/contributor/person/emailPrimary"},
				{"life-person-email-alt", "lifecycle/contributors/contributor/person/emailAlt"},
				{"life-org-email", "lifecycle/contributors/contributor/organization/instEmail"},
				{"meta-person-email", "metaMetadata/contributors/contributor/person/emailPrimary"},
				{"meta-person-alt", "metaMetadata/contributors/contributor/person/emailAlt"},
				{"meta-org-email", "metaMetadata/contributors/contributor/organization/instEmail"}};
		...
		fields = doc.getXmlFields(0, 0, xpath);
		if (fields.length == 0) {
			rsd.addWarning(new Warning(DpcErrors.IDMAP_ACCESSION_DATE_MISSING, rsd.getId(), rsd.getFileName(), xpath,
					null, null, "Accession Date Missing", null));
			// url, msg, auxinfo
		}
	}
	...
	void check_dlese_anno(ResourceDesc rsd, XMLDoc doc, HashMap dupmap, int cksumtype)
			throws IdmapException, XMLException {
		int ii;
		String[] fields;
		String xpath;
		...
		String[][] syntaxXpaths1 = {{"org-url", "item/contributors/contributor/organization/instUrl"}};

		String[][] syntaxXpaths2 = {{"org-url", "annotation/contributors/contributor/organization/url"}};
		...
		// misc emails to be syntax checked ...
		String[][] emailXpaths1 = {{"person-email", "item/contributors/contributor/person/emailPrimary"},
		...
		String[][] emailXpaths2 = {{"person-email", "annotation/contributors/contributor/person/email"},
	...
	void check_news_opps(ResourceDesc rsd, XMLDoc doc, HashMap dupmap, int cksumtype)
			throws IdmapException, XMLException {
		int ii;
		String[] fields;
		String xpath;
		...
		try {
			...
			// min, max, retrieveFlag
			/*
			 * else { rsd.addWarning( new Warning( DpcErrors.IDMAP_NEWS_PAST_ARCHIVE_DATE,
			 * rsd.getId(), rsd.getFileName(), archDate, "announcementURL", fields[0],
			 * "Check date is past archive date", null)); }
			 */
			 ...
			 * rsd.getId(), rsd.getFileName(), archDate, "announcementURL", fields[0],
			 * "archive date could not be parsed", null));
			 */
		}

		// misc emails to be syntax checked ...
		String[][] emailXpaths = {{"person-email", "contributors/contributor/person/email"},
	...
	/**
	 * For metastyle "dlese_collect" records, extracts all the important urls and
	 * email addresses, checks them for syntax, and fills in the ResourceDesc from
	 * them.
	 *
	 * @param rsd
	...
	void check_dlese_collect(ResourceDesc rsd, XMLDoc doc, HashMap dupmap, int cksumtype)
			throws IdmapException, XMLException {
		int ii;
		String[] fields;
		String xpath;
		...
		fields = doc.getXmlFields(0, 0, xpath);
		checkUrls(xpath, "scope-url", fields, rsd, foundit ? 1 : 0,
				// min num to find
				0,
				// max num (no limit)
				true,
		...
		// misc urls to be syntax checked ...
		String[][] syntaxXpaths = {{"life-org-url", "lifecycle/contributors/contributor/organization/instUrl"},
				{"meta-org-url", "metaMetadata/contributors/contributor/organization/instUrl"},
				{"meta-terms", "metaMetadata/termsOfUse@URI"},
				{"appr-org-url", "approval/contributors/contributor/organization/instUrl"}};
		for (ii = 0; ii < syntaxXpaths.length; ii++) {
		...
		String[][] emailXpaths = {{"life-person-email", "lifecycle/contributors/contributor/person/emailPrimary"},
				...
				{"meta-person-alt", "metaMetadata/contributors/contributor/person/emailAlt"},
				{"meta-org-email", "metaMetadata/contributors/contributor/organization/instEmail"},
				{"appr-person-email", "approval/contributors/contributor/person/emailPrimary"},
				{"appr-person-email-alt", "approval/contributors/contributor/person/emailAlt"},
				{"appr-org-email", "approval/contributors/contributor/person/instEmail"}};
		for (ii = 0; ii < emailXpaths.length; ii++) {
			String urllabel = emailXpaths[ii][0];
			xpath = emailXpaths[ii][1];
			fields = doc.getXmlFields(0, 0, xpath);
	 ...
	 *
	 * @param xpath
	 *            The XML path of the field; used only in error messages.
	 * @param fields
	 *            The array of IDs: should have exactly one element.
	 * @param rsd
	 *            The corresponding ResourceDesc.
	 * @param dupmap
	 *            HashMap used for checking for duplicate ids. # dupmap maps: String
	 *            resourceId -> ResourceDesc.
	 * @param urllabel
	...
	boolean checkId(String xpath, String urllabel, String[] fields, ResourceDesc rsd, HashMap dupmap)
			throws IdmapException {
		boolean allok = true;
		if (fields.length < 1) {
			allok = false;
			rsd.addWarning(new Warning(DpcErrors.IDMAP_MISSING_FIELD, rsd.getId(), rsd.getFileName(), xpath, urllabel,
					null, "no id", null));
			// url, msg, auxinfo
		} else if (fields.length > 1) {
			allok = false;
			rsd.addWarning(new Warning(DpcErrors.IDMAP_MULT_FIELD, rsd.getId(), rsd.getFileName(), xpath, urllabel,
			...
			if (id.length() == 0 || id.length() > 100) {
				allok = false;
				rsd.addWarning(new Warning(DpcErrors.IDMAP_ID_SYNTAX, rsd.getId(), rsd.getFileName(), xpath, urllabel,
						null, "id syntax", null));
				// url, msg, auxinfo
			}
			ResourceDesc prevrsd = (ResourceDesc) dupmap.get(id);
			if (prevrsd != null) {
				rsd.addWarning(new Warning(DpcErrors.IDMAP_DUP_ID, rsd.getId(), rsd.getFileName(), xpath, urllabel,
	...
	/**
	 * Checks that an array of URLs is valid. If retrieveFlag is set, adds a
	 * PageDesc for each valid URL to rsd, so the pages will be retrieved later.
	 *
	 * @param xpath
	 *            The XML path of the field; used only in error messages.
	 * @param minnum
	 *            The minimum number of URLs that may be in fields.
	 * @param maxnum
	 *            The maximum number of URLs that may be in fields.
	 * @param retrieveFlag
	 *            If true, for each valid URL add a PageDesc to rsd so the page will
	 *            be retrieved later.
	 * @param fields
	 *            The array of URLs.
	 * @param rsd
	...
	boolean checkUrls(String xpath, String urllabel, String[] fields, ResourceDesc rsd, int minnum, int maxnum,
			boolean retrieveFlag, int cksumtype) throws IdmapException {
		int ii;
		int jj;
		boolean allok = true;
		...
		for (ii = 0; ii < fields.length; ii++) {
			...
			for (jj = 0; jj < urlstg.length(); jj++) {
				...
				if (okchars.indexOf(cc) < 0) {
					allok = false;
					rsd.addWarning(new Warning(DpcErrors.IDMAP_URL_SYNTAX, rsd.getId(), rsd.getFileName(), xpath,
							urllabel, urlstg, "invalid char", "offset: " + jj));
				}
			}
			...
			if (allok) {
				...
				try {
					...
					allok = false;
					rsd.addWarning(new Warning(DpcErrors.IDMAP_URL_SYNTAX, rsd.getId(), rsd.getFileName(), xpath,
							urllabel, urlstg, "malformed url", mfe.getMessage()));
				}
			}
			if (allok && retrieveFlag) {
				rsd.addPage(new PageDesc(rsd, xpath, urllabel, urlstg, cksumtype));
			}
		}
		// for ii
		return allok;
	 ...
	 *            The XML path of the field; used only in error messages.
	 * @param minnum
	 *            The minimum number of emails that may be in fields.
	 * @param maxnum
	 *            The maximum number of emails that may be in fields.
	 * @param fields
	...
	boolean checkEmails(String xpath, String urllabel, String[] fields, ResourceDesc rsd, int minnum, int maxnum)
			throws IdmapException {
		int ii;
		boolean allok = true;
		...
		for (ii = 0; ii < fields.length; ii++) {
			...
			if (!emailstg.equalsIgnoreCase("Unknown")) {
				try {
					...
					allok = false;
					rsd.addWarning(new Warning(DpcErrors.IDMAP_EMAIL_SYNTAX, rsd.getId(), rsd.getFileName(), xpath,
							urllabel, emailstg, iex.getMessage(), null));
				}
			}
		}
		return allok;
	...
	/**
	 * Checks that the specified email address is valid. If not, throws an
	 * IdmapException. <br>
	...
	void checkEmailAddress(String emailaddrparm) throws IdmapException {
		int ii;
		...
		String[] tlds = {"com", "edu", "gov", "info", "int", "mil", "net", "org", "ac", "ad", "ae", "af", "ag", "ai",
				"al", "am", "an", "ao", "aq", "ar", "as", "at", "au", "aw", "az", "ba", "bb", "bd", "be", "bf", "bg",
				"bh", "bi", "bj", "bm", "bn", "bo", "br", "bs", "bt", "bv", "bw", "by", "bz", "ca", "cc", "cd", "cf",
				"cg", "ch", "ci", "ck", "cl", "cm", "cn", "co", "cr", "cu", "cv", "cx", "cy", "cz", "de", "dj", "dk",
				...
				"li", "lk", "lr", "ls", "lt", "lu", "lv", "ly", "ma", "mc", "md", "mg", "mh", "mk", "ml", "mm", "mn",
				"mo", "mp", "mq", "mr", "ms", "mt", "mu", "mv", "mw", "mx", "my", "mz", "na", "nc", "ne", "nf", "ng",
				"ni", "nl", "no", "np", "nr", "nu", "nz", "om", "pa", "pe", "pf", "pg", "ph", "pk", "pl", "pm", "pn",
				"pr", "ps", "pt", "pw", "py", "qa", "re", "ro", "ru", "rw", "sa", "sb", "sc", "sd", "se", "sg", "sh",
				"si", "sj", "sk", "sl", "sm", "sn", "so", "sr", "st", "sv", "sy", "sz", "tc", "td", "tf", "tg", "th",
				"tj", "tk", "tm", "tn", "to", "tp", "tr", "tt", "tv", "tw", "tz", "ua", "ug", "uk", "um", "us", "uy",
		...
		for (ix = 0; ix < emailaddr.length(); ix++) {
			...
			if (okchars.indexOf(cc) < 0) {
				throw new IdmapException(
						"illegal char: \"" + cc + "\" (decimal " + ((int) cc) + ", at offset " + ix + ")");
			}
		}
		if (numat != 1) {
		...
		if (!foundtld) {
			throw new IdmapException("unknown suffix: \"" + suffix + "\"");
		}
	}

	/**
	 * For each ResourceDesc in rsds, starts a ScanThread to retrieve and analyze
	 * its pages, then joins all the threads and accumulates the warnings in each
	 * ResourceDesc.
	 * <p>
	 *
	 * We could use standard Java synchronization here with a thread pool and wait()
	 * and notify(). But this is much simpler and just as effective in the current
	 * situation. Java reclaims memory as threads exit, so even though the thdlist
	 * is large not all are active or taking memory.
	 *
	 * @param bugs
	 *            debug level
	 * @param maxThreads
	 ...
	 *            timeout value for sockets, in seconds
	 * @param rsds
	 *            An array of Resoucedesc, each of which describes a resource (an
	 *            Id) to be checked. The results from scanning are put in the rsds
	 *            entries.
	 * @exception IdmapException
	 *                DESCRIPTION
	 */

	void runThreads(int bugs, int maxThreads, int timeoutSeconds, ResourceDesc[] rsds) throws IdmapException {
			...
			for (ipage = 0; ipage < rsd.numpages; ipage++) {
				...
				while (thdgrp.activeCount() >= maxThreads) {
					if (bugs >= 50 && !sentwaitmsg) {
						sentwaitmsg = true;
						prtln("runThreads: waiting on thread:  irsd: " + irsd + "  ipage: " + ipage + "  \""
								+ page.urlstg + "\"");
					}
					try {
						Thread.sleep(100);
				...
				page.scanThread = new ScanThread(bugs, thdgrp, threadId, timeoutSeconds, page);

				if (bugs >= 10) {
					prtln("runThreads: start thread:" + "  irsd: " + irsd + "  ipage: " + ipage + "  \"" + page.urlstg
							+ "\"");
				}
				if (bugs >= 10) {
					prtmemorystats("runThreads: before start" + "  irsd: " + irsd + "  ipage: " + ipage, starttime);
				}
	...
	int harvestThreads(ResourceDesc[] rsds) throws IdmapException {
		int irsd;
		int ipage;
		...
		for (irsd = 0; irsd < rsds.length; irsd++) {
			...
			for (ipage = 0; ipage < rsd.numpages; ipage++) {
				PageDesc page = rsd.pages[ipage];
				if (bugs >= 50) {
					prtln("runThreads: before join thread:  irsd: " + irsd + "  ipage: " + ipage + "  \"" + page.urlstg
							+ "\"");
				}
				...
				if (page.scanThread != null) {
					if (page.scanThread.isAlive()) {
						...
						if (bugs >= 10) {
							prtln("    joined irsd: " + irsd + "  ipage: " + ipage + "  respcode: " + page.respcode
									+ "  resptime: " + page.resptime + "  \"" + page.urlstg + "\"");
						}
	...
	void touchDbconn() {
		try {
			dbconn.getDbTable("SELECT CURRENT_DATE", new String[]{"string"}, true);
	 ...
	 *            The current data, in milliseconds since 1970.
	 * @param historyDays
	 *            The numbers of past history we examine when computing a site's
	 *            vitality.
	 * @param ivitalCutoff
	 *            The percent of the time a site must be fully fully functional. If
	 *            the site's vitality falls below ivitalCutoff, a warning message is
	 *            sent.
	 * @param rsds
	 *            An array of Resoucedesc, each of which describes a resource (an
	 *            Id) and the results from scanning it.
	 * @exception IdmapException
	 *                DESCRIPTION
	 * @exception MmdException
	 *                DESCRIPTION
	...
	void storeResults(String collKey, String metastyle, long curdate, int historyDays,
			// num days past history we examine.
			int ivitalCutoff,
			// % vitality below which we issue warning
			ResourceDesc[] rsds) throws IdmapException, MmdException, XMLException {
		int ii;
		int irsd;
		int jrsd;
		Object[][] dbmat;
		...
		// logit ("about to select idmapMmd records for \'" + collKey + "\'
		// collection");
		touchDbconn(); // make sure db is awake
		dbmat = dbconn.getDbTable(
				"SELECT id, fileName, status, primaryUrl, hasFile FROM idmapMmd" + " WHERE collKey = "
						+ dbconn.dbstring(collKey),
				new String[]{"string", "string", "string", "string", "boolean"},
				// types
				true);
		...
		for (ii = 0; ii < dbmat.length; ii++) {
			...
			if ((!idmap.containsKey(dbid)) && dbhasFile) {
				...
				dbconn.updateDb("UPDATE idmapMmd SET hasFile = " + dbconn.dbstring(0) + " WHERE collKey = "
						+ dbconn.dbstring(collKey) + " AND id = " + dbconn.dbstring(dbid));
			} else if ((idmap.containsKey(dbid)) && !dbhasFile) {
				errornum = DpcErrors.IDMAP_XML_FILE_REAPPEARED;
				dbconn.updateDb("UPDATE idmapMmd SET hasFile = " + dbconn.dbstring(1) + " WHERE collKey = "
						+ dbconn.dbstring(collKey) + " AND id = " + dbconn.dbstring(dbid));
			} else if ((idmap.containsKey(dbid)) && dbhasFile) {
				...
				if (!dbfilename.equals(fname)) {
					...
					dbconn.updateDb("UPDATE idmapMmd SET fileName = " + dbconn.dbstring(fname) + " WHERE collKey = "
							+ dbconn.dbstring(collKey) + " AND id = " + dbconn.dbstring(dbid));
					dbfilename = fname;
				}
			}
			if (errornum != -1) {
				...
				dbconn.updateDb("INSERT INTO idmapMessages" + " (collKey, id, recCheckDate, msgType, fileName, "
						+ " xpath, urllabel, url, msg, auxinfo)" + " VALUES ( " + dbconn.dbstringcom(collKey)
						// collkey
		...
		for (irsd = 0; irsd < rsds.length; irsd++) {
			...
			if (basersd.getDuplicateRsd() == null) {
				...
				for (jrsd = irsd + 1; jrsd < rsds.length; jrsd++) {
					...
					if ((basechecksum == 0) && (testchecksum == 0) && (basersd.getPrimaryUrl() == null)
							&& (testrsd.getPrimaryUrl() == null)) {
						;
					} else {
						CatchDup c = new CatchDup(basechecksum, basersd.getPrimaryUrl(), basersd.getId(), testchecksum,
								testrsd.getPrimaryUrl(), testrsd.getId(), exc);
				...
				if (numdups > 0) {
					...
					for (jrsd = irsd + 1; jrsd < rsds.length; jrsd++) {
						...
						boolean isdup = c.isDup();
						if (isdup == true) {
							idstg += "  id: " + testrsd.getId() + "  file: " + testrsd.getFileName() + "  URL: "
									+ testrsd.getPrimaryUrl() + "\n";
							testrsd.addWarning(new Warning(DpcErrors.IDMAP_DUP, testrsd.getId(), testrsd.getFileName(),
									testrsd.getPrimaryXpath(), testrsd.getPrimaryUrllabel(), testrsd.getPrimaryUrl(),
					...
					basersd.addWarning(new Warning(DpcErrors.IDMAP_DUP, basersd.getId(), basersd.getFileName(),
							basersd.getPrimaryXpath(), basersd.getPrimaryUrllabel(), basersd.getPrimaryUrl(),
							"This rec has " + numdups + " duplicates", "The duplicates are:\n" + idstg));
					totalwarnings++;
				}
			}
		}
		...
		for (irsd = 0; irsd < rsds.length; irsd++) {
			try {
				...
				storeResultsSingle(rsd, collKey, metastyle, curdate, historyDays, ivitalCutoff);
			} catch (IdmapException iex) {
		...
		if (bugs >= 10) {
			prtln("\n==========================\n" + "storeResults: warnings summary: totalwarnings: " + totalwarnings);
			for (irsd = 0; irsd < rsds.length; irsd++) {
				ResourceDesc rsd = rsds[irsd];
				prtln("" + irsd + ": " + rsd.getId() + "  warns: " + rsd.numWarnings());
	 ...
	 *
	 * @param rsd
	 *            The ResourceDesc containing results to be saved.
	 * @param collKey
	 *            The collection key.
	 * @param metastyle
	 *            metadata style: one of the MS_ values defined in
	...
	void storeResultsSingle(ResourceDesc rsd, String collKey, String metastyle, long curdate, int historyDays,
			// num days past history we examine.
			int ivitalCutoff)
			// % vitality below which we issue warning
		...
		if ((!rsd.hasSevereError()) && rsd.numpages > 0 && rsd.pages[0].pagewarning == null
				&& rsd.pages[0].urllabel.equals("primary-url") && rsd.pages[0].pageChecksum != 0) {
			for (ii = 1; ii < rsd.numpages; ii++) {
				if (rsd.pages[ii].urllabel.equals("mirror-url") && rsd.pages[ii].pagewarning == null
						&& rsd.pages[ii].pageChecksum != rsd.pages[0].pageChecksum) {
					rsd.addWarning(new Warning(DpcErrors.IDMAP_MIRROR_DIFFERS, rsd.getId(), rsd.getFileName(),
							rsd.pages[0].xpath, rsd.pages[0].urllabel, rsd.pages[0].urlstg, "mirror urls",
							rsd.pages[ii].urlstg));
				}
			}
		}
		...
		try {
			...
			dbmat = dbconn.getDbTable("SELECT fileName, primaryUrl, status, firstAccessionDate,"
					+ " metaChecksum, primaryChecksum, primaryContent, primarycontentType" + " FROM idmapMmd"
					+ " WHERE collKey = " + dbconn.dbstring(collKey) + " AND id = " + dbconn.dbstring(rsd.getId()),
					new String[]{"string", "string", "string", "date", "long", "long", "string", "string"},
					// types
					true);
		...
		if (dbmat == null) {
			...
			dbmat = dbconn.getDbTable(
					"SELECT fileName, primaryUrl, status, firstAccessionDate," + " metaChecksum, primaryChecksum"
							+ " FROM idmapMmd" + " WHERE collKey = " + dbconn.dbstring(collKey) + " AND id = "
							+ dbconn.dbstring(rsd.getId()),
					new String[]{"string", "string", "string", "date", "long", "long"},
					// types
					true);
			// allow primaryUrl to be null
			if (dbmat == null)
		...
		if (dbmat.length == 0) {
			// if rec is new...
			forceEmail = true;
			rsd.addWarning(new Warning(DpcErrors.IDMAP_NEW_XML_FILE, rsd.getId(), rsd.getFileName(),
					rsd.getPrimaryXpath(), rsd.getPrimaryUrllabel(), rsd.getPrimaryUrl(), null, null));

			try {
				// logit ("attempting to insert into idmapMmd");
				dbconn.updateDb("INSERT INTO idmapMmd" + " (collKey, id, hasFile, fileName, primaryUrl, status,"
						+ " firstAccessiondate, lastMetaModDate, recCheckDate,"
						+ " metaChecksum, primaryChecksum, primaryContent, primarycontentType)" + " VALUES ( "
						+ dbconn.dbstringcom(rsd.getCollKey()) + dbconn.dbstringcom(rsd.getId()) + dbconn.dbstringcom(1)
						// hasFile
						+ dbconn.dbstringcom(rsd.getFileName()) + dbconn.dbstringcom(rsd.getPrimaryUrl())
						+ dbconn.dbstringcom(MmdRec.statusNames[MmdRec.STATUS_ACCESSIONED_DISCOVERABLE])
						+ dbconn.dbstringcom(new Timestamp(curdate))
						// firstAccession
						+ dbconn.dbstringcom(new Timestamp(curdate))
						// lastMetaModDate
						+ dbconn.dbstringcom(new Timestamp(curdate))
						// recCheckDate
						+ dbconn.dbstringcom(rsd.getMetaChecksum()) + dbconn.dbstringcom(rsd.getPrimaryChecksum())
						+ dbconn.dbstringforcecom(rsd.PrimaryContent) + dbconn.dbstring(rsd.primarycontentType) + ")");
			} catch (Throwable t) {
				try {
				...
				try {
					dbconn.updateDb("INSERT INTO idmapMmd" + " (collKey, id, hasFile, fileName, primaryUrl, status,"
							+ " firstAccessiondate, lastMetaModDate, recCheckDate," + " metaChecksum, primaryChecksum)"
							+ " VALUES ( " + dbconn.dbstringcom(rsd.getCollKey()) + dbconn.dbstringcom(rsd.getId())
							...
							+ dbconn.dbstringcom(new Timestamp(curdate))
							// recCheckDate
							+ dbconn.dbstringcom(rsd.getMetaChecksum()) + dbconn.dbstring(rsd.getPrimaryChecksum())
							+ ")");
		...
		// else idmapMmd rec exists
			...
			if (!rsd.getFileName().equals(oldFilename)) {
				rsd.addWarning(new Warning(DpcErrors.IDMAP_CHANGE_FILENAME, rsd.getId(), rsd.getFileName(),
						rsd.getPrimaryXpath(), rsd.getPrimaryUrllabel(), rsd.getPrimaryUrl(), "Old file name:",
						oldFilename));
				dbconn.updateDb("UPDATE idmapMmd SET fileName = " + dbconn.dbstring(rsd.getFileName())
						+ " WHERE collKey = " + dbconn.dbstring(collKey) + " AND id = " + dbconn.dbstring(rsd.getId()));
			}

			// If metaChecksum has changed, update it
			if (rsd.getMetaChecksum() != oldMetaChecksum) {
				rsd.addWarning(new Warning(DpcErrors.IDMAP_CHANGE_MMD, rsd.getId(), rsd.getFileName(),
						rsd.getPrimaryXpath(), rsd.getPrimaryUrllabel(), rsd.getPrimaryUrl(), "Old, new mmd checksums:",
						"" + oldMetaChecksum + " " + rsd.getMetaChecksum()));
				dbconn.updateDb("UPDATE idmapMmd SET metaChecksum = " + dbconn.dbstring(rsd.getMetaChecksum())
						+ " WHERE collKey = " + dbconn.dbstring(collKey) + " AND id = " + dbconn.dbstring(rsd.getId()));
			}

			// Always update recCheckDate
			dbconn.updateDb("UPDATE idmapMmd SET recCheckDate = " + dbconn.dbstring(new Timestamp(curdate))
					+ " WHERE collKey = " + dbconn.dbstring(collKey) + " AND id = " + dbconn.dbstring(rsd.getId()));

			try {
				// Always update primaryContent
				dbconn.updateDb("UPDATE idmapMmd SET primaryContent = " + dbconn.dbstringforce(rsd.PrimaryContent)
						+ " WHERE collKey = " + dbconn.dbstring(collKey) + " AND id = " + dbconn.dbstring(rsd.getId()));

				// Always update primarycontentType
				dbconn.updateDb("UPDATE idmapMmd SET primarycontentType = "
						+ dbconn.dbstringforce(rsd.primarycontentType) + " WHERE collKey = " + dbconn.dbstring(collKey)
			...
			if (metastyle.equals(MmdRec.metastyleNames[MmdRec.MS_ADN]) && rsd.numpages > 0
					&& rsd.pages[0].urllabel != null && rsd.pages[0].urllabel.equals("primary-url")) {
				if (!rsd.pages[0].urllabel.equals("primary-url")) {
					mkerror("storeResultsSingle: adn first page not primary." + "  id: " + rsd.getId());
				}

				// If url has changed, update it
				if (oldPrimaryUrl == null && rsd.pages[0].urlstg != null
						|| oldPrimaryUrl != null && !rsd.pages[0].urlstg.equals(oldPrimaryUrl)) {
					rsd.addWarning(new Warning(DpcErrors.IDMAP_CHANGE_PRIMARY_URL, rsd.getId(), rsd.getFileName(),
							rsd.getPrimaryXpath(), rsd.getPrimaryUrllabel(), rsd.getPrimaryUrl(), "old primary url:",
							oldPrimaryUrl));
					dbconn.updateDb("UPDATE idmapMmd SET primaryurl = " + dbconn.dbstring(rsd.getPrimaryUrl())
							+ " WHERE collKey = " + dbconn.dbstring(collKey) + " AND id = "
							+ dbconn.dbstring(rsd.getId()));
				}
				...
				if (bugs >= 10) {
					prtln("storeResults: rsd.id: " + rsd.getId() + "  oldPrimaryChecksum: " + oldPrimaryChecksum
				...
				/*
				 * IMPORTANT : Removing the newPrimaryChecksum !=0 condition * if
				 * (newPrimaryChecksum != 0 && newPrimaryChecksum != oldPrimaryChecksum)
				 */
				if (newPrimaryChecksum != oldPrimaryChecksum) {
					dbconn.updateDb("UPDATE idmapMmd SET primaryChecksum = " + dbconn.dbstring(newPrimaryChecksum)
							+ " WHERE collKey = " + dbconn.dbstring(collKey) + " AND id = "
							+ dbconn.dbstring(rsd.getId()));
				}
			}
		...
		for (ipage = 0; ipage < rsd.numpages; ipage++) {
			...
			dbmat = dbconn.getDbTable(
					"SELECT lastMsgType, lastDateUp, lastDateDown FROM idmapVitality" + " WHERE collKey = "
							+ dbconn.dbstring(collKey) + " AND id = " + dbconn.dbstring(rsd.getId()) + " AND url = "
							+ dbconn.dbstring(page.urlstg) + " AND recCheckDate >= "
							+ dbconn.dbstring(new Timestamp(prevdate)) + " ORDER BY recCheckDate",
			...
			try {
				dbconn.updateDb(
						"INSERT INTO idmapVitality" + " (collKey, id, recCheckDate, fileName, xpath, urllabel, url,"
								+ " lastMsgType, vitality, lastDateUp, lastDateDown)" + " VALUES ( "
								+ dbconn.dbstringcom(rsd.getCollKey()) + dbconn.dbstringcom(rsd.getId())
								+ dbconn.dbstringcom(new Timestamp(curdate))
								// recCheckDate
								+ dbconn.dbstringcom(rsd.getFileName()) + dbconn.dbstringcom(page.xpath)
								+ dbconn.dbstringcom(page.urllabel) + dbconn.dbstringcom(page.urlstg)
								+ dbconn.dbstringcom(lastMsgType) + dbconn.dbstringcom(ivitality)
			...
			if (page.pagewarning != null && ivitality < ivitalCutoff) {
				...
				if (lastDateUp == 0) {
					...
					downdays = (curdate - lastDateUp) / (double) (1000 * 60 * 60 * 24);
				}

				rsd.addWarning(
						new Warning(DpcErrors.IDMAP_VITALITY, rsd.getId(), rsd.getFileName(), page.xpath, page.urllabel,
								page.urlstg, "URL was up " + ivitality + " % over the last " + historyDays + " days",
								// msg
		...
		if (rsd.numWarnings() > 0) {
			...
			while (iter.hasNext()) {
				...
				dbconn.updateDb("INSERT INTO idmapMessages" + " (collKey, id, recCheckDate, msgType, fileName,"
						+ " xpath, urllabel, url, msg, auxinfo)" + " VALUES ( " + dbconn.dbstringcom(collKey)
						+ dbconn.dbstringcom(wng.id) + dbconn.dbstringcom(new Timestamp(curdate))
						// recCheckDate
						+ dbconn.dbstringcom(DpcErrors.getMessage(wng.msgType)) + dbconn.dbstringcom(rsd.getFileName())
						+ dbconn.dbstringcom(wng.xpath) + dbconn.dbstringcom(wng.urllabel) + dbconn.dbstringcom(wng.url)
						+ dbconn.dbstringforcecom(wng.msg) + dbconn.dbstringforce(wng.auxinfo) + ")");
			}
		}
	}
	...
	/**
	 * Returns all the file names in a dir that end in ".xml" and have been modified
	 * since prevmoddate.
	 *
	 * @param prevmoddate
	 *            The previous date, expressed as milliseconds since 1970.
	 * @param dirPath
	 *            The path of the directory containing collKey's XML files.
	...
	File[] getDirFiles(final long prevmoddate, File dirPath) throws IdmapException {
		File[] resfiles = dirPath.listFiles(new FileFilter() {
			public boolean accept(File file) {
				if (file.isFile() && file.getPath().endsWith(".xml") && (file.lastModified() >= prevmoddate)) {
					return true;
				} else {
					return false;
				}
	 ...
	 *            {@link org.dlese.dpc.services.mmd.MmdRec MmdRec}.
	 * @param dirPath
	 *            The directory containing the XML metadata files.
	 * @param numRecords
	 *            The number of records that were checked
	 * @param curdate
	 *            The current data, in milliseconds since 1970.
	 * @param emailDays
	 *            bit vec of days of the week to send email.
	 * @param emailHost
	 *            host name of the email server
	 * @param emailFrom
	 *            "from" name to be used in email messages
	...
	boolean testEmailTime(int bugs, String collKey, String metastyle, File dirPath, int numRecords, long curdate,
			boolean[] emailDays, String emailHost, String emailFrom, int emailType, String[] emailAddrs,
			String emailSubject) throws IdmapException, MmdException {
		int ii;
		Object[][] dbmat;
		...
		Object[][] datemat = dbconn.getDbTable(
				"SELECT lastEmailDate FROM idmapCollection" + " WHERE collKey = " + dbconn.dbstring(collKey),
				new String[]{"date"},
				// types
				true);
		// allow nulls
		...
		if (prevdate > 0 && sentEmail) {
			...
			if (prevcal.get(Calendar.YEAR) == curcal.get(Calendar.YEAR)
					...
					&& prevcal.get(Calendar.DAY_OF_MONTH) == curcal.get(Calendar.DAY_OF_MONTH)) {
				sentEmail = false;
			}
		}

		if (forceEmail || emailType == EMAIL_FORCE || emailType == EMAIL_SINGLE || emailType == EMAIL_PRINT) {
			sentEmail = true;
		}

		// However, if there's nobody to send it to, don't send it.
		if (emailType != EMAIL_PRINT && (emailAddrs == null || emailAddrs.length == 0)) {
			sentEmail = false;
		}
		...
		if (sentEmail) {
			...
			dbmat = dbconn.getDbTable(
					"SELECT id, msgType, fileName, xpath, urllabel, url, msg, auxinfo" + " FROM idmapMessages"
							+ " WHERE collKey = " + dbconn.dbstring(collKey) + " AND recCheckDate = "
							+ dbconn.dbstring(new Timestamp(curdate)),
					new String[]{"string", "string", "string", "string", "string", "string", "string", "string"}, true);
			// allow nulls in fields
			...
			for (ii = 0; ii < dbmat.length; ii++) {
				...
				warns[ii] = new Warning(msgtype, id, filename, xpath, urllabel, url, msg, auxinfo);
			}

			Arrays.sort(warns, new Comparator() {
				public int compare(Object obja, Object objb) {
					int ires;
			...
			for (ii = 0; ii < warns.length; ii++) {
				if (bugs >= 1) {
					prtln("testEmailTime: warning " + ii + ":  id: \"" + warns[ii].id + "\"" + "  msgType: "
							+ DpcErrors.getMessage(warns[ii].msgType) + "  filename: \"" + warns[ii].filename + "\""
							+ "  xpath: \"" + warns[ii].xpath + "\"" + "  urllabel: \"" + warns[ii].urllabel + "\""
							+ "  url: \"" + warns[ii].url + "\"" + "  msg: \"" + warns[ii].msg + "\"" + "  auxinfo: \""
							+ warns[ii].auxinfo + "\"");
				}
				...
				if (ii == 0 || warns[ii - 1].msgType != warns[ii].msgType) {
					msgbuf.append("\n==========\n\n");
					msgbuf.append(
							"The following ids have \"" + DpcErrors.getMessage(warns[ii].msgType) + "\" errors.\n\n");
				}
			...
			String subj = emailSubject + ", " + collKey + ", " + totalwarnings + " warnings";
	...
	void sendemail(String emailHost, String emailFrom, String[] emailAddrs, String emailSubject, String msgtext)
	 ...
	 * @param starttime
	 *            The time at which the current run started.
	 */
	...
	static void prtmemorystats(String msg, long starttime) {
		...
		prtln("");
		prtln(msg);
		prtln("  elapsed time: " + (0.001 * (System.currentTimeMillis() - starttime)) + " seconds");
		Runtime rt = Runtime.getRuntime();
		prtln("  memory stats:");
		prtln("  total: " + (rt.totalMemory() / (1024 * 1024)) + " MB");
		prtln("  max: " + (rt.maxMemory() / (1024 * 1024)) + " MB");
		prtln("  free: " + (rt.freeMemory() / (1024 * 1024)) + " MB");
		prtln("  used: " + ((rt.totalMemory() - rt.freeMemory()) / (1024 * 1024)) + " MB");
	}
	...
	static void mkerror(String msg) throws IdmapException {
