...
public final class DCSServicesAction extends Action {
	 ...
	 *                If error.
	 */
	protected ActionForward doGetId(HttpServletRequest request, HttpServletResponse response, RepositoryManager rm,
			DCSServicesForm dcssf, ActionMapping mapping) throws Exception {
	...
	protected ActionForward doUrlCheck(HttpServletRequest request, HttpServletResponse response, RepositoryManager rm,
		...
		prtln("doUrlCheck() search for: '" + q + "' had " + (resultDocs == null ? -1 : resultDocs.size())
				+ " resultDocs");
	 ...
	 *                If error.
	 */
	protected ActionForward doPutRecord(HttpServletRequest request, HttpServletResponse response, RepositoryManager rm,
			DCSServicesForm dcssf, ActionMapping mapping) throws Exception {
		...
		try {
			...
			/*
			 * ISSUE - do we want to connect annotations to the records they annotate at
			 * this point? if we are saving an annotation record, then we have to call
			 * putRecord for the annotated records that are found in the current DCS index.
			...
			if (xmlFormatParam.equals("dlese_anno")) {
				...
				try {
					RepositoryService repositoryService = (RepositoryService) servlet.getServletContext()
	 ...
	 * <p>
	 *
	 * Arguments: id, dcsStatus, dcsStatusNode (optional), and
	 * dcsStatusEditor(optional).
	 * <p>
	 *
	 * Error Exception Conditions: <br>
	 ...
	 *                If error.
	 */
	protected ActionForward doUpdateStatus(HttpServletRequest request, HttpServletResponse response,
			RepositoryManager rm, DCSServicesForm dcssf, ActionMapping mapping) throws Exception {
	...
	protected ActionForward doExportCollection(HttpServletRequest request, HttpServletResponse response,
			RepositoryManager rm, DCSServicesForm dcssf, ActionMapping mapping) throws Exception {
		...
		CollectionRegistry collectionRegistry = (CollectionRegistry) servlet.getServletContext()
				.getAttribute("collectionRegistry");
		if (collectionRegistry == null) {
			dcssf.setErrorMsg("Server Error: \"collectionRegistry\" not found in servlet context");
			return mapping.findForward("dcsservices.error");
		}

		ExportingService exportingService = (ExportingService) servlet.getServletContext()
				.getAttribute("exportingService");
		...
		if (collectionConfig != null) {
			// exportDir is not required. it defaults to configured exportDir for this
			// collection
	...
	/**
	 * Ensure that the record will not choke the index. Currently only suported for
	 * "adn" format.
	 * <p>
	 * Notes:
	 * <li>This level of validation may not be neccesary since repository manager
	 * performs the same checks
	 * <li>If we do want to perform this level of validation, we should be using
	 * required paths as defined in framework-config.
	 */
	private List validateRecordXml(PutRecordData prData) throws Exception {
		...
		/*
		 * else if (format.equals("dlese_collect")) errorList.addAll
		 * (validateDleseCollect (prData)); else if (format.equals("news_opps"))
		 * errorList.addAll (validateNewsOpps(prData)); else if
		 * (format.equals("dlese_anno")) errorList.addAll (validateDleseAnno (prData));
