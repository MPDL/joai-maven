...
/**
 * An <strong>Action</strong> controller that handles requests for the
 * JavaScript HTML search service. This class handles JSHTML search service
 * verions 1.1. Note that web_service_connection.jsp in the context acts
...
public final class JSHTMLAction_1_1 extends Action {
	...
	/**
	 * Processes the JavaScript service request by forwarding to the appropriate
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if (request.getParameter("fullDescription") != null
					|| request.getParameter("collectionDescription") != null)
	...
	protected ActionForward doGetRecord(HttpServletRequest request, HttpServletResponse response, RepositoryManager rm,
			JSHTMLForm_1_1 jsform, ActionMapping mapping) throws Exception {
		...
		if (idParam == null || idParam.length() == 0) {
			jsform.setErrorMsg(
					"The fullDescription or collectionDescription parameter is required but is missing or empty");
		...
		// Note that the following two cases are not currently used, but could be in the
		// future...
	 ...
	 * @param jsform
	 *            The Form
	 * @param customSearchMappings
	 *            A Map of search queries keyed by menu-item ID
	 * @return An ActionForward to the JSP page that will handle the response
	 * @exception Exception
	 *                If error.
	 */
	protected ActionForward doUserSearch(HttpServletRequest request, HttpServletResponse response, RepositoryManager rm,
			JSHTMLForm_1_1 jsform, Map customSearchMappings, ActionMapping mapping) throws Exception {
		...
		try {
			...
			if (jsform.getS() < 0) {
				jsform.setErrorMsg(
						"The parameter 's' must be greater than or equal to 0. s specifies the start position within the search results to display.");
				return mapping.findForward("jshtml.default");
			}
		} catch (NumberFormatException nfe) {
			jsform.setErrorMsg(
					"The parameter 's' must be a number. s specifies the start position within the search results to display.");
			return mapping.findForward("jshtml.default");
		}
		try {
			if (n == null)
			...
			if (jsform.getN() < 1 || jsform.getN() > MAX_SEARCH_RESULTS) {
				jsform.setErrorMsg("The command 'MaxResultsPerPage' must be a number from 1 to " + MAX_SEARCH_RESULTS
						+ ". MaxResultsPerPage specifies the maximum number of search results to display.");
				return mapping.findForward("jshtml.default");
			}
		} catch (NumberFormatException nfe) {
			jsform.setErrorMsg(
					"The command 'MaxResultsPerPage' must be a number. MaxResultsPerPage specifies the maximum number of search results to display.");
			return mapping.findForward("jshtml.default");
		}
		...
		if (xmlFormat != null && !xmlFormat.startsWith("adn")) {
			...
			jsform.setErrorMsg(
					"The so (search over) parameter may not be used with the UserSearch request. Try using the Search request to search over discoverable and non-discoverable records");
			return mapping.findForward("jshtml.default");
		}
		...
		// Perform the search...
		DDSStandardSearchResult standardSearchResult = DDSQueryAction.ddsStandardQuery(request, smartLinkQuery, rm,
		...
		if (ex != null) {
			...
			if (ex instanceof org.apache.lucene.queryParser.ParseException) {
				...
				try {
					...
					// If the error did NOT reside in the user's query, send an error message to the
					// developer
					jsform.setErrorMsg(
							"There was an error parsing the search query. Possible reason: " + ex.getMessage());
					return mapping.findForward("jshtml.default");
		...
		// prtln("doUserSearch() search for: '" + request.getParameter("q") + "' had " +
		// (resultDocs == null ? -1 : resultDocs.length) + " results");
	...
	protected ActionForward doSearch(HttpServletRequest request, HttpServletResponse response, RepositoryManager rm,
		...
		try {
			if (n == null)
				...
				jsform.setN(Integer.parseInt(n));
			if (jsform.getN() < 1 || jsform.getN() > MAX_SEARCH_RESULTS) {
				jsform.setErrorMsg("The parameter 'n' must be a number from 1 to " + MAX_SEARCH_RESULTS
						+ ". n specifies the maximum number of search results to display.");
				return mapping.findForward("jshtml.default");
			}
		} catch (NumberFormatException nfe) {
			jsform.setErrorMsg(
					"The parameter 'n' must be a number. n specifies the maximum number of search results to display.");
			return mapping.findForward("jshtml.default");
		}

		// The possible params (excluding vocab-managed params including su, re, cs, ky,
		// gr)
		...
		if (xmlFormat != null && xmlFormatsQuery == null) {
			...
			DDSQueryAction.logQuery(userQuery, servlet.getServletContext(), request, 200, -1, 0, getNumRecords(rm),
					jsform.getS(), DDSQueryAction.SEARCHTYPE_JSHTML_SEARCH);
		...
		if (dateField != null && fromDate == null && toDate == null) {
			jsform.setErrorMsg(
					"The 'fromDate' and/or 'toDate' parameter must be specified when the 'dateField' argument is indicated");
			return mapping.findForward("jshtml.default");
		}
		...
		if (fromDate != null || toDate != null) {
			if (dateField == null) {
				jsform.setErrorMsg(
						"The 'dateField' parameter must be specified when either the 'fromDate' or 'toDate' argument is indicated");
				return mapping.findForward("jshtml.default");
			}
			...
			try {
				...
				jsform.setErrorMsg("One or more dates indicated is incorrect: " + pe.getMessage()
		...
		if ((userQuery == null || userQuery.length() == 0) && (xmlFormat == null || xmlFormat[0].length() == 0)
				&& vocabQuery.length() == 0 && dateFilter == null) {
			isEmptySearch = true;
		}
		...
		if (index != null && !isEmptySearch) {
			...
			try {
				...
				jsform.setErrorMsg(
						"Error parsing the q parameter ' " + userQuery + "'. Possible reason: " + pe.getMessage());
				return mapping.findForward("jshtml.default");
			}
			...
			// prtln("doSearch() query for: '" + query + "' had " + (resultDocs == null ? -1
			// : resultDocs.length) + " results");
		}
		...
		if (resultDocs == null || resultDocs.size() == 0) {
			...
			/*
			 * String q = request.getParameter("q"); if (q == null || q.trim().length() ==
			 * 0) jsform.setErrorMsg("Your search had no matching records"); else
			 * jsform.setErrorMsg("Your search for '" + request.getParameter("q") +
			 * "' had no matching records");
			 */
			return mapping.findForward("jshtml.default");
		}
		...
		DDSQueryAction.logQuery(userQuery, servlet.getServletContext(), request, 200, -1, resultDocs.size(),
				getNumRecords(rm), jsform.getS(), DDSQueryAction.SEARCHTYPE_JSHTML_SEARCH);
