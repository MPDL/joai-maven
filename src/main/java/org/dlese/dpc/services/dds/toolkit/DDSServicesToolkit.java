...
/**
 * Toolkit for working with DDS repository Search and Update Web Services (DDSWS
 * and DDSUpdateWS) for read and write operations. All methods use only local
 * variables and are thread safe but are not synchronized. See <a href=
 * "http://www.dlese.org/dds/services/ddsws1-1/service_specification.jsp"> DDSWS
 * documentation </a> and <a href=
 * "http://www.dlese.org/dds/services/ddsupdatews1-1/ddsupdatews_api_documentation.jsp">
...
public class DDSServicesToolkit {
	 ...
	 *
	 * @param ddsServicesUrl
	 *            Common baseUrl for DDS services, for example
	 *            http://www.dlese.org/dds/services/
	 * @param clientName
	 *            The client name, or null to send none
	 */
	 ...
	 * @param xmlFormat
	 *            The XML format to return, or null for native format
	 * @param additionalRequestParams
	 *            A Map that contains param/value pairs. Values must be of type
	 *            String to indicate a single value, or a String [] to indicate
	 *            multiple values. Map may be null for none.
	 * @param soAllRecords
	 *            True to search all records including non-discoverable ones (client
	 *            must be authorized by IP)
	 * @param localizeXml
	 *            True to localize the response
	 * @return The service response
	 * @exception DDSServiceErrorResponseException
	 *                Standard service error if one was returned
	 * @exception Exception
	 *                If other error occurs
	 */
	public DDSServicesResponse getRecord(String id, String showRelation, String xmlFormat, Map additionalRequestParams,
			boolean soAllRecords, boolean localizeXml) throws DDSServiceErrorResponseException, Exception {

		String serviceRequest = ddswsBaseUrl + "?verb=GetRecord" + "&id=" + URLEncoder.encode(id, "UTF-8");
		...
		if (additionalRequestParams != null) {
			...
			while (keyIterator.hasNext()) {
				...
				if (val instanceof String)
					queryString += "&" + URLEncoder.encode((String) key, "UTF-8") + "="
							+ URLEncoder.encode((String) val, "UTF-8");
	...
	/**
	 * Determines whether the given record is in this DDS repository. If a format is
	 * specified, then also determines if the record can be dissiminated in the
	 * given format.
	 *
	 * @param id
	 *            The record id
	 * @param xmlFormat
	 *            The XML format to check availability for, or null for any/all
	...
	public boolean hasRecord(String id, String xmlFormat) throws DDSServiceErrorResponseException, Exception {
		...
		try {
			...
			if (de.getServiceResponseCode().equals("idDoesNotExist")
					|| de.getServiceResponseCode().equals("cannotDisseminateFormat")) {
				return false;
			}
			throw de;
		}
	 ...
	 * @param xmlFormat
	 *            The xmlFormat for which results must be dissiminated, or null for
	 *            any/all
	 * @param startOffset
	 *            Starting offset in the returns
	 * @param numReturns
	 ...
	 *            True to localize the response
	 * @param sortByField
	 *            Indicates the search field to sort the results by, or null for
	 *            none
	 * @param sortOrder
	 *            The sort order to apply (ascending, descending). Ignored if no
	 *            sortByField has been indicated
	 * @return The service response
	 * @exception DDSServiceErrorResponseException
	 *                Standard service error if one was returned
	 * @exception Exception
	 *                If other error occurs
	 */
	public DDSServicesResponse search(String query, String xmlFormat, int startOffset, int numReturns,
			String sortByField, int sortOrder, String showRelation, Map additionalRequestParams, boolean soAllRecords,
			boolean localizeXml) throws DDSServiceErrorResponseException, Exception {

		String serviceRequest = ddswsBaseUrl + "?verb=Search" + "&n=" + numReturns + "&s=" + startOffset;
	...
	/**
	 * Gets the index version, which can be useful to determine if a service
	 * response cache needs to be updated.
	 *
	...
	public String getIndexVersion() throws DDSServiceErrorResponseException, Exception {
		...
		try {
			...
			String indexVersion = serviceInfoDoc.valueOf(
					"/*[local-name()='DDSWebService']/*[local-name()='ServiceInfo']/*[local-name()='indexVersion']");
	...
	/**
	 * Performs a ListTerms request from a DDSWS repository. The request accepts one
	 * or more fields to list the terms data for. Note that this request is much
	 * less efficient when more than one field is requested.
	 *
	 * @param fields
	 *            One or more fields to list terms for
	 * @param localizeXml
	 *            True to localize the response XML
	 * @return The service response
	 * @exception DDSServiceErrorResponseException
	 ...
	 *                If other error occurs
	 */
	public DDSServicesResponse listTerms(String[] fields, boolean localizeXml)
			throws DDSServiceErrorResponseException, Exception {
	 ...
	 *
	 * @param collectionKey
	 *            The collection key, for example 'dcc'
	 * @param xmlFormat
	 *            The xml format for this collection, for example 'adn'
	 * @return True if the given collection is in the repository with the given
	 * @exception DDSServiceErrorResponseException
	 *                If service error
	 * @exception Exception
	 *                If other error
	 */
	public boolean hasCollection(String collectionKey, String xmlFormat)
			throws DDSServiceErrorResponseException, Exception {
		DDSServicesResponse ddsServicesResponse = null;
		try {
			String searchQuery = "(key:\"" + collectionKey
					+ "\" AND /key//collectionRecord/access/key/@libraryFormat:\"" + xmlFormat
					+ "\" AND xmlFormat:\"dlese_collect\")";

			ddsServicesResponse = search(searchQuery, null, 0, 1, null, DDSServicesToolkit.SORT_ORDER_NO_SORT, null,
					null, false, true);
		} catch (DDSServiceErrorResponseException de) {
		...
		String formatFound = response
				.valueOf("/DDSWebService/Search/results/record/metadata/collectionRecord/access/key/@libraryFormat");
	 ...
	 *                Standard service error if one was returned
	 */
	public Document putRecord(String recordId, String recordXml, String collectionKey, String xmlFormat)
			throws DDSServiceErrorResponseException, Exception {

		// Save the user's anno record:
		// prtln("Saving record:\n" + recordXml);

		String postData = "verb=PutRecord" + "&id=" + URLEncoder.encode(recordId, "UTF-8") + "&collectionKey="
				+ URLEncoder.encode(collectionKey, "UTF-8") + "&xmlFormat=" + URLEncoder.encode(xmlFormat, "UTF-8")
		...
		Document ddsupdateResponse = Dom4jUtils
				.localizeXml(Dom4jUtils.getXmlDocumentPostData(new URL(ddsupdatewsBaseUrl), postData, 90000));
	...
	public DDSServicesResponse deleteRecord(String recordId) throws DDSServiceErrorResponseException, Exception {
		...
		String serviceRequest = ddsupdatewsBaseUrl + "?verb=DeleteRecord" + "&id="
				+ URLEncoder.encode(recordId, "UTF-8");
	 ...
	 *            The collection key, for example 'dcc'
	 * @param xmlFormat
	 *            The xml format, for example 'adn'
	 ...
	 *                If other error occurs
	 */
	public DDSServicesResponse putCollection(String collectionKey, String xmlFormat, String collectionName,
			String collectionDescription) throws DDSServiceErrorResponseException, Exception {
		String serviceRequest = ddsupdatewsBaseUrl + "?verb=PutCollection" + "&collectionKey="
				+ URLEncoder.encode(collectionKey, "UTF-8") + "&xmlFormat=" + URLEncoder.encode(xmlFormat, "UTF-8")
				+ "&name=" + URLEncoder.encode(collectionName, "UTF-8") + "&description="
				+ URLEncoder.encode(collectionDescription, "UTF-8");
	...
	/**
	 * Checks for the existance of an error response from a DDS update service
	 * request (DDSUpdateWS). If no error was recieved, this method returns null,
	 * otherwise returns the error code and message.
	 *
	 * @param ddsupdateResponse
	 *            The DDS response
	 * @return The error code and message [code,message], or null if none
	 */
	protected String[] checkForErrorResponseDDSUpdateWS(Document ddsupdateResponse) {
		if (ddsupdateResponse != null
				&& ddsupdateResponse.selectSingleNode("/DDSRepositoryUpdateService/error") != null) {
