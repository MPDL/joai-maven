...
/**
 * An Action that implements a RESTful Web service for performing updates to a
 * DDS repository.
 *
...
public final class DDSRepositoryUpdateServiceAction extends Action {
	...
	/**
	 * Processes the Web service request by forwarding to the appropriate
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			String enableService = (String) servlet.getServletContext()
					.getInitParameter("enableRepositoryUpdateWebService");
			...
			// Disallow any action if the service is not configured from the file system
			// data source:
			...
			if (recordDataSource == null || !recordDataSource.equals("fileSystem")) {
				ddsusf.setErrorMsg(
						"The service is disabled. The data source " + recordDataSource + " is not supported.");
				ddsusf.setErrorCode(DDSRepositoryUpdateServiceForm.ERROR_CODE_SERVICE_DISABLED);
				return mapping.findForward("ddsupdateservice.error");
			}

			RepositoryManager repositoryManager = (RepositoryManager) servlet.getServletContext()
	 ...
	 *                If error.
	 */
	protected ActionForward doPutCollection(HttpServletRequest request, HttpServletResponse response,
			RepositoryManager repositoryManager, DDSRepositoryUpdateServiceForm ddsusf, ActionMapping mapping)
			throws Exception {
		...
		try {
			String collId = repositoryManager.putCollection(collectionKeyParam, xmlFormatParam, nameParam,
					descriptionParam, additionalMetadata);
	 ...
	 *                If error.
	 */
	protected ActionForward doDeleteCollection(HttpServletRequest request, HttpServletResponse response,
			RepositoryManager repositoryManager, DDSRepositoryUpdateServiceForm ddsusf, ActionMapping mapping)
			throws Exception {
	...
	/**
	 * Handles a request to put a metadata record into a DDS repository. Wraps
	 ...
	 *                If error.
	 */
	protected ActionForward doPutRecord(HttpServletRequest request, HttpServletResponse response,
			RepositoryManager repositoryManager, DDSRepositoryUpdateServiceForm ddsusf, ActionMapping mapping)
			throws Exception {
		...
		try {
			...
			String actualRecordID = repositoryManager.putRecord(recordXmlParam, xmlFormatParam, collectionKeyParam,
					idParam, true);
			ddsusf.setXmlFormat(xmlFormatParam);
			ddsusf.setCollectionKey(collectionKeyParam);
			 ...
			 * prtln("indexing annotated record ..."); try { RepositoryService
			 * repositoryService = (RepositoryService)
			 * servlet.getServletContext().getAttribute("repositoryService"); if
			 * (repositoryService == null) throw new
			 * Exception("repositoryService not found in servlet context"); Document doc =
			 * DocumentHelper.parseText(recordXmlParam); doc = Dom4jUtils.localizeXml(doc);
			 * repositoryService.indexAnnotatedRecord(new DocMap(doc)); } catch (Exception
			 * e) { prtlnErr("error indexing annotated record: " + e.getMessage()); } }
			 */
	 ...
	 *                If error.
	 */
	protected ActionForward doDeleteRecord(HttpServletRequest request, HttpServletResponse response,
			RepositoryManager repositoryManager, DDSRepositoryUpdateServiceForm ddsusf, ActionMapping mapping)
			throws Exception {
		...
		try {
			...
			/*
			 * ISSUE - do we want to update annotations to the records they annotate at this
			 * point?
			 */
