...
public class RepositoryUtils {
	 ...
	 * Constructor for the RepositoryUtils object
	 *
	 * @param ddsServicesToolkit
	 *            A DDSServicesToolkit
	 */
	 ...
	 * @exception Exception
	 *                if the itemRecordMap cannot be constructed
	 */
	public Map getItemRecordMap(String collection) throws Exception {
		...
		prtln("\nFetching " + collection + " (" + collectionInfo.formatOfRecords + ")  " + collectionInfo.numRecords
				+ " records");
	...
	public Map getItemRecordMap(CollectionInfo collectionInfo, int startOffset, int numReturns) throws Exception {
		...
		try {
			response = toolkit.search(query, formatOfRecords, startOffset, numReturns, sortByField, sortOrder,
					showRelation, additionalRequestParams, soAllRecords, localizeXml);
		...
		try {
			...
			List recordNodes = doc.selectNodes(
					"/*[local-name()='DDSWebService']/*[local-name()='Search']/*[local-name()='results']/*[local-name()='record']");
		...
		if (this.collectionInfos == null) {
			...
			for (Iterator i = collectionNodes.iterator(); i.hasNext();) {
				...
				try {
					...
					myCol.formatOfRecords = this.getValueAtPath(collectionElement,
							"additionalMetadata/dlese_collect/formatOfRecords");
					myCol.numRecords = this.getIntAtPath(collectionElement,
							"additionalMetadata/dlese_collect/numRecords");
	 ...
	 * @exception Exception
	 *                if there is no node at relativePath from baseElement
	 */
	 ...
	 * @exception Exception
	 *                if there is no node at relativePath from baseElement, or if
	 *                the value found is not an integer
	 */
	 ...
	 *
	 * @param parent
	 *            Element whose child element supplies the text
	 * @param childTagName
	...
	/**
	 * Get terms data for one or more fields in a DDS repository index. The request
	 * accepts one or more fields to list the terms data for. Note that this request
	 * is much less efficient when more than one field is requested.
	 *
	 * @param fields
	...
	public TermsInfo getTermsInfoForFields(String[] fields) throws DDSServiceErrorResponseException, Exception {
		...
		String indexVersion = getValueAtPath(responseDoc,
				"/*[local-name()='DDSWebService']/*[local-name()='ListTerms']/*[local-name()='head']/*[local-name()='indexVersion']");
		int totalNumTerms = getIntAtPath(responseDoc,
				"/*[local-name()='DDSWebService']/*[local-name()='ListTerms']/*[local-name()='head']/*[local-name()='totalNumTerms']");

		// Get the fields that were processed:
		List fieldsNodes = responseDoc.selectNodes(
				"/*[local-name()='DDSWebService']/*[local-name()='ListTerms']/*[local-name()='head']/*[local-name()='fields']/*[local-name()='field']");
		...
		List termNodes = responseDoc.selectNodes(
				"/*[local-name()='DDSWebService']/*[local-name()='ListTerms']/*[local-name()='terms']/*[local-name()='term']");
