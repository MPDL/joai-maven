...
/**
 * An <strong>Action</strong> that handles DDS Web service requests. This class
 * handles DDSWS service version 1.0.
 *
...
public class DDSServicesAction extends Action {
	...
	/**
	 * Gets the key used for caching the ListCollections response in the application
	 * scope. Should be overwritten by the subclasses and updated in the
	 * ListCollections.jsp page when a new version is created.
	 *
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			/*
			 * else if (rm.getProviderStatus().toLowerCase().equals("disabled")) { return
			 * (mapping.findForward("ddsservices.disabled")); }
			 */
			...
			else if (verb.equals(LIST_GRADE_RANGES) || verb.equals(LIST_SUBJECTS) || verb.equals(LIST_RESOURCE_TYPES)
	 ...
	 *                If error.
	 */
	protected ActionForward doGetRecord(HttpServletRequest request, HttpServletResponse response, RepositoryManager rm,
			DDSServicesForm df, ActionMapping mapping) throws Exception {
		...
		if (xmlFormat == null) {
			...
			if (xml == null || xml.length() == 0) {
				df.setErrorMsg("Record " + idParam + " exists in the repository but is not available in xmlFormat "
						+ xmlFormat);
	...
	/**
	 * Sets the record XML in the form bean using the localized version of XML. This
	 * method is overidden in at least one sub-class.
	 *
	 * @param df
	 *            The form
	 * @param resultDoc
	...
	/**
	 * Handles a request to perform a search over item-level records using the User
	 * Query Language. This request exposes the same search options that users
	 * experience when performing a search for educational resources in the DDS.
	 * <p>
	 *
	 * Arguments: identifier, metadataPrefix.
	 * <p>
	 ...
	 *                If error.
	 */
	protected ActionForward doUserSearch(HttpServletRequest request, HttpServletResponse response, RepositoryManager rm,
			DDSServicesForm df, ActionMapping mapping) throws Exception {
		...
		if (n == null || s == null) {
			...
			try {
				...
				if (df.getS() < 0) {
					df.setErrorMsg(
							"The argument 's' must be greater than or equal to 0. s specifies the start position of the results to return.");
					df.setErrorCode(DDSServicesForm.ERROR_CODE_BADARGUMENT);
					return mapping.findForward("ddsservices.error");
				}
			} catch (NumberFormatException nfe) {
				df.setErrorMsg(
						"The argument 's' must be an integer. s specifies the start position of the results to return.");
				df.setErrorCode(DDSServicesForm.ERROR_CODE_BADARGUMENT);
				return mapping.findForward("ddsservices.error");
			}
			try {
				...
				if (df.getN() < 1 || df.getN() > MAX_SEARCH_RESULTS) {
					df.setErrorMsg("The argument 'n' must be a number from 1 to " + MAX_SEARCH_RESULTS
							+ ". n specifies the maximum number of results to return.");
					df.setErrorCode(DDSServicesForm.ERROR_CODE_BADARGUMENT);
					return mapping.findForward("ddsservices.error");
				}
			} catch (NumberFormatException nfe) {
				df.setErrorMsg(
						"The argument 'n' must be in integer. n specifies the maximum number of results to return.");
				df.setErrorCode(DDSServicesForm.ERROR_CODE_BADARGUMENT);
				return mapping.findForward("ddsservices.error");
			}
		}
		...
		if (xmlFormat != null && !xmlFormat.startsWith("adn")) {
			df.setErrorMsg(
					"The UserSearch request operates over the ADN format only. Try using the Search request to search over the xml format '"
							+ xmlFormat + "'");
			df.setErrorCode(DDSServicesForm.ERROR_CODE_BADARGUMENT);
			return mapping.findForward("ddsservices.error");
		} else if (so != null) {
			df.setErrorMsg(
					"The so (search over) argument may not be used with the UserSearch request. Try using the Search request to search over discoverable and non-discoverable records");
			df.setErrorCode(DDSServicesForm.ERROR_CODE_BADARGUMENT);
			return mapping.findForward("ddsservices.error");
		}

		// Perform the search...
		DDSStandardSearchResult standardSearchResult = DDSQueryAction.ddsStandardQuery(request, null, rm,
				(MetadataVocab) servlet.getServletContext().getAttribute("MetadataVocab"), servlet.getServletContext(),
				DDSQueryAction.SEARCHTYPE_DDSWS_USER_SEARCH);

		// org.apache.lucene.queryParser.ParseException pe =
		// standardSearchResult.getParseException();
		...
		// prtln("doUserSearch() search for: '" + q + "' had " + (resultDocs == null ?
		// -1 : resultDocs.length) + " results");
	...
	/**
	 * Handles a request to perform a search over all records using the Lucene Query
	 * Language.
	 * <p>
	 *
	 * Arguments: identifier, metadataPrefix.
	 ...
	 *                If error.
	 */
	protected ActionForward doSearch(HttpServletRequest request, HttpServletResponse response, RepositoryManager rm,
			DDSServicesForm df, ActionMapping mapping) throws Exception {
		...
		// Get a query that pulls formats that may can be converted to the requested
		// format
		...
		// If a format was requested but there is no available query for that format,
		// return empty
		if (xmlFormat != null && xmlFormatsQuery == null) {
			// Log the query that the client requested:
			DDSQueryAction.logQuery(userQuery, servlet.getServletContext(), request, 200, -1, 0, getNumRecords(rm),
					df.getS(), DDSQueryAction.SEARCHTYPE_DDSWS_SEARCH);
		...
		if (dateField != null && fromDate == null && toDate == null) {
			df.setErrorMsg(
					"The 'fromDate' and/or 'toDate' argument must be specified when the 'dateField' argument is indicated");
			df.setErrorCode(DDSServicesForm.ERROR_CODE_BADARGUMENT);
			return mapping.findForward("ddsservices.error");
		}
		...
		if (fromDate != null || toDate != null) {
			...
			if (dateField == null) {
				df.setErrorMsg(
						"The 'dateField' argument must be specified when either the 'fromDate' or 'toDate' argument is indicated");
				df.setErrorCode(DDSServicesForm.ERROR_CODE_BADARGUMENT);
				return mapping.findForward("ddsservices.error");
			}
			...
			try {
				if (fromDate != null && toDate != null) {
					dateFilter = new DateRangeFilter(dateField, MetadataUtils.parseDate(fromDate),
							MetadataUtils.parseDate(toDate));
				...
				df.setErrorMsg("One or more dates indicated is incorrect: " + pe.getMessage()
						+ ". Dates must be of the form YYYY-MM-DD, YYYY-MM, YYYY or yyyy-MM-ddTHH:mm:ssZ");
				df.setErrorCode(DDSServicesForm.ERROR_CODE_BADARGUMENT);
				return mapping.findForward("ddsservices.error");
			}
		}
		...
		/*
		 * System.out.println(""); prtln("UserQuery was: '" + userQuery + "'"); String
		 * query = DDSQueryAction.formatFieldsInQuery(userQuery);
		 * prtln("UserQuery is: '" + query + "'"); prtln("Query was the same? " +
		 * userQuery.equals(query) ); System.out.println("");
		 */
		...
		// Limit the search by DCS status, if requested (not currently in protocol ddsws
		// v1.0)
		...
		if ((userQuery == null || userQuery.length() == 0) && (xmlFormat == null || xmlFormat[0].length() == 0)
				&& vocabQuery.length() == 0 && dateFilter == null && geospatialQuery == null) {
			isEmptySearch = true;
		}
		...
		if (index != null && !isEmptySearch) {
			...
			// prtln("doSearch() query for: '" + booleanQuery + "' had " + resultDocs.size()
			// + " results");
			// prtln("doSearch() query for: '" + query + "' had " + resultDocs.size() + "
			// results");
		}
		...
		// Pre-fetch the docs to ensure they are available for read and if not send
		// error
		// (can fail if indexer re-loads the index reader between search time and read
		// time):
		try {
		...
		// Log the query that the client requested:
		DDSQueryAction.logQuery(userQuery, servlet.getServletContext(), request, 200, -1, resultDocs.size(),
				getNumRecords(rm), df.getS(), DDSQueryAction.SEARCHTYPE_DDSWS_SEARCH);
	 ...
	 * @return An ActionForward to the JSP page that will handle the response
	 */
	protected ActionForward doUrlCheck(HttpServletRequest request, HttpServletResponse response, RepositoryManager rm,
			DDSServicesForm df, ActionMapping mapping) {
		...
		// prtln("doUrlCheck() search for: '" + q + "' had " + (resultDocs == null ? -1
		// : resultDocs.length) + " resultDocs");
	 ...
	 *                If error.
	 */
	protected ActionForward doListXmlFormats(HttpServletRequest request, HttpServletResponse response,
			RepositoryManager rm, DDSServicesForm df, ActionMapping mapping) throws Exception {
	 ...
	 *                If error.
	 */
	protected ActionForward doListCollections(HttpServletRequest request, HttpServletResponse response,
			RepositoryManager rm, DDSServicesForm df, ActionMapping mapping) throws Exception {
		...
		if (rm.getSetStatusModifiedTime() > setStatusModifiedTime
				|| rm.getIndexLastModifiedCount() > indexLastModifiedCount) {
			context.removeAttribute(getListCollectionsCacheKey());
		...
		// Records in the 'collect' collection are those that the Repository configures
		// as collections:
		...
		/*
		 * if (results != null) { Collections.sort(results, new
		 * LuceneFieldComparator("shorttitle", LuceneFieldComparator.ASCENDING)); }
		 */
	 ...
	 *                If error.
	 */
	protected ActionForward doListFields(HttpServletRequest request, HttpServletResponse response, RepositoryManager rm,
			DDSServicesForm df, ActionMapping mapping) throws Exception {
	 ...
	 *                If error.
	 */
	protected ActionForward doListTerms(HttpServletRequest request, HttpServletResponse response, RepositoryManager rm,
			DDSServicesForm df, ActionMapping mapping) throws Exception {
		...
		if (fields == null || fields.length == 0) {
			df.setErrorMsg(
					"The 'field' argument is required for request ListTerms. Please indicate one or more fields.");
			df.setErrorCode(DDSServicesForm.ERROR_CODE_BADARGUMENT);
			return mapping.findForward("ddsservices.error");
		}
		...
		if (isEmptyFields) {
			df.setErrorMsg(
					"The 'field' argument is required for request ListTerms, but is empty. Please indicate one or more fields.");
			df.setErrorCode(DDSServicesForm.ERROR_CODE_BADARGUMENT);
			return mapping.findForward("ddsservices.error");
		}

		/*
		 * DDSReportingForm drf = new DDSReportingForm(); drf.setIndex(rm.getIndex());
	...
	/**
	 * Handles a request to list a vocab and it's values including GradeRanges,
	 * Subjects, ResourceTypes and ContentStandards.
	 * <p>
	 *
	 * Error Exception Conditions: <br>
	 * badArgument - The request includes illegal arguments.
	 ...
	 *                If error.
	 */
	protected ActionForward doListVocabEntries(HttpServletRequest request, HttpServletResponse response,
			RepositoryManager rm, DDSServicesForm df, ActionMapping mapping, String verb) throws Exception {
	 ...
	 *                If error.
	 */
	protected ActionForward doServiceInfo(HttpServletRequest request, HttpServletResponse response,
			RepositoryManager rm, DDSServicesForm df, ActionMapping mapping) throws Exception {
	...
	/**
	 * Gets the number of records that have been indexed inclucing adn, collection,
	 * anno and other metadata types regardless of status. Specifically, number of
	 * index entries that are not errordocs.
	 *
	 * @param rm
	...
	/**
	 * Sets the maximum number of search results that can be returned by the
	 * service.
	 *
	 * @param maxResults
	 *            The new maxSearchResults value
	 */
