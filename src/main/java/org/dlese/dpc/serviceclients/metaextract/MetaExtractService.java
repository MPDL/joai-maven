...
public class MetaExtractService {
	...
	/*
	 * metaExtractUrl = " http://ada.syr.edu:8080/mast/service.do? method=collect&
	 * query=http://www.nationalgeographic.com/xpeditions/lessons/01/gk2/friends.
	 * html& queryType=URL"
	 * 
	 * http://ada.syr.edu:8080/mast/service.do&method=collect
	 * &query=http://interactive2.usgs.gov/learningweb/teachers/globalchange_earth.
	 * htm &queryType=URL
	 * 
	 * 
	 */
	...
	protected String getErrorMsg(Element errorElement) {
		...
		/*
		 * try { String returnCode = errorElement.element("ReturnCode").getTextTrim();
		 * if (returnCode.length() > 0) errorMsg += " (returnCode: " + returnCode + ")";
		 * } catch (Throwable t) {}
		 * 
		 * try { String helpMessage = errorElement.element("HelpMessage").getTextTrim();
		 * if (helpMessage.length() > 0) { // errorMsg += " HelpMessage: " +
		 * helpMessage; errorMsg += " " + helpMessage; } } catch (Throwable t) {}
		 */
