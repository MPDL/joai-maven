 ...
 * connections (time out is adjustable).
 * <p>
 ...
 * <li>parse the response into either a value or exception, which are returned
 * to the caller.
 * </ol>
 ...
 * <ol>
 * <li>UrlCheck - finds resources matching a URL (possibly wildcarded), and</li>
 * <li>GetRecord - retrieves a given record by ID</li>
 * </ol>
 * <p>
 * The DCS helpers support the following repository services:
 * <ol>
 * <li>PutRecord - inserts a metadata record into a specified collection in a
 * remote DCS repository
...
public class WebServiceClient {
	...
	/**
	 * Submit a request (query) to the UrlCheck Web service and return the response
	 * as a {@link org.dom4j.Document}. The UrlCheck service returns items that
	 * match the query. the query is a url and may contain asterisks as wildcards.
	 *
	 * @param s
	 *            query to be submitted to UrlCheck service.
	 * @return result from UrlCheck Service as Document
	 * @exception WebServiceClientException
	...
	public Document doSearch(String queryStr) throws WebServiceClientException {
		String verb = "Search";
	 ...
	 *
	 * @param recordXml
	 *            xml record to be put
	 * @param xmlFormat
	 *            metadata format of xml record (e.g., "adn")
	 * @param collection
	 ...
	 *                Description of the Exception
	 */
	public String doPutRecord(String recordXml, String xmlFormat, String collection, String id, String status,
			String statusNote) throws WebServiceClientException {
		// prtln("doPutRecord()");
		// prtln (recordXml);
		String errorMsg;
		try {
	 ...
	 * exception that contains the error message. otherwise, the Id is returned as a
	 * string
	 *
	 * @param collection
	 *            Description of the Parameter
	...
	/**
	 * Submits a request to the GetRecord DDS Web Service and returns response as a
	 * {@link org.dom4j.Document}. The GetRecord service returns an ADN record
	...
	/**
	 * retreives the contents of the <b>requestUrl</b> field as a
	 * {@link org.dom4j.Document}
	 *
	 * @return dom4j Document representation of response
	 * @exception WebServiceClientException
	 *                if unsuccessful retrieving url or parsing doc
	 */
	 ...
	 *                Description of the Exception
	 */
	public static Document getResponseDoc(URL url) throws WebServiceClientException {
	...
	/**
	 * Uses a {@link org.dlese.dpc.util.TimedURLConnection} to get the repsonse from
	 * the web service (the request is a URL), which is returned as a String.
	 *
	 * @param url
	 *            Description of the Parameter
	...
	public static String getTimedURL(URL url) throws WebServiceClientException {
		...
		try {
			...
			if (exc.getMessage().matches(".*respcode.*")) {
				msg = "The request for data resulted in an invalid response from the server."
						+ " The baseURL indicated may be incorrect or the service may be unavailable."
						+ " HTTP response: " + exc.getMessage();
			} else {
				msg = "The request for data resulted in an invalid response from the server. Error: "
						+ exc.getMessage();
			}
	 ...
	 */
	/*
	 * public static WebServiceClient getInstance(String baseUrl) { return new
	 * WebServiceClient(baseUrl); }
	 */
	/**
	 * gets the contents of a URL via {@link #getTimedURL(URL)} and then parses the
	 * contents into a dom4j Document, which is returned
	 *
	 * @param url
	 *            url to retrieve
	 * @return contents of url as dom4j Document, or null if unsuccessful
	 * @exception WebServiceClientException
	 *                Description of the Exception
	 * @exception DocumentException
	 *                Description of the Exception
	 */
	public static Document getTimedXmlDocument(URL url) throws WebServiceClientException, DocumentException {
