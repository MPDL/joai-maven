...
public class ADNItemDocReader {
	...
	/*
	 * the following fields are defined for ItemDocReader and therefore are defined
	 * here for compatability. at the end of this file are the corresponding
	 * accessors and utilities for manipulating these fields, although for now they
	 * have been deactivated by short-circuting initAnnosByType
	 */
	/**
	 * Description of the Field
	 */
	...
	/**
	 * parse a GetRecordResponse (a response from a GetRecord request), to create an
	 * ADNItemDocReader. This constructor called from RemoteResultDoc and
	 * SchemEditAction
	 *
	 * @param id
	 *            Description of the Parameter
	...
	public String getCollectionKey() {
		if (vocab != null && collection != null && collection.trim().length() > 0) {
			try {
	...
	// ------------------------------ utility ------------------------------
	/**
	 * Gets the vocab encoded keys for the given values as a String Array. Keys are
	 * found by VocabUtils.getFieldContent and converted from '+' delimited string
	 * into an Array.
	 *
	 * @param values
	...
	/*
	 * ---------------- Annotations display, etc. -------------------------- these
	 * methods are from ItemDocReader, and return the empty list (EMPTY_LIST) or
	 * "false" where appropriate. at this point in time the information required to
	 * calculate real results for these methods is not available via the DDS Web
	 * Services. When the required information becomes available, these methods can
	 * be fleshed out
	 */
	/**
	...
	public ResultDocList getAnnotationResultDocs() {
		return null;
		/*
		 * try { if (annotationDocReadersInitialized) { return annotationResultDocs; }
		 * annotationDocReadersInitialized = true; if (recordDataService == null ||
		 * !hasAnnotations()) { return null; } String[] ids = getId().split("\\s+");
		 * ArrayList allIds = new ArrayList(); for (int i = 0; i < ids.length; i++) {
		 * allIds.add(ids[i]); } ids = getAssociatedIds(); if (ids != null) { for (int i
		 * = 0; i < ids.length; i++) { allIds.add(ids[i]); } }
		 * //prtln("getAnnotationResultDocs() index query"); annotationResultDocs =
		 * recordDataService.getDleseAnnoResultDocs((String[]) allIds.toArray(new
		 * String[]{})); return annotationResultDocs; } catch (Throwable e) {
	 ...
	 * @param type
	 *            The annotation type.
	 * @return A list of all completed annotataions for this item of the given type,
	 *         or an empty list.
	 */
	 ...
	 * @param status
	 *            Annotation status.
	 * @return True if the item has an annotation in progress of the given status,
	 *         otherwise false.
	 */
	 ...
	 * @param status
	 *            Annotation status.
	 * @return All annotations for this item that have the given status, or empty
	 *         list.
	 */
	...
	protected void initAnnosByType() {
		...
		/*
		 * ArrayList annoList; boolean isCompleted; for (int i = 0; i <
		 * annoResults.length; i++) { DleseAnnoDocReader annoDocReader =
		 * (DleseAnnoDocReader) annoResults[i].getDocReader(); String annoType =
		 * annoDocReader.getType(); String annoStatus = annoDocReader.getStatus();
		 * isCompleted = annoDocReader.isCompleted(); if (isCompleted) {
		 * numCompletedAnnos++; annoList = (ArrayList)
		 * completedAnnosByType.get(annoType); completedAnnos.add(annoDocReader); } else
		 * { if (annoDocReader.isTextInProgress()) { numTextInProgress++; } if
		 * (annoDocReader.isAudioInProgress()) { numAudioInProgress++; } if
		 * (annoDocReader.isGraphicalInProgress()) { numGraphicalInProgress++; }
		 * numInProgressAnnos++; annoList = (ArrayList)
		 * inProgressAnnosByStatus.get(annoStatus); } if (annoList == null) { annoList =
		 * new ArrayList(); } annoList.add(annoDocReader); if (isCompleted) {
		 * completedAnnosByType.put(annoType, annoList); } else {
