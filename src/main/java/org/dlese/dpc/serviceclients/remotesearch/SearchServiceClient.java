 ...
 * <p>
 *
 * SearchServiceClient employs the WebServiceClient to do the actual search (via
 * the <b>doSearch</b> method (so this class can be thought of as a wrapper for
...
public class SearchServiceClient {
	...
	public SearchServiceClient(String searchServiceBaseUrl) {
		try {
			...
			if (wsc == null) {
				throw new Exception(
						"falied to create a WebServiceClient.  searchServiceBaseUrl: " + searchServiceBaseUrl);
			}
		} catch (Throwable t) {
	 ...
	 * {@link org.dlese.dpc.serviceclients.remotesearch.WebServiceClient}) and
	 * returns results as a List of
	 * {@link org.dlese.dpc.serviceclients.remotesearch.reader.ADNItemDocReader}
	 * instances.
	 *
	 * @param s
	...
	/**
	 * Constructs a query string to find records in a specified collection having a
	 * specified url in either the primaryUrl or mirrorUrl fields
	 *
	 * @param url
	 *            url to search for
	 * @param collection
