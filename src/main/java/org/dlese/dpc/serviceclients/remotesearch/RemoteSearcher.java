 ...
 * {@link org.dlese.dpc.index.SimpleLuceneIndex} in the DDS, only it performs
 * searches for records using a DDS Web Service (version 1.0). RemoteSearcher
 * employs the WebService client to do the actual search (via the
 * <b>urlCheck</b> method (so this class can be thought of as a wrapper for the
 * urlCheck Service).
 * <p>
 *
 ...
 * <li>the result Document is parsed into an array of {@link RemoteResultDoc}
 * </li>
 * </ol>
 *
 *
...
public class RemoteSearcher {
	 ...
	 *
	 * @param ddsWebServicesBaseUrl
	 *            DDS Web Service URL
	 * @param vocab
	 *            MetadataVocab object used to parse search results
	 */
	...
	public RemoteResultDoc[] searchDocs(String s) {
		...
		if (matchingRecords.size() == 0) {
			...
			for (Iterator i = matchingRecords.iterator(); i.hasNext();) {
				...
				List alsoCatalogedBys = matchingRecord
						.selectNodes("//additionalMetadata[@realm='adn']/alsoCatalogedBy");
