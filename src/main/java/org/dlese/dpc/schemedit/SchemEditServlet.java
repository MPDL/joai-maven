...
public final class SchemEditServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		try {
			...
			/*
			 * String instanceHelp = (String)
			 * servletContext.getInitParameter("instanceHelp");
			...
			MetadataVocabServlet metadataVocabServlet = (MetadataVocabServlet) servletContext
			...
			try {
				...
				collectionRegistry = new CollectionRegistry(collectionConfigDir, idFilesDir.getAbsolutePath(),
			...
			if (dcsDataManager == null) {
				...
				/*
				 * explicitly call loadCollectionRecords once the dcsDataManager is available to
				 * DcsFileIndexingPlugIn. The collectionRecords may have already been loaded,
				 * but without the dcsDataManager, DcsFileIndexingPlugin can't access the data
				 * it needs to index.
				 */
			...
			String contributorOutXform = getAbsolutePath(
					(String) servletContext.getInitParameter("contributorOutXform"));

			ExportingService exportingService = new ExportingService(servletContext,
					repositoryData + "/exporting_service_data", exportBaseDir, contributorOutXform);
			servletContext.setAttribute("exportingService", exportingService);

			ValidatingService validatingService = new ValidatingService(servletContext,
					repositoryData + "/validating_service_data");
			...
			/*
			 * Ensure that the ndrServiceEnabled context attribute has a value. This context
			 * attribute is set by NdrServlet, so if it is not set at this point we set it
			 * to false and it will be overwritten by NdrServlet if that servlet is
			 * configured to run.
			 */
			...
			// if OAI-PMH is enabled, set up OAI provider. This must be called AFTER
			// repository
	...
	private void oaiProviderInit(RepositoryManager rm) throws Exception {
		...
		// supply xmlformat schema and namespaces from registered frameworks to
		// repository manager
	 ...
	 *            The new repositoryWriterPlugins value
	 */
			...
			while (enumeration.hasMoreElements()) {
				...
				if (param.toLowerCase().startsWith("repositorywriterplugin")) {
					...
					try {
						...
						// Make the ServletContext available to all
						// ServletContextRepositoryWriterPlugins
						...
						prtlnErr("Error: setRepositoryWriterPlugins(): could not instantiate class '" + paramVal.trim()
								+ "'. " + e);
						continue;
					}
				}
	 ...
	 *
	 * @param repositoryService
	 *            The new repositoryEventListeners value
	 */
			...
			while (enumeration.hasMoreElements()) {
				...
				if (param.toLowerCase().startsWith("repositoryeventlistener")) {
					...
					try {
						...
						prtlnErr("Error: setRepositoryEventListeners(): could not instantiate class '" + paramVal.trim()
								+ "'. " + e);
						continue;
					}
				}
	...
	void convertionServiceReport(RepositoryManager rm, FrameworkRegistry reg) {
		...
		/*
		 * / test the rm schema map prtln ("\n repositoryManager schemaURLs"); Map
		 * schemaURLs = rm.getMetadataSchemaURLs(); for (Iterator i =
		 * schemaURLs.entrySet().iterator();i.hasNext();) { Map.Entry entry =
		 * (Map.Entry)i.next(); prtln ("\t" + entry.getKey() + ": " + entry.getValue());
		 * }
		 */
	...
	/**
	 * Build a query that finds all items with having the given statusValue as their
	 * statusLabel
	 *
	 * @param statusLabels
	 *            NOT YET DOCUMENTED
