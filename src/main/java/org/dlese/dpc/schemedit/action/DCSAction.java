 ...
 * variables. NOTE: using instance variables this way in an Action is dangerous.
 * We have to at least make sure we don't use any session-specific variables
 * this way, since the Action class is shared by all threads, and the
...
public class DCSAction extends Action {
	 ...
	 */
	public ActionErrors initializeFromContext(ActionMapping mapping, HttpServletRequest request)
		...
		// we should test these to make sure they aren't null and do something
		// reasonable if they are
		try {
			repositoryManager = (RepositoryManager) getRequiredContextAttributeValue("repositoryManager", errors);

			repositoryService = (RepositoryService) getRequiredContextAttributeValue("repositoryService", errors);

			frameworkRegistry = (FrameworkRegistry) getRequiredContextAttributeValue("frameworkRegistry", errors);

			collectionRegistry = (CollectionRegistry) getRequiredContextAttributeValue("collectionRegistry", errors);

			dcsDataManager = (DcsDataManager) getRequiredContextAttributeValue("dcsDataManager", errors);

			sessionRegistry = (SessionRegistry) getRequiredContextAttributeValue("sessionRegistry", errors);
			...
			accessManager = (AccessManager) getRequiredContextAttributeValue("accessManager", errors);

			userManager = (UserManager) getRequiredContextAttributeValue("userManager", errors);

			ndrServiceEnabled = (Boolean) getRequiredContextAttributeValue("ndrServiceEnabled", errors);
	 ...
	 * @exception Exception
	 *                if attribute is not found in ServletContext
	 */
	 ...
	 * supplied Request object) or the UNKNOWN_EDITOR value if a user is not found.
	 *
	 * @param request
	 *            the Request
	...
	protected String getSessionUserName(HttpServletRequest request) {
		...
		return (sessionUser != null ? sessionUser.getUsername() : Constants.UNKNOWN_EDITOR);
	}
