...
/**
 * A Struts Action for handling operations on single records, such as Copy,
 * Delete, Move, and New.
 *
 *
 *
...
public final class RecordOperationsAction extends DCSAction {
	 ...
	 * Struts forwarding name "search," which must be configured in
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		rof.setSets(repositoryService.getAuthorizedSets(getSessionUser(request), this.requiredRole));
	...
	private ActionForward handleMoveRecord(ActionMapping mapping, ActionForm form, HttpServletRequest request) {
		...
		// editRec -- signifies that the user is in the editor (rather than viewing or
		// searching)
		String exit = request.getParameter("exit");
		if (exit != null) {
			// WARNING: kludge follows: we need a way to handle moving from within the
			// editor
			// we could copy the code from this method to schemeEdit, OR we can stick an
			// editRec
			// control param in to help us ... editRec is flag that tells us if we are in
			// the editor.
			// if we are in the editor, then we don't use forwardToCaller, but rather we
			// invoke the
			...
			// NOTE: we don't have anything to release here, since we have just completed
			// the move
		...
		if (collection == null || collection.length() == 0) {
			...
			try {
				...
				errors.add("message", new ActionError("generic.message", "record moved to " + setInfo.getName()));
			} catch (Exception e) {
	 ...
	 */
	protected ActionForward handleCopyRecord(ActionMapping mapping, ActionForm form, HttpServletRequest request)
		...
		try {
			...
			// obtain info necessary for creating forwardPath from docReader for copied
			// record
			String collection = docReader.getCollection();
			...
			errors.add("error", new ActionError("record.op.failure", "copied", e.getMessage()));
			saveErrors(request, errors);
			return mapping.findForward("browse.query");
		} catch (Throwable t) {
			t.printStackTrace();
			errors.add("error", new ActionError("record.op.failure", "copied", "server error"));
			saveErrors(request, errors);
			return mapping.findForward("browse.query");
		}
	}
	...
	protected ActionForward handleNewRecord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			Document record = framework.makeMinimalRecord(id, collectionRegistry.getCollectionConfig(collection),
			...
			e.printStackTrace();
			errors.add("error", new ActionError("record.op.failure", "created", e.getMessage()));
			saveErrors(request, errors);
			return mapping.findForward("error.page");
		}
	 ...
	 * @return Description of the Return Value
	 */
	private ActionForward handleDeleteRecord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			errors.add("error", new ActionError("record.op.failure", "deleted: ", e.getMessage()));
		} finally {
			sessionBean.releaseLock(recId);
		}
