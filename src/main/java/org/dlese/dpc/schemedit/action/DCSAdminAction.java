 ...
 * metadata repository.
 *
 * @author John Weatherley, ostwald
 *         <p>
 *
 *
 */
public final class DCSAdminAction extends DCSAction {
		...
		try {
			...
			if (req.getParameter("command") != null) {
				...
				if (paramVal.equalsIgnoreCase("exportAll")) {
					...
					exportTask.run();
					errors.add("message",
							new ActionError("generic.message", "Export is underway as a background process"));
					saveErrors(req, errors);
					return mapping.findForward("config.settings");
				}
				...
				if (paramVal.equals("showIndexingMessages")) {
					...
					/*
					 * int maxMessagesToShow = 10; int totalMessages = indexingMessages.size(); int
					 * lastMessage = (totalMessages >= maxMessagesToShow ? totalMessages -
					 * maxMessagesToShow : 0);
					 * 
	 ...
	 */
	private final boolean isValidSet(String setName, String setSpec, String setDescription, DCSAdminForm daf,
			HttpServletRequest req, boolean checkSetSpec, RepositoryManager repositoryManager) {
	 ...
	 */
	private ActionForward handleDebug(ActionMapping mapping, ActionForm form, HttpServletRequest req,
			HttpServletResponse response) throws Exception {
		prtln("debug");
		ActionErrors errors = new ActionErrors();
		DCSAdminForm daf = (DCSAdminForm) form;
