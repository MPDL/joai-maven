...
public class DCSSchemEditAction extends AbstractSchemEditAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (errors.isEmpty() && xmlFormat == null && recId != null) {
			...
			if (errors.isEmpty() && !repositoryService.isAuthorizedSet(docReader.getCollection(),
					this.getSessionUser(request), this.requiredRole)) {
				errors.add("error", new ActionError("generic.error", "You are not authorized to edit record " + recId));
			}
	...
	/**
	 * Handles requests containing a "command" parameter that was not handled in the
	 * superclass.
	 *
	 * @param mapping
	 *            the ActionMapping
	 ...
	 *                if a lock to the record being edited is not held.
	 */
	protected ActionForward handleOtherCommands(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			} else if (command.equals("suggestStandards") || command.equals("moreLikeThis")) {
				if (!sessionBean.ownsLock(sef.getRecId())) {
					throw new MissingLockException();
				}
			...
			if ("true".equals(request.getParameter("async")) || "async".equals(command)) {
	 ...
	 *                Description of the Exception
	 */
	protected ActionForward handleMissingCommand(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	 ...
	 *                Description of the Exception
	 */
	protected ActionForward handleSaveRequest(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response, SchemEditValidator validator) throws ServletException, MissingLockException {
		SchemEditForm sef = (SchemEditForm) form;
		SessionBean sessionBean = this.getSessionBean(request);
		...
		if (uniqueUrlPath != null) {
			/*
			 * currentUrl is the current value of the uniqueUrlPath element if currentUrl is
			 * same as validatedUrl, then we don't validate again, since this means the user
			 * has hit "save" after getting the similarUrl message
			 */
		...
		try {
			...
			// report required paths - need to more gracefully handle missing required
			// values!!
			...
			errors.add("error", new ActionError("generic.error",
					"Could not save record: A field required by the indexer is missing"));
			saveErrors(request, errors);
			return getEditorMapping(mapping);
		} catch (Exception e) {
	...
	private void confirmNdrWrite(ResultDoc resultDoc, SchemEditForm sef, ActionErrors errors) {
		try {
			...
			if (config.isNDRCollection()) {
				DcsDataRecord dcsDataRecord = dcsDataManager.getDcsDataRecord(sef.getRecId(), repositoryManager);
				...
				if (handle != null && handle.trim().length() > 0
					...
					errors.add("message", new ActionError("ndr.save.confirmation"));
				}
			}
		} catch (Throwable t) {
	...
	/**
	 * Wrap handleDeleteElement so we can update suggestion service helper in the
	 * case that the standards field has been deleted.
	 */
	 ...
	 *                NOT YET DOCUMENTED
	 */
	protected ActionForward handleCancelRequest(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws ServletException {
	 ...
	 *                Description of the Exception
	 */
	protected ActionForward handleEditRequest(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws ServletException {
		...
		if (!getSessionBean(request).getLock(sef.getRecId())) {
			errors.add("recordLocked", new ActionError("lock.not.obtained.error", sef.getRecId()));
			saveErrors(request, errors);
			return mapping.findForward("error.page");
		}
		...
		try {
			 ...
			 */
			if (helper != null) {
				// we want to start out with a standards doc that has some selections, or the
				// default

				// NOTE: ADN helper has no available docs, so we can't throw exception if this
				// test returns 0
	...
	/**
	 * Placeholder method to overide that of AbstractSchemEditAction. Never used but
	 * necessary to avoid compile error.
	 *
	 * @param mapping
	 *            the ActionMapping
	 * @param form
	 ...
	 *                Description of the Exception
	 */
	protected ActionForward handleNewRecordRequest(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws ServletException {

		ActionErrors errors = new ActionErrors();

		errors.add("error", new ActionError("generic.error", "bad command name: \"newRecord\""));
		saveErrors(request, errors);
		return mapping.findForward("error.page");
	}

	/**
	 * Saves a metadata record either to disk in the case of a stand-alone metadata
	 * editor, or to the repository manager in the case of the integrated metadata
	 * editor.
	 *
	 * @param mapping
	 *            Description of the Parameter
	 ...
	 *                Description of the Exception
	 */
	protected void putRecord(ActionMapping mapping, ActionForm form, HttpServletRequest request)
	 ...
	 *                Description of the Exception
	 */
	private ActionForward getAssignedStandards(ActionMapping mapping, ActionForm form, HttpServletRequest request)
	 ...
	 *                NOT YET DOCUMENTED
	 */
	private ActionForward doUpdateSuggestions(ActionMapping mapping, ActionForm form, HttpServletRequest request)
			throws ServletException {
		...
		try {
			...
			if ("true".equals(request.getParameter("async"))) {
				...
				errors.add("error", new ActionError("generic.message", "Unable to communicate with Suggestion server"));
			}
		}
	 ...
	 *                Description of the Exception
	 */
	private ActionForward handleAsyncRequest(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	 ...
	 * parameter contained in request.
	 * <p>
	 *
	 ...
	 *                NOT YET DOCUMENTED
	 */
	private ActionForward handleStandardsDisplay(ActionMapping mapping, ActionForm form, HttpServletRequest request)
			throws ServletException {
		...
		/*
		 * prtln ("\n==========================="); prtln ("Display State"); prtln
		 * ("request.pathArg: " + pathArg); prtln ("request.displayMode: " +
		 * displayMode); prtln ("request.displayContent: " + displayContent); prtln
		 * ("request.currentStdDocKey: " + currentStdDocKey); prtln (""); prtln
		 * ("helper.displayMode: " + helper.getDisplayMode()); prtln
		 * ("helper.displayContent: " + helper.getDisplayContent()); prtln
		 * ("helper.currentDoc: " + helper.getCurrentDoc()); prtln ("===========\n");
		 */
	 ...
	 *                NOT YET DOCUMENTED
	 */
	private ActionForward handleResQualBenchmarksDisplay(ActionMapping mapping, ActionForm form,
	 ...
	 * @param sef
	 *            The ActionForm
	 * @return The suggestionServiceHelper value, or null if this framework does not
	 *         implement a suggestionService
	 * @exception Exception
	 *                if a SuggestServiceHelper cannot be initialized
	 */
	protected CATServiceHelper initSuggestionServiceHelper(SchemEditForm sef) throws Exception {
		...
		if (suggestionServiceHelper == null) {
			// initialize serviceHelper and set sef.currentStdDocKey to that helper's
			// default
			try {
		...
		// prtln ("suggestServiceHelper class: " +
		// suggestionServiceHelper.getClass().getName());
		...
		if (suggestionServiceHelper instanceof AsnSuggestionServiceHelper) {
			try {
				...
				if (!asnServiceHelper.getCurrentDoc().equals(currentStdDocKey)) {
					...
					/*
					 * prtln ("\nasnServiceHelper.currentDocKey: " +
					 * asnServiceHelper.getCurrentDoc()); prtln ("sef.currentStdDocKey: " +
					 * currentStdDocKey); prtln ("SETTING ASN DOC TO: " + currentStdDocKey);
					 */
