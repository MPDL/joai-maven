...
public final class StatusAction extends DCSAction {
	 ...
	 * {@link org.dlese.dpc.repository.RepositoryManager} must be available to this
	 * class via a ServletContext attribute under the key "repositoryManager."
	 * Returns an {@link org.apache.struts.action.ActionForward} instance which must
	 * be configured in struts-config.xml to forward to the JSP page that will
	 * handle the request.
	 *
	 * @param mapping
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			statusForm.clear();
			errors.add("error", new ActionError("generic.error", "No command submitted - no action taken"));
			saveErrors(request, errors);
			return mapping.findForward("edit.status");
			...
			if (e instanceof NullPointerException)
				e.printStackTrace();
			errors.add("actionSetupError", new ActionError("generic.error", msg));
	 ...
	 *                NOT YET DOCUMENTED
	 */
	private ActionForward handleDeleteEntry(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	 ...
	 *                NOT YET DOCUMENTED
	 */
	private ActionForward handleUpdateEntry(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ActionErrors errors = new ActionErrors();
		StatusForm statusForm = (StatusForm) form;
		String entryKey = statusForm.getEntryKey();
	...
	/**
	 * Set up to edit a history item (corresponding to "entryKey" of the current
	 * status record
	 *
	 * @param mapping
	 *            NOT YET DOCUMENTED
	 ...
	 *                NOT YET DOCUMENTED
	 */
	private ActionForward handleEditEntry(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		StatusForm statusForm = (StatusForm) form;
		DcsDataRecord dcsDataRecord = statusForm.getDcsDataRecord();
		String entryKey = statusForm.getEntryKey();
	 ...
	 *                NOT YET DOCUMENTED
	 */
	private ActionForward handleEditRecord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ActionErrors errors = new ActionErrors();
		StatusForm statusForm = (StatusForm) form;
		String id = request.getParameter("recId");
		if (id == null || id.trim().length() == 0) {
			errors.add("error", new ActionError("generic.error", "a record id is required to edit status"));
			saveErrors(request, errors);
			return forwardToCaller(request, statusForm);
		}
	 ...
	 * @return NOT YET DOCUMENTED
	 */
	private ActionForward handleSaveRecord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {
		ActionErrors errors = new ActionErrors();
		StatusForm statusForm = (StatusForm) form;
		String changeDate = SchemEditUtils.fullDateString(new Date());
		String username = this.getSessionUserName(request);
		StatusEntry statusEntry = new StatusEntry(statusForm.getStatus(), statusForm.getStatusNote(), username,
				changeDate);
		String recId = statusForm.getRecId();
		String entryKey = statusForm.getEntryKey();
		...
		try {
			...
			// don't forward anywhere if we are modal (we risk landing on a page that
			// releases locks)
			if (request.getParameter("modal") != null) {
				return null;
			}
			...
			errors.add("message", new ActionError("generic.message", errorMsg));
			saveErrors(request, errors);
			return mapping.findForward("edit.status");
		}
	}
	...
	/**
	 * Generate an ActionForward that returns control to the calling page if
	 * possible.
	 *
	 * @param request
	 *            NOT YET DOCUMENTED
	...
	private ActionForward forwardToCaller(HttpServletRequest request, StatusForm statusForm) {
		...
		try {
			...
			// strip the context path from forward so it conforms to the syntax of the
			// action forward path
			if (contextPath.length() > 0 && forwardPath.startsWith(contextPath))
				forwardPath = forwardPath.substring(contextPath.length());
			else
				throw new Exception(
						"forwardToCaller: expected context path \"" + contextPath + "\" not found in referer");
