...
public abstract class CreateRecordAction extends DCSAction {
	 ...
	 */
	protected abstract ActionForward getConfirmForward(ActionMapping mapping, CreateADNRecordForm carForm,
	 ...
	 * {@link org.apache.struts.action.ActionForward} instance that maps to a Struts
	 * forwarding name, which must be configured in struts-config.xml to forward to
	 * the JSP page that will handle the request.
	 *
	 * @param mapping
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			carForm.clear();
			errors.add("message", new ActionError("generic.message", "No command submitted - no action taken"));
			saveErrors(request, errors);
			return this.getCreateForward(mapping);
		} catch (Throwable e) {
	 ...
	 */
	protected ActionForward handleSubmit(CreateADNRecordForm carForm, MetaDataFramework framework,
		...
		if (framework.getUniqueUrlPath() != null) {
			/*
			 * 1 - if we have not seen this url (primaryUrl) before we need to check for
			 * sims. validatedUrl is set to preserve a url that is valid, but which also has
			 * sims - note: validatedUrl is set below, so it was set in the previous call to
			 * handleSubmit) case A) if validatedUrl is null, then we have not tested for
			 * sims and must do so now. if sim(s) are found, then we will preserve the
			 * primaryUrl value as "validatedUrl" and present a confirmation message to user
			 * (this has sims, do you really want to save?). case B) if primaryUrl is not
			 * equal to validatedUrl, then the user has changed the previous url and we have
			 * to evaluate again for sims (setting validatedUrl if sims are found, etc) 2 -
			 * if validatedUrl and primaryUrl are the same, then user has confirmed that the
			 * primaryUrl is to be used (even though it is similar), and we can perform the
			 * submit operation
			 */
			if (carForm.getValidatedUrl() == null || !carForm.getPrimaryUrl().equals(carForm.getValidatedUrl())) {
				prtln("checking sims");
				...
				if (sims.size() > 0) {
					...
					errors.add("message", new ActionError("similar.urls.found", Integer.toString(sims.size())));
					saveErrors(request, errors);
					return this.getCreateForward(mapping);
				}
			}
	 ...
	 */
	private ActionForward handleNewRecordRequest(CreateADNRecordForm carForm, MetaDataFramework framework,
			ActionMapping mapping, HttpServletRequest request, HttpServletResponse response) {
		...
		errors.add("message", new ActionError("adn.creation.confirmation"));
		saveErrors(request, errors);
	 ...
	 */
	protected abstract Document makeRecordDoc(CreateADNRecordForm carForm, MetaDataFramework framework,
			ActionMapping mapping, HttpServletRequest request, HttpServletResponse response) throws Exception;
	 ...
	 * @return Description of the Return Value
	 */
	protected abstract ActionErrors validateForm(HttpServletRequest request, CreateADNRecordForm carForm,
