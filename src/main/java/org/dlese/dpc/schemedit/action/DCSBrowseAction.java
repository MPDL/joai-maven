 ...
 * DCS. This class works in conjunction with the
 * {@link org.dlese.dpc.schemedit.action.form.DCSBrowseForm} Struts form bean
 * class.
 *
...
public final class DCSBrowseAction extends DCSAction {
	 ...
	 * Struts forwarding name "browse.collection," which must be configured in
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		/*
		 * clearing search params effectively forgets the state of the search page. this
		 * line commented out so search is remembered ...
		 */
	...
	/**
	 * Return mapping from collectionKey to permission for authorized sets. Used to
	 * display the sessionUser's role for each collection in the collections table.
	 *
	 * @param user
	 *            NOT YET DOCUMENTED
	 * @param setInfos
	...
	private ActionErrors indexCheck(ActionForm form, HttpServletRequest request) {
		...
		for (Iterator i = sets.iterator(); i.hasNext();) {
			...
			// prtln ("indexCheck for " + set.getName() + " numIndexed + numErrors = " +
			// numIndexed + ", numFiles = " + numFiles);
	...
	/*
	 * private ActionErrors getFrameworkRegistryErrors ( ActionForm form,
	 * HttpServletRequest request) { DCSBrowseForm browseForm = (DCSBrowseForm)
	 * form; ActionErrors errors = new ActionErrors ();
	 * 
	 * ErrorLog errorLog = browseForm.getFrameworkLoadErrors (); if (errorLog !=
	 * null && !errorLog.isEmpty()) { for (Iterator
	 * i=errorLog.getEntries().iterator();i.hasNext();) { ErrorLog.LogEntry entry =
	 * (ErrorLog.LogEntry) i.next(); String name = entry.getName(); String msg =
	 * entry.getMsg(); errors.add ("frameworkMessages", new ActionError
	 * ("generic.error", name + ": " + msg)); } } prtln
	 * ("frameworkRegistryErrors returning " + errors.size() + " messages"); return
	 * errors; }
	 */
	// -------------- Debug ------------------
