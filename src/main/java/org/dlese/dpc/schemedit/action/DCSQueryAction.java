 ...
 * {@link org.dlese.dpc.index.SimpleLuceneIndex}. This class works in
 * conjunction with the {@link org.dlese.dpc.schemedit.action.form.DCSQueryForm}
...
public final class DCSQueryAction extends DCSAction {
	 ...
	 * Struts forwarding name "browse.query," which must be configured in
	 ...
	 * @param request
	 */
	private void initializeDisplayParams(ActionMapping mapping, ActionForm form, HttpServletRequest request) {

		DCSQueryForm queryForm = (DCSQueryForm) form;
		SessionBean sessionBean = this.getSessionBean(request);

		// important: initialize searchHelper (results) so that it will be present even
	...
	/**
	 * Collect all the values for "lastEditor" in the index for the collections the
	 ...
	 * of collections the current user is authorized to access.
	 * <p>
	 *
	 * For each of the terms that occur in the index for the given indexField,
	 * perform a query to determine if the term occurs in an authorized collection,
	 * and if so, add it to the list of terms to be returned.
	 *
	 * @param sb
	 *            NOT YET DOCUMENTED
	 * @param indexField
	...
	private List findTermsInAuthorizedCollections(SessionBean sb, String indexField) {
		...
		for (int i = 0; i < authorizedCollections.size(); i++) {
			// OR the collections
			colQuery.add(new TermQuery(new Term("collection", "0" + authorizedCollections.get(i))),
	 ...
	 *                if a servlet exception occurs
	 */
	public ActionForward handleMetadataSearchRequest(SessionBean sessionBean, ActionMapping mapping,
			DCSQueryForm queryForm, HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if (request.getParameter("q") != null) {
				// ---------- make boolean Query
				BooleanQuery booleanQuery = dcsStandardQuery(request, this.repositoryManager, sessionBean, queryForm,
			...
			/*
			 * Display the text in the given file (handles viewing source, validating XML
			 * for admin e.g., Files that could not be indexed
			 */
				...
				if (resultDocs == null || resultDocs.size() == 0) {
					resultDocs = index
							.searchDocs("docsource:\"" + OAIUtils.encode(fileName).replaceAll("%2F", "/") + "\"");
				}
				...
				String metadata = ((FileIndexingServiceDocReader) ((ResultDoc) resultDocs.get(0)).getDocReader())
						.getFullContent();
				if (metadata != null && metadata.length() > 0) {
					// Filter out the XML and DTD declaration if requested
					if (request.getParameter("filterxml") != null) {
						metadata = XMLConversionService
								.stripXmlDeclaration(new BufferedReader(new StringReader(metadata))).toString();
					}
			...
			// Display the text in the given file given an item id (handles "view XML" &
			// "Validate Record")
				...
				String metadataPath = ((FileIndexingServiceDocReader) ((ResultDoc) resultDocs.get(0)).getDocReader())
						.getDocsource();
			...
			// No recognizable param existed:
				...
				errors.add("error", new ActionError("generic.error", "The request is not valid in this context."));
				saveErrors(request, errors);
				return mapping.findForward("browse.query");
			}
	...
	/**
	 * Create a query object to search for results specified in request, with custom
	 * query expansion.
	 * <p>
	 *
	 ...
	 */
	public static BooleanQuery dcsStandardQuery(HttpServletRequest request, RepositoryManager rm,
			SessionBean sessionBean, ActionForm form, ServletContext context) {
		...
		if (searchMode != null && !q.matches(".*\\s+.*") && (searchMode.equals("id") || searchMode.equals("url"))) {
			try {
		...
		/*
		 * In most cases we want to leave the sortField as it was. But if a searchString
		 * was provided, and there wasn't a sort specified by the request, then we make
		 * the sort by relevance. Otherwise then we use default ordering
		 */
		if (sortField == null) {
			if (sortByRelevance) {
		...
		// if Query.toString() fails, then we have a bad query (probably of the form
		// myfield:"")
		try {
	...
	private final static Query formatSearchModeQuery(String q, String queryMode, RepositoryManager rm,
			ServletContext context) throws org.apache.lucene.queryParser.ParseException {
	...
	private final static Query formatQuery(String q, RepositoryManager rm, ServletContext context)
		...
		try {
			...
			luceneQuery = FieldExpansionQueryParser.parse(fullQuery, rm.getIndex().getAnalyzer(), defaultSearchFields,
					fieldsUsedForBoosting, boostingValues, null, rm.getIndex().getLuceneOperator());
		} catch (org.apache.lucene.queryParser.ParseException pe) {
	 ...
	 */
	public static String getSelectionParamsQueryString(HttpServletRequest request, DCSQueryForm queryForm,
			RepositoryManager rm, SessionBean sessionBean) {
		String validity = request.getParameter("vld");
		String syncErrors = request.getParameter("syncErrors");
		// String selectedCollection = request.getParameter("sc"); // why have this AND
		// selectedCollections?
		...
		if (selectedEditors != null && selectedEditors.length > 0 && !selectedEditors[0].equals(SELECT_ALL)) {
		...
		if (selectedCreators != null && selectedCreators.length > 0 && !selectedCreators[0].equals(SELECT_ALL)) {
		...
		if (selectedFormats != null && selectedFormats.length > 0 && !selectedFormats[0].equals(SELECT_ALL)) {
		...
		if (selectedStatuses != null && selectedStatuses.length > 0 && !selectedStatuses[0].equals(SELECT_ALL)) {
	...
	/**
	 * Create a list of statusFlag values for querying the index.
	 * <p>
	 *
	 ...
	 */
	private static String[] getSelectedStatuses(HttpServletRequest request, DCSQueryForm queryForm,
		...
		if (statusValues.size() == 0) {
			/*
			 * the selected status labels are no longer in the statusMap. this will happen
			 * when: 1 - search is preformed for records with a reserved status, 2 - only
			 * one result is found, and then 3 - the status of the lone record is changed.
			 * in this case, the statusMap will not contain the searched-for status (since
			 * there are no records having this status), and therefore that value is not
			 * added to the statusValues list. since we didn't add the value to
			 * statusValues, we have effectively removed it from the query, so we must empty
			 * Sss so the sarch_user_selections will reflect this
			 */
	 ...
	 *
	 * @param sb
	 *            The new querySelectors value
	 */
	private void setQuerySelectors(SessionBean sb) {

		SimpleLuceneIndex index = repositoryManager.getIndex();

		if (!sb.isQuerySelectorsInitialized() || index.getLastModifiedCount() > sb.getIndexLastModified()
	...
	/**
	 * Create a mapping from unique status labels to a list of all the status values
	...
	/**
	 * Sets the nonPaigingParams attribute of the DCSQueryAction object (all request
	 * params except those that control the search results page being displayed).
	 *
	 * @param queryForm
	 *            The new nonPaigingParams value
	 * @param request
	 *            The new nonPaigingParams value
	 */
	...
	/**
	 * Sets the sortWidgetParams attribute of the DCSQueryAction object
	 * <p>
	 *
	 ...
	 *
	 * @param queryForm
	 *            The new sortWidgetParams value
	 * @param request
	 *            The new sortWidgetParams value
	 */
		...
		try {
			while (params.hasMoreElements()) {
				...
				if (!excludeParams.containsKey(param)) {
					...
					/*
					 * we encode only the query string. all other params (except status) are
					 * single-token and status has been quote-wrapped
					 */
