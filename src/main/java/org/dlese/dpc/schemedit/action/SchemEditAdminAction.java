...
import org.dlese.dpc.schemedit.action.form.SchemEditAdminForm;
 ...
 * @author ostwald
 *         <p>
 *         $Id: SchemEditAdminAction.java,v 1.16 2009/03/20 23:33:55 jweather
 *         Exp $
 */
public final class SchemEditAdminAction extends DCSAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (command.equals("update")) {
			...
			/*
			 * FormFile sampleFile = adminForm.getSampleFile(); if (sampleFile != null &&
			 * sampleFile.getFileSize() > 0) { try { doUploadSampleFile(sampleFile,
			 * adminForm, framework); String uploadMsg =
			 * "Sample File successfully uploaded"; errors.add("message", new
			 * ActionError("generic.message", uploadMsg)); saveErrors(request, errors); }
			 * catch (Exception e) { errorMsg = e.getMessage(); prtln(errorMsg);
			 * errors.add("pageErrors", new ActionError("generic.error", errorMsg)); } }
			 */
		...
		if (command.equals("loadFramework")) {
			try {
				...
				errors.add("pageErrors", new ActionError("generic.error",
						"Couldn't load framework for " + xmlFormat + ": " + t.getMessage()));
				saveErrors(request, errors);
				return mapping.findForward("admin");
			}
		}
		...
		if (command.equals("unloadFramework")) {
			try {
				...
				errors.add("pageErrors", new ActionError("generic.error",
						"Couldn't unload framework for " + xmlFormat + ": " + t.getMessage()));
				saveErrors(request, errors);
			}
		}
		...
		if (command.equals("reloadFieldInfo")) {
			...
			if (fieldInfoMap == null || fieldInfoMap.getKeySet().size() == 0) {
				errors.add("message", new ActionError("generic.message", "No Field Info found for this framework"));
				saveErrors(request, messages);
			} else {
				try {
					fieldInfoMap.reload();
					messages.add("message", new ActionError("generic.message", "Field Info reloaded"));
					saveErrors(request, messages);
				} catch (Exception e) {
				...
				if (errors.size() == 0) {
					try {
						...
						prtln(e.getMessage());
						errors.add("pageErrors", new ActionError("generic.error",
								"Editor pages could not be created because: " + e.getMessage()));
					}
				}
			}
		}
	 ...
	 *            Description of the Parameter
	 */
	public void doReloadSchema(SchemEditAdminForm adminForm, MetaDataFramework framework, ActionErrors errors,
		...
		try {
			...
			messages.add("message", new ActionError("generic.message", "Schema successfully loaded"));
		...
		if (errors.size() == 0) {
			try {
				framework.renderEditorPages();
				messages.add("message", new ActionError("generic.message", "Editor pages successfully regenerated"));
			} catch (Exception e) {
				prtln(e.getMessage());
				errors.add("pageErrors", new ActionError("generic.error",
						"Editor pages could not be created because: " + e.getMessage()));
	 ...
	 */
	/*
	 * private void doUploadSampleFile(FormFile sampleFile, SchemEditAdminForm
	 * adminForm, MetaDataFramework framework) throws Exception {
	 * 
	 * //retrieve the content type String contentType = sampleFile.getContentType();
	 * //retrieve the file size String size = (sampleFile.getFileSize() + " bytes");
	 * prtln("sampleFile size in bytes: " + size); String uploadMsg = ""; String
	 * data = null;
	 * 
	 * //retrieve the file data ByteArrayOutputStream baos = new
	 * ByteArrayOutputStream(); InputStream stream = sampleFile.getInputStream();
	 * 
	 * //write the file to the sampleFile String sampleFilePath =
	 * framework.getSampleRecordFile();
	 * 
	 * OutputStream bos = new FileOutputStream(sampleFilePath); int bytesRead = 0;
	 * byte[] buffer = new byte[8192]; while ((bytesRead = stream.read(buffer, 0,
	 * 8192)) != -1) { bos.write(buffer, 0, bytesRead); } bos.close(); uploadMsg =
	 * "The sampleFile has been written to \"" + sampleFilePath + "\""; //close the
	 * stream stream.close();
	 * 
	 * }
	 */
	...
	private ActionErrors validateFilePath(String path, String propertyId) {
		...
		if (!file.exists()) {
			...
			errors.add(propertyId, new ActionError("generic.error", "warning: file does not exist"));
		}
