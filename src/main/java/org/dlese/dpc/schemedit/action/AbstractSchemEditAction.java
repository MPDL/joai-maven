...
public abstract class AbstractSchemEditAction extends DCSAction {
	...
	protected SchemEditValidator getValidator(ActionForm form, ActionMapping mapping, HttpServletRequest request) {
		// prtln ("getValidator(): xmlFormat: " +
		// this.getMetaDataFramework().getXmlFormat());
	 ...
	 */
	protected abstract void putRecord(ActionMapping mapping, ActionForm form, HttpServletRequest request)
			throws Exception;
	 ...
	 * Struts forwarding name "xxx.xxx," which must be configured in
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		// SchemEditUtils.showRequestParameters(request, new String[]{"_^_"}); //
		// excluding collapse info

		// ----- framework-dependent -------------
		MetaDataFramework metadataFramework = getMetaDataFramework();
		if (metadataFramework == null) {

			// experimental (9/18/08) most of the time xmlFormat is null is after a timeout
			// experienced
		...
		try {
			...
			// change editor form (the input page)
				...
				// if a pathArg is present, expose that element and set the hash so the exposed
				// element
			...
			// newElement - add element to the instanceDocument
				...
				/*
				 * pathArg refers to the element that has just been added. if the element is a
				 * multiselect, then the hash is simply this path. otherwise, the element is
				 * indexed and the hash must point to the element just added
				 */
				String hash;
			...
			// CHIOCE
				...
				// guard against adding an illegal choice caused by refreshing the page after
				// adding a choice
				...
				try {
					/*
					 * we strip the leaf index from the path because with choices, we want the new
					 * choice to go at the end of the element, and the provided index does not make
					 * sense to docMap/targetedInsert, which expects the target to exist...
					 */
			 ...
			 */
				...
				if (sErrors.size() > 0) {
					...
					errors.add("message", new ActionError("validation.confirmation"));
					saveErrors(request, errors);
				}
				return getEditorMapping(mapping);
			}
			...
			// commands that REQUIRE ACCESSING REPOSITORY - MUST OBTAIN LOCK
				...
				try {
					...
					errors.add("error", new ActionError("generic.error",
							"The specified record cannot be edited:\n" + e.getMessage()));
					saveErrors(request, errors);
					return mapping.findForward("error.page");
				}
			}
			...
			prtln("Exception: " + e);
			e.printStackTrace();
			errors.add("error", new ActionError("generic.error", "There has been a system error: " + e.getMessage()));
			saveErrors(request, errors);
			return mapping.findForward("error.page");
		}
	 ...
	 * "uniqueValue" path for duplicate values in the same field in other records in
	 * the collection.
	 *
	 * @param mapping
	 *            the actionMapping
	 ...
	 */
	protected ActionErrors uniqueValueCheck(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			for (Iterator i = uniqueValuePaths.iterator(); i.hasNext();) {
				...
				/*
				 * we have a normalized path, - now test all the values for that path for
				 * uniqueness - Iterate through the InputFields to find the values (and xpaths)
				 * for the "uniqueValue" field. The (indexed) xpath allows us to associated
				 * feedback (in the UI) with the offending input element.
				 */
	 ...
	 */
	protected ActionErrors uniqueUrlCheck(ActionMapping mapping, ActionForm form, HttpServletRequest request) {
		...
		try {
			...
			// - if it null or empty we don't bother checking (this assumes empty urls are
			// okay)
			...
			// get cached Url from the savedContent map, in which values are stored in Lists
			// that
			...
			// no element found at uniqueUrlPath in savedContent - check for dups unless
			// dupCheck is disabled
			if (!doDupCheck) {
			...
			// the currentUrl is valid (it has no dups although there may be similars in
			// primary or mirror fields)
	...
	/**
	 * Gets the cachedValue from the savedContentMap
	 * <p>
	 *
	 ...
	 *
	 * @param xpath
	 *            path at which to retrieve value
	 * @param sef
	 *            NOT YET DOCUMENTED
	 ...
	 *                NOT YET DOCUMENTED
	 */
	protected ActionForward isDocDirty(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		Map currentContent = DocContentMap.getDocContentMap(sef.getDocMap().getDocument(), getMetaDataFramework());
	 ...
	 *                NOT YET DOCUMENTED
	 */
	protected ActionForward handleGuardedExit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			SchemEditValidator validator) throws ServletException {
		SchemEditForm sef = (SchemEditForm) form;
		ActionErrors errors = new ActionErrors();
		validator.updateMultiValueFields();
		...
		if (docIsDirty && !confirmedExit) {
			...
			// here we should check to see if the record exists in the index. if NOT, then
			// it
			// is new and must be explicitly saved or it will be lost altogether!
			// NOTE: DCSSchemEditAction works on the index, while StandAloneSchemEditAction
			// does not,
			...
			if ((this instanceof DCSSchemEditAction) && repositoryManager.getRecord(sef.getRecId()) == null) {
				...
				errors.add("error", new ActionError("generic.message", "Record contains unsaved changes"));
			}
			saveErrors(request, errors);
			return getEditorMapping(mapping);
		} else {
	...
	protected ActionForward handleExitCommand(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			SchemEditValidator validator) throws ServletException {
		SchemEditForm sef = (SchemEditForm) form;
		String forwardPath = request.getParameter("pathArg");
		String recId = sef.getRecId();
	 ...
	 *                NOT YET DOCUMENTED
	 */
	protected ActionForward handleTimedOutGuardedExit(ActionMapping mapping, ActionForm form,
	...
	/**
	 * Determines the appropriate editor mapping depending on whether the editor is
	 * configured to use a frame-based display.
	 *
	 * @param mapping
	 *            Description of the Parameter
	 ...
	 *
	 * @param mle
	 *            the missingLockException we are handling
	 * @param mapping
	 *            the actionMapping
	 * @param form
	 *            the actionForm
	 ...
	 *                NOT YET DOCUMENTED
	 */
	protected ActionForward handleMissingLockException(MissingLockException mle, ActionMapping mapping, ActionForm form,
		...
		if (command.equals("exit")) {
			/*
			 * for guardedExit, we want to let the navigational requests (browse, search,
			 * view) go through, but others (copyRecord, new, delete) we put up a session
			 * time out sign if possible and if not we put up general-purpose (missingLock)
			 * message. i.e., if exitCommand contains {"new", "copyRecord", ...) - then show
			 * message, otherwise, simply forward to location specified by pathArg
			 */
		...
		if (isTimedOut) {
			...
			errors.add("missingLock", new ActionError("session.timeout.msg", command));
		} else {
			prtln("not timed out: assume navigation error");
			errors.add("missingLock", new ActionError("editor.re.entry.msg", command));
		}
	...
	/**
	 * A hook for extensions of AbstractSchemEditAction to handle misc unanticipated
	 * requests.
	 *
	 * @param mapping
	 *            the actionMapping
	 ...
	 *                Description of the Exception
	 */
	protected ActionForward handleDeleteElement(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			SchemEditValidator validator) throws ServletException {
		...
		/*
		 * we are NOT validating the input fields before deletion. but we have to
		 * process the MultiValue fields to preserve any edits that have been made.
		 */
		validator.updateMultiValueFields();

		/*
		 * if patharg refers to the last of a required repeating element add an empty
		 * element give user a message to this effect collapse parent of deleted Element
		 * zoom to top else parentPath give user message zoom to element
		 */
		...
		if (validator.isLastRequiredRepeatingElement(pathArg)) {
			...
			errors.add("message", new ActionError("required.repeating.element.confirmation",
					XPathUtils.decodeXPath(pathArg), XPathUtils.getNodeName(pathArg)));
		} else {
			// normal case
			sef.getDocMap().removeElement(pathArg);
			errors.add("message", new ActionError("element.deleted.confirmation", XPathUtils.decodeXPath(pathArg)));
			/*
			 * set hash so user is taken to context of delete operation for multi-select
			 * objects, pathArg refers to the PARENT of the item deleted, and therefore hash
			 * can use this path. For indexed items, however, pathArg refers to the item
			 * deleted and hash must refer to the PARENT of the deleted item
			 */
	 ...
	 * @exception ServletException
	 *                Description of the Exception
	 * @exception MissingLockException
	 *                NOT YET DOCUMENTED
	 */
	protected ActionForward handleOtherCommands(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws ServletException, MissingLockException {

		String command = request.getParameter("command");
		ActionErrors errors = new ActionErrors();

		prtln("unknown command");
		errors.add("message", new ActionError("unrecognized.command", command));
		saveErrors(request, errors);
	...
	/**
	 * Handle the case where the request contains a command that is not processed by
	 * this Class by calling the "handleMissingCommand" method of a subclass.
	 *
	 * @param mapping
	 *            the ActionMapping
	 * @param form
	 ...
	 * @return Description of the Return Value
	 * @exception ServletException
	 *                If the subclass does not handle the command
	 */
	protected abstract ActionForward handleMissingCommand(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) throws ServletException;
	 ...
	 *                Description of the Exception
	 */
	protected abstract ActionForward handleEditRequest(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) throws ServletException;
	 ...
	 *                NOT YET DOCUMENTED
	 */
	protected abstract ActionForward handleCancelRequest(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) throws ServletException;
	...
	protected abstract ActionForward handleNewRecordRequest(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) throws ServletException;
	 ...
	 *                Description of the Exception
	 */
	protected abstract ActionForward handleSaveRequest(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response, SchemEditValidator validator)
	 ...
	 *
	 * @param pathArg
	 *            path of element to be attached
	 * @param sef
	 *            the form bean
	 * @param schemaHelper
	 *            the schemaHelper
	 * @exception Exception
	 *                if new element could not be created and attached
	 */
