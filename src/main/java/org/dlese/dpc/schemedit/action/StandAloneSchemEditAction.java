 ...
 * particular disk directory rather than reading and writing to a indexed
 * repository.
 *
 * @author ostwald
 */
public class StandAloneSchemEditAction extends AbstractSchemEditAction {
	 ...
	 */
	/*
	 * protected File getSampleFile() { return new
	 * File(getMetaDataFramework().getSampleRecordFile()); }
	 */
	...
	protected ActionForward handleOtherCommands(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		/*
		 * if (command.equalsIgnoreCase("standaloneDone")) { return handleDoneRequest
		 * (mapping, form, request, response, validator);
		 * 
		 * }
		 */
	...
	/*
	 * protected ActionForward handleDoneRequest (ActionMapping mapping, ActionForm
	 * form, HttpServletRequest request, HttpServletResponse response,
	 * SchemEditValidator validator) throws ServletException, MissingLockException {
	 * 
	 * prtln ("handleDoneRequest");
	 * 
	 * SchemEditForm sef = (SchemEditForm) form; MetaDataFramework metadataFramework
	 * = getMetaDataFramework();
	 * 
	 * ActionErrors errors = saveRecord (mapping, form, request, response,
	 * validator);
	 * 
	 * if (errors.size() > 0) { saveErrors(request, errors); return
	 * getEditorMapping(mapping); }
	 * 
	 * // save is successful sessionBean.releaseLock(sef.getRecId());
	 * errors.add("message", new ActionError("save.confirmation"));
	 * saveErrors(request, errors); return mapping.findForward("error.page"); }
	 */

	/**
	 * Save a record to disk. With the exception of MissingLock errors and
	 * Exceptions thrown by putRecord, exceptions are caught and returned as
	 * ActionErrors.
	 */
	protected ActionErrors saveRecord(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response, SchemEditValidator validator) throws MissingLockException {
		...
		if (sef.getRecId() == null || sef.getRecId().trim().length() == 0) {
			errors.add("error", new ActionError("record.id.notspecified"));
			return errors;
		}
	 ...
	 *                Description of the Exception
	 */
	protected File getFileToEdit(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			SchemaHelper schemaHelper) throws Exception {
	...
	protected ActionForward handleEditRequest(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (src.equals("remote")) {
			...
			try {
				...
				errors.add("pageErrors", new ActionError("remote.record.notfound", recId));
				saveErrors(request, errors);
				return mapping.findForward("error.page");
			}
		}
		...
		// get the file (whether a sample record or from disk)
			...
			try {
				...
				if (!file.exists()) {
					...
					errors.add("pageErrors", new ActionError("file.notfound", file.toString()));
					saveErrors(request, errors);
					return mapping.findForward("error.page");
				} else {
					// prtln("file exists");
				...
				errors.add("error", new ActionError("file.get.error", msg));
				saveErrors(request, errors);
				sef.setRecId("");
				return mapping.findForward("error.page");
			}
	 ...
	 */
	protected void putRecord(ActionMapping mapping, ActionForm form, HttpServletRequest request) throws Exception {
	 ...
	 * @return Description of the Return Value
	 */
	protected ActionForward handleChangeIdRequest(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			SchemaHelper schemaHelper, SchemEditValidator validator) {
		/*
		 * we are NOT validating the input fields before changingID. but we have to
		 * process the mulitvalue fields to preserve any edits that have been made.
		 */
		SchemEditForm sef = (SchemEditForm) form;
		validator.updateMultiValueFields();
		...
		ActionErrors errors = new ActionErrors();
		errors.add("message", new ActionError("record.name.changed.confirmation", pathArg));
		saveErrors(request, errors);
