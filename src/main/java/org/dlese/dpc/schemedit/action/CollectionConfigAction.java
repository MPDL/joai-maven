...
public final class CollectionConfigAction extends StandAloneSchemEditAction {
	 ...
	 *                if this config is currently being edited by someone else
	 */
	protected ActionForward handleSaveRequest(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		// BEFORE SAVING RECORD, we compare new and cashed values of the mdp: just in
		// case this collection is
		// registered with the NDR and the mdp has changed, then we need to adjust item
		// records - see below
		...
		try {
			/*
			 * no need to save record again, but we do need to alert the registry to rebuild
			 * the data structures that compile info from the individual collections, such
			 * as workflow statuses, idPrefixes, ...
			 */
			config.refresh();
			...
			errors.add("message", new ActionError("generic.message", "Collection info is updated"));
		} catch (Exception e) {
		...
		if (config.isNDRCollection()) {
			...
			if (ndrServiceIsActive) {
				try {
					...
					errors.add("message", new ActionError("generic.message", "Collection Info synced to NDR"));
				...
				errors.add("message", new ActionError("generic.error", errMsg));
			}
		}
	...
	/**
	 * Returns true if the value of the field specified by xpath has changed (and is
	 * not empty), indicating that a follow-on action is required as a consequence
	 * of the change.
	 *
	 * @param sef
	 *            the Form
	 * @param xpath
	 *            xpath of field to test
