			...
			if (req.getParameter("command") != null) {
				...
				if (command.equals("unlockAllRecords")) {
					...
					errors.add("message", new ActionError("generic.message",
							"All Locked Records (" + lockCount + ") have been released"));
				}
				...
				if (command.equals("unlockRecords")) {
					...
					if (ids != null && ids.length > 0) {
						...
						errors.add("error",
								new ActionError("generic.message", "no records selected - no records unlocked"));
					}

				} else {
					// errors.add("error", new ActionError("generic.message", "unrecognized command
					// (\"" + command + "\")"));
				}
			}
