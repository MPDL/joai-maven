...
/**
 * ActionForm bean for handling requests to support MetaDataEditor. Most methods
...
public class SchemEditForm extends ActionForm {
	 ...
	 *
	 * @param key
	 *            The new currentStdDocKey value
	 */
	 ...
	 *
	 * @param helper
	 *            The new suggestionServiceHelper value
	 */
	 ...
	 *
	 * @param vocabInterface
	 *            The new vocabInterface value
	 */
	...
	public ArrayList getVocabList() {
		...
		/*
		 * // debugging - can we test for presense of node? VocabNode vNode =
		 * vocab.getVocabNode( getXmlFormat(), vocabAudience, "en-us", vocabField,
		 * null); if (vNode != null) prtln ("vNode found");
		 */
	 ...
	 *
	 * @param results
	 *            The new duplicateUrlRecs value
	 */
	...
	/**
	 * Make a node visible in the editor. First create the node if necessary and set
	 * its displayState to OPEN. Then, if the node has a compositor, expose the
	 * compositor members. Fintally, walk back up the node's path setting each
	 * ancestor's displayState to OPEN (accomplished by call to
	 * getCollapseBean().exposeElement()).
	 *
	 * @param xpath
	 *            xpath to node to be exposed
	 */
		...
		if (schemaHelper != null && schemaHelper.hasCompositor(xpath)) {
			try {
				...
				for (Iterator i = members.iterator(); i.hasNext();) {
					...
					if (memberNode != null) {
						if (this.schemaHelper.isRepeatingElement(memberNode) && XPathUtils.getIndex(memberPath) == 0) {
	 ...
	 *
	 * @param fieldInfoReader
	 *            The new fieldInfoReader value
	 */
	 ...
	 * specified path. FieldInfoReaders are stored in the fieldInfoMap attribute of
	 * the SchemEditForm object (key is xpath).
	 *
	 * @param encodedPath
	 *            a jsp-encoded xpath
	...
	public FieldInfoReader getFieldInfo(String encodedPath) {
		...
		if (fieldInfoMap == null) {
			...
			/*
			 * // print terms for debugging prtln ("\nFieldInfoReader for " + xpath +
			 * "\n\tterms"); for (Iterator i=reader.getTermList().iterator();i.hasNext();) {
			 * prtln ("\t\t" + (String)i.next()); }
			 */
			return reader;
		}
	...
	/**
	 * Return TRUE if this node has a value. Used by fullView to determine whether a
	 * value should be shown as "missing or not".
	 * <p>
	 *
	...
	public String getNodeHasValue(String key) {
		...
		/*
		 * // diagnostics prtln("nodeHasValue(" + key + ")"); if (value == null ||
		 * value.trim().length() > 0) { prtln(" ... value is NULL"); } else {
		...
		// value.trim().length() > 0
				value.length() > 0 && !value.equals(SchemEditForm.UNSPECIFIED));
	 ...
	 * Nodes that are defined as "string" and are extension element of a derivedType
	 * are not considered to be missing a value, even if they are empty (these nodes
	 * are not even rendered as input fields in the editor).
	 *
	 * @param key
	 *            encoded path
			...
			if (schemaNode != null) {
				...
				if (typeDef != null && typeDef.isComplexType()) {
					...
					if (complexTypeDef.isDerivedType()) {
						...
						String stringType = NamespaceRegistry.makeQualifiedName(this.schemaHelper.getSchemaNamespace(),
								"string");
	...
	public String getNodeExistsWithContent(String key) {
		...
		boolean nodeExistsWithContent = docMap.nodeExists(xpath) && !docMap.isEmpty(xpath);
		return nodeExistsWithContent ? TRUE : FALSE;
	}

	/**
	 * Return true if the node specified by key exists in the instance document and
	 * it has a required attribute in the instance document.
	 *
	 * @param key
	 *            a jsp-encoded xpath
	 ...
	 * <p>
	 *
	 * A node can be expanded if: - it is a required branch - the node exists and it
	 * is NOT empty (it should also be complex??) -
	 *
	 * @param key
	 *            a jsp-encoded xpath
	...
	public String getNodeIsExpandable(String key) {
		/*
		 * getBranchIsRequired test commented out 9/23/09 - rationale: it makes no sense
		 * to have an expand widget if branch is empty (but would a branch ever be empty
		 * if it is required???
		 */
		/*
		 * if (getBranchIsRequired(key).equals(TRUE)) { prtln
		 * (" ... returning TRUE (branch is required)"); return TRUE; }
		 */
		String xpath = XPathUtils.decodeXPath(key);

		// docMap.isEmpty returns true even if there are children elements - as long as
		// they are empty
	...
	public String getParentNodeExists(String key) {
		...
		/*
		 * boolean parentNodeExists = (parentXPath.trim().length() > 0 &&
		 * docMap.nodeExists(parentXPath));
		 */
		boolean parentNodeExists = (parentXPath.length() > 0 && docMap.nodeExists(parentXPath));
	 ...
	 * elements satisfying this path would be returned .
	 * /itemRecord/general/subjects/subject[1] would be the first member of this
	 * list.
	 *
	 * @param encodedPath
	 *            a jsp-encoded xpath
	 * @return a list of xpaths corresponding to the values of enumeration.
	 */
	 ...
	 * Document. This method can be called very many times for the same page, so we
	 * cache values in the multiValuesCache
	 *
	 * @param encodedPath
	 *            a jsp-encoded xpath
	...
	/**
	 * Select all substitutionGroup elements by building up complex selection xpath
	 * ORing together the xpaths for the substitutionGroup members
	 *
	 * @param encodedPath
	 *            a jsp-encoded xpath
	...
	/**
	 * Gets the legal values an enumeration (controlled vocab defined in schema) can
	 * assume.
	 *
	 * @param encodedPath
	...
	public List getLegalEnumerationValuesOf(String encodedPath) {
		 ...
		 * NOTE also, that it is possible that a complexType defines simpleContent that
		 * extends an enumerated type. So we use the the "validatingType" for the
		 * complexType, which might refer to a SimpleType def (the validatingTypeName is
		 * different from the typeName in the case of ComplexTypes that define either
		 * Simple- or ComplexContent)
		 */
		...
		if (!globalDef.isSimpleType()) {
			prtlnErr(
					"getLegalEnumerationValuesOf(): a SIMPLE_TYPE is required! (" + globalDef.getQualifiedName() + ")");
			return emptyList;
		}
	 ...
	 * @param encodedPath
	 *            a jsp-encoded xpath
	 * @return array of LabelValue beans for controlled vocab defined for the xpath
	 */
	public LabelValueBean[] getEnumerationOptions(String encodedPath) {
		...
		// List labels = schemaHelper.getEnumerationValues(typeName, true); // get leaf
		// values (aka labels)
		List labels = null;
		...
		/*
		 * if metadata field at xpath contains a value that is not contained in the
		 * controlled vocab, we add it to the select options so it shows up in the UI
		 */
		String xpath = XPathUtils.decodeXPath(encodedPath);
		...
		// prtln("getEnumerationOptions() returning " + options.length + "
		// LabelValueBeans");
		return options;
	}
	 ...
	 * <p>
	 *
	 * If the field being edited contains a value other than those specified in the
	 * schema, then add this value to the options returned.
	 *
	 * @param encodedPath
	...
	public LabelValueBean[] getComboSelectOptions(String encodedPath) {
		...
		/*
		 * if (currentValue == null || currentValue.trim().length() == 0) { return
		 * options;
		 */
	...
	/**
	 * Determines whether the parent of the element corresponding to encodedPath can
	 * accept a new sibling of encodedPath.
	 * <p>
	 *
	 * The indexing of the target element is important, since it is required by the
	 * compositor to determine whether a new sibling can be accepted.
	 *
	 * @param encodedPath
	 *            a jsp-encoded xpath
	...
	public String getAcceptsNewSibling(String encodedPath) {
		...
		// prtln ("\t parentXpath: " + parentXpath + " elementName: " + elementName +"
		...
		try {
			...
			if (compositor != null) {
				...
				if (instanceElement == null) {
					/*
					 * prtln ("WARNING:  getAcceptsNewChild() couldn't get instance node at " +
					 * xpath); sometimes there won't be an element in the instance document (the
					 * field in the editor is created before the element in the instanceDoc. in this
					 * case we want to return false so the control will be presented (it is
				...
				boolean accepts = compositor.acceptsNewMember(instanceElement, elementName, elementIndex);
	...
	public String getAcceptsNewSubstitionGroupMember(String encodedPath) {
		...
		boolean accepts = getSubstitutionGroupMemberCountOf(encodedPath) < schemaNode.getMaxOccurs();
	...
	public LabelValueBean[] getChoiceOptions(String encodedPath) {
		...
		for (int i = 0; i < choices.size(); i++) {
			...
			if (schemaHelper.isMultiChoiceElement(xpath) || schemaHelper.isRepeatingElement(choicePath)) {
	...
	/**
	 * Cache of repeating values that stores repeating elements which may have empty
	 * values after the form is submitted. The cache is cleared after validation of
	 * form.
	 *
	...
	/**
	 * Clear the cache of repeating values that stores the repeating elements which
	 * may have empty values after form is submitted. Called by
	 * {@link org.dlese.dpc.schemedit.input.SchemEditValidator#pruneRepeatingFields}
	 */
	 ...
	 * values after the form is submitted. Necessary to support the case in which a
	 * user unselects all previously selected values
	 *
	 * @param xpath
	 *            The new repeatingField value
	 */
	...
	public SchemEditActionErrors validate(org.apache.struts.action.ActionMapping mapping,
		...
		try {
			...
			/*
			 * framework will not be initialized the first time this is executed for each
			 * record, so we skip it in these cases.
			 */
	...
	private void rebuildAnyTypeFields(InputManager inputManager, SchemEditActionErrors errors) {
		for (Iterator i = inputManager.getAnyTypeFields().iterator(); i.hasNext();) {
			...
			Element valueElement = hasParseError ? DocumentHelper.createElement("error") : field.getValueElement();
	...
	/**
	 * Make a list of all empty AnyType fields, and then call
	 * InputManager.removeAnyTypeFields to actually remove them from the fieldList
	 * and also from the instanceDoc
	 *
	 * @param inputManager
	 *            NOT YET DOCUMENTED
	 */
