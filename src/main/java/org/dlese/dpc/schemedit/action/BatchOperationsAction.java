 ...
 * {@link org.dlese.dpc.schemedit.action.form.BatchOperationsForm}
 *
...
public final class BatchOperationsAction extends DCSAction {
	 ...
	 * {@link org.apache.struts.action.ActionForward} instance to forward to the JSP
	 * page that will handle the request.
	 *
	 * @param mapping
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		bof.setSets(repositoryService.getAuthorizedSets(sessionUser, this.requiredRole));
		...
		errors.add("message", new ActionError("generic.message", "No operation specified - no action taken"));
		saveErrors(request, errors);
		return mapping.findForward("error.page");
	}
	 ...
	 */
	protected ActionForward handleBatchMove(HttpServletRequest request, BatchOperationsForm bof,
		...
		if (command != null && command.equalsIgnoreCase("move")) {
			...
			if (containsMasterCollectionRecord(recordBatch)) {
				errors.add("error", new ActionError("generic.error",
						"This list contains the master collection record, which cannot be moved."));
				saveErrors(request, errors);
			...
			try {
				...
				RecordList failedRecordList = repositoryService.batchMoveRecords(recordBatch, destCollection);
			...
			bof.setRecordList(null);
			// the list of records to move comes from the current result list (results of
			// last search)
	 ...
	 * @return Description of the Return Value
	 */
	protected ActionForward handleBatchCopyMove(HttpServletRequest request, BatchOperationsForm bof,
			ActionMapping mapping) {
		...
		if (command != null && command.equalsIgnoreCase("copyMove")) {
			...
			try {
				...
				RecordList failedRecordList = repositoryService.batchCopyMoveRecords(recordBatch, destCollection);
				bof.setRecordList(null);
				bof.setFailedRecordList(failedRecordList);
				int failCount = failedRecordList.size();
				int moveCount = recordBatch.size() - failCount;
	...
	/**
	 * Returns the distinct collections to which the individual records belong. Used
	 * to test whether the results are all from the same collection.
	 *
	 * @param recordList
	 *            NOT YET DOCUMENTED
	 ...
	 *
	 * @param recordBatch
	 *            an array of ResultDoc instances
	...
	/**
	 * Control interaction with user to delete a set of records. The set of records
	 * to be deleted is defined by a search, the results of which are accessible via
	 * the SessionBean.
	 *
	 * @param request
	 *            Description of the Parameter
	 ...
	 * @return Description of the Return Value
	 */
	protected ActionForward handleBatchDelete(HttpServletRequest request, BatchOperationsForm bof,
			ActionMapping mapping) {
		...
		if (command != null && command.equalsIgnoreCase("delete")) {
			...
			if (containsMasterCollectionRecord(recordBatch)) {
				errors.add("error", new ActionError("generic.error",
						"This list contains the master collection record, which cannot be deleted."));
				saveErrors(request, errors);
			...
			try {
				...
				RecordList failedRecordList = repositoryService.batchDeleteRecords(recordBatch);
				bof.setRecordList(null);
				bof.setFailedRecordList(failedRecordList);
				int failCount = failedRecordList.size();
				int moveCount = recordBatch.size() - failCount;
				errors.add("message", new ActionError("generic.message", moveCount + " records deleted"));
				if (failCount > 0) {
	 ...
	 * @return Description of the Return Value
	 */
	private ActionForward handleRemoveRecords(HttpServletRequest request, BatchOperationsForm bof,
			ActionMapping mapping, String forward) {
		ActionErrors errors = new ActionErrors();
		SimpleLuceneIndex index = repositoryManager.getIndex();
		...
		if (!recsToRemove.isEmpty()) {
			...
			errors.add("message",
					new ActionError("generic.message", recsToRemove.size() + " items removed from list."));

		} else {
			errors.add("error",
					new ActionError("generic.error", "Record list unchanged - no items were selected to remove"));
		}
		saveErrors(request, errors);
	...
	/**
	 * Control interaction with user to accomplish a Batch Status update. The set of
	 ...
	 * @return Description of the Return Value
	 */
	protected ActionForward handleBatchStatus(HttpServletRequest request, BatchOperationsForm bof,
			ActionMapping mapping) {
		...
		if (command != null && command.equalsIgnoreCase("status")) {
			...
			StatusEntry statusEntry = new StatusEntry(bof.getStatus(), bof.getStatusNote(), userName, entryDate);
			...
			try {
				...
				RecordList failedRecordList = repositoryService.batchStatusUpdate(recordBatch, statusEntry);
				bof.setRecordList(null);
				bof.setFailedRecordList(failedRecordList);
				int failCount = failedRecordList.size();
				int moveCount = recordBatch.size() - failCount;
				...
				if (failCount > 0) {
					errors.add("error", new ActionError("generic.error",
							"status could not be update for " + failCount + " records"));
				}
			} catch (Exception e) {
	...
	/**
	 * Return a list of {@link StatusFlag} beans that describe the statuses that can
	 * be assigned to a {@link StatusEntry}. This list is composed of the
	 * UNKNOWN_status plus those statuses that defined for the collection. The
	 * IMPORT status is not available in this context, since it is only assigned by
	 * the system.
	 *
	 * @param collection
	 *            Description of the Parameter
	...
	/**
	 * Validate the input from user. Put changed or default values into statusForm.
	 * After this method returns, statusForm (rather than request) is used to
	 * process user input
	 *
	 * @param request
	 *            Description of the Parameter
	...
	private ActionErrors validateStatusForm(HttpServletRequest request, BatchOperationsForm bof) {
		...
		if (status == null) {
			...
			errors.add("status", new ActionError("generic.error",
					"Please assign a status other than \"" + StatusFlags.IMPORTED_STATUS + "\""));
		} else {
