...
public class AbstractConfigReader implements Serializable {
	...
	public DocMap getDocMap() throws Exception {
		...
		if (docMap == null) {
			if (source != null && source.exists()) {
				...
				throw new Exception("configReader error: source file not found at " + this.source.getAbsolutePath());
			}
		}
		return docMap;
	}
	 ...
	 * default config file and then assigning a new source so it will be written to
	 * a new config file
	 *
	 * @param file
	...
	/**
	 * Write Document to disk and then set docMap to null, forcing a re-read upon
	 * next access.
	 */
	public void refresh() {
		this.docMap = null;
