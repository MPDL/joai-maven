...
public class CollectionConfigReader extends AbstractConfigReader {
	...
	public void setMetadataProviderHandle(String handle) {
		...
		Node node = DocumentHelper.makeElement(this.getDocument(), metadataProviderHandlePath);
		node.setText(handle);
	}
	...
	/*
	 ** return a Map where the keys are status flags and the values are the
	 * descriptions
	 */
	/**
	...
	/**
	 * NOTE: if we edit the config record via schemedit, the record is automatically
	 * updated and then we have to reload it. But if we edit the statusMap via
	 * another editor, then this method is necessary to update the config file
	 *
	 * @param sMap
