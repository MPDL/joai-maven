...
/**
 * Holds information about a collection that is accessed via a
 * {@link org.dlese.dpc.schemedit.config.CollectionConfigReader}.
 * <p>
 *
 ...
 * are read at system startup time, but there need not be a config file for each
 * collection. Config files are created by copying a <b>default.xml</b> file and
 * modifying it as settings are edited.
 * <p>
 *
 * CollectionConfig instances are registered in a
 * {@link org.dlese.dpc.schemedit.config.CollectionRegistry} instance.
 *
 * @author ostwald
 *         <p>
 *
...
public class CollectionConfig {
	...
	/**
	 * Convenience method to determine whether this collection is managed in the
	 * NDR.
	 *
	...
	/**
	 * Sets the authority attribute for this collection, which is used to determine
	 * whether the collection is managed in the NDR.
	 *
	 * @param authority
	 *            The new authority value
	 */
	 ...
	 *
	 * @param aggregatorHandle
	 *            The new aggregatorHandle value
	 */
	...
	/**
	 * Gets the ndrOaiLink for this collection, which is the "itemId" of Collection
	 * Metadata records stored in the NDR, and which is inserted in the InfoStream
	 * of item-level metadata records when they are written to the NDR.
	 *
	...
	/**
	 * Return the status flag label (the readable version) for the given status flag
	 * value (which must be decoded if it is a final status flag).
	 *
	 * @param statusFlagValue
	 *            NOT YET DOCUMENTED
	 * @return The statusFlagLabel value
	 */
	public String getStatusFlagLabel(String statusFlagValue) {

		return StatusFlags.isFinalStatusValue(statusFlagValue) ? this.getFinalStatusLabel() : statusFlagValue;
	}
	 ...
	 * be assigned to a record of this collection. The selectableStatusFlags list is
	 * composed of the UNKNOWN_status plus those statuses that are defined for the
	 * collection. The IMPORT status is not selectable, since it is only assigned by
	 * the system.
	 *
	...
	/**
	 * Returns a list of StatusFlags defined by this collection that can be selected
	 * for searching. The selectable flags includes the assignable flags plus the
	 * reserved IMPORTED_STATUS_FLAG.
	 *
	...
	/**
	 * Sets the finalStatusLabel (the display label) for this collection's final
	 * Status.
	 *
	 * @param s
	 *            The new finalStatusLabel value
	 */
	...
	/**
	 * Writes config file to disk and re-initializes data structures based on
	 * config.
	 *
