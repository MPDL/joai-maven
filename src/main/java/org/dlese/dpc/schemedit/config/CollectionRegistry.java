...
public class CollectionRegistry {
	...
	public CollectionRegistry(File configDir, String idFilesPath, String defaultCollConfigPath) {
		...
		prtln("\n\n Initializing Collection Registry ... \n\tabout to read " + configFiles.length + " files from "
	...
	public CollectionConfig getCollectionConfig(String collection, boolean createIfNotFound) {
		...
		if (config == null) {
			...
			if (configFile.exists()) {
				...
				if (config.getFinalStatusLabel() == null || config.getFinalStatusLabel().trim().length() == 0) {
	...
	public boolean isDuplicateIdPrefix(String collectionKey, String idPrefix) {
		...
		for (Iterator i = getIdPrefixMap().entrySet().iterator(); i.hasNext();) {
			...
			if (aPrefix != null && aPrefix.trim().toUpperCase().equals(idPrefix.trim().toUpperCase()) && aKey != null
					&& !aKey.equals(collectionKey)) {
				return true;
			}
		}
		return false;
	...
	/**
	 * Convenience method to obtain a new record ID from specified collection (which
	 * must exist)
	 */
	...
	/*
	 * Convenience method to obtain IdPrefix from specified collection (which must
	 * exist)
	 */
	...
	/**
	 * Obtain IDGenerator instance for specified collection (which must be
	 * registered).
	 */
	...
	/**
	 * Initialize the IDGenerator for each collection in the index so that the next
	 * ID to be assigned will increment the last assigned ID for that collection.
	 */
	public void initializeIDGenerators(SimpleLuceneIndex index) {
		prtln("\ninitializing idGenerators");

		// is there any reason to re-create the idPrefixMap now that it is under
		// CollectionRegistry control??
	...
	public String toString() {
		...
		for (Iterator i = getIds().iterator(); i.hasNext();) {
			...
			s += "\n\t" + key;
			/*
			 * s += "\n **" + key + " **"; CollectionConfig config = (CollectionConfig)
			 * collections.get(key); s += config.toString();
			 */
		}
		return s;
	}
