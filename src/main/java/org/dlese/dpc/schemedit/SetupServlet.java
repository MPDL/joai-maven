...
public final class SetupServlet extends HttpServlet {
	...
	public void init(ServletConfig config) throws ServletException {
		...
		// make sure the directory where idFiles (saving current id for collection
		// IDGenerators) exists
	...
	/**
	 * Activate the standards suggestion service as configured.
	 * <p>
	 *
	 ...
	 * configure multiple frameworks, or frameworks other than ncs_item. If this is
	 * not present, then look for the file or directory pointed to by the
	...
	void standardsSuggestionServiceSetup() throws ServletException {
		...
		String configParam = (String) getServletContext().getInitParameter("standardsSuggestionServiceConfig");
		if (configParam != null && configParam.trim().length() > 0) {
			...
			configParam = (String) getServletContext().getInitParameter("asnStandardsDocument");
			if (configParam != null && configParam.trim().length() > 0) {
	 ...
	 * @param asnFile
	 *            asnDocument file
	 * @return a standards service configuration file pointing to asnFile
	 * @exception Exception
	 *                if unable to create config file
	 */
	 ...
	 *
	 * @param loginConfig
	 *            login config file to be altered
	 * @exception Exception
	 *                Description of the Exception
	 */
	private void loginConfInit(File loginConfig) throws Exception {
		...
		// content = content.replaceFirst("@MY_DIR@",
		// loginConfig.getParentFile().getCanonicalPath());
	 ...
	 * deploy directory if necessary, finally setting "frameworkConfigDir" attribute
	 * in servletContext.
	 * <p>
	 *
	...
	private void populateConfigDir(File src, File dest) throws Exception {
		...
		for (int i = 0; i < files.length; i++) {
			...
			if (newConfigFile.exists() || configFile.isDirectory()) {
				// prtln ("config file already exists - not overwriting " +
	...
	/**
	 * Initialize logo from init params, and copy logo file to auth directory, where
	 * it can be accessed from logon/logoff pages.
	 */
