...
public class CollapseBeanInitializer {
	 ...
	 *
	 * @param collapseBean
	 *            collapseBean to be initialized
	 * @param doc
	 *            instance document
	 * @param framework
	 *            framework of instance document
	 */
	 ...
	 *            instance document
	 * @param baseLevel
	 *            Configured baseLevel for the framework - used to compute relative
	 *            "level"
	 * @param framework
	 *            Description of the Parameter
	 */
	public CollapseBeanInitializer(CollapseBean collapseBean, Document doc, int baseLevel,
	...
	/**
	 * Returns the level of a path relative to the baseLevel, which is specified by
	 * the metadataFramework.
	 *
	 * @param xpath
	 *            NOT YET DOCUMENTED
	 * @param baseLevel
	...
	/**
	 * Initializes visiblity of elements in the metadata editor. Currently, expands
	 * all nodes except those at a level below THRESHOLD_LEVEL.
	 */
	public void init() {
		collapseBean.clear();
	 ...
	 *
	 * @param encodedXPath
	 *            encoded xpath specifying element to be opened
	 */
	 ...
	 *
	 * @param encodedXPath
	 *            encoded xpath specifying element to be closed
	 */
	...
	/**
	 * Set the displayState of the collapseBean node specified by "path"
	 * <p>
	 *
	 ...
	 * framework-config using the "initialFieldCollapse" parameter.
	 * <p>
	 *
	 * As of 7/2/209 we are initializing top-level multibox displays as "opened".
	 *
	 * @param path
	 *            xpath specifying a node of the collapseBean
	 */
	private void initializeNode(String path) {
		...
		else if (level == 1 && schemaNode != null && (sh.isMultiSelect(schemaNode) || sh.isChoiceElement(schemaNode)
				|| sh.isMultiChoiceElement(schemaNode))) {
			openElement(encodedXPath);
	 ...
	 *
	 * @param path
	 *            specifies node of collapseBean
	 */
	...
	/**
	 * Initialize children of the element specified by path - note: we must manually
	 * add indexing for repeating elements
	 *
	 * @param path
	 *            xpath to element whose children are to be processed
	 * @param element
	 *            instance document element at path
	 */
	private void initializeChildren(String path, Element element) {
		...
		/*
		 * otherChildrenNames used to collect the member names that are NOT present in
		...
		// set displayState of otherChildren to open so their "add" controls will be
		// available
