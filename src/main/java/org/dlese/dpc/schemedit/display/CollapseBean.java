...
public class CollapseBean implements Serializable {
	 ...
	 *
	 * @param key
	 *            String designating an element in the instance document
	 */
	 ...
	 * CollapseBean need not pass a parameter to specify the element.
	 * <p>
	 *
	 * For example:
	 * <ul>
	 * <li>&lt;jsp:setProperty name="collapseBean" property="id" value="${id}"/&gt;
	 ...
	 *
	 * @param state
	 *            The new defaultState value
	 */
	public void setDefaultState(String state) {
		if (state.equalsIgnoreCase(OPEN) || state.equalsIgnoreCase(CLOSED)) {
			defaultState = state;
		} else {
	 ...
	 * key is an xpath encoded to be acceptable as a javascript var (see
	 * {@link org.dlese.dpc.schemedit.SchemEditUtils#pathToId(String)})
	 *
	 * @param key
	 *            An encoded xpath
	 * @param state
	 *            The new displayState value
	 */
	public void setDisplayState(String key, String state) {
		// prtln ("setting displayState for " + CollapseUtils.ppKey(key) + " to: " +
		// state);
	 ...
	 *
	 * @param key
	 *            an identifier for the element to open
	 */
	...
	/**
	 * Find the request parameters that specify displayStates for the content boxes
	 * of the metadataEditor's instance document, and update the collapseBean so it
	 * reflects the displayStates specified by the request.
	 *
	 * @param request
	 *            Description of the Parameter
	 */
