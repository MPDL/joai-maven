...
/**
 * Reads spreadsheet data (xml file created from spreadsheet) with data supplied
 * by NSDL but augmented from NCS Collect records, with the purpose of
...
public class Collection {
	...
	public static void tester() {
		try {
			...
			/*
			 * NCSCollectReader modified = collection.integrateServiceDescription(); pp
			 * (modified.getWritableDocument()); List contacts = modified.getContacts();
			 * prtln ("there are " + contacts.size() + " contacts"); for (Iterator
			 * i=contacts.iterator();i.hasNext();) { Contact c = (Contact)i.next(); pp
			 * (c.asElement()); }
			 */
		} catch (Throwable t) {
	...
	/**
	 * Supply values for these params to communicate with specific ndr server as
	 * specified agant
	 */
	...
	/**
	 * we want to put an arbitray info stream into the Metadata Object for this
	 * collection
	 */
