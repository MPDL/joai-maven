...
public class SyncService extends MonitoredTask {
	...
	public SyncService(SessionBean sessionBean, ServletContext servletContext) throws Exception {
		...
		try {
			...
			frameworkRegistry = (FrameworkRegistry) getServletContextAttribute("frameworkRegistry");
	 ...
	 * <p>
	 *
	 * NOTE: if a record is modified when the NDR is not enabled, it's fileMod will
	 * be greater than the lastSync date, and therefore should be synced.
	 *
	 * @param dcsDataRecord
	 *            the dcsDataRecord
	 * @param docReader
	 *            NOT YET DOCUMENTED
	 * @return true if the metadata record is out of sync with the ndr
	 */
	private boolean recordOutOfSync(DcsDataRecord dcsDataRecord, XMLDocReader docReader) {
		 ...
		 * prtln("\t fileMod: " + fileMod + " (" + SchemEditUtils.fullDateString(new
		 * Date(fileMod)) + ")"); prtln("\t fileMod - lastSyncDate: " + (fileMod -
		 * lastSyncDate.getTime()));
		 */
		// HERE we decide whether this record has changed or not ...
		return ((fileMod - lastSyncDate.getTime() > 2000) || changeDate.after(lastSyncDate)
				|| lastTouchDate.after(lastSyncDate));
	}
	...
	public void sync(String collectionKey) throws Exception {
		...
		try {
			...
			if (itemFramework == null) {
				throw new Exception("collection cannot be synced because \"" + collectionConfig.getXmlFormat()
						+ "\" is not loaded");
			}
	 ...
	 *
	 * @param setInfo
	 *            represents the collection to be synced
	 * @param report
	...
	/**
	 * Sync the item-level records with the NDR.
	 * <p>
	 *
	...
	private void syncItems(SyncReport syncReport, boolean syncAll) throws Exception {
		...
		RepositoryService repositoryService = (RepositoryService) this.servletContext.getAttribute("repositoryService");
		...
		for (Iterator i = idList.iterator(); i.hasNext() && !stopProcessing;) {
			...
			try {
				/*
				 * NOTE: we have to account for the case in which the record's file has been
				 * modified/touched without being indexed if the record is out of sync with the
				 * index, we must first update the index, THEN resync
				 */
				...
				try {
					if (repositoryService.indexedRecordIsStale(id)) {
						...
						repositoryManager.putRecord(xmlRecord, docReader.getNativeFormat(), docReader.getCollection(),
								id, true);
					}
				} catch (Exception e) {
		...
		if (deleteUnmatchedNDRRecords && !stopProcessing) {
			...
			for (Iterator i = ndrHandles.iterator(); i.hasNext();) {
				...
				if (!indexedHandles.contains(handle)) {
					...
					try {
						syncReport
								.addEntry(new SyncReportEntry(handle, "delete", "", NdrUtils.deleteNDRObject(handle)));
					} catch (Exception e) {
	 ...
	 *
	 * @param statuses
	 *            Statuses that will be searched over
	 * @return A list of record Ids
	 */
	 ...
	 * @exception Exception
	 *                If collectionConfig or setInfo cannot be found for
	 *                collectionKey
	 */
		...
		public void run() {
			try {
				...
				if (this.report.getEntryList().size() > 0) {
					prtln("NOW INDEXING ...");
					repositoryManager.indexCollection(collectionKey,
