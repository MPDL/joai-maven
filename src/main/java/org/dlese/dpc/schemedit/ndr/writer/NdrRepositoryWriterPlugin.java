 ...
 * <p>
 *
 * NOTE: currently, this class implements a repositoryWriter plugin for the NDR.
 * When we understand what the plugin INTERFACE should be, then the interface
 * will be in this package, and the NDR implementation will be elsewhere...
 *
 * @author ostwald
 *         <p>
 *
...
public class NdrRepositoryWriterPlugin extends ServletContextRepositoryWriterPlugin {
	...
	/**
	 * Gets the repositoryService attribute of the NdrRepositoryWriterPlugin object
	 *
	 ...
	 * format), we check to ensure the record is "FinalStatus" and valid, and if so
	 * we update the entire collection definition in the NDR.
	 * <p>
	 *
	 ...
	 *            metadata record Id
	 * @param dcsDataRecord
	 *            dcsData for the record to be written
	 * @param recordXml
	 *            metadata as an xml String
	 * @param xmlFormat
	 *            format of metadata record
	 * @exception NdrRepositoryWriterPluginException
	 *                NOT YET DOCUMENTED
	 */
	public void putRecord(String recId, String recordXml, String xmlFormat, DcsDataRecord dcsDataRecord)
		...
		if (!(this.isNdrServiceEnabled() && this.isNdrEnabledCollection(recId)))
			return;
		...
		/*
		 * Do not write to NDR if status is "Recommended" - this is a reserved status
		 * for records that enter the system via a Recommend Service. This keeps spam
		 * from being writen to NDR.
		 */
	...
	public void putCollectionData(String id, CollectionConfig collectionConfig, DcsDataRecord dcsDataRecord)
		...
		if (!(this.isNdrServiceEnabled() && this.isNdrEnabledCollection(id)))
			return;
	...
	/**
	 * Gets the ndrServiceEnabled attribute of the NdrRepositoryWriterPlugin object
	 *
	...
	private boolean isNdrEnabledCollection(String recId) {
		...
		try {
			...
			CollectionRegistry reg = (CollectionRegistry) getServletContext().getAttribute("collectionRegistry");
