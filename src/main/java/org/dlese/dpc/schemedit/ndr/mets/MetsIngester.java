 ...
 * <p>
 *
 * For now, assume the collection (mdp and agg) objects already exist in the NDR
 * and are provided to this class.
 *
 * @author ostwald
 */
public abstract class MetsIngester extends NameSpaceXMLDocReader {
	...
	void ndrToolkitInit() throws Exception {
		// MimeTypes.setPath ("C:/Program Files/Apache Software Foundation/Tomcat
		// 5.5/conf/web.xml");
	...
	void ingestItem(String dmdID, String fileID) throws Exception {
		...
		if (useExistingResource) {
			...
			String resHandle = ndrToolkit.setResourceContent(resource, fileData.binData, dmdSec.filename,
					fileData.mimeType);
		...
		Datastream.Name itemDatastream = new Datastream.Name("format_" + this.getXmlFormat());
	...
	/**
	 * Returns RELATIVE xpath to URL starting from root (E.g., if the absolute path
	 * to url is "/record/URL", this method should return "URL");
	 */
	...
	public static void main(String[] args) throws Exception {
		...
		if (env.equals("sanluis")) {
			...
			// propsPath = "C:/Documents and
			// Settings/ostwald/devel/ndrServerProps/dls.ndr.properties";
