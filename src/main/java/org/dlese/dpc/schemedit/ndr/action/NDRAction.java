...
public final class NDRAction extends DCSAction {
		...
		try {
			...
			if (param.equals("manage")) {
				if (request.getParameter("activate") != null) {
					...
					errors.add("message", new ActionError("generic.message", "NDR Service has been activated"));
				} else if (request.getParameter("deactivate") != null) {
					servlet.getServletContext().setAttribute("ndrServiceActive", false);
					errors.add("message", new ActionError("generic.message", "NDR Service has been deactivated"));
			...
			if (param.equals("browse")) {
				...
				if (objType != null) {
					...
					/*
					 * if (objType.equals("agent")) { ndrForm.setBrowserHandles
					 * (NdrUtils.getAgentHandles()); }
					 */
				}
			...
			if (param.equals("validate")) {
				try {
					...
					String xml = TimedURLConnection.importURL(url.toString(), NDRConstants.NDR_CONNECTION_TIMEOUT);
			...
			if (param.equals("inactiveMembers")) {
				...
				if (collection != null) {
					try {
						...
						findRequest.addQualifiedCommand(NDRConstants.FEDORA_MODEL_NAMESPACE, "property", "state",
			...
			e.printStackTrace();
			errors.add("error", new ActionError("generic.error", "NDRAction caught exception"));
		} catch (Throwable e) {
	 ...
	 * <ul>
	 * <li>ndrCollections - a list of SetInfos for all authorized collections</li>
	...
	private void assignAuthorizedSetGroups(HttpServletRequest request, ActionMapping mapping, ActionForm form) {
		...
		List authorizedSets = repositoryService.getAuthorizedSets(getSessionUser(request), this.requiredRole);
	 ...
	 */
	private ActionForward handleImportNDRCollection(String mdpHandle, ActionMapping mapping, ActionForm form,
		...
		try {
			...
			if (repositoryManager.indexCollection(collectionId, observer, true)) {
				// if (rm.indexCollection(collectionId, new
				// SimpleFileIndexingObserver("Collection indexer for '" + collectionName + "'",
				// "Starting indexing"), true)) {
				errors.add("message",
						new ActionError("generic.message", "Collection '" + collectionName + "' imported from NDR."));
	...
	private ActionForward handleGeneralProxy(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	 ...
	 *                NOT YET DOCUMENTED
	 */
	private ActionForward handleNdrProxy(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		NDRForm ndrForm = (NDRForm) form;
	 ...
	 * @return NOT YET DOCUMENTED
	 */
	private ActionForward handleWriteCollectionInfo(String collection, ActionMapping mapping, ActionForm form,
			HttpServletRequest request) {
		ActionErrors errors = new ActionErrors();
		...
		try {
			// String username = (sessionUser == null ? Constants.UNKNOWN_USER :
			...
			errors.add("message", new ActionError("ndr.export.success", setInfo.getName(), collection));
			saveErrors(request, errors);
			return mapping.findForward("ndr.sync.report");
		} catch (Exception e) {
	 ...
	 * @return NOT YET DOCUMENTED
	 */
	private ActionForward handleUnregisterCollection(String collection, ActionMapping mapping, ActionForm form,
			HttpServletRequest request) {
		ActionErrors errors = new ActionErrors();
		...
		try {
			...
			DcsDataRecord collDcsDataRecord = this.dcsDataManager.getDcsDataRecord(collectionRecordId,
					repositoryManager);
			...
			if (repositoryManager.indexCollection(collection, observer, true)) {
				// if (rm.indexCollection(collection, new SimpleFileIndexingObserver("Collection
				// indexer for '" + collectionName + "'", "Starting indexing"), true)) {
				errors.add("message",
						new ActionError("generic.message", "Collection '" + collectionName + "' removed from NDR."));
				errors.add("message", new ActionError("generic.message",
						"Files for collection '" + collectionName + "' are being re-indexed."));
				errors.add("message",
						new ActionError("generic.message", "Changes may take several several minutes to appear"));
				errors.add("showIndexMessagingLink", new ActionError("generic.message", ""));
			} else {
			...
			e.printStackTrace();
			errors.add("error", new ActionError("ndr.unregister.error", collectionName, collection, e.getMessage()));
			saveErrors(request, errors);
			return mapping.findForward("ndr.manage");
		}
	}
	 ...
	 * @return NOT YET DOCUMENTED
	 */
	private ActionForward handleSync(String collection, ActionMapping mapping, ActionForm form,
			HttpServletRequest request) {
		ActionErrors errors = new ActionErrors();
		...
		try {
			...
			e.printStackTrace();
			errors.add("error",
					// new ActionError("ndr.sync.error", setInfo.getName(), collection,
					// e.getMessage()));
