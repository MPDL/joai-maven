...
public class NCSWebServiceClient extends WebServiceClient {
	...
	public NCSCollectReader getNCSRecord(URL resourceUrl) {
		...
		Element metadata = (Element) response.selectSingleNode(metadataPath);
		if (metadata == null) {
	...
	public NCSCollectReader getNCSRecord(String recId) throws Exception {
		...
		Element metadata = (Element) doc.selectSingleNode(metadataPath);
	...
	public List getNCSCollectionIDs() throws Exception {
		...
		List idNodes = response.selectNodes("/DDSWebService/ListCollections/collections/collection/searchKey");
		for (Iterator i = idNodes.iterator(); i.hasNext();) {
	...
	void tester1() {
		// String resourceUrl = "http://dlcenter.larc.nasa.gov/"; // works with no
		// collection
		// String resourceUrl = "http://www.alexandria.ucsb.edu/"; // works WITH
		// collection
