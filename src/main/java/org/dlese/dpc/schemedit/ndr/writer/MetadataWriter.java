...
public class MetadataWriter {
	...
	protected void init(String recordXml) throws Exception {
		...
		try {
			xmlConversionService = (XMLConversionService) rm.getIndex().getAttribute("xmlConversionService");
		} catch (Throwable t) {
		}
	 ...
	 *
	 * @param recId
	 *            recordId, used to obtain metadata from index if necessary
	 * @param recordXml
	 *            metadata as String (optionally provided)
	 * @param dcsDataRecord
	...
	/**
	 * Returns NdrRequest to write metadata to NDR, either as a new metadata object
	 * or by updating an existing object.
	 *
	 * @param resHandle
	 ...
	 * @exception Exception
	 *                if unable to build ndr request
	 */
	protected NdrRequest getWriteRequest(String resourceUrl, String resHandle) throws Exception {
		...
		/*
		 * based on whether there is a NdrHandle stored in the DcsDataRecord, determine
		 * whether we are - creating an NEW metadata Object OR - modifying an existing
		 * one
		 */
		...
		// add new or modify existing metadata object by completing the request and
		// submitting
		if (metadataExists) {
	...
	/**
	 * Augment provided NdrRequest to form an "addMetadataRequest" request to create
	 * a new NDR object.
	 *
	 * @param resHandle
	 *            resource object handle
	...
	protected NdrRequest modifyMetadataRequest(NdrRequest ndrRequest, String resHandle, Element nsdl_dc_stream)
		...
		/*
		 * if (nsdl_dc_stream == null) { ndrRequest.addDataStreamCmd("nsdl_dc", null,
		 * "delete"); }
		 */
	...
	/**
	 * Gets the nsdlAboutCategory attribute of the MetadataWriter object.
	 * <p>
	 *
	 * "item" except when framework is ncs_collect, and then it is "collect"
	 * nsdlAboutCategory is "item" except when framework is ncs_collect, and then it
	 * is "collect"
	 *
	 ...
	 *
	 * @param ndrRequest
	 *            The new metadataForRelationships value
	 * @param resourceHandle
	 *            The new metadataForRelationships value
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 ...
	 * resource is created if one does not already exist in the NDR. In either case,
	 * a memberOf relationship is created between the resource and the collection's
	 * aggregator.
	 *
	 * @param resourceUrl
	 ...
	 * @exception Exception
	 *                if a collection aggregator is not found, or if the resource
	...
	protected String getResourceHandle(String resourceUrl) throws Exception {
		...
		/*
		 * All Collections managed by the NCS will have an aggregator, which must be
		 * related to the resource by the "memberOf" relationship.
		 */
	...
	/**
	 * Gets the metadata record as a dom4j.Document, converting from recordXml if it
	 * is provided, or obtaining from the index (via docReader) otherwise.
	 *
	 * @param recordXml
	 *            metadata record as XML String
	 * @return The itemRecord value
	 * @exception Exception
	 *                if provided recordXml is not well-formed.
	 */
	protected Document getItemRecord(String recordXml) throws Exception {

		return (recordXml != null ? DocumentHelper.parseText(recordXml) : docReader.getXmlDoc());
	}
	...
	private Element getNSDL_DC(Document itemRecord) {
		...
		if (xmlConversionService == null || !xmlConversionService.canConvert(fromFormat, toFormat)) {
			prtln("conversionService could not convert from \"" + fromFormat + "\" to \"" + toFormat
					+ "\" - likely no transform is registered");
			return null;
		} else {
			try {
	...
	/**
	 * Gets data stream for metadata in native xml format, which is the root element
	 * of the provided Document. If the native xml format is "oai_dc" we perform a
	 * (LKLUDGE) tranformation of the namespaces to convert from dlese_oai to
	 * oai_dc.
	 *
	 * @param itemRecord
	 *            NOT YET DOCUMENTED
	 ...
	 * @exception Exception
	 *                if resourceUrl cannot be obtained
	 */
	...
	protected CollectionConfig getCollectionConfig(String collection) throws Exception {
		CollectionRegistry reg = (CollectionRegistry) getServletContextAttribute("collectionRegistry");
	...
	protected MetaDataFramework getMetaDataFramework(String xmlFormat) throws Exception {
		FrameworkRegistry reg = (FrameworkRegistry) getServletContextAttribute("frameworkRegistry");
