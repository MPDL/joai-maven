...
public class NSDLCollectionWriter extends MetadataWriter {
	 ...
	 * collection in the NDR, including collection-level metadata object, collection
	 * resource, and collection aggregator.
	 * <p>
	 *
	 * NOTE: if the NCSL collection already exists, then there is a possibility that
	 * the collection's resource URL has been modified. For this reason, we have to
	 * locate the collection definition objects in the NDR by other means than the
	 * resource URL ...
	 *
	 * @param recId
	 *            record id, used to obtain collection metadata from index if
	 *            necessary
	 * @param recordXml
	 *            collection metadata as String (optionally provided)
	 * @param dcsDataRecord
	 *            auxillary information about the collection metadata record
	 * @return NOT YET DOCUMENTED
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	public SyncReportEntry write(String recId, String recordXml, DcsDataRecord dcsDataRecord) throws Exception {
		...
		/*
		 * as of 2/13/2008 we don't act upon the application agent String
		 * collectionAppAgent = this.getCollectionAppAgent(ncsCollectReader);
		 ...
		 * collectionAggregator and collectionMP objects. otherwise we have to create
		 * them (and this requires a collectionResource)
		 */
		if (getValueOrNull(collectionMetadata) != null) {
			...
			/*
			 * the first time through, there won't be a mdp yet, but otherwise we can obtain
			 * a mdp reader
			 */
		...
		/*
		 * we have to update aggregator and metadataprovider because the
		 * serviceDescription stream of these objects is created from information in the
		 * ncs_collect record. if the record is not valid and final, then we need to set
		 * the visibility of the MDP to "private" (hiding it from OAI and Search
		 * service). if the record IS valid and final, then we need to ensure that the
		 * visibility is set to the value of the OAIProvider field in the ncs_collect
		 * record.
		 */
	...
	/**
	 * Validate the Collection Agent (if any) specificied in the
	 * NCSCollectionReader.
	 * <p>
	 *
	 ...
	 * @exception Exception
	 *                if an invalid agent handle is specified in the
	 *                NCSCollectionReader
	 */
	...
	private String getValidIngestAppAgent() throws Exception {
		...
		if (ingestAppAgent == null || ingestAppAgent.trim().length() == 0)
			...
			if (reader.getObjectType() != NDRConstants.NDRObjectType.AGENT)
				throw new Exception(
						"Specified Collection Agent (" + ingestAppAgent + ") either does not exist or is not an Agent");
		}
	 ...
	 *
	 * @param ndrRequest
	 *            The feature to be added to the MetadataRequest attribute
	 * @param resHandle
	 *            The feature to be added to the MetadataRequest attribute
	...
	protected NdrRequest addMetadataRequest(NdrRequest ndrRequest, String resHandle) throws Exception {
		ndrRequest = super.addMetadataRequest(ndrRequest, resHandle);
	...
	protected NdrRequest modifyMetadataRequest(NdrRequest ndrRequest, String resHandle, Element nsdl_dc_stream)
			throws Exception {
		NdrRequest modifyRequest = super.modifyMetadataRequest(ndrRequest, resHandle, nsdl_dc_stream);
	...
	/**
	 * Get the Collection Aggregator Object from the NDR, creating one if necessary.
	 * <p>
	 *
	 * Even if the aggregator already exists, we want to update it in case the
	 * collection metadata has changed.
	 * <p>
	 *
	 * Aggregator components:
	 * <ul>
	 * <li>datastream - Aggregator Service Description - created from ncs_collect
	 * record</li>
	 * <li>relationships:
	 * <ul>
	 ...
	 *                NOT YET DOCUMENTED
	 */
	private String updateAggregator() throws Exception {

		/*
		 * The aggregator object requires: - relationships - "memberOf"
		 * 2200/NSDL_Collection_of_Collections_Aggregator - "associatedWith"
		 * collectionResource - datastreams - "serviceDescription"
		 */
		...
		/*
		 * For records that will get items through OAI harvests: Both MetadataProvider
		 * and Aggregator objects need to have authorized-to-change properties allowing
		 * the Harvest/Ingest process to update them.
		 */
		if (collectionAggregator == null) {
			...
			String aggregatorFor = (this.collectionAgent != null ? this.collectionAgent : NDRConstants.getNcsAgent());
			...
			if (NDRConstants.getMasterCollection() != null)
				request.addCommand("relationship", "memberOf", NDRConstants.getMasterCollection());
			...
			// aggregatorFor - only modify this relationship if there is a value in the
			// ncs_collect record
		...
		ServiceDescription sd = ServiceDescription.makeServiceDescription(ncsCollectReader,
	...
	/**
	 * Returns mdpReader for the collectionMetadataProvider (CMDP), if the CMDP has
	 * a value.
	 * <p>
	 *
	 * The CMDP is initially not assigned, and in the case of a new collection, will
	 * not be assigned until {@link #updateMetadataProvider} returns. So this method
	 * must always test for the existence of CMDP before attempting to create the
	 * mdpReader. Callers of this method must test for null before proceeding.
	 *
	 * @return The mdpReader value or null if collectionMetadataProvider has not
	 *         been assigned
	 * @exception Exception
	 *                if collectionMetadataProvider has a value but a MDP reader
	 *                cannot be created
	 */
	private MetadataProviderReader getMetadataProviderReader() throws Exception {
		...
		if (this.mdpReader != null && !this.mdpReader.getHandle().equals(this.collectionMetadataProvider)) {
	 ...
	 * possible. Used to test whether the NSDL Collection governs a collection whose
	 * items are managed in the NCS.
	 *
	 * @return The the collection_config data stream from the collection mdpReader
	 */
	...
	/**
	 * For Collections whose items are managed by this NCS instance, we can find the
	 * itemFormat by inspecting the "collection_config" stream of the Collection
	...
	private String updateMetadataProvider() throws Exception {
		...
		/*
		 * The mdp requires - relations - "aggregatedBy" collectionAggregator -
		 * datastreams - "serviceDescription"
		 */
		SignedNdrRequest request = new SignedNdrRequest();
		...
		if (collectionMetadataProvider == null) {
			...
			// otherwise the NCS agent
			String metadataProviderFor = (this.collectionAgent != null
					? this.collectionAgent
					: NDRConstants.getNcsAgent());
			...
			// metadataProviderFor - only modify this relationship if there is a
			// collectionAgent
		...
		// always update an aggregator ServiceDescription (based on collection_info of
		// ncs_collect)
		ServiceDescription sd = ServiceDescription.makeServiceDescription(ncsCollectReader,
				NDRConstants.NDRObjectType.METADATAPROVIDER);
		request.addServiceDescriptionCmd(sd.asElement());
