...
public class NCSPrep {
	...
	// String recordsPath = "C:/Documents and
	// Settings/ostwald/devel/dcs-instance-data/ndr/records";
	String recordsPath = CIGlobals.RECORDS_DIR;

	MappingsManager mm = null;

	// String dcs_data_config =
	// "/Users/ostwald/devel/projects/dcs-project/web/WEB-INF/data/configuration/common-frameworks/dcs_data.xml";
	String frameworks = CIGlobals.FRAMEWORK_CONFIG_DIR;
	// "C:/Program Files/Apache Software Foundation/Tomcat
	// 5.5/var/dcs_conf/frameworks/";
	...
	/*
	 * void initMap () throws Exception { idMap = new HashMap(); int ii = 0; for
	 * (Iterator i=mm.getMappings().iterator();i.hasNext();) { MappingInfo
	 * mappingInfo = (MappingInfo)i.next(); String metadatahandle =
	 * mappingInfo.getMetadataHandle(); String ncsrecordid = mappingInfo.getId();
	...
	NSDLCollectionReader getNSDLCollectionReader(String id) throws Exception {
		...
		/*
		 * try { String mdpHandle = mdReader.getRelationship
		 * ("ncs:collectionMetadataFor"); if (mdpHandle == null ||
		 * mdpHandle.trim().length() == 0) throw new Exception
		 * ("no collectionMetadataFor MDP for: " + mdpHandle); MetadataProviderReader
		 * mdpReader = new MetadataProviderReader (mdpHandle); String aggHandle =
		 */
	 ...
	 * Collection Record corresponding with the provided id.
	 * <p>
	 *
	 * NOTE: these values come from the CollectionConfigMappings.xml file.
	 * <p>
	 *
	...
	/**
	 * insert the mdHandle from the MappingsManagerData in the dcsDataRecord for id
	 *
	 * @param id
	 *            NOT YET DOCUMENTED
	 * @exception Exception
	 ...
	 * specified in the ncs_collect record (unless we want to change the collection
	 * agent).
	 *
	 * @return List of errors
	 */
	...
	/**
	 * insert the collection agent in the ncs_collect for id.
	 * <p>
	 *
	 * NOTE: not used for now. things work fine if the collection agent is not
	 * specified in the ncs_collect record (unless we want to change the collection
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			// prepster.updateDcsDataRecord("NSDL-COLLECTION-2669651"); // to test single
			// record
			...
			/*
			 * Test updateCollectionMetadata with a single collection (NOTE: probably want
			 * to set verbose to TRUE
			 */
			// prepster.updateCollectionMetadata("NSDL-COLLECTION-3111259");

			/*
			 * Update ALL collection metadata records (NOTE: probably want to set verbose to
			 * FALSE
			 */
			// prepster.updateAllCollectionMetadata();

			/*
			 * update a single collectionConfig record using data from
			 * CollectionConfigMappings.xml params: ncsrecordid, collectionkey
			 */
			// prepster.updateCollectionConfig("NSDL-COLLECTION-000-003-111-903",
			// "1200091746017");
