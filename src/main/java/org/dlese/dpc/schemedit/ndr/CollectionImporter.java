...
public class CollectionImporter {
	...
	public CollectionImporter(String mdpHandle, ServletContext servletContext) throws Exception {
		...
		try {
			...
			frameworkRegistry = (FrameworkRegistry) getRequiredContextAttribute("frameworkRegistry");

			collectionRegistry = (CollectionRegistry) getRequiredContextAttribute("collectionRegistry");

			dcsDataManager = (DcsDataManager) getRequiredContextAttribute("dcsDataManager");

			repositoryManager = (RepositoryManager) getRequiredContextAttribute("repositoryManager");

			xmlConversionService = null;
			try {
				xmlConversionService = (XMLConversionService) repositoryManager.getIndex()
						.getAttribute("xmlConversionService");
			} catch (Throwable t) {
			}
	 ...
	 * @exception Exception
	 *                if mdpHandle does not yield a metadataProvider object, if the
	 *                nativeFormat is unknown, or if the required file structures
	 *                cannot be created.
	 */
	...
	/**
	 * Returns MetaDataFramework for specified format.
	 * <p>
	 *
	 ...
	 * datastream, which reflect the collection's history, including status changes,
	 * imports and exports.
	 *
	 ...
	 */
	/*
	 * private CollectionMetadata getCollectionMetadata(String mdpHandle) throws
	 * Exception { cm = new CollectionMetadata(); MetadataProviderReader mdp = new
	 * MetadataProviderReader(mdpHandle); // populate HEADER
	 ...
	 * header.setDataStream("collection_config",
	 * mdp.getDataStream("collection_config"));
	 * header.setMetadataProviderDescription(mdp.getServiceDescription());
	 * header.setMetadataProviderHandle(mdp.getHandle()); // get METADATA List
	 * mdHandles = mdp.getItemHandles(); String nativeFormat =
	 * header.getNativeFormat(); prtln("about to fetch " + mdHandles.size() +
	 * " metadata items"); for (Iterator i = mdHandles.iterator(); i.hasNext(); ) {
	 * String mdHandle = (String) i.next(); MetadataReader reader = new
	 * MetadataReader(mdHandle, nativeFormat); MetaDataWrapper wrapper = new
	 * MetaDataWrapper(); wrapper.setHandle(reader.getHandle());
	 * wrapper.setPid(reader.getPid()); try { wrapper.setDataStream(nativeFormat,
	 * reader.getDataStream(nativeFormat)); } catch (Exception e) {
	 * prtln("failed to set \"" + nativeFormat + "\" datastream for " + mdHandle +
	 * " : " + e.getMessage()); } try { wrapper.setDataStream("dcs_data",
	 * reader.getDataStream("dcs_data")); } catch (Exception e) {
	 * prtln("failed to set dcs_data datastream for " + mdHandle + " : " +
	 * e.getMessage()); } cm.addMetadataWrapper(wrapper); } return cm; }
	 */
	/**
	...
	private String getSessionUserName() {
		...
		/*
		 * try { SessionRegistry sessionRegistry = (SessionRegistry)
		 * getRequiredContextAttribute("sessionRegistry"); SessionBean sessionBean =
		 * sessionRegistry.getSessionBean(request); if (sessionBean == null) { throw new
		 * Exception("SessionBean not found in SessionRegistry"); } else { sessionUser =
		 * sessionBean.getUser(); } } catch (Throwable t) {} return (sessionUser == null
		 * ? Constants.UNKNOWN_USER : sessionUser.getUsername());
		 */
		return Constants.UNKNOWN_USER;
	}

	/**
	 * Determines a collectionName for the collection to be imported.
	 * <P>
	 *
	 * Tries the following until a value is found:
	 * <ul>
	...
	/**
	 * Gets the nativeFormat for the collection to be imported.
	 * <p>
	 *
	...
	/**
	 * The location of the metadata records.
	 * <p>
	 *
	 * NOTE: The recordsDir is obtained va
	 * RepositoryManager.getMetadataRecordsLocation() when we are running as a
	 * webapp, and is provided explicitly when running from command line.
	 * <p>
	 *
	 *
	 *
	...
	private void loadCollectionDcsData() throws Exception {
		...
		DcsDataRecord dcsDataRecord = dcsDataManager.getDcsDataRecord("collect", "dlese_collect",
				collectionRecordID + ".xml", collectionRecordID);
	...
	/**
	 * Loads Extracts a collection_config stream from the metadataProvider object in
	 * the NDR and loads it into the NCS
	 *
	 * @return NOT YET DOCUMENTED
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	...
	private CollectionConfig loadCollectionConfig() throws Exception {
		 ...
		 * collection_config will NOT contain the following (which should be inserted at
		 * this time: - metadataProviderHandle, aggregatorHandle, agentHandle -
		 * collectionPrefix ...
		 */
		// Register Collection - this will create and register config if it doesn't
		// already exist
	 ...
	 *            itemRecord id
	 * @param meta
	 *            Element containing itemRecor record as sole child element
	 * @exception Exception
	 *                if meta contains other than 1 child, or if there is trouble
	 *                writing to disk
	 */
	 ...
	 *            NOT YET DOCUMENTED
	 * @exception Exception
	 *                if dcsData contains other than 1 child, or if there is trouble
	 *                writing to disk
	 */
	void writeDcsDataRecord(String id, MetadataReader md, CollectionConfig collectionConfig) throws Exception {
		...
		if (dcsData != null) {
			...
			DcsDataRecord dcsDataRecord = this.dcsDataManager.getDcsDataRecord(collectionConfig.getId(), nativeFormat,
					dcsDataRecordFileName, id);
	...
	/**
	 * Get the primary datastream as a dom4j.Element.
	 * <p>
	 *
	 * If a nativeFormat is unknown assume there is a "nsdl_dc" and transform it to
	 * "ncs_item".
	 *
	 * @param md
	 *            NOT YET DOCUMENTED
	...
	private Element getMetadataRecord(MetadataReader md, CollectionConfig config) {
		...
		if (this.nativeFormat.equals("ncs_item") && metadata == null) {
			...
			if (xmlConversionService == null || !xmlConversionService.canConvert(fromFormat, toFormat)) {
				prtln("conversionService could not convert from \"" + fromFormat + "\" to \"" + toFormat + "\"");
				return null;
			} else {
				try {
