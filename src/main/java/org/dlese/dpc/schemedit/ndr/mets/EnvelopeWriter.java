...
public abstract class EnvelopeWriter {
	...
	/**
	 * Constructor for the EnvelopeWriter object
	 * <p>
	...
	EnvelopeWriter(String ddsServiceUrl, String collection) throws Exception {
		...
		for (Iterator i = itemRecordMap.keySet().iterator(); i.hasNext();) {
			...
			try {
				/*
				 * Node urlElement = doc.selectSingleNode ("/record/general/url"); if
				 * (urlElement == null) throw new Exception ("urlElement not found"); String
				 * urlStr = urlElement.getText().trim(); if (urlStr == null || urlStr.length()
				 * == 0) throw new Exception ("url not found for record " + id); URL url = new
				 * URL (urlStr);
				 */
	...
	void metsInit() throws Exception {
		...
		// the struct map element is already present, but it has a div element that must
		// be removed
	 ...
	 *
	 * @param url
	 *            an URL pointing to a binary object
	 * @param fileID
	 *            the id to be assigned to this file
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	void addFile(URL url, String fileID) throws Exception {
		 ...
		 * not schema-valid as IDs. Instead, get file name (when needed) from image URL,
		 * which is in the metadata
		 */
		...
		/*
		 * // track memory usage java.lang.Runtime runtime =
		 * java.lang.Runtime.getRuntime(); long totalMemory = runtime.totalMemory();
		 * long freeMemory = runtime.freeMemory(); prtln ("total memory: " +
		 * totalMemory); prtln ("free memory: " + freeMemory); prtln ("total - free: " +
		 * (totalMemory - freeMemory));
		 */

		/*
		 * // the steps below do not seem to matter dlf = null;
		 * System.runFinalization(); System.gc();
		 */

		// to use fake data ...
		// binData.setText("UjBsR09EbGhjZ0dTQUxNQUFBUUNBRU1tQ1p0dU1GUXhEUzhi"); // fake
		// data
	}
	...
	public static void main(String[] args) throws Exception {
		/*
		 * TesterUtils.setSystemProps(); prtln("Mets Doc"); String baseUrl =
		 * "http://dcs.dls.ucar.edu/schemedit/services/ddsws1-1"; String collection =
		 * "1247859732310"; EnvelopeWriter md = new EnvelopeWriter(baseUrl, collection);
		 * // pp (md.reader.getDocument());
		 * 
		 * // URL contentUrl = new
		 * URL("http://www.libpng.org/pub/png/img_png/png-gammatest-ie52mac.png"); //
		 * Document metadata =
		 * DocumentHelper.createDocument(DocumentHelper.createElement("record")); //
		 * md.addRecord (id, contentUrl, metadata, "ncs_item"); // pp
		 * (md.reader.getDocument());
		 * 
		 * String path = "H:/Documents/Alliance/mets-work/mets-record.xml";
		 * Dom4jUtils.writeDocToFile(md.reader.getDocument(), new File (path)); prtln
		 * ("wrote to " + path);
		 */
	}
