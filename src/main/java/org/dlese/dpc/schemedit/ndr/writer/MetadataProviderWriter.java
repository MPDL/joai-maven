...
/**
 * The MetadataProviderWriter syncs collection-level information between the NCS
 * and the NDR. It updates (and creates, if necessary) both an aggregator and a
 * metadataProvider object in the NDR, and then records the handles for these
 * NDR objects in the collection-configuration record in the NCS.
 * <p>
 *
 * The MetadataProvider object will have the following data streams:
 * <ul>
 * <li>dcs_data - holding the contents of the collection's DcsDataRecord</li>
 * <li>collection_config - holding the collections' configuration record</li>
...
public class MetadataProviderWriter {
	 ...
	 *            NOT YET DOCUMENTED
	 * @param collDcsDataRecord
	 *            NOT YET DOCUMENTED
	 * @return NOT YET DOCUMENTED
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	public SyncReport write(String collectionRecordId, CollectionConfig collectionConfig,
			DcsDataRecord collDcsDataRecord) throws Exception {
		...
		DleseCollectionDocReader docReader = (DleseCollectionDocReader) RepositoryService
				.getXMLDocReader(collectionRecordId, rm);
		if (docReader == null)
		...
		/*
		 * CollectionID (this is an unfortunately-named property, since it is really the
		 * KEY) we write the collectionID each time to ensure that it is present when
		 * the mdpHandle is changed (e.g., when a ncs collection is ingested into the
		 * NDR)
		 */
		...
		if (metadataProviderHandle == null || metadataProviderHandle.trim().length() == 0) {
			...
			/*
			 * create a Collection MDP - the following are written ONLY at MDP creation time
			 * - they don't change over the life of the collection, or if the collection is
			 * later associated with a NDR Collection (in which case the MDP and AGG are
			 * managed by the NCS Collect record
			 */
			 ...
			 * and will be overwritten by NCS Collection Manager if this collection becomes
			 * an NDR Collection.
			 */
			ServiceDescription sd = ServiceDescription.makeServiceDescription(docReader,
	 ...
	 * <p>
	 *
	 * Updates the collection configuration, and returns the collection aggregator's
	 * handle.
	 * <p>
	 *
	 ...
	 *                NOT YET DOCUMENTED
	 */
	private String getAggregatorHandle(DleseCollectionDocReader docReader, SyncReport report) throws Exception {
		prtln("getAggregatorHandle ()");

		/*
		 * We must also test to see if this metadataProvider has a "aggregatedBy"
		 * relationship! when a collection is defined by NSDLCollectionWriter, the
		 * aggregator IS defined, but if the MDP value is simply plugged into the
		 * collection config, it won't be known by the simple
		...
		/*
		 * if mdpHandle has been manually entered in the collectionConfig (see above) it
		 * may not yet be known to collectionConfig ...
		 */
		...
		/*
		 * if the aggHandle is still not found, we create one. ISSUE: should we update
		 * the aggregator Object when it already exists? - working decision: NO - since
		 * the agg info provided by the ncs_collect is authoritative, we let the
		 * collection management side control this via the collection record
		 * (ncs_collect).
		 */
		if (aggHandle == null || aggHandle.trim().length() == 0) {
			...
			/*
			 * request.addServiceDescriptionCmd(getServiceDescription(collectionName,
			 * "aggregator"));
			 */
			InfoXML response = request.submit();
	 ...
	 * @exception Exception
	 *                if not found in ServletContext
	 */
