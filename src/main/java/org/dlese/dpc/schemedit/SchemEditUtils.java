...
public class SchemEditUtils {
	 ...
	 *            the request object
	 * @param editedRecId
	 *            id of record edited in metadata editor (if any)
	 * @param sessionBean
	 *            a sessionBean instance
	 * @return ActionForward directing to the caller of last operation.
	 */
	public static ActionForward forwardToCaller(HttpServletRequest request, String editedRecId,
	...
	/**
	 * Contract expanded ampersands (&amp;) that are part of entity references. For
	 * example, "&amp;#169;" and "&amp;amp;#169;" both become "&#169". "&amp;" by
	 * itself is not contracted. Used to convert xml record into a form desireable
	 * for xml files
	 *
	 * @param in
	 ...
	 * scheme of either "file" or "http".
	 * <P>
	 *
	 * NOTEs:
	 * <ul>
	 * <li>this method should use getUriRef to resolve the uri into either a file or
	 * url.
	 ...
	 * @exception Exception
	 *                If scheme is not recognized or if a Document could not be
	 *                created
	 */
	public static Document getLocalizedXmlDocument(URI uri) throws Exception {
		...
		if (scheme != null && scheme.equals("file")) {
			...
			try {
				...
				/*
				 * String rootElementName = rawDoc.getRootElement().getName(); doc =
				 * Dom4jUtils.localizeXml(rawDoc, rootElementName);
				 */
			} catch (Exception e) {
	 ...
	 *
	 * @param docRoot
	 *            Path to docRoot for this application to aid in resolving relative
	 *            paths.
	 * @param uri
	 *            Description of the Parameter
	 * @return The uriRef value
	...
	public static Object getUriRef(URI uri, String docRoot) throws Exception {
		...
		try {
			...
			// non-absolute URIs do not specify a scheme - THESE ARE ASSUMED TO BE FILE
			// PATHS
			// relative file paths do not begin with a "/" - these require docRoot to be
			// resolved.
			if (!uri.isAbsolute() || scheme.equals("file")) {
				...
				// now we have to determine if the FILE path specified by the URI is relative or
				// absolute
	 ...
	 *                Description of the Exception
	 */
	private static String getEditableXml(String docUri, String docRoot) throws Exception {
	...
	/**
	 * Removes all namespace information from a Document, including schemaLocation.
	 *
	 * @param doc
	 *            NOT YET DOCUMENTED
	 ...
	 *                Description of the Exception
	 */
	private static Document getEditableDocument(String docUri) throws DocumentException {
	 ...
	 *                Description of the Exception
	 */
	private static Document getEditableDocument(String docUri, boolean localize) throws DocumentException {
		try {
			...
			// prtln("schemEditUtils.getEditableDocument before localizing:" +
			// Dom4jUtils.prettyPrint(doc));

			if (localize) {
				Document localizedDoc = localizeXml(doc);
				// prtln("\n\n\nschemEditUtils.getEditableDocument AFTER localizing:" +
				// Dom4jUtils.prettyPrint(localizedDoc));
				return localizedDoc;
			} else {
				// prtln("\n\n\nschemEditUtils.getEditableDocument NOT localized:" +
				// Dom4jUtils.prettyPrint(doc));
				return doc;
			}
		} catch (Exception e) {
	 ...
	 *            uri of xml document
	 * @param framework
	 *            MetaDataFramework instance for consistency checking.
	 * @return localized dom4j.Document with ampersands expanded for editing.
	 * @exception DocumentException
	 *                Description of the Exception
	 */
	public static Document getEditableDocument(String docUri, MetaDataFramework framework) throws DocumentException {
		...
		/*
		 * prtln("framework attributes"); prtln("\t schemaTargetNameSpace: " +
		 * schemaTargetNameSpace); prtln("\t schemaLocation: " + schemaLocation);
		 * prtln("");
		 */
		try {
			...
			if (!rootName.equals(framework.getRootElementName())) {
				throw new Exception("Root element (\"" + rootName + "\") does not match schema (expected \""
						+ framework.getRootElementName() + "\")");
			}
			/*
			 * if (!root.getName().equals(framework.getRootElementName())) { throw new
			 * Exception("Root element (\"" + root.getName() +
			 * "\") does not match schema (expected \"" + framework.getRootElementName() +
			 * "\")"); }
			 */
			// schema location
			...
			// trouble with windows paths as schemaLocations (schemaLocations are NOT
			// URIs??)
			if (!framework.getXmlFormat().equals("collection_config")) {
				...
				if (!uri.isAbsolute()) {
					docSchemaLocation = "file:"
							+ GeneralServletTools.getAbsolutePath(docSchemaLocation, framework.getDocRoot());
				}

				/*
				 * prtln(" document attributes"); prtln("\t docSchemaLocation: " +
				 * docSchemaLocation); prtln("\t docTargetNameSpace: " + docTargetNameSpace);
				 * prtln("");
				 */
				// if schemaTargetNameSpace is specified, then the document must declare a
				// targetNameSpace
				if (schemaTargetNameSpace != null
						&& (docTargetNameSpace == null || !docTargetNameSpace.equals(schemaTargetNameSpace))) {
					throw new Exception(
							"document target namespace does not match schema (" + schemaTargetNameSpace + ")");
				}

				if (!docSchemaLocation.equals(schemaLocation)) {
					errorMsg = "schema location (" + schemaLocation + ") does not match metadata framework ("
							+ docSchemaLocation + ")";
	 ...
	 * which, if found in a parameter name, will filter that param from the display.
	 * E.g.,
	 * <ul>
	 ...
	 * {@link SimpleLuceneIndex#encodeToTerm}.
	 *
	 * @param q
	 *            the query string
	...
	/**
	 * formats the given date in the form "yyyy-MM-dd'T'HH:mm:ss'Z'". If given date
	 * is null the string return reflects "the epoch", namely January 1, 1970,
	 * 00:00:00 GMT.
	 *
	 * @param date
	...
	/**
	 * Return a string representation of the elapsed time since an event date (which
	 * is represented as "clicks" since "the epoch").
	 * <p>
	 *
	 * NOTE: this method does not consider elapsed times greater than 24 hours (the
	 * elapsed days are simply not shown).
	 *
	 * @param clicks
	 *            Description of the Parameter
	 ...
	 * @param id
	 *            id to insert into the metadata record
	 * @param framework
	...
	public static String stuffId(String recordXml, String id, MetaDataFramework framework) throws Exception {
		...
		if (!framework.getSchemaHelper().getNamespaceEnabled()) {
			...
			if (doc == null) {
				throw new Exception(
						"doc could not be localized - please unsure the record's root element contains namespace information");
			}
		}
	...
	/**
	 * Based on attribute name, attempts to get the value of that attribute using a
	 * bean-style "gettter".
	 *
	 * @param obj
	 *            object for which to get attr
	 * @param attr
	 *            attribute to get
