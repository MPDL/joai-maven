...
public final class RequestProcessor extends org.apache.struts.action.RequestProcessor {
	 ...
	 * which is thrown when a session has timed out. The problem is that we can't
	 * return a forward from here, so we probably have to create a new exception and
	 * catch it elsewhere (but where?).
	 *
	 * @param request
	...
	public void process(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 * here is the place (before the request is acted upon by any other class) to
		 * explicitly set the encoding of the request. The is necessary ONLY if the
		 * content-type/charset of the page containing a form has been set to UTF: <%@
		 * page contentType="text/html; charset=UTF-8" %> In this case, the input from
		 * the form (i.e., the request params) are garbled if the request encoding is
		 * not also set to UTF-8 as below. NOTE: if the content-type/charset is NOT
		 * explicitly set, then setting the request encoding to UTF-8 as below will also
		 * garble the input!!
		 */
	...
	/**
	 * Handle bean population errors that are not crucial to flow of control. For
	 * example, if we have a beanUtils.populate exception when the user is trying to
	 * navigate somewhere outside of the editor, then let the navication happen
	 * without throwing an exception. Otherwise test for timeout and present
	 * message, else, assume its a "back button" problem and show message.
	 *
	 * @param request
	 *            NOT YET DOCUMENTED
	 * @param response
	...
	private void handleBeanUtilsPopulateError(HttpServletRequest request, HttpServletResponse response)
		...
		if (sessionBean != null && sessionBean.getNumSecsToTimeout() < 1) {
			...
			// prtln("NOT TIMED OUT!");
			/*
			 * prtln("request parameters"); for (Enumeration i =
			 * request.getParameterNames(); i.hasMoreElements(); ) { prtln("\t" + (String)
			 * i.nextElement()); }
			 */
			...
			if (command != null && command.equals("exit")) {
				...
				if (pathArg != null && pathArg.equals("forwardToCaller")) {
					ActionForward forward = SchemEditUtils.forwardToCaller(request, request.getParameter("recId"),
							sessionBean);
	...
	protected void processPopulate(HttpServletRequest request, HttpServletResponse response, ActionForm form,
		...
		// but is there any advantage to worrying about it here as opposed to in
		// process()?
	...
	/**
	 * Since we do our roles processing in the AuthenticationFilter, the only job to
	 * do here is to create a sessionBean when necessary.
	 */
	protected boolean processRoles(HttpServletRequest request, HttpServletResponse response, ActionMapping mapping)
		 ...
		 * servlet.getServletContext().getAttribute("sessionRegistry"); // creates
		 * session bean if one did not exist. SessionBean sessionBean =
		 * sessionRegistry.getSessionBean(request);
		 */
