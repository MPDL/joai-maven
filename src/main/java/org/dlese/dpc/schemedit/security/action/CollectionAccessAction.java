		...
		if (command == null || isCancelled(request)) {
			...
			if (username != null) {
				...
				if (user.getRole(setInfo.getSetSpec()) == Roles.NO_ROLE) {
					errors.add("message",
							new ActionError("assign.user.access.to.collection", user.getFullName(), setInfo.getName()));
				}
			}
			...
			errors.add("error", new ActionError("generic.error", "Unrecognized command " + command));
			saveErrors(request, errors);
		}
