...
public final class UserManagerAction extends DCSAction {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		if (!authenticationEnabled) {
			errors.add("auth-not-enabled", new ActionError("authentication.not.enabled"));
			saveErrors(request, errors);
		...
		if (command == null || isCancelled(request)) {
			...
			errors.add(ActionErrors.GLOBAL_ERROR, new ActionError("generic.error", "Unrecognized command " + command));
		}

		User sessionUser = getSessionUser(request);
		List authorizedSets = repositoryService.getAuthorizedSets(sessionUser, this.requiredRole);
	 ...
	 * @return NOT YET DOCUMENTED
	 */
	private ActionErrors handleSaveCommand(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	 ...
	 * @return NOT YET DOCUMENTED
	 */
	private ActionErrors handleDeleteCommand(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {
		ActionErrors errors = new ActionErrors();
		UserManagerForm umForm = (UserManagerForm) form;
		String username = umForm.getUsername();
		...
		if (user == null) {
			errors.add("error",
					new ActionError("generic.error", "Could not delete \"" + username + "\" - user not found"));
			return errors;
		}

		/*
		 * Cannot delete user unless sessionUser "controls" all collections in which the
		 * user participates. for all user collections sessionUser.role (collection)
		 * must control user.role (collection)
		 */
		if (errors.isEmpty()) {
			// users can't delete themselves!
			User sessionUser = getSessionUser(request);
			if (user == sessionUser) {
				errors.add("error", new ActionError("You may not delete yourself"));
			}
			...
			// admins can delete any user except root or themselves
				...
				if (sessionUser == rootUser) {
					errors.add("error", new ActionError("Root user may not be deleted"));
				}
			} else {
		...
		if (errors.isEmpty()) {
			...
			prtln("   ... done updatingUserSessionBean");
			errors.add("message",
					new ActionError("generic.message", "User \"" + username + "\" - has been successfully deleted"));
		}
	 ...
	 * sorted by collection. Don't worry about sorting users, this will be done on a
	...
	public Map getUserRoleMap(Collection managedUsers, User sessionUser) {
		...
		// Debugging
		/*
		 * prtln (" ... done - getUserRoleMap has " + map.size() + " entries"); for
		 * (Iterator i=map.keySet().iterator();i.hasNext();) prtln ("\t" +
		 * (String)i.next());
		 */
	...
	private List getCollections(User sessionUser) {
		...
		Iterator sets = repositoryService.getAuthorizedSets(sessionUser, Roles.MANAGER_ROLE).iterator();
		while (sets.hasNext()) {
