...
public final class UserInfoAction extends DCSAction {
		...
		try {
			...
			if ("edit".equals(command)) {
				...
				if (user == null) {
					...
					errors.add("error", new ActionError("error.username.notfound", username));
					saveErrors(request, errors);
					return (mapping.findForward("edit.user"));
				}
			...
			if ("save".equals(command)) {
				...
				if (userForm.getFileLoginEnabled()) {
					// if password is optional and there is no value, then delete password entry for
					// this user
				...
				populateUserForm(user, sessionUser, userForm);
				errors.add("message", new ActionError("generic.message", "User info saved for \"" + username + "\""));
				saveErrors(request, errors);
			}
			...
			t.printStackTrace();
			errors.add("error", new ActionError("generic.error", "System Error: " + t.getMessage()));
			saveErrors(request, errors);
		}
	 ...
	 */
	private ActionForward getLdapUserInfo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
	 ...
	 */
	private ActionForward getUcasUserInfo(ActionMapping mapping, ActionForm form, HttpServletRequest request,
			HttpServletResponse response) {
	 ...
	 * sent in the header from event handlers. First looks for a request param named
	 * "referer" and uses this if found, otherwise uses the "referer" from the
	 * header.
	 *
	 * @param request
	 *            NOT YET DOCUMENTED
	...
	void populateUserForm(User user, User sessionUser, UserInfoForm userForm) {
		...
		boolean authenticationEnabled = (Boolean) servlet.getServletContext().getAttribute("authenticationEnabled");
		userForm.setCanAssignCollectionAccess(!authenticationEnabled || sessionUser.hasRole(Roles.MANAGER_ROLE));
		userForm.setUserHasAdminRole(user.hasRole(Roles.ADMIN_ROLE));
		/*
		 * prtln("populateUserForm() isAdmin? " + user.isAdminUser() +
		 * "  userForm.getUserHasAdminRole: " + userForm.getUserHasAdminRole());
		 */
	}
