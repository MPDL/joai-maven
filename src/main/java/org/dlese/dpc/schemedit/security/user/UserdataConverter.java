 ...
 * <p>
 * Up to ncs version 2.7.10, all user data was stored in a single "user.xml"
 * file. Release 2.7.10 stores user data in individual files. This class is
 * responsible for
 * <ul>
 * <li>read a user.xml file, parse into user-based structures (storing info for
 * a single user)
 * <li>create a subdirectory in the parent of user.xml
 * <li>write user data to individual files within the new user direcotry (named
 * by username)
...
public class UserdataConverter {
	...
	Document makeUserDocument(Element userElement) {
		...
		root.addElement("username").setText(userElement.attributeValue("username", ""));
		String isAdminUser = userElement.attributeValue("adminUser", "");
		if (!"true".equals(isAdminUser))
			isAdminUser = "false";
		root.addElement("isAdminUser").setText(isAdminUser);

		Element general = root.addElement("general");

		general.addElement("firstname").setText(userElement.attributeValue("firstname", ""));
		general.addElement("lastname").setText(userElement.attributeValue("lastname", ""));
		general.addElement("department").setText(userElement.attributeValue("department", ""));
		general.addElement("institution").setText(userElement.attributeValue("institution", ""));
		general.addElement("email").setText(userElement.attributeValue("email", ""));
