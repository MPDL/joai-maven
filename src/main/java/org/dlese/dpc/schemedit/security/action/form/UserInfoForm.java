...
public final class UserInfoForm extends ActionForm {
	...
	/**
	 * Ensure that values are present for required fields before passing control to
	 * UserInfoAction.
	 */
	public ActionErrors validate(ActionMapping mapping, HttpServletRequest request) {
		...
		if (!"create".equals(command) && (username == null || username.trim().length() == 0)) {
			errors.add("error", new ActionError("generic.error", "username required"));
		}
		...
		if ("save".equals(command)) {
			...
			if (isNewUser() && userManager.getUser(username) != null) {
				...
				errors.add("error", new ActionError("generic.error", "username \"" + username + "\" is already taken"));
			...
			if (this.getPasswordRequired() || (pass != null && pass.trim().length() > 0)
