...
public class AuthorizationFilter implements Filter {
	...
	/**
	 * Sets {@link org.dlese.dpc.schemedit.SessionRegistry} attribute from
	 * servletContext. NOTE: SessionRegistry is instantianted before filters are
	 * initialized only because it is registered as a Listener. Otherwise, the
	 * SessionRegistry would not yet be available in the servletContext.
	 *
	 * @param filterConfig
	 *            NOT YET DOCUMENTED
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		/*
		 * user is null when either: - a user has not yet logged in OR - they have just
		 * logged out OR - the PasswordHelper has not yet been initialized autoLogon if
		 * autoGuestLoginEnabled is enabled
		 */
		...
		if (accessManager == null)
			throw new ServletException(
					"AuthorizationFilter could not obtain accessManager - check system configuration!");
		...
		if (user != null) {
			...
			if (user.hasRole(requiredRole)) {
				...
				// debugging info
				/*
				 * filterInfo.put ("user", user.getUsername()); filterInfo.put ("hasRole",
				 * requiredRole.toString()); prtln (filterInfo.toString());
				 */
		...
		try {
			...
			if (dest.indexOf(req.getContextPath()) != 0) {
				throw new Exception("requestUrl path (" + dest + ") does not begin with \"" + req.getContextPath());
			}
	...
	private boolean isBypassRequest(String servletPath) {
		return (servletPath.endsWith(".jpg") || servletPath.endsWith(".gif") || servletPath.endsWith(".js")
	...
	private void autoLogon(HttpSession session) throws Exception {
		...
		/*
		 * String username = "guest"; String passwd = "guest"; Auth fa = new
		 * org.dlese.dpc.schemedit.security.auth.FileAuth(username, passwd); if
		 * (fa.authenticate()) { user = (User) userManager.getUser(username); Subject
		 * subject = fa.getSubject(); // setUserRoles (subject, user);
		...
		prtln("LogonAction: User '" + user.getUsername() + "' logged on in session " + session.getId());
	}
	...
	/*
	 * private String requestInfo (HttpServletRequest request) { String s =
	 * "\n Request Info:"; s += "\n" + "\t requestURI: " + request.getRequestURI();
	 * s += "\n" + "\t queryString: " + request.getQueryString(); s += "\n" +
	 * "\t servletPath: " + request.getServletPath(); return (s + "\n"); }
	 */
