...
public class ActionPath {
	...
	/**
	 * Helper method to extract a single role from a roleStr (like the one returned
	 * from ActionMapping.getRoles()) that in principle could return mulitiple
	 * values. Since we are using a single role string, we need to ensure that only
	 * a single value is extracted, and we use the FIRST.
	 */
	public static Role getRole(ActionMapping actionMapping) {
		String mappingRoles = actionMapping.getRoles();
		// prtln ("getRole (" + actionMapping.getPath() + ") mappingRoles: " +
		// mappingRoles);
		String roleStr = "";
