...
public final class LogonAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form, HttpServletRequest request,
		...
		try {
			...
			if (userManager == null) {
				errors.add("error", new ActionError("error.database.missing"));
				...
				if (user == null) {
					errors.add("error",
							new ActionError("generic.error", "Guest user account is not activated for this app"));
				}
			} else {
				...
				if (user == null) {
					...
					errors.add("error", new ActionError("error.password.mismatch"));
				}
			}
			...
			prtln("User '" + user.getUsername() + "' logged on in session " + session.getId());
