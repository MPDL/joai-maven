...
public class AccessManager extends AbstractConfigReader {
	...
	public AccessManager(File source, List actionMappings) throws Exception {
		super(source);
		// prtln ("\nconfig document: " + Dom4jUtils.prettyPrint
		// (docMap.getDocument()));
	...
	public Collection getRoles(Roles.Role maxRole) {
		/*
		 * List roleValues = new ArrayList (); Roles roles = Roles.getInstance(); for
		 * (Iterator i=roles.getRoles().iterator();i.hasNext();) { String role =
		 * (String)i.next(); if (roles.satisfies(maxRole, role)) { roleValues.add
		 * (role); } } return roleValues;
		 */
	 ...
	 *
	 * @param gp
	 *            The feature to be added to the GuardedPath attribute
	 */
	...
	public static void main(String[] args) throws Exception {
		...
		// String path =
		// "/Users/ostwald/Devel/projects/roles/web/WEB-INF/roles-config.xml";
