...
public class AccessTester {
	...
	AccessTester() {
		...
		/*
		 * prtln ("\ncompiled URL patterns"); for (Iterator
		 * i=urlPatternMatchers.iterator();i.hasNext();) { UrlPatternMatcher pm =
		 * (UrlPatternMatcher)i.next(); prtln (pm.toString()); }
		 */
		showPatterns();
	}
	...
	boolean isMatch(String path, String pattern) {
		...
		if (wch.match(map, path, wch.compilePattern(pattern))) {
			/*
			 * prtln ("Got a match!"); for (Iterator i=map.keySet().iterator();i.hasNext();)
			 * { String key = (String)i.next(); prtln ("\t" + key + ": " +
			 * (String)map.get(key)); }
			 */
			return true;
		}
