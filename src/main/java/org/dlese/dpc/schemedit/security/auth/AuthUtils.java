...
public class AuthUtils {
	 ...
	 *            the subject to display
	 * @param msg
	 *            a message to display along with subject
	 */
	public static void showSubject(Subject _mySubject, String msg) {
		System.out.println("\n(" + msg + ")");
		if (_mySubject != null)
			System.out.println(
					" ..." + _mySubject.toString() + "  (" + _mySubject.getPrincipals().size() + " principals)");
		else
	 ...
	 *            class name of loginModule to get
	...
	public static String getPasswordFile() {
		AppConfigurationEntry entry = getConfiguredLoginModule("org.dlese.dpc.schemedit.security.login.FileLogin");
		if (entry == null)
			return null;
		String path = null;
		try {
	 ...
	 * dom4j.Document. Used by UserInfoAction.
	 * <p>
	 *
	...
	public static Document getLdapUserInfo(String searchString, String ldapField) throws Exception {
		...
		LdapEntry[] entries = ldapClient.search("ou=DefaultOrigin,dc=nsdl,dc=org", // base = null: start at top of DB
																					// tree
				filter, // filter
	 ...
	 *            list to be joined
	 * @param joinStr
	 *            string to be inserted between tokens
