...
public class JspFunctions {
	 ...
	 *            the user oject
	 * @param role
	 *            role represented as a string (e.g., "manager")
	...
	/**
	 * Returns the xpath for the provided named schema path (pathName) as configured
	 * for the specified framework (xmlFormat). Used on the "create collection page"
	 * to give background information about some optional information associated
	 * with some frameworks.
	 *
	 * @param pathName
	 *            configured path name (from framework config)
	 * @param xmlFormat
	 *            speicified metadata framework (e.g., "adn")
	 * @param reg
	...
	/**
	 * Test for existence of file relative to the jsp page in which the function is
	 * called.
	 *
	 * @param relativePath
	 *            NOT YET DOCUMENTED
	...
	public static String getStatusLabel(String rawLabel, String collection, SessionBean sessionBean) {
		if (StatusFlags.isFinalStatusValue(rawLabel)) {
			...
			if (!derivedCollection.equals(collection)) {
				// prtln ("WARNING: statusLabel (" + rawLabel + ") could not be resolved for
				// given collection (" + collection + ")");
			}
		}
	...
	/**
	 * Gets the prompts defined in the fieldFile for the current xpath and
	 * framework.
	 *
	 * @param xpath
	 *            xpath for prompt
