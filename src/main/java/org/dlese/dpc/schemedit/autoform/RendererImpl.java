 ...
 * Element or Attribute defined in an XML Schema). Editor pages are Rendered via
 * recursive instantiations of Renderers starting with the base element for that
 * page.
 * <p>
 *
...
public class RendererImpl extends Renderer {
	...
	/**
	 * RenderMode determines whether we are editing a record, or simply viewing its
	 * contents.
	 */
	...
	/**
	 * Returns true if we are editing field contents (as opposed to simply viewing
	 * them).
	 */
	 ...
	 *
	 * @param b
	 *            The new showNSPrefixInLabels value
	 */
	 ...
	 *
	 * @param xpath
	 *            path to the node to be rendered
	 * @param parent
	 *            element to which the rendered node will be attached
	 * @param typeDef
	 *            schema-defined type definition for the node to be rendered
	 * @param rhelper
	 *            Helper class provided utility functions to aid rendering
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	...
	/**
	 * Renders a complexType element by creating an containerBox and then populating
	 * it with a formattedLabel and the complexType element contents (via
	 * renderSubElements).
	 */
	...
	/**
	 * Render a derived content model (complexType). Currently only "exensions" are
	 * supported. Derivation of ComplexTypes by "restriction" will be supported in a
	 * future version.
	 */
	 ...
	 *
	 * @param choiceElement
	 *            choice compositor element from the parent node's typeDefinition.
	 */
	...
	public Element bestPracticesLink(String xpath) {
		Element bpLink = df.createElement("span").addAttribute("class", "action-button");
		...
		Element tag = df.createElement("st__bestPracticesLink").addAttribute("pathArg", pathArg)
	 ...
	 * assuming it is an xpath. The field definition is obtained from a
	 * {@link FieldInfoReader} for the xpath. If a FieldInfoReader cannot be found,
	...
	public Element getInputHelperElement(String xpath) {
		...
		if (inputHelperPath != null) {
			...
			inputHelper = df.createElement("div").addAttribute("class", "input-helper");
	...
	public Element getInputElement(String xpath, SchemaNode schemaNode, GlobalDef typeDef) {
		 ...
		 * responsible for building their input fields. These classes would have
		 * attributes such as: - inputHelper - inputElement - ... And their "render"
		 * method would produce the required XML/JSP
		 */

		Element inputHelper = getInputHelperElement(xpath);

		Element prompts = df.createElement("st__fieldPrompt").addAttribute("pathArg", xpath);

		/*
		 * we're about to have to handle the various inputs that are possible: - html
		 * tags (these require "styleId" param - other vanilla input tags (which require
		 * "id" param) - tags, such as that produced by getInputSingleSelect
		 * 
		 * look at below and decide: should we test i 1. test for jsp tag 2. test for
		 * html tag 3. all others (table being a special case)
		 * 
		 */
		...
		/*
		 * if the 'inputElement' is a table, this is a complex element and there is
		 * input for this path. in this case we assign the id to the table to support
		 * hide/show
		 */
		if (inputElement.getName().equals("table")) {
			...
			/*
			 * If the inputElement is just a single element, then we assume this is truly an
			 * input (textarea, text, etc). In this case we create a wrapper, then add the
			 * prompt and helper and the inputElement inside the wrapper. NOTE: the id
			 * attribute gets attached to the input. Since the input is rendered using an
			 * "html" tag, the id attribute name is 'styleId'
			 */
	...
	protected Element getReadOnlyElement(String xpath) {
		...
		Element text = readOnlyElement.addElement("bean__write").addAttribute("name", formBeanName)
				.addAttribute("property", "valueOf(" + xpath + ")");
		// we add a hidden variable so that if no id is assigned, the validator will
		// catch it
		Element hidden = readOnlyElement.addElement("html__hidden").addAttribute("property", "valueOf(" + xpath + ")");
	...
	protected Element getTextInput(String xpath, SchemaNode schemaNode, GlobalDef typeDef) {
		...
		// by convention, input for elements named "description" are rendered as
		// textArea
		...
		if (typeDef.isBuiltIn() && "any".equals(NamespaceRegistry.stripNamespacePrefix(typeDef.getQualifiedName()))) {
			return getTextAreaInput(xpath);
		}
		...
		Element input = df.createElement("html__text").addAttribute("property", "valueOf(" + xpath + ")")
	...
	protected Element getBooleanInput(String xpath) {
		Element booleanInput = df.createElement("st__booleanSelect").addAttribute("elementPath",
				"valueOf(" + xpath + ")");
		return booleanInput;
	}
	...
	public Element getTextAreaInput(String xpath, int rows) {
		Element input = df.createElement("html__textarea").addAttribute("property", "valueOf(" + xpath + ")")
				.addAttribute("style", "width__98%").addAttribute("rows", String.valueOf(rows));
		return input;
	}
	...
	public Element getSelectInput(String xpath) {
		Element input = df.createElement("html__select").addAttribute("name", formBeanName).addAttribute("property",
				"valueOf(" + xpath + ")");
		Element options = input.addElement("html__optionsCollection").addAttribute("property",
				"selectOptions(" + RendererHelper.normalizeXPath(xpath) + ")");
		return input;
	}
	...
	public Element getLanguageInput(String xpath) {
		Element select = df.createElement("st__languageSelect").addAttribute("elementPath", xpath);
		return select;
	}

	/*
	 * <logic:iterate name=formBeanName indexId="index"
	 * property="enumerationOptions(${subjectsPath})" id="item"> <html:multiSelect
	 * property="enumerationValuesOf(${subjectsPath})"> <bean:write name="item"
	 * property="value"/> </html:multiSelect> <bean:write name="item"
	 * property="label"/><br> </logic:iterate>
	 */
	/**
	...
	public Element getMultiSelectInput(String xpath) {
		...
		Element row = multiSelectTable.addElement("tr").addAttribute("valign", "top");

		Element multiSelectCell = row.addElement("td");

		/*
		 * a hidden field is necessary to compensate for the fact that we don't have a
		 * reset method in the form bean. the hidden field points to the subjectsPath
		 * without an index modifier on the leaf..
		 */
		Element hidden = multiSelectCell.addElement("input").addAttribute("type", "hidden")
	...
	protected Element getMultiBoxInput(String xpath) {
		...
		Element multiBoxInput = df.createElement("logic__iterate").addAttribute("indexId", "index")
				.addAttribute("id", "item").addAttribute("name", formBeanName)
				.addAttribute("property", "enumerationOptions(" + XPathUtils.getSiblingXPath(normalizedXPath) + ")");

		Element labelFormat = multiBoxInput.addElement("div").addAttribute("class", "multiSelect-label");

		String labelId = CollapseUtils.pathToId(xpath) + "_${item.value}_label";

		Element multiSelect = labelFormat.addElement("html__multibox")
				.addAttribute("property", "enumerationValuesOf(" + xpath + ")").addAttribute("styleId", labelId);

		/*
		 * "filter=false" addresses problem where controlled vocab contains an apos
		 * ("'") by supressing bean:write's default behavior of quoting. but will this
		 * have any side effects? and where else do we need to display controlled vocabs
		 * this way?
		 */
		Element value = multiSelect.addElement("bean__write").addAttribute("name", "item")
				.addAttribute("property", "value").addAttribute("filter", "false");

		Element label = labelFormat.addElement("label").addAttribute("for", labelId);

		Element labelText = label.addElement("bean__write").addAttribute("name", "item").addAttribute("property",
				"label");
	...
	protected Element getComboUnionInput(String xpath) {
		Element select = df.createElement("st__comboInput").addAttribute("elementPath", xpath);
		return select;
	}
	...
	/**
	 * Label text is indexed if siblingPath and indexId provided, otherwise based on
	 * xpath
	 *
	 * @param xpath
	 *            NOT YET DOCUMENTED
	...
	public Element getDeleteController(String itemPath, String elementName) {
		...
		Element deleteController = df.createElement("span").addAttribute("class", "action-button");
		...
		Element deleteItemLink = deleteController.addElement("a").addAttribute("href",
				"javascript:" + jsCall + "(" + RendererHelper.jspQuotedString(itemPath) + ")");
	...
	public Element getOptionalItemControl(String xpath) {
		...
		Element newItemAction = elementNotExists.addElement("span").addAttribute("class", "action-button");

		Element newItemLink = newItemAction.addElement("a").addAttribute("href",
				"javascript:doNewElement(" + RendererHelper.jspQuotedString(xpath) + ")");
		...
		Element delItemAction = elementExists.addElement("span").addAttribute("class", "action-button");

		Element delItemLink = delItemAction.addElement("a").addAttribute("href",
				"javascript:doDeleteElement(" + RendererHelper.jspQuotedString(xpath) + ")");
	...
	public ComplexTypeLabel getComplexTypeLabel(String xpath, String siblingPath, String indexId) {
		...
		if (getLevel(xpath) == 0) {
			...
			/*
			 * label an element as required if - it is a requiredBranch OR - it is
			 * requiredContent AND - the element is not repeating this is not perfect, since
			 * sometimes (such as the last repeating element) we might want to designate as
			 * required)
			 */
			if ((sh.isRequiredBranch(schemaNode) || sh.isRequiredContentElement(schemaNode))
		...
		// label text inexed if siblingPath and indexId provided, otherwise based on
		// xpath
		labelObj.setText(getLabelText(xpath, siblingPath, indexId));
	...
	public Element getRenderedField(String xpath, Element label, Element input) {
		...
		Element table = df.createElement("table").addAttribute("class", "input-field-table");

		Element row = table.addElement("tr").addAttribute("class", "form-row");

		Element labelCell = row.addElement("td").addAttribute("class", "label-cell");
		rhelper.attachToolHelp(labelCell, xpath);
		labelCell.add(label);

		Element inputCell = row.addElement("td").addAttribute("class", "input-cell");
