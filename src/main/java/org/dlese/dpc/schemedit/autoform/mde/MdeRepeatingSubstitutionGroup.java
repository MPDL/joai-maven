...
import org.dlese.dpc.schemedit.autoform.ComplexTypeLabel;
...
/**
 * Renders JSP for Repeating Substitution Group elements for the metadata
 * editor.
 *
 * @author ostwald
 *         <p>
...
public class MdeRepeatingSubstitutionGroup extends MdeRepeatingNode {
	...
	/**
	 * Does not attempt to preserve order of elements in the instance document, but
	 * rather groups them by their element name.
	 */
	public void render() {
	...
	/**
	 * Renders the contents of this node, which are in turn rendered inside an
	 * iteration element.
	 *
	 * @param itemPath
	...
	protected Element getRepeatingContent(String itemPath, String siblingPath, String indexId) {
		...
		if (typeDef.isBuiltIn() || typeDef.isSimpleType()) {
			...
			Element subElementsBox = repeatingContent.addElement("div").addAttribute("id", "${id}")
	...
	protected Element getSubstitutionGroupNewItemControl() {
		...
		Element newItemAction = df.createElement("div").addAttribute("class", "action-button");
		// make a drop-down menu with choices, and present as controller

		/*
		 * do not present control if there are no children - this case is handled
		 * elsewhere (search for "required choice") -- but should it be handled here?
		 */
		Element notEmptyTest = newItemAction.addElement("logic__notEmpty").addAttribute("name", formBeanName)
		...
		// schema constraints
		Element canAcceptTest = notEmptyTest.addElement("logic__equal").addAttribute("name", formBeanName)
		...
		Element tmpArgInit = selectWrapper.addElement("jsp__setProperty").addAttribute("name", formBeanName)
				.addAttribute("property", "tmpArg").addAttribute("value", "");

		// the select element for making a choice
		Element newItemSelect = selectWrapper.addElement("html__select").addAttribute("name", formBeanName)
				.addAttribute("property", "tmpArg").addAttribute("onchange", "doNewChoice (this.value)");

		// the permissible choices for this element
		Element newChoiceOptions = newItemSelect.addElement("html__optionsCollection").addAttribute("property",
				"substitutionGroupOptions(" + xpath + ")");

		rhelper.attachToolHelp(newItemSelect, "Choose an element to create");

		// wrap the newItemAction in a table. the left cell is the controller, and the
		// right is blank
		...
		Element controllerCell = row.addElement("td").addAttribute("class", "action-box").addAttribute("nowrap", "1");

		controllerCell.add(newItemAction);

		Element emptyCell = row.addElement("td").addAttribute("width", "95%");
		emptyCell.setText(" ");
