...
public class RendererHelper {
	...
	/**
	 * Constructor for the RendererHelper object with renderer class name explicitly
	 * specified. Used by View Renders, such as {@link EditorViewerRenderer}, for
	 * which the renderer cannot be obtained from the MetaDataFramework.
	 *
	 * @param root
	 ...
	 *
	 * @param root
	 *            root {@link org.dom4j.Element} to be rendered
	 * @param framework
	 *            MetaDataFramework representing the XML Schema to be used
	 * @param formBeanName
	 *            Description of the Parameter
	 */
	...
	public void attachElementDebugInfo(Element target, String displayText, String borderColor) {
		if (showElementDebugInfo) {
			Element info = DocumentHelper.createElement("div").addAttribute("class", "element-debug-info");
			// s + " (" + getLevel() + ")";
	...
	/**
	 * Removes any and all indexing information from an XPath. In particular, cleans
	 * up any JSP-encoding, including curly bracketed indexing (e.g., ${indexId+1}).
	 * This form of XPath is necessary for accessing schemaNodes, which contain no
	 * indexing.
	 *
	 * @param encodedXPpath
	 *            Description of the Parameter
	...
	/**
	 * Returns the configured inputHelper value for the provided xpath (inputHelper
	 * value is relative to the /editor/input_helper directory in the deployed app
	 *
	 * @param xpath
	 *            NOT YET DOCUMENTED
	 ...
	 * with underscore to avoid javascript problems with qualified names. Then add a
	 * string ("Index") and indexId index, if required, to avoid name collisions.
	 *
	 * @param xpath
	 *            NOT YET DOCUMENTED
	 * @return NOT YET DOCUMENTED
	...
	public static Element jspInclude(String page) {
		return DocumentHelper.createElement("jsp__include").addAttribute("page", page);
	}
	 ...
	 * @param xpath
	 *            NOT YET DOCUMENTED
	 * @return An Element representing the test for the existance of a parent node
	 */
	public Element parentNodeExistsTest(String xpath) {
		Element tester = DocumentHelper.createElement("logic__equal").addAttribute("name", formBeanName)
				.addAttribute("property", "parentNodeExists(" + xpath + ")").addAttribute("value", "true");
		return tester;
	}
	...
	public Element parentNodeNotExistsTest(String xpath) {
		Element tester = DocumentHelper.createElement("logic__equal").addAttribute("name", formBeanName)
				.addAttribute("property", "parentNodeExists(" + xpath + ")").addAttribute("value", "false");
		return tester;
	}
	...
	public Element nodeExistsTest(String path) {
		Element tester = DocumentHelper.createElement("logic__equal").addAttribute("name", formBeanName)
				.addAttribute("property", "nodeExists(" + path + ")").addAttribute("value", "true");
		return tester;
	}
	...
	public Element nodeHasChildrenTest(String path) {
		Element tester = DocumentHelper.createElement("logic__greaterThan").addAttribute("name", formBeanName)
				.addAttribute("property", "childElementCountOf(" + path + ")").addAttribute("value", "0");
		return tester;
	}
	...
	public Element nodeHasMembersTest(String path) {
		Element tester = DocumentHelper.createElement("logic__greaterThan").addAttribute("name", formBeanName)
				.addAttribute("property", "memberCountOf(" + path + ")").addAttribute("value", "0");
		return tester;
	}
	...
	public Element acceptsNewSiblingTest(String path) {
		Element tester = DocumentHelper.createElement("logic__equal").addAttribute("name", formBeanName)
				.addAttribute("property", "acceptsNewSibling(" + path + ")").addAttribute("value", "true");
		return tester;
	}
	...
	public Element acceptsNewChioceTest(String path) {
		Element tester = DocumentHelper.createElement("logic__equal").addAttribute("name", formBeanName)
				.addAttribute("property", "acceptsNewChoice(" + path + ")").addAttribute("value", "true");
		return tester;
	}
	...
	public Element nodeNotExistsTest(String path) {
		Element tester = DocumentHelper.createElement("logic__notEqual").addAttribute("name", formBeanName)
				.addAttribute("property", "nodeExists(" + path + ")").addAttribute("value", "true");
		return tester;
	}
	...
	public Element viewNodeTest(String xpath) {
		...
		Element test = DocumentHelper.createElement("logic__equal").addAttribute("name", formBeanName)
				.addAttribute("property", "viewNode(" + xpath + ")").addAttribute("value", "true");
		return test;
	}
	...
	public void attachMessages(Element container, String xpath) {
		/*
		 * propertyId is determined by the type of input element this xpath is
		 * associated with. most elements will use "valueOf", but multiSelectes require
		 * "enumerationValuesOf"
		 */
		...
		Element elementMessagesTag = container.addElement("st__elementMessages").addAttribute("propertyId", propertyId);
	}
	 ...
	 *
	 * @param e
	 *            The element to recieve the tool help
	 * @param helpMsg
	 *            Description of the Parameter
	 */
