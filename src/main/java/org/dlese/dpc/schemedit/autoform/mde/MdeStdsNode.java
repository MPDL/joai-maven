...
/**
 * Renders JSP for nodes that are controlled by Standards Manager (in conjuction
 * with suggestion service).
 *
 * @author ostwald
 *         <p>
 *
...
public class MdeStdsNode extends MdeSimpleType {
	...
	/**
	 * Renders a managed standards field, unless this collection does not allow the
	 * suggestion service, in which case the field is rendered as a repeating simple
	 * type
	 */
	protected void render_edit_mode() {
		...
		// create box
		Element box = df.createElement("div").addAttribute("id", "${id}_box");
		...
		try {
			...
			// render this xpath as a repeatingSimpleType, attached to the "otherwise"
			// clause
