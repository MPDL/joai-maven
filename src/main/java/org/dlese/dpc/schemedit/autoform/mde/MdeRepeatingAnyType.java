 ...
 * <p>
 * All existing nodes are rendered, and then an empty input is rendered for a
 * new one.
 *
 * @author ostwald
 *
...
public class MdeRepeatingAnyType extends MdeRepeatingNode {
	...
	Element newItemInput() {
		...
		String itemPath = siblingPath + "_${" + indexId + "+1}_";
		Element id = this.df.createElement("bean__define").addAttribute("id", indexId)
				.addAttribute("name", this.formBeanName).addAttribute("property", "memberCountOf(" + xpath + ")");
		...
		int rows = 8;
		Element input = df.createElement("html__textarea").addAttribute("property", "anyTypeValueOf(" + itemPath + ")")
