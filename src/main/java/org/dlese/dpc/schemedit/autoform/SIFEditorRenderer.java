...
/**
 * Renders JSP for SIF (Schools Interoperability Framework) metadata frameworks,
 * adding functionality for selecting from SIF objects, and other sif-specific
 * input objects.
 *
 * @author ostwald
 *         <p>
...
public class SIFEditorRenderer extends DleseEditorRenderer {
	 ...
	 * construct and does not (at this point) contain any fields we need to supply.
	 */
	 ...
	 *
	 * @param xpath
	 *            path to element to be rendered
	 * @param siblingPath
	 *            sibling path to support indexing
	 * @param indexId
	...
	public String getLabelText(String xpath, String siblingPath, String indexId) {
		if (siblingPath != null && indexId != null) {
			String nodeName = NamespaceRegistry.stripNamespacePrefix(XPathUtils.getNodeName(siblingPath));
			return (nodeName + " ${" + indexId + "+1}");
		} else {
			String nodeName = NamespaceRegistry.stripNamespacePrefix(XPathUtils.getNodeName(normalizedXPath));
			return nodeName;
		}
	}
	 ...
	 *
	 * @param xpath
	 *            xpath of node to be rendered
	 * @param schemaNode
	 *            schemaNode of node to be rendered
	 * @param typeDef
	 *            typeDef
	...
	protected Element getTextInput(String xpath, SchemaNode schemaNode, GlobalDef typeDef) {
		...
		if (refIdMgr != null) {
			...
			if (refIdMap != null && refIdMap.hasPath(this.normalizedXPath)) {
				prtln("\t***");
				Element refIdTag = df.createElement("st__sifRefId").addAttribute("property", "valueOf(" + xpath + ")")
