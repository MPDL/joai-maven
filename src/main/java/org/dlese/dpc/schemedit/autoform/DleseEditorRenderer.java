...
import org.dlese.dpc.schemedit.autoform.mde.MdeStdsNode;
 ...
 * <li>Rendering repeating elements as MdeStdNode if element has been configured
 * to use standards manager
 * <li>Rendering of textInput elements as textAreas for configured elements of
 * "concepts" and "fields_files" frameworks
...
public class DleseEditorRenderer extends EditorRenderer {
	 ...
	 * rendered:
	 * <li>mdvMultiBoxStateful - collapsible hierarchies that remember their rhelper
	 * <li>mdvMultiBoxStateless - collapsible hierarchies that DON'T remember their
	 * rhelper
	...
	/**
	 * Create the multiBoxTag corresonsponding to the value of the mdvMultiBox value
	 *
	...
	/**
	 * Renders a multibox input (a set of of checkboxes) as an Element using one of
	 * several methods of rendering a multiBoxInput, depending on what information
	 * is available for the current element.
	 * <p>
	 *
	 *
	 * <ul>
	 ...
	 * <li>If the current element has fieldInfo, then the multibox is rendered by
	 * the "fieldInfoMultiBox" tag</li>
	 * <li>If neither of these are available, then the mulitbox is rendered by a
	 * superclass, such as
	 * {@link org.dlese.dpc.schemedit.autoform.SimpleJspRenderer#getMultiBoxInput()}
	 * </li>
	 * </ul>
	 *
	 *
	...
	public Element getMultiBoxInput(String xpath) {
		...
		if (rhelper.hasVocabLayout(xpath)) {
			prtln("\t has LAYOUT");
			Element layoutMultiBox = df.createElement("vl__vocabLayoutMultiBox").addAttribute("elementPath",
					"enumerationValuesOf(" + XPathUtils.getSiblingXPath(xpath) + ")");
			multiBoxInput = layoutMultiBox;
		} // does this xpath have a MUI group?
		else if (rhelper.hasMuiGroupInfo(xpath)) {
			prtln("\t hasMuiGroupInfo");
			Element mdvMultiBox = df.createElement(getMultiBoxTag()).addAttribute("elementPath",
					"enumerationValuesOf(" + XPathUtils.getSiblingXPath(xpath) + ")");
			multiBoxInput = mdvMultiBox;
		} // 5/23/08 check not only that there is a fields file, but also that there is a
			// termlist (vocabs) defined
		else if (rhelper.hasFieldInfo(xpath) && !rhelper.getFieldInfo(xpath).getTermList().isEmpty()) {
			prtln("\t hasFieldInfo");
			Element fieldInfoMultiBox = df.createElement("st__fieldInfoMultiBox").addAttribute("elementPath",
					XPathUtils.getSiblingXPath(xpath));
		...
		if (sm != null && normalizedXPath.equals(sm.getXpath())) {
			...
			Element helperPresent = df.createElement("c__when").addAttribute("test",
					"${not empty " + this.formBeanName + ".suggestionServiceHelper}");
	...
	/**
	 * If there is a vocabLayout (groups file) for this xpath, use it to layout the
	 * element.
	 * <p>
	 *
	 * NOTES<br/>
	 * 1 - the path passed to the "vl__vocabLayoutSingleSelect" is not a
	 * "siblingPath" as is the case with multi-selects, but the actual xpath, since
	 * here we are collecting a single value<br/>
	...
	protected Element getTextInput(String xpath, SchemaNode schemaNode, GlobalDef typeDef) {
		...
		// concepts hack: is this an element from the concepts framework that ends in
		...
		if (xmlFormat.equals("fields_file")) {
			...
			if (nodeName.equals("practice") || nodeName.equals("definition") || nodeName.equals("termAndDeftn")) {
		...
		if (schemaNode.isDerivedModel()) {
			...
			// because it is derived, the typeName for this element has been forced to that
			// of the
	...
	private List getXsdStringInputsToHide() {
		if (xsdStringInputsToHide == null) {
			...
			xsdStringInputsToHide
					.add("/itemRecord/geospatialCoverages/geospatialCoverage/detGeos/detGeo/longLats/longLat");
	 ...
	 * used as the extention type for a derivedTextOnly Model schematype.
	 * <p>
	 *
	 * Bottom Line, by DLESE convention, we almost NEVER show xsd:string elements
	 * that are the base extension of a derivedTextOnlyModel. This method only makes
	 * exceptions for the ADN framework. In all other frameworks, the xsd:string
	 * extension element is NOT shown.
	 *
	 * @param xpath
	 *            xpath of element to test
