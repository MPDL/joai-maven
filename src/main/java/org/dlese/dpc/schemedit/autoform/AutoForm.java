 ...
 * {@link org.dlese.dpc.schemedit.autoform.DleseEditorRenderer}) for editing and
 * viewing of schemedit-based xml documents.
 * <p>
 *
 * Called from command line for debugging as well as from
 * {@link org.dlese.dpc.schemedit.MetaDataFramework} at start-up time and after
...
public class AutoForm {
	...
	public void batchRenderAndWrite() throws Exception {
		...
		for (Iterator i = elements.iterator(); i.hasNext();) {
			...
			Element e = render(xpath);
			if (e != null && !writeJsp(e, getBatchJspDest(pageName), this.getComponentJspHeader())) {
	...
	/**
	 * Perform any modifications to the XML to create legal JSP. Some strings, such
	 * as tag-like notation (e.g., "c:set") are not convenient in XML processing,
	 * since they are interpreted as namespaces, so they are encoded by the renderer
	 * using a convention (e.g., "c__set") and then decoded here to convert to JSP
	 *
	 * @param e
	...
	protected String getMasterJspHeader() {
		...
		header += "<bean:define id=\"collapseBean\" name=\"" + formBeanName
				+ "\" property=\"collapseBean\" type=\"CollapseBean\" />\n\n";
		return header;
	}
	 ...
	 *
	 * @param element
	 *            Element representing editor page
	 * @param dest
	 *            NOT YET DOCUMENTED
	 * @param jspHeader
	 *            NOT YET DOCUMENTED
