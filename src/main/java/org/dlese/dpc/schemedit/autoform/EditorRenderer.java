...
public class EditorRenderer extends RendererImpl {
	 ...
	 * <P>
	 *
	 * Based on getComplexTypeLabel, but will always display collapse widget, rather
	 * than first testing for nodeIsExpandable as getComplexTypeLabel does.
	 * <P>
	 *
	...
	public Label getMultiBoxLabel(String xpath) {
		...
		Element alwaysExpandable = df.createElement("c__if").addAttribute("test", "${true}");
		Element labelLink = alwaysExpandable.addElement("a").addAttribute("href",
				"javascript__toggleDisplayState(" + RendererHelper.jspQuotedString("${id}") + ");");

		Element notExpandableTest = df.createElement("c__if").addAttribute("test", "${false}");
	...
	public ComplexTypeLabel getComplexTypeLabel(String xpath, String siblingPath, String indexId) {
		...
		if (getLevel(xpath) > 0) {
			...
			Element nodeIsExpandable = df.createElement("logic__equal").addAttribute("name", formBeanName)
					.addAttribute("property", "nodeIsExpandable(" + itemPath + ")").addAttribute("value", "true");

			// the clickable version
			Element labelLink = nodeIsExpandable.addElement("a").addAttribute("href",
					"javascript__toggleDisplayState(" + RendererHelper.jspQuotedString("${id}") + ");");

			Element nodeNotExpandable = df.createElement("logic__notEqual").addAttribute("name", formBeanName)
					.addAttribute("property", "nodeIsExpandable(" + itemPath + ")").addAttribute("value", "true");
	...
	protected Element getCollapseWidget() {
		Element img_template = df.createElement("img").addAttribute("id", "${id}_img").addAttribute("border", "0")
				.addAttribute("hspace", "3").addAttribute("height", "12").addAttribute("width", "12");

		Element choose = df.createElement("c__choose");

		Element isOpen = choose.addElement("c__when").addAttribute("test",
				"${" + formBeanName + ".collapseBean.isOpen}");
