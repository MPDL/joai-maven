...
public class MdeRepeatingComplexType extends MdeRepeatingNode {
	...
	public void render() {
		...
		if (isEditMode()) {
			...
			/*
			 * PROBLEM - /nsdl_anno/structuredOutline/header/outline is being evaluated as a
			 * isRepeatingComplexSingleton, but we want the emptyRepeatingElement to be
			 * added!
			 * 
			 * WHY don't we add emptyRepeating conrole here?? Assumption: we DON'T want this
			 * control for "wrapped repeating singltons", e.g, - SimplePlacesAndEvents (this
			 * is where the add control is when there are no children) - placeAndEvent
			 * (repeat) - don't want here (cause its above)
			 */
			...
			if (!sh.isRepeatingComplexSingleton(normalizedXPath)) {
				...
				/*
				 * we only put addItemControl on OPTIONAL repeatingComplexSingletons, Since if
				 * the element does not exist, it won't be displayed without the addItemControl!
				 */
	...
	protected Element getRepeatingContent(String itemPath) {
		...
		if (getLevel() > 0 && isEditMode()) {
			...
			/*
			 * this.attachElementDebugInfo(subElementsBox, "subElementsBox: id=${id}",
			 * "green");
			 */
		}
