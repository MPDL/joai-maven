...
public class MdeChoice extends MdeComplexType {
	 ...
	 * can be displayed for it. So renderChoice creates an iterator that will
	...
	public void render(Element choiceElement) {
		...
		if (isEditMode()) {
			...
			Element newChoiceTest = nodeExistsTest.addElement("logic__empty").addAttribute("name", formBeanName)
			...
			Element label = df.createElement("div").addAttribute("class", "field-label");
	...
	protected Element getChoiceSelect() {
		// --- CHOICE SELECT ----
		// tmpArg holds choice. It is initialized to "" so the "-- choice --" option is
		// selected,
		...
		Element tmpArgInit = wrapper.addElement("jsp__setProperty").addAttribute("name", formBeanName)
				.addAttribute("property", "tmpArg").addAttribute("value", "");

		// the select element for making a choice
		Element newChoiceSelect = wrapper.addElement("html__select").addAttribute("name", formBeanName)
				.addAttribute("property", "tmpArg").addAttribute("onchange", "return (doChoice(this.value))");

		// the permissible choices for this element
		Element newChoiceOptions = newChoiceSelect.addElement("html__optionsCollection").addAttribute("property",
				"choiceOptions(" + xpath + ")");
