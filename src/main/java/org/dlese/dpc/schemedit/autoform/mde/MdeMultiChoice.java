...
public class MdeMultiChoice extends MdeChoice {
	 ...
	 * which of the choices actually exists in the instance document before an input
	 * field can be displayed for it. So renderChoice creates an iterator that will
	 * construct the paths to the possible choices, and when one of the paths
	 * actually exists in the document, then that path is used to construct an input
	 * element. If there are no children elements, then display
	 *
	 * @param choiceElement
	 *            NOT YET DOCUMENTED
	 */
	public void render(Element choiceElement) {
		 ...
		 * compositor), do an existence check to see if that choice is present in the
		 * document. Then render the node
		 */
		...
		if (isEditMode()) {
			/*
			 * If the choice node exists but a choice has not been made, render the select
			 * input by which the user makes a choice. This select input is presented as a
			 * "required choice", not in the sense that the element is schema-required, but
			 * more in the sense that we want the user to know there is a choice to be made.
			 * If the choice element is schema-optional, the user can remove it via the
			 * remove action.
			 */
	...
	/**
	 * Renders input that is presented when a choice has not yet been made for this
	 * element.
	 *
	...
	Element firstChoiceSelect() {
		...
		Element newChoiceTest = DocumentHelper.createElement("logic__empty").addAttribute("name", formBeanName)
		...
		/*
		 * Add a hidden element here so that the validator will detect non-made choices.
		 */
		Element hidden = newChoiceDiv.addElement("input").addAttribute("type", "hidden")
				.addAttribute("name", "valueOf(" + xpath + ")").addAttribute("value", "");

		Element label = DocumentHelper.createElement("div").addAttribute("class", "field-label");
		...
		Element choiceField = this.renderer.getRenderedField(xpath, label, input);
	...
	Element getChoiceSiblingController() {
		...
		/*
		 * jsp to create an id for the "add child controller" that is based on the
		 * indexed path (see MdeNode.insertDisplaySetup for pattern) the id is named
		 * "controlId" and is used in two places: 1 - "add new child" link (which makes
		 * a "toggleVisibility" javascript call) 2 - the id for the "choice select"
		 * element, which is opened by the "add new child" link
		 */
		Element childPath = df.createElement("c__set").addAttribute("var", "childPath").addAttribute("value", xpath);
		newItemControl.add(childPath);

		Element setId = df.createElement("c__set").addAttribute("var", "controlId").addAttribute("value",
				"${sf__pathToId(childPath)}_add_child_controller");
		...
		Element newItemLink = style.addElement("a").addAttribute("href",
				"javascript:toggleVisibility(" + RendererHelper.jspQuotedString("${controlId}") + ")");
		newItemLink.setText("add new child");
		rhelper.attachToolHelp(newItemLink, "Add a child for " + xpath);

		// wrap prompt and select in div
		Element choiceSelect = getChoiceSelect().addAttribute("id", "${controlId}").addAttribute("style",
				"display:none;");
