...
/**
 * Abstract class for rendering an editor for a SchemaNode (an Element or
 * Attribute defined in an XML Schema). Editor pages are Rendered via recursive
 * instantiations of Renderers starting with the base element for that page.
 * <p>
 *
 * The product is a <b>RenderTree</b> is a {@link org.dom4j.Element} structure
 * that expresses a metadata editor interface. The RenderTree is almost leagal
 * jsp but must still be slightly massaged before it can be processed as jsp
 * (see AutoForm).
 * <p>
 *
 * All concrete classes render JSP-based editors, but in the future we may want
 * to support other editors. This class contains no JSP-specific methods, but it
 * does enforce a particular style of rendering in which individual schema nodes
 * are rendered and attached to a <b>parent</b> element. The Renderer is called
 * recursively as it traverses the schemaStructure, starting at the <b> root</b>
 * Element of the schema (a {@link SchemaNode}) and building the RenderTree as
 * it traverses the schema structure.
 *
 * @author ostwald
 *
 */
public abstract class Renderer {
	...
	/**
	 * A {@link SchemaHelper} instance that provides schema-related information and
	...
	public String xpath;
	/**
	 * A {@link org.dom4j.Element} representing the parent of the node being
	 * rendered.
	 */
	public Element parent;
	/**
	 * The {@link SchemaNode} which wraps the schema element (which may represent an
	 * Element Attribute) being rendered
	 */
	public SchemaNode schemaNode;
	/**
	 * The XPath corresponding to the SchemaNode. This path contains no indexing or
	 * jsp-encoding since it points to a the schema's instanceDocument.
	 */
	...
	/**
	 * Data structure containing global information required by individual Renderer
	 * instances.
	 */
	...
	/**
	 * Name of the element-to-be-rendered's DataType. This field is required because
	 * in some cases the element is rendered as a different type from which it is
	 * defined.
	 */
	...
	/**
	 * Place holder for a path to a repeating child. This field is null for elements
	 * that do not contain a repeating child.
	 */
	 ...
	 *
	 * @param xpath
	 *            xpath to the Node to be rendered
	 * @param parent
	 *            The element to which the rendered Node is attached as a child
	 * @param rhelper
	 *            Pointer to the RendererHelper
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	...
	/**
	 * Constructor for the Renderer object in which the dataType to render is
	 * specified (rather than being determined by the schemaNode at <b>xpath </b> .
	 * This constructor is used to render schema nodes that have a derivedModel,
	 * which must be rendered using the dataType they extend.
	 *
	 * @param xpath
	 *            xpath to the Node to be rendered
	 * @param parent
	 *            The element to which the rendered Node is attached as a child
	 * @param dataTypeName
	 *            The name of the dataType as which the node is rendered
	 * @param rhelper
	 *            Pointer to the RendererHelper
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 ...
	 *
	 * @param xpath
	 *            The jsp-encoded xpath to the element to be rendered.
	 * @param parent
	 *            The {@link SchemaNode} which wraps the schema element (which may
	 *            represent an Element Attribute) being rendered
	 * @param rhelper
	 *            Data structure containing global information required by
	 *            individual Renderer instances
	 */
	...
	/**
	 * Render an editor for the schemaNode (i.e., a Node of the schema's
	 * instanceDocument) specified by the <b> xpath</b> field.
	 */
	public void renderNode() {
		...
		if (schemaNode.isRecursive()) {
			/*
			 * IMPORTANT to set the text of the parent node to empty string so that the jsp
			 * rendered is <div></div>, rather than <div/>, which is NOT processed as a
			 * closed element by browsers and leads to nasty layout problems.
			 */
			parent.setText("");
			return;
		}
	...
	/**
	 * Implementation-specific method to render an editor for a schemaNode defined
	 * as {@link SimpleType}
	 */
	...
	public void renderComplexType() {
		...
		// derivedModels (simpleContent and complexContent) are rendered by their own
		// methods
	...
	/**
	 * Render a derived Text-Only content model (ComplexType having SimpleContent
	 * element)
	 * <p>
	 *
	...
	/**
	 * Render a choice node. Each of the possible choices is tested for existance,
	 * and finally a test for an empty choice is included to account for the
	 * situation in which a new parent element has been added but the choice has not
	 * yet been made.
	 *
	 * @param choice
	 *            The choice element of the type definition (see
	 *            {@link ComplexType})
	 */
	 ...
	 *
	 * @param sequence
	 *            the sequence compositor {@link org.dom4j.Element}
	 */
	 ...
	 * children elements of the type definition.
	 * <p>
	 *
	 ...
	 * quotation mark when the rendered Element is writen to disk. This is necessary
	 * because dom4J turns quotation marks and appostrophies into entities, which
	 * then cause the JSP processer to barf when the jsp page is rendered
	 *
	 * @param s
	 *            Description of the Parameter
