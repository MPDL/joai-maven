 ...
 * node. Principle contribution is getRepeatingContentBox method, which builds a
 * box (a "div" element) into which each repetition of the repeating nodes is
...
public abstract class MdeRepeatingNode extends MdeNode {
	 ...
	 *
	 * @param renderer
	 *            the Renderer instance for this node
	 */
	...
	protected Element getRepeatingContentBox(String itemPath) {
		...
		/*
		 * the following works because "itemPath" has been defined in the jsp (see
		 * getIteration) prior to the repeating repeatingContent
		 */
	 ...
	 * iteration element takes care of assigning an indexed path (i.e., *itemPath*)
	 * to the repeating items. The iteration construct is used as a framework to
	 * render the repeating elements (siblings) and their contents.
	 * <p>
	 *
	 * jsp example:<code>
	 *  <logic:iterate indexId="index" id="item" name="sef"
	 *       property="repeatingMembersOf(/itemRecord/general/keywords/keyword)">
	 *  <c:set var="itemPath" scope="page" value=
	"/itemRecord/general/keywords/keyword_${keword+1}" />
	 *  </code>
	 *
	 * @param itemPath
	 *            path to individual nodes (including indexing)
	 * @param siblingPath
	 *            The path shared by each of the repeating nodes
	 * @param indexId
	 *            The symbol used to create the index in jsp that differentiates the
	 *            repeating nodes
	 * @return A jsp iteration construct in which the repeating nodes are rendered.
	 */
	protected Element getIteration(String itemPath, String siblingPath, String indexId) {
		Element iteration = DocumentHelper.createElement("logic__iterate").addAttribute("indexId", indexId)
				.addAttribute("id", "item").addAttribute("name", formBeanName)
				.addAttribute("property", "repeatingMembersOf(" + siblingPath + ")");

		Element itemPathAssignElement = iteration.addElement("c__set").addAttribute("var", "itemPath")
				.addAttribute("scope", "page").addAttribute("value", itemPath);
		return iteration;
	}

	/**
	 * Creates JSP to set the *id* for a particular node, and then to determine the
	 * display_state of that node from the collapseBean and assign it to a
	 * javascript variable. These elements all attached to provided baseDiv.
	 *
	 * @param baseDiv
	 *            Element to which created elements are attached.
	 */
	protected void insertRepeatingDisplaySetup(Element baseDiv) {
		...
		Element setId = DocumentHelper.createElement("c__set").addAttribute("var", "id").addAttribute("value",
				"${sf__pathToId(itemPath)}");
		baseDiv.add(setId);
		...
		Element setProp = DocumentHelper.createElement("jsp__setProperty").addAttribute("name", "collapseBean")
		...
		Element hiddenVar = DocumentHelper.createElement("input").addAttribute("type", "hidden")
				.addAttribute("name", "${id}_displayState").addAttribute("id", "${id}_displayState")
	...
	public Element newSiblingController() {
		...
		// new sibling control only displayed when there are already members (i.e.,
		// siblings!)
		...
		Element actionBox = newItemControl.addElement("div").addAttribute("style", "width:100px;text-align:center");

		Element style = actionBox.addElement("div").addAttribute("class", "action-button");
		Element newItemLink = style.addElement("a").addAttribute("href",
				"javascript:doNewElement(" + rhelper.jspQuotedString(itemPath) + ")");
	...
	/**
	 * Render jsp element for adding a new repeating element that will be shown when
	 * there are no existing values for this field in the instance document.
	 *
	...
	protected Element emptyRepeatingElement() {
		...
		Element newItemAction = controlElement.addElement("div").addAttribute("class", "action-button");
