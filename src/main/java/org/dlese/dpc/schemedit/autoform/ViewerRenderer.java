...
/**
 * Renders JSP for viewing metadata records within the DCS (as opposed to within
 * a metadata editor).
 * <p>
 *
...
public class ViewerRenderer extends RendererImpl {
	 ...
	 *
	 * @param xpath
	 *            xpath of inputElement
	 * @param schemaNode
	 *            schemaNode of element
	 * @param typeDef
	 *            typeDef of element
	...
	public Element getInputElement(String xpath, SchemaNode schemaNode, GlobalDef typeDef) {
		...
		if (typeDef == null) {
			// prtln("\t ... no typeDef found for " + normalizedXPath + " assuming built-in
			// and punting with Text Input");
			prtln("\t typeDef is NULL");
			return getStaticSimpleValue(xpath);
		}

		/*
		 * sanity check - we must have either a SIMPLE_TYPE, or a COMPLEX_TYPE with
		 * simpleContent to render an input Element
		 */
		...
		if (sh.isEnumerationType(typeDef)) {
			...
			// prtln ("getInputElement(): typeName: " + typeName + ", xpath: " + xpath + "
			// is an enumeration");
	...
	protected Element getStaticSimpleValue(String xpath) {
		...
		Element valueDiv = df.createElement("div").addAttribute("class", "static-value");
		Element valueElement = valueDiv.addElement("bean__write").addAttribute("name", formBeanName)
				.addAttribute("property", "valueOf(" + xpath + ")").addAttribute("filter", "false");
		return valueDiv;
	}
	...
	public Element getMultiSelectInput(String xpath) {
		...
		Element valueDiv = df.createElement("div").addAttribute("class", "static-enumerated");

		Element enums = valueDiv.addElement("bean__define").addAttribute("id", "enums")
				.addAttribute("name", formBeanName).addAttribute("property", "enumerationValuesOf(" + xpath + ")");

		Element iterator = valueDiv.addElement("c__forEach").addAttribute("var", "item")
				.addAttribute("items", "${enums}").addAttribute("varStatus", "status");
	 ...
	 *
	 * @param xpath
	 *            normalized xpath to the field for this label
	 * @param siblingPath
	 *            xpath to reach all siblings
	 * @param indexId
	 *            jsp variable used to generate indicies for repeating fields
	...
	/**
	 * creates a jsp tag (st:dcsViewEditMeLabel) that renders a label as a link that
	 * will open the metadata editor to this field.
	 * <p>
	 *
	 * If provided, siblingPath parameter includes indexing (e.g.,
	 * /record/general_${index+1}_) to make the proper link to an indexed field.
	 *
	 * @param xpath
	 *            NOT YET DOCUMENTED
	 * @param siblingPath
	...
	protected Element getEditMeTag(String xpath, String siblingPath, String indexId) {
		...
		Element editMeTag = df.createElement("st__dcsViewEditMeLabel")
				.addAttribute("reader", "${" + formBeanName + ".docReader}").addAttribute("xpath", tagPath);

		// the destination page in the editor depends on the baseRenderLevel attribute
		// of this framework, which
		// specifies whether the editor is split up into several pages (e.g., adn) or
		// rendered
	...
	public Element getRenderedField_2(String xpath, Label label, Element inputField) {
		...
		Element displayFieldTag = df.createElement("mde__displayField").addAttribute("xpath", xpath)
				.addAttribute("page", page).addAttribute("label", label.getText())
	 ...
	 * <p>
	 *
	 * NOTE: this probably shouldn't be here, since it is misnamed (it does not have
	 * anything to do with "input"). To fix this problem we have to change super
	 * classes to differentiate between renders that create edit inputs and those
	 * that simply display values.
	 *
	 * @param xpath
	 *            NOT YET DOCUMENTED
	 * @param label
	...
	public Element getRenderedField_1(String xpath, Label label, Element inputField) {
		...
		Element debugging = emptyTest.addElement("span").addAttribute("class", "element-error-msg");
		debugging.setText("missing value");

		Element notEmptyTest = hasValueTest(xpath);
		notEmptyTest.add(inputField);

		Element inputFieldCell = row.addElement("td").addAttribute("class", "input-cell");
	...
	public Element missingValueTest(String xpath) {
		...
		Element test = df.createElement("logic__equal").addAttribute("name", formBeanName)
				.addAttribute("property", "nodeIsMissingValue(" + xpath + ")").addAttribute("value", "true");
	...
	public Element hasValueTest(String xpath) {
		// test to see if child node (the multiSelect) is empty
		Element test = df.createElement("logic__notEqual").addAttribute("name", formBeanName)
				.addAttribute("property", "nodeIsMissingValue(" + xpath + ")").addAttribute("value", "true");
