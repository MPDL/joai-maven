 ...
 * <ul>
 * <li>Renderers - implement the recursive process of implenting MetdataEditor
 * Nodes, including methods to render input fields,
 * <li>RendererHelper - provides state information for the rendering process, as
 * well as a Factory for obtaining new renderers, and
...
public abstract class MdeNode {
	...
	public MdeNode(RendererImpl renderer) {
		...
		/*
		 * if (this.inputHelperFile != null) { prtln ("inputHelperFile: " +
		 * this.inputHelperFile); }
		 */
	}
	 ...
	 * <p>
	 *
	 * NOTE: apparently css class ("action-button") must be applyed to both the link
	 * and the text within to get consistent results in both IE and FireFox ...
	 *
	 * @param xpath
	...
	/**
	 * Gets the schema namespace (associated with
	 * "http://www.w3.org/2001/XMLSchema") for the root schema for the metadata
	 * framework for this Renderer.
	 */
	...
	/**
	 * Resolve the prefix of the qualified name using the typeDef as context.
	 * <p>
	 *
	 ...
	 * method determines the namespace for a locally defined prefix and returns the
	 * prefix corresponding to that namespace at the instance document level.
	 *
	 * @param name
	 *            NOT YET DOCUMENTED
	 * @param typeDef
	...
	protected String getQualifiedElementName(String name, GlobalDef typeDef) {
		...
		if (!NamespaceRegistry.isQualified(name)) {
			...
			/*
			 * IMPORTANT -- don't resolve built-ins (for now), since doing so will confuse
			 * the schemaReader such that it won't create a BuiltIn (if the schemaPrefix
			 * local to schemaReader is not that of global schemaPrefix). This is pretty
			 * ugly, and i BELIEVE that if BuiltIns were available via the globalRefMap
			 * under the schemaNamespaceUri, this would no longer be a problem ... but for
			 * now, just test that qualifiedName doesn't refer to a built in ....
			 */
	 ...
	 * <p>
	 *
	 * NOTE: i don't think this method is required at all. if the instance document
	 * is constructed correctly, the attributes are already qualified as needed and
	 * there is no need to mess with it any further ..
	 *
	 * @param name
	 *            NOT YET DOCUMENTED
	 * @param typeDef
	...
	protected String getQualifiedAttributeName(String name, Element element, GlobalDef typeDef) {
		...
		if (NamespaceRegistry.isQualified(name)) {
			...
			// get the namespace corresponding to the name's prefix - in the context of
			// provided typeDef
			qualifiedName = resolveQualifiedName(name, typeDef);
	...
	/**
	 * Creates Element for displaying labels, action controllers and collapseWidgets
	 * for elements that have no input field. Action controllers enable users to
	 * perform actions such as deleting a node or adding a new one.
	 *
	 * @param label
	 *            Description of the Parameter
	 * @param action
	...
	protected Element getRenderedNoInputField(Label label, Element action, Element inputHelper) {
		...
		Element table = df.createElement("table").addAttribute("class", "no-input-field-table");
		...
		if (action != null) {
			Element controllerCell = row.addElement("td").addAttribute("class", "action-box");
			controllerCell.add(action);
		}

		Element prompts = df.createElement("st__fieldPrompt").addAttribute("pathArg", xpath);

		Element cell = row.addElement("td").addAttribute("class", "action-box");
		if (this.isEditMode())
			cell.add(prompts);
		if (inputHelper != null)
	...
	/**
	 * Attach a debugging message to this element that can be viewed in the editing
	 * form.
	 *
	 * @param e
	 *            NOT YET DOCUMENTED
	 ...
	 *
	 * @param target
	 *            The Element to which the debugging element is attached
	 * @param displayText
	...
	protected void attachLabelDebugInfo(Element e, String s, String borderSpec) {
		if (rhelper.showLabelDebugInfo) {
			Element info = df.createElement("div").addAttribute("class", "label-debug-info");
			String debugInfo = s;
	 ...
	 *
	 * @param e
	 *            The feature to be added to the HtmlComment attribute
	 * @param s
	 *            The feature to be added to the HtmlComment attribute
	 */
	...
	/**
	 * Add an "id" attribute to the provided element, with a value derived from this
	 * node's "xpath".
	 *
	 * @param e
	 *            NOT YET DOCUMENTED
	...
	protected void attachElementId(Element e) {
		...
		e.addElement("c__set").addAttribute("var", "elementPath").addAttribute("value", xpath);
		e.addElement("c__set").addAttribute("var", "id").addAttribute("value", "${sf__pathToId(elementPath)}");
	}

	/**
	 * Creates element representing a mousable field label that will open or close
	 * its contents. Used to expand and collapse the hierarchical strucuture of the
	 * xml document
	 *
	 * @param baseDiv
	 *            Description of the Parameter
	 */
	/*
	 * <%-- define variable to hold the key/id for this element --%> <c:set var="id"
	 * value="${sf:pathToId('/news-oppsRecord/announcements')}" scope="page" /> <%--
	 * use jsp:setProperty to set the id prop of collapseBean prior to accessing
	 * rhelper --%> <jsp:setProperty name="collapseBean" property="id" value="${id}"
	 * /> <%-- set hidden var to currentState --%> <input type="hidden"
	 * name="${id}_displayState"
	 * value="${"+formBeanName+".collapseBean.displayState}" />
	 */
	protected void insertDisplaySetup(Element baseDiv) {
		if (isEditMode()) {
			Element elementPath = df.createElement("c__set").addAttribute("var", "elementPath").addAttribute("value",
					xpath);
			baseDiv.add(elementPath);

			Element setId = df.createElement("c__set").addAttribute("var", "id").addAttribute("value",
					"${sf__pathToId(elementPath)}");
			baseDiv.add(setId);

			Element setProp = df.createElement("jsp__setProperty").addAttribute("name", "collapseBean")
					.addAttribute("property", "id").addAttribute("value", "${id}");
			baseDiv.add(setProp);

			Element hiddenVar = df.createElement("input").addAttribute("type", "hidden")
					.addAttribute("id", "${id}_displayState").addAttribute("name", "${id}_displayState")
