...
import org.dlese.dpc.xml.schema.AttributeGroup;
 ...
 * Used as a superClass for other JSP-based renderers, such as
 * {@link org.dlese.dpc.schemedit.autoform.DleseEditorRenderer}.
 *
 * @author ostwald
 *
 */
public class MdeComplexType extends MdeNode {
	...
	private void render_edit_mode() {
		...
		// test for the existence of a parent, unless we are at the 0th level of the
		// page
		...
		/*
		 * prtln ("---------- Control Element --------------"); prtln
		 * (Dom4jUtils.prettyPrint(controlElement));
		 */
		...
		// this.attachElementDebugInfo(subElementsBox, "MdeComplexType - subElementsBox
		// - ${id}");
	...
	public void render() {
		...
		/*
		 * Label label = renderer.getComplexTypeLabel(xpath); Element controlElement =
		 * null;
		 */
	...
	/**
	 * Attach a specific control (e.g., "add element") the the controlElement of
	 * this node
	 *
	...
	private void attachControl(Element controlElement) {
		...
		if (hasRepeatingComplexSingletonChild()) {
			...
			if (getLevel() != 0) {
				/*
				 * add "choose" controller for the case where empty requiredMultiSelect is
				 * closed NOTE: this is needed only for collapsible editors, since when the
				 * parent is closed we need to indicate when a choice needs to be made. In
				 * non-collapsible editors the choices are always visible.
				 */
	...
	/**
	 * Render the subelements of specified proxyTypeDef, used to render derived
	 * models.
	 *
	 * @param proxyTypeDef
	 *            NOT YET DOCUMENTED
			...
			try {
				if (childType.equals("attribute")) {
					...
					for (Iterator a = attrGroup.getAttributes().iterator(); a.hasNext();) {
						...
						String attPath = xpath + "/@"
								+ getQualifiedAttributeName(attrName, attrElement, complexTypeDef);
						newRenderer(attPath, parent).renderAttribute();
					}
				}
				...
				else if (childType.equals("all") || childType.equals("sequence") || childType.equals("choice")) {
					...
					String childPath = xpath + "/" + NamespaceRegistry.makeQualifiedName(prefix, "any");
					...
					// throw new Exception ("render SubElements got unexpected childType: (" +
					// childType + ")");
	 ...
	 * renderSubElements so that the attributes are shown first (before subelements)
	 * in the form
	 *
	 * @param children
	 *            a list of elements, some of which may be attributes
	 * @return list re-ordered to put attributes at top of list
	 */
	protected List orderSubElements(List children) {

		/*
		 * prtln ("\n orderSubElements"); prtln ("\n Before Sort"); showSubElements
		 * (children);
		 */
		...
		/*
		 * prtln ("\n After Sort"); showSubElements (sortedChildren);
		 */
	 ...
	 * provides a prompt for user when the required multiselect boxes are empty and
	 * not visible
	 *
	...
	public Element getRequiredMultiSelectControl() {
		...
		Element actionElement = df.createElement("span").addAttribute("class", "action-button");

		// test to see if child node (the multiSelect) is empty
		Element isEmptyTest = actionElement.addElement("logic__equal").addAttribute("name", formBeanName)
				.addAttribute("property", "nodeIsEmpty(" + xpath + ")").addAttribute("value", "true");

		// test to see if the node is closed
		Element isOpenTest = isEmptyTest.addElement("c__if").addAttribute("test",
				"${not " + formBeanName + ".collapseBean.isOpen}");

		// "choose" controller link that toogles DisplayState of node
		Element chooseController = isOpenTest.addElement("a").addAttribute("href",
				"javascript__toggleDisplayState(" + RendererHelper.jspQuotedString("${id}") + ")");
	...
	protected Element getChoiceDeleteController(String itemPath, String elementName) {
		...
		Element deleteItemAction = df.createElement("span").addAttribute("class", "action-button");
		Element deleteItemLink = deleteItemAction.addElement("a").addAttribute("href",
				"javascript:doDeleteChoiceElement(" + RendererHelper.jspQuotedString(itemPath) + ")");
	...
	protected Element getRepeatComplexSingletonControl() {
		...
		Element newItemControl = df.createElement("logic__lessThan").addAttribute("name", formBeanName)
				.addAttribute("property", "childElementCountOf(" + xpath + ")").addAttribute("value", "1");

		Element newItemAction = newItemControl.addElement("div").addAttribute("class", "action-button");

		Element newItemLink = newItemAction.addElement("a").addAttribute("href", "javascript:doNewElement("
				+ RendererHelper.jspQuotedString(getRepeatingComplexSingletonChildPath()) + ")");
	...
	protected String getRepeatingComplexSingletonChildPath() {
		if (this.repeatingComplexSingletonChildPath == null) {
			...
			// schemaHelper.getRepeatingComplexSingletonChildName returns an empty string if
			// the schemaNode at
	...
	/**
	 * Creates a "box" (a decorated Div element) containing this element's id and
	 * display state.
	 *
	...
	protected Element getComplexTypeBox() {
		...
		/*
		 * make a collapsible box except for those with parents that are simpleOrComplex
		 * types with a requiredContentElements
		 */
