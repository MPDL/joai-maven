...
public class RoleManager {
	...
	/*
	 * quick and dirty isAuthorized methods to call from jsp. this is only a bandaid
	 * implementation!!
	 */
	public boolean isAuthorized(String operation, SessionBean sessionBean, String collection) {
		...
		try {
			...
			if (true) {
				...
				/*
				 * prtln ("is instance name dcc? " + instanceName.equals("dcc")); prtln
				 * ("is operation a delete record? " + operation.equals(DELETE_RECORD) +
				 * " --- *" + operation + "*"); prtln ("is the users role NOT admin? " +
				 * !userRole.equals(Roles.ADMIN_ROLE));
				 */
			}
			...
			if (instanceName.equals("dcc")) {
				NOTauth = ((operation.equals(DELETE_RECORD) || operation.equals(BATCH_DELETE))
			...
			if (operation.equals(EXPORT_COLLECTION) || operation.equals(SETTINGS) || operation.equals(STATUS_HISTORY))
				ISauth = user.hasRole(Roles.ADMIN_ROLE, collection);

			if (operation.equals(MANAGE_COLLECTIONS) || operation.equals(MANAGE))
			...
			/*
			 * String errorMsg =
			 * "WARNING: Authorization could not find user info - blindly allowing operation"
			 * ; prtln (errorMsg + " : " + t.getMessage()); return true;
			 */
