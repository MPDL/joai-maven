 ...
 * <li>a {@link org.dlese.dpc.schemedit.vocab.FieldInfoMap} for information such
 * as best practices and definitions</li>
...
public class MetaDataFramework implements Serializable {
	...
	/**
	 * Constructor for the MetaDataFramework object given the path to a config file
	 * for the particular framework (e.g., "adn"). The docRoot parameter enables
	 * expanding of partial paths into absolute paths.
	 *
		...
		if (this.schemaHelper != null && fieldInfoMap != null) {
			for (Iterator i = fieldInfoMap.getAllFieldInfo().iterator(); i.hasNext();) {
				...
				if (this.schemaHelper.getSchemaNode(path) == null) {
					errors.add("FieldsFile (" + reader.getSource() + ") has a BOGUS XPATH (" + path + ")");
				}
			}
		}
		return errors;
	...
	/**
	 * Gets the userInfo been for this framework, which specifies xpaths that can be
	 * populated with attributes from the SessionUser object, such as name,
	 ...
	 * by loadSchemaHelper)
	 *
	 * @param standardsManager
	 *            The new standardsManager value
	 */
	 ...
	 *
	 * @param r
	 *            The new bestPracticesLabel value
	 */
	 ...
	 *
	 * @param uri
	 *            The new workingSchemaURI value
	 */
	...
	public void loadSchemaHelper() throws Exception {
		...
		try {
			...
			// prtln ("after loading: schemaHelper.getRootElementName: " +
			// getSchemaHelper().getRootElementName());
			/*
			 * issues: 1 - for frameworks where we have to "assign" a namespace prefix to
			 * the rootElement (e.g. lead), we can't give the prefix in the config, but
			 * after schema is read, the prefix is known.
			 * 
			 * however, the editor has issues when we change the rootElementName here ....
			 * (document these issues ...). (NOTE: as it is now, supplying the prefix in
			 * this case chokes schemaHelper, but the metadata editor needs it.)
			 * 
			 * 2 - rootElementName is not required for unambigous cases, but if it is not
			 * known to framework- config, then we HAVE to assign it HERE so it will be
			...
			// if rootElementName was not specified in framework-config, obtain it from
			// schemaHelper
	...
	/*
	 * does this framework specify a uniqueUrl? currently, to specify a uniqueUrl,
	 * the config file must define a path must named "url" and the path must have a
	 * "valueType" attribute of "uniqueUrl".
	 */
	/**
	 ...
	 * element which is level 1). if the editor is split into pages representing the
	 * next level of the schema (as is the case with ADN) then the base level is 3
	 *
	 ...
	 */
	private Document populateFields(Document doc, List schemaPaths, String id, CollectionConfig collectionConfig,
			boolean copyRecord) throws Exception {
		...
		for (Iterator i = schemaPaths.iterator(); i.hasNext();) {
			...
			/*
			 * - if the path refers to a repeating node, we want to eliminate all but one of
			 * the siblings - if the path refers to a complexType (contains children rather
			 * than a textual element) we want to empty it --> we first remove all nodes at
			 * the specified xpath - if the specified node is a SimpleType, a new element
			 * will be created by docMap.smartPut - otherwise (the node ComplexType), the
			 * user creates a new one in the copied record
			 */
			...
			if (schemaNode == null)
				continue;
			if (schemaNode.getTypeDef().isComplexType() && !schemaNode.isDerivedTextOnlyModel()) {
				continue;
			}
			...
			if (pathName.equals("id")) {
				...
				// as of 2/2/05 we are using the simple yyyy-MM-dd date format as default format
				// for all date fields
			...
			} else if (schemaPath.valueType != null && schemaPath.valueType.equals("collectionInfo")
			...
			// if a value has not yet been assigned, set with default if possible, or derive
			// a
	...
	private Document populateUserFields(Document doc, User user) throws Exception {
		...
		/*
		 * Check for configuration errors NOTE: this field should be REQUIRED in
		...
		if (userInfo == null || !userInfo.autoPopulate.equals("Record Creation") || this.getSchemaHelper() == null)
			return doc;
	 ...
	 */
	/*
	 * public Document copyRecord(String xml, String id, CollectionConfig
	 * collectionConfig) throws Exception { return copyRecord (xml, id,
	 * collectionConfig, null); }
	 */
	/**
	 * Copy the given xml record, inserting new values as specified by the
	 ...
	 *            the collection configuration
	 * @param user
	 *            the current user (from which to populate user fields if so
	 *            configured)
	 * @return new Document
	 * @exception Exception
	 *                Description of the Exception
	 */
	public Document copyRecord(String xml, String id, CollectionConfig collectionConfig, User user) throws Exception {
		// prtln("copyRecord()");
		Document doc = null;
		try {
			...
			/*
			 * if we don't have a user, create a blank user so the user field values will be
			 * overwritten if framework is configured to autofill
			 */
			if (user == null)
				user = new User();
	 ...
	 *
	 * @param id
	 *            id value to be inserted into the id field for created record
	 * @return minimal instance Document for this framework.
	 * @exception Exception
	 */
	public Document makeMinimalRecord(String id) throws Exception {
	...
	public void renderEditorPages() throws Exception {
		...
		try {
			...
			if (baseLevel == 2) {
				...
				throw new Exception(
						"renderEditorPages failed because baseRenderLevel is out of range (" + baseLevel + ")");
			}
	 ...
	 *                Description of the Exception
	 */
	public Document getEditableDocument(File file) throws DocumentException {
	 ...
	 *                Description of the Exception
	 */
	public Document getEditableDocument(String filePath) throws DocumentException {
	 ...
	 *                NOT YET DOCUMENTED
	 */
	public String getWritableRecordXml(Document doc) throws DocumentException {
	 ...
	 * @return string representation of xml record suitable to be written to disk.
	 * @exception DocumentException
	 *                Description of the Exception
	 */
	public Document getWritableRecord(Document doc) throws DocumentException {
		try {
			...
			if (!rootElementName.equals(fwrkRootElementName)) {
				throw new Exception("Root element (\"" + rootElementName + "\") does not match schema (expected \""
						+ fwrkRootElementName + "\")");
			}
			...
			if (!getSchemaHelper().getNamespaceEnabled()) {
				...
				/*
				 * prtln ("\nAttributes before"); for (Iterator i =
				 * root.attributeIterator();i.hasNext();) { Attribute att = (Attribute)i.next();
				...
				if (targetNs != null && targetNs.trim().length() > 0) {
					...
					// localized records are likely to still have the schemaLocation attribute, but
					// without a prefix
					...
					// we don't want the "file:" part (scheme) in the noNamespaceSchemaLocation
					// value
					// getSchemaURI() must return a complete URI string because it is used by
					// schemaHelper to find
					// the actual schema file, so we chop off the "file:" part here rather than in
					// getSchemaURI()
					try {
	 ...
	 *
	 * @param doc
	 *            instanceDocument containing elements to be ordered
	 */
	 ...
	 * {@link #getWritableRecordXml(Document) getWritableRecordXml} and writes it to
	 * disk.
	 *
	 * @param doc
	 *            an editable Document
	 ...
	 *                Description of the Exception
	 */
	public void writeEditableDocument(Document doc, File file) throws DocumentException {
	...
	/*
	 * Ensure that required metadata elements are present, and take care of any
	 * framework-specific document preprocessing (e.g., smile_item element).
	 */
	/**
	 * NOT YET DOCUMENTED
	 *
	...
	public DocMap preprocessEditableDocument(DocMap docMap) {
		...
		/*
		 * all frameworks - make sure all required nodes(?? parents ??) are present in
		 * document ( kind of the opposite of PRUNE DOCUMENT ) - walk the schema
		 * instance doc (recursively) - for each element - get the schemaNode using the
		 * element path -- test that all required elements are in the instanceDoc
		 */
		try {
