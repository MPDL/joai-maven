...
/**
 * Extends SetInfo to provide DCS-specific information about sets (aka
 * collections), such as the number of valid records.
 *
 * @author Jonathan Ostwald
 */
public class DcsSetInfo extends SetInfo {
	int numDone = -1, numWorking = -1, numValid = -1, numNotValid = -1, numSyncErrors = -1, numFinalAndNotValid = -1;
	...
	public DcsSetInfo(SetInfo setInfo) {
		super(setInfo.getName(), setInfo.getSetSpec(), setInfo.getDescription(), setInfo.getEnabled(),
				setInfo.getDirectory(), setInfo.getFormat(), setInfo.getId());
		// constructor for SetInfo generates a new uniqueID for THIS instance, so we
		// have to explicitly
	...
	/**
	 * Adds DcsSetInfo properties to those of repository.SetInfo for this DcsSetInfo
	 * instance.
	 *
	 * @param rm
	 *            The new setInfoData value
	 */
	public void setSetInfoData(RepositoryManager rm) {
		...
		numFinalAndNotValid = rm.getIndex().getNumDocs("collection:0" + getSetSpec() + " NOT " + FIELD_NS
				+ "isValid:true" + " AND " + FIELD_NS + "isFinalStatus:true");
		this.numSyncErrors = rm.getIndex()
				.getNumDocs("collection:0" + getSetSpec() + " AND " + FIELD_NS + "hasSyncError:true");
	}
