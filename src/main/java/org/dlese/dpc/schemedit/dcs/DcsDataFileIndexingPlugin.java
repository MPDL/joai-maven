 ...
 * <p>
 *
 * Status information is obtained from a
 * {@link org.dlese.dpc.schemedit.dcs.DcsDataRecord} via the
 * {@link org.dlese.dpc.schemedit.dcs.DcsDataManager}.
 * <p>
 *
...
public class DcsDataFileIndexingPlugin extends ServletContextFileIndexingPlugin {
	 ...
	 * @param docType
	 *            The feature to be added to the Fields attribute
	 * @param docGroup
	 *            The feature to be added to the Fields attribute
	 * @exception Exception
	 *                If an error occurs
	 */
	public final void addFields(File file, org.apache.lucene.document.Document newDoc,
			org.apache.lucene.document.Document existingDoc, String docType, String docGroup) throws Exception {
		...
		/*
		 * System.out.println("dcsDataFile: " +
		 * dcsDataRecord.getSource().getAbsolutePath() + " myDocType: " + docType +
		 * " myCollection: " + docGroup);
		 */
		...
		newDoc.add(new Field(FIELD_NS + "lastEditor", lastEditor, Field.Store.YES, Field.Index.NOT_ANALYZED));

		// lastEditorName is used to sort search results, so it must be "NOT_ANALYZED"
		newDoc.add(new Field(FIELD_NS + "lastEditorName", getFullName(lastEditor), Field.Store.YES,
				Field.Index.NOT_ANALYZED));

		newDoc.add(
				new Field(FIELD_NS + "isValid", dcsDataRecord.getIsValid(), Field.Store.YES, Field.Index.NOT_ANALYZED));

		newDoc.add(
				new Field(FIELD_NS + "status", dcsDataRecord.getStatus(), Field.Store.YES, Field.Index.NOT_ANALYZED));

		newDoc.add(new Field(FIELD_NS + "statusLabel", dcsDataRecord.getStatusLabel(), Field.Store.YES,
				Field.Index.NOT_ANALYZED));

		String isFinalStatus = (dcsDataRecord.isFinalStatus() ? "true" : "false");
		newDoc.add(new Field(FIELD_NS + "isFinalStatus", isFinalStatus, Field.Store.YES, Field.Index.NOT_ANALYZED));

		newDoc.add(new Field(FIELD_NS + "statusNote", dcsDataRecord.getStatusNote(), Field.Store.YES,
				Field.Index.ANALYZED));

		newDoc.add(new Field(FIELD_NS + "hasSyncError", dcsDataRecord.getHasSyncError(), Field.Store.YES,
				Field.Index.NOT_ANALYZED));

		newDoc.add(new Field(FIELD_NS + "ndrHandle", dcsDataRecord.getNdrHandle(), Field.Store.YES,
				Field.Index.NOT_ANALYZED));

		Date lastTouchDate = getLastTouchDate(dcsDataRecord, file);
		newDoc.add(new Field(FIELD_NS + "lastTouchDate", DateField.dateToString(lastTouchDate), Field.Store.YES,
				Field.Index.NOT_ANALYZED));
		...
		for (Iterator i = dcsDataRecord.getEntryList().iterator(); i.hasNext();) {
			...
			newDoc.add(new Field(FIELD_NS + "statusEntryLabel", dcsDataRecord.getStatusLabel(statusEntry.getStatus()),
					Field.Store.NO, Field.Index.NOT_ANALYZED));

			newDoc.add(new Field(FIELD_NS + "statusEntryNote", statusEntry.getStatusNote(), Field.Store.YES,
					Field.Index.ANALYZED));

			newDoc.add(new Field(FIELD_NS + "statusEntryChangeDate", DateField.dateToString(statusEntry.getDate()),
					Field.Store.NO, Field.Index.NOT_ANALYZED));

			newDoc.add(new Field(FIELD_NS + "statusEntryEditor", statusEntry.getEditor(), Field.Store.NO,
					Field.Index.NOT_ANALYZED));

			newDoc.add(new Field(FIELD_NS + "statusEntryEditorName", getFullName(statusEntry.getEditor()),
					Field.Store.NO, Field.Index.ANALYZED));

			// entries processed in reverse chron order
			if (!i.hasNext()) {
				// we're looking at the first status entry
				newDoc.add(new Field(FIELD_NS + "recordCreationDate", DateField.dateToString(statusEntry.getDate()),
						Field.Store.YES, Field.Index.NOT_ANALYZED));

				String recordCreator = statusEntry.getEditor().trim();
				if (recordCreator.length() == 0)
					recordCreator = Constants.UNKNOWN_EDITOR;
				newDoc.add(new Field(FIELD_NS + "recordCreator", recordCreator, Field.Store.YES,
						Field.Index.NOT_ANALYZED));
			}
	...
	/**
	 * calculate a lastTouchDate for the DcsDataRecord
	 * <p>
	 *
	 * New records will not have a lastTouchDate, so we give it a default date for
	 * now so it may be indexed. The default date uses the lastModified time of the
	 * indexed file, and if this is not found, then the "epoch" date is used.
	 *
	 * @param file
	 *            NOT YET DOCUMENTED
