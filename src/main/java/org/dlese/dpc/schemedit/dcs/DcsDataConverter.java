...
/**
 * Converts old style DcsData records (that only mantained a single-entry) into
 * history-aware records.
 *
 * @author ostwald
 *         <p>
 *         $Id $
...
public class DcsDataConverter {
	...
	/**
	 * DcsDataConverter Constructor. The dcsDataRecords param points to the root
	 * directory containing dcs_data records. the subdirectories are named for the
	 * metadata formats of the directories they contain, which in turn hold
	 * directories for each collection of that format.
	 *
	 * 
	 * @param path
	 *            directory holding default record and records directory
	 */
	...
	/**
	 * Convert a directory of dcs_data files into new format by calling
	 * <b>convertFile </b> on each file. Idempotent: will only convert old-style
	 * records and leaves new-style records unchanged.
	 */
	public void convert() {
	 ...
	 * @exception Exception
	 *                thrown if file cannot be converted or written to disk after
	 *                conversion
	 */
	public boolean convertFile(File file) throws Exception {
		...
		/*
		 * tmpRec.setLastEditor(lastEditor); tmpRec.setStatus(status);
	...
	public static void main(String[] args) {
		...
		/*
		 * DcsDataConverter c = new DcsDataConverter(path); c.convert();
		 */
		/*
		 * File file = new File ("/devel/ostwald/projects/tmp/Suggestor-tmp/rec1.xml");
		 * try { c.convertFile (file); } catch (Exception e) { prtln (e.getMessage()); }
		 */
	}
