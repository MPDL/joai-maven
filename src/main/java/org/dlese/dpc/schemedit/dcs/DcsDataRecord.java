 ...
 * A DcsDataRecord reads from and writes to a file on disk that is named the
 * same as it's "source" item-level record, but in a different directory.
 * <P>
 *
 * NOTE: the Document attribute should be implemented as a
 * {@link org.dlese.dpc.schemedit.DocMap}!!
 *
 * @author ostwald
 */
public class DcsDataRecord implements Serializable {
	 ...
	 *
	 * @param dcsDataFile
	 *            file on disk containing xml source
	 * @param framework
	 *            this record's framework
	 * @param collectionConfig
	 *            this record's collection
	 * @param dcsDataManager
	 *            manager for all dcsDataRecords
	 */
	public DcsDataRecord(File dcsDataFile, MetaDataFramework framework, CollectionConfig collectionConfig,
	...
	/**
	 * Returns true if this record has "Final" status. A record has final status if
	 * the value of the status element in the current status entry has the form:
	 * "_|-final-{collection-key}-|_", which is referred to as the final status
	 * VALUE. The final status LABEL is a human-readable string that is configured
	 * in the collection configuration and stored internally in the
	 * collectionConfig.
	 * <p>
	 * This method returns true if the status value has the pattern
	 * "_|-final-{*}-|_" and as a side effect calls "normalize" to ensure that the
	 * collection key part of the status value refers to the correct collection.
	 *
	 ...
	 * status label also have the correct status VALUE, and 2) records having a
	 * final status refer to the correct collection.
	 */
	protected void normalizeStatus() {
		if (collectionConfig == null) {
			// prtln ("WARNING: unable to normalize status for " + getId() + " because no
			// collectionConfig is available");
			return;
		}

		/*
		 * make sure that this record's label and value are correct - is it a final
		 * status? - are both the value and label correct?
		 */
		...
		if (StatusFlags.isFinalStatusValue(myStatus) && !myStatus.equals(finalStatusValue)) {
	 ...
	 *
	 * @param status
	 *            new status for this record
	 * @param statusNote
	 *            new status note
	 * @param editor
	 *            user performing status update
	 * @exception Exception
	 *                if status cannot be updated
	 */
	public synchronized void updateStatus(String status, String statusNote, String editor) throws Exception {
	 ...
	 *
	 * @param statusEntry
	 *            The feature to be added to the StatusEntry attribute
	 */
	 ...
	 * @param statusEntry
	 *            new statusEntry
	 * @param retainChangeDate
	 *            if true, change date is not altered
	 * @exception Exception
	 *                if unable to update
	 */
	public synchronized void updateStatus(StatusEntry statusEntry, boolean retainChangeDate) throws Exception {
		if (!retainChangeDate) {
	...
	/**
	 * Returns List of managed suggestions records. Record entry list is sorted and
	 * cashed. When new entryList are added, entryList is set to null so it will be
	 * regenerated
	 *
	...
	/**
	 * Removes statusEntry corresponding to provided changeDate, and then adds new
	 * statusEntry
	 *
	 * @param changeDate
	 *            key used to locate existing status entry
	 ...
	 * @exception Exception
	 *                if unable to add status entry (no exception thrown if entry to
	 ...
	 * Returns true
	 *
	 * @return true if the validitity of the metadata record for this dcsDataRecord
	 *         is unknown
	 */
	 ...
	 *
	 * @param report
	 *            The new validationReport value
	 */
	...
	/**
	 * Gets the lastTouchDate attribute of the DcsDataRecord object, reflecting the
	 * last time the metadata was changed.
	 *
	...
	/**
	 * Gets the lastEditor attribute of the DcsDataRecord object, reflecting the
	 * last time the metadata was changed.
	 *
	 ...
	 *
	 * @param mdpHandle
	 *            The new metadataProviderHandle value
	 */
	...
	/**
	 * Clear any errors associated with External repositories, such as the NDR. This
	 * method should be called before each write.
	 */
	 ...
	 *
	 * @exception Exception
	 *                If record cannot be written to disk.
	 */
	...
	/**
	 * gets a single Node satisfying give XPath. if more than one Node is found, the
	 * first is returned (and a msg is printed)
	 *
	 * @param xpath
	 *            an XPath
	 * @return a dom4j Node
