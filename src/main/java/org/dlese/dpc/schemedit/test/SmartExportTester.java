	...
	public void export() {
		...
		for (int i = 0; i < srcFiles.length; i++) {
			...
			try {
				...
				/*
				 * if (false) { String xml = XSLTransformer.transformFile (srcFile,
				 * transformer); prtln ("transformed xml:\n" + xml); Files.writeFile(xml,
				 * exportedFile); }
				 */
				...
				/*
				 * } catch (IOException ioe) { numNotExported++; errorMsg = "readFile error: " +
				 * ioe.getMessage(); prtln(errorMsg); // throw new Exception (errorMsg);
				 */
			} catch (Exception e) {
				numNotExported++;
			...
			/*
			 * } catch (Throwable we) { numNotExported++; errorMsg = "writeFile error: " +
			 * we.getMessage(); prtln(errorMsg); // throw new Exception (errorMsg); }
			 */
		}
