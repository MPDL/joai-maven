...
import org.dlese.dpc.xml.schema.XSDatatypeManager;
...
public class SimpleSchemaHelperTester {
	...
	void metsTester() {
		...
		/*
		 * List nodes = SchemaUtils.getDerivedTextOnlyNodes(this.sh); prtln
		 * ("Derived Text Only Nodes"); for (Iterator i=nodes.iterator();i.hasNext();) {
		 * SchemaNode schemaNode = (SchemaNode)i.next(); // prtln
		 * (schemaNode.getXpath()); prtln (schemaNode.toString()); }
		 */
	}
	...
	/**
	 * Assumptions - RootElement has ComplexType - If a globalElement has a member
	 * name of another globalElement, it CAN't be the root.
	 * 
	 * Algorithm 1 - find the globalElements defined in the targetNameSpace (they
	 * must have a compositor?) 2 - for each ComplexType in the GlobalDefMap, if the
	 * compositor contains a globalMember having the name of a globalElement, that
	 * globalElement is eliminated from consideration 3 - if there is still more
	 * than one root candidate we are in trouble ...
	 *
	...
	void findRootElement() {
		...
		for (Iterator i = globalElements.iterator(); i.hasNext();) {
			...
			prtln("\n-------\nglobalElement: " + globalElement.getQualifiedInstanceName() + "typeName: " + typeName);
	...
	void doCdNodeProbes() {
		...
		nodeProbe("/cd:cd/@tp:rating");
		/*
		 * nodeProbe ("/cd:cd/cd:tape/@newAttr"); nodeProbe
		 * ("/cd:cd/cd:tape/@tp:newAttr"); nodeProbe ("/cd:cd/cd:tape/@newAttr");
