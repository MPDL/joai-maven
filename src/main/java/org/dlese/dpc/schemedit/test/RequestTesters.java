...
public class RequestTesters {
	...
	public static void enumTester() {
		 ...
		 * for (int i=0;i<types.length;i++) prtln ( types[i].getTag() );
		 */
	...
	/*
	 * aggregator: 2200/test.20070806193858997T member (agent):
	 * 2200/test.20070806194051429T
	 */
	...
	public static void deleteMdp() throws Exception {
		...
		/*
		 * DeleteRequest request = new DeleteRequest(mdpHandle, true); request.submit();
		 */
	...
	public static void modifyMetadata() throws Exception {
		...
		try {
			...
			// was modifyMetadata
			// request.addCommand ("relationship", "metadataFor",
			// "2200/test.20080825134906293T", "add");
			request.submit();
		} catch (Throwable t) {
			prtln(t.getMessage());
			t.printStackTrace();
	...
	/**
	 * NOTE: this does not work for dlese.org metadata: no metadatadata is found
	 * that is metadataFor 2200/NSDL_Recommended_Resources_Collection
	 */
	public static void getInfoLink() throws Exception {
		...
		/*
		 * MetadataReader md = new MetadataReader (mdHandle); String mdpHandle =
		 * md.getRelationship("metadataProvidedBy");
		 * 
		 * MetadataProviderReader mdp = new MetadataProviderReader (mdpHandle); String
		 * aggHandle = mdp.getRelationship("aggregatedBy");
		 * 
		 * FindRequest request = new FindRequest(NDRConstants.NDRObjectType.METADATA);
		 * request.addCommand("relationship", "metadataFor", aggHandle); String
		 * aggMetadataHandle = request.getResultHandle(); prtln
		 * ("Super Collection Metadata handle: " + aggMetadataHandle); MetadataReader
		 * aggMetadata = new MetadataReader(aggMetadataHandle);
		 * 
		 * // finally, grab the itemId from the aggregator's MD String link =
		 * aggMetadata.getProperty("itemId");
		 */
	...
	public static void modifyMetadataProvider() throws Exception {
		...
		/*
		 * Element empty = DocumentHelper.createElement("empty"); Element sd =
		 * DocumentHelper.createElement("serviceDescription");
		 * sd.add(DocumentHelper.createNamespace("dc",
		 * "http://purl.org/dc/elements/1.1/"));
		 */

		ModifyMetadataProviderRequest request = new ModifyMetadataProviderRequest(mdpHandle);
		/*
		 * request.addQualifiedCommand (NDRConstants.DLESE_NAMESPACE, "property",
		 * "collectionName", "Zombie MDP 2"); request.addDataStreamCmd ("dcs_data",
		 * empty); request.addDataStreamCmd ("dlese_collect", empty);
	...
	public static void listMembers() throws Exception {
		...
		try {
			/*
			 * ListMembersRequest request = new ListMembersRequest(mdpHandle); List
			 * mdHandles = request.getResultHandles();
			 */
			// request.submit();
			...
			/*
			 * for (Iterator i=mdHandles.iterator();i.hasNext();) prtln ("\t" +
			 * (String)i.next());
			 */
		} catch (Throwable t) {
			prtln(t.getMessage());
	...
	public static void listAgents() throws Exception {
		...
		try {
			 ...
			 * List mdHandles = new ListMembersRequest(mdpHandle).getResultHandles();
			 * 
			 * if (mdHandles == null) { prtln ("mdHandles == NULL!"); return; } prtln
			 * (mdHandles.size() + " handles found"); for (Iterator
			 * i=mdHandles.iterator();i.hasNext();) prtln ("\t" + (String)i.next());
			 */
		} catch (Throwable t) {
			prtln(t.getMessage());
			t.printStackTrace();
	...
	public static void lister() throws Exception {
		...
		/*
		 * request.setObjectType (NDRConstants.NDRObjectType.AGGREGATOR); String
		 * agenthandle = "2200/test.20070806194051429T"; request.addCommand
		 * ("relationship", "aggregatorFor", agenthandle);
		 */
		request.submit();
	}
	...
	public static void find() throws Exception {
		// String url =
		// "http://www.dlese.org/dds/histogram.do?group=subject&key=eserev";
	...
	/**
	 * to find non-DLESE MDP, we have to start with an aggregator and then find the
	 * MDP that is aggregated by it.
	 */
	...
	public static void addResource() throws Exception {
		...
		// request.setIdentifier ("http://www.dlsciences.org" + "/" +
		// org.dlese.dpc.schemedit.SchemEditUtils.getUniqueId());
		...
		// "2200/NSDL_Collection_of_Resources");
		// request.report();
		request.submit();
	}
	...
	/*
	 * 2200/test.20070621192337930T 2200/test.20070711172802573T
	...
	public static void modifyAggregator() throws Exception {
		...
		// request.addCommand("relationship", "memberOf",
		// NDRConstants.COLLECTION_OF_COLLECTIONS_AGGREGATOR);
	...
	public static void modifyResource() throws Exception {
		...
		// request.addCommand("relationship", "memberOf", "2200/20080317171343451T",
		// "add");
	...
	public static void isNDRCollectionMetadataProvider() throws Exception {
		...
		if (NdrUtils.isNDRCollectionMetadataProvider(mdpHandle))
			...
			prtln("NO");
		/*
		 * try { MetadataProviderReader mdpReader = new MetadataProviderReader
		 * (mdpHandle); String aggHandle = mdpReader.getRelationship("aggregatedBy");
		 * AggregatorReader aggReader = new AggregatorReader (aggHandle); String
		 * resHandle = aggReader.getRelationship("associatedWith");
		 * 
		 * FindRequest request = new FindRequest (NDRConstants.NDRObjectType.METADATA);
		 * request.addCommand ("relationship", "metadataFor", aggHandle);
		 * request.addCommand ("relationship", "metadataFor", resHandle);
		 * 
		 * if (request.getResultHandle() == null) prtln ("NO"); else prtln ("YES"); }
	...
	public static void authorizeToChangeMdp() throws Exception {
		...
		// request.addCommand("property", "foo", "foo value (settable by dlese.test
		// agent but not ncs agent)");
