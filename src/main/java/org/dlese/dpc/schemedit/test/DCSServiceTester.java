...
public class DCSServiceTester {
	...
	/**
	 * Perform GetId via the
	 * {@link org.dlese.dpc.schemedit.WebServiceClient#doGetId(String)} helper
	 * method
	 *
	 * @param collection
	 *            Description of the Parameter
	...
	/**
	 * returns xmlRecord with namespace info removed for the purpose of testing
	 * error handling
	 */
	...
	/**
	 * Performs a PutRecord request using file on disk, and invoking the PutRecord
	 * service INDIRECTLY (via WebServiceClient doPutRecord).
	 */
	 ...
	 */
	/*
	 * private static String stuffId (String recordXml, String format, String id)
	 * throws Exception { // create a localized Document from the xmlString Document
	 * doc = Dom4jUtils.getXmlDocument (recordXml); String rootElementName =
	 * doc.getRootElement().getName(); String nameSpaceInfo =
	 * Dom4jUtils.getNameSpaceInfo(doc, rootElementName); doc =
	 * Dom4jUtils.localizeXml(doc, rootElementName); String adnIdPath =
	 * "/itemRecord/metaMetadata/catalogEntries/catalog/@entry"; Node idNode =
	 * doc.selectSingleNode(adnIdPath); if (idNode == null) { prtln
	 * (Dom4jUtils.prettyPrint(doc)); throw new Exception ("idNode not found at " +
	 * adnIdPath); } idNode.setText (id); doc = Dom4jUtils.delocalizeXml(doc,
	 * rootElementName, nameSpaceInfo); if (doc == null) throw new Exception
	 * ("not able to delocalize xml"); // prtln (Dom4jUtils.prettyPrint(doc));
	 * return doc.asXML(); }
	 */
