...
public class FrameworkTester {
	...
	static String getWritableRecordXmlFoo(Document doc, MetaDataFramework framework) throws DocumentException {
		try {
			...
			// Document delocalizedDoc = Dom4jUtils.delocalizeXml(doc, rootElementName,
			// framework.getNameSpaceInfo());
	...
	static Document getEditableDocumentFoo(String filePath, MetaDataFramework framework) throws DocumentException {
		...
		try {
			...
			if (!root.getName().equals(framework.getRootElementName())) {
				throw new Exception("Root element (\"" + root.getName() + "\") does not match schema (expected \""
			...
			if (enforceSchemaLocation && !docSchemaLocation.equals(schemaLocation)) {
			...
			return doc;
			/*
			 * } catch (IOException ioe) { throw new DocumentException (ioe.getMessage()); }
			 * catch (DocumentException de) { throw new DocumentException (de.getMessage());
			 */
		} catch (Exception e) {
			e.printStackTrace();
	...
	void ensureMinimalDocumentTester() throws Exception {
		// String itemPath =
		// "/Users/ostwald/devel/dcs-records/2009_06_10-records/smile_item9/1246053008647/S9-000-000-000-005.xml";
		// String itemPath =
		// "/Users/ostwald/devel/dcs-records/2009_06_10-records/library_dc/1246122420417/LFOO-000-000-000-001.xml";
		...
		/*
		 * EnsureMinimalDocument ensurer = new EnsureMinimalDocument (doc,
		 * tester.framework.getSchemaHelper()); ensurer.process (doc,
	...
	/*
	 * Document ensureRequiredPaths (Document instanceDoc) { Element root =
	 * instanceDoc.getRootElement(); ensureRequiredPaths (root); return instanceDoc;
	 * }
	 */
	...
	/*
	 * private void ensureRequiredPaths(Element e) { // prtln
	 * ("SchemaHelper.pruneTree(" + e.getPath() + ")");
	 * 
	 * SchemaNode schemaNode = getSchemaNode(e.getPath());
	 * 
	 * // Ignore ABSTRACT ELEMENTS if (schemaNode == null) { String msg =
	 * "WARNING: ensureRequiredPaths could not find schemaNode for" + e.getPath();
	 * prtln(msg); return; }
	 * 
	 ...
	 * 
	 * if (compositor != null) { List leafMemberNames =
	 * compositor.getLeafMemberNames(); List children
	 * 
	 * List attributes = e.attributes(); for (int i = attributes.size() - 1; i > -1;
	 * i--) { Attribute attr = (Attribute) attributes.get(i); // String path =
	 * attr.getPath(); String path = e.getPath() + "/@" + attr.getQualifiedName();
	 * SchemaNode attrNode = getSchemaNode(path); if (attrNode == null) { //
	 * prtln("pruneTree: schemaNode not found for attribute " + path); continue; }
	 * 
	 * if (!attrNode.isRequired()) { // prtln("pruneTree: removing attribute " +
	 * attr.getName() + " from " + path); e.remove(attr); } }
	 * 
	 * List children = e.elements(); for (int i = children.size() - 1; i > -1; i--)
	 * { Element child = (Element) children.get(i); String path = child.getPath();
	 * SchemaNode childSchemaNode = getSchemaNode(path); if (childSchemaNode ==
	 * null) { prtln("pruneTree: schemaNode not found for element at " + path);
	 * continue; }
	 * 
	 * if (!childSchemaNode.isRequired()) { // prtln("removing " +
	 * childSchemaNode.getXpath()); if (!e.remove(child)) {
	 * prtln("pruneTree: failed to remove element " + child.getName() + " from " +
	 * path); } continue; } if (isChoiceElement(childSchemaNode) ||
	 * this.isAnyTypeElement(path)) { // prtln("removing choice element at " +
	 * path); if (!e.remove(child)) { prtln("pruneTree: failed to remove " +
	 * child.getName() + " from " + path); } continue; } else { pruneTree(child); }
	 * } }
	 */
