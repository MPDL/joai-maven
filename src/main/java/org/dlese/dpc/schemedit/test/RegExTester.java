...
public class RegExTester {
	...
	public static void main(String[] args) {
		...
		// String xpath =
		// "/concept/relations/hierarchy_${hierarchyIndex0+1}_/hierarchy_${hierarchyIndex2+1}_/hierarch";
		...
		/*
		 * Pattern p = Pattern.compile(".*?/content/text"); String s =
		 * "/as/asdf/content/text";
		 */

		// Pattern p = Pattern.compile("hierarchy_\\$\\{hierarchyIndex[\d]*.*?\\}");
		/*
		 * Pattern p = Pattern.compile("hierarchy_\\$\\{hierarchyIndex([0-9]*)"); String
		 * s =
		 * "/concept/relations/hierarchy_${hierarchyIndex0+1}_/hierarchy_${hierarchyIndex1+1}_hierarchy";
		 * boolean found = false; Matcher m = p.matcher(s); while (m.find()) { found =
		 * true; prtln ("FOUND: " + m.end() + ", " + s.length()); prtln ("\t group: " +
		 * m.group(1)); } if (!found) prtln ("NOT found");
		 */
