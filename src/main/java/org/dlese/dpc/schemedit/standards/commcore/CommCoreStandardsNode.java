 ...
 * @see Standard
 */
public class CommCoreStandardsNode implements StandardsNode {
	...
	private CommCoreStandardsNode parent = null;
	...
	public List getAncestors() {
		if (this.ancestors == null) {
			...
			CommCoreStandardsNode myParent = parent;
	 ...
	 */
	public CommCoreStandardsNode getChild(String id) {
		return (CommCoreStandardsNode) this.childMap.get(id);
	}
	 ...
	 *
	 * @param node
	 *            The feature to be added to the SubNode attribute
	 */
	public void addSubNode(StandardsNode node) {
		subList.add(node);
		childMap.put(((CommCoreStandardsNode) node).getId(), node);
	}
	...
	/**
	 * Gets the concatenated text of this standard and its ancestors, prepended with
	 * a gradeRange indicator
	 *
	...
	public List getLineage() {
		if (this.lineage == null) {
			...
			for (Iterator i = this.getAncestors().iterator(); i.hasNext();) {
				CommCoreStandardsNode node = (CommCoreStandardsNode) i.next();
