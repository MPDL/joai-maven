...
public class AsnSuggestionServiceHelper extends CATServiceHelper {
	...
	public AsnStandardsDocument getStandardsDocument() {
		if (this.standardsDocument == null) {
			...
			try {
				this.standardsDocument = StandardsRegistry.getInstance().getStandardsDocument(this.getDefaultDoc());
			} catch (Throwable t) {
	...
	/**
	 * Write the list of selected standards, as well as the currentStandards Doc to
	 * the SelectedStandardsBean
	 */
	public void updateSelectedStandardsBean() {
		if (this.selectedStandardsBean == null)
			this.selectedStandardsBean = new SelectedStandardsBean(this.getSelectedStandards(), this.getCurrentDoc());
		else
	...
	public void setStandardsDocument(String key) throws Exception {
		...
		/*
		 * prtln("\nAfter setting AsnDocument"); prtln("\t helper author is " +
		 * this.getAuthor()); prtln("\t mgr author is " + this.getAuthor());
		 */
	}
