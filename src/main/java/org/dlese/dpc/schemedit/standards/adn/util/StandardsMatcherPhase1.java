...
/**
 * Tool to help match asf and adn standards, and to identify standards that
 * cannot be matched. The output of reports generated here can be manipulated
 * into tab-delimited form and then imported into spreadsheets with the goal of
 * creating a table in which the mappings between asfId and adn records is made.
 *
...
public class StandardsMatcherPhase1 {
	...
	/**
	 * Constructor for the StandardsMatcherPhase1 object - builds two data
	 * structures that are then used to generate reports: 1 - "idMap" -- which is
	 * initialized to hold an empty entry for each ASN id (the entry will later be
	 * filled) 2 - "matchKeyIdMap" -- which holds an entry (an ASN ID) for each
	...
	public StandardsMatcherPhase1(String standardsDocPath, int level) {
		...
		try {
			...
			// idMap is keyed by asf identifier, contains entries for standards at specified
			// level
			idMap = new HashMap();
	...
	void showASNMatchKeys() {
		/*
		 * prtln ("ASN Match Keys"); for (Iterator
		 * i=matchKeyIdMap.keySet().iterator();i.hasNext();) { String matchKey =
		 * (String)i.next(); String id = (String) matchKeyIdMap.get(matchKey);
		 * AsnStandard std = asnDoc.getStandard(id); // prtln ("\n" + std.toString());
	...
	/**
	 * Try to find an ASN standard for each ADN 4th level standard. Populates
	 * following data structures:
	 * <li>unmatchedADN - the adn standards for which no asf standard could be
	 * found. a mapping from adnMatchKey to adnText
	 * <li>idMap - (previously populated by creating an entry for each leaf ASN
	 * node) - mapping from asfId. When a match is made, the idMap entry is set to
	 * the adn text.
	 * <li>matched - list of matched keys (the values can be found in idMap)
	 * <li>unmatchedASN - list of asfKeys that were not matched. These structures
	 * are printed out in "report"
	 */
	...
	void report() {
		...
		prtln("\n----------------------------------------------------------------------");
		prtln(adnNthLevelStandards.size() + " standards tried -- " + matched.size() + " found, " + unmatchedADN.size()
				+ " not found");
