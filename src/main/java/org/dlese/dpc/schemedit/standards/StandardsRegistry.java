 ...
 * the same standards doc.
 * <p>
 *
 * The StandardsRegistry contains {@link AsnDocInfo} instances to represent each
 * loaded Standards Doc. The hierarchical structure of each ASN Document is
 * represented as an {@link StandardsDocument} instance. The registry makes use
 * of a {@link TreeCache} to cache StandardTrees and loads them when needed, so
 * that many standards documents can be managed.
 *
...
public class StandardsRegistry {
	 ...
	 * recursively
	 *
	 * @param standardsDirectory
	 *            a directory containing standards Documents
	 * @return A list of AsnDocInfo instances representing loaded docs
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	public List load(String standardsDirectory) throws Exception {
		...
		for (int i = 0; i < files.length && i < numToRegister; i++) {
			...
			try {
				...
				prtln((i + 1) + "/" + files.length + ": processed " + files[i].getName() + " (" + docInfo.getKey()
						+ ")");
			} catch (Exception e) {
	 ...
	 * @exception Exception
	 *                if unable to register document.
	 */
	public AsnDocInfo register(String path) throws Exception {
		...
		try {
			...
			/*
			 * PROBLEM! - this method of identifying potentially older versions does not
			 * work because the call to "docMap.get (key)" will only find existing of the
			 * SAME YEAR!
			 */
	 ...
	 *
	 * @param docInfo
	 *            the docInfo for the document to be deleted
	 */
	 ...
	 *
	 * @param key
	 *            key designating the doc to unregister
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	 ...
	 * Asn Identifier (i.e, purl) for that Standards Doc.
	 *
	 * @param docId
	...
	public static void main(String[] args) throws Exception {
		...
		/*
		 * prtln ("\nRegistry Contents"); for (Iterator i = docs.iterator();
		 * i.hasNext(); ) { AsnDocInfo docInfo = (AsnDocInfo) i.next(); if (docInfo !=
		 * null) docInfo.report(); else prtln ("docInfo not found!"); }
		 */
		String asnId = "http://purl.org/ASN/resources/S1005D1Ex";
		prtln("\nfinding node for " + asnId);
		/*
		 * AsnStandardsNode node = reg.getStandardsNode(asnId); if (node == null)
		 * prtln("not found"); else { prtln("found: " + node.getId()); AsnStandard std =
		 * node.getAsnStandard (); prtln (std.toString()); }
		 */
	...
	/*
	 * Represents a DocInfo key used to access the registered documents using
	 * wildcarding for various key fields.
	 */
	/**
	...
	private class DocMatchKey {
		...
		/**
		 * Constructor for the DocMatchKey object from provided string, using wildcards
		...
		boolean matches(DocMatchKey otherKey) {
			// prtln ("matches this: \"" + this.toString() + "\" vs otherKey: \"" +
			// otherKey.toString() + "\"");
