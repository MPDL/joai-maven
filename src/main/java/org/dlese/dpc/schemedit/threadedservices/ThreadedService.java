...
public abstract class ThreadedService extends MonitoredTask {
	...
	public ThreadedService(ServletContext servletContext, String threadedServiceDataDir) {
		...
		this.dcsDataManager = (DcsDataManager) servletContext.getAttribute("dcsDataManager");
	...
	protected void validate(XMLDocReader docReader, DcsDataRecord dcsDataRecord) {
		...
		/*
		 * we were getting different results when passing a file or a string. the string
		 * (encoded as 'utf-8' gives us results consistent with command-line parsers, as
		 * well as Xerces.
		 */
		// repositoryService.validateRecord(docReader.getFile(), dcsDataRecord,
	 ...
	 * collection and statuses. This list is used by validate and export to iterate
	 * over items to process. The reason we don't use the resultDocs obtained by
	 * querying the index is that, for long result lists, the index may change out
	 * from under the resultDocs, causing the resultDocs to be unreliable.
	 *
	 * @param collection
	 *            NOT YET DOCUMENTED
	 * @param statuses
	 ...
	 *
	 * @param list
	 *            a list to be coverted
	 * @param delimiter
	 *            delimiter used to separate items
	 ...
	 *            collectionKey
	 * @param statuses
	 *            statuses to be included in query
	 * @return a query string to find items in collection with specified statuses
	 */
	 ...
	 *
	 * @param msg
	 *            The feature to be added to the ValidatingMessage attribute
	 */
	protected void addStatusMessage(String msg) {
		prtln(getSimpleDateStamp() + ": " + msg);

		ArrayList statusMessages = (ArrayList) dataStore.get("STATUS_MESSAGES");
	...
	/**
	 * Sets whether or not to validate the files being indexed. If set to true, the
	 * files will be validated, otherwise they will not. Default is true.
	 *
	 * @param validateFiles
	 ...
	 *
	 * @param indexOnValidation
	 *            The new indexOnValidation value
	 */
