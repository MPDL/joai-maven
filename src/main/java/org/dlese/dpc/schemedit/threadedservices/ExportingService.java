...
public class ExportingService extends ThreadedService {
	 ...
	 *            NOT YET DOCUMENTED
	 */
	public ExportingService(ServletContext servletContext, String exportingServiceDataDir, String exportBaseDir,
			String xslPath) {
		// set up
	...
	public boolean isLegalExportDest(File destDir) {
		...
		try {
			...
			// prtln ("\t" + "exportRoot.getCanonicalPath(): " +
			// exportRoot.getCanonicalPath());
	 ...
	 *            export directory
	 * @param dcsSetInfo
	 *            setInfo for collection to be exported
	 * @param statuses
	 *            record statuses to be exported
	 * @param sessionBean
	 *            NOT YET DOCUMENTED
	 * @exception ExportingServiceException
	 *                if unable to export
	 */
	 ...
	 *            NOT YET DOCUMENTED
	 * @param observer
	 *            observer receives notification when export is complete.
	 * @exception ExportingServiceException
	 *                NOT YET DOCUMENTED
	 */
	public void exportRecords(File destDir, DcsSetInfo dcsSetInfo, String[] statuses, SessionBean sessionBean,
			ThreadedServiceObserver observer) throws ExportingServiceException {
		String destPath = destDir.getAbsolutePath();
		// prtln("exportRecords(" + destPath + ")");

		if (!isLegalExportDest(destDir)) {
			String errorMsg = "Export destination Path (" + destPath + ") is not contained by exportBaseDir ("
					+ exportBaseDir + ")";
	...
	public void exportRecords(List idsToExport, File destDir, ThreadedServiceObserver observer) {
		...
		for (int i = 0; i < idsToExport.size() && !stopProcessing; i++) {
			...
			if (this.validationIsStale(sourceFile, docReader) || dcsDataRecord.getIsValidityUnknown()) {
				...
				// ...
				/*
				 * prtln ("\tsourceFile.lastModified(): " + sourceFile.lastModified()); prtln
				 * ("\tdocReader.getLastModified(): " + docReader.getLastModified()); prtln
			...
			// if we haven't eliminated the current record from export due to invalid, then
			// add
			// it to exportedFileNames, which is later used to delete records from the
			// export directory that
	...
	/**
	 * Get the XML to be exported, performing necessary transformations.
	 * <p>
	 *
	...
	private String getExportXml(XMLDocReader docReader) throws Exception {
		/*
		 * previously we had blindly run "cataloger-out" over all records, but really it
		 * only applied to a couple frameworks. (NOTE: we're still appying the cataloger
		 * out to every record, even though it really applies only to ADN (ask Katy for
		 * verification)). - we'll need some way to specify (on a per-framework basis,
		 * at least) what transform, if any, to apply here. - also, we have to
		 * accomodate java-based transforms ...
		 * 
		 */
