...
public class AutoExportTask extends TimerTask {
	...
	public void run() {
		...
		ExportingService exportingService = (ExportingService) servletContext.getAttribute("exportingService");
		RepositoryService repositoryService = (RepositoryService) servletContext.getAttribute("repositoryService");
		CollectionRegistry collectionRegistry = (CollectionRegistry) servletContext.getAttribute("collectionRegistry");
		...
		for (Iterator i = repositoryService.getSetInfos().iterator(); i.hasNext();) {
			try {
				...
				File destDir = ExportingService.validateExportDestination(exportingService.getExportBaseDir(), relPath);
				...
				String errorMsg = "AutoExport error: collection \"" + collection + "\" could not be exported: "
						+ t.getMessage();
				prtln(errorMsg);
				t.printStackTrace();
