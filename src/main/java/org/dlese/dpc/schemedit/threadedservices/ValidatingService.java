...
public class ValidatingService extends ThreadedService {
	 ...
	 *
	 * @param validatingServiceDataDir
	 ...
	 * @param sessionBean
	 *            NOT YET DOCUMENTED
	 * @exception ValidatingServiceException
	 *                NOT YET DOCUMENTED
	 */
	public void validateRecords(DcsSetInfo dcsSetInfo, String[] statuses, SessionBean sessionBean)
		...
		for (int i = 0; i < idsToValidate.size() && !stopProcessing; i++) {
			...
			if (validationIsStale || validityUnknown || this.ignoreCachedValidation) {
				...
				if (validationIsStale) {
					...
					try {
						/*
						 * record on disk has been changed, so we first index it so that the index is
						 * consistent with the file on disk - RATIONALE - files on disk are primary!
						 */
			...
			Thread.yield();
			// prtln("validateRecords() stopProcessing: " + stopProcessing + ", " + i
			// +"/"+records.length);
