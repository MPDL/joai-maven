...
/**
 * Class to perform searches and cache the results as well as the query and sort
 * information. Used to provide access to search results from different JSP
 * pages (e.g., search, view).
 *
...
public class SearchHelper {
	...
	/**
	 * Returns the current searchResults, doing a fresh search and sort if the index
	 * has changed since the searchResults were calculated.
	 * <p>
	 *
	 * When do we FORCE a new search?? - during sort (where the sort may have
	 * changed, but the index has not, so search would not have otherwise have been
	 * performed. NOTE: we could also force search by reseting the lastIndexMod to
	 * -1!
	 * <p>
	 *
	 * RETHINK this logic. why cache anything? why not just return the searchResults
	 * or an empty list??
	 *
	 ...
	 *
	 * @param sortSpec
	 *            the SortSpec instance to be converted
	 ...
	 * only if a new query string is provided or if the index has changed since the
	 * last search.
	 * <p>
	 *
	 *
	...
	public ResultDocList search(Query query, Object sortObj) {
		...
		/*
		 * prtln("\nSEARCH"); prtln ("\t new Query ? " + isNewQuery(query)); prtln
		 * ("\t new Index ? " + isNewIndex());
		 */
	...
	public ResultDoc getResultDoc(String id) {
		...
		ResultDoc[] resultDocs = index.searchDocs("id:" + SimpleLuceneIndex.encodeToTerm(id)).toArray();
	 ...
	 *
	 * @param sortWidget
	 *            SortWidget to compare to cached sort
	 * @return true if cashedSort is not null and field or order of cashed sort
	 *         differs from that of provided SortWidget.
	 */
	 ...
	 * @param query
	 *            Description of the Parameter
	 * @return Returns true if provided query is different than current query.
	 */
	...
	/**
	 * Determines whether the index has changed since the last search operation was
	 * performed.
	 *
	...
	/**
	 * Stores information necessary to specify a sort (i.e., field and order), used
	 * to cache information so a query can be re-executed if necessary (e.g., if the
	 * index has changed we want to re-execute a query so the user sees the most
	 * current information.
	 *
	 * @author ostwald
	 */
	private class SortSpec {
		 ...
		 * @param widget
		 *            the sortWidget
		 * @return true if the provided sortWidget specifies a sort other than this
		 *         sortSpec
		 */
		public boolean isNewSort(SortWidget widget) {
			...
			return (widget.getOrder() != this.order || !widget.getFieldName().equals(this.field));
		}

		/**
		 * Returns true if the provided widget specifies a different sort field than the
		 * sort field of this SortSpec.
		 *
		 * @param widget
		 *            provided SortWidget
		 * @return true if provided sortWidget is not null and sort fields do not match
		 */
