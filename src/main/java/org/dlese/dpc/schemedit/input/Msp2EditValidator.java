...
public class Msp2EditValidator extends SchemEditValidator {
	 ...
	 *            NOT YET DOCUMENTED
	 */
	public Msp2EditValidator(SchemEditForm sef, MetaDataFramework framework, ActionMapping mapping,
	...
	public SchemEditActionErrors validateForm() {
		...
		// validation: we flag error at the SUBJECTS_PATH level if none of it's children
		// have a value
		if (this.sef.getCurrentPage().equals("general")) {
			...
			if (!hasValue) {
				...
				// exposeField(subjectsField); / no inputField for this element - should we
				// create one?
			}
		}
