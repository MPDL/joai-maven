...
/**
 * Methods to create ActionErrors for the Metadata Editor. Most create two error
 * messages :
 * <ol>
 * <li>At the top of the page with a hyperlink to the field in which the error
 * occurs
 ...
 * follows: msgKey+".link". For example, if the msgKey was "invalid.value", then
 * the linked message key is named "invalid.value.link"
 *
 * @author ostwald
 */
public class SchemEditErrors {
	 ...
	 * <p>
	 *
	 * Example use: to notify of an element that could not be deleted. Here there is
	 * not a specific InputField involved, but rather an branch of the XML document
	 * (which is identified by xpath).
	 * <p>
	 *
	 ...
	 *
	 * @param errors
	 *            ActionErrors object to which the new errors are added
	 * @param fieldProperty
	 *            The property to which this error is attached (e.g.,
	 *            "valueOf(/itemRecord/general/description_1_)")
	 * @param msgKey
	 *            Reference to a message in the "ApplicationResources"
	 * @param elementName
	 *            The feature to be added to the Error attribute
	 * @param pathArg
	 *            xpath (encoded for jsp) to the field in which the error occurs
	 */
	public static void addError(SchemEditActionErrors errors, String fieldProperty, String msgKey, String elementName,
			String pathArg) {
		prtln("addError() #1");
		...
		errors.add(fieldProperty, new ActionMessage(msgKey, elementName));
		errors.add("pageErrors", new ActionMessage(msgKey + ".link", elementName, pathArg));
	}

	/**
	 * Creates two ActionErrors signifying a Generic error, one for the top of page
	 * message, and one for the error message that is attached to a specific field.
	 *
	 * @param errors
	 *            error list to which we add
	 * @param msg
	 *            error msg content
	 * @param xpath
	 *            The feature to be added to the GenericError attribute
	 */
	public static void addGenericError(SchemEditActionErrors errors, String xpath, String msg) {
		...
		errors.add(fieldProperty, new ActionMessage(msgKey, msg));
		errors.add("pageErrors", new ActionMessage(msgKey + ".link", msg, id));
	}

	/**
	 * Creates two ActionErrors signifying an invalidUrlError error, one for the top
	 * of page message, and one for the error message that is attached to a specific
	 * field.
	 *
	 * @param errors
	 *            error list to which we add
	 * @param uniqueUrlPath
	 *            path of field that should contain uniqueUrl
	 * @param msg
	 *            error msg content
	 */
	 ...
	 *            error list to which we add
	 * @param msgKey
	 *            msgKey to select message from applicationResources
	 * @param uniqueUrlPath
	 *            path of field that should contain uniqueUrl
	 */
	 ...
	 *
	 * @param errors
	 *            the error list to which we add
	 * @param inputField
	 *            the input field containing a dup value
	 * @param dupRecId
	 *            The Id of a record containing a dup value
	 */
	public static void addDuplicateValueError(SchemEditActionErrors errors, InputField inputField, String dupRecId) {
		...
		errors.add(fieldProperty, new ActionMessage(msgKey, dupRecId));
		errors.add("duplicateValues", new ActionMessage(msgKey + ".link", inputField.getValue(), dupRecId, fieldId));
	}
	 ...
	 *            path of field that should contain uniqueUrl
	 * @param msgProperty
	 *            identifies the type of error and how it is displayed in UI
	 */
	public static void addUniqueUrlError(SchemEditActionErrors errors, String uniqueUrlPath, String msgKey,
			String msgProperty) {
		...
		errors.add(fieldProperty, new ActionMessage(msgKey, elementName));
		errors.add(msgProperty, new ActionMessage(msgKey + ".link", elementName, id));
		/*
		 * SchemEditErrors.addError(errors, fieldProperty, "unique.url.required",
		 * elementName, encodedPath, "similarUrls");
		 */
	}
	 ...
	 *            ActionErrors object to which the new errors are added
	 * @param field
	 *            The InputField containing an error
	 * @param msgKey
	 *            Reference to a message in the "ApplicationResources"
	 */
	public static void addError(SchemEditActionErrors errors, InputField field, String msgKey) {
		...
		errors.add(fieldProperty, new ActionMessage(msgKey, elementName));
		errors.add("pageErrors", new ActionMessage(msgKey + ".link", elementName, id));
	}
	 ...
	 *            The InputField containing an error
	 * @param errorMsg
	 *            The feature to be added to the EntityError attribute
	 */
	 ...
	 *            error list to which we add
	 * @param field
	 *            inputField that error message is attached
	 * @param errorMsg
	 ...
	 *            Reference to a message in the "ApplicationResources"
	 * @param errorMsg
	 *            The feature to be added to the XSDdatatypeError attribute
	 */
	/*
	 * public static void addEmptyWithChildError(ActionErrors errors, InputField
	 * field, String msgKey) { // prtln ("addEmptyWithChildError() xpath: " +
	 * field.getXPath()); String elementName = field.getFieldName(); String pathArg
	 * = XPathUtils.encodeXPath(field.getXPath()); String fieldProperty =
	 * field.getParamName(); errors.add(fieldProperty, new ActionMessage(msgKey));
	 * errors.add("pageErrors", new ActionMessage(msgKey + ".link", pathArg)); }
	 */
	/**
	 ...
	 *            The text of the error message
	 */
	public static void addXSDdatatypeError(SchemEditActionErrors errors, InputField field, String msgKey,
			String errorMsg) {
		String fieldProperty = field.getParamName();
