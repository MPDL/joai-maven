 ...
 * validation. Based on
 * {@link org.dlese.dpc.webapps.servlets.filters.XMLValidationFilter}
 *
...
public final class RecordValidationFilter extends XMLPostProcessingFilter {
	...
	protected String getValidationMessage(String xml, ServletRequest req) {
		...
		try {
			...
			RepositoryService repositoryService = (RepositoryService) this.context.getAttribute("repositoryService");
			...
			FrameworkRegistry frameworkRegistry = (FrameworkRegistry) this.context.getAttribute("frameworkRegistry");
			...
			// finish up by normalizing validation message (code below expects message to be
			// null
			// if record is valid
	...
	public void init(FilterConfig config) throws ServletException {
		if (context == null) {
			try {
				...
				/*
				 * if (((String)
				 * context.getInitParameter("debug")).toLowerCase().equals("true")) { debug =
				 * true; prtln("Outputing debug info"); }
				 */
				debug = true;
	 ...
	 */
	/*
	 * protected final void prtlnErr(String s) { System.err.println(getDateStamp() +
	 * " " + s); }
	 */
