 ...
 * {@link org.dlese.dpc.schemedit.SchemEditErrors}.
 *
 * @author ostwald
 *
 *
...
public class SchemEditValidator {
	 ...
	 *            NOT YET DOCUMENTED
	 */
	public SchemEditValidator(SchemEditForm sef, MetaDataFramework framework, ActionMapping mapping,
	 ...
	 * <code>validateMultiValueFields</code> , then
	 * <code>validateAttributeFields</code>, and finally
	 * <code>validateElementFields</code> .
	 *
	...
	/**
	 * Predicate to identify if the input contains entities that could not be
	 * resolved.
	 *
	...
	public SchemEditActionErrors getEntityErrors() {
		...
		for (Iterator f = errorFields.iterator(); f.hasNext();) {
			...
			for (Iterator e = entityErrors.iterator(); e.hasNext();) {
				...
				errorMsg += "<br/>&nbsp;-&nbsp;" + ref + " could not be resolved (at offset "
						+ entityError.getErrorOffset() + ")";
			}
	...
	protected SchemEditActionErrors validateElementFields() {
		...
		/*
		 * For now, we do not attempt to validate anyType fields. This probably has to
		 * be done as a seperate method, since there are tricky interactions with
		 * SchemEditForm.validate()
		 */
		...
		while (fields.hasNext()) {
			...
			// the element is (a choice element OR (requiredContent AND NOT nillable)), warn
			// user
			...
			if ((value == null) || (value.length() == 0)) {
				...
				/*
				 * disabling the following check: 4/9/04 - because sometimes it's okay to have
				 * an empty parent field with populated children (see
				 * Collection.relations.relation)!?
				 */
				else if (!docMap.isEmpty(xpath)) {
					// prtln("element with children missing value: " + field.getParamName());
					// SchemEditErrors.addEmptyWithChildError (errors, field,
					// "empty.element.with.children");
				} else {
					/*
					 * we have an empty element. we used to delete such empty elements but now are
					 * leaving them for the user to delete 5/11/04 prtln ("deleting empty element: "
					 * + field.getParamName()); docMap.remove(xpath);
					 */
		...
		for (Iterator ai = im.getAttributeFields().iterator(); ai.hasNext();) {
			...
			try {
				if (!docMap.nodeExists(xpath)) {
					/*
					 * why wasn't a node for this attribute created already?? we could simply create
					 * the attribute (as long as it exists in the schemaNodeMap), but i think it
					 * should have already been created ... // prtln
					 * ("docMap node does not exist for " + xpath); // continue; /* docMap
					 */
	 ...
	 * then add the NON-EMPTY params from the input, and finally validate, returning
	 * errors.
	 */
	public void updateMultiValueFields() {
		...
		for (Iterator i = im.getMultiValueFields().iterator(); i.hasNext();) {
			...
			// is this a new group? groups are identified by the siblingXPath (which
	 ...
	 * element to be automatically inserted after the last one is deleted. But this
	 * guard is relaxed for choice and substitutionGroup elements, since we can't
	 * know which element to replace the last one with.
	 *
	 * @param pathArg
	 *            NOT YET DOCUMENTED
	...
	public boolean isLastRequiredRepeatingElement(String pathArg) {
		...
		if (siblingList.size() == 1) {
			...
			if (schemaNode.getMinOccurs() == 1 && !schemaHelper.isChoiceElement(schemaNode)
