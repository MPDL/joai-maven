...
public class ElementsOrderer {
	...
	/**
	 * Constructor for the ElementsOrderer object given an instance Document and an
	 * orderSpec.
	 * <p>
	 *
	 * The orderSpec is an xpath with an attribute as the leaf, and is interpreted
	 * as follows:
	 * <li>The attribute name of the xpath specifieds the orderingAttribute, e.g.,
	 * "//farb/@foo", the "foo" attribute is used to compute an order.
	 ...
	 * are sorted. If the nodeName is a string, then only elements of that name will
	 * be sorted.
	 * <p>
	 *
	 ...
	 * @param orderSpec
	 *            an expath that specified the ordering attribute as well as the
	 ...
	 * operations required, and orders each one.
	 * <p>
	 *
	 * For example, if the orderSpec is "//contents/@order" there may be several tag
	 * sets containing "contents" selected by the xpath, and each must be sorted
	 * individually.
	 *
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	...
	private void processDoc() throws Exception {
		...
		/*
		 * orderedElementPaths - identify the separate groups that will be ordered
		 * (e.g., the orderNodes might contain members from different parent elements,
		 * which we will want to sort separately).
		 */
	 ...
	 *
	 * @param orderedElementPath
	 *            an xpath to an element containing elements to be ordered
	 */
	 ...
	 * @param children
	 *            list of unordered instance doc elements
	 * @param replaceNodeName
	 *            either "*" or a nodeName
	 */
	private void replaceNamedNodes(List orderedNodes, List children, String replaceNodeName) {
		...
		for (int i = 0; i < children.size(); i++) {
			...
			if ("*".equals(replaceNodeName) || replaceNodeName.equals(child.getName())) {
				...
				prtln("\t replacing with " + replacement.attributeValue("title", "N/A") + " (" + r_order + ")");
				if (children.size() == i)
	...
	public class ElementOrderComparator implements Comparator {
		...
		/**
		 * Get order num based on the named attribute, returning a BIG num if there is
		 * no attribute, or if the value cannot be parsed as an integet greater than 0
		 * (1 is the smallest meaningful order number).
		 *
		 * @param o
		 *            NOT YET DOCUMENTED
		...
		private Integer getOrderNum(Object o) {
			...
			try {
				...
				// System.out.println ("getOrder error for \"" + orderAttrPath + "\": " +
				// e.getMessage());
			}
