 ...
 * Pruning is performed in place (i.e., as a side effect. no value is returned);
 *
 * @author ostwald
...
public class DocumentPruner {
	...
	/**
	 * Recursively delete all non-required, empty fields and elements.
	 * <P>
	 *
	...
	private void pruneTree(Element e) {
		...
		for (int i = attributes.size() - 1; i > -1; i--) {
			...
			// 2/16/07 - no longer trim values (thereby removing fields with only
			// whitespace)
			/*
			 * Handling attributes containing *only whitespace* - if the attribute value is
			 * schema-valid, leave it alone. - if the value is NOT schema-valid, prune it
			 */
			if ((value == null) || (value.trim().length() == 0)) {
				...
				if (schemaNode == null) {
					...
					// remove attributes containing only white space if they are NOT schema-valid
						...
						try {
							...
							prtln("\t removing non-valide whitespace attribute " + attr.getQualifiedName() + " from "
									+ path);
							e.remove(attr);
						}
					}
				...
				// prtln("\t non-empty attribute not pruned: " + attr.getQualifiedName() + " at
				// " + path + ": value=\"" + value + "\"");
			}
		}
		...
		for (int i = children.size() - 1; i > -1; i--) {
			...
			if ((def != null) && def.isComplexType()) {
				...
				// simpleType - if there is no text value and the element is not required, then
				// remove it.
