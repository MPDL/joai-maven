...
/**
 * Extends SchemEditValidator to provide validation services for collection
 * configuration records. Specifically, ensures that status flags are not
 * duplicated, nor do they redefine reserved flags
 * <p>
 *
 *
 *
 ...
 *         <p>
 *
 *         $Id: CollectionConfigValidator.java,v 1.10 2005/06/02 18:45:40
 *         ostwald Exp $
 */
public class CollectionConfigValidator extends SchemEditValidator {
	 ...
	 *            Description of the Parameter
	 */
	public CollectionConfigValidator(CollectionRegistry collectionRegistry, SchemEditForm sef,
			MetaDataFramework framework, ActionMapping mapping, HttpServletRequest request) {
		super(sef, framework, mapping, request);
		this.collectionRegistry = collectionRegistry;
	}

	/**
	 * In addition to validating against the schema, check statusFlags for
	 * duplicates and create SchemEditErrors if dups found.
	 *
	...
	public SchemEditActionErrors validateForm() {
		...
		/*
		 * prtln("\n reserved status labels"); for (Iterator i =
		 * unavailableFlags.iterator(); i.hasNext(); ) { prtln("\t" + (String)
		 * i.next()); }
		 */
		...
		// don't flag dup error if the idPrefix is null or empty - it will be caught as
		// an empty required field.
		if (idPrefixField != null && idPrefixField.getValue().trim().length() > 0
	...
	private InputField getFinalStatusFlagField() {
		...
		/*
		 * for (Iterator i = im.getInputFields().iterator(); i.hasNext(); ) { InputField
		 * field = (InputField) i.next(); // prtln (field.toString()); if
		 * (field.getXPath().equals(finalStatusLabelPath)) { return field; } } return
		 * null;
		 */
