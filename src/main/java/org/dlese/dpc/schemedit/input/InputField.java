 ...
 * field uses to obtain its value (e.g.,
...
public class InputField {
	 ...
	 * </dl>
	 * paramName is used with the error-report mechanism to locate the field element
	 * in the UI
	 *
	 * @param paramName
	 *            paramName as received from the request
	 * @param value
	 *            the value as received from the request
	 * @param schemaNode
	 *            SchemaNode instance for this field
	 * @param xpath
	 *            the xpath of this field
	 * @param inputManager
	 *            the inputManager instance for this field
	 */
	protected InputField(String paramName, String value, SchemaNode schemaNode, String xpath,
	...
	/**
	 * Returns an xpath containing no indexing for the element corresponding to this
	 * InputField.
	 * <p>
	 *
	 * Note: this is the same path used to key the SchemaNodeMap, so why do we need
	 * it as an attribute? Could this accessor just as well return
	 * schemaNode.getXPath?
	 *
	...
	private String handleWhiteSpace(String str) {
		...
		com.sun.msv.datatype.xsd.XSDatatypeImpl xsdDataType = (com.sun.msv.datatype.xsd.XSDatatypeImpl) schemaHelper
				.getXSDataType(vTypeName);
		if (xsdDataType == null) {
			// we don't expect an xsdDataType to be found for a derived complex type
			// prtln ("xsdDataType not found for " + vTypeName + " (" +
			// schemaNode.getXpath() + ")");
			return str;
		} else {
			com.sun.msv.datatype.xsd.WhiteSpaceProcessor whiteSpaceProcessor = xsdDataType.whiteSpace;

			// here is where we would massage value (str) depending on value of
			// whiteSpaceProcessor
