...
public class ReferenceResolver {
	...
	/**
	 * Returned by ReferenceResolver.resolve storing the resolved input string and a
	 * list of errors describing references that could not be resolved.
	 *
	 * @author ostwald
	 */
	...
	public ResolverResults resolve(String in) {
		...
		/*
		 * prtln("refs found"); for (Iterator i = rr.refs.iterator(); i.hasNext(); ) {
		 * prtln("\t" + (String) i.next()); }
		 */
	...
	/**
	 * Convert a single character or numeric reference (with leading & and trailing
	 * ; stripped) to an equivalent unicode string.
	 *
	 * @param ref
	 *            A numerical or character reference
	 * @return String representation of unicode equivalent
	 * @exception IllegalArgumentException
	 *                If unicode equivalent cannot be derived.
	 */
	String resolveRef(String ref) throws IllegalArgumentException {
		if (ref.charAt(0) == '#') {
	 ...
	 *
	 * @param ref
	 *            character reference (With leading & and trailing ; stripped)
	 * @return String representation of unicode equivalent.
	 * @exception IllegalArgumentException
	 *                If given reference cannot be mapped to a numeric equivalent.
	 */
	String resolveCharacterRef(String ref) throws IllegalArgumentException {
		String characterRef = "&" + ref + ";";
		if (ref.equals("lt") || ref.equals("gt") || ref.equals("amp") || ref.equals("quot") || ref.equals("apos")) {
			return characterRef;
		}
	...
	public static void main(String[] args) throws Exception {
		...
		if (errors.size() > 0) {
			...
			for (Iterator i = errors.iterator(); i.hasNext();) {
				/*
				 * ParseException error = (ParseException) i.next(); prtln("\t" +
				 * error.getMessage() + " could not be resolved (at offset " +
	...
	/**
	 * Converts a numeric character reference in either decimal (e.g., "&#169;") or
	 * hex (e.g., "&x00a9") into an equivalent unicode representation
	 *
	 * @param s
	 *            Numeric reference (with leading & and trailing ; stripped)
	 * @return Equivelant unicode representation
	 * @exception IllegalArgumentException
	 *                If given ref cannot be converted to unicode
	 */
	static String toUnicode(String s) throws IllegalArgumentException {
		int codePoint = 0;
		...
		try {
			...
			// throw new IllegalArgumentException("Unable to parse input into codePoint: " +
			// nfe.toString());
