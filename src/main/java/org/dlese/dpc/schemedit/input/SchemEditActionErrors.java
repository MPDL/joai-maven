...
public class SchemEditActionErrors extends ActionErrors {
	...
	/**
	 * Constructor for the SchemEditActionErrors object with SchemaHelper argument,
	 * which is needed to maintain errors in document order.
	 *
	 * @param schemaHelper
	 *            the schemaHelper
	 */
	...
	/**
	 * Add an error, and sort into document order if schemaHelper is present and if
	 * specified propery is contained in sortedProperties list.
	 *
	 * @param property
	 *            error property
	 * @param message
	...
	public Iterator get_Off(String property) {
		...
		if (item == null) {
			...
			if (property == null || !property.equals("pageErrors")) {
				...
				// memebers of sortedList are org.apache.struts.action.ActionMessage
				// (ActionError))
	 ...
	 * that editing errors can be presented in the same order in which they occur in
	 ...
	 * errors (see SchemEditValidator & SchemEditErrors).
	 * <p>
	 *
	 ...
	 * indexing info before tie is broken, compare by schemaPaths
	 * <p>
	 *
	 ...
	 * are no indexes (since indexing information is removed from the schema paths).
	 * <p>
	 *
	 * If two paths are judged equal by schema order, then they can be simply
	 * compared as strings to bring the indexing information in and resolve the tie.
	 *
	 * @author Jonathan Ostwald
	 */
	public class DocOrderComparator implements Comparator {
		 ...
		 * (e.g., [1] vs [2], then make comparison based on the index - if segments are
		 * different leafNames, then return a 0 so the paths will be compared by
		 * schemaPath.
		 *
		 * @param p1
		 *            NOT YET DOCUMENTED
		...
		/**
		 * sorts by order in which paths are processed by StructureWalker (and therefore
		 * are added to the SchemaNodeMap)
		 *
		 * @param o1
		 *            NOT YET DOCUMENTED
		 * @param o2
		...
		public int compare(Object o1, Object o2) {
			...
			if (schemaOrder != null) {
				cmp = schemaOrder.compare(getSchemaCmpPath(path1), getSchemaCmpPath(path2));
			} else {
