...
class FieldFilesChecker {
	...
	public FieldFilesChecker(File frameworkDir, URI schemaUri) throws Exception {
		...
		if (!listingFile.exists()) {
			/*
			 * prtln ("ERROR: fieldsFileListing not found at " + listingFile); return;
			 */
		...
		try {
			...
			throw new Exception("ERROR: File listing document (\"" + listingFile
					+ "\") either does not exist or cannot be parsed as XML");
		}
	...
	void initializeErrorLists() {
		...
		missingPaths = new ErrorList("Missing Paths", "Schema xpaths not defined in any field file");

		multiPaths = new ErrorList("Duplicate Paths", "Xpaths contained in mulitiple fields files");
	...
	/**
	 * Process a set of FieldsFiles (read from an xml file) and perform an integrity
	 * check on each fieldsFile.
	 *
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	void doCheck() throws Exception {
		...
		if (!initialized) {
			// throw exception so this reporter will be removed from the master list and no
		...
		for (Iterator i = fieldFilesElement.elementIterator(); i.hasNext();) {
			...
			try {
				...
				if (!schemaPaths.isLegalPath(xpath)) {
					/*
					 * throw new Exception ("ERROR: Fields file at " + myUri.toString() +
					 * " contains an illegal path: " + xpath);
					 */
	 ...
	 * 
	 * if (!initialized) { // throw exception so this reporter will be removed from
	 * the master list and no reports will be attempted throw new Exception
	 * ("not initialized!"); }
	 * 
	 ...
	 * initializeErrorLists();
	 * 
	 * FieldInfoReader reader = null; String xpath; try { // URI myUri =
	 * baseUri.resolve(fileSpec); URI myUri =
	 * NewFieldInfoMap.getFieldsFileUri(fileListURI, fileSpec); try { reader = new
	 * FieldInfoReader(myUri); } catch (Exception e) { // readerErrors.add(new
	 * ReaderError(myUri, e.getMessage())); prtln (e.getMessage()); return; }
	 * 
	 * filesRead++;
	 * 
	 * try { xpath = reader.getPath(); } catch (Throwable pathEx) { //
	 * illegalPaths.add(new PathError(myUri, "path not found")); prtln
	 * ("path not found"); return; }
	 * 
	 * if (!schemaPaths.isLegalPath(xpath)) { // throw new Exception
	 * ("ERROR: Fields file at " + myUri.toString() + " contains an illegal path: "
	 * + xpath); // illegalPaths.add(new PathError(myUri, xpath)); } else {
	 * schemaPaths.markAsSeen(myUri, xpath); }
	 * 
	 * try { vocabCheck (reader, fileSpec); } catch (Exception e) { prtln
	 * ("VoabCheck error: " + e.getMessage());
	 * 
	 * }
	 * 
	 * } catch (Throwable t) { prtln(t.getMessage()); t.printStackTrace(); return; }
	 * }
	 */
	 ...
	 * something is pretty wrong -Bail
	 * 
	 * three cases
	 * <ol>
	 * <li>simple Enumeration (GenericType.isEnumerationType()) ->
	 * genericType.getEnumerationValues
	 * <li>- comboUnion Type (SimpleType.isComboUnionType() ->
	 * genericType.getEnumerationValues (?will this work with comboUnions?)
	 ...
	 * can't treat the failure to find vocab fields as an error - ASSUMPTION: vocabs
	 * terms are always defined in the schema as ENUMERATIONs.
	 */
	List getSchemaVocabTerms(FieldInfoReader reader) throws Exception {
		...
		try {
			if (!globalDef.isTypeDef()) {
				msg = "GlobalDef found at " + path + "(" + globalDef.getQualifiedInstanceName()
			...
			if (globalDef.isComplexType()) {
				...
				if (!complexType.isDerivedTextOnlyModel())
					throw new Exception(
							"globalDef is complexType but not derivedTextOnlyModel - cannot define enumeration");
	 ...
	 * defined in the field file - report duplicate term definitions (terms must be
	 * defined only once
	 */
	...
	abstract class ICError {
		 ...
		 *            URI of fieldsFile
		 * @param data
		 *            An Oject to hold information about this error
		 */
	...
	class SchemaPaths {
		...
		List getMultiPaths() {
			...
			for (Iterator i = multiplesMap.entrySet().iterator(); i.hasNext();) {
				...
				multiplesList.add(new MultiPaths((String) multiple.getKey(), (List) multiple.getValue()));
			}
			return multiplesList;
		}

		/*
		 * Map getMultiPaths() { Map multiples = new TreeMap(); for (Iterator i =
		 * map.keySet().iterator(); i.hasNext(); ) { String key = (String) i.next();
		 * List locs = getLocs(key); if (locs.size() > 1) { multiples.put(key, locs); }
		 * } return multiples; }
		 */
	...
	List getSchemaVocabTermsOLD(FieldInfoReader reader) throws Exception {
		...
		try {
			...
			if (!typeDef.isEnumerationType()) {
				// msg = "schemaNode at " + path + " is \"" + typeDef.getName() + "\" (not an
				// enumeration type)";
