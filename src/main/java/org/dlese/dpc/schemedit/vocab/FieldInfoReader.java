 ...
 * <p>
 *
 * FieldInfoReader reads FieldInfo xml documents and is used by
 * {@link org.dlese.dpc.schemedit.autoform} classes as well as
 * {@link org.dlese.dpc.schemedit.MetaDataFramework} instances.
 * <p>
 *
 * Note: why doesn't this class extend the
 * {@link org.dlese.dpc.schemedit.config.AbstractConfigReader} class?
 *
 * @author ostwald
 */
public class FieldInfoReader {
	 ...
	 *                Description of the Exception
	 */
	public FieldInfoReader(URI uri) throws Exception {
		this.uri = uri;
		try {
	...
	/**
	 * Gets a List of {@link TermAndDeftn} instances for specified term (usually
	...
	/**
	 * Gets the {@link org.dlese.dpc.schemedit.vocab.TermAndDeftn} instances defined
	 * for this field.
	 *
	...
	/**
	 * Build an array of terms (represented as LabelValueBean instances) to be used
	 * in a html select object.
	 *
	...
	/**
	 * Gets the mapping from vocab term to corresponding
	 * {@link org.dlese.dpc.schemedit.vocab.TermAndDeft} instance.
	 *
	 ...
	 *
	 * @param text
	 *            A string of which a substring will receive formatting
	 * @param link
	 *            the substring to recieve formatting
	 * @param url
	 *            if present, provides destination for link (link becomes a
	 *            hyperlink to url)
	 * @param style
	 *            specifies the formatting to be applied to link (bold, italic, bold
	 *            and italic)
	...
	public static String applyMarkup(String text, String link, String url, String style) {
		...
		if (link != null && text != null && text.indexOf(link) != -1) {
			...
			if (url != null) {
				Element hyperlink = df.createElement("a").addAttribute("target", "_blank").addAttribute("href", url);
			...
			// if neither style or url is specified, then there is no linkElement (i.e.,
	...
	/**
	 * Return formatted contents for given Element, inserting hyperlink when "link"
	 * and "url" attributes are present
	 *
	 * @param e
	 *            Description of the Parameter
	 ...
	 * <p>
	 *
	 * needed to replace strings with regex pattern chars, which don't get replaced
	 * by String.replace
	 *
	 * @param in
	 *            Description of the Parameter
	...
	public class OtherPractice {
		 ...
		 *
		 * @param header
		 *            Header for this set of otherPractices
		 * @param practices
		 *            List of formattedPractices
		 */
