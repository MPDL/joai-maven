...
public class NewFieldInfoMap extends FieldInfoMap {
	 ...
	 * the listed files as FieldInfoReader objects, which are stored in a map.
	 *
	 * @exception Exception
	...
	public static URI getFieldsFileUri(URI baseUri, String relativePath) throws Exception {
		/*
		 * prtln ("baseUri: " + baseUri.toString()); prtln ("authority: " +
		 * baseUri.getAuthority()); prtln ("schemeSpecificPart: " +
		 * baseUri.getSchemeSpecificPart()); prtln ("userInfo: " +
		 * baseUri.getUserInfo()); prtln ("scheme: " + baseUri.getScheme()); prtln
		 * ("host: " + baseUri.getHost()); prtln ("port: " + baseUri.getPort()); prtln
		...
		if (x == -1)
			throw new Exception(
					"framework/version key (\"" + key + "\") not found in baseUri (\"" + baseUri.toString() + "\")");
	 ...
	 *                Description of the Exception
	 */
	void downLoadFieldsFiles() throws Exception {
		prtln("hello world");
