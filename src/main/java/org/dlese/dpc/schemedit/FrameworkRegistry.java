...
public class FrameworkRegistry implements MetadataVocabReloadListener {
	...
	public FrameworkRegistry(ServletContext servletContext) {
		...
		this.suggestionServiceManager = (SuggestionServiceManager) servletContext
				.getAttribute("suggestionServiceManager");

		if (suggestionServiceManager == null)
			prtln("suggestionServiceManager is NULL");

		this.allowDleseCollectItems = "true".equals((String) servletContext.getInitParameter("allowDleseCollectItems"));
	 ...
	 *
	 * @param configDirPath
	 *            path to directory containing framework config files
	 * @param docRoot
	 *            path to servlet baseDir
	 */
	 ...
	 * @exception Exception
	 *                if the framework could not be loaded
	 */
	...
	/**
	 * Instantiate a {@link MetaDataFramework} from the provided configFile, render
	 * the metadata editor pages for the framework, and register it in this
	 * FrameworkRegistry.
	 *
	 * @param configFile
	 *            Framework configuration file
	 * @exception Exception
	 *                if the framework could not be loaded, or the metadata editor
	...
	private void loadFramework(File configFile) throws Exception {
		...
		if (suggestionServiceManager != null && suggestionServiceManager.hasConfig(mdf.getXmlFormat())) {
			try {
				...
				prtln("WARNING: unable to instantiate StandardsManager for \"" + mdf.getXmlFormat() + "\": "
						+ t.getMessage());
			}
		}
	...
	public void load() {
		...
		for (int i = 0; i < configFiles.length; i++) {
			prtln("Processing framework config file (" + (i + 1) + " of " + configFiles.length + ") : "
					+ configFiles[i].getName());
	...
	/**
	 * Returns a list of xmlFormats corresponding to framework config files that are
	 * present in the framework config directory, but are not currently loaded in
	 * the FrameworkRegistry object
	 *
	 ...
	 * "dlese_anno", as opposed to frameworks used internally (e.g., "dcs_data") by
	 * the system.
	 *
