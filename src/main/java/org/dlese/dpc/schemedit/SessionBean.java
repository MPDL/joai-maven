...
/**
 * A Session-scoped Bean for information that needs to be available to different
 * controllers and their jsp pages. Encapsulates access to servlet context
 * attributes as well as session scope attributes, and provides other
 ...
 * </ul>
 * SessionBeans are bound to Session objects when they are created. This is done
 * in {@link org.dlese.dpc.schemedit.SessionRegistry}. Because they are bound to
 * the session context, SessionBeans are available to jsp pages.
 * <p>
 *
 * This class aims as much as possible to be a <b>read-only</b> structure that
 * gets information from the application's form beans which are accessed via the
 * session.
 * <p>
 *
...
public class SessionBean implements HttpSessionBindingListener, Serializable {
	...
	/**
	 * Specifies the default sort order for display of records. Used in
	 * {@link org.dlese.dpc.schemedit.action.form.DCSQueryForm#getSortRecsBy()}.
	 */
	...
	/**
	 * Gets the timeSinceLastAccessed of the session as a formated String for use in
	 * jsp.
	 * <p>
	 *
	 *
	...
	/**
	 * Returns the number of seconds left until this session times out. Dependent on
	 * the session's MaxInactiveInterval
	 *
	...
	/**
	 * Gets the recId attribute of the SessionBean object, which keeps track of the
	 * record the user has last edited so that particular record can be highlighted
	 * in lists of records.
	 * <p>
	 *
	 ...
	 * {@link org.dlese.dpc.schemedit.SessionRegistry#getLock(String, String)}.
	 *
	 * @param recId
	 *            Description of the Parameter
	...
	public boolean isAuthorized(String operation, String collection) {
		// if "Operation" is a path, then see if sessionUser has permission to for this
		// path
	 ...
	 * DCSQueryAction, but needed by other actions that need to operate over the
	 * results (e.g., {@link org.dlese.dpc.schemedit.action.BatchOperationsAction}).
	 */
	...
	/**
	 * A string representation (http request's query parameters) of the last search
	 * performed by the user.
	 * <p>
	 *
	 * Enables system to take user back to the last search they performed. For
	 * example,searchParams is used by the "Search" link in the page header, so when
	 * user returns to search it is as they left it. queryForm
	 *
	...
	/**
	 * Compute the start record index of the page on which the specified record will
	 * be found.
	 *
	 * @param id
	 *            Description of the Parameter
	...
	/**
	 * Gets a List of {@link org.dlese.dpc.repository.SetInfo} objects that provide
	 * information about the collections known to RepositoryManager.
	 * <p>
	 *
	...
	/**
	 * Return a query clause ORing together all the collections the current user is
	 * authorized to search over.
	 *
	 * @param sessionBean
	 *            NOT YET DOCUMENTED
	...
	/**
	 * mapping from unique status labels to a list of all the status values that
	 * have that label.
	 */
