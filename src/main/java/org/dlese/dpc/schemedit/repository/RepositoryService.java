...
public class RepositoryService {
	...
	private void autoExportInit() {
		...
		if (autoExportStartTime != null) {
			...
			try {
				...
				startTime = Utils.convertStringToDate(year + " " + dayInYear + " " + autoExportStartTime,
						"yyyy D H:mm");
				...
				prtlnErr("Syntax error in context parameter 'autoExportStartTime.' AutoExport timer not started: "
						+ e.getMessage());
			}
		} else
	 ...
	 * {@link #changeautoExportFrequency(long autoExportFrequency)}.
	 *
	 * @param autoExportFrequency
	...
	private CollectionRegistry getCollectionRegistry() throws Exception {
		CollectionRegistry cr = (CollectionRegistry) servletContext.getAttribute("collectionRegistry");
		if (cr == null) {
	...
	private FrameworkRegistry getFrameworkRegistry() {
		FrameworkRegistry fr = (FrameworkRegistry) servletContext.getAttribute("frameworkRegistry");
		if (fr == null) {
	...
	public static boolean indexedRecordIsStale(File sourceFile, XMLDocReader docReader) {
		...
		// long diff = Math.abs(sourceFile.lastModified() -
		// docReader.getLastModified());
		long diff = sourceFile.lastModified() - docReader.getLastModified();
		/*
		 * if (diff > 0) { prtln ("\tsourceFile.lastModified(): " +
		 * sourceFile.lastModified()); prtln ("\tdocReader.getLastModified(): " +
		 * docReader.getLastModified()); prtln ("\t --> diff: " + diff); }
		 */
		return diff > granularity;
	}

	/**
	 * Reindexes the annotated item record of an annotation record so the annotated
	 * item record is linked to its annotation.
	 *
	 * @param annoDocMap
	 *            NOT YET DOCUMENTED
	 * @exception Exception
	...
	public void indexAnnotatedRecord(DocMap annoDocMap) throws Exception {
		...
		if (itemID != null && itemID.length() > 0 && repositoryManager.getRecord(itemID) != null)
			try {
	 ...
	 * @exception Exception
	 *                if record cannot be saved and indexed with provided status
	 */
	 ...
	 *
	 * @param recId
	 *            id of metadata record to be saved
	 * @param doc
	 *            metadata record as dom4j.Document
	 * @param user
	 *            NOT YET DOCUMENTED
	 * @exception Exception
	 *                if unable to successfully update index
	 */
	public void saveEditedRecord(String recId, Document doc, User user) throws Exception {
		...
		DcsDataRecord dcsDataRecord = dcsDataManager.getDcsDataRecord(recId, getRepositoryManager());
		...
		if (dcsDataRecord.getStatus().equals(StatusFlags.UNKNOWN_STATUS) && dcsDataRecord.getEntryList().size() == 1) {

			String statusNote = "Auto-changing status from \"" + StatusFlags.UNKNOWN_STATUS + "\" to \""
					+ StatusFlags.NEW_STATUS + "\"";
	...
	/**
	 * Saves and indexes a newly created record. New records cannot be found in the
	 * index, so recId,recordXml, and collection must be passed as parameters,
	 * rather than derived from a docReader.
	 * <p>
	 *
	 * Callers of this method are controllers who collect key information from user
	 * and create an item-level metadata record outside of the metadata editor
	 * (currently only CreateADNRecordAction.handleNewRecordRequest does this).
	 *
	 * @param recId
	 *            record id
	 * @param recordXml
	 *            record content as delocalized xml string
	 * @param collection
	 *            collection key (e.g., "dcc")
	 * @param username
	 *            NOT YET DOCUMENTED
	 * @exception Exception
	 *                if unable to save new record
	 */
	 ...
	 * DcsDataRecord dcsDataRecord = null; CollectionConfig config = null; try {
	 * RepositoryManager rm = getRepositoryManager(); dcsDataRecord =
	 * getDcsDataManager().getDcsDataRecord (setInfo.getId(), rm); config =
	 * getCollectionRegistry().getCollectionConfig(setInfo.getSetSpec()); } catch
	 * (Throwable t) { throw new Exception ("saveCollectionData setup error: " + t);
	 * } this.repositoryWriter.writeCollectionData (config, dcsDataRecord); }
	 */

	/**
	 * Updates the record by writing it to the repository. NOTE: Does NOT affect the
	 * metadata but updates the indexed record, which includes DcsDataRecord
	 * information. Therefore, this method is appropriate for indexing updated
	 * DcsDataRecord information, but it is not appropriate for updating changes to
	 * metadata content.
	 * <p>
	 *
	 * Called by DCSSchemEditAction.indexAnnotatedRecord() and
	 * ThreadedServices.validate().
	 *
	 * @param recId
	...
	/**
	 * Validate the record and update the DcsDataRecord. NOTE: DcsDataRecord is not
	 * saved here - it must be saved (flushed to disk) by the caller.
	 *
	 * @param dcsData
	 *            status record corresponding to the xml record.
	 * @param record
	 *            An xml record, represented either as String or File
	 * @param xmlFormat
	 *            NOT YET DOCUMENTED
	 */
	public void validateRecord(Object record, DcsDataRecord dcsData, String xmlFormat) {
		...
		if (framework == null) {
			...
			// throw new Exception ("validateRecord ERROR: framework not found for " +
			// xmlFormat);
		}
		String validationReport = (framework != null ? framework.validateRecord(record) : Constants.UNKNOWN_VALIDITY);
	 ...
	 */
	/*
	 * public boolean collectionContainsUrlX(String url, String collection) { return
	 * (getDups(url, collection).size() > 0); }
	 */
	 ...
	 *
	 * @param records
	 *            A list of records to move to collection
	 * @param collection
	 *            collection key of the destination collection
	 * @return returns list of records that could not be moved, if any
	 * @exception Exception
	 *                NOT YET DOCUMENTED
	 */
	 ...
	 *
	 * @param records
	 *            an array of records represented as ResultDocs
	 * @param statusEntry
	 *            contains status, statusNote and editor information to be added to
	 *            each record
		...
		if (records.isEmpty()) {
			...
			for (Iterator i = records.iterator(); i.hasNext();) {
				...
				try {
					...
					prtlnErr("batchStatusUpdate failed to update status for " + id + ": " + e.getMessage());
					failures.add(id);
				}
			}
		}
	 ...
	 * move, so batch moves don't have to make this check.
	 * <p>
	 *
	 *
	 *
	 * @param recId
	 *            id of the record to be moved
	 * @param collection
	...
	public String moveRecord(String recId, String collection) throws Exception {
		...
		/*
		 * update status to reflect move - set the status of the dcsDataRecord to
		 * "Imported". NOTE: we assume the fileName will be of the form newId+".xml"
		 * because that is the default behaviour of the RepositoryManager and this is a
		 * NEW record so we know the default behaviour will be used
		 */
		...
		// put the modified dcsDataRecord in the cache where it will be found by the
		// indexer
	 ...
	 * DcsDataRecord is created with a status of "Uknown" and statusNote signifying
	 * it is copied. Validation is not performed - the new record gets the isValid
	...
	/**
	 * Returns true if the specified user is authorized for the specified collection
	 * (set).
	 *
	 * @param collection
	 *            NOT YET DOCUMENTED
	...
	public boolean isAuthorizedSet(String collection, User user, Roles.Role requiredRole) {
		/*
		 * the following logic required to enable admins to edit collection records
		 * without having the collection of collection records to be considered an
		 * "authorizedSet".
		 */
	...
	public List getAuthorizedSets(User user, Roles.Role requiredRole) {
		...
		if (requiredRole == Roles.NO_ROLE || !authenticationEnabled) {
