...
public class CollectionReaper {
	...
	public void init() throws Exception {
		...
		if (!baseTrashDir.exists()) {
			if (!baseTrashDir.mkdir()) {
				throw new Exception("baseTrashDir directory does not exist and could not be created at "
						+ baseTrashDir.getAbsolutePath());
			}
		}

		if (!masterCollectionRecords.exists()) {
			throw new Exception(
					"collection Records directory does not exist at " + masterCollectionRecords.getAbsolutePath());
		}
	...
	public void reap() throws Exception {
		...
		try {
			// for collections containing collection records, unregister them and remove
			// vocab entries
	 ...
	 * is assumed that the collection records are not associated with item records.
	 *
	 * @exception Exception
	 *                Description of the Exception
	 */
	 ...
	 *                Description of the Exception
	 */
	private void moveFiles(File sourceDir, File destDir) throws Exception {
	 ...
	 * repeatedly try (baseDir + path + "_i") until the resulting directory does not
	 * exist.
	 *
	 * @param baseDir
	 *            NOT YET DOCUMENTED
	 ...
	 *                Description of the Exception
	 */
	private void trashStatusFiles() throws Exception {
		...
		if (!sourceDir.delete()) {
			throw new Exception(
					"WARNING: status file directory could not be deleted at " + sourceDir.getAbsolutePath());
		}
	...
	private void trashCollectionFile() throws Exception {
		...
		if (!collectionRecord.renameTo(dest)) {
			prtln("WARNING: collection record at " + collectionRecord.getAbsolutePath() + " could not be renamed as "
					+ dest);
		}
	 ...
	 *                NOT YET DOCUMENTED
	 */
	private void trashCollectionStatusFile() throws Exception {
		prtln("\ntrashCollectionStatusFile()");
		String sep = Files.getFileSeparatorStr();
		...
		if (!collectionStatusRecord.exists()) {
			// throw new Exception ("collection record does not exist at " +
			// collectionStatusRecord.getAbsolutePath());
			prtln("WARNING: collection STATUS record to be trashed does not exist at "
					+ collectionStatusRecord.getAbsolutePath());
			return;
		}
