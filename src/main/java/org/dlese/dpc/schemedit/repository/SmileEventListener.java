...
public class SmileEventListener extends RepositoryEventListener {
	...
	/**
	 * Handle a RepositoryEvent - in this case we only respond to "moveRecord"
	 * events
	 *
	 * @param event
	 *            the repositoryEvent
	...
	public void handleEvent(RepositoryEvent event) {
		...
		if ("deleteRecord".equals(event.getName()) || "copyRecord".equals(event.getName())
				|| "moveRecord".equals(event.getName()) || "copyMoveRecord".equals(event.getName())) {
