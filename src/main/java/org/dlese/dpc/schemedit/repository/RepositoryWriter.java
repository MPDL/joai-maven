...
/**
 * The class includes methods to create, copy, and put Records to the main
 * Repository, and RepositoryWriterPlugins if so configured.
 *
 * @author ostwald
 *         <p>
 *
...
public class RepositoryWriter {
	 ...
	 *            format of metadata
	 * @param collection
	 *            collection key of metadata record (e.g., "dcc")
	 * @param id
	 *            metadata record id
	 * @param dcsDataRecord
	 *            dcsDataRecord for metadata
	 ...
	 *                NOT YET DOCUMENTED
	 */
	protected void writeRecord(String recordXml, String xmlFormat, String collection, String id,
		...
		try {
			...
			if (plugins != null) {
				// if we weren't able to update the index, then don't attempt to update metadata
				// in NDR
				try {
	 ...
	 * <p>
	 *
	 * Current plugin exception policy: If there is a pluginError, the record is NOT
	 * deleted, but instead it is re-indexed as a sync-error.
	 *
	 * @param recId
	 *            NOT YET DOCUMENTED
	 * @param dcsDataRecord
	...
	protected boolean deleteRecord(String recId, DcsDataRecord dcsDataRecord) throws Exception {
		...
		/*
		 * We call putRecord to index failure. TODO - Keep track of if ANY plug failed,
		 * and if so, write record (plugins are responsible for inserting appropriate
		 * error into DcsDataRecord)
		 */
		if (!this.getPlugins().isEmpty()) {
			...
			if (dcsDataRecord.hasSyncError()) {
				/*
				 * ISSUE: what is appropriate action if there was a plugin error? FOR NOW - do
				 * not delete from index: save record to index failure and then throw exception
				 */
				getRepositoryManager().putRecord(docReader.getXml(), docReader.getNativeFormat(),
						docReader.getCollection(), recId, true);
	...
	/**
	 * Delete a collection from the local repository. Called by
	 * {@link org.dlese.dpc.schemedit.repository.RepositoryService}
	 *
	 * @param config
	 *            NOT YET DOCUMENTED
	 * @return NOT YET DOCUMENTED
	 ...
	 * @exception Exception
	 *                If the repositoryManager is not found in the ServletContext
	 */
	...
	protected void init() throws Exception {
		...
		try {
			...
			frameworkRegistry = (FrameworkRegistry) getContextAttributeValue("frameworkRegistry");

			/*
			 * collectionRegistry = (CollectionRegistry)
			 * getContextAttributeValue("collectionRegistry");
			 */
			/*
			 * dcsDataManager = (DcsDataManager) getContextAttributeValue("dcsDataManager");
			 */
		} catch (Throwable t) {
	 ...
	 */
	/*
	 * protected void writeCollectionData (CollectionConfig collectionConfig,
	 * DcsDataRecord collDcsDataRecord) { collDcsDataRecord.clearSyncErrors();
	 * RepositoryWriterPlugin plugin = getRepositoryWriterPlugin
	 * (collectionConfig.getId()); if (plugin != null) { try {
	 * plugin.putCollectionData (collectionConfig, collDcsDataRecord); } catch
	 * (RepositoryWriterPluginException e) {
