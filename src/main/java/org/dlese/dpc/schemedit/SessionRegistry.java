 ...
 * <p>
 *
 * As ServletContextListener, receives "contextInitialized" event, and sets
 * servletContext attribute.
 * <p>
 *
 * As HttpSessionListener, receives "sessionCreated" and "sessionDestroyed"
 * events. {@link org.dlese.dpc.schemedit.SessionBean}s are created when the
 * session is created, and destroyed when the session is destroyed.
 *
 * @author ostwald
 */
public class SessionRegistry implements ServletContextListener, HttpSessionListener {
	...
	/**
	 * Method called when a context is destroyed. Unregisters all sessions and
	 * empties lockedRecords.
	 *
	 * @param event
	 ...
	 * <p>
	 *
	 * <b>activeSessionsMap</b> map maintains the set of ALL active sessions, and is
	 * only modified by {@link #sessionCreated(HttpSessionEvent)} and
	 * {@link #sessionDestroyed(HttpSessionEvent)} listeners.
	 * <p>
	 *
	 * <b>sessionBeanMap</b> maintains the set of sessionBean instances, which are
	 * created only for <i> interactive sessions</i> - which excludes sessions
	 * created by webservice requests.
	 */
	...
	/**
	 * Gets the lockedRecords attribute of the SessionRegistry object, which stores
	 * all locked records and the sessions that owns the lock.
	 *
	...
	/**
	 * Called when a session is created with sole effect of putting the created
	 * session into the activeSessions map. Items are removed from the
	 * activeSessionsMap wehn sessions are destroyed.
	 *
	 * @param se
	 *            notification event
	 */
	...
	/**
	 * Notified when a session is about to be destroyed - removes the session from
	 * the activeSessions map and unregisteres sessionBean.
	 *
	 * @param se
	 *            notification event
	 * @see #unregisterSessionBean(String)
	...
	/**
	 * Return the sessionId that has lock on given record. If an inactive session
	 * has the lock, unregister the sessionBean and return null to signify that the
	 * record has no locking session.
	 *
	 * @param recId
	 *            A record Id
	 ...
	 *
	 * @param sessionId
	 *            Id of session that is releasing records.
	 */
	...
	/**
	 * A version of getLock that only allows a single record to be locked by a
	 ...
	 * {@link org.dlese.dpc.schemedit.SessionBean#destroy() destroy} method
	 * (releasing all held locks), and removing its entry from the sessionBeanMap.
	 *
	 * @param sessionId
	 *            Description of the Parameter
	 */
	public void unregisterSessionBean(String sessionId) {
		...
		if (sessionBean != null) {
			...
			// if the session has been timed-out, sessionBean will be null, so we must
			// explicitly release locks ...
	...
	/**
	 * Return the SessionBean associated with the HttpSession for the provided
	 * request.
	 * <p>
	 *
	 * If a sessionBean does not yet exist for a valid session, then it is created
	 * and bound to the session context. This method is the standard way of
	 * obtaining of obtaining a sessionBean from within a Controller or FormBean.
	 *
	 * @param request
	...
	public SessionBean getSessionBean(HttpServletRequest request) {
		...
		if (sessionBean == null) {
			/*
			 * here we have no sessionBean, and yet we have sessionID for an inActive
			 * session! invalidate the sessionto get us out of the jam
			 */
	...
	/**
	 * Gets the sessionBean associated with the given sessionID from the
	 * sessionBeanMap.
	 * <p>
	 *
	 * Retrieves a sessionBean from the sessionBeanMap after checking to ensure that
	 * the session is in the activeSessionMap. Does NOT create a new SessionBean if
	 * one is not found.
	 *
	 * @param sessionId
	 *            a session id
	 * @return The sessionBean value or null if a sessionBean is not found or the
	 ...
	 * <p>
	 *
	 * Unregisters sessionBeans corresponding to sessions that have been
	 * invalidated.
	 *
	...
	public List getSessionBeans() {
		...
		synchronized (sessionBeanMap) {
			...
			// prtln(" ... " + sbList.size() + " live beans, " + deadBeans.size() + " dead
			// ones");
