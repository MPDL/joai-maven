...
public class SerializedDataManagerTest extends TestCase {
	...
	public void test_locking_methods() throws Exception {
		...
		for (int i = 0; i < NUM_RECORDS; i++) {
			if (is_locked[i])
				Assert.assertTrue("Record " + original_ids[i] + " should be locked but is not",
						dm.isLocked(original_ids[i]));
			else
				Assert.assertTrue("Record " + original_ids[i] + " should not be locked but is",
						!dm.isLocked(original_ids[i]));
		}

		// Be sure all locked records properly return an lock exception when called with
		// bogus key:
