...
/**
 * A Hashtable-like interface for reading and writing persistent Java objects
 * from and to disc. Objects are stored in serialized form on disc - any object
 * that implements {@link java.io.Serializable} can be stored in this
 * DataManager for persistent retrieval. The objects can also be held in a RAM
 * cache to provide faster retrieval. Serialized objects are stored in a
 * directory specified at the time of construction. Objects stored in a
 * SerializedDataManager can be of mixed type. This class is safe to use with
 * multiple process and/or JVMs that access the same dataPath concurrently.
 *
...
public class SerializedDataManager extends DataManager implements Serializable {
	...
	/**
	 * Constructs a new SerializedDataManager that reads and writes data to the
	 * given dataPath directory. Allows control over whether or not to hold a cache
	 * of the objects in RAM memory for faster retrieval. Note that if caching is
	 * turned on in this SerializedDataManager, the objects that are returned from
	 * the {@link #get(String)} and {@link #get(List)} methods are not safe to
	 * modify. Specifically, any changes made to the objects will be reflected in
	 * subsequent calls to those methods. To get a copy of the data items that are
	 * safe to modify the caller should use the {@link #getCopy(String)} or
	 * {@link #getCopy(List)} methods instead. If caching is not turned on, the
	 * effect of the get methods are the same as the getCopy methods.
	 * <p>
	 *
	 * This class is safe to use with multiple process and/or JVMs that access the
	 * same dataPath.
	 *
	 * @param dataPath
	 *            The directory where the serialzed objects are stored.
	 * @param useCache
	...
	/**
	 * Constructor for the SerializedDataManager object, restores a serialized
	 ...
	 * reflected in subsequent calls to this method and the {@link #get(List)}
	 * method. To get a copy of a data item that is safe to modify use the
	 * {@link #getCopy(String)} method instead. If caching is not enabled, the
	 * effect of this method is the same as {@link #getCopy(String)}.
	 *
	 * @param oid
	 *            The data identifier
	 * @return <code>Object</code> of data
	 * @exception OIDDoesNotExistException
	 *                If no object exists for the given uid.
	 * @see #getCopy(String)
	 */
	public synchronized Object get(String oid) throws OIDDoesNotExistException {
		Object o = null;
	...
	/**
	 * Retrieves a copy of a single data object, sutable for modifying. The object
	 * that is returned is safe for modifying without affecting the data that is in
	 * this SerializedDataManager.
	 *
	 * @param oid
	 *            The data identifier.
	 * @return A copy of an <code>Object</code> of data that is in this
	 *         SerializedDataManager.
	 * @exception OIDDoesNotExistException
	 *                If no object exists for the given uid.
	 * @see #get(String)
	 */
	public synchronized Object getCopy(String oid) throws OIDDoesNotExistException {
		Object o = null;
	...
	/**
	 * Retrieves a <code>List</code> of data objects. Note that if caching is turned
	 * on in this SerializedDataManager, the objects that are returned are not safe
	 * to modify. Specifically, if caching is enabled, any changes made to the
	 * objects will be reflected in subsequent calls to this method and the
	 * {@link #get(String)} method. To get a copy of a data items that are safe to
	 * modify use the {@link #getCopy(List)} method instead. If caching is not
	 * ebabled, the effect of this method is the same as {@link #getCopy(List)}.
	 *
	 * @param oids
	 *            <code>List</code> of data identifiers
	 * @return <code>List</code> of corresponding data objects
	 * @exception OIDDoesNotExistException
	 *                If no object exists for the given uid.
	 * @see #getCopy(List)
	 */
	public synchronized List get(List oids) throws OIDDoesNotExistException {
	...
	/**
	 * Retrieves a <code>List</code> of copied data objects, suitable for modifying.
	 * The objects that are returned are safe for modifying without affecting the
	 * data that is in this SerializedDataManager.
	 *
	 * @param oids
	 *            <code>List</code> of data identifiers.
	 ...
	 * @see #get(List)
	 */
	public synchronized List getCopy(List oids) throws OIDDoesNotExistException {
		List list = new ArrayList(oids.size());
	 ...
	 * SerializedDataManager. Results are not guaranteed to be in lexagraphical
	 * order, however the order is guaranteed to be in the same order from one
	...
	/**
	 * Adds a new object of data. This method is safe for concurrent use among
	 * multiple processes and JVMs accessing the same SerializedDataManager.
	 *
	 * @param oid
	 *            The unique identifier that references the new data object
	 * @param obj
	 *            The new data object
	 * @return The added data object iff successful, otherwise null
	 * @exception OIDAlreadyExistsException
	 *                If an object with given oid already exists in the data store.
	 * @exception ObjectNotSupportedException
	 ...
	 * SerializedDataManager. This method, together with
	 * {@link #releaseLock(String objectPath)}, provide reliable locking method for
	 * use across multiple processes or JVMs that may access the SDM data
	 * concurrently.
	 *
	 * @param objectPath
		...
		try {
			...
			while (!lock.createNewFile()) {
				try {
					...
					/*
					 * Check if writelock might not have been released upon last shutdown, and
					 * release it. Note: in an environment where multiple apps are accessing the
					 * same index, this may cause problems
					 */
					if (time > 6000) {
	 ...
	 * recieved the necessary lock for the object. After successful completion the
	 * object is removed from the DataManager and returned to the caller.
	 *
	 * @param oid
	 *            The unique identifier that references the data object to be
	 *            removed
	 * @param lockKey
	 ...
	 * @exception OIDDoesNotExistException
	 *                If no object exists for the given uid.
	 * @exception InvalidLockException
	 *                If the lock provided for the object is not valid.
	 */
	 ...
	 * If the object is locked a LockNotAvailableException is thrown. After
	 * successful completion the object is removed from the DataManager and returned
	 * to the caller.
	 *
	 * @param oid
	 *            The unique identifier that references the data object to be
	 ...
	 * @exception OIDDoesNotExistException
	 *                If no object exists for the given uid.
	 * @exception LockNotAvailableException
	 *                If another user currently has a lock on this object.
	 */
	public synchronized Object remove(String oid) throws OIDDoesNotExistException, LockNotAvailableException {
		if (isLocked(oid))
	 ...
	 * recieved the necessary lock. Similar to <code>remove()</code> except no
	 * object is returned and thus is more efficient if an object is not required.
	 *
	 * @param oid
	 *            The unique identifier that references the data object to be
	 *            deleted
	 ...
	 * If the object is locked a LockNotAvailableException is thrown. Similar to
	 * <code>remove()</code> except no object is returned and thus is more
	 * efficient. After successful completion the object is deleted from the
	 * DataManager.
	 *
	 * @param oid
	 *            The unique identifier that references the data object to be
	 ...
	 *                If another user currently has a lock on this object.
	 */
	public synchronized boolean delete(String oid) throws OIDDoesNotExistException, LockNotAvailableException {
		File f = new File(dataPath + "/" + encodeFileName(oid));
		if (!f.isFile())
			throw new OIDDoesNotExistException(oid);
		if (isLocked(oid))
	...
	/**
	 * Updates a new object of data, assuming the caller had requested and recieved
	 * the necessary lock. The caller retains the lock after this method returns.
	 *
	 * @param oid
	 *            The unique identifier that references the data object to be
	 *            updated
	...
	/**
	 * Updates a new object of data iff the object is not locked by another user. If
	 * the object is locked a LockNotAvailableException is thrown.
	 *
	 * @param oid
	 *            The unique identifier that references the data object to be
	 *            updated
	...
	/**
	 * Get a user-requested lock for a data object, preventing it from being written
	 * by other threads that are using this SerializedDataManager. Note that this
	 * type of locking only works within a single instance of the
	 * SerializedDataManager. Other instances of SerializedDataManager will not know
	 * about this lock.
	 *
	 * @param oid
	 ...
	 *                If another user currently has a lock on this object.
	 */
	public synchronized String lock(String oid) throws OIDDoesNotExistException, LockNotAvailableException {
		if (!oidExists(oid))
	 ...
	 *
	 * @param oid
	 *            The unique identifier that references the locked data object
	 * @param lockKey
	 *            The key corresponding to the lock on this data object
	 * @return <b>true</b> iff the lock has been removed for the given object
	 *         regardless of whether it had been locked prior to calling this
	 *         method.
	 * @exception OIDDoesNotExistException
	 *                If no object exists for the given uid.
	 * @exception InvalidLockException
	...
	/**
	 * Serializes an object to the given file path, placing it in a RAM cache if
	...
	/**
	 * Restores a Serialized object from the given file path, pulling if from a RAM
	 * cache if caching indicated at construction time. Returns null if no file was
	...
	/**
	 * Encodes a String so that it is suitable for use as a file name by encoding
	 * all non letter or digit chars such as "/" and ":" into escaped hex values of
	 * the form _HEX. Note: Unix commands cannot include the following chars: * ? !
	 * | \ / ' " { } < > ; , ^ ( ) $ ~ Windows file names may not contain: \ / : * ?
	 * " < > | nor does Win 2000 like it when a . is at the end of the filename (it
	 * removes it when returning the file name).
	 *
	 * @param name
	...
	/**
	 * Converts a string that contains escaped hex encoding of the form %HEX back to
	 * plain text.Provides the inverse operation of the method encodeFileName()
	 */
	protected static Hashtable codes = null;

	/**
	 * Decodes a String that was encoded using the method
	 * {@link #encodeFileName(String name)}.
	 *
	 * @param name
	...
	/**
	 * Return the number of files in this DataManager as reported by the file
	 * system.
	 *
