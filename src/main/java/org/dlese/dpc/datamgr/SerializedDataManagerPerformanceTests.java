...
public class SerializedDataManagerPerformanceTests extends TestCase {
	...
	public void test_getIDsSorted_method() throws Exception {
		...
		// Make the call
		Date end = new Date();
		TestTools.printElapsedTime("Time call getIDsSorted with " + NUM_RECORDS + " records in the DataManager: ",
				start, end);

		// Verify all is well...
		for (int i = 0; i < NUM_RECORDS; i++) {
			Assert.assertTrue("Not equals: original: \"" + original_ids[i] + "\"            returned: \""
					+ returned_ids[i] + "\"", original_ids[i].equals(returned_ids[i]));
		}
	}
	...
	public void test_put_and_get_methods() throws Exception {
		...
		Date end = new Date();
		TestTools.printElapsedTime("Time to put " + NUM_RECORDS + " records in the DataManager: ", start2, end2);
		TestTools.printElapsedTime(
				"Time to put and then get and assertEquals " + NUM_RECORDS + " records in the DataManager: ", start,
				end);
	}
	...
	public void test_getIDs() {
		...
		if (IDs.length > 0) {
			...
			prtln("First record is: " + IDs[0] + ". " + mid + "th record is: " + IDs[mid] + ". " + last
					+ "th record is: " + IDs[last]);
		}
	}
	...
	public void test_getIDsSorted() {
		...
		if (IDs.length > 0) {
			int mid = (int) (Math.floor(IDs.length / 2));
			int last = IDs.length - 1;
			TestTools.printElapsedTime("Time to call getIDsSorted with " + IDs.length + " records in the DM: ", start,
					end);
			prtln("First record is: " + IDs[0] + ". " + mid + "th record is: " + IDs[mid] + ". " + last
					+ "th record is: " + IDs[last]);
		}
	...
	public void test_random_filename_encode_decode() {
		...
		Date end = new Date();
		TestTools.printElapsedTime("Encoding then decoding " + NUM_IDS + " IDs of length " + ID_LEN + " took: ", start,
				end);

	}

	/**
	 * Gets the uniqueString attribute of the SerializedDataManagerPerformanceTests
	 * object
	 *
