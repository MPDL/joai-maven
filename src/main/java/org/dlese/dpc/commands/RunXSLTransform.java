...
/**
 * Command-line routine that performs XSL transforms of XML files or directories
 * of files. Has option for validating records and will return a report of the
 * validation as well as the outcome of the transform
 *
...
public class RunXSLTransform {
	...
	public static void main(String[] args) {
		try {
			...
			// Read properties file
				...
				tFormer.processRecords(txfmProps.xslFile, txfmProps.inputDir, txfmProps.outputDir,
						parseDate(txfmProps.modifiedSinceDate), txfmProps.validateInputFiles,
						txfmProps.validateOutputFiles);
			}
			// Read input from command line
				...
				tFormer.processRecords(args[0], args[1], args[2], parseDate(args[3]), args[4].equalsIgnoreCase("true"),
						false);
	...
	/**
	 * Process the given harvested records using the preferences in the given props
	 * file.
	 *
	 */
	private void processRecords(String xslFile, String inputDir, String outputDir, Date modifiedSinceDate,
			boolean validateInputFiles, boolean validateOutputFiles) throws Exception {
		...
		if (xslFile != null && !xslFile.equals("")) {
			...
			try {
				...
				reportTxt = transformer.transform(xslFileFull, inputFilesDirFull, null, outputDirFull,
						modifiedSinceDate);
	...
	/**
	 * Append text to the given report file. If the file does not exist, it will be
	 * created. If reportFilePath is null, then the ouput will be sent to standard
	 * out
	 */
	...
	private TxfmProps getProps(String propsName) throws Exception {
