 ...
 * conjunction with the {@link org.dlese.dpc.action.form.SimpleQueryForm} Struts
 * form bean class.
 * <p>
 *
 * This class works by processing the HTTP request and outputting the
 * corresponding HTTP response by forwarding to a JSP that will create it.
 * Returns an {@link org.apache.struts.action.ActionForward} instance that maps
 * to the Struts forwarding name "simple.query," "report.query," "weblog.query,"
 * or "weblogreport.query" which must be configured in struts-config.xml to
 * indicate the JSP page that will handle and render the request.
 * <p>
 *
 * An application that uses this class must set a
 * {@link javax.servlet.ServletContext} attribute containing a
 * {@link org.dlese.dpc.index.SimpleLuceneIndex} under the key "index." This
 * index will be used to perform the query. Optionally there may also be a web
 * log index uder the key "webLogIndex".
 *
...
public final class SimpleQueryAction extends Action {
	 ...
	 *                if a servlet exception occurs
	 */
	public ActionForward handleMetadataSearchRequest(ActionMapping mapping, SimpleQueryForm queryForm,
		...
		try {
			...
			if (request.getParameter("q") != null) {
				...
				if (selStatus != null && selStatus.length > 0 && !selStatus[0].equals(SELECT_ALL)) {
					...
					for (int i = 0; i < selStatus.length; i++) {
						if (selStatus[i].equals("deletedNotInDir"))
							...
							statusQuery.add(new TermQuery(new Term("deleted", selStatus[i])),
				...
				if (selectedSetSpecs != null && selectedSetSpecs.length > 0
	 ...
	 *                if a servlet exception occurs
	 */
	public ActionForward handleWebLogSearchRequest(ActionMapping mapping, SimpleQueryForm form,
	...
	private final void setCollectionsAndFormats(SimpleLuceneIndex index, SimpleQueryForm queryForm,
