...
/**
 * This implements the DLESE extended form of the combined log format as
 * documented at <a href="http://httpd.apache.org/docs/logs.html"> Apache logs
 * docs</a>.
 * <p>
 * The combined log format has the same first seven fields as the common log
 * format, and adds two more fields: referrer and user-agent.
 * <p>
 * The DLESE extension to the combined log format adds several more fields,
 * documented below.
 * <p>
 ...
 * <dt>id
 * <dd>See rfc1413 (obsoletes rfc931) remote logname of the user. Unreliable,
 * seldom used. Usually appears as "-".<br>
 * Example: <code>-</code><br>
 * <dt>REMOTE_USER
 * <dd>The username as which the user has authenticated via password
 * protection.<br>
 * Example: <code>-</code><br>
 * Example: <code>samuel</code><br>
 * <dt>Date/time
 * <dd>The date and time of request, UTC (GMT) time. <br>
 * Note: this is the time when the server wrote the log record, not the time
 * extracted from the possibly bogus date header. <br>
 * Example: <code>[10/Nov/2002:07:48:54 -0700]</code><br>
 * <dt>Request string
 * <dd>The request line as it came from the client. Note: this is reconstructed
 * using HttpServletRequest calls, so there may be slight differences in
 * formatting from the original request string.<br>
 ...
 * <dl>
 * <dt>Referer
 * <dd>The referer HTTP request header, in quotes. Someone long ago ago
 * misspelled "referrer" and it's never been fixed. If unavailable, it appears
 * quoted as "-", not as -.<br>
 * Example: <code>"-"</code><br>
 * Example: <code>"http://quake.dpc.ucar.edu/index.html"</code><br>
 * <dt>User Agent
 * <dd>The User_Agent HTTP request header, in quotes. If unavailable, it appears
 * quoted as "-", not as -.<br>
 * Example: <code>"-"</code><br>
 * Example:
 * <code>"Mozilla/5.0 (X11; U; Linux i686; en-US; rv:1.0.0) Gecko/20020529"</code><br>
 * </dl>
 * <p>
 ...
 * <dt>numSearchResults
 * <dd>Number of search results for "search" requests; 0 otherwise.<br>
 * Example: <code>98</code><br>
 * <dt>numDbRecords
 * <dd>Total number of records in the database.<br>
 * Example: <code>9000</code><br>
 * <dt>rankNum
 * <dd>If "search": rank of first result in the displayed page. If "full": rank
 * of this result. If "other": 0.<br>
 * Example: <code>1</code><br>
 * <dt>dleseId
 * <dd>If "full": the DLESE ID of the record returned. Otherwise: "-"<br>
 * Example: <code>"-"</code><br>
 * <dt>xmlString
 * <dd>An arbitrary string, generally XML. Internal backslashes, quotes, double
 * quotes, newlines, etc. will be escaped.<br>
 * Example:
 * <code>"&lt;criteria testattr=\"testval\"&gt;&lt;field&gt;Free Text&lt;/field&gt;&lt;textSearchValue&gt;oceanography&lt;/textSearchValue&gt;&lt;textSearchTerm&gt;oceanography&lt;/textSearchTerm&gt;&lt;field&gt;Grade Level&lt;/field&gt;&lt;LearningContext&gt;High school&lt;/LearningContext&gt;&lt;/criteria&gt;"</code><br>
 * </dl>
 * <p>
 * <h4>Sample Usage</h4>
 * 
...
public class ClfLogger {
	 ...
	 * 
	 * @param loggerLevel
	 *            The minumum level message this logger will accept.
	 * @param appendFlag
	 *            If true, we append to outfile
	 * @param outfilename
	 *            The name of output file
	 */

	ClfLogger(Level loggerLevel, // one of java.util.logging.Level.*
		...
		if (loggerLevel.intValue() < Level.FINEST.intValue() || loggerLevel.intValue() > Level.SEVERE.intValue())
			throw new LogException("ClfLogger.const: invalid loggerLevel: " + loggerLevel);

		try {
			outfile = new File(outfilename);
			outdir = outfile.getParentFile();
		} catch (Throwable t) {
			throw new LogException("ClfLogger: cannot open output file \"" + outfilename + "\": " + t);
		}
	}

	/**
	 * Returns the BufferedWriter to write to and ensures the file being written to
	 * exists.
	 * 
	 * @return The BufferedWriter to write to
	 */
	private BufferedWriter getWriter() throws Throwable {
		if (!outfile.exists()) {
			if (outdir != null)
				outdir.mkdirs();
			if (wtr != null)
				wtr.close();
			wtr = new BufferedWriter(new FileWriter(outfilename, appendFlag));
		}
		if (wtr == null)
			wtr = new BufferedWriter(new FileWriter(outfilename, appendFlag));
		return wtr;
	...
	public void close() throws LogException {
		try {
			...
			throw new LogException("ClfLogger: I/O error closing file \"" + outfilename + "\": " + ioe);
		}
	}

	public void log(Level level, HttpServletRequest req, // the incoming request
								...
								// newlines, etc. will be escaped.
	{
		...
		if (level.intValue() >= loggerLevel.intValue()) {
			logit(req, statusCode, contentLen, requestType, numSearchResults, numDbRecords, rankNum, dleseId,
					xmlString);
		}
	}

	private void logit(HttpServletRequest req, // the incoming request
			int statusCode, // returned http status code
			int contentLen, // returned content len, bytes
			String requestType, // type: search, full, or other
			int numSearchResults, // num search results
								...
								// newlines, etc. will be escaped.
	{
		...
		outbuf.append(req.getRemoteAddr()); /*
											 * getRemoteHost() was returning null when run through the apache connector,
											 * so using getRemoteAddr instead.
											 */
		...
		if (req.getRemoteUser() == null)
			outbuf.append('-'); // REMOTE_USER
		else
			outbuf.append(req.getRemoteUser());
		...
		SimpleDateFormat sdf = new SimpleDateFormat("[dd/MMM/yyyy:HH:mm:ss +0000]");
		...
		if (req.getHeader("user-agent") == null)
			...
			outbuf.append(filterString(req.getHeader("user-agent")));
		outbuf.append('\"');
		...
		if (req.getServerName() == null)
			...
			outbuf.append(req.getServerName());
		...
		if (dleseId == null || dleseId.length() == 0)
			outbuf.append('-');
		else
			outbuf.append(dleseId);
		outbuf.append('\"');
		...
		if (xmlString == null || xmlString.length() == 0)
			...
			outbuf.append(filterString(xmlString));
		outbuf.append('\"');
		...
		try {
			...
			mkerror("ClfLogger.logit: I/O error on file \"" + outfilename + "\": " + t);
		}
	} // end logit
	...
	StringBuffer filterString(String stg) {
		...
		for (ii = 0; ii < stg.length(); ii++) {
			...
			if (cc == '\\')
				...
				resbuf.append("\\r");
			else if (Character.isLetterOrDigit(cc) || " !#$%&()*+,-./:;<=>?@[]^_`{|}~".indexOf(cc) >= 0)
				resbuf.append(cc);
			else {
