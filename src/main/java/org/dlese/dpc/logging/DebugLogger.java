...
public class DebugLogger extends Logger {
	 ...
	 * 
	 * @param outputType
	 *            Must always be LOG_STD. For future flexibility.
	 * @param timeType
	 *            The type of time stamp: Either LOG_UTCTIME or LOG_LOCALTIME
	 * @param outputFormat
	 *            Either LOG_XML or LOG_TEXT. Normally LOG_TEXT.
	 * @param loggerLevel
	...
	static DebugLogger getDebugLogger(int outputType, // For now, always LOG_STD
		...
		if (outputType != DleseLogManager.LOG_STD)
			throw new LogException("DebugLogger: invalid outputType: " + outputType);
		...
		if (outfilename.equals("-"))
			...
			try {
				outstrm = new BufferedOutputStream(new FileOutputStream(outfilename, appendFlag));
			} catch (IOException ioe) {
				throw new LogException("DebugLogger: cannot open file \"" + outfilename + "\"  exc: " + ioe);
			}
		}
		Formatter fmtr;
		if (outputFormat == DleseLogManager.LOG_TEXT)
			...
			throw new LogException("DebugLogger: invalid outputFormat: " + outputFormat);
...
class DleseFormatter extends Formatter {
	...
	DleseFormatter(int timeType) throws LogException {
		super();
		this.timeType = timeType;
		if (timeType != DleseLogManager.LOG_UTCTIME && timeType != DleseLogManager.LOG_LOCALTIME)
			throw new LogException("DleseFormatter: invalid timeType: " + timeType);
	}
		...
		if (parms != null && parms.length > 0) {
			for (ii = 0; ii < parms.length; ii++) {
				...
				if (parms[ii] == null)
					...
					outbuf.append(parms[ii].toString());
				outbuf.append("\n");
			}
		}
