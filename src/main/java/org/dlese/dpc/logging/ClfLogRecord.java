...
/**
 * Represents a single log record created by {@link ClfLogger ClfLogger}. Log
 * files may be parsed by calling {@link ClfLogRecord#parse ClfLogRecord.parse}.
 */
...
public class ClfLogRecord {
	...
	/**
	 * id. See rfc1413 (obsoletes rfc931) remote logname of the user. Unreliable,
	...
	/**
	 * The date and time of request, UTC (GMT) time. Note: this is the time when the
	 * server wrote the log record, not the time extracted from the possibly bogus
	 * date header.
	 */
	...
	/**
	 * The referrer, but long ago was misspelled it as the "referer" HTTP request
	 * header. If unavailable, it appears quoted as "-", not as -.
	 */
	public String referrer;

	/**
	 * The User_Agent HTTP request header, in quotes. If unavailable, it appears
	 * quoted as "-", not as -.
	 */
	...
	/**
	 * If search: rank of first result in the displayed page. If full: rank of this
	 * result.
	 */
	public int rankNum;
	 ...
	 * 
	 * <pre>
	 * int linenum = 0; BufferedReader rdr = new BufferedReader( new FileReader(
	 * inname)); while (true) { String inline = rdr.readLine(); if (inline == null)
	 * break; linenum++; ClfLogRecord rec = ClfLogRecord.parse( inline, linenum);
	 * System.out.println("\nrecord " + linenum + ":\n" + rec); } rdr.close();
	 * 
	 * <pre>
	 */

	public static ClfLogRecord parse(String inline, // input log line
	...
	private void parseSub(String inline, // input log line
			int linenum) // line number, for error msgs only
			throws LogException {
		this.inline = inline;
	...
	String getField(String quotes) // If not quoted, null. Otherwise, 2 chars:
		...
		int iend = ipos;
		if (quotes != null) {
			if (inline.charAt(iend) != quotes.charAt(0))
				baddata("invalid quotes");
			iend++;
			if (iend >= linelen)
				baddata("invalid quotes");
		}
	...
	void baddata(String msg) throws LogException {
		throw new LogException(
				"parseRec: Invalid line: " + msg + ".  Line num: " + linenum + ".  Contents: \"" + inline + "\"");
	}
